...
This is a living document. **Current release facts are generated in
[`docs/release-facts.json`](docs/release-facts.json).**
...
> **Authoritative platform totals:** [`docs/release-facts.json`](docs/release-facts.json)
> is the machine-readable source; [`docs/CURRENT_PLATFORM_TOTALS.md`](docs/CURRENT_PLATFORM_TOTALS.md)
> is its generated readable rendering. Current-state numbers on this page must agree with those facts.
...
## Long-horizon development register

The broader review list for agent operations, oversight, multimodal work,
long-running autonomy, memory, model routing, and future integrations lives in
the [Future Agent Operations & Platform Development Register](docs/future-agent-operations-roadmap.md).
It distinguishes shipped equivalents from genuine gaps and records the
evidence required before deferred ideas become active work.
...
| R25 | Self-Improving Codebase | Nightly research generates real code edits; `proposal-verifier.ts` compiles each through an in-memory source overlay; only verified proposals reach the admin Apply button. |
...
**420 total registered tools · 386 public documented tools · 68 total skills · 18 personas · 141 active capabilities · 182 declared / 262 live tables · 839 platform indexes · 41 governance rules · 78 curated AI models in the core registry plus daily catalog discovery against 1000+ models on OpenRouter · ~320k LOC across 1,100+ TypeScript files.** ([authoritative release facts](docs/release-facts.json))
