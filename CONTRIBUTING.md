...
### Review-before-adoption policy

Pull requests from outside contributors are treated as **untrusted proposals**.
They are tested in isolated GitHub Actions runners without repository secrets,
but they are never automatically adopted into the private development project.

Before a PR can be replayed into the private repository:

1. The normal CI and security checks must pass.
2. The repository owner must approve the **exact current head commit**.
3. Any new commit invalidates the previous approval and requires another review.
4. High-risk changes (workflows, dependencies, authentication, safety, tools,
   schema, migrations, scripts, or deployment files) receive explicit scrutiny
   before replay.

The `External PR review gate` enforces the current-commit approval check. The
repository's branch protection settings must require that check and a
CODEOWNER review on `main`; without those GitHub settings, a status check is
informational rather than a merge barrier.
...
- **New tools** — VisionClaw has 417 tools. If you build a useful one, we want it. See "How to add a new tool" below for the recipe. (Authoritative counts live in [docs/CURRENT_PLATFORM_TOTALS.md](docs/CURRENT_PLATFORM_TOTALS.md) — please update there first.)
