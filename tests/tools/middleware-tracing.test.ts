...
test("S24: executeTool imports and delegates to the tracing middleware", () => {
  ...
  // Action Ledger S2 sits between the span and the inner dispatch; the span
  // must remain OUTERMOST (contract: data/feature-contracts/action-ledger/plan.md §S2).
  assert.ok(
    toolsSrc.includes("runWithToolSpan(name, params, () =>"),
    "executeTool must call runWithToolSpan as the outermost dispatch wrapper",
  );
  assert.ok(
    toolsSrc.includes(
      "withActionLedger(name, params, () => _executeToolInner(name, params))",
    ),
    "the span must wrap the action-ledger wrap of _executeToolInner",
  );
});
