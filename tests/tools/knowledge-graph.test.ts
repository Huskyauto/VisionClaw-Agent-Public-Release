...
const schema = readFileSync("shared/models/operational-learning.ts", "utf8");
...
test("dedup is an atomic UPSERT on (tenant_id, norm_key), reinforcement refreshes created_at", () => {
  ...
  assert.match(conflict, /seen_count = research_evidence_triples\.seen_count \+ 1/);
  assert.match(conflict, /created_at = now\(\)/);
  assert.match(kg, /uq_ret_tenant_norm/);
  assert.ok(!/SELECT id FROM research_evidence_triples\s+WHERE tenant_id[\s\S]{0,200}lower\(subject\)/.test(kg), "no check-then-insert dedup");
});

test("retention: TTL delete + per-tenant row cap after persist", () => {
  assert.match(kg, /DELETE FROM research_evidence_triples WHERE tenant_id = \$\{tenantId\} AND created_at < \$\{cutoff\}/);
  assert.match(kg, /id NOT IN \(\s*SELECT id FROM research_evidence_triples WHERE tenant_id = \$\{tenantId\}\s*ORDER BY created_at DESC LIMIT \$\{MAX_ROWS_PER_TENANT\}/);
});

test("ensureTable bootstrap present with retry-on-failure", () => {
  assert.match(kg, /CREATE TABLE IF NOT EXISTS research_evidence_triples/);
