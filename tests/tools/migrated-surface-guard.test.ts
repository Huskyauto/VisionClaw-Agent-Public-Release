...
import { revenueMissionCreateDefinition, revenueMissionListDefinition, revenueMissionStatusDefinition, revenueMissionDraftExperimentDefinition, missionPortfolioReviewDefinition } from "../../server/tools/domains/revenue-missions/definitions";
import { productListingCreateDefinition, productListingListDefinition, createPaymentLinkDefinition } from "../../server/tools/domains/commerce/definitions";
import { generateAuditFixKitDefinition } from "../../server/tools/domains/audit-fixkit/definitions";
import {
...
} from "../../server/tools/domains/derived-api/definitions";
import { discoverCmmcProspectsDefinition, lookupSamCompanyCageDefinition, lookupSamExactCompanyDefinition } from "../../server/tools/domains/cmmc-prospecting/definitions";
...
const EXPECTED_MIGRATED = new Map([
  ["discover_cmmc_prospects", discoverCmmcProspectsDefinition],
  ["lookup_sam_company_cage", lookupSamCompanyCageDefinition],
  ["lookup_sam_exact_company", lookupSamExactCompanyDefinition],
  // S4 (system domain)
  ...
  // S25k (research-intel domain) — the 9 original shared arms plus one
  // born-migrated project-scoped relationship query:
  ...
  ["query_project_relationships", queryProjectRelationshipsDefinition],
  ...
  // Verified Revenue Missions S5a (revenue-missions domain) — 4 owner-only
  // tools → server/lib/revenue-missions + mission-sample-harvest (call-time
  // dynamic imports). SEAM read-from-ctx: ctx.tenantId is the ONLY trust
  // signal (owner gate: tenantId===1 or fail closed). approve/kill are
  // DELIBERATELY not tools (HITL-only via admin UI). Born-migrated.
  ["revenue_mission_create", revenueMissionCreateDefinition],
  ["revenue_mission_list", revenueMissionListDefinition],
  ["revenue_mission_status", revenueMissionStatusDefinition],
  ["revenue_mission_draft_experiment", revenueMissionDraftExperimentDefinition],
  // S5d capital allocator — read-only advisory portfolio review (no LLM, no
  // writes; kill/approve/autonomy remain HITL-only). Born-migrated.
  ["mission_portfolio_review", missionPortfolioReviewDefinition],
  // S5b commerce domain — 3 owner-only sell-and-fulfill tools (DB-backed
  // catalog + idempotent Stripe payment links; metadata carries ONLY
  // bundle_sku, never file paths). SEAM read-from-ctx: ctx.tenantId is the
  // ONLY trust signal (owner gate via ownerTenantId(), fail closed).
  // Born-migrated.
  ["product_listing_create", productListingCreateDefinition],
  ["product_listing_list", productListingListDefinition],
  ["create_payment_link", createPaymentLinkDefinition],
  ["generate_audit_fix_kit", generateAuditFixKitDefinition],
  // derived-api domain — 5 tools: watch-once / skip-the-browser capability.
  // Capture is passive (in-memory, fail-open). Derive + replay are Sensitive.
  // Born-migrated.
  ["derived_api_capture_list", captureListDefinition],
  ["derived_api_derive", recipeDeriveDefinition],
  ["derived_api_replay", recipeReplayDefinition],
  ["derived_api_list", recipeListDefinition],
  ["derived_api_delete", recipeDeleteDefinition],
]);
...
test("registered migrated definitions are the IDENTICAL objects the facade splices in", () => {
  ...
  assert.deepEqual(
    ...
    "migrated set must be exactly the S4 trio + S5 files six + S6 security five + S7 memory six + S8 knowledge seven + S9 web eight + S10 quality twelve + S11 documents eight + S12 browser three + S25c browser-pair two + S13 workspace six + S14 social four + S15 x-twitter nine + S16 outreach eight + S17 finance seven + S18 legal five + S19 multiagent three + S20 agentic three + S21 media six + S22 delivery three + S23 governance one + research ten + S26f seo-and-content-ops five + S26g skill-evolution fatigue-pair two + S26h strategic-memory four + S29 custom-tools three + manage_skills one + S30 video-editor three + S31 character-portraits three + S32 video-selectors two + S33 outlook three + S33 sessions two + S34 google-workspace one + simulation-sandbox three + revenue-missions five + commerce three + audit-fixkit one + derived-api five (update this test deliberately when a slice migrates more)",
  );
...
const BORN_MIGRATED_DEF_CONSTS = ["lookupSamCompanyCageDefinition", "sandboxRunDefinition", "sandboxReportDefinition", "sandboxPromoteDefinition", "revenueMissionCreateDefinition", "revenueMissionListDefinition", "revenueMissionStatusDefinition", "revenueMissionDraftExperimentDefinition", "missionPortfolioReviewDefinition", "productListingCreateDefinition", "productListingListDefinition", "createPaymentLinkDefinition", "generateAuditFixKitDefinition", "captureListDefinition", "recipeDeriveDefinition", "recipeReplayDefinition", "recipeListDefinition", "recipeDeleteDefinition"];
