...
const EXPECTED_MIGRATED = new Map([
  ...
  ["scrapling_scrape", scraplingScrapeDefinition],
  ["multi_source_collect", multiSourceCollectDefinition],
  ...
  // TypeSafe Jev — bounded, advisory-only System One judgment. Born migrated.
  ["typesafe_jev_advisory", typesafeJevDefinition],
]);
...
test("registered migrated definitions are the IDENTICAL objects the facade splices in", () => {
  ...
  assert.deepEqual(
    ...
    "migrated set must exactly match the deliberately imported domain definitions, including the advisory-only TypeSafe Jev tool (update this test deliberately when a slice migrates more)",
  );
