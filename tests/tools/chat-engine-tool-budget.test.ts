...
const ROUTING_SRC = fs.readFileSync(
  path.resolve(process.cwd(), "server/chat-routing-helpers.ts"),
  "utf-8",
);
const PROJECT_CONTEXT_SRC = fs.readFileSync(
  path.resolve(process.cwd(), "server/chat-engine/project-context.ts"),
  "utf-8",
);

test("resolveToolBudget exists and is keyed on ADMIN_TENANT_ID", () => {
  assert.match(ROUTING_SRC, /export function resolveToolBudget\(tenantId: number\)/);
  assert.match(ROUTING_SRC, /const isAdmin = tenantId === ADMIN_TENANT_ID;/,
...
test("admin lane defaults are larger than the customer lane and env-overridable", () => {
  assert.match(ROUTING_SRC, /parseIntCap\(process\.env\.MAX_TOOL_ROUNDS_ADMIN, 20, 1, 50/,
    "admin rounds default 20, env-overridable");
  assert.match(ROUTING_SRC, /parseIntCap\(process\.env\.MAX_TOTAL_TOOL_CALLS_ADMIN, 40, 1, 40/,
    "admin total calls have a hard maximum of 40");
  assert.match(ROUTING_SRC, /parseIntCap\(process\.env\.MAX_TOTAL_TOOL_CALLS, DEFAULT_MAX_TOTAL_TOOL_CALLS, 1, 200/,
...
test("project context frames prior files as reuse-before-research", () => {
  assert.match(PROJECT_CONTEXT_SRC, /PRIOR WORK LIVES HERE\. REUSE BEFORE RESEARCH\./,
    "project files section must tell the agent to reuse filed work before re-researching");
});

test("completion reserve is enforced by call index before the hard-cap check", () => {
  assert.match(SRC, /const allowedIndexes = new Set\(completionPlan\.allowedToolIndexes\);/);
  assert.match(SRC, /if \(deferredToolCalls\.length > 0 && !resume\) \{[\s\S]*?for \(const index of deferredIndexes\) allowedIndexes\.add\(index\);[\s\S]*?toolCalls = toolCalls\.filter\(\(_tc, index\) => allowedIndexes\.has\(index\)\);/,
    "failed persistence must restore every research index before admission filtering");
  const filterIndex = SRC.indexOf("toolCalls = toolCalls.filter((_tc, index) => allowedIndexes.has(index));");
  const capIndex = SRC.indexOf("if (totalToolCalls + toolCalls.length > MAX_TOTAL_TOOL_CALLS)", filterIndex);
  assert.ok(filterIndex > 0 && capIndex > filterIndex,
    "reserved call selection must happen before the execution hard-cap branch");
});

test("resume persistence failure never creates a synthetic non-resumable deferral", () => {
  assert.doesNotMatch(SRC, /resumable:\s*false/);
  assert.match(SRC, /Resume persistence unavailable; no calls were deferred/);
});

test("env parsing fails safe on junk values", () => {
  const fn = ROUTING_SRC.slice(ROUTING_SRC.indexOf("function parseIntCap"), ROUTING_SRC.indexOf("export function resolveToolBudget"));
