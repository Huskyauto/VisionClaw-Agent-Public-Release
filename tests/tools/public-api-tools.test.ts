...
test("Wikipedia article-not-found is a normal miss, not a tool failure", async () => {
  const { readAllowedJsonResponse, wikipediaNotFoundResult } = await import("../../server/public-api-tools");
  ...
  assert.deepEqual(
    await readAllowedJsonResponse(response, new Set([404])),
    { type: "https://mediawiki.org/wiki/HyperSwitch/errors/not_found" },
  );
  assert.deepEqual(wikipediaNotFoundResult("Definitely Missing"), {
    ok: true,
    source: "en.wikipedia.org",
    data: {
      found: false,
      query: "Definitely Missing",
      message: 'No Wikipedia article found for "Definitely Missing"',
    },
  });
});
