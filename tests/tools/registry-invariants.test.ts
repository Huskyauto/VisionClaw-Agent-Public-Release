...
test("package is acyclic w.r.t. server/: ALL modules (incl. domains/**) import only within server/tools/", () => {
  ...
  // S3 carve-out (plan.md §S3): the zero-dep ALS ledger-context module is the
  // ONE permitted out-of-package static import — from the action-ledger
  // middleware only. Prove the carve-out stays valid: the target must import
  // node: builtins exclusively, so it creates no edge back into the app graph.
  const carveOutTarget = path.resolve(process.cwd(), "server/lib/action-ledger-context");
  const carveOutSources = new Set([
    path.join(process.cwd(), "server/tools/middleware/action-ledger.ts"),
  ]);
  const ctxSrc = fs.readFileSync(carveOutTarget + ".ts", "utf8");
  const ctxImports = ctxSrc.split("\n").filter((l) => /^import /.test(l));
  assert.ok(
    ctxImports.every((l) => l.includes('"node:')),
    "action-ledger-context must stay zero-dep (node: builtins only) or the acyclicity carve-out is invalid",
  );
  ...
  for (const file of files) {
    ...
    for (const m of src.matchAll(importRe)) {
      ...
      if (resolved === carveOutTarget && carveOutSources.has(file)) continue; // proven zero-dep above
      const within =
