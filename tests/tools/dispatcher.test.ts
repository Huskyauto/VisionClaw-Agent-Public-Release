...
test("Action Ledger S4: dispatcher threads the ALS abort signal acyclically into ctx", () => {
  const dispatcherSrc = readFileSync(
    path.resolve(process.cwd(), "server/tools/dispatcher.ts"),
    "utf8",
  );
  // Call-time dynamic import ONLY — a static `import ... from "../lib/tool-abort-context"`
  // would add a server/tools/ → server/lib edge and trip registry-invariants acyclicity.
  assert.ok(
    dispatcherSrc.includes('await import("../lib/tool-abort-context")'),
    "dispatcher must load tool-abort-context via call-time dynamic import",
  );
  assert.ok(
    !/^import .* from ["']\.\.\/lib\/tool-abort-context["']/m.test(dispatcherSrc),
    "dispatcher must NOT static-import tool-abort-context (acyclicity guard)",
  );
  // The signal read from ALS is passed into buildToolContext as ctx.abortSignal.
  assert.ok(
    dispatcherSrc.includes("getCurrentToolAbortSignal()"),
    "dispatcher must read the signal from the ALS accessor",
  );
  const buildIdx = dispatcherSrc.indexOf("buildToolContext({");
  assert.ok(buildIdx > -1, "dispatcher must build ctx via buildToolContext");
  const ctxBlock = dispatcherSrc.slice(buildIdx, dispatcherSrc.indexOf("});", buildIdx));
  assert.ok(ctxBlock.includes("abortSignal"), "ctx must carry the ALS abortSignal");
  // Unforgeable: no caller-supplied `_abortSignal` param channel may exist.
  assert.ok(
    !dispatcherSrc.includes("params._abortSignal"),
    "abort signal must come from trusted ALS only, never a caller param",
  );
});
