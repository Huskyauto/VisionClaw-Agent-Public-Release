...
test("CSRF bootstrap bypasses the browser response cache", async () => {
  const originalFetch = globalThis.fetch;
  const calls: Array<{ input: RequestInfo | URL; init?: RequestInit }> = [];
  globalThis.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
    calls.push({ input, init });
    if (String(input).includes("/api/auth/csrf-token")) {
      return Promise.resolve(new Response(JSON.stringify({ csrfToken: "fresh-csrf-token" }), { status: 200 }));
    }
    return Promise.resolve(new Response("ok", { status: 200 }));
  }) as typeof fetch;

  try {
    setAuthToken("csrf-cache-test-token");
    await apiRequest("POST", "/api/protected", { action: "save" });

    const csrfCall = calls.find(({ input }) => String(input).includes("/api/auth/csrf-token"));
    assert.equal(csrfCall?.init?.cache, "no-store");
  } finally {
    globalThis.fetch = originalFetch;
    setAuthToken(null);
  }
