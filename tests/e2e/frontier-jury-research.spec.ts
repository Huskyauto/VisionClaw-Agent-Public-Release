    ...
    if (requestPath === `/api/frontier-revenue/runs/${RUN_ID}`) {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          proposers: [
            { modelId: "deepseek/deepseek-v4.1-flash", providerLane: "openrouter", provider: "openrouter", ok: true, error: null },
            { modelId: "gpt-5.6-sol", providerLane: "profundo", provider: "profundo", ok: true, error: null },
