...
describe("integrity — every TOOL_MAP vcTool name exists in TOOL_DEFINITIONS", () => {
  it("verifies all mapped VC tool names are real (no dead references)", async () => {
    ...
    // Tools-layer-split: migrated domains carry their `name: "..."` strings in
    // server/tools/domains/*/definitions.ts (spliced into the facade array as
    // consts), so the text-scan must cover the facade AND every domain file.
    let toolsSrc = await fs.readFile(path.resolve("server/tools.ts"), "utf-8");
    const domainsDir = path.resolve("server/tools/domains");
    for (const domain of await fs.readdir(domainsDir)) {
      const defFile = path.join(domainsDir, domain, "definitions.ts");
      try {
        toolsSrc += "\n" + (await fs.readFile(defFile, "utf-8"));
      } catch {
        // domain without a definitions.ts — nothing to scan
      }
    }
