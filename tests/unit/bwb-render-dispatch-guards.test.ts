...
const githubWrapper = readFileSync(resolve(process.cwd(), "scripts/bwb-render-github.ts"), "utf8");
const localBuilder = readFileSync(resolve(process.cwd(), "scripts/build-bwb-video.ts"), "utf8");
const orchestrator = readFileSync(resolve(process.cwd(), "scripts/bwb-weekly-orchestrator.ts"), "utf8");
const weeklyBuilder = readFileSync(resolve(process.cwd(), "scripts/build-bwb-weekly.ts"), "utf8");
...
test("both BWB delivery paths supply a stable delivery idempotency key", () => {
  assert.match(githubWrapper, /idempotencyKey:\s*`bwb-weekly:\$\{[^}]+\}:delivery`/);
  assert.match(localBuilder, /idempotencyKey:\s*`bwb-weekly:\$\{[^}]+\}:delivery`/);
});

test("GitHub render wrapper exits nonzero when canonical delivery reports failure", () => {
  assert.match(githubWrapper, /if\s*\(!delivery\.success\)\s*\{[\s\S]*?return\s+[1-9]\d*;/);
});

test("weekly orchestrator rejects an unsuccessful or incomplete delivery sidecar", () => {
  assert.match(orchestrator, /result\.success\s*!==\s*true/);
  assert.match(orchestrator, /result\.orchestratorRunId\s*!==\s*orchestratorRunId/);
  assert.match(orchestrator, /findResultForRunSince\(start,\s*orchestratorRunId\)/);
  assert.match(orchestrator, /verifyCompletedArtifactReceipt\(/);
  assert.match(orchestrator, /result\.deliveryId/);
  assert.match(orchestrator, /result\.driveFileId/);
  assert.match(orchestrator, /failBwbJob\(/);
  assert.match(orchestrator, /await completeBwbJob\(/);
  assert.ok(
    orchestrator.indexOf("verifyCompletedArtifactReceipt(") < orchestrator.indexOf("await completeBwbJob("),
    "the orchestrator must verify durable delivery before marking the job done",
  );
  assert.doesNotMatch(
    weeklyBuilder,
    /completeBwbJob\(/,
    "the child builder must never mark a job done from an unverified sidecar",
  );
});
