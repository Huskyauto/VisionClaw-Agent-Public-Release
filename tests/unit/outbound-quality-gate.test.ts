...
test("scanner crash fails open but is marked degraded, never a silent clean verdict", () => {
  const bomb = { toString() { throw new Error("scanner bomb"); } } as unknown as string;
  const r = scanCustomerFacingText(bomb);
  assert.equal(r.blocked, false); // fail-open by design
  assert.equal(r.degraded, true); // but flagged, not silently "clean"
  // a real clean verdict must NOT carry the degraded flag
  assert.equal(scanCustomerFacingText("All good here.").degraded, undefined);
});
