...
test("createBwbJob no-ops on bad job id or non-positive tenant (no DB touch)", async () => {
  assert.equal(await createBwbJob({ jobId: "not-a-vj-id", tenantId: 1 }), false);
  assert.equal(await createBwbJob({ jobId: newBwbJobId(), tenantId: 0 }), false);
  assert.equal(await createBwbJob({ jobId: newBwbJobId(), tenantId: -5 }), false);
});
...
test("the tool creates the live row + threads env BEFORE spawning", () => {
  ...
  assert.match(tools, /const bwbJobCreated = await createBwbJob\(\{ jobId: bwbJobId, tenantId: adminTenant/);
  assert.match(tools, /if \(!bwbJobCreated\)[\s\S]*?return \{[\s\S]*?error:/);
  assert.match(tools, /env\.BWB_JOB_ID = bwbJobId/);
  assert.match(tools, /env\.BWB_TENANT_ID = String\(adminTenant\)/);
  assert.match(
    tools,
    /failBwbJob\([\s\S]{0,300}?bwbJobId,[\s\S]{0,100}?adminTenant/,
    "the server-side early-failure watchdog must persist failure with the owning tenant",
  );
  assert.match(tools, /await spawnOrchestrator\(1\)/);
  assert.match(tools, /const launchFailure = `weekly recap orchestrator failed to launch:[\s\S]*?await failBwbJob\(launchFailure/);
});

test("an asynchronous weekly-build launch is not product-verified as a finished deliverable", () => {
  const registry = read("server/tool-registry.ts");
  assert.match(
    registry,
    /registerTool\("bwb_weekly_build", \{[^}]*isProductOutput: false/,
  );
});

test("launch-row creation uses a cancellable database-side timeout", () => {
  const progress = read("server/lib/bwb-job-progress.ts");
  assert.match(progress, /SET LOCAL statement_timeout = '6000ms'/);
  assert.match(progress, /const inserted = await db\.transaction/);
  assert.doesNotMatch(progress, /const inserted = await raceDb\(db/);
});
