 ...
 *
 *   1. server/lib/outbound-email-preflight.ts — send path (metadata kind:
 *      "send"; task #161 moved the shared send gates here so every transport
 *      — AgentMail AND Gmail-direct — runs them)
...
test("shared send preflight files the degraded incident (outbound-email-preflight.ts)", () => {
  const src = readStripped("server/lib/outbound-email-preflight.ts");
  const wins = degradedIncidentWindows(src);
  assert.ok(
    wins.length >= 1,
    `expected >=1 degraded→reportQualityIncident site in server/lib/outbound-email-preflight.ts, found ${wins.length} — a refactor dropped the fail-open telemetry wiring`,
  );
  assert.ok(
    wins.some((w) => /kind:\s*"send"/.test(w)),
    'send path missing degraded incident (metadata kind: "send")',
  );
  // And email.ts must actually USE the shared preflight for sends, or the
  // wiring above guards dead code.
  const emailSrc = readStripped("server/email.ts");
  assert.ok(
    emailSrc.includes("preflightOutboundEmail"),
    "server/email.ts no longer routes sends through preflightOutboundEmail",
  );
});

test("email reply path files the degraded incident (server/email.ts)", () => {
  const src = readStripped("server/email.ts");
  const wins = degradedIncidentWindows(src);
  assert.ok(
    wins.length >= 1,
    `expected >=1 degraded→reportQualityIncident site in server/email.ts (reply path), found ${wins.length}`,
  );
  assert.ok(
