...
test("jury FIX 2-of-3 STRICT MAJORITY → repo_surgeon (owner majority-rule policy, 2026-07-28)", () => {
  const r = mapJuryDecision({ verdict: "FIX", majority: 2, fixConcordance: 0.9 });
  assert.equal(r.classification, "code_defect");
  assert.equal(r.routedTo, "repo_surgeon");
});

test("INVARIANT: jury FIX WITHOUT a strict majority (1-of-3) → escalate_owner (never auto-fix a minority view)", () => {
  const r = mapJuryDecision({ verdict: "FIX", majority: 1, fixConcordance: 0.9 });
  assert.equal(r.classification, "code_defect");
  assert.equal(r.routedTo, "escalate_owner");
  assert.notEqual(r.routedTo, "repo_surgeon");
});

test("even jury size: FIX 2-of-4 is NOT a strict majority → escalate_owner; 3-of-4 is → repo_surgeon", () => {
  const four = [{ verdict: "FIX" }, { verdict: "FIX" }, { verdict: "ACCEPT" }, { verdict: "REJECT" }];
  assert.equal(mapJuryDecision({ verdict: "FIX", majority: 2, fixConcordance: 0.9, votes: four }).routedTo, "escalate_owner");
  assert.equal(mapJuryDecision({ verdict: "FIX", majority: 3, fixConcordance: 0.9, votes: four }).routedTo, "repo_surgeon");
});

test("malformed/legacy cardinality: votes=[] or missing defaults to a 3-seat jury — a sub-majority NEVER closes or auto-fixes", () => {
  // Empty votes array = legacy shape → 3-seat default; majority >= 2 still required.
  assert.equal(mapJuryDecision({ verdict: "ACCEPT", majority: 1, votes: [] }).routedTo, "escalate_owner");
  assert.equal(mapJuryDecision({ verdict: "REJECT", majority: 0, votes: [] }).routedTo, "escalate_owner");
  assert.equal(mapJuryDecision({ verdict: "FIX", majority: 1, fixConcordance: 0.9, votes: [] }).routedTo, "escalate_owner");
  // With a real majority under the 3-seat default, decisions still land.
  assert.equal(mapJuryDecision({ verdict: "ACCEPT", majority: 2, votes: [] }).routedTo, "surface");
});

test("missing votes array defaults to a 3-seat jury (legacy entries still decide)", () => {
  assert.equal(mapJuryDecision({ verdict: "ACCEPT", majority: 2 }).routedTo, "surface");
  assert.equal(mapJuryDecision({ verdict: "FIX", majority: 2, fixConcordance: 0.9 }).routedTo, "repo_surgeon");
});

test("INVARIANT: jury shouldEscalate=true overrides a majority FIX → escalate_owner", () => {
