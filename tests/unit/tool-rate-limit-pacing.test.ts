...
test("owner research capacity is not shared with the customer-sized 40/hour ceiling", () => {
  assert.equal(getRateLimitConfig("web_fetch", 1).maxPerHour, 160);
  assert.equal(getRateLimitConfig("web_search", 1).maxPerHour, 120);
  assert.equal(getRateLimitConfig("web_fetch", 990099).maxPerHour, 40);
});

test("owner web_fetch remains available after 40 calls and denies at the 160/hour ceiling", () => {
  for (let i = 0; i < 40; i++) recordToolUsage(1, "web_fetch");
  Date.now = () => realNow() + 61_000;
  try {
    assert.equal(checkToolRateLimit(1, "web_fetch").allowed, true,
      "the screenshot's old 40/hour boundary must no longer starve owner reports");
    for (let i = 0; i < 120; i++) recordToolUsage(1, "web_fetch");
  } finally {
    Date.now = realNow;
  }
  Date.now = () => realNow() + 122_000;
  try {
    const exhausted = checkToolRateLimit(1, "web_fetch");
    assert.equal(exhausted.allowed, false);
    assert.equal(exhausted.window, "hour");
    assert.match(exhausted.reason || "", /160\/160/);
  } finally {
    Date.now = realNow;
  }
});
...
test("alternates hint: hour/day ceilings refuse avenue-spreading", () => {
  for (const w of ["hour", "day"] as const) {
    ...
    assert.match(hint, /finish|finalize/i);
  }
});
