...
import { computeWeekWindow, autoPinWindowYmd } from "../../scripts/lib/drive-discover";
...
// --- AUTO-PIN glue (2026-07-12): the exact values main() pins into env ---
// Run day Sun 2026-07-12 → just-completed week is 07-05 (Sun) → 07-11 (Sat).
test("autoPinWindowYmd (no anchor) yields the last completed week as YMD strings + suffix", () => {
  const p = autoPinWindowYmd({ now: new Date(2026, 6, 12, 9, 0, 0) });
  assert.equal(p.start, "2026-07-05");
  assert.equal(p.end, "2026-07-11");
  assert.equal(p.jobKeySuffix, "-2026-07-05_2026-07-11");
  assert.match(p.windowDesc, /last completed week/);
});
test("autoPinWindowYmd on a Saturday still pins the PRIOR completed week", () => {
  const p = autoPinWindowYmd({ now: new Date(2026, 6, 11, 23, 0, 0) }); // Sat 07-11
  assert.equal(p.start, "2026-06-28");
  assert.equal(p.end, "2026-07-04");
});
test("autoPinWindowYmd with BWB_WEEK_ANCHOR pins the anchor's containing week", () => {
  const p = autoPinWindowYmd({ anchorRaw: "2026-07-08" }); // Wed of the 07-05 week
  assert.equal(p.start, "2026-07-05");
  assert.equal(p.end, "2026-07-11");
  assert.equal(p.jobKeySuffix, "-2026-07-05_2026-07-11");
  assert.match(p.windowDesc, /anchor 2026-07-08/);
});
test("autoPinWindowYmd suffix matches the builder's normalizeYmd shape (\\d{4}-\\d{2}-\\d{2})", () => {
  const p = autoPinWindowYmd({ now: new Date(2026, 0, 2) });
  assert.match(p.jobKeySuffix, /^-\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2}$/);
});
test("autoPinWindowYmd with an unparseable anchor throws loud", () => {
  assert.throws(() => autoPinWindowYmd({ anchorRaw: "garbage" }), /invalid anchorDate/);
});
