...
test("findFallbackModel never returns routing pseudo-models", () => {
  const pool = [
    { id: "auto", provider: "replit", tier: "fast" },
    { id: "auto-ensemble:test", provider: "replit", tier: "fast" },
  ] as any;
  const fb = findFallbackModel("z-ai/glm-4.7-flash", pool);
  assert.equal(fb, null, "routing selectors must never be sent to a provider");
});
