...
import { _parseVote, _tallyVotes, _meanPairwiseCosine, juryTriage, type JuryVerdict } from "../../server/lib/jury-triage";
...
test("jury triage automatically attaches a report-only TypeSafe validation without changing the verdict", async () => {
  let validationCalls = 0;
  const result = await juryTriage({
    issueText: "A real issue that needs an independent decision.",
    context: "Two traces show the same failure.",
    tenantId: 42,
    _executeMoAFn: async () => ({
      proposers: [
        { ok: true, answer: "VERDICT: ACCEPT\nRATIONALE: bounded deferral.", modelId: "a", provider: "p", role: "proposer" },
        { ok: true, answer: "VERDICT: ACCEPT\nRATIONALE: bounded deferral.", modelId: "b", provider: "p", role: "proposer" },
        { ok: true, answer: "VERDICT: REJECT\nRATIONALE: non-issue.", modelId: "c", provider: "p", role: "proposer" },
      ],
      jurySeatCount: 3,
      concordance: 0.8,
      shouldEscalate: false,
      aggregated: "The majority recommends a bounded deferral.",
      totalLatencyMs: 25,
      responseId: "test-response",
    }) as any,
    _validationFn: async (input) => {
      validationCalls++;
      assert.equal(input.subjectKind, "jury");
      assert.match(input.output, /ACCEPT/);
      return {
