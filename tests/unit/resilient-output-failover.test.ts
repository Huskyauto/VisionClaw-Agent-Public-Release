...
const AVAILABLE = [
  { id: "gpt-5", provider: "replit", tier: "powerful" },
...
test("no exclusions → start on the requested model, no seed (normal path)", () => {
  const r = planOutputFailoverStart("gpt-5", [], AVAILABLE, new Set());
  assert.equal(r.startModel, "gpt-5");
  assert.deepEqual(r.triedSeed, []);
});

test("requested model produced unusable output → fail over to a DIFFERENT model", () => {
  const r = planOutputFailoverStart("gpt-5", ["gpt-5"], AVAILABLE, new Set());
  assert.notEqual(r.startModel, "gpt-5", "must not re-pick the unusable model when an alt exists");
  assert.equal(r.startModel, "claude-opus-4-8");
  assert.ok(r.triedSeed.includes("gpt-5"), "unusable model is seeded as already-tried");
});
...
test("ALL available models excluded → gracefully fall back to the requested model", () => {
  ...
  assert.ok(!r.triedSeed.includes("gpt-5"));
...
test("a non-requested model is excluded → still start on the requested model, seed the other", () => {
  const r = planOutputFailoverStart("gpt-5", ["claude-opus-4-8"], AVAILABLE, new Set());
  assert.equal(r.startModel, "gpt-5");
