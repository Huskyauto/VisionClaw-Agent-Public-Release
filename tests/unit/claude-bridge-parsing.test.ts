...
import {
  ...
  buildSafeEnv,
  isClaudeRunnerSubscriptionConfigured,
  processNdjsonEvent,
...
test("buildSafeEnv drops ANTHROPIC_API_KEY when a valid OAuth subscription token is set", () => {
  const oauthToken = `sk-ant-oat01-${"x".repeat(40)}`;
  withEnv({ ANTHROPIC_API_KEY: "sk-fake", CLAUDE_CODE_OAUTH_TOKEN: oauthToken }, () => {
    const env = buildSafeEnv();
    assert.equal("ANTHROPIC_API_KEY" in env, false);
    assert.equal(env.CLAUDE_CODE_OAUTH_TOKEN, oauthToken);
  });
});

test("buildSafeEnv keeps ANTHROPIC_API_KEY when the OAuth value is invalid", () => {
...
test("subscription child env uses the captured token and can never contain ANTHROPIC_API_KEY", () => {
  const capturedToken = `sk-ant-oat01-${"x".repeat(40)}`;
  withEnv({
    ANTHROPIC_API_KEY: "sk-fake",
    CLAUDE_CODE_OAUTH_TOKEN: "changed-after-validation",
  }, () => {
    const env = buildClaudeSubscriptionEnv(capturedToken);
    assert.equal(env.CLAUDE_CODE_OAUTH_TOKEN, capturedToken);
    assert.equal("ANTHROPIC_API_KEY" in env, false);
  });
});
...
test("Claude Runner subscription configuration requires a setup-token shape, not merely a non-blank value", () => {
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ ANTHROPIC_API_KEY: "sk-fake" }),
    false,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: "   " }),
    false,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: "oauth-fake" }),
    false,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: `sk-ant-oat-${"x".repeat(40)}` }),
    false,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: "sk-ant-oat01-short" }),
    false,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: `sk-ant-oat01-${"x".repeat(39)}!` }),
    false,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: `sk-ant-oat01-${"x".repeat(40)}` }),
    true,
  );
  assert.equal(
    isClaudeRunnerSubscriptionConfigured({ CLAUDE_CODE_OAUTH_TOKEN: `  sk-ant-oat01-${"x".repeat(40)}  ` }),
...
test("Claude Runner never listens when a shaped token fails the live entitlement probe", async () => {
  const { startClaudeRunnerBridge, stopClaudeRunnerBridge } = await import("../../server/claude-runner");
  const oauthToken = `sk-ant-oat01-${"x".repeat(40)}`;
  ...
  try {
    await withEnvAsync({
      ...
      assert.equal(
        await startClaudeRunnerBridge({
          verifyCli: async (capturedToken: string) => {
            assert.equal(capturedToken, oauthToken);
            process.env.CLAUDE_CODE_OAUTH_TOKEN = "changed-after-validation";
            return true;
          },
          verifyEntitlement: async (capturedToken: string) => {
            assert.equal(capturedToken, oauthToken);
            return false;
          },
          listen: async () => {
...
test("Claude Runner refuses to start with only a metered Anthropic API key", async () => {
  const { startClaudeRunnerBridge, stopClaudeRunnerBridge } = await import("../../server/claude-runner");
  await withEnvAsync({ ANTHROPIC_API_KEY: "sk-fake", CLAUDE_CODE_OAUTH_TOKEN: undefined }, async () => {
    assert.equal(await startClaudeRunnerBridge(), false);
  });
  await stopClaudeRunnerBridge();
});

test("concurrent unauthenticated Claude Runner starts are serialized and fail closed", async () => {
  const { startClaudeRunnerBridge, stopClaudeRunnerBridge } = await import("../../server/claude-runner");
  await withEnvAsync({ ANTHROPIC_API_KEY: "sk-fake", CLAUDE_CODE_OAUTH_TOKEN: undefined }, async () => {
    assert.deepEqual(
      await Promise.all([startClaudeRunnerBridge(), startClaudeRunnerBridge()]),
      [false, false],
    );
  });
  await stopClaudeRunnerBridge();
