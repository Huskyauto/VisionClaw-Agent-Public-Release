...
test("majority safe FIX (dev) → dispatched to repo_surgeon, no ledger close", async () => {
  const { deps, calls } = mkDeps({
    // Majority-rule (owner policy 2026-07-28): a FIX needs a strict majority
    // (2-of-3, 3-of-4, …); unanimity is sufficient but no longer required.
    runJury: async () =>
...
test("jury escalate residue + Felix ACCEPT → closed accepted by felix", async () => {
  const { deps, calls } = mkDeps({
    runJury: async () => jd({ verdict: "FIX", majority: 1 }), // no strict majority → escalate_owner
