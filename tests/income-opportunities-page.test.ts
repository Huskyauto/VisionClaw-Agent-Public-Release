...
test("AI task fit audit has an owner-only launch workspace and actionable opportunity card", () => {
  ...
  assert.match(app, /\{isPlatformAdmin && <Route path="\/income-opportunities\/ai-task-fit-audit"/);
  assert.match(
    sidebar,
    /<NavSection title="Income Opportunities"[\s\S]*?path="\/income-opportunities\/ai-task-fit-audit"[\s\S]*?label="AI Task Fit & Human Agency Audit"/,
  );
  assert.match(catalog, /slug: "ai-task-fit-human-agency-audit"[\s\S]*?actionPath: "\/income-opportunities\/ai-task-fit-audit"/);
