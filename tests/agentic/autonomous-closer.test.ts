...
type Calls = { approved: number[]; applied: number[]; rejected: number[]; reverified: Array<{ id: number; tenantId: number }>; rolledBack: number[]; notified: string[] };
...
function makeDeps(over: Partial<CloserDeps>, calls: Calls): Partial<CloserDeps> {
  return {
    async reverify(id, tenantId) { calls.reverified.push({ id, tenantId }); return { status: "passed" }; },
...
async function run() {
  // 1. FIX majority → re-verify, approve, apply.
  {
    ...
    assert(calls.reverified.some(c => c.id === 1 && c.tenantId === 1) && calls.approved.includes(1) && calls.applied.includes(1), "FIX path re-verifies with tenant scope, approves, applies");
  ...
  // 8. Re-verify regressed → not applied (stale source guard).
  {
    ...
    const deps = makeDeps({
      ...
      async runJury() { return jury("FIX", 3); },
      async reverify(id, tenantId) { calls.reverified.push({ id, tenantId }); return { status: "failed", details: "stale" }; },
    }, calls);
    const r = await runAutonomousCodeCloser({ tenantId: 1, deps });
