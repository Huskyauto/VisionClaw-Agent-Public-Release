...
test("parameterizedTextArray: adversarial names remain an opaque JS-array parameter", () => {
  const names = [
    String.raw`quote"and\backslash`,
    `"}'); DROP TABLE capability_reviews; --`,
    "comma,name",
    "{brace}",
    "",
  ];
  const query = new PgDialect().sqlToQuery(parameterizedTextArray(names));

  assert.equal(query.sql, "$1::text[]");
  assert.deepEqual(query.params, [names]);
  assert.strictEqual(query.params[0], names, "the exact array must be bound, not serialized by application code");
  assert.ok(!query.sql.includes(names[1]), "adversarial content must never enter SQL source text");
});
