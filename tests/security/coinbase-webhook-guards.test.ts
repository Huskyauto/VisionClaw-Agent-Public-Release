...
test("Coinbase webhook retains global/per-tenant candidates and claim retry semantics", () => {
  ...
  assert.match(source, /if \(claim\.status === 'completed'\)/);
  assert.match(source, /if \(claim\.status === 'in_flight'\)/);
  assert.match(source, /if \(claim\.status === 'retry'\)/);
  assert.match(source, /await markWebhookEventCompleted\('coinbase', eventId, claim\.claimToken\)/);
});