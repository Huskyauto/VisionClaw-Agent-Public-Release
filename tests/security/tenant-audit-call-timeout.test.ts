...
import { writeTenantAuditReportFiles } from "../../scripts/lib/tenant-audit-report-writer";
...
test("tenant-isolation audit model calls are abortable and bounded", () => {
  ...
  assert.match(
    source,
    /cov\.sourceChangedDuringRun\s*=[\s\S]*?computeTenantAuditEvidenceHash\(ROOT\) !== evidenceHash;[\s\S]*?writeReport\([\s\S]*?if \(cov\.sourceChangedDuringRun\)[\s\S]*?checkpoint retained[\s\S]*?process\.exit\(5\)/,
    "the final evidence-hash check must happen before the only report write",
  );
...
test("tenant-audit evidence hash includes report destination logic", () => {
  const root = mkdtempSync(join(tmpdir(), "tenant-audit-hash-"));
  try {
    mkdirSync(join(root, "server"), { recursive: true });
    mkdirSync(join(root, "scripts", "lib"), { recursive: true });
    writeFileSync(join(root, "server", "route.ts"), "export const route = true;\n");
    writeFileSync(join(root, "scripts", "lib", "tenant-audit-report-writer.ts"), "export const destination = 'latest';\n");
    const before = computeTenantAuditEvidenceHash(root);
    writeFileSync(join(root, "scripts", "lib", "tenant-audit-report-writer.ts"), "export const destination = 'degraded';\n");
    assert.notEqual(computeTenantAuditEvidenceHash(root), before);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("degraded tenant audits preserve the authoritative completed report", () => {
  const root = mkdtempSync(join(tmpdir(), "tenant-audit-report-"));
  const targets = {
    reportMd: join(root, "docs", "tenant-isolation-audit-report.md"),
    reportJson: join(root, "data", "tenant-isolation-audit", "latest.json"),
    degradedReportMd: join(root, "docs", "tenant-isolation-audit-report-degraded.md"),
    degradedReportJson: join(root, "data", "tenant-isolation-audit", "degraded-latest.json"),
  };
  try {
    mkdirSync(join(root, "docs"), { recursive: true });
    mkdirSync(join(root, "data", "tenant-isolation-audit"), { recursive: true });
    writeFileSync(targets.reportMd, "complete markdown");
    writeFileSync(targets.reportJson, '{"degraded":false}');

    for (const degradation of ["coverage failure", "precision failure", "late source mutation"]) {
      writeTenantAuditReportFiles({
        degraded: true,
        markdown: `degraded markdown: ${degradation}`,
        json: JSON.stringify({ degraded: true, reason: degradation }),
        targets,
      });

      assert.equal(readFileSync(targets.reportMd, "utf8"), "complete markdown", degradation);
      assert.equal(readFileSync(targets.reportJson, "utf8"), '{"degraded":false}', degradation);
      assert.match(readFileSync(targets.degradedReportMd, "utf8"), new RegExp(degradation));
      assert.match(readFileSync(targets.degradedReportJson, "utf8"), new RegExp(degradation));
    }
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
