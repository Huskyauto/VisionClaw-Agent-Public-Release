...
const UPLOAD_DIRECT_BASELINE: CallsiteMap = {
  ...
  // Task 102 girth split: the research-digest callsite moved verbatim from
  // server/research-engine.ts (line 1865) into the extracted proposal cluster.
  "server/research-proposals.ts": [
    { line: 830, text: "const driveResult = await uploadAndShare({" },
  ],
  "server/routes.ts": [
  ...
  // R125+137.52 triage: tools-layer split moved two audited callsites out of
  // tools.ts into server/tools/domains/files/handlers.ts (same code, new home;
  // gd.uploadAndShare is the namespaced import of the same audited function).
  // No NEW direct-upload sites were introduced — baseline re-keyed only.
  "server/tools.ts": [
    { line: 3775, text: "driveResult = await uploadAndShare({ filePath, fileName: filename, mimeType: \"image/png\", folderLabel, parentFolderId: params._projectDriveFolderId || undefined });" },
    { line: 6340, text: "const driveResult = await uploadAndShare({" },
    { line: 6876, text: "const driveResult = await uploadAndShare({" },
    { line: 7119, text: "const driveResult = await uploadAndShare({ filePath: localPath, fileName, mimeType, folderLabel, parentFolderId: params._projectDriveFolderId || undefined });" },
  ],
  "server/tools/domains/files/handlers.ts": [
    { line: 516, text: "const shareResult = await gd.uploadAndShare({" },
    { line: 786, text: "const driveResult = await uploadAndShare({" },
  ],
