 ...
 *
 * Why: workspace uploads/ and the configured UPLOADS_DIR are SHARED flat
 * directories across tenants.
...
import { UPLOADS_DIR, isPathWithin, resolveUploadsChild } from "../../server/uploads-path";
...
test("configured upload root rejects traversal and sibling-prefix paths", () => {
  assert.equal(isPathWithin(UPLOADS_DIR, resolveUploadsChild("safe-file.pdf")), true);
  assert.throws(() => resolveUploadsChild("../outside.pdf"), /escapes/);
  assert.equal(isPathWithin(UPLOADS_DIR, `${UPLOADS_DIR}-other/file.pdf`), false);
});
...
test("gate fails closed: missing tenant context and DB errors both deny", () => {
  ...
  assert.ok(
    /UPLOADS_ROOT_WITH_SEP/.test(gate),
    "configured uploads root no longer covered by the uploads gate",
  );
});

test("realpath policy allows only workspace and configured uploads, after symlink rejection", () => {
  const src = fs.readFileSync(HANDLERS, "utf-8");
  const symlinkIdx = src.indexOf("isSymbolicLink()");
  const realpathIdx = src.indexOf("isPathInsideRoot(realPath, WORKSPACE_ROOT)");
  ...
  assert.ok(
    /isPathInsideRoot\(realPath,\s*UPLOADS_ROOT\)/.test(src),
    "configured uploads realpath allowance missing — declared search root would be un-readable again",
  );
});

test("/uploads disk fallback is reserved for signed rowless capabilities", () => {
  const src = fs.readFileSync(ROUTES, "utf-8");
  const start = src.indexOf('app.get("/uploads/:filename"');
  const end = src.indexOf('app.post("/api/voice/conversations', start);
  assert.ok(start >= 0 && end > start, "uploads route block not found");
  const route = src.slice(start, end);

  assert.match(route, /let diskFallbackAuthorized = false/);
  assert.match(route, /diskFallbackAuthorized = true;\s*tenantId = tid/);
  assert.doesNotMatch(
    route,
    /if \(stored\) \{\s*diskFallbackAuthorized = true/,
    "a scoped metadata row cannot authorize ambiguous bytes in a flat shared directory",
  );
  assert.match(
    route,
    /if \(stored\) \{[\s\S]*return res\.status\(404\).*File not found/,
    "a database-backed upload with no retrievable scoped bytes must fail closed",
  );
  assert.match(
    route,
    /if \(!diskFallbackAuthorized\) \{\s*return res\.status\(404\)/,
    "a bare tenant session must not fall through to shared disk after its scoped DB lookup misses",
  );
  assert.match(
    route,
    /catch \(dbErr\) \{[\s\S]*return res\.status\(503\)/,
    "DB ownership failures must fail closed rather than falling through to shared disk",
  );
});
