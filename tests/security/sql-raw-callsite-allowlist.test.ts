...
const SQL_RAW_BASELINE: CallsiteMap = {
  ...
  "server/embeddings.ts": [
    ...
    { line: 236, text: "1 - (embedding_vec <=> ${sql.raw(`'${vec}'::vector`)}) AS similarity" },
    { line: 244, text: "ORDER BY embedding_vec <=> ${sql.raw(`'${vec}'::vector`)}" },
    { line: 440, text: "1 - (embedding_vec <=> ${sql.raw(`'${vec}'::vector`)}) AS similarity" },
    { line: 446, text: "ORDER BY embedding_vec <=> ${sql.raw(`'${vec}'::vector`)}" },
    { line: 548, text: "// compile time but the runtime sees a plain string sunk into sql.raw()." },
    { line: 562, text: "await db.execute(sql`UPDATE ${sql.raw(table)} SET embedding_vec = ${sql.raw(`'${vec}'::vector`)}, embedding = ${JSON.stringify(embedding)}::jsonb WHERE id = ${id}`);" },
    { line: 604, text: "await db.execute(sql`UPDATE memory_entries SET embedding_vec = ${sql.raw(`'${vec}'::vector`)} WHERE id = ${row.id}`);" },
    { line: 621, text: "await db.execute(sql`UPDATE agent_knowledge SET embedding_vec = ${sql.raw(`'${vec}'::vector`)} WHERE id = ${row.id}`);" },
  ],
  ...
  "server/plan-executor.ts": [
    { line: 347, text: "AND updated_at < NOW() - INTERVAL '${sql.raw(String(STALE_EXECUTING_MIN))} minutes'" },
  ],
  ...
  "server/research-engine.ts": [
    { line: 151, text: "AND id NOT IN (${sql.raw(Array.from(activeSessions.keys()).join(\",\") || \"0\")})" },
  ],
