...
const SQL_RAW_BASELINE: CallsiteMap = {
  ...
  // Comment-only mention of sql.raw inside the CI-rule classifier doc — the line
  // literally describes the "sql.raw drift" guard. No executable callsite, no
  // user/LLM input; the walker matches the substring in the comment text.
  "server/agentic/repair-incident.ts": [
    { line: 289, text: "// 2. CI rule that intentionally demands human review (sql.raw drift, stale" },
  ],
  ...
  "server/embeddings.ts": [
    ...
    { line: 769, text: "SET embedding_vec = ${sql.raw(`'${vec}'::vector`)}," },
    { line: 798, text: "SET embedding_vec = ${sql.raw(`'${vec}'::vector`)}," },
  ...
  // R125+129 — comment-only mention in the Evidence Docket κ query doc; the
  // actual moa_responses query is fully parameterized (audited 2026-07-09).
  "server/evidence-docket.ts": [
    { line: 0, text: "// κ. Tenant-scoped + parameterized (never sql.raw with input)." },
  ],
  ...
  "server/seed.ts": [
    { line: 3893, text: "await db.execute(sql.raw(ddl));" },
    { line: 4078, text: "await db.execute(sql.raw(ddl));" },
    // R125+147 — plans approval-TTL DDL ensure: both arguments are static
    // string literals (constraint widen DO-block + partial index), no
    // interpolated/tainted input. Audited 2026-08-01.
    { line: 3905, text: "await db.execute(sql.raw(`" },
    { line: 3921, text: "await db.execute(sql.raw(" },
  ],
