...
import { canReadWebhookStatus, configureWebhooks, registerWebhookRoutes } from "../../server/webhooks";
...
test("webhook status: only the platform-admin tenant can read global status via session", () => {
  assert.equal(
    canReadWebhookStatus({ tenantId: 2, isAdmin: true }),
    false,
    "a tenant-local admin must not read process-global webhook state",
  );
  assert.equal(canReadWebhookStatus({ tenantId: 1, isAdmin: false }), false);
  assert.equal(canReadWebhookStatus({ tenantId: 1, isAdmin: true }), true);
  assert.equal(canReadWebhookStatus(null), false);
});
