...
// --- Blocklist is NOT load-bearing (Fable 5 suggestion, 2026-07-13): prove the
// keyword-free probes above were stopped by the VM LAYER, not the regex. If a
// probe were caught by BLOCKED_PATTERNS, its error would be the validateCode
// "Blocked: ..." message and these tests would pass for the WRONG reason —
// silently letting a future contributor believe the regex is the control.
// Asserting the error is NOT the regex-block message pins the real invariant:
// codeGeneration:{strings:false} + no-host-objects hold even when the regex
// sees nothing. Do not "fix" a failure here by adding regex patterns.
for (const [name, code] of keywordFreeEscapeProbes) {
  test(`blocklist not load-bearing: ${name} passes the regex yet still fails closed`, () => {
    const r = executeCode(code);
    assert.equal(r.success, false, `${name} should fail closed at the vm layer`);
    assert.ok(
      !(r.error ?? "").startsWith("Blocked:"),
      `${name} was caught by the regex blocklist ("${r.error}") — probe no longer proves the vm layer; rewrite it to evade BLOCKED_PATTERNS`,
    );
  });
}
