...
test("R125+136 reconciled ids are PRESENT in the cost-ledger map (absence = silent fallback mispricing)", () => {
  ...
  const required: Record<string, Price> = {
    ...
    "deepseek/deepseek-v3.2": { inPerM: 0.269, outPerM: 0.40 },
...
test("live-rate-sensitive models are present in all three maps (missing-row = silent $0)", () => {
  // Live-rate sync 2026-09-02 (weekly-maintenance Pass 17). The cross-map drift tests
  ...
  const expected: Record<string, Price> = {
    "nvidia/nemotron-3-super-120b-a12b": { inPerM: 0.085, outPerM: 0.40 },
    "z-ai/glm-5.3-flash": { inPerM: 0.075, outPerM: 0.25 },
    "z-ai/glm-5.2": { inPerM: 0.966, outPerM: 3.036 },
    "z-ai/glm-5.1": { inPerM: 0.966, outPerM: 3.036 },
    "moonshotai/kimi-k2.6": { inPerM: 0.95, outPerM: 4 },
    "deepseek/deepseek-v4-flash": { inPerM: 0.088606, outPerM: 0.177212 },
    "tencent/hy4-preview": { inPerM: 0.834, outPerM: 2.501 },
  };
...
test("platform default model gpt-5.6-sol present in all three maps (R125+137.3 regression)", () => {
  ...
  const want: Price = { inPerM: 2, outPerM: 10 };
  for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
...
test("DeepSeek V4 Flash 0731 is present in all three maps at the live rate", () => {
  const expected: Record<string, Price> = {
...
test("premium-pool models are present in all three maps at live rates", () => {
  const expected: Record<string, Price> = {
    "google/gemini-3.7-flash": { inPerM: 0.75, outPerM: 3.75 },
    "deepseek/deepseek-v4-pro-0813": { inPerM: 1.1154, outPerM: 3.3462 },
  };
  for (const [id, want] of Object.entries(expected)) {
    for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
      const p = map[id];
