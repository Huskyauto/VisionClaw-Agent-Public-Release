...
test("R125+136 reconciled ids are PRESENT in the cost-ledger map (absence = silent fallback mispricing)", () => {
  ...
  const required: Record<string, Price> = {
    ...
    "z-ai/glm-5.1": { inPerM: 0.952, outPerM: 2.992 },
    "gpt-5.4": { inPerM: 2.5, outPerM: 15 },
    "gpt-4.1": { inPerM: 2, outPerM: 8 },
    "deepseek/deepseek-v3.2": { inPerM: 0.269, outPerM: 0.4 },
...
test("2026-08-07 refreshed trio present in ALL THREE maps at live rates (missing-row = silent $0)", () => {
  // Live-rate sync 2026-08-07 (weekly-maintenance Pass 17). The cross-map drift tests
  // only compare OVERLAPPING ids — kimi-k2.6 was priced in predictor/insights but
  // MISSING from the ledger, so real K2.6 calls ledgered as free via the unknown-model
  // fallback. This assertion pins presence + rate in all three maps.
  const expected: Record<string, Price> = {
    "z-ai/glm-5.2": { inPerM: 0.4746, outPerM: 1.4916 },
    "z-ai/glm-5.1": { inPerM: 0.952, outPerM: 2.992 },
    "moonshotai/kimi-k2.6": { inPerM: 0.5795, outPerM: 2.44 },
  };
  for (const [id, want] of Object.entries(expected)) {
    for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
      const p = map[id];
      assert.ok(p, `${id} missing from ${name} pricing map — usage prices via silent fallback`);
      assert.ok(
        close(p.inPerM, want.inPerM) && close(p.outPerM, want.outPerM),
...
test("platform default model gpt-5.6-sol present in all three maps (R125+137.3 regression)", () => {
  ...
  // Live OpenRouter rate verified 2026-07-21 by scripts/model-pricing-audit.ts.
  const want: Price = { inPerM: 5, outPerM: 30 };
  for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
...
test("Grok ids present in all three maps at live OpenRouter rates (R125+134 regression)", () => {
  const expected: Record<string, Price> = {
    "x-ai/grok-4.6": { inPerM: 2, outPerM: 6 },
...
test("insights prefix table: no entry is shadowed by an earlier shorter prefix (72h-review HIGH regression)", () => {
  // pricingFor() in insights-engine is FIRST-MATCH-WINS over startsWith prefixes.
  // deepseek/deepseek-v4-flash-0731 shipped BELOW the generic deepseek/deepseek-v4-flash
  // row would price at $0.14/$0.28 instead of $0.09/$0.18 — the cross-map drift tests
  // could not see it because they compare exact keys only. Guard the ORDER generically:
  // whenever key A is a strict prefix of key B, B must appear BEFORE A in the array.
  const block = sliceDecl("server/insights-engine.ts", "PRICING_PER_M_TOKENS");
  const ordered: string[] = [];
  for (const m of block.matchAll(/\["([^"]+)",\s*\{\s*in:\s*([\d.]+),\s*out:\s*([\d.]+)\s*\}\]/g)) {
    ordered.push(m[1].toLowerCase());
  }
  assert.ok(ordered.length >= 15, "insights ordered parse yielded suspiciously few entries — regex drift?");
  const shadowed: string[] = [];
  for (let i = 0; i < ordered.length; i++) {
    for (let j = i + 1; j < ordered.length; j++) {
      // ordered[j] comes AFTER ordered[i]; if ordered[j] startsWith ordered[i], it is dead.
      if (ordered[j] !== ordered[i] && ordered[j].startsWith(ordered[i])) {
        shadowed.push(`"${ordered[j]}" is unreachable — shadowed by earlier prefix "${ordered[i]}" (move the longer key ABOVE the shorter one)`);
      }
    }
  }
  assert.deepStrictEqual(shadowed, [], shadowed.join("\n"));
});

test("DeepSeek V4 Flash 0731 present in all three maps at live rate (72h-review HIGH regression)", () => {
  const expected: Record<string, Price> = {
    "deepseek/deepseek-v4-flash-0731": { inPerM: 0.09, outPerM: 0.18 },
  };
  for (const [id, want] of Object.entries(expected)) {
    for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
      const p = map[id];
...
test("2026-08-13 premium-pool models present in all three maps at live rates (72h-review MEDIUM regression)", () => {
  // Gemini 3.7 Flash (escalation lane + premium drafter, $0.75/$3.75 per M — doubles
  // 2027-01-01) and DeepSeek V4 Pro 0813 (premium drafter + jury proposer). The
  // Gemini id was priced in the ledger only — predictor fell back to the generic
  // powerful-tier rate and insights reported $0. Pin presence + rate in all three.
  const expected: Record<string, Price> = {
    "google/gemini-3.7-flash": { inPerM: 0.75, outPerM: 3.75 },
    "deepseek/deepseek-v4-pro-0813": { inPerM: 0.435, outPerM: 0.87 },
  };
  for (const [id, want] of Object.entries(expected)) {
    for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
      const p = map[id];
...
test("Kimi K3 present in all three maps at live OpenRouter rate (FRONTIER_RESERVE proposer)", () => {
  // moonshotai/kimi-k3 — $3/M in, $15/M out (live OpenRouter, verified 2026-07-27).
  // Absent from the cost-ledger it billed at $0 via the unknown-model fallback
  // while resource-predictor/insights already carried it.
  const expected: Record<string, Price> = {
    "moonshotai/kimi-k3": { inPerM: 3, outPerM: 15 },
  };
  for (const [id, want] of Object.entries(expected)) {
    for (const [name, map] of [["ledger", ledger], ["predictor", predictor], ["insights", insights]] as const) {
      const p = map[id];
