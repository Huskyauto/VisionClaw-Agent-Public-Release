...
test("ownerJuryMeteredEnabled requires positive opt-in", () => {
  const saved = process.env.OWNER_JURY_METERED;
  try {
    delete process.env.OWNER_JURY_METERED;
    assert.equal(ownerJuryMeteredEnabled(), false);
    for (const off of ["false", "0", "no", "off", "FALSE", "Off"]) {
      process.env.OWNER_JURY_METERED = off;
      assert.equal(ownerJuryMeteredEnabled(), false, `expected ${off} to remain disabled`);
    }
