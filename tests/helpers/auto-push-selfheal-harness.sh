...
case "$SCENARIO" in
  diverged|conflict|capped|fetch-first|unrelated-fetch-first)
...
# Trigger wording varies by scenario: git phrases the divergence reject as
# either "non-fast-forward" or a rejected-ref line ending in "(fetch first)".
# unrelated-fetch-first carries the phrase only in unrelated hook stderr and
# must NOT trigger a heal.
case "$SCENARIO" in
  fetch-first)
    PUSH_MSG=" ! [rejected]            main -> main (fetch first)
error: failed to push some refs to 'origin'" ;;
  unrelated-fetch-first)
    PUSH_MSG="remote: pre-receive hook declined: please fetch first thing tomorrow
error: failed to push some refs to 'origin'" ;;
  *)
    PUSH_MSG="push failed: ! [rejected] main -> main (non-fast-forward)" ;;
esac

set +e
auto_push_selfheal "$PUSH_MSG" "main" >"$WORK/heal.out" 2>&1
RC=$?
set -e
