...
test("home initially shows exactly the three newest release cards", () => {
  ...
  assert.deepEqual(
    initiallyVisibleIds,
    [
      "banner-whats-new-r128",
      "banner-whats-new-r127",
      "banner-whats-new-r126_sec",
    ],
  );

  const historicalMarkup = source.split('id="home-recent-historical-releases"')[1];
  assert.match(historicalMarkup, /banner-whats-new-r125_155_23_sec/);
  assert.match(historicalMarkup, /banner-whats-new-r125_155_19/);
});