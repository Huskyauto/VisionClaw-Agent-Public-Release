...
test("buildModelSwapReport: high similarity ⇒ NO_CHANGE; mid ⇒ CHANGES; none graded ⇒ INCONCLUSIVE (never a confident all-clear)", async () => {
  const { buildModelSwapReport } = await import("../../server/lib/sandbox/replay");
  const meta = { errored: 0, capStopped: 0, skippedNoContent: 0, totalCandidates: 5, stubbedToolCalls: 0, perRunCapUsd: 5 };
  const mk = (sims: Array<number | null>, errored = 0) => buildModelSwapReport("orchestration", { model: "m" }, 5,
    sims.map((s, i) => ({ itemRef: `x:${i}`, similarity: s, costUsd: 0, tokensIn: 0, tokensOut: 0, latencyMs: 0 })), { ...meta, errored });
  assert.equal(mk([0.95, 0.97]).verdict, "NO_CHANGE");
  assert.equal(mk([0.85, 0.88]).verdict, "CHANGES");
  // Absence of evidence must never read as an all-clear:
  const empty = mk([null, null]);
  assert.equal(empty.verdict, "INCONCLUSIVE", "all-null grades (embedding lane down) ⇒ INCONCLUSIVE, not NO_CHANGE");
  assert.equal(empty.similarity.mean, null);
  assert.equal(mk([]).verdict, "INCONCLUSIVE", "zero replays ⇒ INCONCLUSIVE");
  assert.equal(mk([0.95], 2).verdict, "INCONCLUSIVE", "errors outnumbering successes ⇒ INCONCLUSIVE");
  assert.equal(mk([0.5], 2).verdict, "DRIFT", "real drift evidence still wins over error noise");
});
