...
# The generated public mirror intentionally ships only its dependency-free
# release-integrity test. Its manifest is generated by the sanitizer after
# private-only tests are removed; validate it before selecting any test.
if [ "${PUBLIC_MIRROR_TESTS:-0}" = "1" ]; then
  npx tsx scripts/verify-public-test-manifest.ts
  # This is intentionally an explicit smoke contract, not a misleading
  # first-N selection from a category. The manifest classifies the full corpus;
  # only these mirror-safe tests are promised by `npm test` on a fresh fork.
  exec node --import tsx --test \
    tests/lib/public-test-classifier.test.ts \
    tests/release-facts-contract.test.ts \
...
FILES=(
  "tests/customer-revenue-launchpad-seed.test.ts"
  "tests/fable-5-seed.test.ts"
  "tests/revenue-idea-catalog.test.ts"
  "tests/revenue-idea-start-flow.test.ts"
  ...
  "tests/lib/project-compass-core.test.ts"
  ...
  "tests/lib/independent-review-brief-validator.test.ts"
  ...
  "tests/security/features-doc-content-verification.test.ts"
  "tests/security/features-doc-delivery-project-guard.test.ts"
  "tests/security/delivery-retry-reconciliation.test.ts"
  "tests/security/ahb-regression.test.ts"
  "tests/security/route-census.test.ts"
  "tests/security/tenant-audit-call-timeout.test.ts"
  "tests/security/video-job-conflict-tenant.test.ts"
  ...
  "tests/storage/project-compass-tenant-isolation.test.ts"
  ...
  "tests/lib/context-policy-eval-core.test.ts"
  ...
  "tests/unit/harness-manifest-core.test.ts"
  ...
  "tests/routes/frontier-concepts-routes.test.ts"
)
...
# storefront Buy button per-mount idempotency token and owner frontier
# concept evidence flows. These complement server-side tests by verifying
# the BROWSER actually emits the expected requests and opens the responses.
# Requires the dev
