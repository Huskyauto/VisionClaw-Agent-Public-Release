...
test("S4 seam: dispatcher surfaces the ALS signal on ToolContext", () => {
  ...
  // The dispatcher reads the ALS signal (via dynamic import of the abort-context
  // module) into a local, then places it on the ToolContext it builds.
  assert.ok(
    dispatcherSrc.includes("getCurrentToolAbortSignal()"),
    "dispatcher must read the ALS abort signal",
  );
  assert.match(
    dispatcherSrc,
    /abortSignal\s*=\s*mod\.getCurrentToolAbortSignal\(\)/,
    "ALS signal must be captured into the ToolContext abortSignal field",
  );
  assert.match(
    dispatcherSrc,
    /\babortSignal\s*,/,
    "captured abortSignal must be placed on the ToolContext object",
  );
});
