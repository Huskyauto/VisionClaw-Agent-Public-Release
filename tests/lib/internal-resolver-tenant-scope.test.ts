...
test("finalizeEvent supports and receives tenant scoping on the plan path", () => {
  assert.match(
    ...
    "finalizeEvent must require a tenantId",
  );
  assert.match(
    src,
...
test("finalizeEvent has no optional or unscoped tenant branch", () => {
  const fnBody = src.slice(src.indexOf("async function finalizeEvent"), src.indexOf("async function resolvePlanProposed"));
  assert.doesNotMatch(fnBody, /tenantId\s*[?!]=|sql``|tenantId\?/);
  assert.match(fnBody, /WHERE id = \$\{eventId\}[\s\S]*AND tenant_id = \$\{tenantId\}/);
});
