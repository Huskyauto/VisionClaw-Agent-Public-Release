...
describe("computeHarnessHealth (arXiv:2605.18747 process-quality)", () => {
  ...
  it("valve-fire rate = valveFires / stalls; zero stalls yields 0 (the favorite number)", () => {
    const none = computeHarnessHealth({ ...base, stalls: 0, valveFires: 0 });
    assert.equal(none.valveFireRate, 0);
    const some = computeHarnessHealth({ ...base, stalls: 4, valveFires: 1 });
    assert.equal(some.stalls, 4);
    assert.equal(some.valveFires, 1);
    assert.equal(some.valveFireRate, 0.25);
  });

  it("valveFires is clamped to stalls (a fire only happens ON a stall) and never breaches", () => {
    const r = computeHarnessHealth({ ...base, stalls: 2, valveFires: 9 });
    assert.equal(r.valveFires, 2);
    assert.equal(r.valveFireRate, 1);
    assert.equal(r.breached, false);
  });
