...
it("A/B target rollout recovers from a transient Profundo failure instead of skipping the case", async () => {
  resetProviderHealth("profundo");
  const routes: string[] = [];
  let resolutions = 0;
  const complete = (params: any, options: { tenantId?: number }) =>
    createBackgroundCompletion(params, {
      tenantId: options.tenantId,
      resolve: async (_modelId, tenantId) => {
        assert.equal(tenantId, 17);
        resolutions++;
        if (resolutions === 1) {
          return {
            route: "profundo",
            ...
            client: {
              chat: {
                completions: {
                  create: async () => {
                    routes.push("profundo");
                    throw Object.assign(new Error("temporary Profundo outage"), { status: 503 });
                  },
                },
              },
            } as any,
            ...
            chat: {
              completions: {
                create: async (request: any) => {
                  routes.push(`fallback:${request.model}`);
                  return { choices: [{ message: { content: "usable A/B output" } }] };
                },
              },
            },
          } as any,
  ...
  try {
    const output = await runEvaluatorTarget(
      "candidate skill",
      "evaluation input",
      { targetModel: "gpt-5.6-sol", tenantId: 17 },
      complete,
    );
    assert.equal(output, "usable A/B output");
    assert.deepEqual(routes, ["profundo", "fallback:gpt-5.4"]);
