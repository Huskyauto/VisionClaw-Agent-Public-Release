...
test("public mirror scrubs complete human-agency audit SKUs and prefix matchers", () => {
  const script = readFileSync("scripts/build-public-mirror.sh", "utf8");

  assert.match(script, /sample-agency-audit-starter\/sample-agency-audit-starter/);
  assert.match(script, /sample-agency-audit-full\/sample-agency-audit-full/);
  assert.match(script, /sample-agency-audit-\/sample-agency-audit-/);

  const starterIndex = script.indexOf("sample-agency-audit-starter");
  const fullIndex = script.indexOf("sample-agency-audit-full");
  const prefixIndex = script.indexOf("sample-agency-audit-/sample-agency-audit-");
  assert.ok(starterIndex < prefixIndex && fullIndex < prefixIndex, "specific SKU rewrites must run before the prefix rewrite");
});