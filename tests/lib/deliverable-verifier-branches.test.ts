...
test("Drive folder URLs cannot be accepted as customer delivery proof", async () => {
  stubDb({});
  const r = await verifyDeliverable({
    tenantId: 1,
    deliverableType: "mime-gated-test",
    fileUrl: "https://drive.google.com/drive/folders/private-folder-id",
    buffer: Buffer.from("%PDF-1.4 fake body"),
  });
  assert.equal(r.status, "failed");
  assert.equal(r.passed, false);
  assert.ok(
    r.failures.some((failure) => /drive folder/i.test(failure)),
    `expected a Drive-folder rejection, got: ${r.failures.join("; ")}`,
  );
});

test("unknown deliverable types FAIL CLOSED instead of being treated as benign skips", async () => {
  (db as any).select = () => ({ from: async () => [] });
  (db as any).execute = async () => ({ rows: [{ id: 4242 }] });
  invalidateContractCache();
  const result = await verifyDeliverable({
    tenantId: 1,
    deliverableType: "unknown-uncontracted-type",
    buffer: Buffer.from("content"),
  });
  assert.equal(result.status, "failed");
  assert.equal(result.passed, false);
  assert.match(result.failures.join("; "), /unknown deliverable_type/i);
});

test("buffer-only artifacts use their supplied filename for contract extension and MIME checks", async () => {
  stubDb({});
  const result = await verifyDeliverable({
    tenantId: 1,
    deliverableType: "mime-gated-test",
    fileName: "in-memory-report.pdf",
    buffer: Buffer.from("%PDF-1.4 in-memory content"),
  });
  assert.equal(result.status, "passed", result.failures.join("; "));
  assert.equal(result.detected.extension, ".pdf");
  assert.equal(result.detected.mime, "application/pdf");
});
