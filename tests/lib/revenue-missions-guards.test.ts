...
test("evidenceGateScore leaves scoring untouched with evidence, or when already ≤3; junk counts gate (fail closed)", () => {
  const s = mkScore(5, 5);
  const withEvidence = evidenceGateScore(s, 2);
  // Scoring fields preserved verbatim; only advisory evidence_confidence is attached.
  assert.deepEqual(withEvidence, { ...s, evidence_confidence: "Medium" });
  const low = mkScore(2, 3);
  assert.deepEqual(evidenceGateScore(low, 0), { ...low, evidence_confidence: "Low" });
...
test("evidence_confidence tiers: 0/junk=Low (gated), 1-2=Medium, 3+=High; persists scoring fields", () => {
  assert.equal(evidenceGateScore(mkScore(5, 5), 0).evidence_confidence, "Low");
  assert.equal(evidenceGateScore(mkScore(5, 5), 1).evidence_confidence, "Medium");
  assert.equal(evidenceGateScore(mkScore(5, 5), 2).evidence_confidence, "Medium");
  assert.equal(evidenceGateScore(mkScore(5, 5), 3).evidence_confidence, "High");
  assert.equal(evidenceGateScore(mkScore(5, 5), 10).evidence_confidence, "High");
  for (const junk of [null, undefined, "x", -1, NaN, 1.5]) {
    assert.equal(evidenceGateScore(mkScore(5, 5), junk).evidence_confidence, "Low", `junk ${String(junk)}`);
  }
});
...
// Fixture matches the SCORER's real write shape: metadata.priority.* (nested),
// never top-level — see server/lib/idea-priority.ts (2026-08-02 mismatch fix).
const goodPriority = { tier: "S", composite: 87, buyer_hypothesis: "Local SMB owners will pay $497 for an audit", rationale: "High urgency, low competition" };
const goodProject = {
  id: 42, name: "AI Trust Audit", description: "Audit local SMB AI readiness",
  metadata: { priority: goodPriority },
};
...
test("buildMissionPacket: non-S/A tier returns null (fail toward not proposing)", () => {
  for (const tier of ["B", "C", "", undefined]) {
    assert.equal(buildMissionPacket({ ...goodProject, metadata: { priority: { ...goodPriority, tier } } }), null, `tier=${String(tier)}`);
  }
});

test("buildMissionPacket: thin evidence returns null", () => {
  assert.equal(buildMissionPacket({ ...goodProject, metadata: { priority: { ...goodPriority, buyer_hypothesis: "  " } } }), null);
  assert.equal(buildMissionPacket({ ...goodProject, metadata: { priority: { ...goodPriority, rationale: undefined } } }), null);
  assert.equal(buildMissionPacket({ ...goodProject, name: "" }), null);
  assert.equal(buildMissionPacket({ ...goodProject, metadata: null }), null);
  // TOP-LEVEL shape (the old bug) must be rejected — scorer writes nested only.
  assert.equal(buildMissionPacket({ ...goodProject, metadata: { ...goodPriority } }), null);
});

// ── idea-priority accessor: strict composite validation (no JS coercion) ────
import { readIdeaPriority } from "../../server/lib/idea-priority";

test("readIdeaPriority: accepts nested scorer shape (number + digit-string composite)", () => {
  assert.deepEqual(readIdeaPriority({ priority: goodPriority }), {
    tier: "S", composite: 87, rationale: goodPriority.rationale, buyerHypothesis: goodPriority.buyer_hypothesis,
  });
  assert.equal(readIdeaPriority({ priority: { ...goodPriority, composite: "87" } })!.composite, 87);
});

test("readIdeaPriority: rejects malformed composites without numeric coercion", () => {
  for (const composite of [null, undefined, "", "  ", false, true, "12.5", 12.5, NaN, Infinity, "1e3", " 87", {}, []]) {
    assert.equal(readIdeaPriority({ priority: { ...goodPriority, composite } }), null, `composite=${String(composite)}`);
  }
  assert.equal(readIdeaPriority({ priority: { ...goodPriority, tier: "X" } }), null);
  assert.equal(readIdeaPriority({ ...goodPriority }), null); // top-level shape
  assert.equal(readIdeaPriority(null), null);
  assert.equal(readIdeaPriority("nope"), null);
});

test("buildMissionPacket: no $ figure leaves priceUsd unset", () => {
  const p = buildMissionPacket({ ...goodProject, metadata: { priority: { ...goodPriority, buyer_hypothesis: "Owners will pay for an audit" } } });
  assert.ok(p);
