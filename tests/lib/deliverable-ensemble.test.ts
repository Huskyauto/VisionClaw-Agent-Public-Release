...
const baseArgs = {
  ...
  label: "test",
  _validationFn: async () => ({
    version: "jev-work-validation-v1" as const,
    advisoryOnly: true as const,
    subjectKind: "deliverable" as const,
    status: "disabled" as const,
    note: "disabled in deterministic ensemble tests",
  }),
};
...
test("successful ensemble output automatically carries a report-only TypeSafe validation receipt", async () => {
  const baseline = "validated baseline ".repeat(20);
  let validationCalls = 0;
  const result = await draftWithEnsemble({
    ...baseArgs,
    _completionFn: async (model) => model === "gpt-5.6-sol" ? baseline : LONG,
    _validationFn: async (input) => {
      validationCalls++;
      assert.equal(input.subjectKind, "deliverable");
      assert.equal(input.output, baseline);
      return {
        version: "jev-work-validation-v1",
        advisoryOnly: true,
  ...
  assert.equal(result?.typesafeValidation?.status, "validated");
  assert.equal(result?.typesafeValidation?.advisoryOnly, true);
});
...
test("explicit low-risk canary may select a reviewed candidate while retaining the ensemble contract", async () => {
  ...
  const fn: CompletionFn = async (model) => {
    ...
    if (model === "deepseek/deepseek-v4.1-flash") return firstDraft;
    return LONG;
  };
  const result = await draftWithEnsemble({
    ...baseArgs,
...
test("override config: unknown drafters/aggregator rejected against allowlists", () => {
  const cfg = _parseEnsembleConfigJsonForTest(JSON.stringify({
    drafters: ["evil/expensive-model", "deepseek/deepseek-v4.1-flash", "z-ai/glm-5.2"],
    aggregator: "claude-fable-5",
  }));
  assert.deepEqual(cfg.drafters, ["deepseek/deepseek-v4.1-flash", "z-ai/glm-5.2"]);
...
test("override config: fewer than 2 allowlisted drafters keeps defaults", () => {
  const cfg = _parseEnsembleConfigJsonForTest(JSON.stringify({ drafters: ["evil/one", "deepseek/deepseek-v4.1-flash"] }));
