...
test("wiring: repurpose_content tool def declares the 6-platform enum", () => {
  // Tools-layer-split S26f: the def migrated to the content-ops domain.
  const t = read("server/tools/domains/content-ops/definitions.ts");
...
test("wiring: content-ops domain registers the repurpose_content handler", () => {
  // Tools-layer-split S26f: the dispatcher case migrated from server/tools.ts
  // to the content-ops domain (registered via the domain registry at import time).
  const src = read("server/tools/domains/content-ops/handlers.ts");
  assert.match(src, /async function repurposeContentHandler/);
  assert.match(src, /import\("\.\.\/\.\.\/\.\.\/lib\/content-repurposer"\)/);
  const idx = read("server/tools/domains/content-ops/index.ts");
  assert.match(idx, /registerTools\(contentOpsDomainTools\)/);
});
