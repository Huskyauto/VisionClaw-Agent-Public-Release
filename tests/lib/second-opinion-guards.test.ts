...
const {
  ...
  getSecondOpinion,
  selectSecondOpinionFallback,
  resolveSecondOpinionRoute,
  settlementUpdatedExactlyOne,
  buildExplicitFallbackOffer,
} = mod as any;
...
test("a tripped drift latch blocks the on-demand spend path too (query-free early return)", async () => {
  ...
  assert.equal(blocked.failureKind, "cost_drift_latched", "chat recovery must recognize the latch as terminal");
  __resetFusionAutoLatch();
});

test("single-model fallback is allowed only when Fusion is known not to have spent", () => {
  assert.deepEqual(
    selectSecondOpinionFallback({ failureKind: "wiring", providerWorkMayHaveStarted: false }),
    { modelId: "z-ai/glm-5.2", reason: "fusion-pre-dispatch-failure" },
  );
  assert.equal(
    selectSecondOpinionFallback({ failureKind: "model_timeout", providerWorkMayHaveStarted: true }),
    null,
    "ambiguous Fusion timeout must not silently trigger a second paid call",
  );
  assert.equal(
    selectSecondOpinionFallback({ failureKind: "provider_error", providerWorkMayHaveStarted: true }),
    null,
    "post-dispatch provider errors are also ambiguous completion",
  );
  assert.equal(
    selectSecondOpinionFallback({ failureKind: "fusion_daily_usd", providerWorkMayHaveStarted: false }),
    null,
    "fallback must not bypass the dedicated daily budget",
  );
});

test("explicit fallback mode is server-pinned to the strongest reviewed OpenRouter candidate", () => {
  assert.deepEqual(resolveSecondOpinionRoute("fusion"), {
    modelId: "openrouter/fusion",
    routeReason: "fusion-direct",
    label: "Fusion",
  });
  assert.deepEqual(resolveSecondOpinionRoute("single_model_fallback"), {
...
test("reservation settlement succeeds only when exactly one ledger row was updated", () => {
  assert.equal(settlementUpdatedExactlyOne({ rows: [{ id: 42 }], rowCount: 1 }), true);
  assert.equal(settlementUpdatedExactlyOne({ rows: [], rowCount: 0 }), false);
  assert.equal(settlementUpdatedExactlyOne({ rows: [{ id: 1 }, { id: 2 }], rowCount: 2 }), false);
  assert.equal(settlementUpdatedExactlyOne(undefined), false);
});

test("fallback reservations carry the routed model identity into the ledger writer", async () => {
  let captured: any;
  const reserve = async (args: any) => {
    captured = args;
    return { ok: true, reservationId: 77, spentUsd: 0 };
  };
  ...
  assert.equal(captured.modelId, "z-ai/glm-5.2");
});

test("fallback offer is explicit authorization metadata, never an automatic retry signal", () => {
  assert.deepEqual(buildExplicitFallbackOffer("fusion"), {
    explicitFallbackAvailable: true,
    fallbackModel: "z-ai/glm-5.2",
    fallbackRequiresExplicitAuthorization: true,
  });
  assert.deepEqual(buildExplicitFallbackOffer("single_model_fallback"), {
    explicitFallbackAvailable: false,
  });
});

test("a reservation grant without a valid ledger row id fails closed", async () => {
  const malformedReserve = async () => ({ ok: true, spentUsd: 0 });
  const result = await reserveFusionBudget(
    1,
    1,
    "second_opinion",
    "second_opinion",
    { reserve: malformedReserve },
  );
