...
describe("shrink guard (ACE brevity-bias defense)", () => {
  const OPTS = { fraction: 0.4, strongEps: 0.05 };

  it("passes candidates that have not shrunk past the floor", () => {
    assert.equal(violatesShrinkGuard(1000, 700, 0.51, 0.5, OPTS), false);
    assert.equal(violatesShrinkGuard(1000, 600, 0.51, 0.5, OPTS), false); // exactly at floor
  });
  it("rejects a big shrink with only a marginal gain", () => {
    assert.equal(violatesShrinkGuard(1000, 400, 0.51, 0.5, OPTS), true);
  });
  it("lets a big shrink through when the gain is strong", () => {
    assert.equal(violatesShrinkGuard(1000, 400, 0.56, 0.5, OPTS), false);
    assert.equal(violatesShrinkGuard(1000, 400, 0.55, 0.5, OPTS), true); // must EXCEED strongEps
  });
  it("fails open on a degenerate seed length", () => {
    assert.equal(violatesShrinkGuard(0, 0, 0.51, 0.5, OPTS), false);
    assert.equal(violatesShrinkGuard(NaN, 400, 0.51, 0.5, OPTS), false);
  });
  it("clamps a hostile fraction instead of disabling the guard", () => {
    // fraction 5 would make the floor negative (guard dead); clamp keeps it alive.
    assert.equal(violatesShrinkGuard(1000, 5, 0.51, 0.5, { fraction: 5, strongEps: 0.05 }), true);
  });

  const cases: EvalCase[] = Array.from({ length: 8 }, (_, i) => ({ input: `q${i}` }));
  const longDoc = "Answer the question carefully. " + "Detail line that matters.\n".repeat(40);

  it("loop rejects an otherwise-accepted edit that collapses the doc (reason shrink-guard)", async () => {
    // Deleting nearly everything IMPROVES the score slightly — the classic
    // brevity-bias trap. The guard must reject it.
    const gut = longDoc.slice(longDoc.indexOf("Detail"));
    const r = await optimizeSkill(longDoc, cases, {
      epochs: 2,
      valSplit: 0.5,
      seed: 3,
      rolloutFn: async (doc: string, c: EvalCase): Promise<ScoredRollout> => ({
        input: c.input,
        output: doc,
        score: doc.length < longDoc.length / 2 ? 0.52 : 0.5,
      }),
      proposeFn: async (): Promise<SkillEdit | null> => ({ op: "delete", target: gut }),
    });
    assert.equal(r.improved, false);
    assert.equal(r.bestSkill, longDoc, "collapsed doc must not become best");
    assert.ok(r.epochs.some((e) => !e.accepted && e.reason === "shrink-guard"));
  });

  it("loop accepts the same collapse when the gain is strong", async () => {
    const gut = longDoc.slice(longDoc.indexOf("Detail"));
    const r = await optimizeSkill(longDoc, cases, {
      epochs: 2,
      valSplit: 0.5,
      seed: 3,
      shrinkStrongEps: 0.05,
      rolloutFn: async (doc: string, c: EvalCase): Promise<ScoredRollout> => ({
        input: c.input,
        output: doc,
        score: doc.length < longDoc.length / 2 ? 0.7 : 0.5,
      }),
      proposeFn: async (): Promise<SkillEdit | null> => ({ op: "delete", target: gut }),
    });
    assert.equal(r.improved, true);
    assert.ok(r.epochs.some((e) => e.accepted && e.reason === "strict-improvement"));
  });

  it("crossover child that collapses the doc is rejected (crossover-shrink-guard)", async () => {
    const seed = "Answer the question. " + "Important detail line.\n".repeat(40);
    const r = await optimizeSkill(seed, cases, {
      epochs: 2,
      valSplit: 0.5,
      seed: 3,
      crossover: true,
      crossoverFn: async () => "SHORT", // tiny merged child, marginally better score
      rolloutFn: async (doc: string, c: EvalCase): Promise<ScoredRollout> => ({
        input: c.input,
        output: doc,
        score:
          doc === "SHORT" ? 0.82
          : doc.includes("concise") ? 0.8
          : doc.includes("polite") ? 0.6
          : 0.5,
      }),
      proposeFn: (() => {
        let call = 0;
        return async (): Promise<SkillEdit | null> =>
          ++call === 1 ? { op: "add", text: "Be concise." } : { op: "add", text: "Be polite." };
      })(),
    });
    assert.equal(r.crossoverApplied, false);
    assert.ok(r.bestSkill.includes("concise"), "best must stay the accepted full doc");
    assert.ok(r.epochs.some((e) => !e.accepted && e.reason === "crossover-shrink-guard"));
  });
});
