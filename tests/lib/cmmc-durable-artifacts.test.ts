...
test("CMMC delivery retries return to durable paired-artifact recovery", () => {
  ...
  const recovery = fs.readFileSync(path.join(process.cwd(), "server/delivery-artifact-recovery.ts"), "utf8");
  const report = fs.readFileSync(path.join(process.cwd(), "server/cmmc-report.ts"), "utf8");

  assert.match(report, /metadata: \{ cmmcReportId: report\.id \}/);
  assert.match(recovery, /const CMMC_DELIVERY_IDEMPOTENCY_KEY = \/\^cmmc-delivery-/);
  assert.match(recovery, /export async function retryCmmcDeliveryIfApplicable/);
...
test("SAM prospect reports persist and verify durable bytes before Drive registration", () => {
  const artifacts = fs.readFileSync(
    path.join(process.cwd(), "server/cmmc-prospect-artifacts.ts"),
    "utf8",
  );
  const handler = fs.readFileSync(
    path.join(process.cwd(), "server/tools/domains/cmmc-prospecting/handlers.ts"),
    "utf8",
  );

  assert.match(artifacts, /const databaseFallback = params\.bytes\.toString\("base64"\)/);
  assert.match(artifacts, /await withArtifactDeadline\([\s\S]*?uploadTenantFile/);
  assert.match(artifacts, /await withArtifactDeadline\([\s\S]*?downloadTenantFile/);
  assert.match(artifacts, /await loadCmmcProspectArtifact\(\{/);

  const persistAt = handler.indexOf("await persistCmmcProspectArtifact");
  const driveAt = handler.indexOf("await uploadAndShare");
  assert.ok(persistAt >= 0, "SAM delivery must persist bytes before upload");
  assert.ok(driveAt > persistAt, "Drive upload must occur only after durable read-back");
  assert.match(handler, /\$\{durable\.filename\}, \$\{viewUrl\}/);
});