...
const SCHEMA = fs.readFileSync(path.resolve(__dirname, "../../shared/models/operational-learning.ts"), "utf8");
...
// Tools-layer-split S25k: the sprint-contract definitions + handlers migrated
// from server/tools.ts to server/tools/domains/sprint-contracts/. These scans
// now target the domain files (the facade splices the same const refs back).
const SC_DEFS = fs.readFileSync(path.resolve(__dirname, "../../server/tools/domains/sprint-contracts/definitions.ts"), "utf8");
const SC_HANDLERS = fs.readFileSync(path.resolve(__dirname, "../../server/tools/domains/sprint-contracts/handlers.ts"), "utf8");

test("domain defs: all 3 sprint-contract tools declared with required fields", () => {
  for (const t of ["pin_done_condition", "get_done_condition", "evaluate_against_contract"]) {
    assert.match(SC_DEFS, new RegExp(`name:\\s*"${t}"`));
  }
  // pin_done_condition required fields
  const pinBlock = SC_DEFS.split(/name:\s*"pin_done_condition"/)[1].split(/name:\s*"/)[0];
  assert.match(pinBlock, /required:\s*\["refKind",\s*"refId",\s*"doneCondition"\]/);
});

test("domain handlers: every sprint-contract handler gates on tenant (tenant isolation)", () => {
  for (const t of ["pin_done_condition", "get_done_condition", "evaluate_against_contract"]) {
    const handlerBlock = SC_HANDLERS.split(`Tenant context required for ${t}`);
    assert.equal(handlerBlock.length, 2, `${t} must error on missing tenant`);
    // The migrated handlers read the trusted ctx.tenantId, not params._tenantId.
    assert.ok(
      handlerBlock[0].includes("ctx.tenantId"),
      `${t} must check ctx.tenantId before the tenant-required error`,
    );
  }
});

test("domain handlers: every sprint-contract handler wraps dynamic import INSIDE try (envelope safety)", () => {
  // Split the handlers file into per-handler blocks by the async fn declarations.
  const blocks = SC_HANDLERS.split(/async function \w+Handler/).slice(1, 4);
  assert.equal(blocks.length, 3, "expected 3 sprint-contract handler blocks");
  for (const [i, block] of blocks.entries()) {
    const tryIdx = block.indexOf("try {");
    const importIdx = block.indexOf('await import("../../../lib/sprint-contract")');
    const catchIdx = block.indexOf("} catch");
    assert.ok(tryIdx >= 0, `handler ${i}: missing try block`);
    assert.ok(importIdx > tryIdx, `handler ${i}: dynamic import must be inside try`);
    assert.ok(catchIdx > importIdx, `handler ${i}: catch must follow import`);
  }
});
