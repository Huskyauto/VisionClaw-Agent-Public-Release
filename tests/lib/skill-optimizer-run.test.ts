...
test("normalizeRunConfig applies defaults", () => {
  ...
  assert.equal(c.optimizerModel, "gpt-5.6-sol");
...
test("retained artifacts classify deterministically without authorizing promotion", () => {
  assert.deepEqual(
    classifyRetainedOptimizerArtifact({ improved: false, dbSkillId: 4 }, true),
    { state: "rejected", reason: "held-out evaluation did not strictly improve" },
  );
  assert.equal(classifyRetainedOptimizerArtifact({ improved: true, dbSkillId: null }, true).state, "rejected");
  assert.equal(classifyRetainedOptimizerArtifact({ improved: true, dbSkillId: 4 }, false).state, "failed");
  assert.equal(classifyRetainedOptimizerArtifact({ improved: true, dbSkillId: 4 }, true).state, "held");
  assert.equal(classifyRetainedOptimizerArtifact({}, true).state, "failed");
});
...
test("mapVerdictToAction applies ONLY on a 2-of-3 FIX majority", () => {
  ...
  assert.equal(mapVerdictToAction({ verdict: "FIX", majority: 3 }), "hold");
});
...
test("mapVerdictToAction holds ACCEPT but terminally rejects REJECT", () => {
  assert.equal(mapVerdictToAction({ verdict: "ACCEPT", majority: 2 }), "hold");
  assert.equal(mapVerdictToAction({ verdict: "REJECT", majority: 3 }), "reject");
});
