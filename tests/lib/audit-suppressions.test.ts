...
test("completed-audit suppressions retain exact immutable snapshot evidence", () => {
  const snapshot = JSON.parse(
    fs.readFileSync("data/tenant-isolation-audit/campaign-2026-09-03-snapshot.json", "utf8"),
  ).findings;
  const entries = loadSuppressions(
    "data/tenant-isolation-audit/suppressions.json",
    new Set(["fk-transitive", "internal-worker-pk", "global-table", "signed-url", "upstream-verified", "system-sweep"]),
  );
  const verified = entries.filter((e) => e.verifiedBy === "independent completed-audit path review 2026-09-03");
  assert.equal(verified.length, 4);
  assert.equal(snapshot.length, 4);
  for (const entry of verified) {
    const finding = snapshot.find((f: { file: string; issue: string }) =>
      f.file === entry.file && f.issue.includes(entry.match),
    );
    assert.ok(finding, `no immutable completed-audit evidence for ${entry.file} :: ${entry.match}`);
    assert.equal(extractStructuralAnchor(finding.issue), entry.anchor);
    assert.ok(matchSuppression(finding, [entry]), `entry did not match ${entry.file}:${finding.line}`);
  }
});
