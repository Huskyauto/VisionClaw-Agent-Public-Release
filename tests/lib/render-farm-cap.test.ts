...
test("BWB render backends: orchestrated Drive delivery is independent from approval email", () => {
  for (const file of ["scripts/bwb-render-github.ts", "scripts/build-bwb-video.ts"]) {
    const src = read(file);
    assert.match(
      src,
      /deliverDigitalProduct\(\{[^]*sendEmail:\s*!process\.env\.BWB_ORCHESTRATOR_RUN_ID[^]*\}\)/,
      `${file} must defer email to the orchestrator while preserving standalone delivery email`,
    );
    assert.match(src, /emailSubject:/, `${file} must preserve its customized standalone subject`);
    assert.match(src, /emailBody:/, `${file} must preserve its customized standalone body`);
  }
});
