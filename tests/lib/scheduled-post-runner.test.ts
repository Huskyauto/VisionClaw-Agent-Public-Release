...
test("tools.ts: registers all 3 scheduler tools with descriptions and required fields", () => {
  ...
  // schedule_cross_platform_post stays legacy-inline in tools.ts (destructive
  // producer, requiresApproval, reads _personaName/_userId) — deferred to a
  // trust-seam slice, so its def + switch case still live in the facade.
  assert.match(t, /name:\s*"schedule_cross_platform_post"/, "tool def missing: schedule_cross_platform_post");
