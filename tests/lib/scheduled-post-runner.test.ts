...
test("heartbeat tick: imports runDueScheduledPosts and calls it inside try/catch", () => {
  const hb = read("server/heartbeat-schedules.ts");
...
test("tools.ts: registers all 3 scheduler tools with descriptions and required fields", () => {
  ...
  // The destructive producer keeps its trust-sensitive dispatch case in the
  // facade, while its inert JSON definition lives in the scheduling domain.
  const producerDef = read("server/tools/domains/cross-platform-scheduling/definitions.ts");
  assert.match(producerDef, /name:\s*"schedule_cross_platform_post"/, "tool def missing: schedule_cross_platform_post");
  assert.match(t, /case\s+"schedule_cross_platform_post"/, "dispatch case missing: schedule_cross_platform_post");
  // cancel_scheduled_post + list_scheduled_posts migrated to the scheduled-posts
  // domain (tools-layer-split S25b): defs now live in the domain file and
  // dispatch is registry-routed via the dispatcher (no more tools.ts arm).
  const defs = read("server/tools/domains/scheduled-posts/definitions.ts");
  for (const name of ["cancel_scheduled_post", "list_scheduled_posts"]) {
    assert.match(defs, new RegExp(`name:\\s*"${name}"`), `migrated tool def missing: ${name}`);
  }
});

test("tools.ts: every dispatch case enforces tenant context before dispatch", () => {
  const t = read("server/tools.ts");
  // Legacy producer still gates on _tenantId in its facade switch arm.
  assert.match(
    t,
    /case\s+"schedule_cross_platform_post"[\s\S]{0,600}_tenantId/,
    "schedule_cross_platform_post dispatch must check _tenantId",
  );
  // Migrated tools enforce tenant via the trusted ctx.tenantId gate in the
  // domain handler (the dispatcher strips + re-stamps _tenantId → ctx.tenantId).
  const h = read("server/tools/domains/scheduled-posts/handlers.ts");
  for (const name of ["cancel_scheduled_post", "list_scheduled_posts"]) {
    assert.match(
      h,
      new RegExp(`!ctx\\.tenantId[\\s\\S]{0,80}Tenant context required for ${name}`),
      `${name} handler must gate on ctx.tenantId`,
    );
  }
});
...
test("Round B allowlist: tool JSON-schema enum matches the 7 wired platforms", () => {
  const t = read("server/tools/domains/cross-platform-scheduling/definitions.ts");
...
test("publishToYouTube: dispatches resumable upload (init POST → PUT bytes) and defaults to privacy=private", () => {
  ...
  // Privacy is allowlisted and falls back to "private" for anything unrecognized.
  assert.match(p, /\["public",\s*"unlisted",\s*"private"\]\.includes\(String\(privacyStatus\)\)/);
  assert.match(p, /:\s*"private";/);
  assert.match(p, /privacyStatus:\s*safePrivacy/);
});
