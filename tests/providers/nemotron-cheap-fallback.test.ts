...
test("tier routing uses free Replit before Nemotron, then skips quarantined OpenRouter", async () => {
  ...
  try {
    ...
    assert.strictEqual(await getModelForTierAsync("reasoning"), "gpt-5.4");

    for (let attempt = 0; attempt < 3; attempt++) {
      markProviderUnhealthy("replit", "test free-lane outage");
    }
    assert.strictEqual(
    ...
    assert.strictEqual(
      await getModelForTierAsync("reasoning", 1, { allowMeteredFallback: true }),
      NEMOTRON,
    );
    ...
    await assert.rejects(
      getModelForTierAsync("reasoning", 1, { allowMeteredFallback: true }),
      /no healthy authorized model lane/,
    );
    await assert.rejects(
