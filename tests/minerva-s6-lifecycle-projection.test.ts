...
test("recovery blocked and ordinary failed repairs remain blocked with bounded reasons", () => {
  ...
  assert.equal(recovery.nextAction, "Review the blocker and resolve it");
...
test("activity query preserves tenant scope and puts unresolved originals first", () => {
  ...
  assert.match(activity, /CASE WHEN status IN \('approved', 'executing', 'failed', 'handoff_pending', 'publish_required'\) THEN 0 ELSE 1 END/);
