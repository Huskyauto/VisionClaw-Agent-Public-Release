...
# .npmrc carries legacy-peer-deps=true (claude-agent-sdk peer-requires zod^4
# while the platform is on zod 3.25.x — resolver-only conflict). Without it,
# npm ci inside the image fails ERESOLVE even though the lockfile is valid.
...
# Railway volume mounts replace image-time ownership with a root-owned mount.
# Start as root only long enough to hand the configured upload root to the
# fixed runtime UID, then exec the requested command as the non-root user.
COPY --from=build /app/scripts/docker-entrypoint.sh /usr/local/bin/visionclaw-entrypoint
RUN chmod 0755 /usr/local/bin/visionclaw-entrypoint
ENTRYPOINT ["/usr/local/bin/visionclaw-entrypoint"]
