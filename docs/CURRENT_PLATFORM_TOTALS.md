...
**Last verified:** 2026-06-26 (manual — `psql $DATABASE_URL` live counts + source-tree greps)
...
| **Database tables (declared)** | **172** | `rg -c "pgTable(" shared/schema.ts` |
| **Database tables (live in `public` schema)** | **211** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Capabilities (active)** | **128** | `SELECT count(*) FROM capabilities` |
| **Production indexes (all)** | **620** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public'` |
...
- **172 declared** — auditable, version-controlled,
...
- **211 live** — includes the externally-managed Stripe
