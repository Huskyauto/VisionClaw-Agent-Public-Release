...
**Last verified:** 2026-07-22 (auto-generated by `scripts/refresh-totals.ts`)
...
| **Built-in tools** | **399** | `rg "registerTool(...)" server/ \| sort -u \| wc -l` |
...
| **Database tables (live in `public` schema)** | **218** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **424** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
...
- **218 live** — includes the four externally-managed Stripe
