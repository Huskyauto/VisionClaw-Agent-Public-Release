...
**Last verified:** 2026-05-06 (auto-generated by `scripts/refresh-totals.ts`)
...
| **Built-in tools** | **298** | `rg "registerTool(...)" server/ \| sort -u \| wc -l` |
...
| **Database tables (declared)** | **119** | `rg -c "= pgTable(" shared/schema.ts` |
| **Database tables (live in `public` schema)** | **154** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **40** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **280** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
