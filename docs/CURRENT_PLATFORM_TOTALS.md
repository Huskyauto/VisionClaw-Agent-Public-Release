...
| **Built-in tools** | **404** | `rg "registerTool(...)" server/ \| sort -u \| wc -l` |
...
| **Database tables (live in `public` schema)** | **221** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **431** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
...
- **221 live** — includes the four externally-managed Stripe
