...
**Last verified:** 2026-07-21 (auto-generated by `scripts/refresh-totals.ts`)
...
| **Database tables (declared)** | **170** | `rg -c "= pgTable(" shared/schema.ts` |
| **Database tables (live in `public` schema)** | **216** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **420** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
