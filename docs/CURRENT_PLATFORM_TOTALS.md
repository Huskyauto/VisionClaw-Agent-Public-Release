...
**Last verified:** 2026-07-15 (manual — `psql $DATABASE_URL` live counts + source-tree greps)
...
tool total (the authoritative 396 comes from the 3-source reconciliation, not the
...
| **Built-in tools** | **396** | Verified across 3 runtime sources (registry + dispatcher + smoke-test manifest; the raw `registerTool(` grep overcounts) |
...
| **Database tables (live in `public` schema)** | **213** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Capabilities (active)** | **129** | `SELECT count(*) FROM capabilities` |
| **Production indexes (all)** | **627** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public'` |
| **Production indexes (non-PK)** | **413** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
...
- `server/tools/domains/` — 70+ domain modules (crm, finance, media, legal,
  ...
  executors (~295/396 tools migrated so far).
...
The tool COUNT is unchanged by the split — 396 total regardless of which file a
tool lives in.
...
- **213 live** — includes the four externally-managed Stripe
