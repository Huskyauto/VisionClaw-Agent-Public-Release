...
**Last verified:** 2026-07-23 (auto-generated by `scripts/refresh-totals.ts`)
...
| **Built-in tools** | **407** | `rg "registerTool(...)" server/ \| sort -u \| wc -l` |
...
| **Database tables (live in `public` schema)** | **222** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **433** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
...
- **222 live** — includes the four externally-managed Stripe
