...
**Last verified:** 2026-08-10 (auto-generated by `scripts/refresh-totals.ts`)
...
| **Database tables (live in `public` schema)** | **227** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **446** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
...
- **227 live** — includes the four externally-managed Stripe
