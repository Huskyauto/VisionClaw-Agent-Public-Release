...
**Last verified:** 2026-07-24 (auto-generated by `scripts/refresh-totals.ts`)
...
| **Database tables (live in `public` schema)** | **223** | `SELECT count(*) FROM information_schema.tables WHERE table_schema='public'` |
| **Governance rules** | **41** | `SELECT count(*) FROM governance_rules` |
| **Production indexes (non-PK)** | **435** | `SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname NOT LIKE '%_pkey'` |
...
- **223 live** — includes the four externally-managed Stripe
