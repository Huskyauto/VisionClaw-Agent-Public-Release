...
\restrict lISrcXhBVaHow9SO76VUp56uoC0cF7niLcDppPRmoMkYmiOoJ6GDxLLycbMqQOs
...
CREATE TABLE public.plans (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT plans_status_check CHECK ((status = ANY (ARRAY['awaiting_approval'::text, 'approved'::text, 'rejected'::text, 'revising'::text, 'revised'::text, 'executing'::text, 'completed'::text, 'failed'::text, 'expired'::text])))
);
...
\unrestrict lISrcXhBVaHow9SO76VUp56uoC0cF7niLcDppPRmoMkYmiOoJ6GDxLLycbMqQOs
