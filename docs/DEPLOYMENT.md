...
The primary VisionClaw application is a single-port Express + Vite app. It runs
anywhere Node 22.12+ runs, but we maintain three first-class targets:

1. **Replit** (current production — `agenticcorporation.net`)
2. **Docker** (multi-stage, non-root, ~slim runtime)
3. **Plain VPS / bare metal** (primary app: Node 22.12+ + Postgres 14+)

---

## Primary application prerequisites (all targets)
