...
export const commerceProducts = pgTable("commerce_products", {
  ...
  // Composite (tenant_id, sku): SKU namespaces are per-tenant — a global
  // unique SKU would let one tenant block another's catalog naming.
  skuUnique: uniqueIndex("idx_commerce_products_sku").on(t.tenantId, t.sku),
