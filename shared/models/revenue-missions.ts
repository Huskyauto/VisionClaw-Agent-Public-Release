...
import { pgTable, serial, integer, text, jsonb, timestamp, index, boolean, unique } from "drizzle-orm/pg-core";
...
export const revenueMissions = pgTable("revenue_missions", {
  ...
  // Post-mission scored retrospective (deterministic, evidence-derived; written
  // once when the mission reaches a terminal stage — killed or scale_ready).
  retrospective: jsonb("retrospective"),
  retrospectiveAt: timestamp("retrospective_at"),
  // Pre-launch validation gate (deterministic, $0 — see server/lib/mission-validation.ts).
  // assertMissionValidated fails CLOSED unless validationScore >= threshold.
  validation: jsonb("validation"),
  validationScore: integer("validation_score"),
  validationAt: timestamp("validation_at"),
  // Capital-pool settlement CAS marker — set exactly once when realized P&L is
  // applied to agent_capital at a terminal stage (killed | scale_ready).
  capitalSettledAt: timestamp("capital_settled_at"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantIdUnique: unique("revenue_missions_tenant_id_unique").on(t.tenantId, t.id),
...
// ============================================================
// AGENT CAPITAL — per-tenant capital pool settled from realized mission P&L
// (server/lib/agent-capital.ts). One row per tenant (UNIQUE tenant_id).
// Unseeded pool ⇒ riskCeilingForPool returns null ⇒ existing hard-cap regime
// applies unchanged; seeding is owner-only.
// ============================================================
export const agentCapital = pgTable("agent_capital", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().unique("agent_capital_tenant_unique"),
  balanceUsdCents: integer("balance_usd_cents").notNull().default(0),
  totalEarnedUsdCents: integer("total_earned_usd_cents").notNull().default(0),
  totalSpentUsdCents: integer("total_spent_usd_cents").notNull().default(0),
  seededAt: timestamp("seeded_at"),
  seededBy: text("seeded_by"),
  updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantIdx: index("idx_agent_capital_tenant").on(t.tenantId),
}));
export const insertAgentCapitalSchema = createInsertSchema(agentCapital).omit({ id: true, updatedAt: true });
export type AgentCapital = typeof agentCapital.$inferSelect;
export type InsertAgentCapital = z.infer<typeof insertAgentCapitalSchema>;
