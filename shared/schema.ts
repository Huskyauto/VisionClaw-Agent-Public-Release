...
import { revenueMissions } from "./models/revenue-missions";

export * from "./models/auth";
export * from "./models/research";
export * from "./models/self-repair";
export * from "./models/loop-portfolio";
...
export const agentRuns = pgTable("agent_runs", {
  ...
  // Monotonic durable-boundary counter for report-only restart evidence.
  // It is intentionally separate from steps because state updates can occur
  // between (or without) logged steps.
  recoveryEventSequence: integer("recovery_event_sequence").notNull().default(0),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  completedAt: timestamp("completed_at"),
}, (t) => ({
  tenantIdUnique: unique("agent_runs_tenant_id_unique").on(t.tenantId, t.id),
}));

export const revenueOrchestratorMissionLinks = pgTable("revenue_orchestrator_mission_links", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  apiKeyId: integer("api_key_id").notNull(),
  sourceRunId: integer("source_run_id").notNull(),
  missionId: integer("mission_id").notNull(),
  inputHash: text("input_hash").notNull(),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantKeyRunUnique: unique("revenue_orchestrator_links_tenant_key_run_unique").on(t.tenantId, t.apiKeyId, t.sourceRunId),
  tenantMissionIdx: index("idx_revenue_orchestrator_links_tenant_mission").on(t.tenantId, t.missionId),
  tenantIdx: index("idx_revenue_orchestrator_links_tenant").on(t.tenantId),
  tenantSourceRunIdx: index("idx_revenue_orchestrator_links_tenant_source_run").on(t.tenantId, t.sourceRunId),
  sourceRunIdx: index("idx_revenue_orchestrator_links_source_run").on(t.sourceRunId),
  missionIdx: index("idx_revenue_orchestrator_links_mission").on(t.missionId),
  sourceRunTenantFk: foreignKey({
    columns: [t.tenantId, t.sourceRunId],
    foreignColumns: [agentRuns.tenantId, agentRuns.id],
    name: "revenue_orchestrator_links_tenant_source_run_fk",
  }),
  missionTenantFk: foreignKey({
    columns: [t.tenantId, t.missionId],
    foreignColumns: [revenueMissions.tenantId, revenueMissions.id],
    name: "revenue_orchestrator_links_tenant_mission_fk",
  }),
}));
export const insertRevenueOrchestratorMissionLinkSchema = createInsertSchema(revenueOrchestratorMissionLinks).omit({ id: true, createdAt: true });
export type RevenueOrchestratorMissionLink = typeof revenueOrchestratorMissionLinks.$inferSelect;
export type InsertRevenueOrchestratorMissionLink = z.infer<typeof insertRevenueOrchestratorMissionLinkSchema>;

// Report-only recovery evidence for long-running agent work. These checkpoints
// intentionally contain a bounded, redacted summary plus a state hash and an
// inspect-only cursor; they are not an execution queue and cannot resume work.
export const agentRecoveryManifests = pgTable("agent_recovery_manifests", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  // Soft IDs preserve recovery evidence when its source conversation/run is
  // deleted. Ownership is verified on write against the same tenant.
  ...
  nextCursor: jsonb("next_cursor").notNull().default(sql`'{}'::jsonb`),
  idempotencyKey: text("idempotency_key").notNull(),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantIdempotencyUnique: uniqueIndex("idx_agent_recovery_manifests_tenant_idempotency").on(t.tenantId, t.idempotencyKey),
  tenantCreatedIdx: index("idx_agent_recovery_manifests_tenant_created").on(t.tenantId, t.createdAt),
  runOrderIdx: index("idx_agent_recovery_manifests_run_order").on(t.tenantId, t.runId, t.eventIndex),
  conversationOrderIdx: index("idx_agent_recovery_manifests_conversation_order").on(t.tenantId, t.conversationId, t.eventIndex),
  traceOrderIdx: index("idx_agent_recovery_manifests_trace_order").on(t.tenantId, t.traceId, t.eventIndex),
}));
export const insertAgentRecoveryManifestSchema = createInsertSchema(agentRecoveryManifests).omit({ id: true, createdAt: true });
export type InsertAgentRecoveryManifest = z.infer<typeof insertAgentRecoveryManifestSchema>;
export type AgentRecoveryManifest = typeof agentRecoveryManifests.$inferSelect;
...
export const orchestrationEfficiency = pgTable("orchestration_efficiency", {
  ...
  // arXiv:2606.24937 (Roitman, ch.20 + 29.2.6) — process-quality / trajectory
  // grading ("evaluation beyond final task success"). Nullable: only populated by
  // callsites that have a finished trajectory in scope (e.g. CEO plan execution).
  processQuality: doublePrecision("process_quality"), // 0..1 composite (scoreTrajectory)
  completionRate: doublePrecision("completion_rate"), // 0..1 completed/steps
  failureRate: doublePrecision("failure_rate"), // 0..1 failed/steps
  redundancyRate: doublePrecision("redundancy_rate"), // 0..1 redundant/steps
  steps: integer("steps"), // total discrete steps in the run
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
...
// Pre-Flight Capability Review (PFCR) telemetry — reuse-vs-rebuild signal.
// Bob 2026-06-28 chose the "directive + telemetry" enforcement model: the
// do-not-rebuild block stays a prompt-level directive, and every fired review is
// recorded here so /admin/ecosystem-health can surface whether agents actually
// reuse the native assets PFCR surfaced or ignore them. tenant_id NOT NULL, no
// default (every INSERT passes it explicitly). reuse_eligible = the review
// surfaced >=1 TOOL-kind do-not-rebuild item (the only kind observable at the
// tool-dispatch chokepoint) — it is the honest denominator for the reuse rate.
export const capabilityReviews = pgTable("capability_reviews", {
  ...
  assetsSurfaced: integer("assets_surfaced").notNull().default(0),
  rebuildRisks: integer("rebuild_risks").notNull().default(0),
  reuseEligible: boolean("reuse_eligible").notNull().default(false),
  surfacedNames: text("surfaced_names").array(),
  reused: boolean("reused").notNull().default(false),
  reusedCapability: text("reused_capability"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantCreatedIdx: index("idx_cap_review_tenant_created").on(t.tenantId, t.createdAt),
}));
export type CapabilityReviewRow = typeof capabilityReviews.$inferSelect;
export const insertCapabilityReviewSchema = createInsertSchema(capabilityReviews).omit({ id: true, createdAt: true });
export type InsertOrchestrationEfficiency = z.infer<typeof insertOrchestrationEfficiencySchema>;

// Self-Directed Initiatives — the "intention engine". Unlike every other
// autonomy loop (scheduled crons, reactive repair/CI, narrow external-feed
// discovery), this is the platform's SELF-ORIGINATED goal formation: a periodic
// loop introspects the platform's OWN internal telemetry (incidents, decline
// events, lead pipeline, heartbeat errors, eval drift, …) and PROPOSES 1–3
// initiatives nobody asked for. It is genuine self-agency with HITL on
// commitment: initiatives are persisted + surfaced to the owner for approval;
// nothing here auto-executes a material change. `evidence` is the telemetry
// snapshot that motivated the proposal (auditable provenance). `signature` is a
// normalized dedup key so the loop doesn't re-propose the same idea every run.
// Tenant-scoped (no FK, app-level WHERE per house convention).
export const selfInitiatives = pgTable("self_initiatives", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  title: text("title").notNull(),
  rationale: text("rationale").notNull().default(""),
  // "revenue" | "reliability" | "quality" | "growth" | "cost" | "safety" | "capability" | "general"
  category: text("category").notNull().default("general"),
  // The telemetry snapshot the proposal was grounded in (auditable provenance).
  evidence: jsonb("evidence").notNull().default(sql`'{}'::jsonb`),
  // 0..1 — the model's self-rated confidence this is worth doing.
  confidence: real("confidence").notNull().default(0),
  // "low" | "medium" | "high" — execution/blast-radius risk if pursued.
  risk: text("risk").notNull().default("medium"),
  // Free-text value estimate (e.g. "~$X/mo", "closes a reliability gap").
  estimatedValue: text("estimated_value").notNull().default(""),
  // "surfaced" | "approved" | "dismissed" | "done" | "acting"
  status: text("status").notNull().default("surfaced"),
  // Which model authored the proposal (provider drift visibility).
  sourceModel: text("source_model").notNull().default(""),
  // Normalized dedup key (lowercased title words) — one open initiative per idea.
  signature: text("signature").notNull().default(""),
  decidedAt: timestamp("decided_at"),
  decidedBy: text("decided_by"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantStatusIdx: index("idx_self_initiatives_tenant_status").on(t.tenantId, t.status, t.createdAt),
  tenantSigIdx: index("idx_self_initiatives_tenant_sig").on(t.tenantId, t.signature),
  // Concurrency backstop for app-level dedup: one OPEN initiative per signature.
  openSigUniqIdx: uniqueIndex("idx_self_initiatives_open_sig")
    .on(t.tenantId, t.signature)
    .where(sql`status IN ('surfaced','approved','acting')`),
}));
export type SelfInitiative = typeof selfInitiatives.$inferSelect;
export const insertSelfInitiativeSchema = createInsertSchema(selfInitiatives).omit({ id: true, createdAt: true });
export type InsertSelfInitiative = z.infer<typeof insertSelfInitiativeSchema>;
...
export const agentCostLedger = pgTable("agent_cost_ledger", {
  ...
  runId: integer("run_id"),
  // Mission-scoped cost attribution (CPT 5.6 priority 4): LLM/tool calls made
  // on behalf of a revenue mission carry its id so true contribution margin
  // (revenue − refunds − spend − fees − variable API cost) is computable per
  // mission. Nullable — non-mission spend is unaffected.
  ...
  // Mission economics hot path: SUM(cost_usd) per (tenant, mission).
  index("idx_agent_cost_ledger_tenant_mission").on(table.tenantId, table.missionId),
  uniqueIndex("idx_agent_cost_ledger_tenant_idempotency")
...
export const agentSettings = pgTable("agent_settings", {
  ...
  defaultModel: text("default_model").notNull().default("gpt-5.4"),
...
// SkillOpt promotion lifecycle. The skills catalog is platform-global, so
// optimizer rows are explicitly stamped to the admin tenant rather than
// pretending the catalog itself is tenant-owned. Candidate content is nullable
// because unsafe candidates are recorded by hash/reason only and never stored
// as executable prompt material.
export const skillOptimizationCandidates = pgTable("skill_optimization_candidates", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  ...
  juryDecision: jsonb("jury_decision"),
  juryDecisionHash: text("jury_decision_hash"),
  reviewDeadline: timestamp("review_deadline", { withTimezone: true }),
  failureReason: text("failure_reason"),
  promotedAt: timestamp("promoted_at", { withTimezone: true }),
  rolledBackAt: timestamp("rolled_back_at", { withTimezone: true }),
  previousVersionId: integer("previous_version_id"),
  promotedVersionId: integer("promoted_version_id"),
  transitionHistory: jsonb("transition_history").notNull().default([]),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (t) => ({
  tenantIdx: index("idx_skill_opt_candidates_tenant").on(t.tenantId),
  skillIdx: index("idx_skill_opt_candidates_skill").on(t.skillId),
  stateIdx: index("idx_skill_opt_candidates_state").on(t.state, t.reviewDeadline),
  identityUnique: uniqueIndex("uidx_skill_opt_candidates_identity").on(t.tenantId, t.identityKey),
  idTenantUnique: unique("uq_skill_opt_candidates_id_tenant").on(t.id, t.tenantId),
}));

// Immutable live-prompt snapshots. A promotion inserts the old prompt and the
// new prompt in one transaction; rollback inserts a new snapshot rather than
// mutating history.
export const skillOptimizationVersions = pgTable("skill_optimization_versions", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  ...
  kind: text("kind").notNull().default("live-snapshot"),
  previousVersionId: integer("previous_version_id"),
  metadata: jsonb("metadata").notNull().default({}),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (t) => ({
  tenantIdx: index("idx_skill_opt_versions_tenant").on(t.tenantId),
  skillIdx: index("idx_skill_opt_versions_skill").on(t.skillId, t.createdAt),
  candidateIdx: index("idx_skill_opt_versions_candidate").on(t.candidateId),
  candidateTenantFk: foreignKey({
    columns: [t.candidateId, t.tenantId],
    foreignColumns: [skillOptimizationCandidates.id, skillOptimizationCandidates.tenantId],
    name: "fk_skill_opt_versions_candidate_tenant",
  }),
}));
...
export const agentKnowledge = pgTable("agent_knowledge", {
  ...
  expiresAt: timestamp("expires_at"),
  archivedAt: timestamp("archived_at"),
  // Usage instrumentation (2026-08-11): mirrors memory_entries' access tracking
  // so the knowledge layer can improve over time — frequently-retrieved rows get
  // a small ranking boost, never-used rows become visible for refresh/pruning.
  // Additive + defaulted: no backfill needed; rollback = DROP COLUMN.
  accessCount: integer("access_count").notNull().default(0),
...
// Immutable, tenant-scoped provenance snapshots for the mutable runtime harness.
// A manifest is content-addressed, so repeated evaluations of the same effective
// configuration reuse one record. It stores only canonical, redacted metadata —
// never credentials — and lets operators reproduce which prompt/memory/router/
// capability configuration produced an evaluation or learned harness delta.
export const harnessManifests = pgTable("harness_manifests", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  ...
  tenantHashUnique: uniqueIndex("idx_harness_manifests_tenant_hash").on(t.tenantId, t.manifestHash),
  tenantIdUnique: unique("uq_harness_manifests_tenant_id").on(t.id, t.tenantId),
  tenantIdIdUnique: unique("uq_harness_manifests_tenant_id_id").on(t.tenantId, t.id),
  tenantCreatedIdx: index("idx_harness_manifests_tenant_created").on(t.tenantId, t.createdAt),
}));
export const insertHarnessManifestSchema = createInsertSchema(harnessManifests).omit({ id: true, createdAt: true });
export type InsertHarnessManifest = z.infer<typeof insertHarnessManifestSchema>;
export type HarnessManifest = typeof harnessManifests.$inferSelect;
...
export const evalRuns = pgTable("eval_runs", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  harnessManifestId: integer("harness_manifest_id"),
  ...
  manifestIdx: index("idx_eval_runs_harness_manifest").on(t.tenantId, t.harnessManifestId),
  manifestTenantFk: foreignKey({
    columns: [t.tenantId, t.harnessManifestId],
    foreignColumns: [harnessManifests.tenantId, harnessManifests.id],
    name: "eval_runs_harness_manifest_tenant_fk",
  }).onDelete("cascade"),
}));
...
export const heartbeatLogs = pgTable("heartbeat_logs", {
  ...
  // Pace-control lane tag. 'interactive' = user/chat-driven delegation (own
  // protected budget); NULL / anything-else = autonomous background run. Lets
  // the pace cap stop background scheduled-task completions from starving
  // user-initiated delegations.
  source: text("source"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const insertSkillOptimizationCandidateSchema = createInsertSchema(skillOptimizationCandidates).omit({ id: true, createdAt: true, updatedAt: true });
export const insertSkillOptimizationVersionSchema = createInsertSchema(skillOptimizationVersions).omit({ id: true, createdAt: true });
...
// Bounded, report-only Human-AI Synergy trial evidence. Raw task work is not
// persisted: measurements and the deterministic result are aggregate JSON.
export const synergyTrials = pgTable("synergy_trials", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  idempotencyKey: varchar("idempotency_key", { length: 80 }).notNull(),
  trialName: varchar("trial_name", { length: 120 }).notNull(),
  taskLabel: varchar("task_label", { length: 200 }).notNull(),
  participantAlias: varchar("participant_alias", { length: 120 }).notNull(),
  ...
  tenantIdx: index("idx_synergy_trials_tenant").on(t.tenantId),
  tenantCreatedIdx: index("idx_synergy_trials_tenant_created").on(t.tenantId, t.createdAt),
  tenantIdempotencyUnique: uniqueIndex("uq_synergy_trials_tenant_idempotency").on(t.tenantId, t.idempotencyKey),
}));
export const insertSynergyTrialSchema = createInsertSchema(synergyTrials).omit({ id: true, createdAt: true });
export type InsertSynergyTrial = z.infer<typeof insertSynergyTrialSchema>;
export type SynergyTrial = typeof synergyTrials.$inferSelect;
...
export const abRuns = pgTable("ab_runs", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  // Optional idempotency key for script-owned experiments. NULL preserves the
  // existing interactive A/B workflow, while a tenant-scoped key prevents a
  // retried shadow experiment from spending twice.
  ...
  sourceKeyUnique: uniqueIndex("ab_runs_tenant_source_key_unique").on(t.tenantId, t.sourceKey),
}));
...
// Production-shadow reasoned-review evidence. This table stores only hashes,
// bounded metadata, and aggregate reviewer signals; customer candidate text,
// prompts, critiques, and transcripts are intentionally not persisted.
export const reasonedReviewEvidence = pgTable("reasoned_review_evidence", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  deliverableType: text("deliverable_type").notNull(),
  mode: text("mode").notNull(), // shadow | canary
  status: text("status").notNull().default("complete"), // reserved | complete
  idempotencyKey: text("idempotency_key").notNull(),
  provenance: jsonb("provenance").notNull().default(sql`'{}'::jsonb`),
  baselineCandidateHash: text("baseline_candidate_hash").notNull(),
  recommendedCandidateHash: text("recommended_candidate_hash"),
  baselineDecision: text("baseline_decision").notNull().default("baseline"),
  decisionUsed: text("decision_used").notNull().default("baseline"),
  degraded: boolean("degraded").notNull().default(false),
  ...
  validReviewerCount: integer("valid_reviewer_count").notNull().default(0),
  actualReviewerModels: jsonb("actual_reviewer_models").notNull().default(sql`'[]'::jsonb`),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (t) => ({
  tenantIdempotencyUnique: uniqueIndex("reasoned_review_evidence_tenant_idempotency").on(t.tenantId, t.idempotencyKey),
  tenantCreatedIdx: index("idx_reasoned_review_evidence_tenant_created").on(t.tenantId, t.createdAt),
}));
export const insertReasonedReviewEvidenceSchema = createInsertSchema(reasonedReviewEvidence).omit({
  id: true,
  createdAt: true,
});
export type ReasonedReviewEvidence = typeof reasonedReviewEvidence.$inferSelect;
export type InsertReasonedReviewEvidence = z.infer<typeof insertReasonedReviewEvidenceSchema>;
...
export type SkillOptimizationCandidate = typeof skillOptimizationCandidates.$inferSelect;
export type InsertSkillOptimizationCandidate = z.infer<typeof insertSkillOptimizationCandidateSchema>;
export type SkillOptimizationVersion = typeof skillOptimizationVersions.$inferSelect;
export type InsertSkillOptimizationVersion = z.infer<typeof insertSkillOptimizationVersionSchema>;
...
export const agentWakeSchedules = pgTable("agent_wake_schedules", {
  ...
  result: jsonb("result"),
  createdBy: text("created_by").notNull().default("agent"),
  // Condition-based wakes (Kimi K3 #1): when trigger_event is set, the wake also
  // fires when a matching event-bus event lands (wake_at acts as the backstop
  // deadline). trigger_filter is an optional jsonb containment predicate on the
  // event's data payload (data @> trigger_filter).
...
// Action Ledger (feature contract: data/feature-contracts/action-ledger/) — durable
// record of side-effecting tool attempts for exactly-once semantics. A `prepared`
// row is persisted BEFORE crossing the external boundary; settled to committed/failed
// after; parked as `unknown` on timeout/crash for the reconciler. S1: table +
// primitives only, nothing writes to it yet.
...
export const insertActionAttemptSchema = createInsertSchema(actionAttempts).omit({ id: true, startedAt: true, settledAt: true, committedAt: true });
export type ActionAttempt = typeof actionAttempts.$inferSelect;
export type InsertActionAttempt = z.infer<typeof insertActionAttemptSchema>;

// Delegation Supervisor Phase 1 — append-only, tenant-scoped report receipts.
// Stores only bounded decision metadata; delegated prompts and credentials are
// intentionally excluded. The composite unique key makes one attempt idempotent
// inside its tenant without creating a cross-tenant identifier namespace.
export const delegationSupervisorReceipts = pgTable("delegation_supervisor_receipts", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  delegationId: text("delegation_id").notNull(),
  attemptId: text("attempt_id").notNull(),
  policyVersion: text("policy_version").notNull(),
  supervisionMode: text("supervision_mode").notNull(),
  source: text("source").notNull(),
  requesterIdentity: text("requester_identity").notNull(),
  targetAgent: text("target_agent").notNull(),
  status: text("status").notNull(),
  finalTier: text("final_tier"),
  structurallyValid: boolean("structurally_valid").notNull(),
  enforcementEnabled: boolean("enforcement_enabled").notNull().default(false),
  decision: jsonb("decision").notNull().default({}),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (t) => ({
  tenantCreatedIdx: index("idx_delegation_supervisor_receipts_tenant_created").on(t.tenantId, t.createdAt),
  tenantDelegationIdx: index("idx_delegation_supervisor_receipts_tenant_delegation").on(t.tenantId, t.delegationId),
  tenantAttemptPolicyUnique: uniqueIndex("uidx_delegation_supervisor_receipts_tenant_attempt_policy").on(t.tenantId, t.delegationId, t.attemptId, t.policyVersion),
}));
export const insertDelegationSupervisorReceiptSchema = createInsertSchema(delegationSupervisorReceipts).omit({ id: true, createdAt: true });
export type DelegationSupervisorReceipt = typeof delegationSupervisorReceipts.$inferSelect;
export type InsertDelegationSupervisorReceipt = z.infer<typeof insertDelegationSupervisorReceiptSchema>;
...
export const fileStorage = pgTable("file_storage", {
  ...
  // Republish-durability review (2026-08-14, rev2): uniqueness is scoped by
  // PARTIAL indexes to ONLY the durability rows (queue backup + delivery
  // assets) so the ON CONFLICT upserts are atomic there, while generic
  // uploads keep their historical allow-duplicates behavior untouched.
  uniqueIndex("file_storage_queue_backup_uidx").on(t.tenantId, t.filename)
    .where(sql`filename = 'service-review-queue-backup.json'`),
  uniqueIndex("file_storage_delivery_asset_uidx").on(t.tenantId, t.filename)
    .where(sql`filename LIKE 'delivery-%'`),
  uniqueIndex("file_storage_cmmc_artifact_uidx").on(t.tenantId, t.filename)
    .where(sql`filename LIKE 'cmmc-report-%'`),
  uniqueIndex("file_storage_local_state_uidx").on(t.tenantId, t.filename)
    .where(sql`filename LIKE 'local-state-%'`),
]);
...
// R125+137.81 — Gold exemplar library. Top-scoring deliverable production text
// (scripts/outlines/copy) captured at grade time (score ≥ bar) or owner-marked,
// retrieved as few-shot exemplars when the same tenant produces that format
// again. Content is TEXT (production-side artifact excerpt), never file bytes.
export const deliverableExemplars = pgTable("deliverable_exemplars", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  format: text("format").notNull(),               // video | audio | pdf | slides | ...
  title: text("title"),                            // topic / product name for display
  content: text("content").notNull(),              // the exemplar text (capped at write time)
  contentSha256: text("content_sha256").notNull(), // dedupe key
  score: integer("score").notNull(),               // 0-100 grade at capture
  source: text("source").notNull().default("auto_grade"), // auto_grade | owner_marked
  metadata: jsonb("metadata"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantFormatScoreIdx: index("deliverable_exemplars_tenant_format_score_idx").on(t.tenantId, t.format, t.score),
  dedupeIdx: uniqueIndex("deliverable_exemplars_dedupe_idx").on(t.tenantId, t.format, t.contentSha256),
}));

// Outcome-driven learning (Grok 4.5 item #2, 2026-07-25): EVERY grade event
// (pass AND fail) is recorded so recurring failure patterns per tenant+format
// can be promoted into skill-optimizer eval seeds. Exemplars capture only the
// wins; this table is the full outcome ledger the promotion scan reads.
export const deliverableGradeEvents = pgTable("deliverable_grade_events", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  ...
  requestExcerpt: text("request_excerpt"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantFormatIdx: index("idx_deliverable_grade_events_tenant_format").on(t.tenantId, t.format, t.createdAt),
}));
export type DeliverableGradeEvent = typeof deliverableGradeEvents.$inferSelect;
...
export const deliveryLogs = pgTable("delivery_logs", {
  ...
  // Generic fulfillment idempotency key. Unlike stripePaymentId, this also
  // covers non-payment jobs such as recovered video delivery.
  idempotencyKey: text("idempotency_key"),
  metadata: jsonb("metadata"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  completedAt: timestamp("completed_at"),
  ...
  tenantIdempotencyKeyUniq: uniqueIndex("delivery_logs_tenant_idempotency_key_unique")
...
// Canonical durable-work manifest. File bytes live in Drive and/or the existing
// file_storage/object-storage layer; this table records the tenant-scoped
// identity, immutable revision, and proof required before work is called durable.
export const artifactRecords = pgTable("artifact_records", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  projectId: integer("project_id"),
  // Stable, non-null namespace avoids PostgreSQL NULL-unique semantics for
  // tenant-wide artifacts that do not belong to a project.
  scopeKey: text("scope_key").notNull().default("tenant"),
  logicalName: text("logical_name").notNull(),
  artifactKind: text("artifact_kind").notNull().default("document"),
  mimeType: text("mime_type").notNull(),
  sourceRunKey: text("source_run_key"),
  sourceRequestId: text("source_request_id"),
  deliveryLogId: integer("delivery_log_id").references(() => deliveryLogs.id, { onDelete: "set null" }),
  fileStorageId: integer("file_storage_id").references(() => fileStorage.id, { onDelete: "set null" }),
  idempotencyKey: text("idempotency_key").notNull(),
  ...
  durableAt: timestamp("durable_at"),
  deletedAt: timestamp("deleted_at"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => ({
  tenantCreatedIdx: index("idx_artifact_records_tenant_created").on(t.tenantId, t.createdAt),
  tenantStatusIdx: index("idx_artifact_records_tenant_status").on(t.tenantId, t.status, t.updatedAt),
  tenantProjectIdx: index("idx_artifact_records_tenant_project").on(t.tenantId, t.projectId, t.createdAt),
  deliveryIdx: index("idx_artifact_records_delivery_log").on(t.deliveryLogId),
  fileStorageIdx: index("idx_artifact_records_file_storage").on(t.fileStorageId),
  // Same operation/retry always resolves to one canonical artifact row.
  tenantIdempotencyUnique: uniqueIndex("idx_artifact_records_tenant_idempotency")
    .on(t.tenantId, t.idempotencyKey),
  // A new source-run is a new immutable revision; retries retain the existing
  // revision through the idempotency key above.
  tenantRevisionUnique: uniqueIndex("idx_artifact_records_tenant_scope_revision")
    .on(t.tenantId, t.scopeKey, t.logicalName, t.revision),
}));
export const insertArtifactRecordSchema = createInsertSchema(artifactRecords).omit({
  id: true, revision: true, durableAt: true, deletedAt: true, createdAt: true, updatedAt: true,
});
export type ArtifactRecord = typeof artifactRecords.$inferSelect;
export type InsertArtifactRecord = z.infer<typeof insertArtifactRecordSchema>;

// Append-only lifecycle events keep partial provider outcomes auditable without
// rewriting the manifest's current state or losing a delivery receipt.
export const artifactEvents = pgTable("artifact_events", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  artifactId: integer("artifact_id").notNull().references(() => artifactRecords.id, { onDelete: "cascade" }),
  eventType: text("event_type").notNull(),
  status: text("status").notNull(),
  ...
  tenantCreatedIdx: index("idx_artifact_events_tenant_created").on(t.tenantId, t.createdAt),
  artifactCreatedIdx: index("idx_artifact_events_artifact_created").on(t.artifactId, t.createdAt),
}));
export const insertArtifactEventSchema = createInsertSchema(artifactEvents).omit({ id: true, createdAt: true });
...
export const orderLookupCodes = pgTable("order_lookup_codes", {
  ...
  // Captured from the trusted storefront configuration at challenge issuance;
  // verification uses it to scope the recovered delivery rows.
  tenantId: integer("tenant_id").notNull(),
...
export const projects = pgTable("projects", {
  ...
  tenantProjectUnique: uniqueIndex("uniq_projects_tenant_id_id").on(t.tenantId, t.id),
  tenantProjectFkTarget: unique("uq_projects_tenant_id_id_fk").on(t.id, t.tenantId),
}));
...
export const projectCompasses = pgTable("project_compasses", {
  id: serial("id").primaryKey(),
  ...
  tenantProjectUnique: uniqueIndex("uniq_project_compasses_tenant_project").on(t.tenantId, t.projectId),
  tenantProjectIdx: index("idx_project_compasses_tenant_project").on(t.tenantId, t.projectId),
  projectIdx: index("idx_project_compasses_project").on(t.projectId),
  projectTenantFk: foreignKey({
    columns: [t.projectId, t.tenantId],
    foreignColumns: [projects.id, projects.tenantId],
    name: "fk_project_compasses_tenant_project",
  }).onDelete("cascade"),
}));
export const insertProjectCompassSchema = createInsertSchema(projectCompasses).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertProjectCompass = z.infer<typeof insertProjectCompassSchema>;
export type ProjectCompass = typeof projectCompasses.$inferSelect;
...
// Mis-pick telemetry (R125+137.64) — record-only ledger, never a gate.
// A row = within one chat turn, tool A failed and a DIFFERENT tool B later
// succeeded (retry-success of the SAME tool is NOT a mis-pick). Feeds the
// Tool Mis-Picks card on /admin/ecosystem-health. Fail-open at every seam.
export const toolMispicks = pgTable("tool_mispicks", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  personaId: integer("persona_id"),
  conversationId: integer("conversation_id"),
  failedTool: text("failed_tool").notNull(),
  succeededTool: text("succeeded_tool").notNull(),
...
export const typesafeFactPrefilterObservations = pgTable("typesafe_fact_prefilter_observations", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id),
  sourceMessageId: integer("source_message_id").notNull(),
  conversationId: integer("conversation_id").notNull(),
  version: text("version").notNull().default("jev-session-fact-prefilter-v1"),
  mode: text("mode").notNull(),
  status: text("status").notNull().default("claimed"),
  durableFactProbability: real("durable_fact_probability"),
  inputTokens: integer("input_tokens"),
  outputTokens: integer("output_tokens"),
  model: text("model"),
  extractorRan: boolean("extractor_ran"),
  factsWritten: integer("facts_written"),
  errorClass: text("error_class"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  tenantIdx: index("idx_typesafe_fact_prefilter_tenant").on(table.tenantId),
  sourceUnique: uniqueIndex("uq_typesafe_fact_prefilter_source")
    .on(table.tenantId, table.sourceMessageId, table.version),
}));
...
export const plans = pgTable("plans", {
  ...
  autoApplySourceRefUnique: uniqueIndex("plans_auto_apply_source_ref_unique")
    .on(t.tenantId, t.source, t.sourceRef)
    .where(sql`${t.source} = 'agentic-engine.auto-apply' AND ${t.sourceRef} IS NOT NULL`),
  staleRecoverySourceRefUnique: uniqueIndex("plans_stale_recovery_source_ref_unique")
    .on(t.tenantId, t.source, t.sourceRef)
    .where(sql`${t.source} = 'stale-plan-recovery' AND ${t.sourceRef} IS NOT NULL`),
}));
...
export const agentJobs = pgTable("agent_jobs", {
  ...
  failureClassIdx: index("agent_jobs_failure_class_idx").on(t.failureClass, t.createdAt.desc()).where(sql`failure_class IS NOT NULL`),
}));
...
export const modelHarnessDeltas = pgTable("model_harness_deltas", {
  ...
  manifestIdx: index("idx_model_harness_deltas_harness_manifest").on(t.tenantId, t.harnessManifestId),
  manifestTenantFk: foreignKey({
    columns: [t.tenantId, t.harnessManifestId],
    foreignColumns: [harnessManifests.tenantId, harnessManifests.id],
    name: "model_harness_deltas_harness_manifest_tenant_fk",
  }).onDelete("cascade"),
}));
...
export const commitments = pgTable("commitments", {
  ...
  sensitivity: varchar("sensitivity", { length: 20 }).notNull().default("routine"),
  // Phase 1 — Proactive Commitment Drafting (additive, applied via psql).
  // Draft lifecycle is a SEPARATE column from `status`: the existing miner /
  // scanner / dedupe partial index all key on `status` ('active'|'escalated'…),
  // so overloading that column with the draft enum would NOT be additive.
  // draft_status: open | draft_pending | draft_ready | approved_sent | dismissed | expired
  draftStatus: varchar("draft_status", { length: 20 }).notNull().default("open"),
  // FK-by-convention to agent_approvals.id — the draft artifact lives in the
  // approval card's context jsonb (no standalone artifacts table exists).
  draftArtifactId: integer("draft_artifact_id"),
...
export const researchEvidence = pgTable("research_evidence", {
  ...
  // MemGraphRAG-inspired durable provenance chain. All fields stay nullable
  // so existing evidence remains readable; only source-backed rows qualify for
  // the opt-in graph retrieval trial.
  schemaKey: text("schema_key"),
  factKey: text("fact_key"),
  passageHash: text("passage_hash"),
  sourceFingerprint: text("source_fingerprint"),
  conflictType: text("conflict_type"),
  conflictGroupKey: text("conflict_group_key"),
  status: text("status").notNull().default("active"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  tenantCreatedIdx: index("idx_research_evidence_tenant_created").on(t.tenantId, t.createdAt),
  tenantFactIdx: index("idx_research_evidence_tenant_fact").on(t.tenantId, t.factKey),
}));
...
export const codeProposals = pgTable("code_proposals", {
  ...
  repairProvenanceUnique: uniqueIndex("code_proposals_repair_provenance_unique")
    .on(table.tenantId, table.source, table.sourceSessionId)
    .where(sql`source = 'production-repair-handoff' AND source_session_id IS NOT NULL`),
}));

/** S5 durable production→development source-repair inbox.  Production may
 * write this inbox only through the signed workspace handoff, never proposals. */
export const repairHandoffRequests = pgTable("repair_handoff_requests", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  repairIdentity: text("repair_identity").notNull(),
  ...
  timestamp: timestamp("request_timestamp").notNull(),
  ...
  tenantIdx: index("repair_handoff_requests_tenant_idx").on(table.tenantId),
  statusIdx: index("repair_handoff_requests_status_idx").on(table.tenantId, table.status),
  claimIdx: index("repair_handoff_requests_claim_idx").on(table.tenantId, table.status, table.claimExpiresAt),
  createdIdx: index("repair_handoff_requests_created_idx").on(table.createdAt),
  identityUnique: uniqueIndex("repair_handoff_requests_identity_unique").on(table.tenantId, table.repairIdentity),
  nonceUnique: uniqueIndex("repair_handoff_requests_nonce_unique").on(table.tenantId, table.nonce),
}));
export const insertRepairHandoffRequestSchema = createInsertSchema(repairHandoffRequests)
  .omit({ id: true, createdAt: true, updatedAt: true, acceptedAt: true, consumedAt: true });
export type InsertRepairHandoffRequest = z.infer<typeof insertRepairHandoffRequestSchema>;
export type RepairHandoffRequest = typeof repairHandoffRequests.$inferSelect;
...
export const capabilityGaps = pgTable("capability_gaps", {
  ...
  resolvedAt: timestamp("resolved_at"),
  priority: text("priority").notNull().default("medium"),
  // Tool Forge Phase 1 (docs/tool-forge-spec.md) — demand-signal strength.
  // Incremented by detectGap() on every repeat miss of the same gap; the
  // ecosystem-health "Unmet Capabilities" card ranks gaps by missCount.
...
// Agent evaluation, operational learning, storefront wedge, and replay telemetry.
// Extracted for the girth guard; re-export preserves @shared/schema compatibility.
export * from "./models/operational-learning";
...
// ============================================================
// SIMULATION SANDBOX — extracted to shared/models/sandbox.ts (girth guard).
// Re-exported here so every consumer keeps importing from @shared/schema.
// ============================================================
export * from "./models/sandbox";

// ============================================================
// VERIFIED REVENUE MISSIONS — extracted to shared/models/revenue-missions.ts
// (girth guard). Re-exported here so consumers import from @shared/schema.
// ============================================================
export * from "./models/revenue-missions";

// ============================================================
// COMMERCE PRODUCTS — extracted to shared/models/commerce.ts (girth guard).
// Re-exported here so consumers import from @shared/schema.
// ============================================================
export * from "./models/commerce";

// ============================================================
// DFY INTAKE FORMS — customer-facing onboarding questionnaire for the
// $1,997 Done-For-You fix kit. One row per form link the admin sends.
// Token is the sole public credential (48-hex, unguessable); responses is
// a flat { fieldKey: answer } jsonb validated against
// shared/dfy-intake-fields.ts on submit.
// ============================================================
export const dfyIntakeForms = pgTable("dfy_intake_forms", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull(),
  token: text("token").notNull(),
  /** Optional link back to the originating service-review-queue item id. */
  orderId: text("order_id"),
  ...
  tokenUq: uniqueIndex("uq_dfy_intake_forms_token").on(t.token),
  tenantCreatedIdx: index("idx_dfy_intake_forms_tenant_created").on(t.tenantId, t.createdAt),
  websiteIdx: index("idx_dfy_intake_forms_website").on(t.website),
}));
export const insertDfyIntakeFormSchema = createInsertSchema(dfyIntakeForms).omit({ id: true, createdAt: true });
export type InsertDfyIntakeForm = z.infer<typeof insertDfyIntakeFormSchema>;
export type DfyIntakeForm = typeof dfyIntakeForms.$inferSelect;

// ============================================================
// DERIVED API RECIPES — "watch once, skip the browser". A recipe is a
// distilled plain-HTTP call (method + URL template + param descriptions)
// derived from network traffic captured during a real browser session.
// Auth header VALUES are never stored — only the names of headers the
// endpoint needs (headerNames), supplied live at call time.
// Rollback: DROP TABLE derived_api_recipes;
// ============================================================
export const derivedApiRecipes = pgTable("derived_api_recipes", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id),
  name: text("name").notNull(),
  description: text("description"),
  ...
  /** URL template with {param} placeholders, e.g. https://api.x.com/v1/items/{id} */
  urlTemplate: text("url_template").notNull(),
  /** { paramName: description } — what each {param} / query param means. */
  paramsSchema: jsonb("params_schema").notNull().default({}),
  /** Names of request headers the endpoint needs (values NEVER stored). */
  headerNames: jsonb("header_names").notNull().default([]),
  /** Optional JSON body template with {param} placeholders (POST/PUT). */
  bodyTemplate: text("body_template"),
  /** One-line description of the response shape, for the LLM. */
  responseShape: text("response_shape"),
  status: text("status").notNull().default("active"), // 'active' | 'needs_reverify'
  lastVerifiedAt: timestamp("last_verified_at"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (t) => ({
  tenantNameUq: uniqueIndex("uq_derived_api_recipes_tenant_name").on(t.tenantId, t.name),
  tenantIdx: index("idx_derived_api_recipes_tenant").on(t.tenantId),
}));
export const insertDerivedApiRecipeSchema = createInsertSchema(derivedApiRecipes).omit({ id: true, createdAt: true });
export type InsertDerivedApiRecipe = z.infer<typeof insertDerivedApiRecipeSchema>;
export type DerivedApiRecipe = typeof derivedApiRecipes.$inferSelect;

export * from "./models/cmmc";

export * from "./models/hvac-missed-call-recovery";
export * from "./models/commercial-research-frontier";

// ============================================================
// UNIFIED MEMORY FORGETTING — S2 durable lifecycle state
//
// These tables are additive and intentionally contain policy metadata and
// opaque identifiers only.  In particular, sourceId/valueDigest are not
// memory values, embeddings, or erased content.  `source`, `mode`, `action`,
// and lifecycle statuses are application-level taxonomies (not model output);
// callers must validate them before insert.
//
// Backfill: none.  Installing this schema must not rewrite existing memory.
// Rollback: disable the forgetting coordinator first, retain required audit
// evidence, then drop the five additive tables and their indexes:
// memory_lifecycle_audits, memory_lifecycle_action_claims,
// memory_tombstones, memory_erasure_requests, memory_retention_policies.
// ============================================================

/** Versioned, deterministic retention/archival policy per tenant. */
export const memoryRetentionPolicies = pgTable("memory_retention_policies", {
  ...
  mode: text("mode").notNull().default("report_only"), // report_only | mutate
  /** Deterministic source rules; never populated from model-generated authority. */
  ...
  tenantIdx: index("idx_memory_retention_policies_tenant").on(t.tenantId),
  tenantStatusIdx: index("idx_memory_retention_policies_tenant_status").on(t.tenantId, t.status),
  tenantVersionUnique: uniqueIndex("uidx_memory_retention_policies_tenant_version").on(t.tenantId, t.version),
}));
export const insertMemoryRetentionPolicySchema = createInsertSchema(memoryRetentionPolicies).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type MemoryRetentionPolicyRow = typeof memoryRetentionPolicies.$inferSelect;
export type InsertMemoryRetentionPolicy = z.infer<typeof insertMemoryRetentionPolicySchema>;

/**
 * Authenticated explicit-erasure request.  requestKey is an opaque
 * caller/server idempotency key; reasonCode is a bounded application code,
 * never a free-form memory explanation.
 */
export const memoryErasureRequests = pgTable("memory_erasure_requests", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  requestKey: text("request_key").notNull(),
  mode: text("mode").notNull().default("explicit"), // explicit | compliance | offboarding
  ...
  tenantIdx: index("idx_memory_erasure_requests_tenant").on(t.tenantId),
  tenantStatusIdx: index("idx_memory_erasure_requests_tenant_status").on(t.tenantId, t.status, t.createdAt),
  tenantRequestKeyUnique: uniqueIndex("uidx_memory_erasure_requests_tenant_request_key").on(t.tenantId, t.requestKey),
}));
export const insertMemoryErasureRequestSchema = createInsertSchema(memoryErasureRequests).omit({
  id: true,
  createdAt: true,
  startedAt: true,
  completedAt: true,
...
export type MemoryErasureRequest = typeof memoryErasureRequests.$inferSelect;
export type InsertMemoryErasureRequest = z.infer<typeof insertMemoryErasureRequestSchema>;

/**
 * Platform-global account-offboarding receipt.  This is the intentional
 * compliance exception to tenant ownership: the receipt must survive the
 * tenant's ON DELETE CASCADE, so it has no tenant_id column or foreign key.
 * tenantFingerprint is an HMAC, never a tenant identifier; this row contains
 * only value-free deletion counts/status and a timestamp.
 */
export const accountErasureReceipts = pgTable("account_erasure_receipts", {
  id: serial("id").primaryKey(),
  tenantFingerprint: text("tenant_fingerprint").notNull(),
  requestKey: text("request_key").notNull(),
  itemCount: integer("item_count").notNull(),
  erasedCount: integer("erased_count").notNull(),
  status: text("status").notNull(),
  recordedAt: timestamp("recorded_at").defaultNow().notNull(),
}, (t) => ({
  requestKeyUnique: uniqueIndex("uidx_account_erasure_receipts_request_key").on(t.requestKey),
  fingerprintIdx: index("idx_account_erasure_receipts_fingerprint").on(t.tenantFingerprint),
}));
export const insertAccountErasureReceiptSchema = createInsertSchema(accountErasureReceipts).omit({
  id: true,
  recordedAt: true,
});
export type AccountErasureReceipt = typeof accountErasureReceipts.$inferSelect;
export type InsertAccountErasureReceipt = z.infer<typeof insertAccountErasureReceiptSchema>;

/**
 * Anti-resurrection record.  One tenant/source/sourceId can have one
 * tombstone; the digest is keyed by a server secret and is not recoverable
 * memory.  No source value, prompt text, or embedding belongs in this row.
 */
export const memoryTombstones = pgTable("memory_tombstones", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  source: text("source").notNull(),
  /** Opaque stable source row identifier, never the source's content. */
  sourceId: text("source_id").notNull(),
  /** Keyed digest used by write/extraction paths to block resurrection. */
  valueDigest: text("value_digest").notNull(),
  requestKey: text("request_key").notNull(),
  reasonCode: text("reason_code").notNull().default("explicit_erasure"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  tenantIdx: index("idx_memory_tombstones_tenant").on(t.tenantId),
  tenantSourceIdx: index("idx_memory_tombstones_tenant_source").on(t.tenantId, t.source, t.sourceId),
  tenantDigestIdx: index("idx_memory_tombstones_tenant_digest").on(t.tenantId, t.valueDigest),
  tenantSourceUnique: uniqueIndex("uidx_memory_tombstones_tenant_source_id").on(t.tenantId, t.source, t.sourceId),
}));
export const insertMemoryTombstoneSchema = createInsertSchema(memoryTombstones).omit({
  id: true,
  createdAt: true,
});
...
/**
 * Provenance edges between memory rows.  Source ids are deliberately
 * polymorphic: the tenant foreign key is the only relational constraint, so
 * an edge cannot accidentally turn into a cross-table or cross-tenant join.
 * `solelyDerived` is the explicit authorization required before an erasure
 * walks from a parent into a derived child.
 */
export const memoryDerivations = pgTable("memory_derivations", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  parentSource: text("parent_source").notNull(),
  parentId: text("parent_id").notNull(),
  childSource: text("child_source").notNull(),
  childId: text("child_id").notNull(),
  relationship: text("relationship").notNull(),
  solelyDerived: boolean("solely_derived").notNull().default(false),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  tenantParentIdx: index("idx_memory_derivations_tenant_parent").on(t.tenantId, t.parentSource, t.parentId),
  tenantChildIdx: index("idx_memory_derivations_tenant_child").on(t.tenantId, t.childSource, t.childId),
  tenantEdgeUnique: uniqueIndex("uidx_memory_derivations_tenant_edge")
    .on(t.tenantId, t.parentSource, t.parentId, t.childSource, t.childId, t.relationship),
}));
export const insertMemoryDerivationSchema = createInsertSchema(memoryDerivations).omit({
  id: true,
  createdAt: true,
});
export type MemoryDerivation = typeof memoryDerivations.$inferSelect;
export type InsertMemoryDerivation = z.infer<typeof insertMemoryDerivationSchema>;

/**
 * Durable, resumable claim for one tenant/source/policy/action.  Claims are
 * idempotent on the full contract key and may be retried after lease expiry;
 * only the coordinator may advance status.
 */
export const memoryLifecycleActionClaims = pgTable("memory_lifecycle_action_claims", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  ...
  action: text("action").notNull(), // archive | purge | erase
  status: text("status").notNull().default("claimed"), // claimed | running | completed | failed
  ...
  tenantIdx: index("idx_memory_lifecycle_action_claims_tenant").on(t.tenantId),
  tenantStatusLeaseIdx: index("idx_memory_lifecycle_action_claims_tenant_status_lease").on(t.tenantId, t.status, t.leaseExpiresAt),
  tenantSourceIdx: index("idx_memory_lifecycle_action_claims_tenant_source").on(t.tenantId, t.source, t.sourceId),
  tenantActionKeyUnique: uniqueIndex("uidx_memory_lifecycle_action_claims_tenant_action_key").on(t.tenantId, t.actionKey),
  tenantActionIdentityUnique: uniqueIndex("uidx_memory_lifecycle_action_claims_tenant_source_policy_action")
    .on(t.tenantId, t.source, t.sourceId, t.policyVersion, t.action),
}));
export const insertMemoryLifecycleActionClaimSchema = createInsertSchema(memoryLifecycleActionClaims).omit({
  id: true,
  claimedAt: true,
  completedAt: true,
  createdAt: true,
...
export type MemoryLifecycleActionClaim = typeof memoryLifecycleActionClaims.$inferSelect;
export type InsertMemoryLifecycleActionClaim = z.infer<typeof insertMemoryLifecycleActionClaimSchema>;

/**
 * Append-only, independently readable lifecycle evidence.  It deliberately
 * has no mutable timestamp/status fields and stores identifiers/reason codes
 * only; audit insertion must be denied for cross-tenant action keys.
 */
export const memoryLifecycleAudits = pgTable("memory_lifecycle_audits", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  actionKey: text("action_key").notNull(),
  source: text("source").notNull(),
  ...
  action: text("action").notNull(), // archive | purge | erase
  mode: text("mode").notNull().default("report_only"), // report_only | mutate | explicit
  outcome: text("outcome").notNull().default("recorded"), // recorded | skipped | failed
  reasonCode: text("reason_code"),
  ...
  tenantIdx: index("idx_memory_lifecycle_audits_tenant").on(t.tenantId),
  tenantCreatedIdx: index("idx_memory_lifecycle_audits_tenant_created").on(t.tenantId, t.createdAt),
  tenantActionIdx: index("idx_memory_lifecycle_audits_tenant_action").on(t.tenantId, t.action, t.source),
  tenantActionKeyIdx: index("idx_memory_lifecycle_audits_tenant_action_key").on(t.tenantId, t.actionKey),
}));
export const insertMemoryLifecycleAuditSchema = createInsertSchema(memoryLifecycleAudits).omit({
  id: true,
  createdAt: true,
});
export type MemoryLifecycleAudit = typeof memoryLifecycleAudits.$inferSelect;
export type InsertMemoryLifecycleAudit = z.infer<typeof insertMemoryLifecycleAuditSchema>;

export const memoryForgettingSchedulerState = pgTable("memory_forgetting_scheduler_state", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  lastScannedAt: timestamp("last_scanned_at"),
  nextSourceOffset: integer("next_source_offset").notNull().default(0),
  /** Operational keyset cursors, keyed by lifecycle source name. */
  sourceCursors: jsonb("source_cursors").$type<Record<string, number>>().notNull().default({}),
  /** A worker owns a tenant only while this token and expiry are current. */
  leaseToken: text("lease_token"),
  leaseExpiresAt: timestamp("lease_expires_at"),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  tenantUnique: uniqueIndex("uidx_memory_forgetting_scheduler_state_tenant").on(t.tenantId),
  scanIdx: index("idx_memory_forgetting_scheduler_state_scan").on(t.lastScannedAt, t.tenantId),
  leaseIdx: index("idx_memory_forgetting_scheduler_state_lease").on(t.leaseExpiresAt, t.tenantId),
}));

/** Durable per-tenant/per-host request events for coordinated web collection. */
export const webDomainUsageEvents = pgTable("web_domain_usage_events", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  hostname: text("hostname").notNull(),
  occurredAt: timestamp("occurred_at").defaultNow().notNull(),
}, (t) => ({
  tenantIdx: index("idx_web_domain_usage_events_tenant").on(t.tenantId),
  tenantHostTimeIdx: index("idx_web_domain_usage_events_tenant_host_time").on(t.tenantId, t.hostname, t.occurredAt),
}));
export const insertWebDomainUsageEventSchema = createInsertSchema(webDomainUsageEvents).omit({
  id: true,
  occurredAt: true,
});
export type WebDomainUsageEvent = typeof webDomainUsageEvents.$inferSelect;
export type InsertWebDomainUsageEvent = z.infer<typeof insertWebDomainUsageEventSchema>;

/** Latest provider-directed cooldown for one tenant and public hostname. */
export const webDomainCooldowns = pgTable("web_domain_cooldowns", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  hostname: text("hostname").notNull(),
  ...
  tenantIdx: index("idx_web_domain_cooldowns_tenant").on(t.tenantId),
  tenantHostUnique: uniqueIndex("uidx_web_domain_cooldowns_tenant_host").on(t.tenantId, t.hostname),
  expiryIdx: index("idx_web_domain_cooldowns_expiry").on(t.cooldownUntil),
}));
export const insertWebDomainCooldownSchema = createInsertSchema(webDomainCooldowns).omit({
  id: true,
  updatedAt: true,
});
export type WebDomainCooldown = typeof webDomainCooldowns.$inferSelect;
export type InsertWebDomainCooldown = z.infer<typeof insertWebDomainCooldownSchema>;

/** Durable idempotency claim and terminal result for one coordinated web run. */
export const webCollectionRuns = pgTable("web_collection_runs", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
  ...
  tenantIdx: index("idx_web_collection_runs_tenant").on(t.tenantId),
  tenantRunUnique: uniqueIndex("uidx_web_collection_runs_tenant_run").on(t.tenantId, t.runKey),
  tenantLeaseIdx: index("idx_web_collection_runs_tenant_lease").on(t.tenantId, t.leaseExpiresAt),
}));
export const insertWebCollectionRunSchema = createInsertSchema(webCollectionRuns).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type WebCollectionRun = typeof webCollectionRuns.$inferSelect;
export type InsertWebCollectionRun = z.infer<typeof insertWebCollectionRunSchema>;
