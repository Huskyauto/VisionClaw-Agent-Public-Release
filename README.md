...
**Latest — R132.6 (September 20, 2026):** **Evidence-bounded capability maturity plus the R130–R132 reliability program.** A deterministic admin report separates repeatable narrow-task performance from task breadth, human benchmarks, operational evidence, and general-intelligence claims. Missing evidence remains insufficient, and the report adds no authority or autonomy. Recent releases also added structured TypeSafe Jev state, evidence-backed expert model routing, plan recovery closure, source-repair handoffs, whole-loop improvement evidence, research expert mode, and hardened payment-webhook finality. Current verification includes focused regressions, TypeScript, production build, route-conservation checks, runtime route checks, and independent review.
...
flowchart TB
    ...
    subgraph TOOLS["Tool layer — 418 total registered tools"]
        direction TB
...
```
   ...
   (Forge,          (Scribe,         (Neptune,               agents (with
    Engineer)        Writer)          Research)              runtime adapter)
        │                │                │                       │
        └──────┬─────────┴────────────────┴───────────────────────┘
               │  tool dispatch (418 total registered tools)
               ▼
   ┌──────────────────────────────────────────────────────────────┐
...
server/                       # Express backend
  ...
  tools/                      # 418 total registered tools — per-domain modules (70+ domains) + shared middleware
