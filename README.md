...
**Latest — Round 128+sec (September 12, 2026):** **Deterministic report-only delegation supervision.** Every known child-agent execution family reports through one server-owned boundary before side effects and records bounded tenant-scoped receipts. Owner-direct authority is derived only from authenticated platform-owner context; malformed or unresolved policy cannot enforce, and existing structural security gates remain authoritative.

Roughly 320k lines of TypeScript across 1,100+ files. 40+ pages. **417 total registered tools · 386 public documented tools · 68 total skills · 137 active capabilities · 18 personas · 169 declared / 247 live tables · 41 governance rules · 777 platform indexes · 80 curated AI models · 6 AI providers · 6 deployment targets.** The public tool index deliberately excludes trusted-persona-only tools. Current release facts: [`docs/release-facts.json`](docs/release-facts.json). Browsable indexes: [`docs/tools.md`](docs/tools.md) · [`docs/personas.md`](docs/personas.md).
...
| Frontend Pages | 40+ |
| API Endpoints | 300+ |
| Database Tables | 169 declared / 247 live |
...
└──────────────────┘     └──────────┬──────────────────┘
        ...
        │ 169 declared / 247 live tables │   │ Storage    │   │ 80 curated │
        └──────────┘   └────────────┘   └────────────┘
```
...
server/                       # Express backend
  ...
  seed.ts                     # Database seeding (169 declared / 247 live tables, 41 rules, 18 personas)
...
shared/
  schema.ts                   # Drizzle ORM schema (169 declared / 247 live — see docs/CURRENT_PLATFORM_TOTALS.md)
scripts/
...
1. The database auto-creates all 247 live tables and full index set
