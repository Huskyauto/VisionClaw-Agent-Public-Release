...
**Latest — R135.4+sec (September 23, 2026):** **Deterministic artifact authority for consequential code apply.** Research code proposals now require a server-issued receipt bound to the exact tenant, proposal, target, current source bytes, canonical diff, verifier version, verdict, and evidence before the first source write. Agents still propose, existing jury or owner approval remains mandatory, and publication remains a separate owner decision.

Roughly 320k lines of TypeScript across 1,100+ files. 40+ pages. **420 total registered tools · 386 public documented tools · 68 total skills · 141 active capabilities · 18 personas · 182 declared / 262 live tables · 41 governance rules · 839 platform indexes · 78 curated AI models · 6 AI providers · 6 deployment targets.** The public tool index deliberately excludes trusted-persona-only tools. Current release facts: [`docs/release-facts.json`](docs/release-facts.json). Browsable indexes: [`docs/tools.md`](docs/tools.md) · [`docs/personas.md`](docs/personas.md).
...
flowchart TB
    ...
    subgraph TOOLS["Tool layer — 420 total registered tools"]
        direction TB
...
```
   ...
   (Forge,          (Scribe,         (Neptune,               agents (with
    Engineer)        Writer)          Research)              runtime adapter)
        │                │                │                       │
        └──────┬─────────┴────────────────┴───────────────────────┘
               │  tool dispatch (420 total registered tools)
               ▼
   ┌──────────────────────────────────────────────────────────────┐
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      <a href="docs/images/tour-command-center.jpg"><img src="docs/images/tour-command-center.jpg" alt="Command Center — 18 agents, 420 total registered tools, 78 curated models + 1000+ daily catalog, live workflows" /></a>
...
| Tools (total registered) | 418 |
| Tools (public documented) | 386 |
| AI Models in Core Registry | 79 curated |
...
| Frontend Pages | 40+ |
| API Endpoints | 300+ |
| Database Tables | 182 declared / 262 live |
...
└──────────────────┘     └──────────┬──────────────────┘
        ...
        │ 182 declared / 262 live tables │   │ Storage    │   │ 79 curated │
        └──────────┘   └────────────┘   └────────────┘
```
...
- **78 Curated AI Models in the Core Registry** with cost-aware auto-routing across OpenAI, Anthropic, Google Gemini, xAI Grok, OpenRouter, and Perplexity. Bring your own provider credentials — API billing is governed by each provider's own terms.
...
server/                       # Express backend
  ...
  tools/                      # 420 total registered tools — per-domain modules (70+ domains) + shared middleware
  ...
  seed.ts                     # Database seeding (182 declared / 262 live tables, 41 rules, 18 personas)
