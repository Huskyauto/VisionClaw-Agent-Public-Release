...
**Latest — R132.5 (September 19, 2026):** **Structured TypeSafe Jev judgments plus the R130–R132 reliability program.** Jev accepts bounded named JSON state with descriptor-safe validation and explicit secret/personal-data minimization rules. Recent releases also added evidence-backed expert model routing, plan recovery closure, source-repair handoffs, whole-loop improvement evidence, research expert mode, and hardened payment-webhook finality. Current verification includes focused regressions, TypeScript, production build, wiring audit, runtime route checks, and independent review.
