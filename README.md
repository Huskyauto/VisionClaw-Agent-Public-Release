...
> **Reviewing this repo (human or AI)?** Start with [`docs/FOR-EXTERNAL-REVIEWERS.md`](docs/FOR-EXTERNAL-REVIEWERS.md) — auto-regenerated at every mirror build with the current code-shape facts, deliberate architecture decisions (e.g. app-level tenant isolation, **not** RLS), and the single source of truth for all platform counts ([`docs/CURRENT_PLATFORM_TOTALS.md`](docs/CURRENT_PLATFORM_TOTALS.md)). Reviews graded against stale snapshots or third-party summaries will contain invalid findings.
...
**Latest — R125+155.4 (September 3, 2026):** Retired Gemini 3.5 identifiers now normalize to Gemini 3.7 and are excluded from active routing, failover, and multimodal selection. Context-overflow recovery reports the actual model returned after metered-policy remapping instead of claiming an unavailable 1M lane. SAM.gov company reports persist and verify a durable database copy before Drive upload, with bounded optional object-storage operations, while Wikipedia article-not-found responses return a normal `found:false` result. Existing free-only remapping and spend controls remain unchanged. Verification: 52/52 focused regressions, all 247 non-E2E Node suites effectively green, TypeScript, production build, stale-string and wiring gates, and final independent architect plus silent-failure reviews passed.
