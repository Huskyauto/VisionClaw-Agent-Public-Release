...
Roughly 180k lines of TypeScript across 460+ files. 40+ pages. **357 tools · 110 active capabilities · 16 personas · 176 tables · 43 governance rules · 454 production indexes · 6 AI providers · 6 deployment targets.** Live, always-current counts: [`docs/CURRENT_PLATFORM_TOTALS.md`](docs/CURRENT_PLATFORM_TOTALS.md). Browsable indexes: [`docs/tools.md`](docs/tools.md) · [`docs/personas.md`](docs/personas.md).
...
That's it. The platform auto-creates the schema, seeds 16 personas + 40 governance rules, and redirects you to `/setup`. First account becomes admin.
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      <a href="docs/images/tour-command-center.jpg"><img src="docs/images/tour-command-center.jpg" alt="Command Center — 16 agents, 357 tools, 36 curated models + 1000+ daily catalog, live workflows" /></a>
...
| Built-in Tools | 298 |
| AI Models in Core Registry | 36 curated |
...
| Frontend Pages | 40+ |
| API Endpoints | 300+ |
| Database Tables | 168 (live) / 119 declared |
...
- **36 Curated AI Models in the Core Registry** with cost-aware auto-routing across OpenAI, Anthropic, Google Gemini, xAI Grok, OpenRouter, and Perplexity. Bring your own provider credentials — API billing is governed by each provider's own terms.
...
server/                       # Express backend
  ...
  tools.ts                    # 338 tool definitions and execution handlers
  routes.ts                   # 300+ API endpoints
  site-config.ts              # Centralized env-driven configuration
  seed.ts                     # Database seeding (176 tables, 42 rules, 16 personas)
...
shared/
  schema.ts                   # Drizzle ORM schema (176 tables)
scripts/
...
1. The database auto-creates all 176 tables and full index set
