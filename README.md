...
**Latest — Round 125+155.22+sec (September 11, 2026):** **Commercial Research Frontier foundation.** Owner-only, tenant-scoped opportunity APIs provide deterministic scoring, evidence state, maturity, risks, kill criteria, and smallest next tests. Atomic row-locked lifecycle updates and database checks reject malformed evidence, forged scores, JSON-null bypasses, and invalid lists. This S1 foundation performs no outreach, payments, missions, publishing, or trading; production publish remains pending explicit owner approval.

Roughly 320k lines of TypeScript across 1,100+ files. 40+ pages. **417 total registered tools · 386 public documented tools · 67 total skills · 137 active capabilities · 18 personas · 168 declared / 245 live tables · 41 governance rules · 761 platform indexes · 80 curated AI models · 6 AI providers · 6 deployment targets.** The public tool index deliberately excludes trusted-persona-only tools. Current release facts: [`docs/release-facts.json`](docs/release-facts.json). Browsable indexes: [`docs/tools.md`](docs/tools.md) · [`docs/personas.md`](docs/personas.md).
...
```mermaid
flowchart TB
    U["User / Autonomous loops<br/>(heartbeat · scheduled tasks · self-repair)"] --> CE["Chat Engine (SSE)"]
    CE --> SAFETY

    subgraph SAFETY["AHB Safety Layer — every call passes through"]
        direction TB
        IG["Intent gate<br/>(fail-OPEN, logged)"] --- DP["Destructive-tool policy<br/>(fail-CLOSED)"]
        RG["Outbound redaction gate"] --- TC["Tenant context<br/>(AsyncLocalStorage)"]
    end

    SAFETY --> CEO["CEO orchestrator (Felix)"]
    CEO --> ROUTER["Agent Router"]
    ROUTER --> P1["Specialist personas<br/>(Engineer · Writer · Research · Finance · Legal · …)"]
    ROUTER --> P2["Imported Claude Code agents<br/>(runtime adapter)"]

    P1 --> TOOLS
    P2 --> TOOLS

    subgraph TOOLS["Tool layer — 417 total registered tools"]
        direction TB
        T1["File I/O · web · email · LLM · payments · drive"]
        T2["TNR snapshots — irreversible calls are undoable"]
        T3["Tracing spans — parent-linked causality tree"]
        T4["Admission control — priority pool + rate limits"]
    end

    TOOLS --> DB[("PostgreSQL + pgvector<br/>tenant-isolated: every WHERE filters tenant_id")]
```
...
```
   ...
   (Forge,          (Scribe,         (Neptune,               agents (with
    Engineer)        Writer)          Research)              runtime adapter)
        │                │                │                       │
        └──────┬─────────┴────────────────┴───────────────────────┘
               │  tool dispatch (417 total registered tools)
               ▼
   ┌──────────────────────────────────────────────────────────────┐
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      <a href="docs/images/tour-command-center.jpg"><img src="docs/images/tour-command-center.jpg" alt="Command Center — 18 agents, 417 total registered tools, 80 curated models + 1000+ daily catalog, live workflows" /></a>
...
| Tools (total registered) | 417 |
| Tools (public documented) | 386 |
| AI Models in Core Registry | 80 curated |
...
| Frontend Pages | 40+ |
| API Endpoints | 300+ |
| Database Tables | 168 declared / 245 live |
...
└──────────────────┘     └──────────┬──────────────────┘
        ...
        │ 168 declared / 245 live tables │   │ Storage    │   │ 80 curated │
        └──────────┘   └────────────┘   └────────────┘
```
...
| **Robert** | Wellness Coach | Warmth, empathy, and practical mindfulness tools for stress- or anxiety-triggered eating urges |
...
server/                       # Express backend
  ...
  tools/                      # 417 total registered tools — per-domain modules (70+ domains) + shared middleware
  ...
  seed.ts                     # Database seeding (168 declared / 245 live tables, 41 rules, 18 personas)
...
shared/
  schema.ts                   # Drizzle ORM schema (168 declared / 245 live — see docs/CURRENT_PLATFORM_TOTALS.md)
scripts/
...
1. The database auto-creates all 245 live tables and full index set
