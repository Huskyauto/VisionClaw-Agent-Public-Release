...
Roughly 320k lines of TypeScript across 1,100+ files. 40+ pages. **413 tools · 134 active capabilities · 18 personas · 135 declared / 230 live tables · 41 governance rules · 679 platform indexes · 6 AI providers · 6 deployment targets.** Live, always-current counts: [`docs/CURRENT_PLATFORM_TOTALS.md`](docs/CURRENT_PLATFORM_TOTALS.md). Browsable indexes: [`docs/tools.md`](docs/tools.md) · [`docs/personas.md`](docs/personas.md).

**Latest — R125+155+sec9 (August 22, 2026):** owner-only frontier revenue research is now organized into accepted concepts and separate raw jury research, with no automatic product, outreach, payment, or customer-record creation. The saved-report boundary requires authenticated owner access, disables browser caching, and rejects malformed, unsafe, or non-regular entries before delivery.
...
flowchart TB
    ...
    subgraph TOOLS["Tool layer — 413 governed tools"]
        direction TB
...
| Built-in Tools | 413 |
| AI Models in Core Registry | 67 curated |
...
| Frontend Pages | 40+ |
| API Endpoints | 300+ |
| Database Tables | 230 live (135 declared in schema.ts) |
...
└──────────────────┘     └──────────┬──────────────────┘
        ...
        │ 135 declared / 230 live tables │   │ Storage    │   │ 67 curated │
        └──────────┘   └────────────┘   └────────────┘
```
...
server/                       # Express backend
  ...
  tools/                      # 413 tools — per-domain modules (70+ domains) + shared middleware
  ...
  seed.ts                     # Database seeding (135 declared / 230 live tables, 41 rules, 18 personas)
...
shared/
  schema.ts                   # Drizzle ORM schema (135 declared; 230 live — see docs/CURRENT_PLATFORM_TOTALS.md)
scripts/
...
1. The database auto-creates all 230 live tables and full index set
