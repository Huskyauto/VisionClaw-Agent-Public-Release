...
Roughly 180k lines of TypeScript across 460+ files. 40+ pages. **394 tools · 129 active capabilities · 16 personas · 212 tables · 41 governance rules · 623 production indexes · 6 AI providers · 6 deployment targets.** Live, always-current counts: [`docs/CURRENT_PLATFORM_TOTALS.md`](docs/CURRENT_PLATFORM_TOTALS.md). Browsable indexes: [`docs/tools.md`](docs/tools.md) · [`docs/personas.md`](docs/personas.md).
...
| `tool-smoke-test` | Staged smoke-test + documentation program across all 394 tools (20 stages of 20; 107 live-safe / 287 doc-only — doc-only tools are never auto-invoked); $0 / no LLM / no tenant, one stage per session | informational — **60/394 tools · 3/20 stages signed off** |
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      <a href="docs/images/tour-command-center.jpg"><img src="docs/images/tour-command-center.jpg" alt="Command Center — 16 agents, 394 tools, 41 curated models + 1000+ daily catalog, live workflows" /></a>
...
server/                       # Express backend
  ...
  tools.ts                    # 394 tool definitions and execution handlers
  routes.ts                   # 300+ API endpoints
  site-config.ts              # Centralized env-driven configuration
  seed.ts                     # Database seeding (212 tables, 41 rules, 16 personas)
...
shared/
  schema.ts                   # Drizzle ORM schema (212 tables)
scripts/
...
1. The database auto-creates all 212 tables and full index set
