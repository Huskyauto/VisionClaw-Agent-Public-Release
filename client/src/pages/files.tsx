...
import {
  ...
  RotateCw, Send, ShieldCheck, AlertTriangle,
} from "lucide-react";
...
interface DurableArtifact {
  ...
  status: "pending" | "durable" | "degraded" | "failed" | "deleted";
...
export default function FilesPage() {
  ...
  const { data: artifacts = [] } = useQuery<DurableArtifact[]>({
    queryKey: ["/api/artifacts"],
    queryFn: async () => {
      const response = await authFetch("/api/artifacts");
      if (!response.ok) throw new Error("Failed to load durable work records");
  ...
  const artifactActionMutation = useMutation({
    mutationFn: async ({ artifactId, action }: { artifactId: number; action: "reconcile" | "resend" | "resolve-resend-uncertainty" }) => {
      await apiRequest("POST", `/api/artifacts/${artifactId}/${action}`);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/artifacts"] });
      toast({ title: variables.action === "reconcile" ? "Durability check completed" : variables.action === "resend" ? "Existing delivery resent" : "Delivery uncertainty resolved" });
    },
    onError: (err: Error) => {
      toast({ title: "Artifact action failed", description: err.message, variant: "destructive" });
    },
  });
  ...
  const handleArtifactDownload = useCallback(async (artifact: DurableArtifact) => {
    try {
      const res = await authFetch(`/api/artifacts/${artifact.id}/download`);
      if (!res.ok) {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = artifact.logicalName;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch (err: any) {
      toast({ title: "Durable copy unavailable", description: err.message, variant: "destructive" });
    }
  }, [toast]);
  ...
  const visibleArtifacts = useMemo(() => {
    ...
    return artifacts.filter((artifact) =>
      artifact.logicalName.toLowerCase().includes(query) ||
      artifact.artifactKind.toLowerCase().includes(query) ||
      artifact.mimeType.toLowerCase().includes(query),
    );
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6">
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        {artifacts.length > 0 && (
          <Card className="border-primary/20" data-testid="durable-artifact-records">
            <CardContent className="p-0">
              <div className="flex flex-col gap-2 border-b px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Verified Drive copies, immutable revisions, and recovery history.
                  </p>
                </div>
                <Badge variant="outline" className="w-fit text-xs">
                  {artifacts.length} tracked
                </Badge>
              </div>
              <div className="divide-y">
                {visibleArtifacts.slice(0, 12).map((artifact) => {
                  const Icon = getFileIcon(artifact.mimeType);
                  const statusClass = artifact.status === "durable"
                    ? "border-emerald-500/30 text-emerald-600"
                    : artifact.status === "degraded" || artifact.status === "failed"
                      ? "border-amber-500/30 text-amber-600"
                      : "border-blue-500/30 text-blue-600";
                  return (
                    <div key={artifact.id} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center" data-testid={`durable-artifact-${artifact.id}`}>
                      <div className="flex min-w-0 flex-1 items-start gap-2.5">
                        <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded bg-muted">
                          <Icon className={`h-4 w-4 ${getFileColor(artifact.mimeType)}`} />
                        </div>
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium">{artifact.logicalName}</p>
                          <p className="mt-0.5 text-[11px] text-muted-foreground">
                            Revision {artifact.revision} · {formatFileSize(artifact.sizeBytes)} · SHA-256 {artifact.sha256.slice(0, 12)}…
                          </p>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        {artifact.driveViewUrl && (
                          <Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={() => window.open(artifact.driveViewUrl!, "_blank", "noopener,noreferrer")}>
                            <ExternalLink className="mr-1 h-3 w-3" /> Drive
                          </Button>
                        )}
                        <Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={() => handleArtifactDownload(artifact)}>
                          <Download className="mr-1 h-3 w-3" /> Download
                        </Button>
                        {artifact.status !== "durable" && artifact.status !== "deleted" && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 px-2 text-xs"
                            disabled={artifactActionMutation.isPending}
                            onClick={() => artifactActionMutation.mutate({ artifactId: artifact.id, action: "reconcile" })}
                          >
                            <RotateCw className="mr-1 h-3 w-3" /> Reconcile
                          </Button>
                        )}
                        {(artifact.errorMessage?.startsWith("Delivery resend outcome uncertain:") || artifact.errorMessage === "Customer email delivery receipt pending") && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 px-2 text-xs"
                            disabled={artifactActionMutation.isPending}
                            onClick={() => artifactActionMutation.mutate({ artifactId: artifact.id, action: "resolve-resend-uncertainty" })}
                          >
                            <ShieldCheck className="mr-1 h-3 w-3" /> Resolve delivery
                          </Button>
                        )}
                        {artifact.deliveryLogId && artifact.status === "durable" && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 px-2 text-xs"
                            disabled={artifactActionMutation.isPending}
                            onClick={() => artifactActionMutation.mutate({ artifactId: artifact.id, action: "resend" })}
                          >
              ...
              {visibleArtifacts.length === 0 && (
                <p className="px-4 py-5 text-center text-sm text-muted-foreground">No durable work records match this search.</p>
              )}
            </CardContent>
          </Card>
        )}
