...
import { Loader2, Play, ChevronDown, ChevronRight, Clock, Bot, Wrench, CheckCircle2, XCircle, CircleDashed, ShieldCheck } from "lucide-react";
...
export default function WorkflowReplayPage() {
  ...
  const { data: recoveryList } = useQuery<{ manifests: RecoveryManifest[] }>({
    queryKey: ["/api/recovery-manifests"],
    queryFn: async () => {
      const r = await fetch("/api/recovery-manifests?limit=20", { credentials: "include" });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      return r.json();
    },
  });
  const { data: recoveryDetail } = useQuery<{ manifest: RecoveryManifest; timeline: RecoveryManifest[] }>({
    queryKey: ["/api/recovery-manifests", selectedRecoveryId],
    enabled: selectedRecoveryId != null,
    queryFn: async () => {
      const r = await fetch(`/api/recovery-manifests/${selectedRecoveryId}`, { credentials: "include" });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      return r.json();
    },
  });
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="container mx-auto p-6 space-y-6 max-w-6xl">
        ...
        {recoveryList && (
          <Card data-testid="card-recovery-manifests">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <ShieldCheck className="w-4 h-4" /> Recovery evidence
              </CardTitle>
              <p className="text-xs text-muted-foreground">
                Durable restart checkpoints are read-only evidence. Selecting one never resumes, retries, or reruns work.
              </p>
            </CardHeader>
            <CardContent className="space-y-3">
              {recoveryList.manifests.length === 0 ? (
                <p className="text-sm text-muted-foreground">No recovery checkpoints have been recorded yet.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {recoveryList.manifests.map((manifest) => (
                    <button
                      key={manifest.id}
                      className={`rounded-md border p-3 text-left hover:bg-muted/50 ${selectedRecoveryId === manifest.id ? "border-primary bg-primary/5" : ""}`}
                      onClick={() => setSelectedRecoveryId(manifest.id)}
                      data-testid={`button-recovery-manifest-${manifest.id}`}
                    >
                      <div className="flex gap-2 items-center">
                        <Badge variant="outline" className="text-[10px]">{manifest.eventType}</Badge>
                        <span className="ml-auto text-[10px] text-muted-foreground">#{manifest.id}</span>
                      </div>
                      <p className="mt-1 text-xs text-muted-foreground">
                        run {manifest.runId ?? "—"} · event {manifest.eventIndex} · {new Date(manifest.createdAt).toLocaleString()}
                      </p>
                    </button>
                  ))}
                </div>
              ...
              {recoveryDetail && (
                <div className="rounded-md border bg-muted/30 p-3 space-y-2" data-testid="recovery-manifest-detail">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    Checkpoint #{recoveryDetail.manifest.id}
                    <Badge variant="secondary" className="text-[10px]">inspect-only</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground font-mono break-all">
                    state {recoveryDetail.manifest.stateHash}
                  </p>
                  <pre className="max-h-44 overflow-auto text-xs whitespace-pre-wrap">
                    {JSON.stringify({
                      payload: recoveryDetail.manifest.payload,
                      nextCursor: recoveryDetail.manifest.nextCursor,
                      timelineEvents: recoveryDetail.timeline.map((item) => item.eventType),
                    }, null, 2)}
                  </pre>
                </div>
              )}
