...
export default function ArchitecturePage() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery<ArchData>({
    queryKey: ["/api/public/architecture"],
    retry: false,
  });
  const { config } = useSiteConfig();
  const pn = config.platformName;
  const errorStatus = error instanceof Error
    ? Number.parseInt(error.message.match(/^(\d{3}):/)?.[1] || "", 10)
    : Number.NaN;
  const accessDenied = errorStatus === 401 || errorStatus === 403;

  if (isError && accessDenied) {
    return (
      <div className="min-h-screen bg-[#0a0a0f] text-white flex items-center justify-center px-6" data-testid="architecture-access-required">
        <div className="max-w-xl text-center">
          <Shield className="w-14 h-14 text-violet-400 mx-auto mb-6" />
          <h1 className="text-4xl font-black mb-4">{pn} Architecture</h1>
          <p className="text-lg text-white/60 mb-3">
            Live architecture telemetry is restricted to signed-in platform administrators.
          </p>
          <p className="text-sm text-white/70 mb-8">
            This protects aggregate operational data while keeping the public product overview and trust documentation available.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <a href="/landing" className="rounded-lg bg-violet-600 hover:bg-violet-500 px-5 py-3 font-semibold transition-colors">
              View platform overview
            </a>
            <a href="/login" className="rounded-lg border border-white/15 hover:bg-white/5 px-5 py-3 font-semibold transition-colors">
              Sign in
            </a>
          </div>
        </div>
  ...
  if (isError) {
    return (
      <div className="min-h-screen bg-[#0a0a0f] text-white flex items-center justify-center px-6" data-testid="architecture-load-error">
        <div className="max-w-xl text-center">
          <Cpu className="w-14 h-14 text-violet-400 mx-auto mb-6" />
          <h1 className="text-4xl font-black mb-4">Architecture data is temporarily unavailable</h1>
          <p className="text-base text-white/70 mb-8">
            The live telemetry request failed. No access settings were changed.
          </p>
          <button
            type="button"
            onClick={() => void refetch()}
            disabled={isFetching}
            className="rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-60 px-5 py-3 font-semibold transition-colors"
          >
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-10">
        ...
        >
          ...
          <p className="mt-3 text-lg text-white/50 max-w-2xl mx-auto">
            ...
            operating with {stats.governanceRules} governance rules, 68 total skills, and {stats.tools || 420} registered tools. The current platform spans 178 declared / 258 live database tables and routes across 79 curated models plus 1000+ models discovered through OpenRouter.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-10">
          ...
          <StatCard label="Tools" value={stats.tools || 420} icon={Wrench} color="from-orange-500 to-red-500" delay={0.2} />
          <StatCard label="DB Skills" value={stats.skills} icon={Lightbulb} color="from-amber-500 to-yellow-500" delay={0.3} />
