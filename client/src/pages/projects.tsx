...
import { FolderOpen, Plus, FileText, MessageSquare, StickyNote, Search, ArrowLeft, Trash2, Tag, User, Clock, ChevronRight, PlayCircle, Upload, Image as ImageIcon, File, X, Loader2, Pencil, Check, Star, ExternalLink, Lightbulb, Briefcase, CircleDollarSign } from "lucide-react";
...
type CompassCategory = "goal" | "desired_outcome" | "definition_of_done" | "open_question" | "concern" | "constraint" | "affected_person" | "guiding_principle" | "assumption" | "unknown";
type CompassProvenance = "user_stated" | "agent_inferred";
type CompassStatus = "stated" | "confirmed" | "inferred" | "rejected";
...
interface RevenueIdeasResponse {
  workspaceKey: "customer-revenue-launchpad" | "fable-5";
  projectId: number;
  ownerOnly: boolean;
  ideas: RevenueIdea[];
}
...
export default function ProjectsPage() {
  ...
  const [fableFolderOpen, setFableFolderOpen] = useState(false);
  const [opportunityFolderOpen, setOpportunityFolderOpen] = useState(false);
  const [incomeFolderOpen, setIncomeFolderOpen] = useState(false);
  ...
  const [compassEntries, setCompassEntries] = useState<CompassEntry[]>([]);
  const [compassCategory, setCompassCategory] = useState<CompassCategory>("goal");
  const [compassStatement, setCompassStatement] = useState("");
  const [editingCompassId, setEditingCompassId] = useState<string | null>(null);
  const [editingCompassStatement, setEditingCompassStatement] = useState("");
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dropZoneRef = useRef<HTMLDivElement>(null);
  const { toast } = useToast();

  const { data: projects = [], isLoading, isError: projectsError, refetch: refetchProjects } = useQuery<Project[]>({
  ...
  const { data: projectDetail, isLoading: isDetailLoading, error: detailError, refetch: refetchProjectDetail } = useQuery<{
    project: Project;
    files: ProjectFile[];
    notes: ProjectNote[];
    conversations: ProjectConversation[];
    compass?: ProjectCompass;
  }>({
    queryKey: ["/api/projects", selectedProject],
    enabled: !!selectedProject,
    retry: 1,
  });
  const compass = projectDetail?.compass;
  useEffect(() => {
    if (compass) setCompassEntries(compass.entries || []);
  }, [compass?.revision, compass?.updatedAt]);

  const compassMutation = useMutation({
    mutationFn: async ({ projectId, revision, entries }: { projectId: number; revision: number; entries: CompassEntry[] }) => {
      const response = await apiRequest("PUT", `/api/projects/${projectId}/compass`, { revision, entries });
      return await response.json() as ProjectCompass;
    },
    onSuccess: (savedCompass, variables) => {
      if (variables.projectId) {
        queryClient.setQueryData<{
          ...
          conversations: ProjectConversation[];
          compass?: ProjectCompass;
        }>(["/api/projects", variables.projectId], (current) => current ? { ...current, compass: savedCompass } : current);
      }
      if (selectedProject === variables.projectId) {
        setCompassEntries(savedCompass.entries);
        setEditingCompassId(null);
      }
      toast({ title: "Compass updated", description: "The project briefing is saved." });
    },
    onError: (error: any) => {
      if (String(error?.message || "").startsWith("409")) {
        refetchProjectDetail();
        toast({ title: "Compass changed elsewhere", description: "The latest version has been reloaded. Review it before saving again.", variant: "destructive" });
      } else {
        toast({ title: "Could not save compass", description: error?.message || "Please try again.", variant: "destructive" });
      }
    },
  });
  ...
  const {
    data: revenueIdeas,
    isLoading: isRevenueIdeasLoading,
    isError: isRevenueIdeasError,
    refetch: refetchRevenueIdeas,
  } = useQuery<RevenueIdeasResponse>({
    queryKey: ["/api/projects", selectedProject, "revenue-ideas"],
    enabled: !!selectedProject && isAdmin,
    retry: false,
  });
  ...
  const incomeProductMutation = useMutation({
    mutationFn: (id: number) => apiRequest("POST", `/api/projects/${id}/transition-income-product`),
    onSuccess: (_response, id) => {
      queryClient.invalidateQueries({ queryKey: ["/api/projects"] });
      if (selectedProject === id) queryClient.invalidateQueries({ queryKey: ["/api/projects", id] });
      toast({ title: "Moved to Income Products" });
    },
    onError: (err: any) => {
      toast({ title: "Could not move opportunity", description: err.message || "Please try again", variant: "destructive" });
    },
  });
  ...
  const startRevenueIdeaChat = async (idea: RevenueIdea) => {
    if (!selectedProject) return;
    try {
      const res = await apiRequest(
        "POST",
        `/api/projects/${selectedProject}/revenue-ideas/${encodeURIComponent(idea.slug)}/conversation`
      );
      const data = await res.json();
      queryClient.invalidateQueries({ queryKey: ["/api/conversations"] });
      queryClient.invalidateQueries({ queryKey: ["/api/projects", data.projectId] });
      navigate(`/chat/${data.id}?draft=${encodeURIComponent(idea.starterPrompt)}`);
    } catch {
      toast({ title: "Failed to start idea workspace", description: "Please try again.", variant: "destructive" });
    }
  };
  ...
  const compassLabels: Record<CompassCategory, string> = {
    goal: "Goal",
  ...
  const saveCompassEntries = (entries: CompassEntry[]) => {
    if (!selectedProject || !compass) return;
    setCompassEntries(entries);
    compassMutation.mutate({ projectId: selectedProject, revision: compass.revision, entries });
  };
  const addCompassEntry = () => {
    const statement = compassStatement.trim();
    if (!statement) return;
    const id = typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `compass-${Date.now()}`;
    saveCompassEntries([
      ...compassEntries,
      { id, category: compassCategory, statement, provenance: "user_stated", confidence: 1, status: "stated" },
    ]);
    setCompassStatement("");
  };
  ...
  const isFableProject = (p: Project) =>
    (p.tags || []).some((t) => t === "fable-5" || t === "fable-5-idea") ||
    p.name === "5 Money Making Ideas (Fable 5 Review — 2026-07-23)" ||
    /^Idea [1-5]: /.test(p.name);
  const isBusinessOpportunity = (p: Project) => (p.tags || []).includes("business-opportunity");
  const isIncomeProduct = (p: Project) => (p.tags || []).includes("income-product");
  const businessOpportunityProjects = filtered.filter(isBusinessOpportunity);
  const incomeProductProjects = filtered.filter(isIncomeProduct);
  const ideaProjects = filtered.filter((p) => isIdeaProject(p) && !isBusinessOpportunity(p) && !isIncomeProduct(p));
  const fableProjects = filtered.filter(isFableProject);
  ...
  const ideaEntries = ideaProjects
    .filter((p) => !isRunProject(p) && !isWedgeProject(p) && !isBusinessOpportunity(p) && !isIncomeProduct(p))
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  ...
  const groupFable = !searchQuery && fableProjects.length > 0;
  const mainList = !searchQuery
    ? filtered.filter((p) => !isIdeaProject(p) && !isFableProject(p) && !isBusinessOpportunity(p) && !isIncomeProduct(p))
    : filtered;
  ...
  const showFableFolder = isAdmin && statusFilter === "all" && groupFable;
  ...
  if (selectedProject && (isDetailLoading || detailError || projectDetail)) {
    ...
    return (
      <div className="h-full overflow-y-auto p-6 space-y-6" data-testid="project-detail">
        ...
        {isAdmin && (isRevenueIdeasLoading || isRevenueIdeasError || revenueIdeas?.ideas?.length) ? (
          <Card className="border-primary/30 bg-primary/[0.02]" data-testid={`card-revenue-ideas-${revenueIdeas?.workspaceKey || "loading"}`}>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <Lightbulb className="w-5 h-5 text-primary" /> Ideas / Workstreams
                {revenueIdeas?.ideas ? <Badge variant="outline" className="ml-auto text-xs">{revenueIdeas.ideas.length}</Badge> : null}
              </CardTitle>
              <p className="text-sm text-muted-foreground">
                Choose one focused workstream. Starting a workspace opens a project-linked conversation; it does not contact anyone or send anything.
              </p>
            </CardHeader>
            <CardContent>
              {isRevenueIdeasLoading ? (
                <p className="text-sm text-muted-foreground">Loading curated workstreams…</p>
              ) : isRevenueIdeasError ? (
                <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                  <span>Couldn’t load the curated workstreams.</span>
                  <Button variant="outline" className="min-h-11" onClick={() => refetchRevenueIdeas()}>Try again</Button>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  {revenueIdeas?.ideas.map(renderRevenueIdeaCard)}
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <Card className="border-primary/20 bg-primary/[0.03]" data-testid="project-compass">
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between gap-4">
              <div>
                <CardTitle className="flex items-center gap-2 text-lg">
                  <Lightbulb className="w-5 h-5 text-primary" /> Project Compass
                </CardTitle>
                <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
                  A shared briefing for what matters here. Keep it honest, and correct anything that no longer fits.
                </p>
              </div>
              {compass?.updatedAt && (
                <span className="text-xs text-muted-foreground whitespace-nowrap">
                  Updated {new Date(compass.updatedAt).toLocaleDateString()}
                </span>
              )}
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            {!compass ? (
              <p className="text-sm text-muted-foreground" data-testid="project-compass-loading">Compass is loading…</p>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-[190px_1fr_auto] gap-2 items-start">
                  <label className="sr-only" htmlFor="compass-category">Entry category</label>
                  <select
                    id="compass-category"
                    value={compassCategory}
                    onChange={(e) => setCompassCategory(e.target.value as CompassCategory)}
                    className="h-10 rounded-md border border-input bg-background px-3 text-sm"
                    data-testid="select-compass-category"
                  >
                    {Object.entries(compassLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                  </select>
                  <label className="sr-only" htmlFor="compass-statement">Compass statement</label>
                  <Textarea
                    id="compass-statement"
                    value={compassStatement}
                    onChange={(e) => setCompassStatement(e.target.value)}
                    onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key === "Enter") addCompassEntry(); }}
                    placeholder="Add something the team should keep in view…"
                    className="min-h-10 h-10 resize-none"
                    data-testid="input-compass-statement"
                  />
                  <Button onClick={addCompassEntry} disabled={!compassStatement.trim() || compassMutation.isPending} data-testid="button-add-compass-entry">
                    <Plus className="w-4 h-4 mr-1" /> Add
                  </Button>
                </div>
                {compassEntries.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-muted-foreground/25 px-4 py-7 text-center" data-testid="project-compass-empty">
                    <p className="text-sm font-medium">Nothing is written down yet.</p>
                    <p className="text-xs text-muted-foreground mt-1">Start with the outcome this project is meant to change.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {compassEntries.map((entry) => {
                      const isEditing = editingCompassId === entry.id;
                      return (
                        <div key={entry.id} className="rounded-lg border border-border/70 bg-background/70 p-3 space-y-2" data-testid={`compass-entry-${entry.id}`}>
                          <div className="flex items-center justify-between gap-2">
                            <Badge variant="outline" className="text-[11px]">{compassLabels[entry.category]}</Badge>
                            <div className="flex items-center gap-1">
                              <Button
                                size="icon"
                                variant="ghost"
                                className="h-7 w-7"
                                aria-label={`Edit ${compassLabels[entry.category]}`}
                                onClick={() => { setEditingCompassId(entry.id); setEditingCompassStatement(entry.statement); }}
                                data-testid={`button-edit-compass-${entry.id}`}
                              >
                                <Pencil className="w-3.5 h-3.5" />
                              </Button>
                              <Button
                                ...
                                aria-label={`Delete ${compassLabels[entry.category]}`}
                                onClick={() => {
                                  if (window.confirm("Remove this compass entry?")) {
                                    saveCompassEntries(compassEntries.filter((item) => item.id !== entry.id));
                                  }
                                }}
                                disabled={compassMutation.isPending}
                                data-testid={`button-delete-compass-${entry.id}`}
                              >
                          ...
                          {isEditing ? (
                            <div className="space-y-2">
                              <Textarea
                                value={editingCompassStatement}
                                onChange={(e) => setEditingCompassStatement(e.target.value)}
                                autoFocus
                                className="min-h-20"
                                aria-label="Edit compass statement"
                                data-testid={`input-edit-compass-${entry.id}`}
                              />
                              <div className="flex justify-end gap-2">
                                <Button size="sm" variant="ghost" onClick={() => setEditingCompassId(null)} data-testid={`button-cancel-compass-${entry.id}`}>Cancel</Button>
                                <Button
                                  size="sm"
                                  disabled={!editingCompassStatement.trim() || compassMutation.isPending}
                                  onClick={() => saveCompassEntries(compassEntries.map((item) => item.id === entry.id ? { ...item, statement: editingCompassStatement.trim() } : item))}
                                  data-testid={`button-save-compass-${entry.id}`}
                                >
                            ...
                            <p className="text-sm leading-5">{entry.statement}</p>
                          )}
                          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-muted-foreground">
                            <span>{entry.provenance === "agent_inferred" ? "Agent inferred" : "User stated"}</span>
                            <span aria-hidden="true">·</span>
                            <span>{Math.round(entry.confidence * 100)}% confidence</span>
                            <span aria-hidden="true">·</span>
                            <span className="capitalize">{entry.status}</span>
                            {entry.provenance === "agent_inferred" && entry.status !== "confirmed" && (
                              <Button
                                size="sm"
                                variant="outline"
                                className="h-6 px-2 text-[11px] ml-auto"
                                onClick={() => saveCompassEntries(compassEntries.map((item) => item.id === entry.id ? { ...item, status: "confirmed" } : item))}
                                disabled={compassMutation.isPending}
                                data-testid={`button-confirm-compass-${entry.id}`}
                              >
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6" data-testid="projects-page">
      ...
      {isLoading ? (
        ...
        <Card className="text-center py-12" data-testid="projects-load-error">
          <CardContent>
            <p className="text-lg font-medium">Projects could not be loaded</p>
            <p className="text-muted-foreground text-sm mt-1">The folders may not be empty. Retry to load the current project list.</p>
            <Button className="mt-4" variant="outline" onClick={() => void refetchProjects()}>Retry</Button>
          </CardContent>
        </Card>
      ) : opportunityFolderOpen ? (
        <div className="space-y-6" data-testid="business-opportunities-folder-view">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setOpportunityFolderOpen(false)} data-testid="button-close-business-opportunities">
              <ArrowLeft className="w-4 h-4 mr-1" /> All Projects
            </Button>
            <h2 className="text-xl font-bold flex items-center gap-2">
              <Briefcase className="w-5 h-5 text-primary" /> Business Opportunities
            </h2>
            <span className="text-sm text-muted-foreground">{businessOpportunityProjects.length} {businessOpportunityProjects.length === 1 ? "opportunity" : "opportunities"}</span>
          </div>
          {businessOpportunityProjects.length === 0 ? (
            <Card className="text-center py-12"><CardContent>
              <Briefcase className="w-12 h-12 mx-auto mb-3 text-muted-foreground" />
              <p className="text-lg font-medium">No business opportunities yet</p>
              <p className="text-muted-foreground text-sm mt-1">Top-scoring IdeaBrowser ideas will appear here after the daily score.</p>
            </CardContent></Card>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {businessOpportunityProjects.map(renderProjectCard)}
            </div>
          )}
        </div>
      ) : incomeFolderOpen ? (
        <div className="space-y-6" data-testid="income-products-folder-view">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setIncomeFolderOpen(false)} data-testid="button-close-income-products">
              <ArrowLeft className="w-4 h-4 mr-1" /> All Projects
            </Button>
            <h2 className="text-xl font-bold flex items-center gap-2">
              <CircleDollarSign className="w-5 h-5 text-primary" /> Income Products
            </h2>
            <span className="text-sm text-muted-foreground">{incomeProductProjects.length} product{incomeProductProjects.length === 1 ? "" : "s"}</span>
          </div>
          {incomeProductProjects.length === 0 ? (
            <Card className="text-center py-12"><CardContent>
              <CircleDollarSign className="w-12 h-12 mx-auto mb-3 text-muted-foreground" />
              <p className="text-lg font-medium">No income products yet</p>
              <p className="text-muted-foreground text-sm mt-1">Move a Business Opportunity here when paid work starts.</p>
            </CardContent></Card>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {incomeProductProjects.map(renderProjectCard)}
            </div>
          )}
        </div>
      ...
      ) : groupFable && fableFolderOpen ? (
        <div className="space-y-6" data-testid="fable-5-folder-view">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setFableFolderOpen(false)} data-testid="button-close-fable-5-folder">
              <ArrowLeft className="w-4 h-4 mr-1" /> All Projects
            </Button>
            <h2 className="text-xl font-bold flex items-center gap-2">
              <Lightbulb className="w-5 h-5 text-primary" /> Fable 5 Money-Making Ideas
            </h2>
            <span className="text-sm text-muted-foreground">{fableProjects.length} workspace{fableProjects.length === 1 ? "" : "s"}</span>
          </div>
          <Card className="border-primary/20">
            <CardContent className="p-4 text-sm text-muted-foreground">
              Five owner-reviewed revenue paths. Open the parent folder for the idea briefs and focused working sessions, or open an individual project to continue its work.
            </CardContent>
          </Card>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {fableProjects
              .slice()
              .sort((a, b) => a.name.startsWith("5 Money") ? -1 : b.name.startsWith("5 Money") ? 1 : a.name.localeCompare(b.name))
              .map(renderProjectCard)}
          </div>
        </div>
      ) : filtered.length === 0 && !showFrontierFolder && !showFrontierResearchFolder && !showFableFolder ? (
        <Card className="text-center py-12">
          <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {showFableFolder && (
            <Card
              ...
              onClick={() => {
                setFrontierFolderOpen(false);
                setIdeaFolderOpen(false);
                setFrontierResearchFolderOpen(false);
                setFableFolderOpen(true);
              }}
              data-testid="card-fable-5-folder"
            >
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-lg leading-tight flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-xs">5 ideas</Badge>
                </div>
                <p className="text-sm text-muted-foreground line-clamp-2 mb-2">
                  Five active revenue workstreams with their own projects, supporting briefs, and focused working conversations.
                </p>
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                  <span className="flex items-center gap-1"><ChevronRight className="w-3 h-3" /> Open folder</span>
                  <span className="flex items-center gap-1 ml-auto"><FolderOpen className="w-3 h-3" /> {fableProjects.length} projects</span>
                </div>
              </CardContent>
            </Card>
          )}
          {isAdmin && statusFilter === "all" && !searchQuery && (
            <Card
              className="cursor-pointer hover:border-primary/50 transition-colors border-dashed"
              onClick={() => {
                setFrontierFolderOpen(false);
                setIdeaFolderOpen(false);
                setFrontierResearchFolderOpen(false);
                setFableFolderOpen(false);
                setIncomeFolderOpen(false);
                setOpportunityFolderOpen(true);
              }}
              data-testid="card-business-opportunities-folder"
            >
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-lg leading-tight flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-xs">{businessOpportunityProjects.length}</Badge>
                </div>
                <p className="text-sm text-muted-foreground line-clamp-2 mb-2">
                  Top-scoring IdeaBrowser ideas selected for business review. Historical entrants remain here until work starts making money.
                </p>
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                  <span className="flex items-center gap-1"><ChevronRight className="w-3 h-3" /> Open folder</span>
                  <span className="flex items-center gap-1 ml-auto"><Briefcase className="w-3 h-3" /> {businessOpportunityProjects.length} opportunities</span>
                </div>
              </CardContent>
            </Card>
          )}
          {isAdmin && statusFilter === "all" && !searchQuery && (
            <Card
              ...
              onClick={() => {
                ...
                setFrontierResearchFolderOpen(false);
                setFableFolderOpen(false);
                setOpportunityFolderOpen(false);
                setIncomeFolderOpen(true);
              }}
              data-testid="card-income-products-folder"
            >
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-lg leading-tight flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-xs">{incomeProductProjects.length}</Badge>
                </div>
                <p className="text-sm text-muted-foreground line-clamp-2 mb-2">
                  IdeaBrowser opportunities that have moved into paid work and income-producing delivery.
                </p>
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                  <span className="flex items-center gap-1"><ChevronRight className="w-3 h-3" /> Open folder</span>
                  <span className="flex items-center gap-1 ml-auto"><CircleDollarSign className="w-3 h-3" /> {incomeProductProjects.length} products</span>
                </div>
              </CardContent>
            </Card>
          )}
  ...
  function renderRevenueIdeaCard(idea: RevenueIdea) {
    const openingLabel = `${idea.slug}-brief`;
    return (
      <div key={idea.slug} className="rounded-lg border border-border/80 bg-card/70 p-4 space-y-3" data-testid={`card-revenue-idea-${idea.slug}`}>
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-semibold leading-tight">{idea.title}</h3>
          <Badge variant="outline" className="shrink-0 text-[11px]">{idea.validationState}</Badge>
        </div>
        <p className="text-sm text-muted-foreground">{idea.summary}</p>
        <p className="text-xs text-muted-foreground">
          <span className="font-medium text-foreground">Best first buyer:</span> {idea.targetBuyer}
        </p>
        <div className="flex flex-wrap gap-2 pt-1">
          <Button size="sm" className="min-h-11 px-4" onClick={() => startRevenueIdeaChat(idea)} data-testid={`button-start-revenue-idea-${idea.slug}`}>
            <PlayCircle className="w-4 h-4 mr-1" /> Start working
          </Button>
          {idea.linkedProjectId && idea.linkedProjectId !== selectedProject ? (
            <Button size="sm" variant="outline" className="min-h-11 px-4" onClick={() => setSelectedProject(idea.linkedProjectId!)} data-testid={`button-open-revenue-project-${idea.slug}`}>
          ...
          <Button
            ...
            onClick={() => openFrontierFile(idea.briefUrl, openingLabel)}
            disabled={!!openingFrontierFile}
            data-testid={`button-open-revenue-brief-${idea.slug}`}
          >
            {openingFrontierFile === openingLabel ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <FileText className="w-4 h-4 mr-1" />}
            Open brief
          </Button>
        </div>
      </div>
    ...
    return (
      <Card key={p.id} className="cursor-pointer hover:border-primary/50 transition-colors"
        ...
        <CardContent className="p-4">
          ...
          {isAdmin && isBusinessOpportunity(p) && (
            <Button
              size="sm"
              variant="outline"
              className="min-h-10 mt-1"
              disabled={incomeProductMutation.isPending}
              onClick={(event) => {
                event.stopPropagation();
                incomeProductMutation.mutate(p.id);
              }}
              data-testid={`button-move-income-product-${p.id}`}
            >
              <CircleDollarSign className="w-4 h-4 mr-1" /> Start making money
            </Button>
          )}
          {(p.tags || []).includes("wedge") && p.current_state && (
            <p className="text-xs text-foreground/80 line-clamp-2 mb-2" data-testid={`text-wedge-progress-${p.id}`}>
              {p.current_state.split("\n").filter(Boolean).at(-1)}
            </p>
          )}
          <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
