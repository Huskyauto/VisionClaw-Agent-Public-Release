...
function UsageTab() {
  ...
  // Task 130 — read-only model-switch savings suggestions (advisory only)
  const recsQuery = useQuery<any>({
    queryKey: ["/api/agent-insights/model-switch-recommendations", days],
    queryFn: async () => {
      const r = await fetch(`/api/agent-insights/model-switch-recommendations?windowDays=${Math.min(days, 90)}`, { credentials: "include" });
      if (!r.ok) throw new Error(await r.text());
      return r.json();
    },
    refetchInterval: 60000,
  });
  const recs: any[] = recsQuery.data?.recommendations || [];
    ...
    <div className="space-y-4 mt-4" data-testid="tab-usage-content">
      ...
      {data && (
        <>
          ...
          {recsQuery.data && recs.length === 0 && (
            <Card><CardContent className="pt-4 pb-4">
              <h3 className="font-medium text-sm mb-1 flex items-center gap-1.5">
                <Lightbulb className="w-4 h-4 text-amber-500" /> Model-switch savings
                <Badge variant="outline" className="text-[10px] ml-1">advisory</Badge>
              </h3>
              <p className="text-xs text-muted-foreground" data-testid="text-model-switch-empty">
                No switch opportunities detected yet. Suggestions appear once workflows have both recorded spend and observed quality scores on multiple models.
              </p>
            </CardContent></Card>
          )}
          ...
          {recs.length > 0 && (
            <Card><CardContent className="pt-4 pb-4">
              ...
              <p className="text-xs text-muted-foreground mb-3">
                Based on your real spend and observed quality scores over the last {recsQuery.data?.windowDays}d. Suggestions only — nothing is changed automatically.
              </p>
              <div className="space-y-2">
                {recs.map((r: any, i: number) => (
                  <div key={`${r.workflow}-${r.fromModel}-${r.toModel}`} className="text-xs border rounded-md p-2.5" data-testid={`row-model-switch-rec-${i}`}>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className="font-medium truncate">{r.workflow}</span>
                        <span className="font-mono text-muted-foreground truncate">{r.fromModel}</span>
                        <ArrowRight className="w-3 h-3 shrink-0 text-muted-foreground" />
                        <span className="font-mono truncate">{r.toModel}</span>
                      </div>
                      <span className="text-emerald-600 dark:text-emerald-400 font-semibold tabular-nums shrink-0">
                        −{r.estSavingsPct}% (${r.estSavingsUsd.toFixed(2)})
                      </span>
                    </div>
                    <div className="text-muted-foreground mt-1 tabular-nums">
                      {r.calls} calls · ${r.currentCostUsd.toFixed(2)} → ${r.projectedCostUsd.toFixed(2)} · quality {r.fromAvgQuality} → {r.toAvgQuality}
                      {" "}({r.qualityDelta >= 0 ? "+" : ""}{r.qualityDelta} pts, {r.toGradedSteps} graded steps)
                    </div>
                  </div>
                ))}
              </div>
