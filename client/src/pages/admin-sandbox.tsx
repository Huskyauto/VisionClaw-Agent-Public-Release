...
import { Loader2, FlaskConical, AlertTriangle, CheckCircle2, ShieldAlert, Play, Lightbulb, ThumbsUp, ThumbsDown } from "lucide-react";
...
interface ImprovementRow {
  ...
  jury_votes: Record<string, any> | null;
  status: string;
  created_at: string;
  decided_at: string | null;
}

function improvementStatusBadge(status: string, verdict: string | null) {
  if (status === "approved") return <Badge className="bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/40" variant="outline">jury approved{verdict === "FIX" ? " (with fix)" : ""}</Badge>;
  if (status === "escalated") return <Badge className="bg-amber-500/15 text-amber-700 dark:text-amber-400 border-amber-500/40" variant="outline">escalated</Badge>;
  if (status === "jury_pending") return <Badge variant="secondary"><Loader2 className="h-3 w-3 mr-1 animate-spin" /> jury pending</Badge>;
  if (status === "implemented") return <Badge variant="secondary"><CheckCircle2 className="h-3 w-3 mr-1" /> implemented</Badge>;
...
export default function AdminSandboxPage() {
  ...
  const { data: improvementsData } = useQuery<{ improvements: ImprovementRow[] }>({
    queryKey: ["/api/admin/sandbox/improvements"],
  });

  const decide = useMutation({
    mutationFn: async ({ id, status }: { id: number; status: "implemented" | "rejected" }) => {
      const res = await apiRequest("PATCH", `/api/admin/sandbox/improvements/${id}`, { status });
      return res.json();
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/admin/sandbox/improvements"] }),
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="page-admin-sandbox">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            {corpus === "safety" ? (
              ...
              <div className="space-y-1">
                ...
                <Input placeholder="e.g. gpt-5.4" value={model} onChange={(e) => setModel(e.target.value)} data-testid="input-model" />
              </div>
            )}
      ...
      <Card data-testid="card-improvements">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Lightbulb className="h-5 w-5 text-primary" /> Improvement list</CardTitle>
          <CardDescription>
            Jury-vetted upgrade proposals promoted from sandbox runs (via the sandbox_promote tool). Nothing here auto-applies — pick a proposal and mark it implemented once adopted, or reject it.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!improvementsData?.improvements?.length ? (
            <div className="text-sm text-muted-foreground" data-testid="text-no-improvements">No promoted improvements yet. Run a sandbox experiment, then promote a winning configuration to the jury.</div>
          ) : (
            <div className="space-y-2">
              {improvementsData.improvements.map((imp) => (
                <div key={imp.id} className="border rounded-md p-3 space-y-1" data-testid={`row-improvement-${imp.id}`}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="font-mono text-xs text-muted-foreground">#{imp.id}</span>
                      <span className="font-medium truncate" data-testid={`text-improvement-title-${imp.id}`}>{imp.title}</span>
                      {imp.run_id != null && <span className="text-xs text-muted-foreground shrink-0">run #{imp.run_id}</span>}
                    </div>
                    {improvementStatusBadge(imp.status, imp.jury_verdict)}
                  </div>
                  <div className="text-xs text-muted-foreground" data-testid={`text-improvement-summary-${imp.id}`}>{imp.summary}</div>
                  {imp.jury_votes?.fixProposal && (
                    <div className="text-xs border-l-2 border-primary/40 pl-2 text-muted-foreground">Jury fix note: {String(imp.jury_votes.fixProposal).slice(0, 400)}</div>
                  )}
                  {(imp.status === "approved" || imp.status === "escalated") && (
                    <div className="flex gap-2 pt-1">
                      <Button size="sm" variant="outline" disabled={decide.isPending} onClick={() => decide.mutate({ id: imp.id, status: "implemented" })} data-testid={`button-implement-${imp.id}`}>
                        <ThumbsUp className="h-3 w-3 mr-1" /> Mark implemented
                      </Button>
                      <Button size="sm" variant="outline" disabled={decide.isPending} onClick={() => decide.mutate({ id: imp.id, status: "rejected" })} data-testid={`button-reject-${imp.id}`}>
