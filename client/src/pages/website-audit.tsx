...
export default function WebsiteAuditPage() {
  ...
  async function downloadQuickPdf() {
    ...
    setQuickError(null);
    try {
      const res = await authFetch("/api/admin/audit/quick-pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(quickResult),
      });
      if (!res.ok) {
      ...
      const fileName = disposition.match(/filename="([^"]+)"/)?.[1] || "quick-website-audit.pdf";
      ...
      setQuickError(e?.message || "PDF download failed");
    } finally {
          ...
          {quickResult && (
            <div className="space-y-4" data-testid="section-quick-result">
              ...
              <Button
                ...
                data-testid="button-download-quick-pdf"
              >
                {quickPdfLoading
                  ? <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  : <Download className="h-4 w-4 mr-2" />}
                Download Quick Audit PDF
              </Button>
            </div>
          )}
        </CardContent>
