...
export function HomeReleaseUpdates({ releaseExpanded, toggleRelease }: {
  ...
  return (
    <>
        {/* R125+155.5+sec (2026-09-03) — NEW (rose): reliable supplied-roster SAM routing. */}
        <button
        ...
        >
          <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-sm bg-rose-600 text-white leading-none shrink-0 mt-0.5">R125+155.5+sec NEW</span>
          <div className="min-w-0 flex-1">
            <div className={`text-sm font-semibold leading-tight ${releaseExpanded.has("banner-whats-new-r125_155_5_sec") ? "" : "line-clamp-2"}`}>{"R125+155.5+sec — Reliable supplied-roster SAM routing. Exact, supplied-batch, and discovery requests now stay on their correct authoritative path across main, engine, and public chat."}</div>
            <div className={`text-xs text-muted-foreground mt-0.5 ${releaseExpanded.has("banner-whats-new-r125_155_5_sec") ? "" : "truncate"}`}>{"CRITICAL: mandatory tools survive production caps. HIGH: server-owned dispatch locks block wrong modes, introspection, and web/delegation fallbacks. Tier labels persist; 56 focused checks plus final 28/28 passed."}</div>
          </div>
