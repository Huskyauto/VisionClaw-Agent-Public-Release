...
export function HomeReleaseUpdates({ releaseExpanded, toggleRelease }: {
  ...
  return (
    <>
        {/* R125+155.20 (2026-09-10) — NEW (cyan): atomic Muse Revenue Orchestrator S2 mission adapter. */}
        <button
          onClick={() => toggleRelease("banner-whats-new-r125_155_20")}
          ...
          data-testid="banner-whats-new-r125_155_20"
        >
          <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-sm bg-cyan-600 text-white leading-none shrink-0 mt-0.5">R125+155.20 NEW</span>
          <div className="min-w-0 flex-1">
            <div className={`text-sm font-semibold leading-tight ${releaseExpanded.has("banner-whats-new-r125_155_20") ? "" : "line-clamp-2"}`}>{"Atomic Muse Revenue Orchestrator S2 mission adapter — structured opportunities normalize into one Verified Revenue Mission with no parallel store."}</div>
            <div className={`text-xs text-muted-foreground mt-0.5 ${releaseExpanded.has("banner-whats-new-r125_155_20") ? "" : "truncate"}`}>{"Dry-run eligible · 25/25/3 caps · hypothesis-only · tenant+key isolation · composite FKs · race-safe semantics · one real database integration covering all 15 behavioral groups · 9 focused tests/build/preflight/wiring checks passed · architect and silent-failure passes clean. Production publish pending explicit owner approval."}</div>
          </div>
          <ChevronDown className={`w-4 h-4 text-muted-foreground shrink-0 mt-0.5 transition-transform ${releaseExpanded.has("banner-whats-new-r125_155_20") ? "rotate-180" : ""}`} />
        </button>
        {/* R125+155.19 (2026-09-09) — DEMOTED (cyan): bounded Human-AI Synergy Trial. */}
        <button
          onClick={() => toggleRelease("banner-whats-new-r125_155_19")}
          className="w-full text-left rounded-lg bg-gradient-to-r from-cyan-500/10 via-primary/5 to-transparent border border-cyan-500/30 hover:border-cyan-500/50 hover:bg-cyan-500/15 transition-colors px-4 py-3 flex items-start gap-3"
          data-testid="banner-whats-new-r125_155_19"
        >
          <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-sm bg-cyan-600 text-white leading-none shrink-0 mt-0.5">R125+155.19 Historical</span>
          <div className="min-w-0 flex-1">
            <div className={`text-sm font-semibold leading-tight ${releaseExpanded.has("banner-whats-new-r125_155_19") ? "" : "line-clamp-2"}`}>{"Bounded Human-AI Synergy Trial — owners can run a deterministic, report-only three-arm comparison for one task: human-alone, AI-alone, and human+AI."}</div>
            <div className={`text-xs text-muted-foreground mt-0.5 ${releaseExpanded.has("banner-whats-new-r125_155_19") ? "" : "truncate"}`}>{"Owner-only and tenant-scoped · zero model calls and zero inference spend · kill-switchable · observational only: no IQ, hiring, causal, autonomous, or generalization claims. Opportunity Bank: 21 ideas; Human-AI Synergy Audit remains an unvalidated Idea."}</div>
          </div>
          <ChevronDown className={`w-4 h-4 text-muted-foreground shrink-0 mt-0.5 transition-transform ${releaseExpanded.has("banner-whats-new-r125_155_19") ? "rotate-180" : ""}`} />
        </button>
        {/* R125+155.18+sec (2026-09-09) — NEW (indigo): durable opportunity discovery. */}
        <button
          onClick={() => toggleRelease("banner-whats-new-r125_155_18")}
          ...
          data-testid="banner-whats-new-r125_155_18"
        >
          <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-sm bg-indigo-600 text-white leading-none shrink-0 mt-0.5">R125+155.18+sec Historical</span>
          <div className="min-w-0 flex-1">
            <div className={`text-sm font-semibold leading-tight ${releaseExpanded.has("banner-whats-new-r125_155_18") ? "" : "line-clamp-2"}`}>{"Durable full-archive opportunity review — IdeaBrowser now screens every scored idea, deeply simulates only a bounded top-30 shortlist, and cannot report success until the final report survives in its run project, Drive, and the verified owner inbox."}</div>
            <div className={`text-xs text-muted-foreground mt-0.5 ${releaseExpanded.has("banner-whats-new-r125_155_18") ? "" : "truncate"}`}>{"309 ideas reviewed · 30 finalists · 27 valid scenarios · 5 distinct winners added · Opportunity Bank now contains 20 ideas."}</div>
          </div>
          <ChevronDown className={`w-4 h-4 text-muted-foreground shrink-0 mt-0.5 transition-transform ${releaseExpanded.has("banner-whats-new-r125_155_18") ? "rotate-180" : ""}`} />
        </button>

        {/* R125+155.9+sec (2026-09-06) — DEMOTED (rose): evidence-backed owner decisions. */}
        <button
