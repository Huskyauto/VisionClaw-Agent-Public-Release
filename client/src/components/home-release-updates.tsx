...
export function HomeReleaseUpdates({ releaseExpanded, toggleRelease }: {
  ...
  return (
    <>
        {/* R125+155.4 (2026-09-03) — NEW (sky): reliable SAM reports and Gemini routing. */}
        <button
        ...
        >
          <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-sm bg-sky-600 text-white leading-none shrink-0 mt-0.5">R125+155.4 NEW</span>
          <div className="min-w-0 flex-1">
            <div className={`text-sm font-semibold leading-tight ${releaseExpanded.has("banner-whats-new-r125_155_4") ? "" : "line-clamp-2"}`}>{"R125+155.4 — Reliable SAM reports and Gemini 3.7 routing. SAM.gov reports now keep a verified durable database copy before Drive delivery, retired Gemini 3.5 routes normalize to 3.7, and Wikipedia article misses return a normal result."}</div>
            <div className={`text-xs text-muted-foreground mt-0.5 ${releaseExpanded.has("banner-whats-new-r125_155_4") ? "" : "truncate"}`}>{"Actual-model context recovery, bounded object-storage fallbacks, 52 focused regressions, all 247 Node suites effectively green, and final independent architect plus silent-failure reviews passed."}</div>
          </div>
