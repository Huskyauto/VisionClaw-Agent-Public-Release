...
function NavLink({ path, icon: Icon, label, badge }: { path: string; icon: any; label: string; badge?: string }) {
    ...
    <SidebarMenuItem>
      ...
      >
        <a href={path} onClick={(e) => { e.preventDefault(); navigate(path); setOpenMobile(false); }}>
          ...
          {badge && (
            <span
              className="ml-auto text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-sm bg-primary/15 text-primary leading-none"
      ...
      <SidebarFooter className="border-t border-sidebar-border p-2 max-h-[40vh] overflow-y-auto">
        ...
        {isAdmin && (
          <NavSection title="Admin" icon={Shield} defaultOpen={["/heartbeat", "/agentic", "/payments", "/settings", "/compare", "/admin/service-orders", "/admin/tools", "/operator"].some(p => location === p)}>
            ...
            <NavLink path="/admin/ecosystem-health" icon={Activity} label="Ecosystem Health" badge="R98.25" />
            <NavLink path="/admin/persona-cost" icon={DollarSign} label="Per-Agent Cost" badge="R98.26" />
            <NavLink path="/activity?filter=security" icon={ShieldCheck} label="Security Audit" badge="R117.1+sec" />
