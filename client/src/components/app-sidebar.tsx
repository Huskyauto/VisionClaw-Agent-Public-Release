...
function NavSection({ title, icon: Icon, defaultOpen, children }: { title: string; icon: any; defaultOpen: boolean; children: React.ReactNode }) {
  ...
  return (
    <div className="mt-1">
      <button
        className="flex items-center gap-2 w-full px-2 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors rounded-md hover:bg-sidebar-accent"
...
function NavLink({ path, icon: Icon, label }: { path: string; icon: any; label: string }) {
  ...
  return (
    <Sidebar className="border-r border-sidebar-border">
      ...
      <SidebarFooter className="border-t border-sidebar-border p-2 max-h-[40vh] overflow-y-auto">
        ...
        {isAdmin && (
          <NavSection title="Admin" icon={Shield} defaultOpen={["/heartbeat", "/agentic", "/payments", "/settings", "/compare"].some(p => location === p)}>
