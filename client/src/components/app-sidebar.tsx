...
import { INCOME_OPPORTUNITIES } from "@/data/income-opportunities";
...
function NavSection({ title, icon: Icon, defaultOpen, children, accent, badge, touchFriendly = false }: { title: string; icon: any; defaultOpen: boolean; children: React.ReactNode; accent?: boolean; badge?: string; touchFriendly?: boolean }) {
...
function NavLink({ path, icon: Icon, label, badge, badgeClass, touchFriendly = false }: { path: string; icon: any; label: string; badge?: string; badgeClass?: string; touchFriendly?: boolean }) {
...
export function AppSidebar() {
  ...
  const isPlatformAdmin = tenant?.id === 1 && tenant?.isAdmin === true;
  const humanAiSynergyTrialEnabled = import.meta.env.VITE_HUMAN_AI_SYNERGY_TRIAL_ENABLED === "1";
  ...
  const { data: setupStatus } = useQuery<{
    ...
    typesafeJev?: { configured: boolean; enabled: boolean; ready: boolean };
  }>({
  ...
  return (
    <Sidebar className="border-r border-sidebar-border">
      ...
      <SidebarFooter className="border-t border-sidebar-border p-2 max-h-[40vh] overflow-y-auto">
        ...
        <NavSection title="Tools" icon={Wrench} defaultOpen={["/skills", "/skills-marketplace", "/personality-files", "/content-writing", "/analytics", "/email", "/whatsapp", "/whatsapp-approval", "/telegram", "/mcp", "/webhook-triggers", "/channel-routing", "/admin/tools/typesafe-jev"].some(p => location === p)}>
            ...
            {isAdmin && <NavLink
              path="/admin/tools/typesafe-jev"
              icon={FlaskConical}
              label={setupStatus?.typesafeJev?.ready
                ? "TypeSafe Jev · Ready"
                : setupStatus?.typesafeJev?.configured
                  ? "TypeSafe Jev · Disabled"
                  : "TypeSafe Jev · Awaiting API key"}
            />}
          </NavSection>
          ...
          <SidebarMenuItem>
            ...
            >
              <a href="/updates" onClick={(e) => { e.preventDefault(); navigateTo("/updates"); }}>
                ...
                <span className="ml-auto px-1.5 py-0.5 text-[10px] font-bold rounded-sm bg-indigo-600 text-white leading-none" data-testid="badge-updates-new">
                  R130.6
                </span>
              </a>
            </SidebarMenuButton>
        ...
        <NavSection title="Organization" icon={Users} defaultOpen={["/team", "/api-keys", "/activity", "/agent-board", "/agent-scorecards", "/capability-map", "/replay"].some(p => location === p)}>
          <NavLink path="/agent-board" icon={Bot} label="Agent Board" />
          <NavLink path="/agent-diagram" icon={Network} label="Live Agent Diagram" />
          <NavLink path="/agent-scorecards" icon={Gauge} label="Agent Scorecards" badge="NEW" />
          <NavLink path="/capability-map" icon={Network} label="Capability Map" badge="NEW" />
          <NavLink path="/replay" icon={Activity} label="Workflow Replay" badge="NEW" />
        ...
        {isAdmin && (
            <NavSection title="Income Opportunities" icon={Lightbulb} defaultOpen={[ "/admin/income-opportunities", "/income-opportunities/ai-task-fit-audit", "/admin/research-frontier" ].includes(location)} accent badge="NEW" touchFriendly>
            <NavLink path="/admin/income-opportunities" icon={Sparkles} label="Opportunity Bank" badge={`${INCOME_OPPORTUNITIES.length} IDEAS`} touchFriendly />
              {isPlatformAdmin && <NavLink path="/income-opportunities/ai-task-fit-audit" icon={ClipboardCheck} label="AI Task Fit & Human Agency Audit" badge="ENABLED" touchFriendly />}
        ...
        {isAdmin && (
           <NavSection title="Income Products" icon={DollarSign} defaultOpen={["/admin/website-audit", "/audit", "/admin/research-report", "/admin/digital-products", "/admin/payment-links", "/admin/archive-rescue", "/admin/wedges", "/admin/revenue-missions", "/admin/service-orders", "/admin/executive-report", "/admin/smart-leads", "/admin/cmmc", "/admin/missed-call-recovery"].some(p => location === p)} accent badge="$$">
            <NavLink path="/admin/payment-links" icon={DollarSign} label="Get Paid" badge="NEW" />
            <NavLink path="/admin/website-audit" icon={Gauge} label="Website Audit ($497)" badge="FREE RUN" />
            <NavLink path="/admin/executive-report" icon={Gauge} label="Company Reports" badge="NEW" />
            <NavLink path="/admin/cmmc" icon={ShieldCheck} label="CMMC Level 1 / FCI" badge="NEW" />
             {config.hvacMissedCallRecoveryEnabled && <NavLink path="/admin/missed-call-recovery" icon={Phone} label="HVAC Missed-Call Recovery" badge="NEW" />}
            <NavLink path="/admin/smart-leads" icon={Users} label="Smart Leads ($99)" badge="NEW" />
            <NavLink path="/audit" icon={Gauge} label="AI Site Review ($1,997)" badge="DFY" />
            <NavLink path="/admin/research-report" icon={FileText} label="Research Report ($49)" badge="FREE RUN" />
            <NavLink path="/admin/digital-products" icon={Package} label="Digital Products" badge="FREE RUN" />
            <NavLink path="/admin/archive-rescue" icon={Archive} label="Archive Rescue Orders" />
            <NavLink path="/admin/wedges" icon={Activity} label="Active Wedges" />
            <NavLink path="/admin/revenue-missions" icon={DollarSign} label="Revenue Missions" />
            <NavLink path="/admin/service-orders" icon={Package} label="Customer Orders" />
          </NavSection>
        )}

        {isAdmin && (
          <NavSection title="Admin" icon={Shield} defaultOpen={["/heartbeat", "/agentic", "/payments", "/settings", "/compare", "/admin/tools", "/operator"].some(p => location === p)}>
            ...
            <NavLink path="/admin/ecosystem-health" icon={Activity} label="Ecosystem Health" badge="R125+72" />
            <NavLink path="/admin/goal-ledger" icon={Activity} label="Goal Ledger" badge="R125+5" />
            <NavLink path="/admin/sandbox" icon={FlaskConical} label="Simulation Sandbox" badge="NEW" />
            ...
            <NavLink path="/activity?filter=security" icon={ShieldCheck} label="Security Audit" badge="R130.6" badgeClass="ml-auto text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-sm bg-indigo-600 text-white leading-none" />
