...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R132.5 lets Jev advisory judgments consume bounded named JSON state with descriptor-safe validation, secret prohibition, and personal-data minimization guidance. R132.4 adds fail-closed streaming evidence to expert model-routing research. Current platform: 418 tools, 178 declared / 258 live tables, 825 live indexes (573 non-PK), 138 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R132.5 gives Jev bounded structured state with descriptor-safe validation plus explicit secret and personal-data handling guidance."
