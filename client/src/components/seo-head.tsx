...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R125+155.7+sec: Deploy an 18-agent AI team with 417 tools, 135 capabilities, and 242 live tables. A shadow-first relational context sidecar connects typed memory evidence on complex turns without recursive model calls, while tenant-scoped endpoints, structural prompt-data encoding, hard packet limits, and database deadlines preserve safety.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R125+155.7+sec: shadow-first relational memory context with tenant-scoped graph edges, structurally encoded recalled data, hard packet limits, and database-enforced deadlines."
