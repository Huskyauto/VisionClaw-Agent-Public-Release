...
export function SeoHead({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const fallback = "Deploy an 18-agent AI team that runs autonomous corporate operations end to end. Run a live Instant AI Readiness Audit at /audit — score any website /100 across AI access, structured data, metadata, social, and technical health into an A–F grade with concrete recommendations ($497 self-serve / $1,997 done-for-you). Multi-layered safety (AHB intent gates + destructive-tool policy + crisis safety guard), strict per-tenant isolation that fails closed, a hardened SSRF jail on every outbound fetch, MoA jury concordance for HITL routing, deterministic deliverable pipelines, instant-play media delivery. Live platform stats + current release notes at /trust.";
