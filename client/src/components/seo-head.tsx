...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R132.4 adds fail-closed TTFT, stream-duration, and output-throughput evidence to expert model-routing research using the existing candidate call and tenant-scoped accounting. R132.3+sec requires complete staged, ablated, tenant-ledgered evidence before verification. Current platform: 418 tools, 178 declared / 258 live tables, 825 live indexes (573 non-PK), 138 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R132.4 measures TTFT and streaming throughput in expert model-routing research with no extra model calls and fail-closed accounting."
