...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R130.3 adds a waitlist-ready TypeSafe Jev advisory integration for Felix and Minerva: fixed jev-latest endpoint, one bounded no-retry request, typed Noul/Choice/Score output, and exact secret plus enable gates. Current platform: 418 tools, 177 declared / 255 live tables, 806 live indexes (557 non-PK), 137 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules. No live provider call, deployment, or activation occurred.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R130.3 TypeSafe Jev advisory integration: waitlist-ready, typed, bounded, no-retry, and restricted to Felix and Minerva."
