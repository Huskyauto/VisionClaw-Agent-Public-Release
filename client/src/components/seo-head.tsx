...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R132 keeps each approved repair attached to one unresolved identity, reconciles signed status across split production/workspace stores, wires Felix, Minerva, and Forge to one lifecycle, and routes source findings through a signed workspace proposal with explicit owner-only publish. Latest review closed split-store/UI/routing/wiring gaps; final review remains pending. R131+sec2 restores reliable Stripe webhook delivery with single-pass signature verification and renewable, token-fenced replay claims. Current platform: 418 tools, 180 declared / 258 live tables, 823 live indexes (572 non-PK), 138 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R132 closes approved repairs through signed workspace proposals and explicit owner-only publish; production source edits remain blocked."
