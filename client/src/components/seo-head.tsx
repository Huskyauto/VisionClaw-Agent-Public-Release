...
export function SeoHead({
  ...
  const currentDescription = description.replace(/\b413 tools\b/g, "414 tools");
  const currentOgDescription = ogDescription?.replace(/\b413 tools\b/g, "414 tools");
  const effectiveDescription = useCurrentPlatformDescription
    ? `R125+155.2+sec: Deploy an 18-agent AI team with 414 tools, 135 capabilities, and 155 reference surfaces. ${pName} protects trusted operational runbooks with reserved provenance and persona-scoped retrieval, while R125+155.2 atomically claims CMMC reports before Drive or email delivery and preserves completed assessment evidence.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R125+155.2+sec: 18 agents, 414 tools, 135 capabilities, and 155 reference surfaces—trusted runbooks are provenance-protected and persona-scoped, while CMMC reports are claimed before Drive or email side effects."
