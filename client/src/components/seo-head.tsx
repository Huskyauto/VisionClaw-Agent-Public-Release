...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R125+155.2+sec2: Deploy an 18-agent AI team with 414 tools, 135 capabilities, and 235 live tables. ${pName} now preserves tenant-scoped, sanitized recovery evidence for inspection without ever resuming or replaying work.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R125+155.2+sec2: 18 agents, 414 tools, 135 capabilities, and 235 live tables—tenant-safe recovery evidence is inspect-only and never resumes or replays work."
