...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `Deploy an 18-agent AI team with 413 tools, 134 capabilities, and 155 reference surfaces. ${pName} keeps frontier revenue concepts and raw jury research owner-only, separate, and research-only; validates saved reports before display; protects account transitions, tenant analysis, commerce links, and password-reset credentials; and requires approval for high-risk actions.`
    : description;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "An AI team that researches, writes, builds, and delivers — with owner-only frontier concepts and jury research, strict saved-report validation, tenant-bound account and commerce protections, and human approval for high-risk actions."
