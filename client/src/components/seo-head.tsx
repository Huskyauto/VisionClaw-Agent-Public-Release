...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R125+155.5+sec: Deploy an 18-agent AI team with 417 tools, 135 capabilities, and 242 live tables. Supplied SAM.gov Tier and company rosters now route through paced authoritative batch lookup, while exact-company and new-roster requests stay on their dedicated paths. A server-owned execution lock blocks wrong SAM modes and non-authoritative fallbacks across every chat entry point.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R125+155.5+sec: reliable exact, supplied-roster, and discovery SAM.gov routing with cap-protected tools and server-enforced authoritative-source locks."
