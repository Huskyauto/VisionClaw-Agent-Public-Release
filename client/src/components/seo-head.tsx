...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R128+sec: deterministic report-only delegation supervision records bounded tenant-scoped receipts before every known child-agent execution family. Owner-direct authority requires authenticated platform-owner server context and cannot bypass structural security gates. Current platform: 417 tools, 137 capabilities, 18 personas, 169 declared / 247 live tables, 777 indexes, 68 skills, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R128+sec: deterministic report-only delegation supervision with tenant-scoped receipts, unforgeable owner-direct authority, and preserved structural security gates."
