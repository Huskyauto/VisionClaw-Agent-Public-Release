...
export function SeoHead({
  ...
  const currentDescription = description.replace(/\b(?:413|414|415|416|417|418) tools\b/g, "420 total registered tools");
  const currentOgDescription = ogDescription?.replace(/\b(?:413|414|415|416|417|418) tools\b/g, "420 total registered tools");
  const effectiveDescription = useCurrentPlatformDescription
    ? `R135.4+sec requires a tenant- and content-bound deterministic verifier receipt before research code proposals can modify source. Current platform: 420 tools, 182 declared / 262 live tables, 839 platform indexes (583 non-PK), 141 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R135.4+sec gates consequential code apply on deterministic, content-bound verifier receipts."
