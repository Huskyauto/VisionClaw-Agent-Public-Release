...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R130.6 adds fail-closed structural validation before executable plan steps run, rejecting malformed identities, dependencies, and cycles while preserving valid safety and ordering barriers. R130.5 adds a fixed-payload, no-persistence Union Alpha synthetic evaluation lane excluded from production model surfaces. Current platform: 418 tools, 177 declared / 255 live tables, 806 live indexes (557 non-PK), 137 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R130.6 rejects malformed executable plan graphs before any step runs while preserving valid ordering barriers."
