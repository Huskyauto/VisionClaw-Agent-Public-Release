...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `Tenant-authority closure and explicit video delivery. R128+sec2 closes equal-tenant relationship backfill, authoritative persisted research-job tenants, mandatory tenant scope for memory and heartbeat mutations, strict webhook and capability handling, and opt-in Drive uploads. Current platform: 417 tools, 137 capabilities, 18 personas, 169 declared / 247 live tables, 777 indexes, 68 skills, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "Tenant-authority closure and explicit video delivery: tenant-scoped mutations, authoritative research jobs, strict webhook/capability handling, and opt-in Drive uploads."
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const fallback = "Tenant-authority closure and explicit video delivery. Deploy an 18-agent AI team that runs autonomous corporate operations end to end, with strict fail-closed tenant isolation, scoped mutations, managed skills validated on write and again before prompt injection, approval gates, deterministic deliverable pipelines, and opt-in Drive delivery. Live platform stats + current release notes at /trust.";
