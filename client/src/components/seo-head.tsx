...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R125+155.20: Muse Revenue Orchestrator S2 atomic mission adapter with dry-run eligibility, bounded caps, hypothesis/autonomy 0, tenant+key isolation, composite FKs, and race-safe semantics; final architect and silent-failure passes are clean. Current platform: 417 tools, 137 capabilities, 18 personas, 167 declared / 244 live tables, 756 indexes, 67 skills, 41 governance rules, and 80 curated models.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R125+155.20: Muse Revenue Orchestrator S2 atomic mission adapter; bounded, tenant-isolated, dry-run eligible, and production publish pending owner approval."
