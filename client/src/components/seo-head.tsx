...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R130.7+sec adds admin-verifiable, read-only improvement evidence with fixed tenant scope, bounded work, and no stored-label or candidate-content exposure. R130.6 adds fail-closed structural validation before executable plan steps run. Current platform: 418 tools, 177 declared / 255 live tables, 806 live indexes (557 non-PK), 137 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R130.7+sec gives platform admins bounded, privacy-safe evidence of improvement without expanding autonomy."
