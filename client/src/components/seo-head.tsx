...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R125+155.4: Deploy an 18-agent AI team with 417 tools, 135 capabilities, and 242 live tables. SAM.gov reports now persist verified durable bytes before Drive delivery, retired Gemini 3.5 routes normalize to Gemini 3.7, context recovery reports actual remaps, and Wikipedia article misses return normal results. Existing free-only remapping and spend controls remain unchanged.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R125+155.4: reliable SAM.gov report recovery, active Gemini 3.7 routing, truthful context-remap reporting, and normal Wikipedia article-miss results with existing spend controls unchanged."
