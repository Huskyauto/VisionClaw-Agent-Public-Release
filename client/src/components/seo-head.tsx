...
export function SeoHead({
  ...
  const effectiveDescription = useCurrentPlatformDescription
    ? `R131+sec adds a tenant-scoped append-only loop outcome graph, evidence-bound convergence, advisory allocation, and bounded report-only Dream-RSI historical replay with no authority expansion. Current platform: 418 tools, 179 declared / 257 live tables, 815 live indexes (564 non-PK), 138 capabilities, 68 total platform skills, 107 reference surfaces, 18 personas, 41 governance rules.`
    : currentDescription;
  const effectiveOgDescription = useCurrentPlatformDescription
    ? "R131+sec adds governed loop evidence and bounded Dream-RSI historical replay without expanding autonomy."
