...
import { buildCustomToolArgsSetup } from "./lib/custom-tool-args";
...
async function tryAndRevert(tool: any): Promise<{ status: "kept" | "reverted" | "fixed"; passed: number; failed: number; failures: string[] }> {
  ...
  try {
    ...
    // Fail CLOSED: an unvalidatable tool must not be kept (post-edit review
    // 2026-07-10 — "kept without validation" was a fail-open acceptance of
    // arbitrary sandboxed tool logic). The revert must actually DELETE the
    // just-inserted row — a "reverted" status alone leaves the tool active.
    console.log(`[tool-learning] Test case generation failed: ${err.message} — reverting (fail closed)`);
    const failures = [`Test generation failed: ${err.message} — tool reverted, cannot validate`];
    await revertToolRow(tool, failures);
    return { status: "reverted", passed: 0, failed: 0, failures };
  }

  if (testCases.length === 0) {
    console.log(`[tool-learning] No test cases generated for "${tool.name}" — reverting (fail closed)`);
    const failures = ["No test cases generated — tool reverted, cannot validate"];
    await revertToolRow(tool, failures);
    return { status: "reverted", passed: 0, failed: 0, failures };
  }
  ...
  for (const tc of testCases) {
    const argsSetup = buildCustomToolArgsSetup(tc.args || {});
  ...
  await revertToolRow(tool, failures, passed, failed);
  return { status: "reverted", passed, failed, failures };
}

/** Delete the just-inserted custom tool row and log the revert experiment.
 *  Every "reverted" return path MUST call this — a "reverted" status without
 *  the DELETE leaves unvalidated tool code active in custom_tools. */
async function revertToolRow(tool: any, failures: string[], passed = 0, failed = 0) {
...
export async function executeCustomTool(name: string, args: Record<string, any>, tenantId: number): Promise<any> {
  ...
  const argsSetup = buildCustomToolArgsSetup(args);

  const code = `const args = {};\n${argsSetup}\n${tool.implementation}`;
  const result = executeCode(code);

  const incremented = await db.update(customTools)
    .set({ usageCount: sql`${customTools.usageCount} + 1` })
    .where(and(
      eq(customTools.id, tool.id),
      eq(customTools.tenantId, tenantId),
    ))
    .returning({ id: customTools.id });
  if (incremented.length !== 1) {
    throw new Error(`Custom tool usage accounting failed for tenant ${tenantId}`);
  }
