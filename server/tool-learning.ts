...
export async function createCustomTool(description: string): Promise<{ tool?: CustomToolDef; error?: string; validation?: any }> {
  try {
    ...
    parsed.name = parsed.name.toLowerCase().replace(/[^a-z0-9_]/g, "_").slice(0, 50);
...
export async function executeCustomTool(name: string, args: Record<string, any>): Promise<any> {
  const [tool] = await db.select().from(customTools).where(eq(customTools.name, name));
  if (!tool) {
    return { error: `Custom tool "${name}" not found` };
  }
  if (!tool.isActive) {
    return { error: `Custom tool "${name}" is disabled` };
  }
...
export async function listCustomTools(): Promise<CustomToolDef[]> {
  const tools = await db.select().from(customTools);
  return tools.map(t => ({
    id: t.id,
    name: t.name,
...
export async function deleteCustomTool(name: string): Promise<{ success: boolean; error?: string }> {
  ...
  await db.delete(customTools).where(eq(customTools.id, tool.id));
  console.log(`[tool-learning] Deleted custom tool: ${name}`);
  return { success: true };
}

export async function getCustomToolDefinitions(): Promise<ToolDefinition[]> {
  const tools = await db.select().from(customTools).where(eq(customTools.isActive, true));
  return tools.map(t => {
