...
export async function critiqueResponse(
  ...
  try {
    ...
    if (score < CRITIQUE_THRESHOLD && parsed.needs_refinement && issues.length > 0) {
      try {
        const refined = await refineResponse(userMessage, assistantResponse, issues, tenantId);
...
function resolveTenantId(explicitTenantId?: number): number {
  ...
  if (!Number.isInteger(tenantId) || (tenantId as number) <= 0) {
    throw new Error("critique agent requires an authenticated tenant context");
  }
