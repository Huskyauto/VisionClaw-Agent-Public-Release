...
export async function connectGoogleViaReplit(tenantId: number): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    for (const env of envOrder) {
      // Primary path: filtered query with secrets.
      ...
      if (resp.ok) {
        const data = await resp.json();
        if (data?.items?.[0]) { conn = data.items[0]; console.log(`[oauth] google-drive connector found via PRIMARY filtered query (env=${env})`); break; }
      }

      // Fallback (R125+137.20): the connector API can return ZERO items for the
      // filtered include_secrets=true query while the UNFILTERED list still
      // carries the connection WITH oauth credentials (observed 2026-07-14 —
      // google-drive lived only in the production env and only the unfiltered
      // list returned it). Match by connector_name and require a real token.
      const listUrl = new URL(`https://${hostname}/api/v2/connection`);
      listUrl.searchParams.set('include_secrets', 'false');
      listUrl.searchParams.set('environment', env);
      const listResp = await fetch(listUrl.toString(), {
        headers: { 'Accept': 'application/json', 'X-Replit-Token': xReplitToken },
      });
      if (!listResp.ok) continue;
      const listData = await listResp.json();
      const candidate = (listData?.items || []).find((i: any) =>
        i?.connector_name === 'google-drive' && !i?.disabled &&
        typeof i?.settings?.oauth?.credentials?.access_token === 'string' &&
        i.settings.oauth.credentials.access_token.length >= 20
      );
      if (candidate) { conn = candidate; console.log(`[oauth] google-drive connector found via FALLBACK unfiltered list (env=${env})`); break; }
    }
