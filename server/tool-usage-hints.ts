...
export const TOOL_USAGE_HINTS: Record<string, ToolUsageHint> = {
  ...
  derived_api_replay: {
    useWhen: [
      "re-fetch data from a site we already browsed, without opening a browser",
      "call a saved API recipe directly for speed",
      "poll or re-check a price, listing, or status we captured before",
    ],
    exampleTriggers: [
      "check that price again",
      "pull the latest listings from that site we looked at",
      "re-run the data fetch without the browser",
      "is that item back in stock",
    ],
    antiPatterns: [
      "first-time visit to a site with no saved recipe — browse it instead",
      "pages that need login/cookies (replay is unauthenticated unless headers are supplied)",
    ],
    pairsWith: ["derived_api_list", "derived_api_derive"],
  },
  derived_api_derive: {
    useWhen: [
      "turn recently browsed traffic into a reusable direct-call recipe",
      "we keep re-browsing the same site for the same data",
    ],
    exampleTriggers: [
      "save a shortcut for that data feed",
      "make a recipe so we can skip the browser next time",
    ],
    antiPatterns: [
      "nothing captured yet — browse the target pages first, then derive",
    ],
    pairsWith: ["derived_api_capture_list", "derived_api_replay"],
  },
  // ─── Content / SEO scoring ───────────────────────────────────────────
  aeo_score: {
    useWhen: [
      "user wants to know if a draft will get cited by ChatGPT, Perplexity, or Google AI Overviews",
      "user asks to check or improve a blog post's AEO / GEO / answer-engine readiness",
      "user asks how citable or extractable a piece of marketing content is",
      "a content draft is finished and needs a citation-readiness check before publishing",
    ],
    exampleTriggers: [
      "will AI engines cite this article?",
      "score this draft for AI Overviews",
      "check the AEO readiness of this post",
      "how do we get this page cited by ChatGPT?",
    ],
    antiPatterns: [
      "scoring code, JSON, or raw data files (prose Markdown only)",
      "fact-checking claims in the draft (it checks structure, not truth — use second_opinion or ensemble_query for that)",
    ],
    pairsWith: ["generate_hooks", "second_opinion"],
  },
  ...
  scrapling_scrape: {
    useWhen: ["extract one public HTTPS page through a separately paced adaptive lane after direct or Firecrawl trouble"],
    exampleTriggers: ["try Scrapling", "extract this difficult page"],
    antiPatterns: ["credentialed pages, explicit site blocks, or switching tools to evade hour/day ceilings"],
  },
  multi_source_collect: {
    useWhen: ["an important public URL needs staggered collection through web_fetch, Firecrawl, and Scrapling with partial-result preservation"],
    exampleTriggers: ["collect this page from multiple sources", "combine scraper results for this URL"],
    antiPatterns: ["bulk crawling, credentialed pages, or bypassing domain/provider/tenant ceilings"],
  },
  firecrawl_crawl: {
  ...
  create_styled_report: {
    useWhen: ["PREMIUM PDF for ANY report, analysis, deliverable, or professional document — always prefer over create_pdf"],
    exampleTriggers: ["pdf report", "premium pdf", "styled report", "branded report", "deliverable", "executive summary", "client report", "flowchart pdf"],
  },
  fill_pdf: {
  ...
  generate_audit_fix_kit: {
    useWhen: [
      "fulfilling a done-for-you ($1,997) AI-readiness audit order",
      "building the remediation files (llms.txt, JSON-LD, meta tags, robots rules) for an audited website",
    ],
    exampleTriggers: ["build the fixes for their site", "generate the fix kit", "done-for-you audit deliverable", "make the llms.txt and schema for the customer"],
  },
