  ...
  try {
    ...
    for (const conv of convs) {
      const msgResult = await db.execute(sql`
        ...
        WHERE conversation_id = ${conv.id} AND tenant_id = ${tenantId}
...
async function updateMemoryEmbeddingByTenant(memoryId: number, tenantId: number, embedding: number[]): Promise<boolean> {
  const result = await db.execute(sql`
    UPDATE memory_entries SET embedding = ${JSON.stringify(embedding)}::jsonb
        ...
        switch (action.type) {
          case "merge": {
            ...
            const mergedId = await db.transaction(async (tx) => {
              ...
              const mergedId = rows[0]?.id as number;
              if (Number.isInteger(mergedId) && mergedId > 0) {
                // The merged fact is wholly synthesized from the selected
                // durable memories and therefore cannot stand independently
                // for erasure purposes. This is not inferred from the action
                // label: the selected durable ids are present in this write.
                for (const parentId of action.ids!) {
                  await recordMemoryDerivation(tx, {
                    tenantId,
                    parentSource: "memory_entries",
                    parentId,
                    childSource: "memory_entries",
                    childId: mergedId,
                    relationship: "dream_merge",
                    solelyDerived: true,
                  });
                }
              }
            ...
            if (mergedId) {
              ...
              if (emb) {
                if (await updateMemoryEmbeddingByTenant(mergedId, tenantId, emb)) {
                  try { await storeEmbeddingVec("memory_entries", mergedId, emb, { tenantId }); } catch (_silentErr) { logSilentCatch("server/dream-consolidation.ts", _silentErr); }
                }
              }
              result.merged++;
          ...
          case "create_summary": {
            ...
            const summaryData: InsertMemoryEntry = {
             ...
             const summaryEntry = await db.transaction(async (tx) => {
               const inserted = await tx.execute(sql`
                 INSERT INTO memory_entries
                   (fact, category, source, status, persona_id, tenant_id)
                 VALUES (${summaryData.fact}, ${summaryData.category},
                   ${summaryData.source}, ${summaryData.status},
                   ${summaryData.personaId}, ${tenantId})
                 RETURNING id, fact, category, source, status, persona_id,
                   tenant_id, created_at
               `);
               const summaryEntry = extractRows(inserted)[0] as any;
               const summaryId = Number(summaryEntry?.id);
               if (!Number.isInteger(summaryId) || summaryId <= 0) {
                 throw new Error("dream summary insert returned no memory ID");
               }
               // A generated summary is wholly derived from the explicitly
               // referenced source memories. Do not invent edges for a model
               // action that supplied no durable source ids.
               for (const parentId of action.ids ?? []) {
                 ...
                 await recordMemoryDerivation(tx, {
                   ...
                   childSource: "memory_entries",
                   childId: summaryId,
                   relationship: "dream_summary",
                   solelyDerived: true,
                 });
               }
               return summaryEntry as any;
             });
            const summaryEmb = await generateEmbedding(summaryEntry.fact);
            if (summaryEmb) {
              if (await updateMemoryEmbeddingByTenant(summaryEntry.id, tenantId, summaryEmb)) {
                try { await storeEmbeddingVec("memory_entries", summaryEntry.id, summaryEmb, { tenantId }); } catch (_silentErr) { logSilentCatch("server/dream-consolidation.ts", _silentErr); }
              }
            }
            result.created++;
