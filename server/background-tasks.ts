...
export function launchBackgroundTask(
  ...
  setImmediate(async () => {
    ...
    try {
      // R-review HIGH fix: nested tool execution used to bypass the destructive-
      // tool policy entirely — the outer run_background_task guard evaluated only
      // itself, then executeTool() ran the nested tool ungated. Enforce the policy
      // here (plan-executor pattern): admin tenant runs as trusted "system"; any
      // other tenant gets no trusted name so gated tools fail CLOSED. A policy-
      // check THROW is treated as a block.
      ...
      try {
        ...
        const pol = await enforceToolPolicy(toolName, params, {
          tenantId,
          personaName: tenantId === ADMIN_TENANT_ID ? "system" : undefined,
          invokedVia: "background-task",
          hasApproval: false,
        });
