...
const TENANT_WEBHOOK_SECRET_CACHE_TTL_MS = 60_000;
const MAX_TENANT_WEBHOOK_SECRET_CANDIDATES = 100;
let tenantWebhookSecretsCache: { expiresAt: number; secrets: string[] } | null = null;
...
/**
 * A Coinbase Commerce webhook has one static endpoint
 * (/api/coinbase/webhook), while tenants may bring their own Commerce account
 * and signing secret. Coinbase signs the body, but does not put the account
 * identity in a separately authenticated request field. Therefore the body
 * metadata cannot safely choose a tenant secret before verification.
 *
 * Test every server-configured tenant secret instead. The secret that verifies
 * the raw body is the server-controlled selector; metadata is only read after
 * that succeeds. This keeps both the legacy global webhook and existing tenant
 * Commerce webhook configurations working without a provider URL change.
 */
async function getConfiguredTenantWebhookSecrets(): Promise<string[]> {
  if (tenantWebhookSecretsCache && tenantWebhookSecretsCache.expiresAt > Date.now()) {
    return tenantWebhookSecretsCache.secrets;
  }
  const result = await db.execute(sql`
    SELECT coinbase_commerce_webhook_secret
    FROM tenants
    WHERE coinbase_commerce_webhook_secret IS NOT NULL
      AND coinbase_commerce_webhook_secret <> ''
    LIMIT ${MAX_TENANT_WEBHOOK_SECRET_CANDIDATES + 1}
  `);
  const rows = (result as any).rows || result;
  if (!Array.isArray(rows)) {
    throw new Error("Could not load configured Coinbase webhook secrets");
  }
  if (rows.length > MAX_TENANT_WEBHOOK_SECRET_CANDIDATES) {
    console.error(`[coinbase] Webhook REJECTED: configured tenant webhook secrets exceed hard limit (${MAX_TENANT_WEBHOOK_SECRET_CANDIDATES})`);
    throw new Error("Too many configured Coinbase webhook secrets; refusing verification");
  }
  const secrets = new Set<string>();
  for (const row of rows || []) {
    const storedSecret = row?.coinbase_commerce_webhook_secret;
    if (storedSecret) secrets.add(decryptApiKey(storedSecret));
  }
  const configuredSecrets = [...secrets];
  tenantWebhookSecretsCache = {
    secrets: configuredSecrets,
    expiresAt: Date.now() + TENANT_WEBHOOK_SECRET_CACHE_TTL_MS,
  };
  return configuredSecrets;
}

function signatureMatchesWebhookSecret(rawBody: Buffer, signature: string, webhookSecret: string): boolean {
...
export function isValidCoinbaseWebhookEventId(eventId: unknown): eventId is string {
  // Coinbase Commerce event IDs are UUIDs. Requiring this bounded provider
  // format makes webhook_events a safe durable dedupe key rather than an
  // attacker-controlled unbounded string.
...
export async function handleCoinbaseWebhook(req: Request, res: Response) {
  try {
    ...
    // Do not parse or consult charge metadata until authenticity is established:
    // the static provider endpoint means the signature itself must select among
    // server-configured tenant secrets, never a body-supplied tenant id.
    const globalWebhookSecret = process.env.COINBASE_COMMERCE_WEBHOOK_SECRET || null;
    const tenantWebhookSecrets = await getConfiguredTenantWebhookSecrets();
    const candidateSecrets = [...new Set([
      ...(globalWebhookSecret ? [globalWebhookSecret] : []),
      ...tenantWebhookSecrets,
    ])];
    // Calculate every candidate HMAC before deciding. Do not short-circuit on
    // a matching tenant secret, which would make candidate position observable
    // through timing.
    const candidateMatches = candidateSecrets.map((secret) =>
      signatureMatchesWebhookSecret(rawBody, signature, secret),
    );
    const verified = candidateMatches.reduce((matched, candidateMatched) => matched || candidateMatched, false);
    ...
    if (!isValidCoinbaseWebhookEventId(eventId)) {
      console.warn("[coinbase] Webhook rejected: missing or malformed provider event id");
      return res.status(400).json({ error: "Invalid webhook event id" });
    }
    ...
    if (claim.status === 'in_flight') {
      throw new Error(`Coinbase webhook event ${eventId} is already being processed`);
    }
    ...
    await markWebhookEventCompleted('coinbase', eventId, claim.claimToken);
