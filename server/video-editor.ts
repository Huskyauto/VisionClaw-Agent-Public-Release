...
import { resolveUploadsChild } from "./uploads-path";
...
function resolveInputPath(input: string): string {
  ...
  if (input.startsWith("/uploads/") || input.startsWith("uploads/")) {
    ...
    return resolveUploadsChild(fname);
  }
...
export async function pushToDrive(filePath: string, label: string, projectFolderId?: string, tenantId?: number): Promise<{ driveUrl?: string; error?: string }> {
  try {
    if (!Number.isSafeInteger(tenantId) || (tenantId as number) <= 0) {
      return { error: "Generated video requires a trusted tenant ID for durable storage" };
    }
    ...
    const r = await uploadAndShare({
      ...
      generatedArtifact: { artifactKind: "video", logicalName: path.basename(filePath), sourceRunKey: `video-editor:${path.basename(filePath)}` },
    });
