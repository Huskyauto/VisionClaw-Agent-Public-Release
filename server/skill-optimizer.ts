...
export async function optimizeSkill(
  ...
  // Crossover parent pool: the highest-scoring REJECTED candidate that still
  // beat the baseline (a genuine second lineage the hill-climb discarded).
  let runnerUpDoc: string | null = null;
  ...
  for (let e = 1; e <= epochs; e++) {
    ...
    if (isStrictImprovement(candidateScore, bestScore, eps)) {
      ...
      if (candidateScore > baselineScore && candidateScore > runnerUpScore && candidate !== bestSkill) {
        runnerUpDoc = candidate;
        runnerUpScore = candidateScore;
      }
    }
  }
