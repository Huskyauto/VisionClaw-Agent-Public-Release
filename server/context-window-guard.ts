import type { CompactionArchiveRef } from "./compaction";
...
export function truncateWithSummary(
  ...
  const archiveBookmark = archive
    ? `\n\n[RECOVERABLE HISTORY BOOKMARK — archive #${archive.id}; messages ${archive.firstMessageOrdinal}–${archive.lastMessageOrdinal} of ${archive.totalMessages}. The complete original transcript is tenant-scoped and durable. If a detail is needed before a consequential action, use recall_context with conversation #${archive.conversationId} and a specific query.]`
    : "";
