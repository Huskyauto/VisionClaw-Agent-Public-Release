...
- lookup_sam_company_cage is the focused read-only path for one or more company names. It queries 1–10 deduplicated names through the official SAM.gov Entity API and does not require an active project. Use it when the user already knows the company name(s) and wants CAGE, UEI, registration status/expiration, address, NAICS, or business type. A unique normalized legal-name match is matched; partial/multiple/truncated results are needs_review and must never be presented as a verified CAGE assignment.
...
- NAMED-LOOKUP LADDER: one company → lookup_sam_exact_company with every known anchor → if ambiguous/near_match, present ranked candidates and ask for a missing address/domain/UEI fact → if retryable=true, retry the exact same call → otherwise report no_attributable_match honestly. Use lookup_sam_company_cage only for 2–10 supplied names. Never guess a CAGE number or replace the official lookup with scraped SAM pages.
