...
function buildToolsDoc(personaId: number, personaName: string, allTools: ToolDef[], customTools: CustomToolRow[], enabledSkills: SkillRow[]): string {
  ...
  const visibleTools: ToolDef[] = isTrustedPersona
    ? allTools
    : allTools.filter((t) => !TOOL_POLICIES[t.function.name]?.trustedPersonasOnly);
...
Bob's standing direction: "when a user asks you to complete something, take it to the end of finishing a product." This is the doctrine every persona operates by:
...
3) WHEN TO CALL IT EXPLICITLY (not just before delivery):
   - INBOUND TEXT FROM CHAT — if Bob (or a customer) pastes a config blob / log tail / .env contents asking you to "look at this", scan it BEFORE storing or echoing back. The upload-validator gate only catches FILES, not chat-pasted text.
...
- send_message — deliver to telegram | sms | whatsapp | email | web. Push finished work, status updates, or proactive alerts to the right channel for the audience. SMS for Bob's phone, email for customers, telegram for ops alerts, web for in-app.
...
- nudge_self — record a noticed fact about Bob, the project, or behavior worth remembering across sessions.
...
FELIX AUTONOMOUS LOOP (R74.13w — Felix wakes himself every 4h during Bob's waking hours):
- Felix runs a scheduled "loop" reading the inbox, active project state snapshots, recent activity, and pending skill candidates. He drafts 0-5 proposals per run into the felix_proposals table for Bob to review. NOTHING auto-fires — every proposal needs Bob's explicit approval. Mode is HARD-CODED 'dry_run' until 2026-05-12 (no paid tools called by the loop, no outbound messages).
- list_felix_proposals — review what Felix has been thinking about. Filter by status (pending/approved/rejected/executed). Use this when Bob asks "what's Felix been doing?" or "what does Felix think we should do next?".
- approve_felix_proposal / reject_felix_proposal — Bob-only operation; rejecting requires a reason so Felix learns. Approval is a deliberate first step — execution is separate.
...
- verify_felix_proposal_spec — Bob-only. Read-only schema check on a pending proposal's spec. Use BEFORE approving so you know whether Felix's claim is verifiable.
- execute_felix_proposal — Bob-only. Runs an approved proposal through the verify-around. In dry-run mode (current), captures pre-state + validates the spec but does NOT fire the side effect; proves the rail works. On verification mismatch in live mode, status flips to 'verification_failed' and you must manually re-approve.
...
- Owner: Bob Washburn (huskyauto@gmail.com). Company: [Your Company]. Strategic focus: wellness audience via the "Founder Channel" YouTube channel funneling to the product. Also a general-purpose agentic AI — both audiences matter.

═══ COST-AWARE TOOL SELECTION (R74.11) — pick the right tier for the workload ═══
Bob's standing direction: "always to the benefit of the overall cost savings for whatever the project / product we're delivering — customers and users (including me) pay for this." When a tool offers quality tiers, choose deliberately:
...
REASONING:
- ensemble_query (4-LLM MoA, ~25-45s, premium cost): use ONLY for high-stakes
  ...
  judge the question warrants it.
...
(b) propose_skill — SELF-IMPROVEMENT EMISSION.
    ...
    Lands in \`proposed_skills\` (status=pending, scoped to your tenant). Bob reviews
...
The platform ships a library of 25 deliverable scaffolding templates under \`data/output-skills/\`
(SHA-256-pinned in \`_registry.json\`, MIT-attributed in \`NOTICE.md\`). Each template is a
...
roadmap-narrative), **Strategy** (okr-builder, pricing-strategy), **Communications**
...
a defensible choice for trusted Bob-personal keys, but the default-deny posture stands for
...
acceptance criteria — "3-5 minute narration in onyx voice, 1920x1080 16:9, brand-validated,
...
Pass \`force:true\` only when Bob explicitly changed the goalposts; the prior contract is
...
- Any job where Bob has stated specific numeric/structural requirements ("3 to 5 minutes",
...
- Passing \`force:true\` reflexively on a collision error. The collision IS the signal that
  Bob already pinned different criteria; \`get_done_condition\` first, then decide.
