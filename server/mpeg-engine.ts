...
// Task 104 girth split — failed-job quarantine + retention sweeper live in
// mpeg-quarantine.ts; re-exported so existing importers are unchanged.
import { quarantineJobDir } from "./mpeg-quarantine";
export { pruneQuarantinedJobs, armQuarantineRetentionSweeper } from "./mpeg-quarantine";
...
export async function produceVideo(options: MpegJobOptions): Promise<MpegJobResult> {
  ...
  const generateTTSForScene = async (scene: MpegScene, index: number): Promise<{ audioPath: string; duration: number; failed: boolean; reason?: string }> => {
    ...
    for (let attempt = 0; attempt < MAX_TTS_RETRIES; attempt++) {
      try {
        const audioResult = await executeTool("generate_audio", markInternalTransientTtsSegment({
  ...
  if (shouldRequireFinalRenderDriveDurability(options)) {
    try {
      ...
      const driveResult = await uploadAndShare({
        ...
        generatedArtifact: { artifactKind: "video", logicalName: `${safeTitle}.mp4`, sourceRunKey: `mpeg-render:${safeTitle}` },
      });
      if (!driveResult.success) return { success: false, scenesProcessed: segmentPaths.length, steps, error: driveResult.error || "Drive durability verification failed" };
      driveUrl = driveResult.viewUrl;
      steps.push(`Uploaded to Google Drive: ${driveUrl}`);
    } catch (err: any) {
      return { success: false, scenesProcessed: segmentPaths.length, steps, error: `Drive durability verification failed: ${err.message}` };
    }
  }
  ...
  if (options.emailTo && driveUrl) {
    try {
      // R-review HIGH fix: this nested send_email used to bypass the tool policy
      // (raw executeTool skips enforceToolPolicy). Gate it: admin tenant runs as
      // trusted "system"; other tenants go through the full policy pipeline and
      // fail CLOSED on block or policy-check error.
      const { enforceToolPolicy } = await import("./safety/destructive-tool-policy");
      const { ADMIN_TENANT_ID } = await import("./tenant-constants");
      const emailPolicyArgs = { to: options.emailTo, subject: `Your video is ready: ${options.title}` };
      // NaN has typeof "number" but is falsy downstream (would select the
      // primary/admin inbox) — require a clean positive integer, fail closed
      // on every other shape (undefined, string, NaN, Infinity, 0, negative).
      const deliveryTenantId = options.tenantId;
      if (typeof deliveryTenantId !== "number" || !Number.isInteger(deliveryTenantId) || deliveryTenantId <= 0) throw new Error("send_email delivery requires a valid tenant context (fail-closed)");
      const pol = await enforceToolPolicy("send_email", emailPolicyArgs, {
        tenantId: deliveryTenantId,
        personaName: deliveryTenantId === ADMIN_TENANT_ID ? "system" : undefined,
        invokedVia: "mpeg-engine-delivery",
        hasApproval: false,
      });
      if (pol.action === "block") throw new Error(`Blocked by destructive-tool policy: ${pol.reason}`);
...
export async function produceVideoParallel(options: ParallelVideoOptions): Promise<MpegJobResult> {
  ...
  for (let batch = 0; batch < chapters.length; batch += maxParallel) {
    ...
    const batchPromises = batchChapters.map(async (chapter, i) => {
      ...
      const chapterResult = await produceVideo(markInternalRenderStaging({
  ...
  // Stitch auto-retry. concatenateClips already has a copy→re-encode fallback
  // ladder, but a TRANSIENT infra fault (overlayFS EIO, an ffmpeg spawn glitch,
  // a momentary timeout) would otherwise fail the whole parallel job and strand
  // every completed chapter render, forcing a manual re-trigger. Retry the
  // stitch up to 3x with short linear backoff on a non-success result before
  // giving up — bounded, so a genuinely broken input still fails fast.
  const stitchName = options.title.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 50);
  const stitchTransition = options.transition || "fade";
  const stitchCrossfadeMs = options.crossfadeMs ?? 400;
  let concatResult = await concatenateClips(clipPaths, stitchName, stitchTransition, stitchCrossfadeMs);
  for (let stitchAttempt = 2; (!concatResult.success || !concatResult.filePath) && stitchAttempt <= 3; stitchAttempt++) {
    const waitMs = 1500 * (stitchAttempt - 1);
    console.warn(`[mpeg-parallel] Stitch attempt ${stitchAttempt - 1} failed (${(concatResult.error || "unknown").slice(0, 120)}) — retrying in ${waitMs}ms (attempt ${stitchAttempt}/3)`);
    steps.push(`⚠️ Stitch failed, auto-retrying (attempt ${stitchAttempt}/3)`);
    await new Promise((r) => setTimeout(r, waitMs));
    concatResult = await concatenateClips(clipPaths, stitchName, stitchTransition, stitchCrossfadeMs);
  }
  ...
  if (shouldRequireFinalRenderDriveDurability(options)) {
    try {
      ...
      const driveResult = await uploadAndShare({
        ...
        tenantId: options.tenantId,
        projectId: options.projectId,
        generatedArtifact: { artifactKind: "video", logicalName: `${safeTitle}.mp4`, sourceRunKey: `mpeg-stitch:${safeTitle}` },
      });
      if (!driveResult.success) return { success: false, scenesProcessed: successChapters.reduce((sum, chapter) => sum + chapter.result.scenesProcessed, 0), steps, error: driveResult.error || "Drive durability verification failed" };
      driveUrl = driveResult.viewUrl;
      steps.push(`Uploaded to Google Drive: ${driveUrl}`);
    } catch (err: any) {
      return { success: false, scenesProcessed: successChapters.reduce((sum, chapter) => sum + chapter.result.scenesProcessed, 0), steps, error: `Drive durability verification failed: ${err.message}` };
    }
  }
