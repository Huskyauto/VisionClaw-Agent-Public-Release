...
export async function produceVideo(options: MpegJobOptions): Promise<MpegJobResult> {
  ...
  try {
    ...
    if (await renderClient.isRenderWorkerHealthy()) {
    ...
    console.warn(`[mpeg-engine] R110.19 render worker failed, falling back to in-process: ${workerErr?.message?.slice(0, 200) || workerErr}`);
    workerOutPath = null;
  }
