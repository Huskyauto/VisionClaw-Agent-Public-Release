...
export function normalizeRunConfig(input: RunConfigInput = {}): NormalizedRunConfig {
  ...
  return {
    ...
    seed,
    minImprovement,
    optimizerModel: input.optimizerModel || "gpt-5.6-sol",
    targetModel: input.targetModel || "gpt-5.4",
    graderModel: input.graderModel || "gemini-2.5-flash",
    tenantId,
  };
}
...
export type RetainedArtifactState = "rejected" | "held" | "failed";

export interface RetainedArtifactClassification {
  state: RetainedArtifactState;
  reason: string;
}

/**
 * Classify historical filesystem evidence without granting it authority.
 * An artifact with a DB id is still held: it must be recreated as an immutable
 * candidate and rerun through current validation and jury policy before it can
 * ever reach the promotion service.
 */
export function classifyRetainedOptimizerArtifact(
  run: unknown,
  hasBestSkill: boolean,
): RetainedArtifactClassification {
  if (!run || typeof run !== "object" || Array.isArray(run)) {
    return { state: "failed", reason: "run artifact is not a JSON object" };
  }
  const record = run as Record<string, unknown>;
  if (typeof record.improved !== "boolean") {
    return { state: "failed", reason: "run artifact has no deterministic improved outcome" };
  }
  if (!record.improved) {
    return { state: "rejected", reason: "held-out evaluation did not strictly improve" };
  }
  if (!hasBestSkill) {
    return { state: "failed", reason: "improved run is missing its best_skill.md evidence" };
  }
  if (!Number.isInteger(record.dbSkillId) || Number(record.dbSkillId) <= 0) {
    return { state: "rejected", reason: "file-seed artifact has no mapped live skill and is non-promotable" };
  }
  return {
    state: "held",
    reason: "filesystem evidence cannot authorize promotion; recreate and rerun every current gate",
  };
}
...
export type UpgradeAction = "apply" | "hold" | "reject" | "escalate";
...
export function mapVerdictToAction(d: { verdict: string; majority: number; shouldEscalate?: boolean }): UpgradeAction {
  ...
  if (d.verdict === "REJECT") return "reject";
  return "hold";
}
