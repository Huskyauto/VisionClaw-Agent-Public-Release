...
import { normalizeAdminPin, requireNormalizedAdminPin } from "./lib/admin-pin-normalization";
import { hashAdminPinWithKey } from "./lib/admin-pin-crypto";
...
if (!PIN_PEPPER && process.env.NODE_ENV === "production") {
  // Consistency with the SESSION_SECRET guard above: a security control that
  // only emits a console.warn can silently be NOT in effect in production
  // (nobody reads the boot log). Since PIN_PEPPER is what makes a DB leak
  // non-crackable, refuse to boot without it in prod rather than degrade to the
  // public static salt unnoticed.
  throw new Error("[auth] FATAL: PIN_PEPPER is required in production. Without it, admin PIN hashing falls back to the public static salt (in an open-source repo), so anyone with DB read access can brute-force the PIN keyspace offline. Set PIN_PEPPER (a long random secret). Refusing to boot.");
}

export const ADMIN_TENANT_ID = 1;

// ─── Admin semantics (Task 124, 2026-08-01) ───────────────────────────────
// There are exactly TWO tiers of admin, and they must derive identically on
// every login path (legacy token session, Replit-Auth session):
//
//   1. `tenants.is_admin` — TENANT-LOCAL admin role. Grants the admin UI and
//      tenant-scoped admin endpoints (event log/subscriptions for the tenant's
//      own data, tenant memory maintenance, etc.). `isAdminRequest(req)` is
//      TRUE for tenant #1 and for any tenant flagged `is_admin`, regardless of
//      how the session was established.
//   2. PLATFORM authority — tenant #1 (ADMIN_TENANT_ID) ONLY. Anything that
//      mutates global/cross-tenant state (provider keys, global templates,
//      code proposals, demo mode, cross-tenant reads) must gate on
//      `requirePlatformAdmin` / `isPlatformAdmin`, never on `isAdminRequest`
//      alone. A flagged non-#1 admin is NOT a platform admin.
// ───────────────────────────────────────────────────────────────────────────
...
function hashPinWith(pin: string, key: string): string {
  return hashAdminPinWithKey(pin, key);
}
...
// Exported for seed.ts (canonical peppered storage form). Any code that WRITES
// a PIN hash must use this — never re-derive with the public static salt.
...
// R125+137.23 — exported comparator for routes that verify a caller-supplied PIN
// against the PLAINTEXT ADMIN_PIN env var (e.g. admin-drive-upload). Both sides
// are hashed with the peppered hashPin so no callsite ever open-codes the public
// static salt again; timing-safe.
export function verifyAdminPinPlaintext(candidatePin: string, adminPinPlaintext: string): boolean {
  return timingSafeHexCompare(hashPin(String(candidatePin || "")), hashPin(adminPinPlaintext));
}
...
/**
 * The auth_sessions primary key is deliberately a keyed digest rather than a
 * bearer credential. The version prefix lets boot cleanup remove legacy raw
 * tokens without touching sessions created by this format.
 */
export function hashSessionToken(token: string): string {
  return `v2:${crypto.createHmac("sha256", EFFECTIVE_SECRET).update(token).digest("hex")}`;
}

async function isTenantSessionActive(tenantId: number): Promise<boolean> {
  try {
    const result = await db.execute(sql`
      SELECT is_active FROM tenants WHERE id = ${tenantId}
    `);
    const row = ((result as any).rows ?? result)?.[0];
    return row?.is_active === true;
  } catch (err: any) {
    // A session may never outlive an authorization-state lookup failure.
    console.error(`[auth] Failed to verify session tenant status: ${err?.message || String(err)}`);
    return false;
  }
}
...
async function createSession(tenantId: number, isAdmin: boolean): Promise<string> {
  ...
  await db.execute(sql`
    ...
    VALUES (${tokenHash}, ${tenantId}, ${isAdmin}, ${now}, ${expiresAt})
  `);
...
export async function getSession(token: string): Promise<{ tenantId: number; isAdmin: boolean } | null> {
  const cached = sessionCache.get(token);
  if (cached) {
    if (Date.now() > cached.expiresAt) {
      sessionCache.delete(token);
      db.execute(sql`DELETE FROM auth_sessions WHERE token = ${hashSessionToken(token)}`).catch(() => {});
      return null;
    }
    if (!await isTenantSessionActive(cached.tenantId)) {
  ...
  const tokenHash = hashSessionToken(token);
  const result = await db.execute(sql`
    SELECT tenant_id, is_admin, expires_at FROM auth_sessions WHERE token = ${tokenHash}
  `);
  ...
  if (Date.now() > expiresAt) {
    db.execute(sql`DELETE FROM auth_sessions WHERE token = ${tokenHash}`).catch(() => {});
    return null;
  }

  const data = { tenantId: Number(row.tenant_id), isAdmin: Boolean(row.is_admin), expiresAt };
  if (!await isTenantSessionActive(data.tenantId)) {
    await deleteSession(token);
    return null;
  }
...
export function getSessionSync(token: string): { tenantId: number; isAdmin: boolean } | null {
  // Tenant activity/revocation is authoritative in Postgres and cannot be
  // proven synchronously. Never authorize a raw bearer from process memory:
  // a tenant may have been disabled or deleted by another process after this
  // cache entry was created. Callers must await getSession() instead.
  return null;
}

async function deleteSession(token: string): Promise<void> {
  sessionCache.delete(token);
  await db.execute(sql`DELETE FROM auth_sessions WHERE token = ${hashSessionToken(token)}`).catch(() => {});
}
...
// Caches BOTH the tenant id and the tenant's is_admin flag so the sync
// isAdminRequest() can derive admin identically to the legacy token path
// (which snapshots tenants.is_admin into the session at login).
const tenantCacheByReplitUser = new Map<string, { tenantId: number; isAdmin: boolean }>();

// Test seam — lets unit tests exercise the Replit-Auth branch of
// isAdminRequest/getTenantFromRequest without a live DB.
export function primeReplitTenantCacheForTest(sub: string, tenantId: number, isAdmin: boolean): void {
  tenantCacheByReplitUser.set(sub, { tenantId, isAdmin });
}

// Pure guard (pinned by tests/security/admin-gate.test.ts): an owner-email
// match may bind the admin tenant only on first bind (no subject recorded yet)
// or when the incoming Replit subject matches the recorded one. Anything else
// is a rebind attempt and must be refused.
export function ownerRebindAllowed(boundReplitUserId: string | null | undefined, incomingReplitUserId: string): boolean {
  return !boundReplitUserId || boundReplitUserId === incomingReplitUserId;
}
...
export async function getOrCreateTenantForReplitUser(replitUserId: string, email?: string | null, name?: string | null): Promise<number> {
  ...
  // OWNER_EMAIL (singular) is the secret that actually exists in this workspace;
  // OWNER_EMAILS (plural, comma-separated) kept for compatibility.
  const ownerEmailsStr = [process.env.OWNER_EMAILS, process.env.OWNER_EMAIL].filter(Boolean).join(",");
  ...
  if (isOwner) {
    ...
    if (adminTenant) {
      // Architect hardening: an email match alone must NOT rebind an admin tenant that is
      // already bound to a DIFFERENT Replit subject — that would let an account presenting
      // the owner email take over tenant #1. First bind (empty replitUserId) or exact
      // subject match are the only paths in; anything else falls through to normal mapping.
      if (!ownerRebindAllowed(adminTenant.replitUserId, replitUserId)) {
        console.warn(`[auth] owner-email match for "${email || replitUserId}" REFUSED admin rebind — tenant #${ADMIN_TENANT_ID} already bound to a different Replit subject`);
      } else {
        if (!adminTenant.replitUserId) {
          await db.update(tenants).set({ replitUserId }).where(eq(tenants.id, ADMIN_TENANT_ID));
        }
        tenantCacheByReplitUser.set(replitUserId, { tenantId: ADMIN_TENANT_ID, isAdmin: true });
  ...
  if (existing) {
    tenantCacheByReplitUser.set(replitUserId, { tenantId: existing.id, isAdmin: existing.id === ADMIN_TENANT_ID || !!existing.isAdmin });
    return existing.id;
  }
  ...
  if (email) {
    ...
    if (byEmail && !byEmail.replitUserId) {
      ...
      tenantCacheByReplitUser.set(replitUserId, { tenantId: byEmail.id, isAdmin: byEmail.id === ADMIN_TENANT_ID || !!byEmail.isAdmin });
      return byEmail.id;
    }
  }
  ...
  tenantCacheByReplitUser.set(replitUserId, { tenantId: newTenant.id, isAdmin: false });
  return newTenant.id;
}
...
export function isAdminRequest(req: Request): boolean {
  ...
  const validatedSession = (req as any)._validatedSession as { tenantId: number; isAdmin: boolean } | undefined;
  if (validatedSession && !isApiKey) {
    return validatedSession.isAdmin;
  }

  const replitUser = getReplitAuthUser(req);
  if (replitUser) {
    const cached = tenantCacheByReplitUser.get(replitUser.sub);
    // Task 124 — derive identically to the legacy token path: tenant #1 OR the
    // tenant's is_admin flag (snapshotted into the cache at tenant resolution),
    // matching what createSession snapshots for token logins.
    if (cached) return cached.tenantId === ADMIN_TENANT_ID || cached.isAdmin;
    return false;
  }
...
const SCOPE_RULES: Array<{ method?: string; pattern: RegExp; scopes: ApiScope[] }> = [
  ...
  { method: "POST", pattern: /^\/api\/v1\/agui\/run$/, scopes: ["chat"] },
  { method: "POST", pattern: /^\/api\/v1\/a2a$/, scopes: ["chat"] },
  { method: "POST", pattern: /^\/api\/v1\/revenue-orchestrator\/runs$/, scopes: ["chat"] },
  { method: "GET", pattern: /^\/api\/v1\/revenue-orchestrator\/runs\/\d+$/, scopes: ["read", "chat"] },
  { method: "POST", pattern: /^\/api\/v1\/revenue-orchestrator\/missions$/, scopes: ["chat"] },
  { method: "GET", pattern: /^\/api\/v1\/revenue-orchestrator\/missions\/\d+$/, scopes: ["read", "chat"] },
...
const PUBLIC_EXACT_PATHS = new Set<string>([
  ...
  // beneath the gmail-direct root from silently becoming public.
  "/api/admin/gmail-direct/auth",
  "/api/admin/gmail-direct/callback",
  "/api/admin/gmail-direct/status",
  "/api/admin/gmail-direct/start",
  // Archive Rescue Admin page: the client sends only an x-admin-pin header
  // (no session/Bearer), and both handlers in server/routes/archive-rescue.ts
  // validate the PIN themselves (timingSafeEqual vs ADMIN_PIN + lockout
  // throttle). Without this entry the global auth middleware 401'd before the
  // PIN check ever ran. Exact path only — the status route is matched by an
  // anchored regex in isPublicPath below.
  "/api/admin/archive-rescue/orders",
]);
...
// was a foot-gun — any future child route beneath gmail-direct would
...
export function isPublicPath(rawPath: string): boolean {
  ...
  // Every other video-job route stays auth-gated.
  if (/^\/api\/video-jobs\/[^/]+\/download$/.test(p)) return true;
  // Archive Rescue Admin status update. PIN-gated inside the handler
  // (timingSafeEqual vs ADMIN_PIN + lockout throttle) — same rationale as the
  // exact /api/admin/archive-rescue/orders entry above. Anchored to exactly
  // one id segment so no other archive-rescue route can inherit public access.
  if (/^\/api\/admin\/archive-rescue\/orders\/[^/]+\/status$/.test(p)) return true;
  return false;
}
...
export async function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (isPublicPath(fullPath)) {
    ...
    // crawl the contract before having a key. All other v1 API routes
  ...
  if (token) {
    ...
    if (session) {
      ...
      (req as any)._validatedSession = session;
...
export async function handleLogin(req: Request, res: Response) {
  ...
  // Trim edge whitespace: pasted PINs (and secret values pasted from managers)
  // routinely carry a trailing newline/space; a PIN is never whitespace-edged.
...
async function ensureEmailVerificationTable() {
  // No `.catch(() => {})` swallow: a DDL/permission failure MUST propagate so
  // getVerificationTableReady() leaves readiness=false (retryable) instead of
  // silently marking ready=true and letting later verify flows fail with a
...
async function ensureResetTokensTable() {
  // No `.catch(() => {})` swallow — see ensureEmailVerificationTable: a DDL
  // failure must propagate so readiness stays false (retryable) instead of
  // masking a broken reset-token table for the process lifetime.
  await db.execute(sql`
...
async function getResetToken(token: string): Promise<{ tenantId: number; email: string; expiresAt: number } | null> {
  ...
  const embeddedTenantId = Number(token.slice(0, separator));
  if (
    separator <= 0 ||
    !Number.isSafeInteger(embeddedTenantId) ||
    embeddedTenantId <= 0 ||
    token.length - separator - 1 !== 64
  ) {
    return null;
  }
  const tokenHash = hashAuthSecret(token);
  const result = await db.execute(sql`
    SELECT tenant_id, email, expires_at FROM password_reset_tokens
...
export async function handleForgotPassword(req: Request, res: Response) {
  ...
  const token = `${tenant.id}.${crypto.randomBytes(32).toString("hex")}`;
...
export async function setAccessPin(pin: string): Promise<string> {
  // Canonical PIN write path: normalize the same way handleLogin does
  // (edge-whitespace is never part of PIN syntax). Without this, a PIN saved
  // with a trailing newline via settings verifies against nothing at login —
  // the exact prod-lockout class R125+137.24 fixed in seed.ts.
  const normalized = requireNormalizedAdminPin(pin);
...
const globalSessionResetAuthorities = new WeakSet<object>();

export type PlatformAdminSessionResetAuthority = {
  readonly kind: "platform-admin-session-reset";
  readonly actorTenantId: number;
  readonly reason: string;
};

/**
 * Mint a one-use, server-owned capability for the exceptional global session
 * reset. A caller cannot obtain a valid capability without passing the same
 * runtime platform-admin check used by cross-tenant routes.
 */
export function authorizePlatformAdminSessionReset(
  req: Request,
  res: Response,
  reason: string,
): PlatformAdminSessionResetAuthority | null {
  if (!requirePlatformAdmin(req, res)) return null;
  if (!reason.trim()) throw new Error("Global session reset requires an audit reason");
  const authority = Object.freeze({
    kind: "platform-admin-session-reset" as const,
    actorTenantId: ADMIN_TENANT_ID,
    reason: reason.trim(),
  });
  globalSessionResetAuthorities.add(authority);
  return authority;
}

/**
 * Revoke every cached and persisted bearer for a tenant. Use this whenever a
 * tenant is disabled or deleted; route authorization still rechecks activity
 * asynchronously so an out-of-process change remains fail closed.
 */
export async function clearSessionsForTenant(tenantId: number): Promise<void> {
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("clearSessionsForTenant requires a positive tenantId");
  }
  await db.execute(sql`DELETE FROM auth_sessions WHERE tenant_id = ${tenantId}`);
  for (const [token, session] of sessionCache) {
    if (session.tenantId === tenantId) sessionCache.delete(token);
  }
}

/**
 * Audit happens after the irreversible reset, so an unavailable audit sink
 * must be loud without converting a successful revocation into a reported
 * failure. The injectable writer keeps this completion contract testable.
 */
export async function recordGlobalSessionResetAuditNonFatal(
  authority: PlatformAdminSessionResetAuthority,
  writeAudit: () => Promise<unknown> = () => db.execute(sql`
    INSERT INTO event_log (tenant_id, event_type, source, data, status)
    VALUES (
      ${authority.actorTenantId},
      'security.sessions.cleared',
      'auth:platform-admin-reset',
      ${JSON.stringify({ scope: "all_tenants", reason: authority.reason })}::jsonb,
      'processed'
    )
  `),
  reportFailure: (message: string, error: unknown) => void = (message, error) =>
    console.error(message, error),
): Promise<void> {
  try {
    await writeAudit();
  } catch (error) {
    reportFailure("[auth] Global session reset completed, but security audit persistence failed", error);
  }
}

/**
 * Exceptional cross-tenant reset. The one-use authority is intentionally
 * checked at this function boundary, rather than trusting a caller comment or
 * route-only guard.
 */
export async function clearAllSessionsAsPlatformAdmin(
  authority: PlatformAdminSessionResetAuthority,
): Promise<void> {
  if (!authority || !globalSessionResetAuthorities.delete(authority)) {
    throw new Error("Platform-admin session-reset authority required");
  }
  await db.execute(sql`DELETE FROM auth_sessions`);
  sessionCache.clear();
  await recordGlobalSessionResetAuditNonFatal(authority);
}
...
export async function loadSessionsFromDb(): Promise<void> {
  try {
    const now = Date.now();
    await db.execute(sql`DELETE FROM auth_sessions WHERE expires_at < ${now}`).catch(() => {});
    // All v1 rows held raw bearer credentials. They cannot be safely
    // transformed without the original bearer, so invalidate them on the
    // first secure boot instead of preserving a replayable credential store.
    await db.execute(sql`DELETE FROM auth_sessions WHERE token NOT LIKE 'v2:%'`);
    // v2 rows are looked up by HMAC when the bearer arrives. Never preload
    // digests into the raw-token cache: a digest is not a valid session token.
