...
export interface HarnessHealthSummary {
  ...
  landRate: number;
  /** prior-collapse stalls detected in the window (attempt grain). */
  stalls: number;
  /** metered escalation valve fires (one frontier diagnostic turn per fire). */
  valveFires: number;
  /** valveFires / stalls — how often a stall consumed the frontier reserve (0 when no stalls). The favorite number is 0. */
...
export interface HarnessHealthCounts {
  ...
  /** prior-collapse stall attempts in the window. */
  stalls: number;
  /** metered escalation valve fires (outcome_detail ? 'valve'). */
  valveFires: number;
}
...
export function computeHarnessHealth(c: HarnessHealthCounts, windowDays = WINDOW_DAYS): HarnessHealthSummary {
  ...
  const stalls = Math.max(0, c.stalls | 0);
  // Metered escalation valve (Anvil borrow): valveFires can never exceed stalls
  // (a fire only happens ON a stall) — clamp defensively against dirty rows.
  const valveFires = Math.min(Math.max(0, c.valveFires | 0), stalls);
  const valveFireRate = stalls > 0 ? valveFires / stalls : 0;
  return {
    windowDays,
    ...
    stalls,
    valveFires,
    valveFireRate: Math.round(valveFireRate * 100) / 100,
  ...
  try {
    ...
    const aRes = await db.execute(sql`
      SELECT
        ...
        -- Prior-collapse stall attempts: identified by the collapse marker in reasons.
        COALESCE(SUM(CASE WHEN outcome = 'rolled_back' AND outcome_detail->'reasons'->>0 LIKE 'Prior-collapse:%' THEN 1 ELSE 0 END), 0)::int AS stalls,
        -- Metered escalation valve fires (Anvil borrow): presence of the valve key.
        COALESCE(SUM(CASE WHEN outcome_detail ? 'valve' THEN 1 ELSE 0 END), 0)::int AS valve_fires
    ...
    return computeHarnessHealth({
      ...
      stalls: Number(a.stalls) || 0,
      valveFires: Number(a.valve_fires) || 0,
    });
  } catch (_silentErr) {
