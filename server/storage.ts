...
export class DatabaseStorage implements IStorage {
  ...
  async updateMemoryEntry(id: number, data: Partial<InsertMemoryEntry>, tenantId?: number) {
    if (tenantId === undefined) _warnUnscoped("updateMemoryEntry", id);
    const conds = [eq(memoryEntries.id, id)];
    const tScope = tenantScope(memoryEntries.tenantId, tenantId);
    if (tScope) conds.push(tScope);
  ...
  async getDueHeartbeatTasks() {
    // Raw db.execute returns SNAKE_CASE keys (prompt_content, cron_expression…)
    // while every consumer reads camelCase (task.promptContent). Unmapped, the
    // maintenance_script runner saw key "" for EVERY task and self-heal
    // disabled them all in prod (2026-07-21 incident). Map explicitly.
    const result = await db.execute(sql`
