...
import { tenantScope, assertValidTenantId, requiredTenantScope, ALL_TENANTS, type TenantScope } from "./storage-helpers/tenant-scope";
export { ALL_TENANTS, type TenantScope };
import { redactRecordFields } from "./storage-helpers/pii-redaction-guard";
import {
  RESERVED_KNOWLEDGE_CATEGORIES,
  isPlatformManagedKnowledgeSource,
} from "./lib/reserved-knowledge-categories";
import {
  assertManagedSkillRecordSafe,
  isManagedSkillSecurityError,
} from "./lib/managed-skill-safety";

const ADMIN_TENANT_ID = 1;
const _unsafeEnabledSkillSeen = new Set<number>();
...
export interface IStorage {
  getConversations(limit: number | undefined, offset: number | undefined, tenantId: TenantScope): Promise<PaginatedResult<Conversation>>;
  ...
  updateSkill(id: number, data: Partial<InsertSkill>, expectedStatus?: string): Promise<Skill | undefined>;
  deleteSkill(id: number): Promise<void>;
  getPersonas(): Promise<Persona[]>;
  getPersona(id: number): Promise<Persona | undefined>;
  getActivePersona(): Promise<Persona | undefined>;
  getPersonaForTenant(id: number, tenantId: number): Promise<Persona | undefined>;
  getActivePersonaForTenant(tenantId: number): Promise<Persona | undefined>;
  ...
  deletePersona(id: number, tenantScope: typeof ALL_TENANTS): Promise<void>;
  ...
  updateMemoryEntry(id: number, data: Partial<InsertMemoryEntry>, tenantId: number): Promise<MemoryEntry | undefined>;
  updateKnowledgeEmbedding(id: number, embedding: number[], tenantId: number): Promise<void>;
  deleteMemoryEntry(id: number, tenantId: TenantScope): Promise<void>;
  touchMemoryEntries(ids: number[], tenantId: number): Promise<void>;
  ...
  getKnowledge(personaId: number | undefined, limit: number | undefined, offset: number | undefined, tenantId: TenantScope): Promise<PaginatedResult<AgentKnowledge>>;
  // R125+130: reserved categories (messaging_pairing) fail closed unless the
  // dedicated trusted caller passes allowReservedCategory:true.
  createKnowledge(data: InsertKnowledge, opts?: { allowReservedCategory?: boolean; allowPlatformManagedKnowledge?: boolean }): Promise<AgentKnowledge>;
  // R74.13d C3: tenantId is mandatory to prevent cross-tenant knowledge mutation/deletion.
  updateKnowledge(id: number, data: Partial<InsertKnowledge>, tenantId: number, opts?: { allowReservedCategory?: boolean; allowPlatformManagedKnowledge?: boolean }): Promise<AgentKnowledge | undefined>;
  deleteKnowledge(id: number, tenantId: number, opts?: { allowPlatformManagedKnowledge?: boolean }): Promise<void>;
  ...
  // Task #119: tenantId is REQUIRED (fail-closed). Platform-wide reads must
  // pass the explicit ALL_TENANTS sentinel — no silent unscoped fallback.
  getMemoryStats(personaId: number | undefined, tenantId: TenantScope): Promise<{ active: number; archived: number; total: number; byCategory: Record<string, number>; knowledgeCount: number }>;
  ...
  getHeartbeatTasks(personaId: number | undefined, tenantId: TenantScope): Promise<HeartbeatTask[]>;
  getHeartbeatTask(id: number, tenantId: number): Promise<HeartbeatTask | undefined>;
  createHeartbeatTask(data: InsertHeartbeatTask & { tenantId?: number }): Promise<HeartbeatTask | any>;
  updateHeartbeatTask(id: number, data: Partial<InsertHeartbeatTask>, tenantId: TenantScope): Promise<HeartbeatTask | undefined>;
  ...
  getHeartbeatLogs(limit: number | undefined, personaId: number | undefined, tenantId: TenantScope): Promise<HeartbeatLog[]>;
  ...
  searchConversations(query: string, tenantId: TenantScope): Promise<Array<Conversation & { snippet?: string }>>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getConversations(limit = 50, offset = 0, tenantId: TenantScope): Promise<PaginatedResult<Conversation>> {
    const conditions = [isNull(conversations.deletedAt)];
    const tScope = requiredTenantScope(conversations.tenantId, tenantId);
  ...
  async createConversation(data: InsertConversation & { tenantId: number }) {
    // Fail CLOSED: tenant_id is .notNull() with no default — every INSERT must
    // pass an explicit, valid tenant. A missing/invalid tenantId here would be
    // an unscoped write, so throw loudly rather than proceed.
    if (!Number.isInteger(data.tenantId) || (data.tenantId as number) <= 0) {
      throw new Error("createConversation: explicit positive tenantId is required");
    }
  ...
  async attachCitations(messageId: number, citations: any[], tenantId: number) {
    assertValidTenantId(tenantId);
    if (!Array.isArray(citations) || citations.length === 0) return;
    try {
      await db.execute(sql`
        UPDATE messages SET citations = ${JSON.stringify(citations)}::jsonb
  ...
  async getEnabledSkillsWithPrompts(personaId?: number | null, disabledSkillNames?: string[] | null) {
    const baseCondition = and(
      ...
      sql`${skills.promptContent} IS NOT NULL`,
    );
    let rows: Skill[];
    if (personaId) {
    ...
    return rows.filter(skill => {
      try {
        assertManagedSkillRecordSafe(skill);
        return true;
      } catch (error) {
        if (!isManagedSkillSecurityError(error)) throw error;
        if (!_unsafeEnabledSkillSeen.has(skill.id)) {
          _unsafeEnabledSkillSeen.add(skill.id);
          console.error(
            `[storage] blocked unsafe enabled skill ${skill.id} from prompt injection (${error.patterns.join(", ")})`,
          );
        }
        return false;
      }
  ...
  async createSkill(data: InsertSkill) {
    assertManagedSkillRecordSafe(data);
    const [skill] = await db.insert(skills).values(data).returning();
    return skill;
  }
  async updateSkill(id: number, data: Partial<InsertSkill>, expectedStatus?: string) {
    return await db.transaction(async (tx) => {
      ...
      if (expectedStatus !== undefined && existing.status !== expectedStatus) return undefined;
      assertManagedSkillRecordSafe({ ...existing, ...data });
      const [skill] = await tx.update(skills).set(data).where(eq(skills.id, id)).returning();
      return skill;
    });
  }
  ...
  /**
   * Personas are platform-managed system records: the personas schema has no
   * tenant_id and persona mutation routes require platform admin. These
   * accessors still bind tenant data paths to an existing validated tenant.
   * tenant_persona_names is only an optional display-name override and is not
   * an ownership/assignment table.
   */
  async getPersonaForTenant(id: number, tenantId: number) {
    assertValidTenantId(tenantId);
    const [row] = await db
      .select()
      .from(personas)
  ...
  async getActivePersonaForTenant(tenantId: number) {
    assertValidTenantId(tenantId);
    const [row] = await db
      .select()
      .from(personas)
  ...
  async deletePersona(id: number, scope: typeof ALL_TENANTS) {
    // Personas are deliberately platform-global (the schema has no tenant_id).
    // Deleting one therefore cannot truthfully be a tenant-local operation:
    // every tenant-owned child referencing it must be cleaned up atomically.
    // Require the explicit global sentinel so a caller cannot accidentally
    // mistake a platform persona for tenant-owned data.
    if (scope !== ALL_TENANTS) {
      throw new Error("deletePersona: personas are global; explicit ALL_TENANTS scope is required");
    }
    const conversationScope = requiredTenantScope(conversations.tenantId, scope);
    const memoryScope = requiredTenantScope(memoryEntries.tenantId, scope);
    const noteScope = requiredTenantScope(dailyNotes.tenantId, scope);
    await db.transaction(async (tx) => {
      const conversationConditions = [eq(conversations.personaId, id)];
      if (conversationScope) conversationConditions.push(conversationScope);
      const memoryConditions = [eq(memoryEntries.personaId, id)];
      if (memoryScope) memoryConditions.push(memoryScope);
      const noteConditions = [eq(dailyNotes.personaId, id)];
      if (noteScope) noteConditions.push(noteScope);
      await tx.update(conversations).set({ personaId: null }).where(and(...conversationConditions));
      await tx.update(memoryEntries).set({ status: "superseded" }).where(and(...memoryConditions));
      await tx.delete(dailyNotes).where(and(...noteConditions));
  ...
  async createMemoryEntry(data: InsertMemoryEntry) {
    // Gate-1 storage-boundary PII/secret redaction (central choke-point for all
    // memory writes — see storage-helpers/pii-redaction-guard.ts).
    const g = redactRecordFields(data, ["fact"]);
    if (g.redactedClasses.length > 0) {
      data = g.data;
      console.info(`[pii-guard] memory_entry redacted classes: ${g.redactedClasses.join(",")}`);
    }
    if (!Number.isInteger(data.tenantId) || Number(data.tenantId) <= 0) {
      throw new Error("createMemoryEntry requires tenantId");
    }
    const { assertMemoryValueNotTombstoned } = await import("./memory/tombstones");
  ...
  async updateMemoryEntry(id: number, data: Partial<InsertMemoryEntry>, tenantId: number) {
    const conds = [
      ...
      requiredTenantScope(memoryEntries.tenantId, tenantId)!,
    ];
    // Gate-1 redaction also on the update path (defense-in-depth — untrusted
    // ingest can reach edits, not just inserts).
    const g = redactRecordFields(data, ["fact"]);
    if (g.redactedClasses.length > 0) {
      data = g.data;
      console.info(`[pii-guard] memory_entry(update) redacted classes: ${g.redactedClasses.join(",")}`);
    }
  ...
  async deleteMemoryEntry(id: number, tenantId: TenantScope) {
    const tenantPredicate = requiredTenantScope(memoryEntries.tenantId, tenantId);
    const conds = [eq(memoryEntries.id, id)];
    if (tenantPredicate) conds.push(tenantPredicate);
    await db.update(memoryEntries).set({ status: "superseded" }).where(and(...conds));
  }
  async touchMemoryEntries(ids: number[], tenantId: number) {
    const tenantPredicate = requiredTenantScope(memoryEntries.tenantId, tenantId)!;
    if (ids.length === 0) return;
    const conds = [inArray(memoryEntries.id, ids), tenantPredicate];
  ...
  async createConversationFact(data: InsertConversationFact): Promise<ConversationFact> {
    // Gate-1 storage-boundary PII/secret redaction — conversation_facts.factText
    // is fed by the untrusted-content extractor.
    const g = redactRecordFields(data, ["factText"]);
    if (g.redactedClasses.length > 0) {
      data = g.data;
      console.info(`[pii-guard] conversation_fact redacted classes: ${g.redactedClasses.join(",")}`);
    }
    const { assertMemoryValueNotTombstoned } = await import("./memory/tombstones");
    await assertMemoryValueNotTombstoned({
      tenantId: Number(data.tenantId),
  ...
  // ─── Knowledge Base ─────────────────────────────────────
  async getKnowledge(personaId: number | undefined, limit = 100, offset = 0, tenantId: TenantScope): Promise<PaginatedResult<AgentKnowledge>> {
    ...
    conditions.push(requiredTenantScope(agentKnowledge.tenantId, tenantId));
    const where = and(...conditions);
  ...
  async createKnowledge(data: InsertKnowledge, opts?: { allowReservedCategory?: boolean; allowPlatformManagedKnowledge?: boolean }) {
    // Reserved-category storage-boundary guard (central choke-point for ALL
    // knowledge writers — the create_knowledge tool, heartbeat knowledge-task
    // insertion, and any future agentic writer). Categories in
    // RESERVED_KNOWLEDGE_CATEGORIES carry trust/routing semantics
    // (`messaging_pairing` maps an inbound phone/channel → tenant+conversation
    // in server/twilio.ts). A model-produced or agent-supplied category must
    // NEVER be able to plant such a row: it enabled a cross-tenant SMS/WhatsApp
    // hijack / targeted DoS (post-edit-code-review HIGH, 2026-07-09). Only the
    // dedicated authenticated pairing path may pass allowReservedCategory:true.
    if (!opts?.allowReservedCategory && data?.category &&
        RESERVED_KNOWLEDGE_CATEGORIES.has(String(data.category).trim().toLowerCase())) {
      throw new Error(`createKnowledge: category '${data.category}' is reserved and cannot be written through this path.`);
    }
    if (!opts?.allowPlatformManagedKnowledge && isPlatformManagedKnowledgeSource(data?.source)) {
      throw new Error(`createKnowledge: source '${data.source}' is platform-managed and cannot be written through this path.`);
    }
    // Gate-1 storage-boundary PII/secret redaction (central choke-point for all
    // knowledge writes — see storage-helpers/pii-redaction-guard.ts).
    const g = redactRecordFields(data, ["title", "content"]);
    if (g.redactedClasses.length > 0) {
      data = g.data;
      console.info(`[pii-guard] agent_knowledge redacted classes: ${g.redactedClasses.join(",")}`);
    }
    if (!Number.isInteger(data.tenantId) || Number(data.tenantId) <= 0) {
      throw new Error("createKnowledge requires tenantId");
    }
    const { assertMemoryValueNotTombstoned } = await import("./memory/tombstones");
    await assertMemoryValueNotTombstoned({
      tenantId: Number(data.tenantId),
      source: "agent_knowledge",
      parts: [data.title, data.content],
    });
  ...
  async updateKnowledge(id: number, data: Partial<InsertKnowledge>, tenantId: number, opts?: { allowReservedCategory?: boolean; allowPlatformManagedKnowledge?: boolean }) {
    if (!tenantId) throw new Error("updateKnowledge requires tenantId");
    // Reserved-category backstop on the UPDATE path too: without this, a caller
    // could create a benign row then PATCH its category into a reserved,
    // trust-sensitive value (`messaging_pairing`) — re-opening the pairing
    // write primitive the create-path guard closes (post-edit-code-review
    // HIGH, 2026-07-09). Fail closed unless the trusted pairing path opts in.
    if (!opts?.allowReservedCategory && data?.category &&
        RESERVED_KNOWLEDGE_CATEGORIES.has(String(data.category).trim().toLowerCase())) {
      throw new Error(`updateKnowledge: category '${data.category}' is reserved and cannot be written through this path.`);
    }
    if (!opts?.allowPlatformManagedKnowledge && data?.source &&
        isPlatformManagedKnowledgeSource(data.source)) {
      throw new Error(`updateKnowledge: source '${data.source}' is platform-managed and cannot be written through this path.`);
    }
    const [existing] = await db.select({
      ...
      .where(and(eq(agentKnowledge.id, id), eq(agentKnowledge.tenantId, tenantId)));
    if (existing && !opts?.allowPlatformManagedKnowledge &&
        isPlatformManagedKnowledgeSource(existing.source)) {
      throw new Error("updateKnowledge: platform-managed knowledge cannot be changed through this path.");
    }
    // Gate-1 redaction also on the update path (defense-in-depth).
    const g = redactRecordFields(data, ["title", "content"]);
    if (g.redactedClasses.length > 0) {
      data = g.data;
      console.info(`[pii-guard] agent_knowledge(update) redacted classes: ${g.redactedClasses.join(",")}`);
    }
    if (data.title !== undefined || data.content !== undefined) {
      ...
      await assertMemoryValueNotTombstoned({
        tenantId,
        source: "agent_knowledge",
        parts: [data.title ?? existing?.title, data.content ?? existing?.content],
      });
    }
  ...
  async deleteKnowledge(id: number, tenantId: number, opts?: { allowPlatformManagedKnowledge?: boolean }) {
    if (!tenantId) throw new Error("deleteKnowledge requires tenantId");
    const [existing] = await db.select({ source: agentKnowledge.source }).from(agentKnowledge)
      .where(and(eq(agentKnowledge.id, id), eq(agentKnowledge.tenantId, tenantId)));
    if (existing && !opts?.allowPlatformManagedKnowledge &&
        isPlatformManagedKnowledgeSource(existing.source)) {
      throw new Error("deleteKnowledge: platform-managed knowledge cannot be removed through this path.");
    }
  ...
  async updateKnowledgeEmbedding(id: number, embedding: number[], tenantId: number) {
    assertValidTenantId(tenantId);
    await db.update(agentKnowledge).set({ embedding })
      .where(and(eq(agentKnowledge.id, id), eq(agentKnowledge.tenantId, tenantId)));
    try {
      const { storeEmbeddingVec } = await import("./embeddings");
      await storeEmbeddingVec("agent_knowledge", id, embedding, { tenantId });
    } catch (err) {
  ...
  // Task #119: tenantId is required — platform-wide reads must pass ALL_TENANTS.
  async getMemoryStats(personaId: number | undefined, tenantId: TenantScope) {
    const conditions = [];
    if (personaId !== undefined) conditions.push(eq(memoryEntries.personaId, personaId));
    const memScope = requiredTenantScope(memoryEntries.tenantId, tenantId);
    if (memScope) conditions.push(memScope);
    ...
    const knScope = requiredTenantScope(agentKnowledge.tenantId, tenantId);
    if (knScope) knowledgeConditions.push(knScope);
  ...
  // Task #119: tenantId is required — platform-wide reads must pass ALL_TENANTS.
  async getHeartbeatTasks(personaId: number | undefined, tenantId: TenantScope) {
    const conditions = [];
    if (personaId !== undefined) conditions.push(eq(heartbeatTasks.personaId, personaId));
    const hbScope = requiredTenantScope(heartbeatTasks.tenantId, tenantId);
    if (hbScope) conditions.push(hbScope);
  ...
  async getHeartbeatTask(id: number, tenantId: number) {
    assertValidTenantId(tenantId);
    const [task] = await db.select().from(heartbeatTasks)
      .where(and(eq(heartbeatTasks.id, id), eq(heartbeatTasks.tenantId, tenantId)));
    return task;
  }
  ...
  async updateHeartbeatTask(id: number, data: Partial<InsertHeartbeatTask>, tenantId: TenantScope) {
    const updates: any = { ...data };
    if (data.cronExpression) {
      updates.nextRunAt = getNextCronRun(data.cronExpression);
    }
    const tenantPredicate = requiredTenantScope(heartbeatTasks.tenantId, tenantId);
    const conditions = [eq(heartbeatTasks.id, id)];
    if (tenantPredicate) conditions.push(tenantPredicate);
    const [task] = await db.update(heartbeatTasks).set(updates).where(and(...conditions)).returning();
    return task;
  }
  ...
  async getHeartbeatLogs(limit = 50, personaId: number | undefined, tenantId: TenantScope) {
    const conds: any[] = [];
    // heartbeat_logs has no tenant column, but validation is still mandatory;
    // use the parent task's tenant column solely to resolve/validate scope.
    requiredTenantScope(heartbeatTasks.tenantId, tenantId);
  ...
  async searchConversations(query: string, tenantId: TenantScope): Promise<Array<Conversation & { snippet?: string }>> {
    const pattern = `%${query}%`;
    ...
    requiredTenantScope(conversations.tenantId, tenantId);
    const validTid = tenantId === ALL_TENANTS ? undefined : tenantId;
