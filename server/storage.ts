...
export interface IStorage {
  ...
  deleteMemoryEntry(id: number, tenantId?: number): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteMemoryEntry(id: number, tenantId?: number) {
    if (tenantId === undefined) _warnUnscoped("deleteMemoryEntry", id);
    const conds = [eq(memoryEntries.id, id)];
    const tScope = tenantScope(memoryEntries.tenantId, tenantId);
    if (tScope) conds.push(tScope);
  ...
  async touchMemoryEntries(ids: number[], tenantId?: number) {
    ...
    if (tenantId === undefined) _warnUnscoped("touchMemoryEntries", ids[0]);
    const conds = [inArray(memoryEntries.id, ids)];
    const tScope = tenantScope(memoryEntries.tenantId, tenantId);
    if (tScope) conds.push(tScope);
