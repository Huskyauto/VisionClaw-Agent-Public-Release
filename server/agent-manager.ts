...
export async function launchAutonomousConversation(params: {
  ...
  personaId?: number;
  model?: string;
  /** R125+144 — AgentRadio passive awareness: channel polled between tool rounds for teammate discoveries. */
  radioChannel?: string;
  ...
  await reportDelegationAttempt({
    ...
    requesterIdentity: "autonomous-tool",
    targetAgent: params.personaId ? `persona:${params.personaId}` : "main",
    supervisionMode: "tiered",
    source: "internal",
    delegationId: `autonomous:${runId}`,
    attemptId: "initial",
  });
  ...
  setImmediate(async () => {
    try {
      const result = await processMessage(
        conv.id,
        params.task,
        { tenantId: params.tenantId, source: `autonomous:${runId}`, enableTools: true, depth: 1, radioChannel: params.radioChannel }
      );
