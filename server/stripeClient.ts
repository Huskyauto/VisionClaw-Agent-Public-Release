...
import { getCurrentLedgerAttempt } from './lib/action-ledger-context';
...
// ── Action Ledger S3 (contract: data/feature-contracts/action-ledger/) ──────
// Sanctioned threading point for the ledger's idempotency key into native
// Stripe requests. When a Stripe mutation executes UNDER a ledgered tool
// dispatch (destructive-risk tools — the S2 middleware sets ALS context after
// a successful prepare write), merge the deterministic `vc-al1-…` key into the
// per-request options so Stripe dedupes a reconciled retry within its 24h
// window. Usage at any money-movement callsite:
//   await stripe.payouts.create(payload, withLedgerIdempotency());
// Rules:
//   - An explicitly supplied idempotencyKey ALWAYS wins (e.g. the checkout
//     routes' buildCheckoutIdempotencyKey above — those are HTTP-route flows,
//     not tool dispatches, and keep their own fingerprint keys).
//   - Outside a ledgered dispatch this returns the options untouched — never
//     invent a fallback key (a made-up key defeats provider dedupe).
export function withLedgerIdempotency(
  options: Stripe.RequestOptions = {},
): Stripe.RequestOptions {
  if (options.idempotencyKey) return options;
  const attempt = getCurrentLedgerAttempt();
  if (!attempt) return options;
  return { ...options, idempotencyKey: attempt.idempotencyKey };
}
