...
import { checkToolRateLimitPaced, recordToolUsage, rateLimitAlternatesHint } from "./tool-rate-limiter";
import { getPersonaBlockedTools } from "./tool-router";

import { logSilentCatch } from "./lib/silent-catch";
import { shouldSkipApprovalGate, trustedExecFlags } from "./safety/trusted-exec-stamp";
import type { CmmcAuthoritativeSourceLock } from "./lib/tool-routing-context";
...
export interface GuardedExecContext {
  ...
  /**
   * Server-owned source constraint derived from trusted conversation history.
   * Never read this from model-authored tool arguments.
   */
  authoritativeSourceLock?: CmmcAuthoritativeSourceLock;
  /**
   * Trusted request-local notification fired after every policy/rate/HITL gate
   * passes and immediately before the underlying tool dispatch begins.
   */
  onDispatch?: () => void;
}

const CMMC_SAM_LOCKED_TOOLS = new Set([
  "browser",
  "stealth_browse",
...
const CMMC_MODE_TOOL: Record<CmmcAuthoritativeSourceLock, string> = {
  cmmc_sam_exact: "lookup_sam_exact_company",
  cmmc_sam_batch: "lookup_sam_company_cage",
  cmmc_sam_discovery: "discover_cmmc_prospects",
};
...
export async function executeGuardedTool(
  ...
  // SECURITY (R125+75): tenant is sourced ONLY from the trusted execution
  // context, NEVER from caller/model-authored args. Every current call site
  // (chat_engine, main_chat, public_chat, self_heal, repo_surgeon,
  // glasses_gateway) sets ctx.tenantId explicitly; lobster/plan-executor/
  // task-planner deliberately do NOT route through here. Reading args._tenantId
  // as a fallback would let a model emit `_tenantId` in toolArgs to steer a
  // cross-tenant execution if a future caller ever forgot ctx.tenantId. Drop it
  // and fail CLOSED below — same "stop the consumer reading the caller field"
  // pattern used for _approvedByGate.
  ...
  const requiredCmmcTool = ctx.authoritativeSourceLock ? CMMC_MODE_TOOL[ctx.authoritativeSourceLock] : undefined;
  const wrongCmmcMode = requiredCmmcTool
    && ["lookup_sam_exact_company", "lookup_sam_company_cage", "discover_cmmc_prospects"].includes(toolName)
    && toolName !== requiredCmmcTool;
  if (requiredCmmcTool && (CMMC_SAM_LOCKED_TOOLS.has(toolName) || wrongCmmcMode)) {
    console.warn(`[guarded-exec] CMMC source lock blocked tool=${toolName} via=${ctx.invokedVia} tenant=${tenantId}`);
    return {
      error: `Tool '${toolName}' is blocked for this SAM.gov request. Use '${requiredCmmcTool}' as selected from the user's trusted conversation context; do not substitute another SAM mode, introspection, delegation, browser, search, fetch, or deep research.`,
      blocked: true,
  ...
  try {
    ...
    const policyResult = await enforceToolPolicy(toolName, args, {
      ...
      // SECURITY: approval is sourced ONLY from the trusted execution context
      // (ctx.skipApprovalGate — set by main_chat's own SSE-streamed HITL gate),
      // NEVER from caller/model-authored args. Nothing in the repo legitimately
      // produces `_approvedByGate: true`; reading it here would let a model emit
      // it in toolArgs to self-approve a destructive tool. Fail-CLOSED.
      hasApproval: ctx.skipApprovalGate === true,
    });
  ...
  // Paced check: when only the per-minute window is full, wait once (≤70s)
  // for a slot instead of failing the call — Felix's research runs were dying
  // on web_fetch 8/8-per-minute here while the dispatcher middleware already
  // paced. Hour/day ceilings still hard-deny.
  const rateCheck = await checkToolRateLimitPaced(tenantId, toolName);
  if (!rateCheck.allowed) {
    console.warn(`[guarded-exec] RATE LIMITED: tool=${toolName} tenant=${tenantId} via=${ctx.invokedVia} reason=${rateCheck.reason}`);
    return { error: `RATE LIMITED: ${rateCheck.reason}${rateLimitAlternatesHint(toolName, rateCheck.window)}` };
  }
  ...
  // is exempt because it runs in a constrained sandbox already.
  //
  // SECURITY: the self-heal exemption is sourced ONLY from the trusted
  // execution context (ctx.invokedVia === "self_heal", set by the self-heal
  // supervisor), NEVER from a model-authored `args._selfHeal`. Tool args are
  // model-authored — reading `_selfHeal` from them let a jailbroken model emit
  // `_selfHeal:true` in toolArgs to skip requestToolConfirmation() for
  // high-risk / confirmation-required tools. Same "stop the consumer reading
  // the caller field" pattern as _approvedByGate / _tenantId above. Fail-CLOSED.
  const skipGate = shouldSkipApprovalGate(ctx);
  if (!skipGate) {
    try {
  ...
  try {
    ...
    // hatch still works because we re-stamp `_selfHeal` from the TRUSTED ctx
    // below — exec / lobster check `_selfHeal` explicitly before running.
    //
    // SECURITY: `_selfHeal` is derived ONLY from ctx.invokedVia (trusted),
    // and the override after the spread DELIBERATELY clobbers any model-authored
    // `args._selfHeal`. Without this, a jailbroken model could emit
    // `_selfHeal:true` in toolArgs to satisfy the `params._selfHeal !== true`
    // owner-gate bypass in tools.ts (exec ~12714, lobster ~13446), since the
    // spread would otherwise carry the forged value straight through. The
    // destructive-tool policy already fail-closes exec for untrusted personas,
    // but this keeps the owner-gate from trusting a caller-supplied field.
    const execArgs = {
      ...args,
      _skipTracking: true,
      _rateLimitChecked: true,
      // Trusted underscore flags spread LAST so they clobber any model-forged
      // _tenantId / _invokedByModel / _invokedVia / _selfHeal in the caller args.
      ...trustedExecFlags(ctx, tenantId),
    };
    ctx.onDispatch?.();
