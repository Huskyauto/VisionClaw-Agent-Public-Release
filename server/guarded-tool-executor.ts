...
export async function executeGuardedTool(
  ...
  if (ctx.authoritativeSourceLock === "cmmc_sam" && CMMC_SAM_LOCKED_TOOLS.has(toolName)) {
    console.warn(`[guarded-exec] CMMC source lock blocked tool=${toolName} via=${ctx.invokedVia} tenant=${tenantId}`);
    return {
      error: `Tool '${toolName}' is blocked while the SAM.gov authoritative-source lock is active. Use lookup_sam_exact_company for one named company or discover_cmmc_prospects for a roster; do not substitute delegated, browser, search, fetch, or deep-research sources.`,
      blocked: true,
