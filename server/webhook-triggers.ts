...
// Keys are bearer capabilities. Keep the accepted representation deliberately
// narrow: createTrigger has always emitted 16 random bytes as lowercase hex,
// and accepting any other representation would make path decoding/coercion
// part of capability resolution.
const WEBHOOK_KEY_BYTES = 16;
const WEBHOOK_KEY_PATTERN = /^[a-f0-9]{32}$/;

export function isValidWebhookKey(value: unknown): value is string {
  return typeof value === "string" && WEBHOOK_KEY_PATTERN.test(value);
}
...
export async function createTrigger(config: {
  ...
  // The database UNIQUE constraint is authoritative. Retrying only a
  // uniqueness collision keeps a mocked/failed RNG or an astronomically rare
  // collision from turning into a duplicate capability.
  for (let attempt = 0; attempt < 3; attempt++) {
    const webhookKey = crypto.randomBytes(WEBHOOK_KEY_BYTES).toString("hex");
    if (!isValidWebhookKey(webhookKey)) {
      throw new Error("Generated webhook key failed validation");
    }
    try {
      const result = await db.execute(sql`
  ...
  throw new Error("Unable to allocate a unique webhook key");
}
...
export async function processTriggerEvent(webhookKey: string, payload: any): Promise<{ success: boolean; response?: string; error?: string }> {
  // Reject malformed bearer capabilities before touching the database. In
  // particular, do not trim, decode, coerce, or otherwise broaden the token.
  if (!isValidWebhookKey(webhookKey)) {
    return { success: false, error: "Webhook trigger not found or disabled" };
  }
  await ensureTriggerTables();

  // Do not narrow this lookup to WHERE w.webhook_key = ${webhookKey} AND w.enabled = true:
  // a disabled duplicate would otherwise be hidden and an ambiguous capability
  // could authorize the enabled row.
  const result = await db.execute(sql`
    SELECT w.*, p.name as persona_name
    FROM webhook_triggers w
    LEFT JOIN personas p ON p.id = w.persona_id
    WHERE w.webhook_key = ${webhookKey}
  `);
  const rows = (result as any).rows || result;
  // Never select the first match. A duplicate key (including a duplicate
  // involving a disabled row) is ambiguous and must not authorize an action.
  const trigger = rows?.length === 1 ? rows[0] : undefined;

  if (!trigger || trigger.enabled !== true || !isValidWebhookKey(trigger.webhook_key)) {
    return { success: false, error: "Webhook trigger not found or disabled" };
  }
