...
export async function runHeartbeatScheduledWork(tickCount: number): Promise<void> {
  ...
  // Unified memory retention/forgetting. Claims make mutations idempotent
  // across concurrent heartbeat instances; report-only is the default.
  if (tickCount % 360 === 0) {
    try {
      const { runScheduledMemoryForgetting } = await import("./memory/forgetting-scheduler");
      const r = await runScheduledMemoryForgetting();
      console.log(`[heartbeat] Memory forgetting (${r.mode}): tenants=${r.tenants} candidates=${r.candidates} archived=${r.archived} purged=${r.purged} failed=${r.failed}`);
    } catch (e) {
      console.error("[heartbeat] Memory forgetting tick failed:", errorMessage(e));
    }
  }
