...
import { normalizeRetiredModelId } from "./lib/param-adaptation";
...
export const MODEL_COST_PER_MILLION: Record<string, { input: number; output: number }> = {
  ...
  // Platform default since the Sol migration; keep this verified rate in lockstep
  // with cost-ledger and insights-engine.
  "gpt-5.6-sol": { input: 2.00, output: 10.00 },
  "gpt-6-luna": { input: 0.10, output: 0.50 },
  "gpt-6-sol": { input: 2.00, output: 10.00 },
  "gpt-4.1": { input: 2.00, output: 8.00 },
  // Task 72 — gpt-5 upgrade; live OpenAI rate $1.25/$10 per M. Keep in sync with
  // cost-ledger + insights-engine (tests/cost/model-pricing-drift.test.ts).
  "gpt-5": { input: 1.25, output: 10.00 },
  "gpt-4.1-mini": { input: 0.40, output: 1.60 },
  "claude-sonnet-4-5": { input: 3.00, output: 15.00 },
  // Live OpenRouter rate $5/$25 per M (verified 2026-07-21); the old $15/$75 here
  // was Opus-4.1-era pricing carried forward.
  ...
  "tencent/hy4-preview": { input: 0.834, output: 2.501 },
  ...
  "meta-llama/llama-4-maverick": { input: 0.20, output: 0.696 },
  "meta/muse-spark-1.1": { input: 1.25, output: 4.25 },
  "nvidia/nemotron-3-super-120b-a12b": { input: 0.085, output: 0.40 },
...
export function estimatePlanCost(
  ...
  const canonicalModelId = normalizeRetiredModelId(modelId);
  const modelCost = getModelCost(canonicalModelId);
...
export function estimateQueryCost(
  ...
  return {
    ...
    modelBreakdown: [{ model: canonicalModelId, calls: 1, costUsd: Number(cost.toFixed(6)) }],
    riskLevel: cost > 0.05 ? "high" : cost > 0.01 ? "medium" : "low",
    recommendation: `Single query on ${canonicalModelId}: ~$${cost.toFixed(4)}, ~${timeSeconds}s.`,
  };
}
