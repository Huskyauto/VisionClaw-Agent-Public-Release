...
export const MODEL_COST_PER_MILLION: Record<string, { input: number; output: number }> = {
  ...
  // R125+137.3 — platform default since the Sol migration; flagship-class estimate
  // (matches gpt-5.4) pending a verified live rate. Keep in sync with cost-ledger +
  // insights-engine (cross-map drift guard: tests/cost/model-pricing-drift.test.ts).
  ...
  // Gemini 3.7 Flash — escalation-only lane (2026-08-13); $0.75/$3.75 per M,
  // synced with cost-ledger + insights-engine (price doubles 2027-01-01).
  "google/gemini-3.7-flash": { input: 0.75, output: 3.75 },
  "deepseek/deepseek-r1": { input: 0.70, output: 2.50 },
  "z-ai/glm-5.2": { input: 0.4746, output: 1.4916 },
  "z-ai/glm-5.1": { input: 0.952, output: 2.992 },
  "z-ai/glm-4.5-air:free": { input: 0.00, output: 0.00 },
  "google/gemma-4-31b-it": { input: 0.10, output: 0.34 },
  "moonshotai/kimi-k2.6": { input: 0.5795, output: 2.44 },
