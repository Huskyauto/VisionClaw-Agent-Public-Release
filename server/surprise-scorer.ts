...
export async function scoreProposalSurprise(proposalId: number, tenantId: number): Promise<SurpriseResult> {
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("scoreProposalSurprise requires a positive tenantId");
  }
  try {
    ...
    const argsRow: any = await db.execute(sql`
      SELECT args_embedding FROM felix_proposals
      WHERE id = ${proposalId} AND tenant_id = ${tenantId}
    `);
    ...
    const distRow: any = await db.execute(sql`
      ...
      FROM felix_proposals
      WHERE id = ANY(${neighborIds}::int[]) AND tenant_id = ${tenantId}
    `);
