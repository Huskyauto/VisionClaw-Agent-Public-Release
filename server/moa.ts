...
import { COST_LEDGER_RECORDED, getClientForModel, MODEL_REGISTRY, meteredLlmEnabled, providerLaneCanServeModel, isProviderHealthy } from "./providers";
import type { ProviderLane } from "./providers";
import { sanitizeTierOverride } from "./model-tier-eval";
import { recordCost, estimateCostUsd, ownerJuryMeteredEnabled, ownerJuryCeiling, noteOwnerJurySpend, ownerJuryRunReserveUsd } from "./agentic/cost-ledger";
import { ADMIN_TENANT_ID } from "./auth";
import { generateEmbedding, cosineSimilarity } from "./embeddings";
import { logSilentCatch } from "./lib/silent-catch";
import type { SecondOpinionResult } from "./second-opinion";

// The default frontier quorum deliberately uses three separate account and
// transport identities. A model name alone is insufficient here: generic
// routing may substitute a free endpoint, silently turning three parallel
// votes into a single provider's rate-limit burst. These lane pins are passed
// to the provider choke point and never fall through across providers.
interface FrontierLaneSpec {
  modelId: string;
  providerLane: ProviderLane;
}

const DEFAULT_FRONTIER_LANES: readonly FrontierLaneSpec[] = [
...
const METERED_JURY_RECOVERY_LANES: readonly FrontierLaneSpec[] = [
  { modelId: "z-ai/glm-5.3", providerLane: "zai" },
  { modelId: "z-ai/glm-5.2", providerLane: "openrouter" },
];
// Owner-operated discovery variant: preserve two independent flat/free
// proposer lanes while allowing a separately selected final synthesizer.
// It intentionally does not alter the default three-seat frontier jury.
const FRONTIER_LITE_LANES: readonly FrontierLaneSpec[] = [
  { modelId: "openference/deepseek-v4-pro", providerLane: "openference" },
  { modelId: "gpt-5.6-sol", providerLane: "profundo" },
];
const FRONTIER_PROPOSERS = DEFAULT_FRONTIER_LANES.map((spec) => spec.modelId);
...
// FRONTIER RESERVE (Bob 2026-07-16): Kimi-K3 (verified live on OpenRouter
// 2026-07-16 — 1M ctx, $3/M in $15/M out). NOT part of the standing jury —
// fires ONLY as a backfill when a main-round proposer fails, so the jury keeps
// its full voice count. Gated on useMetered: with metered OFF the $0 policy
// would silently swap kimi-k3 → modelfarm gpt-5.4, duplicating an existing
// lane instead of adding one (see memory: modelfarm-zero-cost-routing).
// Max ONE reserve fire per run — this is a backstop, not a cost multiplier.
export const FRONTIER_RESERVE = ["moonshotai/kimi-k3"];

// Pure gate for the reserve backfill (exported for tests). FRONTIER-ONLY:
// never fires for cheap/mixed/polarity pools or caller-explicit proposer sets,
// never on the metered-off path, never on total failure (that's the escalate
// path), never when the reserve id is already a proposer.
export function shouldFireFrontierReserve(args: {
  useMetered: boolean;
  poolChoice: ProposerPool | undefined;
  explicitProposerIds: boolean;
  successCount: number;
  specCount: number;
  proposerIds: string[];
}): boolean {
  const frontierRun = !args.explicitProposerIds && (args.poolChoice === undefined || args.poolChoice === "frontier");
  return (
    frontierRun &&
    args.useMetered &&
    args.successCount > 0 &&
    args.successCount < args.specCount &&
    FRONTIER_RESERVE.length > 0 &&
    !args.proposerIds.includes(FRONTIER_RESERVE[0])
  );
}

// Bob 2026-06-26 — the only $0 modelfarm lanes the OpenAI-SDK router can drive.
// When metered is off (no owner override) the jury proposers are spread across
// these DISTINCT free models instead of all collapsing onto gpt-5.4 (which 429s
// under the parallel burst and silently drops proposers). Order = powerful /
// reasoning / balanced for a little intra-family diversity.
const DISTINCT_FREE_PROPOSERS = ["gpt-5.4", "gpt-5"];
...
// across lineage (NVIDIA / Ant / Xiaomi / Google / Zhipu) so κ-concordance
// has a real signal rather than echo-chamber agreement.
export const CHEAP_PROPOSERS = [
  "nvidia/nemotron-3-super-120b-a12b",  // NVIDIA, capable low-cost parallel worker
...
// PREMIUM_PROPOSERS (Bob 2026-08-13 — the "best of the best" program): the
// three cheapest frontier-CLASS models draft in parallel, then the aggregator
// redrafts. Near-frontier quality at a fraction of the all-frontier price;
// this is the default pool for auto-detected hard-reasoning work.
const PREMIUM_PROPOSERS = [
  "deepseek/deepseek-v4.1-flash",    // $0.15/$0.60 per M — multimodal agentic flagship
  "gpt-5.6-sol",                     // Profundo flat lane, $0 marginal
  "google/gemini-3.7-flash",         // .75/3.75 per M
];
...
export const MIXED_PROPOSERS = [
  ...FRONTIER_PROPOSERS,                                  // all frontier (4 incl deepseek-v4-pro)
  "nvidia/nemotron-3-super-120b-a12b",                    // + 3 lineage-diverse cheap; preferred economical worker first
  "inclusionai/ling-2.6-1t:free",
  "xiaomi/mimo-v2-flash",
];

export const DEFAULT_AGGREGATOR = "claude-opus-5";
const DEFAULT_AGGREGATOR_SPEC: FrontierLaneSpec = {
  modelId: DEFAULT_AGGREGATOR,
  providerLane: "claude-runner",
};
const FALLBACK_AGGREGATOR_SPEC: FrontierLaneSpec = {
  modelId: "openference/deepseek-v4-pro",
  providerLane: "openference",
};

export function defaultAggregatorSpecForTenant(tenantId: number): FrontierLaneSpec {
  return tenantId === ADMIN_TENANT_ID
    ? DEFAULT_AGGREGATOR_SPEC
    : FALLBACK_AGGREGATOR_SPEC;
}
...
// R125+52.16: claude-opus-5 (default Anthropic flagship) runs TWO traditions (munger + taleb), gpt-5.6-sol + google/gemini-3.7-flash
...
export interface ProposerResult {
  ...
  provider: string;
  providerLane?: ProviderLane;
  /** Requested lane/model and actual routed model are audit evidence, not labels. */
  requestedModel?: string;
  actualModel?: string;
  routeReason?: string;
  failureKind?: JuryFailureKind;
  retryAfterMs?: number;
  ok: boolean;
  answer?: string;
  latencyMs: number;
  ...
  tokensIn?: number;
  tokensOut?: number;
  // True when the provider response contained non-zero usage.
  usageReported?: boolean;
  // True only when the request-local provider facade durably recorded usage.
...
/** Classify failures without treating a provider's prose as a successful vote. */
export function _classifyJuryFailure(err: any): JuryFailureKind {
  const status = Number(err?.status ?? err?.response?.status);
  const message = String(err?.message || err || "").toLowerCase();
  if (status === 529 || /\b529\b|overload/.test(message)) return "provider_overload";
  if (/timed out|timeout|etimedout|abort/.test(message)) return "model_timeout";
  if (status === 429 || /rate.limit|retry-after/.test(message)) return "provider_rate_limit";
  if (status >= 500 && status <= 599 || /\bunavailable\b/.test(message)) return "provider_unavailable";
  if (status >= 400 && status <= 499) return "provider_error";
  return "unknown";
}

function retryAfterMs(err: any): number | undefined {
  const raw = err?.headers?.get?.("retry-after") ?? err?.headers?.["retry-after"] ??
    err?.response?.headers?.get?.("retry-after") ?? err?.response?.headers?.["retry-after"];
  const seconds = Number(raw);
  return Number.isFinite(seconds) && seconds >= 0 ? Math.min(Math.round(seconds * 1000), 1_000) : undefined;
}

export function juryLaneMarginalCostUsd(providerLane: ProviderLane | undefined): number | undefined {
  return providerLane === "openference" ||
    providerLane === "profundo" ||
    providerLane === "claude-runner"
    ? 0
    : undefined;
}

export function isJuryCostExempt(providerLane: ProviderLane | undefined): boolean {
  // Cost exemption is an attribute of a verified flat/subscription lane, never
  // of the general jury feature. An unpinned caller-selected model can still
  // be metered and must flow through the normal breaker/accounting path.
  return juryLaneMarginalCostUsd(providerLane) === 0;
}

export function shouldUseJuryProviderMarker(providerLane: ProviderLane | undefined): boolean {
  // The provider-level :jury marker bypasses the metered breaker. Only the
  // closed, verified flat/subscription lane union is entitled to that bypass.
  return isJuryCostExempt(providerLane);
}

export function shouldRecordEstimatedMoACost(
  usageReported: boolean,
  usagePersisted: boolean | undefined,
  tokensIn: number | undefined,
  tokensOut: number | undefined,
): boolean {
  return (!usageReported || usagePersisted !== true) && Boolean(tokensIn || tokensOut);
}
...
export interface MoAResult {
  ...
  /** Original ballot size; failed seats do not reduce this quorum requirement. */
  jurySeatCount?: number;
  ...
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => {
      try {
        ...
        console.warn(`[moa] ${label} timeout cleanup failed: ${(err as Error).message?.slice(0, 120)}`);
      }
...
export interface ProposerCallSpec {
  modelId: string;
  providerLane?: ProviderLane;
  systemPrompt?: string;
  label?: string;
...
/**
 * Select a replacement without ever reusing a lane occupied by another seat.
 * This is intentionally a selector, not a fallback router: a failed seat is
 * never silently rerouted through a surviving provider account.
 */
export function _selectJuryLaneReplacement(args: {
  failed: Pick<ProposerCallSpec, "modelId" | "providerLane">;
  occupiedLanes: ReadonlySet<ProviderLane>;
  healthy?: (lane: ProviderLane) => boolean;
  allowMetered?: boolean;
}): FrontierLaneSpec | null {
  const healthy = args.healthy ?? isProviderHealthy;
  const candidates = args.allowMetered
    ? [...DEFAULT_FRONTIER_LANES, ...METERED_JURY_RECOVERY_LANES]
    : DEFAULT_FRONTIER_LANES;
  for (const candidate of candidates) {
    if (
      candidate.providerLane !== args.failed.providerLane &&
      !args.occupiedLanes.has(candidate.providerLane) &&
      providerLaneCanServeModel(candidate.providerLane, candidate.modelId) &&
      healthy(candidate.providerLane)
    ) return { ...candidate };
  }
  return null;
}

async function callProposer(specOrId: string | ProposerCallSpec, question: string, tenantId: number, meteredOverride = false): Promise<ProposerResult> {
  const spec: ProposerCallSpec = typeof specOrId === "string" ? { modelId: specOrId } : specOrId;
  const modelId = spec.modelId;
  const provider = spec.providerLane || MODEL_REGISTRY.find(m => m.id === modelId)?.provider || "unknown";
  const t0 = Date.now();
  try {
    const { client, actualModelId } = await getClientForModel(modelId, tenantId, {
      juryLane: shouldUseJuryProviderMarker(spec.providerLane),
      meteredOverride,
      providerLane: spec.providerLane,
    });
    ...
    const resp = await withTimeout(
      ...
      }, { signal: abort.signal }) as Promise<any>,
    ...
    const usagePersisted = resp?.[COST_LEDGER_RECORDED] === true;
    ...
    const tokensIn = usage?.prompt_tokens ?? usage?.input_tokens ?? usage?.promptTokenCount ?? Math.ceil(question.length / 4);
    const tokensOut = usage?.completion_tokens ?? usage?.output_tokens ?? usage?.candidatesTokenCount ?? Math.ceil(answer.length / 4);
    return {
      modelId, provider, providerLane: spec.providerLane,
      requestedModel: modelId, actualModel: actualModelId,
      routeReason: spec.providerLane ? "pinned-jury-lane" : "provider-router",
      ok: !!answer, answer,
      latencyMs: Date.now() - t0,
      error: answer ? undefined : "empty response",
      failureKind: answer ? undefined : "provider_error",
      tokensIn, tokensOut, usageReported, usagePersisted,
      label: spec.label,
    ...
    return {
      ...
      failureKind: _classifyJuryFailure(err),
...
export function shouldRetryAggregatorOnFallback(spec: { modelId: string; providerLane?: ProviderLane }): boolean {
  // A lane pin is an isolation and evidence-integrity contract. Retrying it on
  // another account would hide the failed lane and turn a partial jury into a
  // falsely healthy one. Only an explicitly unpinned caller-selected aggregator
  // can use the named, explicit fallback policy.
  return !spec.providerLane && spec.modelId !== FALLBACK_AGGREGATOR_SPEC.modelId;
}

export function assertExactAggregatorRoute(requestedModelId: string, actualModelId: string): void {
  if (actualModelId !== requestedModelId) {
    throw new Error(
      `aggregator model mismatch: required ${requestedModelId}, routed ${actualModelId}`,
    );
  }
}

async function callAggregator(
  spec: ProposerCallSpec,
  prompt: string,
  tenantId: number,
  meteredOverride = false,
  requireExactModel = false,
  forbidMeteredFallback = false,
): Promise<{ answer: string; modelUsed: string; providerLane?: ProviderLane; tokensIn: number; tokensOut: number; usageReported: boolean; usagePersisted: boolean }> {
  const modelId = spec.modelId;
  try {
    const { client, actualModelId } = await getClientForModel(modelId, tenantId, {
      juryLane: shouldUseJuryProviderMarker(spec.providerLane),
      meteredOverride,
      providerLane: spec.providerLane,
      forbidMeteredFallback,
    });
    if (requireExactModel) assertExactAggregatorRoute(modelId, actualModelId);
    const abort = new AbortController();
    const resp = await withTimeout(
      client.chat.completions.create({
        model: actualModelId,
    ...
    const tokensIn = usage?.prompt_tokens ?? usage?.input_tokens ?? usage?.promptTokenCount ?? Math.ceil(prompt.length / 4);
    const tokensOut = usage?.completion_tokens ?? usage?.output_tokens ?? usage?.candidatesTokenCount ?? Math.ceil(answer.length / 4);
    if (answer) return { answer, modelUsed: actualModelId, providerLane: spec.providerLane, tokensIn, tokensOut, usageReported, usagePersisted };
    throw new Error("empty aggregator response");
  } catch (err) {
    if (!requireExactModel && shouldRetryAggregatorOnFallback(spec)) {
      console.warn(`[moa] aggregator ${modelId} failed (${(err as Error).message?.slice(0, 120)}), falling back to explicit ${FALLBACK_AGGREGATOR_SPEC.providerLane} lane`);
      return callAggregator(FALLBACK_AGGREGATOR_SPEC, prompt, tenantId, meteredOverride, false, false);
    }
    throw err;
  }
}

export type ProposerPool = "frontier" | "frontier-lite" | "cheap" | "mixed" | "polarity" | "premium";
...
type TierOverride = {
  frontier: string[];
  mundane: string[];
  frontierLanes: FrontierLaneSpec[] | null;
};
let _tierOverrideCache: ({ at: number } & TierOverride) | null = null;

export function validateFrontierLanes(raw: unknown, frontier: string[]): FrontierLaneSpec[] | null {
  if (!raw || typeof raw !== "object" || !Array.isArray((raw as any).frontierLanes)) return null;
  const entries = (raw as any).frontierLanes;
  if (entries.length !== frontier.length || entries.length < TIER_OVERRIDE_MIN_FRONTIER) return null;
  const seenLanes = new Set<ProviderLane>();
  const specs: FrontierLaneSpec[] = [];
  for (let i = 0; i < entries.length; i++) {
    const entry = entries[i];
    if (!entry || typeof entry !== "object") return null;
    const modelId = typeof entry.modelId === "string" ? entry.modelId.trim() : "";
    const providerLane = entry.providerLane;
    if (
      modelId !== frontier[i] ||
      (providerLane !== "openference" && providerLane !== "profundo" && providerLane !== "claude-runner" && providerLane !== "anthropic-api") ||
      seenLanes.has(providerLane) ||
      !providerLaneCanServeModel(providerLane, modelId)
    ) {
      return null;
    }
    seenLanes.add(providerLane);
    specs.push({ modelId, providerLane });
  }
  return specs;
}

function loadTierOverride(): TierOverride | null {
  ...
  try {
    if (!fs.existsSync(TIER_OVERRIDE_FILE)) {
      _tierOverrideCache = { at: nowMs, frontier: [], mundane: [], frontierLanes: null };
      return null;
    }
    ...
    const result: TierOverride = {
      ...
      mundane: clean.mundane,
      frontierLanes: validateFrontierLanes(raw, clean.frontier),
    };
    _tierOverrideCache = { at: nowMs, ...result };
    return result;
  } catch {
    _tierOverrideCache = { at: nowMs, frontier: [], mundane: [], frontierLanes: null };
    return null;
...
export function resolveProposerPool(pool: ProposerPool): string[] {
  ...
  switch (pool) {
    ...
    // "premium" is a FIXED value trio by design — the weekly tier-override file
    // must not swap in pricier frontier ids under this cost-conscious pool.
    case "premium": return [...PREMIUM_PROPOSERS];
    case "frontier-lite": return FRONTIER_LITE_LANES.map((spec) => spec.modelId);
    case "frontier":
    default:
      return getFrontierLaneSpecs().map((spec) => spec.modelId);
  }
}

function getFrontierLaneSpecs(): FrontierLaneSpec[] {
  const override = loadTierOverride();
  // A dynamically refreshed roster is trusted only when every seat states a
  // distinct concrete lane. Missing/malformed lane metadata cannot downgrade us
  // to generic routing; retain the known provider-diverse fallback instead.
  return override?.frontierLanes
    ? override.frontierLanes.map((spec) => ({ ...spec }))
    : DEFAULT_FRONTIER_LANES.map((spec) => ({ ...spec }));
}
...
export function resolveProposerSpecs(pool: ProposerPool | undefined, explicitIds: string[] | undefined): ProposerCallSpec[] {
  ...
  if (pool === "frontier-lite") {
    return FRONTIER_LITE_LANES.map((spec) => ({ ...spec }));
  }
  if (pool === "polarity") {
  ...
  if (pool === "frontier" || pool === undefined) {
    return getFrontierLaneSpecs().map((spec) => ({ ...spec }));
  }
  const ids = resolveProposerPool(pool);
...
export interface MoAOptions {
  ...
  proposerIds?: string[];
  aggregatorId?: string;
  // Require the provider router to return the requested final model exactly.
  // Used by diagnostics whose evidence contract names a particular model; it
  // rejects free-model and cross-provider substitutions instead of mislabeled output.
  strictAggregatorModel?: boolean;
  // Per-call billing floor for a strict aggregator. When the owner ceiling or
  // explicit override does not grant metered use, deny generic paid fallbacks
  // even if ALLOW_METERED_LLM is enabled globally.
  forbidMeteredAggregatorFallback?: boolean;
  invokedVia?: string;
  ...
  // Bob 2026-06-26 — OWNER explicit-jury paid override. Set true ONLY by the
  // ensemble_query / jury_triage tool executors when the invoker is the admin
  // tenant, so the owner's deliberate jury runs use REAL cross-provider metered
  // models (Claude + Gemini + DeepSeek) instead of collapsing onto the free
  // gpt-5.4 lane. Gated at runtime by ownerJuryMeteredEnabled() + a daily $
  // ceiling (ownerJuryCeiling). The AUTO heavy-loop path never sets this.
...
export function deriveRestateSpecs(specs: ProposerCallSpec[]): ProposerCallSpec[] {
  return specs.map(s => ({
    modelId: s.modelId,
    providerLane: s.providerLane,
...
async function runRestateGate(
  ...
  const restateSpecs = deriveRestateSpecs(specs);
...
async function runSteelmanRound(
  ...
  // top-tier models (claude-opus-5 + gpt-5.6-sol) — distinct providers chosen for
  // provider diversity. R125+63 (Bob 2026-06-23): Claude Fable 5 was removed
  // entirely (Anthropic shut it down); the Anthropic slot is now Claude Opus 5.
...
export async function executeMoA(opts: MoAOptions): Promise<MoAResult> {
  ...
  if (poolChoice === "polarity" || poolChoice === "frontier-lite" || baseSpecs.some((spec) => spec.providerLane)) {
  ...
  // ── Jury lane selection (Bob 2026-06-26) ──────────────────────────────────
  // The $0 policy substitutes every metered proposer (Claude/Gemini/DeepSeek +
  // the unsupported gpt-5.6-sol) onto the SINGLE free gpt-5.4 lane — so a 4-way
  // parallel jury hammers one endpoint, 429s, and silently drops proposers (κ
  // collapses to a singleton). Two fixes:
  //   (1) OWNER paid override — when the owner (admin tenant) EXPLICITLY runs the
  //       jury, allow the real cross-provider metered models, gated by a daily $
  //       ceiling backstop so a bug can't repeat the $933 Jun-11/12 burst.
  //   (2) FREE diversity — otherwise spread the proposers across DISTINCT free
  //       distinct modelfarm lanes (gpt-5.4 / gpt-5) so all of them answer
  //       and no single lane is saturated. Preserves each spec's prompt/label/role.
  let useMetered = false;
  let meteredRecoveryActive = false;
  let ceilingForcedFree = false;
  let reservedJuryUsd = 0;
  // The default roster is entirely flat/subscription-backed. Do not reserve a
  // fictional metered budget merely because the owner supplied an override;
  // only a caller-selected generic proposer or aggregator can spend it.
  const hasPotentialMeteredSeat = proposerSpecs.some((spec) =>
    !spec.providerLane || spec.providerLane === "anthropic-api" || spec.providerLane === "zai" || spec.providerLane === "openrouter",
  ) || !!opts.aggregatorId;
  // Defense-in-depth: the metered override is honored ONLY for the owner (admin
  // tenant). The tool wrappers (ensemble_query / jury_triage) already gate on
  // this, but re-check here so no internal caller can smuggle meteredOverride
  // under a non-owner tenant and bill the paid lanes.
  const ownerOverride = opts.meteredOverride === true && opts.tenantId === ADMIN_TENANT_ID;
  const ownerMeteredRecoveryEligible = ownerOverride && ownerJuryMeteredEnabled();
  if (ownerOverride && ownerJuryMeteredEnabled() && hasPotentialMeteredSeat) {
    const ceiling = ownerJuryCeiling();
    if (ceiling.exceeded) {
      console.warn(`[moa] tenant=${opts.tenantId} owner-jury metered override REQUESTED but daily ceiling reached ($${ceiling.spent.toFixed(2)}/$${ceiling.ceiling}) — forcing free lanes`);
      ceilingForcedFree = true;
    } else {
      useMetered = true;
      // Reserve-then-settle: accrue a conservative per-run reservation NOW so an
      // OVERLAPPING owner-initiated run sees this in-flight spend in its own
      // ceiling pre-check above (the read-only check alone overshoots under
      // concurrency). Reconciled to the actual cost at settle time below.
      reservedJuryUsd = ownerJuryRunReserveUsd();
      if (reservedJuryUsd > 0) noteOwnerJurySpend(reservedJuryUsd);
    }
  } else if (ownerOverride && !hasPotentialMeteredSeat) {
    console.log(`[moa] tenant=${opts.tenantId} owner-jury override requested for an all flat/subscription roster — no metered reservation needed`);
  }
  // Free-diversity remap fires when we are NOT running metered AND either:
  //   (a) the owner daily ceiling forced us off the paid path — an ABSOLUTE
  //       backstop that holds even when ALLOW_METERED_LLM=true globally (without
  //       this, an exceeded ceiling under global metered would keep the frontier
  //       metered model IDs and blow straight past the ceiling), OR
  //   (b) metered is globally off ($0 policy) — the default jury-collapse fix.
  if (!useMetered && (ceilingForcedFree || !meteredLlmEnabled())) {
    proposerSpecs = proposerSpecs.map((s, i) => ({
      ...s,
      // Dedicated flat/subscription lanes remain pinned even under the global
      // $0 policy. Remapping their model IDs would make a strict lane fail and,
      // worse, would reintroduce the single-Modelfarm collapse this roster fixes.
      modelId: s.providerLane ? s.modelId : DISTINCT_FREE_PROPOSERS[i % DISTINCT_FREE_PROPOSERS.length],
    }));
  } else if (useMetered) {
    const c = ownerJuryCeiling();
    console.log(`[moa] tenant=${opts.tenantId} OWNER metered jury — real cross-provider proposers (spent $${c.spent.toFixed(2)}/$${c.ceiling} today)`);
  }

  const proposerIds = proposerSpecs.map(s => s.modelId);
  const aggregatorSpec: ProposerCallSpec = opts.aggregatorId
    ? { modelId: opts.aggregatorId }
    : defaultAggregatorSpecForTenant(opts.tenantId);
  ...
  const settled = await Promise.allSettled(proposerSpecs.map(spec => callProposer(spec, question, opts.tenantId, useMetered)));
  ...
  // Bounded jury-seat recovery: exactly one transient failed pinned seat may be
  // supplemented by an UNUSED healthy lane. We retain the failed result as audit
  // evidence, but it is never a ballot and does not lower jurySeatCount. A
  // full three-lane frontier jury normally has no spare lane; that honest
  // outcome escalates rather than duplicating one of its two survivors.
  const failedPinned = proposers.filter(p =>
    !p.ok && p.providerLane &&
    (p.failureKind === "provider_overload" || p.failureKind === "model_timeout" || p.failureKind === "provider_unavailable" || p.failureKind === "provider_rate_limit"),
  );
  if (failedPinned.length > 0) {
    const occupiedLanes = new Set<ProviderLane>(
      proposers.map(p => p.providerLane).filter((lane): lane is ProviderLane => !!lane),
    );
    const majorityNeeded = Math.floor(proposerSpecs.length / 2) + 1;
    for (const failed of failedPinned) {
      if (successful.length >= majorityNeeded) break;
      let replacement = _selectJuryLaneReplacement({
        failed,
        occupiedLanes,
      });
      if (!replacement && ownerMeteredRecoveryEligible) {
        if (!useMetered && !meteredRecoveryActive) {
          const ceiling = ownerJuryCeiling();
          if (!ceiling.exceeded) {
            meteredRecoveryActive = true;
            reservedJuryUsd = ownerJuryRunReserveUsd();
            if (reservedJuryUsd > 0) noteOwnerJurySpend(reservedJuryUsd);
            console.log(`[moa] tenant=${opts.tenantId} activating bounded metered jury recovery (spent $${ownerJuryCeiling().spent.toFixed(2)}/$${ceiling.ceiling} today)`);
          } else {
            console.warn(`[moa] tenant=${opts.tenantId} metered jury recovery denied: daily ceiling reached ($${ceiling.spent.toFixed(2)}/$${ceiling.ceiling})`);
          }
        }
        if (useMetered || meteredRecoveryActive) {
          replacement = _selectJuryLaneReplacement({
            failed,
            occupiedLanes,
            allowMetered: true,
          });
      ...
      occupiedLanes.add(replacement.providerLane);
      // Retry-After is advisory evidence from the failed provider. Never sleep
      // beyond one second and never retry that provider; the sole bounded
      // recovery attempt uses the selected independent lane.
      if (failed.retryAfterMs) {
        await new Promise(resolve => setTimeout(resolve, failed.retryAfterMs));
      }
      const replacementResult = await callProposer(
        { ...replacement, label: `recovery(${failed.modelId})` },
        question,
        opts.tenantId,
        useMetered || meteredRecoveryActive,
      );
      replacementResult.routeReason = `jury-recovery:${failed.failureKind}`;
      proposers.push(replacementResult);
      successful = proposers.filter(p => p.ok && p.answer);
    }
  }

  // FRONTIER_RESERVE backfill — if a main-round proposer failed AND metered is
  // on, fire ONE reserve proposer (Kimi-K3) cloned from the first failed spec
  // (same prompt/label semantics) so the jury keeps its full voice count.
  // Best-effort / fail-open: a reserve failure never degrades the run below
  // what it already was. Skipped when the reserve id is already a proposer.
  // FRONTIER-ONLY (architect fix): the reserve is a FRONTIER backstop — it must
  // not fire for cheap/mixed/polarity pools or caller-explicit proposer sets
  // (those pools chose their own economics; backfilling with a $3/$15 flagship
  // would silently change their cost profile).
  if (
    !proposerSpecs.some((spec) => spec.providerLane) &&
    shouldFireFrontierReserve({
      useMetered,
      poolChoice,
      explicitProposerIds,
      successCount: successful.length,
      specCount: proposerSpecs.length,
      proposerIds,
    })
  ) {
    const failedIdx = proposers.findIndex(p => !p.ok);
    if (failedIdx >= 0) {
      const reserveSpec: ProposerCallSpec = {
        ...proposerSpecs[failedIdx],
        modelId: FRONTIER_RESERVE[0],
        label: `reserve(${proposerSpecs[failedIdx].label || proposers[failedIdx].modelId})`,
      };
      try {
        console.log(`[moa] tenant=${opts.tenantId} proposer ${proposers[failedIdx].modelId} failed — firing FRONTIER_RESERVE backfill ${FRONTIER_RESERVE[0]}`);
        const reserveResult = await callProposer(reserveSpec, question, opts.tenantId, useMetered);
        proposers.push(reserveResult);
        if (reserveResult.ok && reserveResult.answer) successful = proposers.filter(p => p.ok && p.answer);
      } catch (err) {
        console.warn(`[moa] FRONTIER_RESERVE backfill failed (non-fatal): ${String((err as Error).message).slice(0, 160)}`);
      }
    }
  }
  ...
  if (successful.length === 0) {
    ...
    return {
      ...
      proposers,
      jurySeatCount: proposerSpecs.length,
      totalLatencyMs,
  ...
  let aggProviderLane: ProviderLane | undefined;
  let aggTokensIn = 0;
  let aggTokensOut = 0;
  let aggUsageReported = false;
  let aggUsagePersisted = false;
  try {
    const r = await callAggregator(
      aggregatorSpec,
      prompt,
      opts.tenantId,
      useMetered,
      opts.strictAggregatorModel === true,
      opts.forbidMeteredAggregatorFallback === true && !useMetered,
    );
    answer = r.answer;
    modelUsed = r.modelUsed;
    aggProviderLane = r.providerLane;
    aggTokensIn = r.tokensIn;
    aggTokensOut = r.tokensOut;
    aggUsageReported = r.usageReported;
    aggUsagePersisted = r.usagePersisted;
  } catch (err) {
  ...
  // getClientForModel returns a request-local provider facade that records each
  // successful completion once, using the provider's actual usage fields. Some
  // OpenAI-compatible providers omit usage, though; only those successful calls
  // need one MoA-side estimate. The run-cost calculation below is intentionally
  // separate—it settles the owner's in-memory jury ceiling, not the durable ledger.
  try {
    const estimates: Promise<boolean>[] = [];
    for (const p of proposers) {
      if (p.ok && shouldRecordEstimatedMoACost(!!p.usageReported, p.usagePersisted, p.tokensIn, p.tokensOut)) {
        estimates.push(recordCost({
          tenantId: opts.tenantId,
          toolName: "ensemble_query",
          model: p.modelId,
          tokensIn: p.tokensIn || 0,
          tokensOut: p.tokensOut || 0,
          operation: `moa-proposer-estimate:${p.modelId}`,
          costExempt: isJuryCostExempt(p.providerLane),
          costUsd: juryLaneMarginalCostUsd(p.providerLane),
        }));
      }
    }
    if (
      modelUsed &&
      !modelUsed.startsWith("(aggregator-failed") &&
      shouldRecordEstimatedMoACost(aggUsageReported, aggUsagePersisted, aggTokensIn, aggTokensOut)
    ) {
      estimates.push(recordCost({
        tenantId: opts.tenantId,
        toolName: "ensemble_query",
        ...
        operation: `moa-aggregator-estimate:${modelUsed}`,
        costExempt: isJuryCostExempt(aggProviderLane),
        costUsd: juryLaneMarginalCostUsd(aggProviderLane),
      }));
    }
    const results = await Promise.allSettled(estimates);
    const fallbackFailed = results.some((result) =>
      result.status === "rejected" || result.value !== true
    );
    if (fallbackFailed && (useMetered || meteredRecoveryActive)) {
      throw new Error("paid jury cost accounting failed after provider usage persistence failure");
    }
  } catch (err) {
    if (useMetered || meteredRecoveryActive) throw err;
    console.warn("[moa] estimated cost-ledger fallback failed (non-fatal):", (err as Error).message?.slice(0, 120));
  }

  // Owner-jury metered backstop: settle THIS run's metered spend against the daily
  // ceiling so an overlapping burst can never repeat the $933 incident. A
  // conservative reservation was already accrued at grant time (reserve-then-settle
  // concurrency guard), so here we add only the EXCESS of actual-over-reserved. If
  // actual < reserved we keep the (slightly higher) reservation — the conservative
  // safe direction for a soft backstop. Only the owner-override path bills metered.
  if (useMetered || meteredRecoveryActive) {
    try {
      let runCost = 0;
      for (const p of proposers) {
        if (p.tokensIn || p.tokensOut) {
          runCost += juryLaneMarginalCostUsd(p.providerLane) ?? estimateCostUsd(p.modelId, p.tokensIn || 0, p.tokensOut || 0);
        }
      }
      if (aggTokensIn || aggTokensOut) {
        runCost += juryLaneMarginalCostUsd(aggProviderLane) ?? estimateCostUsd(modelUsed, aggTokensIn, aggTokensOut);
      }
      noteOwnerJurySpend(Math.max(0, runCost - reservedJuryUsd));
    } catch (_err) {
      logSilentCatch("server/moa.ts", _err);
    }
  }
  ...
  if (shouldEscalate && opts.autoSecondOpinion !== false) {
    try {
      ...
      if (fusionAutoEnabled()) {
        ...
        // Preserve failed/over-budget Fusion evidence for the caller. It is
        // advisory only and cannot change the jury verdict, but hiding it made
        // a 60s timeout indistinguishable from "no cross-check requested".
        secondOpinion = so;
      }
    } catch (err) {
