...
import { getClientForModel, MODEL_REGISTRY, meteredLlmEnabled } from "./providers";
...
// declared top-tier models. R125+62.x (Bob 2026-06-23): Claude Fable 5 REMOVED
// entirely — Anthropic shut it down (no longer on api.anthropic.com), so it is
// gone from MODEL_REGISTRY and every cost/context map. The Anthropic slot is
// Claude Opus 5 (covered by the Claude Runner flat-rate lane). GLM 5.2
// (z-ai/glm-5.2, OpenRouter, 1M ctx, cheap costClass) added as a top-tier
// registry model in the powerful + reasoning ladders.
// The four canonical high-end models: claude-opus-5, gpt-5.6-sol,
// gemini-3.5-flash, deepseek/deepseek-v4-pro-0813. DeepSeek V4 Pro is kept as a 4th
// top-tier proposer (Bob 2026-06-09); it is a cheap costClass flagship so the
// marginal jury cost of the 4th proposer is small.
// Proposer #1: Claude Opus 5 (Anthropic flagship; Claude Runner CLI bridge when available, else API).
// Proposer #2: GPT-5.6 Sol (OpenAI via Replit OAuth, free).
...
// Synthesizer: Claude Opus 5 (DEFAULT_AGGREGATOR; fallback GPT-5.6 Sol). Self-aggregation
...
const FALLBACK_AGGREGATOR = "gpt-5.6-sol";
...
// R125+52.16: claude-opus-5 (default Anthropic flagship) runs TWO traditions (munger + taleb), gpt-5.6-sol + gemini-3.5-flash
...
async function callProposer(specOrId: string | ProposerCallSpec, question: string, tenantId: number, meteredOverride = false): Promise<ProposerResult> {
  ...
  const t0 = Date.now();
  try {
    const { client, actualModelId } = await getClientForModel(modelId, tenantId, { juryLane: true, meteredOverride });
...
async function callAggregator(modelId: string, prompt: string, tenantId: number, meteredOverride = false): Promise<{ answer: string; modelUsed: string; tokensIn: number; tokensOut: number }> {
  try {
    const { client, actualModelId } = await getClientForModel(modelId, tenantId, { juryLane: true, meteredOverride });
    const resp = await withTimeout(
      client.chat.completions.create({
    ...
    if (modelId !== FALLBACK_AGGREGATOR) {
      ...
      return callAggregator(FALLBACK_AGGREGATOR, prompt, tenantId, meteredOverride);
    }
    throw err;
  }
}

export type ProposerPool = "frontier" | "cheap" | "mixed" | "polarity" | "premium";
...
export async function executeMoA(opts: MoAOptions): Promise<MoAResult> {
  ...
  if (ownerOverride && ownerJuryMeteredEnabled()) {
  ...
  if (!useMetered && (ceilingForcedFree || !meteredLlmEnabled())) {
    proposerSpecs = proposerSpecs.map((s, i) => ({
      ...s,
      modelId: DISTINCT_FREE_PROPOSERS[i % DISTINCT_FREE_PROPOSERS.length],
    }));
  } else if (useMetered) {
  ...
  let aggTokensOut = 0;
  try {
    const r = await callAggregator(aggregatorId, prompt, opts.tenantId, useMetered);
    answer = r.answer;
  ...
  if (useMetered) {
    try {
      let runCost = 0;
      for (const p of proposers) {
        if (p.tokensIn || p.tokensOut) runCost += estimateCostUsd(p.modelId, p.tokensIn || 0, p.tokensOut || 0);
      }
      if (aggTokensIn || aggTokensOut) runCost += estimateCostUsd(modelUsed, aggTokensIn, aggTokensOut);
