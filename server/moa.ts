...
function resolveProposerSpecs(pool: ProposerPool | undefined, explicitIds: string[] | undefined): ProposerCallSpec[] {
  if (explicitIds && explicitIds.length > 0) {
    return explicitIds.map(id => ({ modelId: id }));
  }
  if (pool === "polarity") {
