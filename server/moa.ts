...
import { getClientForModel, MODEL_REGISTRY, meteredLlmEnabled, providerLaneCanServeModel } from "./providers";
...
const PREMIUM_PROPOSERS = [
  "deepseek/deepseek-v4-pro-0813",   // .435/.87 per M — near-Opus agentic scores
...
export interface ProposerResult {
  ...
  tokensIn?: number;
  tokensOut?: number;
  // True only when the provider response contained non-zero usage that the
  // request-local provider facade already persisted to the durable ledger.
...
export function juryLaneMarginalCostUsd(providerLane: ProviderLane | undefined): number | undefined {
  // ProviderLane is intentionally a closed union of the three known
  // flat/subscription accounts. Generic routing remains estimate-priced.
  return providerLane ? 0 : undefined;
}
...
export function shouldRecordEstimatedMoACost(
  ...
  return !usageReported && Boolean(tokensIn || tokensOut);
}
...
async function callProposer(specOrId: string | ProposerCallSpec, question: string, tenantId: number, meteredOverride = false): Promise<ProposerResult> {
  ...
  try {
    ...
    return {
      modelId, provider, providerLane: spec.providerLane, ok: !!answer, answer,
      latencyMs: Date.now() - t0,
      error: answer ? undefined : "empty response",
      tokensIn, tokensOut, usageReported,
      label: spec.label,
      role: spec.role || "proposer",
    };
  } catch (err) {
    return {
      modelId, provider, providerLane: spec.providerLane, ok: false, latencyMs: Date.now() - t0,
...
): Promise<{ answer: string; modelUsed: string; providerLane?: ProviderLane; tokensIn: number; tokensOut: number; usageReported: boolean }> {
  const modelId = spec.modelId;
  try {
    const { client, actualModelId } = await getClientForModel(modelId, tenantId, {
      juryLane: shouldUseJuryProviderMarker(spec.providerLane),
    ...
    const tokensOut = usage?.completion_tokens ?? usage?.output_tokens ?? usage?.candidatesTokenCount ?? Math.ceil(answer.length / 4);
    if (answer) return { answer, modelUsed: actualModelId, providerLane: spec.providerLane, tokensIn, tokensOut, usageReported };
...
export function validateFrontierLanes(raw: unknown, frontier: string[]): FrontierLaneSpec[] | null {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (
      modelId !== frontier[i] ||
      (providerLane !== "openference" && providerLane !== "profundo" && providerLane !== "claude-runner") ||
...
export async function executeMoA(opts: MoAOptions): Promise<MoAResult> {
  ...
  const hasPotentialMeteredSeat = proposerSpecs.some((spec) => !spec.providerLane) || !!opts.aggregatorId;
  ...
  try {
    ...
    for (const p of proposers) {
      if (p.ok && shouldRecordEstimatedMoACost(!!p.usageReported, p.tokensIn, p.tokensOut)) {
    ...
    if (
      ...
      shouldRecordEstimatedMoACost(aggUsageReported, aggTokensIn, aggTokensOut)
    ) {
      estimates.push(recordCost({
        tenantId: opts.tenantId,
        toolName: "ensemble_query",
    ...
    await Promise.allSettled(estimates);
  } catch (err) {
