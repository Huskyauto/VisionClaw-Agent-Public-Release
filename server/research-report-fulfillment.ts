...
export async function fulfillResearchReport(params: {
  ...
  // Evidence Confidence (advisory, fail-soft): lift the mandated first-line
  // self-report out of each section body and render it as a labeled lead
  // line; a missing/malformed line renders as 'n/a', never a refusal.
  const sectionsWithConfidence = sections.map((s) => (
    { heading: s.heading, body: `Evidence Confidence: ${s.confidence ?? "n/a"}\n\n${s.body}` }
  ));
  ...
  // Republish durability (2026-08-14): persist the EDITABLE report source
  // (intake + final section text + model) to file_storage so a later
  // update/regeneration never has to rebuild the research from scratch —
  // the PDF alone is not editable source. Insert-only (fileName is
  // timestamped-unique); fail-open with a loud log.
  try {
    ...
    console.log(`[research-report] Order ${params.orderId} — report source persisted to DB (${fileName}.source.json)`);
  } catch (srcErr: any) {
