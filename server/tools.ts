...
} from "./lib/internal-render-markers";
...
import { errorMessage, errorMessageSlice, getHeartbeatFns } from "./tools/lib/runtime-helpers";
import { getAllToolDefinitions as getAllToolDefinitionsFromBase } from "./tools/lib/facade-helpers";
import type { ToolDefinition } from "./tools/types";
export type { ToolDefinition } from "./tools/types";
export { PROVIDERS_SUPPORTING_TOOLS } from "./tools/lib/runtime-helpers";
...
import { testApiKeys, checkSystemStatus } from "./lib/system-probes";
...
import {
  ...
  scraplingScrapeDefinition,
  multiSourceCollectDefinition,
...
import {
  ...
  queryProjectRelationshipsDefinition,
...
import { discoverCmmcProspectsDefinition, lookupSamCompanyCageDefinition, lookupSamExactCompanyDefinition, configureCmmcProspectingService } from "./tools/domains/cmmc-prospecting";
import { typesafeJevDefinition } from "./tools/domains/typesafe-jev";
import { acquireCmmcProspectLease, cmmcProspectCacheKey, cmmcProspectSourceRunKey, discoverOrResumeCmmcProspects, deliverCmmcProspectArtifacts, isCmmcProspectDeliveryReady, loadCmmcProspectCheckpoint, lookupSamCompanies, persistCmmcProspectCheckpoint, releaseCmmcProspectLease } from "./lib/cmmc-prospecting";
import { lookupSamExactCompany } from "./lib/sam-exact-company";
import {
...
// Born-migrated domains (sandbox S4, revenue-missions S5a): defs spliced here.
import {
  sandboxRunDefinition,
  sandboxReportDefinition,
  sandboxPromoteDefinition,
} from "./tools/domains/sandbox";
import { revenueMissionCreateDefinition, revenueMissionListDefinition, revenueMissionStatusDefinition, revenueMissionDraftExperimentDefinition, missionPortfolioReviewDefinition } from "./tools/domains/revenue-missions/definitions";
import { productListingCreateDefinition, productListingListDefinition, createPaymentLinkDefinition } from "./tools/domains/commerce/definitions";
import { generateAuditFixKitDefinition } from "./tools/domains/audit-fixkit/definitions";
import {
...
} from "./tools/domains/derived-api/definitions";
// Tools-layer-split S23: governance domain (destructive/owner-only stragglers
// LAST) — set_policy def moves; other stragglers (exec/lobster/run_command/kill
// shell family; write_file/google_drive trust-channel tools) stay legacy —
// moving them would touch the safety boundary the contract forbids.
import {
  setPolicyDefinition,
...
import { getDailyNotesDefinition, listConversationsDefinition, writeDailyNoteDefinition } from "./tools/domains/daily-notes/definitions";
import { scheduleCrossPlatformPostDefinition } from "./tools/domains/cross-platform-scheduling/definitions";
import { stress_intervention, stressInterventionDefinition } from "./tools/domains/stress-intervention";
import { queryTraceDefinition } from "./tools/domains/query-trace/definitions";
...
export { stress_intervention } from "./tools/domains/stress-intervention";

// Application-owned dependency injection keeps server/tools/ acyclic.
configureCmmcProspectingService({ acquireCmmcProspectLease, cmmcProspectCacheKey, cmmcProspectSourceRunKey, discoverOrResumeCmmcProspects, deliverCmmcProspectArtifacts, isCmmcProspectDeliveryReady, loadCmmcProspectCheckpoint, lookupSamCompanies, lookupSamExactCompany, persistCmmcProspectCheckpoint, releaseCmmcProspectLease });
...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  scheduleCrossPlatformPostDefinition,
  // Tools-layer-split S25b: scheduled-posts domain (const refs — original positions).
  cancelScheduledPostDefinition,
  listScheduledPostsDefinition,
  ...
  listPdfFieldsDefinition,
  {
    ...
    function: {
      name: "project",
      description: "Manage projects, continuity, assets, and the Project Compass. Read the Compass before recommendations; update only explicit or labeled inferred guidance.",
      parameters: {
        type: "object",
        properties: {
          command: { type: "string", enum: ["create", "get", "list", "update", "get_state", "update_state", "get_compass", "update_compass", "add_file", "add_note", "link_conversation", "search"], description: "Operation to perform" },
          operation: { type: "string", enum: ["create", "get", "list", "update", "get_state", "update_state", "get_compass", "update_compass", "add_file", "add_note", "link_conversation", "search"], description: "Legacy alias for command" },
          action: { type: "string", enum: ["create", "get", "list", "update", "get_state", "update_state", "get_compass", "update_compass", "add_file", "add_note", "link_conversation", "search"], description: "Legacy alias for command" },
          ...
          revision: { type: "number", description: "Current Project Compass revision (required for update_compass; use 0 for an empty profile)" },
          compassEntries: {
            type: "array",
            description: "Complete replacement Project Compass entries. Inferences must stay labeled agent_inferred/inferred until the user confirms them.",
            maxItems: 64,
            items: {
              type: "object",
              properties: {
                id: { type: "string" },
                category: { type: "string", enum: ["goal", "desired_outcome", "concern", "constraint", "affected_person", "guiding_principle", "assumption", "unknown"] },
                statement: { type: "string", maxLength: 500 },
                provenance: { type: "string", enum: ["user_stated", "agent_inferred"] },
                confidence: { type: "number", minimum: 0, maximum: 1 },
                status: { type: "string", enum: ["stated", "confirmed", "inferred", "rejected"] },
              },
              required: ["id", "category", "statement", "provenance", "confidence", "status"],
            },
          },
  ...
  sandboxReportDefinition,
  sandboxPromoteDefinition,
  revenueMissionCreateDefinition, revenueMissionListDefinition, revenueMissionStatusDefinition, revenueMissionDraftExperimentDefinition, missionPortfolioReviewDefinition,
  productListingCreateDefinition, productListingListDefinition, createPaymentLinkDefinition,
  generateAuditFixKitDefinition,
  captureListDefinition,
  recipeDeriveDefinition,
  recipeReplayDefinition,
  recipeListDefinition,
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          model: { type: "string", description: "Model to use (default: gpt-5.4). Must be an available model." },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          modelId: { type: "string", description: "Model ID to estimate costs for (default: gpt-5.4)" },
        },
        required: ["steps"],
      },
    },
...
async function handleProject(params: Record<string, any>) {
  ...
  const { resolveProjectCommand } = await import("./lib/project-command");
  const resolvedCommand = resolveProjectCommand(params);
  if (!resolvedCommand.ok) return resolvedCommand;
  ...
  switch (command) {
    ...
    case "get": {
      ...
      const compass = await getProjectCompass(Number(params.id), tenantId);
      return { project, compass, files: (files as any).rows || files, notes: (notes as any).rows || notes, conversations: (convs as any).rows || convs };
    }
    case "list": {
    ...
    case "get_compass": {
      ...
      const { getProjectCompass } = await import("./project-compass");
      const compass = await getProjectCompass(Number(params.id), tenantId);
      return compass || { error: "Project not found" };
    }
    case "update_compass": {
      if (!params.id) return { error: "id is required" };
      if (!Array.isArray(params.compassEntries)) return { error: "compassEntries array is required" };
      const { ProjectCompassConflictError, replaceProjectCompass } = await import("./project-compass");
      try {
        const compass = await replaceProjectCompass(Number(params.id), tenantId, Number(params.revision), params.compassEntries);
        return compass || { error: "Project not found" };
      } catch (error) {
        if (error instanceof ProjectCompassConflictError) {
          return { error: "Project Compass changed; call get_compass and apply your update to the latest revision" };
        }
        return { error: error instanceof Error ? error.message : "Invalid Project Compass" };
      }
    }
    case "add_file": {
    ...
    default:
      return { error: `Unknown project command: ${command}` };
  }
}
...
async function handleSendEmail(to: string, subject: string, text: string, html?: string, tenantId?: number) {
  try {
    ...
    return { error: `Failed to send email: ${errorMessage(err)}` };
  }
}
...
async function handleCheckInbox(limit: number, tenantId?: number) {
  try {
    ...
    return { error: `Failed to check inbox: ${errorMessage(err)}` };
  }
}
...
async function delegateTask(targetAgent: string, taskName: string, description: string, prompt: string, schedule: string, tenantId?: number, callerContext?: string, currentDepth?: number) {
  ...
  if (delegationAbortSignal?.aborted) {
    return { success: false, error: "Delegation cancelled because the parent tool deadline elapsed." };
  }
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const result = await delegateTaskFromChat(
        ...
        prompt,
        schedule || "once",
        attempt === 0 ? "gpt-5.6-sol" : "google/gemini-3.7-flash",
        tenantId,
        delegationDepth
      );
      if (result.success) {
        if (result.result && targetAgent.toLowerCase() !== "proof" && delegationDepth <= 1) {
          try {
            ...
            console.warn(`[auto-qa] Skipped: ${errorMessage(qaErr)}`);
          }
        }
        return result;
      }
      ...
      if (!result.success && result.error) {
        return {
          ...result,
          _fallbackHint: `Delegation to ${targetAgent} failed after ${attempt + 1} attempt(s). You should execute this task yourself using your available tools (check_system_status, recall_context, search_memory, project, etc.) instead of delegating. Do the work directly.`,
        };
      }
      return result;
    } catch (err) {
      console.error(`[delegation] Attempt ${attempt + 1} threw: ${errorMessage(err)}`);
      if (delegationAbortSignal?.aborted) {
        return { success: false, error: "Delegation cancelled because the parent tool deadline elapsed." };
      }
      ...
      return {
        success: false,
          error: `Delegation failed after ${MAX_RETRIES + 1} attempts: ${errorMessage(err)}`,
        _fallbackHint: `Delegation to ${targetAgent} failed. You should execute this task yourself using your available tools (check_system_status, recall_context, search_memory, project, etc.) instead of delegating. Do the work directly.`,
      };
    }
  }
...
import { getProductOutputTools, getNetworkTools } from "./tool-registry";
import { getToolTimeoutMs } from "./lib/tool-timeout-policy";
...
export async function executeTool(name: string, params: Record<string, any>): Promise<any> {
  ...
  // A browser retry of an owner/Felix weekly-recap launch must be able to
  // discover the already-active durable job before the expensive-tool limiter.
  // This is read-only and authority-bound; it cannot create, spawn, or expose a
  // different tenant's job. New/terminal attempts still hit the normal limiter.
  if (
    name === "bwb_weekly_build" &&
    params._tenantId === (Number(process.env.ADMIN_TENANT_ID) || 1) &&
    params._personaId === 2 &&
    typeof params._launchKey === "string" &&
    /^owner-web-chat:[a-zA-Z0-9:._-]{8,180}$/.test(params._launchKey)
  ) {
    const { findActiveBwbLaunchJob } = await import("./lib/bwb-job-progress");
    const active = await findActiveBwbLaunchJob(params._launchKey, params._tenantId);
    if (active) {
      return {
        ok: true,
        started: true,
        ...
        note: "This exact owner web-chat recap request already has an active durable job. Reusing it instead of starting or rate-limiting a duplicate render.",
      };
    }
  }
  ...
  // Speculative prefetch consume (arXiv:2607.25816 adaptation) — serve a
  // pre-warmed result for STRICTLY allowlisted read-only tools when the live
  // call exactly matches (tenant, tool, public args). The prefetch itself ran
  // through this full dispatch path with the same tenant+args, so consuming
  // skips no guard it didn't already pass. Single-use, short TTL, fail-open.
  if (!params._speculativePrefetch) {
    try {
      const { consumeSpeculativeResult } = await import("./speculative-prefetch");
      const _spec = consumeSpeculativeResult(name, params);
      if (_spec.hit) return _spec.result;
    } catch (_silentErr) { logSilentCatch("server/tools.ts", _silentErr); }
  }
  ...
  try {
    ...
    if (_policy?.irreversible) {
      ...
      try {
        ...
        console.warn(`[TNR] capture failed for ${name}: ${errorMessage(capErr, String(capErr))} — refusing call (fail-closed)`);
        return { error: `[TNR] pre-action snapshot failed: ${errorMessage(capErr, String(capErr))} — refusing irreversible call without undo path` };
      }
    }
  } catch (tnrGateErr) {
    ...
    console.error(`[TNR] gate plumbing error (falling through): ${errorMessage(tnrGateErr, String(tnrGateErr))}`);
  }

  const _ledgerStart = Date.now();
  let _execError: unknown = null;
  let result: unknown;
  try {
    ...
    if (_tnrActionId && result && typeof result === "object" && !("error" in result)) {
      Object.assign(result, { __tnr: { actionId: _tnrActionId, expiresAt: _tnrExpiresAt?.toISOString() } });
    }
  } catch (err) {
    _execError = err;
    throw err;
  ...
  if (shouldPublishLocalProductOutput(params)) {
  ...
  switch (name) {
    ...
    case "get_minerva_roster": {
      try {
        ...
        return { error: `get_minerva_roster failed: ${errorMessage(e, String(e))}` };
      }
    }
    ...
    case "schedule_cross_platform_post": {
      ...
      try {
        ...
        return { ok: false, error: errorMessage(e, "schedule_cross_platform_post failed") };
      }
    }
    ...
    case "propose_procedure_edit": {
      ...
      try {
        ...
        return { ok: false, error: errorMessage(e, "propose_procedure_edit failed") };
      }
    }
    ...
    case "approve_procedure_edit": {
      ...
      try {
        ...
        return { ok: false, error: errorMessage(e, "approve_procedure_edit failed") };
      }
    }
    case "reject_procedure_edit": {
      ...
      try {
        ...
        return { ok: false, error: errorMessage(e, "reject_procedure_edit failed") };
      }
    }
    ...
    case "render_diagram": {
      ...
      try {
        ...
        try {
          ...
          console.warn(`[render_diagram] mermaid.ink failed: ${errorMessage(e)}, trying Kroki fallback`);
        }
        ...
        if (!buffer) {
          try {
            ...
            console.warn(`[render_diagram] Kroki fallback also failed: ${errorMessage(e)}`);
          }
        }
        ...
        try {
          driveResult = await uploadAndShare({
            filePath, fileName: filename, mimeType: "image/png", folderLabel,
            parentFolderId: params._projectDriveFolderId || undefined,
            tenantId: params._tenantId,
            generatedArtifact: { artifactKind: "diagram", logicalName: filename, sourceRunKey: `diagram:${filename}` },
          });
          if (!driveResult.success) return { error: driveResult.error || "Drive durability verification failed" };
        } catch (driveErr) {
          return { error: `Diagram is not durable: ${errorMessage(driveErr)}` };
        }
        ...
        console.error(`[render_diagram] Failed:`, errorMessage(err));
        return { error: `Diagram rendering failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "generate_dashboard": {
      ...
      try {
        ...
        console.error("[generate_dashboard] HTML→PDF failed:", errorMessage(err));
        return { error: `HTML→PDF conversion failed: ${errorMessage(err)}` };
      }
    }
...
Topic: ${topic}`,
        ...
        if (slides && Array.isArray(slides)) {
          ...
          const result = await executeTool("render_diagram", {
            ...
            const runDiagramBatches = async () => {
              if (diagramSlides.length === 0) return;
              console.log(`[create_slides] Pre-generating ${diagramSlides.length} Mermaid diagrams in PARALLEL (max ${DIAGRAM_CONCURRENCY} concurrent)...`);
            ...
            // Pipeline-over-barrier: AI images for non-diagram slides don't depend on
            // diagram rendering — run both pools concurrently. Diagram slides keep their
            // image-gen fallback (diagram failed but has an image prompt) in a 2nd pass.
            const aiImageSlides = slides
              .map((s: any, idx: number) => ({ s, idx }))
              .filter(({ s }: any) => s.generateImage && !s.imageUrl && !s.diagramCode);
            const CONCURRENCY = 4;
            let imagesGenerated = 0;
            let imagesFailed = 0;
            const generateOne = async (item: { s: any; idx: number }, i: number, total: number) => {
              try {
              const result = await executeTool("generate_social_image", {
                ...
                if (imgUrl) {
                  ...
                  imagesGenerated++;
                  console.log(`[create_slides] AI image ${i + 1}/${total} embedded on slide ${item.idx}: ${imgUrl.slice(0, 80)}`);
                } else if (result?.error) {
                  imagesFailed++;
                  console.warn(`[create_slides] AI image ${i + 1}/${total} for slide ${item.idx} returned error: ${result.error}`);
                }
              } catch (err) {
                imagesFailed++;
                console.warn(`[create_slides] AI image ${i + 1}/${total} for slide ${item.idx} failed: ${(err as Error).message}`);
              }
            };
            const runImageBatches = async (items: { s: any; idx: number }[]) => {
              if (items.length === 0) return;
              console.log(`[create_slides] Pre-generating ${items.length} AI images in PARALLEL (${CONCURRENCY} concurrent)...`);
              for (let batch = 0; batch < items.length; batch += CONCURRENCY) {
                const chunk = items.slice(batch, batch + CONCURRENCY);
                await Promise.allSettled(chunk.map((item: { s: any; idx: number }, ci: number) => generateOne(item, batch + ci, items.length)));
              }
            };
            await Promise.all([runDiagramBatches(), runImageBatches(aiImageSlides)]);
            const diagramFallback = diagramSlides.filter(({ s, idx }: any) => !slides[idx].imageUrl && s.generateImage);
            await runImageBatches(diagramFallback);
            const totalImages = aiImageSlides.length + diagramFallback.length;
            if (totalImages > 0) {
              console.log(`[create_slides] Image generation complete: ${imagesGenerated} succeeded, ${imagesFailed} failed out of ${totalImages}`);
            }
        ...
        try {
          const { db } = await import("./db");
          const { sql } = await import("drizzle-orm");
          const qaTenantId = Number(params._tenantId);
          if (!Number.isInteger(qaTenantId) || qaTenantId <= 0) {
            throw new Error(`create_slides QA-lesson lookup requires a valid tenantId (got: ${JSON.stringify(params._tenantId)})`);
          }
          const lessons = await db.execute(sql`
            SELECT fact FROM memory_entries
            WHERE tenant_id = ${qaTenantId} AND persona_id = 2 AND source = 'slides_qa' AND category = 'lesson'
...
- Every slide must have a title and content`,
                    ...
                    if (fixResult.success && fixResult.json?.slides?.length >= Math.floor(slides.length * 0.7)) {
                      ...
                      if (correctedResult?.presentationId) {
                        ...
                        try {
                          ...
                          const correctionLesson = `SELF-CORRECTION SUCCESS (${new Date().toISOString().slice(0, 10)}): Rebuilt presentation from score ${overall}/10. Fixed: ${qaResult.json.issues.slice(0, 3).join("; ")}. New score: ${correctedResult.qualityScore?.overall || 'pending'}/10.`;
                          await assertMemoryValueNotTombstoned({
                            tenantId: Number(params._tenantId),
                            source: "memory_entries",
                            parts: [correctionLesson],
                          });
                          await memDb.execute(memSql`
                            INSERT INTO memory_entries (tenant_id, persona_id, fact, category, source, created_at)
                            VALUES (${params._tenantId}, 2, ${correctionLesson}, 'lesson', 'slides_qa', NOW())
                          `).catch(() => {});
                        } catch (_silentErr) { logSilentCatch("server/tools.ts", _silentErr); }
          ...
          if (!slideResult.narratedPresentationUrl) {
            ...
            try {
              ...
              const existingSession = await repairDb.execute(repairSql`
                SELECT token FROM presenter_sessions
                WHERE presentation_id = ${slideResult.presentationId}
                  AND tenant_id = ${params._tenantId}
    ...
    case "delegate_task": {
      ...
      {
        const { isForbiddenCmmcFallbackDelegation } = await import("./lib/tool-routing-context");
        const delegationIntent = [
          String(params.taskName || ""),
          String(params.description || ""),
          prompt,
        ].join("\n");
        if (isForbiddenCmmcFallbackDelegation(delegationIntent)) {
          return {
            error: "CMMC source discovery cannot be delegated to browser, scraping, direct-fetch, USAspending, or alternate-source fallbacks. Use discover_cmmc_prospects only. If SAM.gov returns rate_limited or unavailable, report that retryable authoritative-source failure without attempting a substitute roster.",
            blocked: true,
            reason: "cmmc_authoritative_source_lock",
          };
        }
      }
      ...
      // co-occurrence), mirroring build_video_from_brief's use_bwb_weekly_build refusal.
      {
        ...
        if (isBwbWeeklyRecapBrief(recapText)) {
          const callerPid = params._personaId;
    ...
    case "bwb_weekly_build": {
      ...
      const { newBwbJobId, createBwbJob, claimBwbLaunchJob, failBwbJob } = await import("./lib/bwb-job-progress");
      const launchKey = typeof params._launchKey === "string" && /^owner-web-chat:[a-zA-Z0-9:._-]{8,180}$/.test(params._launchKey)
        ? params._launchKey
        : null;
      let bwbJobId = newBwbJobId();
      let bwbJobCreated = false;
      if (launchKey) {
        const claim = await claimBwbLaunchJob({
          launchKey,
          tenantId: adminTenant,
          title: "Built With Bob — Weekly Recap (building…)",
        });
        if (claim && !claim.created) {
          return {
            ...
            job_id: claim.jobId,
            watch_progress_url: "/jobs",
            note: "This exact owner web-chat recap request already has an active durable job. Reusing it instead of starting a duplicate render.",
          };
        }
        if (claim) {
          bwbJobId = claim.jobId;
          bwbJobCreated = true;
        }
      } else {
        bwbJobCreated = await createBwbJob({ jobId: bwbJobId, tenantId: adminTenant, title: "Built With Bob — Weekly Recap (building…)" });
      }
      if (!bwbJobCreated) return {
          error: "bwb_weekly_build could not create its live progress record, so the render was not started. Retry after the video-jobs database is healthy.",
          started: false,
        };
      ...
      // Fail fast on pre-main crashes so the job cannot remain "building"; after
      // 90s the orchestrator owns reporting. Listeners bind synchronously.
      const spawnOrchestrator = (attempt: number): Promise<void> => new Promise((resolve, reject) => {
        let settled = false;
        ...
        try {
          ...
          child = spawn(orchCmd.cmd, orchCmd.args, {
            detached: true,
        ...
        const handleEarlyFailure = (why: string): void => {
          ...
          if (!mainStarted && attempt < ORCH_MAX_SPAWN_ATTEMPTS) {
            ...
            const backoff = setTimeout(() => {
              void spawnOrchestrator(attempt + 1).then(resolve, reject);
            }, 2_500 * attempt);
            if (typeof (backoff as any).unref === "function") (backoff as any).unref();
            return;
          }
          const failure = `weekly recap orchestrator ${why}${attempt > 1 ? ` (after ${attempt} startup attempts)` : ""}`;
          void failBwbJob(failure, bwbJobId, adminTenant);
          if (!spawnConfirmed) reject(new Error(failure));
        };
      ...
      try {
        await spawnOrchestrator(1);
      } catch (launchErr) {
        const launchFailure = `weekly recap orchestrator failed to launch: ${errorMessage(launchErr, String(launchErr))}`;
        await failBwbJob(launchFailure, bwbJobId, adminTenant);
        return { error: launchFailure, started: false, job_id: bwbJobId, watch_progress_url: "/jobs" };
      }
      return {
        ok: true,
        started: true,
    ...
    case "orchestrate": {
      ...
      for (const s of executed.steps) {
        for (const url of s.deliverableLinks || []) {
          ...
          const checked = await Promise.all(uniqueLinks.map(async (l) => {
            ...
            try {
              ...
              return { link: l, keep: v.exists };
               ...
               return { link: l, keep: false };
            }
          }));
          keptLinks = checked.filter(c => c.keep).map(c => c.link);
         } catch (_silentErr) {
           logSilentCatch("server/tools.ts", _silentErr);
           keptLinks = [];
         }
      }
      const keptSet = new Set(keptLinks);
      const unverifiedDeliverableUrl = /https:\/\/(?:docs\.google\.com\/(?:presentation|document|spreadsheets)\/d\/[a-zA-Z0-9_-]+(?:\/[^\s"')\]},]+)?|drive\.google\.com\/file\/d\/[a-zA-Z0-9_-]+(?:\/[^\s"')\]},]+)?|[a-z0-9-]+\.replit\.app\/present\/[a-f0-9]+)/g;
      for (const step of executed.steps) {
        step.deliverableLinks = (step.deliverableLinks || []).filter(link => keptSet.has(link));
        if (step.result) {
          step.result = step.result.replace(unverifiedDeliverableUrl, link =>
            keptSet.has(link) ? link : "[unverified deliverable link omitted]",
          );
        }
      }
      summary = synthesizeResults(executed);
    ...
    case "estimate_cost": {
      ...
      return estimatePlanCost(steps, params.modelId || "gpt-5.4");
    }
    case "debate": {
    ...
    case "generate_audio": {
      ...
      const tryFishFallback = async (reason: string): Promise<Buffer | null> => {
        ...
        try {
          ...
          console.error(`[generate_audio] Fish Audio fallback failed: ${errorMessageSlice(fishErr, 160)}`);
          return null;
        }
      };
      const tryOpenAIFallback = async (reason: string): Promise<Buffer | null> => {
        ...
        try {
          ...
          console.error(`[generate_audio] OpenAI fallback failed: ${errorMessageSlice(oaErr, 160)}`);
          return null;
        }
      };
      const tryEdgeFallback = async (reason: string): Promise<Buffer | null> => {
        try {
          ...
          console.error(`[generate_audio] Edge TTS fallback failed: ${errorMessageSlice(edgeErr, 160)}`);
          return null;
        }
      };
      ...
      if (provider === "fish") {
        ...
        if (!fishKey) {
          ...
          try {
            ...
            const errMsg = errorMessage(fishErr);
            if (strictVoice) {
        ...
        try {
          ...
          return { error: `Edge TTS failed: ${errorMessageSlice(edgeErr, 200)}` };
        }
        ...
        if (!apiKey) {
          ...
          try {
            ...
            const errMsg = errorMessage(ttsErr);
        ...
        if (!key) {
          ...
          let elResponse: Response | { ok: false; status: number; text: () => Promise<string> };
          try {
            ...
            elResponse = { ok: false, status: 0, text: async () => errorMessage(netErr, "network error") };
          }
          ...
          if (!elResponse.ok) {
            ...
            if (oaiKey) {
              try {
                ...
                console.warn(`[generate_audio] OpenAI fallback also failed (${errorMessageSlice(fallbackErr, 100)}), cascading to Edge TTS`);
              }
            }
            if (!openaiOk) {
      ...
      if (shouldRequireAudioDriveDurability(params)) {
        try {
          const { uploadAndShare } = await import("./google-drive");
          const driveResult = await uploadAndShare({
            ...
            generatedArtifact: { artifactKind: "audio", logicalName: `${filename}.${ext}`, sourceRunKey: `audio:${filename}` },
          });
          if (!driveResult.success) return { error: driveResult.error || "Drive durability verification failed" };
          driveUrl = driveResult.viewUrl;
        } catch (driveErr) {
          return { error: `Drive durability verification failed: ${errorMessage(driveErr)}` };
        }
      }
    ...
    case "run_okr_review": {
      ...
      } catch (e) { return { error: errorMessage(e, "run_okr_review failed") }; }
    }
    ...
    case "auto_memorize_now": {
      if (!Number.isInteger(params._tenantId) || params._tenantId <= 0) {
        return { error: "auto_memorize_now requires trusted tenant context." };
      }
      const { runAutoMemorizeForTenant } = await import("./auto-memorize");
      const r = await runAutoMemorizeForTenant(params._tenantId, { force: true, windowHours: Number(params.windowHours) || 6 });
      return r;
    }
      ...
      try {
        ...
        try {
          ...
          const driveResult = await uploadAndShare({
            ...
            tenantId: params._tenantId,
            projectId: params.project_id,
            generatedArtifact: { artifactKind: "video", logicalName: `${outputFilename}.mp4`, sourceRunKey: `slideshow-video:${outputFilename}` },
          });
          if (!driveResult.success) return { error: driveResult.error || "Drive durability verification failed" };
          driveUrl = driveResult.viewUrl;
        } catch (driveErr: any) {
          return { error: `Drive durability verification failed: ${driveErr.message}` };
        }
    ...
    case "search_stock_media": {
      ...
      try {
        ...
        if (searchType === "videos") {
          ...
          if (params.download && videos.length > 0 && videos[0].download_url) {
            try {
              ...
              videos[0].download_status = "failed";
              videos[0].download_error = errorMessage(dlErr);
            }
          }
          ...
          if (params.download && photos.length > 0 && photos[0].src_large) {
            try {
              ...
              photos[0].download_status = "failed";
              photos[0].download_error = errorMessage(dlErr);
            }
          }
        ...
        return { error: `Stock search failed: ${errorMessage(stockErr)}` };
      }
    }
    case "generate_social_image": {
      ...
      try {
        ...
        const driveResult = await uploadAndShare({
          filePath: localPath, fileName, mimeType, folderLabel,
          parentFolderId: params._projectDriveFolderId || undefined,
          tenantId: params._tenantId,
          projectId: params.project_id,
          generatedArtifact: { artifactKind: "image", logicalName: fileName, sourceRunKey: `social-image:${fileName}` },
        });
        if (!driveResult.success) return { error: driveResult.error || "Drive durability verification failed" };
        ...
        return { error: `Image generation failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "publish_social_post": {
      ...
      if (!params._tenantId) return { error: "Tenant context required for publish_social_post" };
      const tenantId = params._tenantId;
      ...
      if (params.platform === "x" && isXConfigured()) {
        ...
        try {
          ...
          return { success: false, platform: "x", error: errorMessage(err) };
        }
      }
    ...
    case "manage_social_accounts": {
      ...
      if (!params._tenantId) return { error: "Tenant context required for manage_social_accounts" };
      const tenantId = params._tenantId;
    ...
    case "manage_desk": {
      ...
      if (!params._tenantId) return { error: "Tenant context required for manage_desk" };
      const tenantId = params._tenantId;
      const personaId = params._personaId || 1;
    ...
    case "post_to_channel": {
      ...
      if (!params._tenantId) return { error: "Tenant context required for post_to_channel" };
      const tenantId = params._tenantId;
      const personaId = params._personaId || 0;
    ...
    case "read_channels": {
      const channelsMod = await import("./agent-channels");
      if (!params._tenantId) return { error: "Tenant context required for read_channels" };
      const tenantId = params._tenantId;
      const personaId = params._personaId || 0;
    ...
    case "emit_event": {
      ...
      if (!params._tenantId) return { error: "Tenant context required for emit_event" };
      const tenantId = params._tenantId;
      const personaId = params._personaId || 0;
    ...
    case "manage_watchlist": {
      ...
      if (!params._tenantId) return { error: "Tenant context required for manage_watchlist" };
      const tenantId = params._tenantId;
      const personaId = params._personaId || 0;
      switch (params.action) {
        case "add": {
    ...
    case "calendar_sync": {
      ...
      try {
        ...
        return { error: `Calendar sync failed: ${errorMessage(err)}` };
      }
    }
      ...
      try {
        ...
        const writer = {
          ...
          handler: async (input: any) => {
            const { client, actualModelId } = await getClientForModel("gpt-5.4");
...
Pick 'finish' when you have enough to answer the goal. Max ${maxTurns} turns total.`;
        ...
        return { error: `run_supervisor failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "list_agent_runs": {
      ...
      try {
        ...
        return { error: `list_agent_runs failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "get_agent_run": {
      ...
      try {
        ...
        return { error: `get_agent_run failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "agentic_cache_stats": {
      ...
      try {
        ...
        return { error: `agentic_cache_stats failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "request_approval": {
      ...
      try {
        ...
        return { error: `request_approval failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "decide_approval": {
      ...
      try {
        ...
        return { error: `decide_approval failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "list_pending_approvals": {
      ...
      try {
        ...
        return { error: `list_pending_approvals failed: ${errorMessage(err)}` };
      }
    }
...
{"choice": "<one option exactly as written>", "confidence": <0..1>, "reasoning": "<short>", "risks": ["<risk 1>", "<risk 2>"]}`;
        ...
        return { error: `commit_decision failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "figma": {
      ...
      try {
        ...
        return { error: `figma tool failed: ${errorMessage(err)}` };
      }
    }
    ...
    case "set_my_profile_photo": {
      ...
      try {
        ...
        return { error: `set_my_profile_photo failed: ${errorMessage(e, String(e))}` };
      }
    }
    case "slash_command": {
      ...
      try {
        ...
        return { error: `slash_command failed: ${errorMessage(e, String(e))}` };
      }
    }
    case "run_command": {
      ...
      try {
        ...
        return { error: `run_command failed: ${errorMessage(e, String(e))}` };
      }
    }
    ...
    case "build_video_from_brief": {
      ...
      try {
        ...
        return r;
      } catch (e) { return { error: `build_video_from_brief failed: ${errorMessage(e, String(e))}` }; }
    }
    case "start_video_job": {
      ...
      } catch (e) { return { error: `start_video_job failed: ${errorMessage(e, String(e))}` }; }
    }
    case "check_video_job": {
      const tid = params._tenantId;
      if (typeof tid !== "number" || tid <= 0) return { error: "check_video_job requires tenant context" };
      try {
        const { getVideoJob, isJobStateCorrupt } = await import("./video-job-runner");
        const state = getVideoJob(String(params.job_id || ""), tid);
        if (!state) {
          if (isJobStateCorrupt(String(params.job_id || ""))) {
            return { error: "job state file is CORRUPT (not missing) — chapter files may still exist on disk under the job directory; operator recovery possible", error_code: "corrupt_state" };
          }
          return { error: "job not found or not owned by this tenant" };
        }
      ...
      } catch (e) { return { error: `check_video_job failed: ${errorMessage(e, String(e))}` }; }
    }
    case "finalize_video": {
      const tid = params._tenantId;
      ...
      try {
        ...
        return r;
      } catch (e) { return { error: `finalize_video failed: ${errorMessage(e, String(e))}` }; }
    }
    // Tools-layer-split S31: register_character_portrait + list_character_portraits
    ...
    case "plan_deliverable": {
      ...
      try {
        ...
        return { error: `plan_deliverable failed: ${errorMessage(e, String(e))}` };
      }
    }
    ...
    case "propose_skill": {
      ...
      try {
        ...
        if (skillBuildApproved(jury.decision)) {
          ...
          let built: any;
          try {
            built = await storage.createSkill({
              name: promoteName,
            ...
            const { isManagedSkillSecurityError } = await import("./lib/managed-skill-safety");
            if (!isManagedSkillSecurityError(error)) throw error;
            await db.update(proposedSkills).set({
              status: "pending",
              reviewedBy: `jury (${jury.majority}/3 BUILD, security scan rejected: ${error.patterns.join(", ")})`,
              reviewedAt: new Date(),
            }).where(eq(proposedSkills.id, row.id));
            return {
              ok: true,
              ...
              review_url: "/admin/proposed-skills",
              jury: juryLine,
              message: "Jury voted BUILD, but the skill security scan rejected the content; left pending for owner review.",
            };
          }
          await db.update(proposedSkills).set({
            status: "accepted",
        ...
        void (async () => {
          try {
            ...
            console.warn(`[propose_skill] escalate owner-ping failed (non-fatal): ${errorMessage(e, String(e))}`);
          }
        })();
        return {
          ok: true,
      ...
      } catch (e) { return { error: `propose_skill failed: ${errorMessage(e, String(e))}` }; }
    }
    case "run_ab_eval": {
          ...
          for (let i = 0; i < runs; i++) {
            tasks.push((async () => {
              try {
                ...
                return { configLabel: c.label, runIndex: i, output: "", score: 0, critique: "", error: errorMessage(e, String(e)) };
              }
            })());
          }
        }
      ...
      } catch (e) { return { error: `run_ab_eval failed: ${errorMessage(e, String(e))}` }; }
    }
    case "build_html_app": {
      ...
      try {
        ...
        return { error: `build_html_app failed: ${errorMessage(e, String(e))}` };
      }
    }
            ...
            if (name.length > needle.length && name.endsWith(needle)) {
              ...
              if (prefix && knownToolNames.has(prefix)) {
                ...
                try {
                  ...
                  console.error(`[tools] glued-recovery policy gate errored for ${suffix}: ${errorMessage(policyErr, String(policyErr))} — failing closed`);
...
export async function getAllToolDefinitions(tenantId?: number): Promise<ToolDefinition[]> {
  return getAllToolDefinitionsFromBase(TOOL_DEFINITIONS, tenantId);
}
...
<p style="color:#666;font-size:12px;">— ${escText(platformName)} Production Team</p>
        ...
        console.warn(`[auto-deliver] Email failed for ${toolName}: ${errorMessageSlice(e, 200)}`);
      }
    });
  } catch (e) {
    // Never let auto-deliver break the tool path
    console.warn(`[auto-deliver] Pre-flight error for ${toolName}: ${errorMessageSlice(e, 200)}`);
  }
}
  ...
  try {
    ...
    if (timedOut) {
      ...
      try {
        ...
        console.error(`[tools] S5 retry lane error for ${name}: ${errorMessage(retryErr, String(retryErr))}`);
      }
    }
    throw err;
  } finally {
