...
export async function generateComprehensiveFeaturePDF(): Promise<{
  ...
  try {
    ...
    const providers = [
      ["OpenAI", "GPT-5.4, GPT-4.1, GPT-4.1 Mini, GPT-5 Mini"],
    ...
    drawBullet("Google OAuth (Gemini 3.1 Pro, 3 Flash) and OpenAI OAuth (GPT-4.1, GPT-5 Mini) prioritized");
    ...
    const result = await uploadAndShare({
      ...
      share: true,
      tenantId: 1,
      generatedArtifact: { artifactKind: "report", logicalName: fileName, sourceRunKey: `feature-report:${dateStr}` },
    });

    if (fs.existsSync(localPath)) fs.unlinkSync(localPath);

    if (result.success) {
      return { success: true, driveUrl: result.viewUrl };
    }
