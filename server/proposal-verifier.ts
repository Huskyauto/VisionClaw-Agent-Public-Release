...
export async function verifyProposalById(proposalId: number, tenantId: number): Promise<VerifyResult> {
  ...
  if (!oldMatch || !newMatch) {
    ...
    await persist(proposalId, tenantId, "skipped", details);
  ...
  try {
    ...
    await persist(proposalId, tenantId, "failed", details);
  ...
  if (tscResult.errors.length === 0) {
    ...
    await persist(proposalId, tenantId, "passed", details);
...
async function persist(proposalId: number, tenantId: number, status: string, details: string): Promise<void> {
  try {
