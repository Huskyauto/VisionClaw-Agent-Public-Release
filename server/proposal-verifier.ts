...
} from "./lib/artifact-authority";
import { parseProposalDiff } from "./lib/proposal-diff";
...
export async function verifyProposalById(proposalId: number, tenantId: number): Promise<VerifyResult> {
  const t0 = Date.now();
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("verifyProposalById: tenantId must be a positive integer");
  }
  const r = await db.execute(sql`
    SELECT id, target_file, code_diff, validation_result, description, rationale
    FROM code_proposals
  ...
  const parsedDiff = parseProposalDiff(codeDiff);
  if (!parsedDiff) {
    const details = "diff missing OLD_CODE / NEW_CODE markers";
    await persistProposalVerification(proposalId, tenantId, "skipped", details);
    return { status: "skipped", details, durationMs: Date.now() - t0 };
  }
  let { oldCode, newCode } = parsedDiff;
  ...
  if (liveContent.includes(oldCode)) {
    ...
    const rebased = alreadyRebased ? null : await attemptRebaseOnDrift(
      proposalId, tenantId, resolved, oldCode, newCode, proposal,
    );
    if (!rebased) {
  ...
  try {
    ...
    await persistProposalVerification(proposalId, tenantId, "failed", details);
  ...
  if (tscResult.errors.length === 0) {
    ...
    // Artifact-authority rule: a consequential source change receives an
    // accepted receipt only when every required deterministic verifier returns
    // a result. An unavailable held-out gate is indeterminate, not a pass.
    ...
    try {
      ...
      const details = `artifact authority indeterminate: held-out eval gate unavailable: ${(e as Error).message}`.slice(0, 4000);
      await persistProposalVerification(proposalId, tenantId, "failed", details);
      return { status: "failed", details, durationMs: Date.now() - t0 };
    }
    ...
    const receipt = issueArtifactAuthorityReceipt({
      ...
      subjectId: String(proposalId),
      subject: {
        targetFile,
        sourceSha256: artifactAuthoritySha256Text(liveContent),
        proposal: { oldCode, newCode },
      },
      verdict: "accepted",
      reasonCodes: [
        "tsc_passed",
        heldOut.severity === "warn" ? "held_out_passed_with_warning" : "held_out_passed",
      ],
      evidence: {
        tscErrorCount: tscResult.errors.length,
        heldOutMode: heldOut.mode,
        heldOutSeverity: heldOut.severity,
        heldOutViolations: heldOut.violations,
      },
    });
    const receiptLine = serializeArtifactAuthorityReceipt(receipt);
    const summary = `tsc --noEmit OK (in-memory, ${Math.round((Date.now() - t0) / 1000)}s)${warnNote}`;
    const details = `${summary.slice(0, Math.max(0, 3999 - receiptLine.length))}\n${receiptLine}`;
    await persistProposalVerification(proposalId, tenantId, "passed", details);
...
export async function persistProposalVerification(
  proposalId: number,
  tenantId: number,
  status: string,
  details: string,
  executeQuery: (query: unknown) => Promise<unknown> = (query) => db.execute(query as any),
): Promise<void> {
  // Do NOT clobber proposal.status; verification is a sidecar signal consumed
  // by safeApplyProposal. A verifier result is not complete until exactly one
  // tenant-scoped row durably carries it.
  const persisted = await executeQuery(sql`
    UPDATE code_proposals
    ...
    RETURNING id
  `);
  assertArtifactAuthorityPersisted(persisted);
...
export function fireAndForgetVerify(proposalId: number, tenantId: number): void {
  verifyQueue.push(async () => {
    try {
      const r = await verifyProposalById(proposalId, tenantId);
