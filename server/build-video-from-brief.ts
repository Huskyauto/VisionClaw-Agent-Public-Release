...
/**
 * Reject a caller-supplied project before it can reach either render backend.
 * A missing project is valid because video delivery does not require one.
 */
export async function verifyVideoProjectOwnership(
  input: Pick<BuildVideoFromBriefInput, "tenantId" | "projectId">,
  ownsProject: typeof assertProjectInTenant = assertProjectInTenant,
): Promise<BuildVideoFromBriefResult | null> {
  if (input.projectId === undefined) return null;
  if (!(await ownsProject(input.projectId, input.tenantId))) {
    return {
      success: false,
      message: `Project #${input.projectId} is not available to this tenant.`,
...
/**
 * 2026-07-18 incident #2 — extract an EXPLICIT week window from recap text.
 * Bob asked for "July 11th through the 18th"; the self-healing redirect passed
 * no args, so the builder auto-pinned the LAST COMPLETED week (Jul 5–11) and
 * silently discarded the stated window. Pure + exported so both delegation
 * chokepoints (tools.ts delegate_task, heartbeat delegateTaskFromChat) can
 * carry the user's explicit dates through to bwb_weekly_build.
 *
 * Supports: "July 11 through the 18th", "July 11 - July 18", "7/11 to 7/18",
 * "2026-07-11 to 2026-07-18". Returns { weekStart, weekEnd } as YYYY-MM-DD or
 * null when no unambiguous range is present (auto-pin remains the default).
 * Year inference: current year, rolled back one year if that would place the
 * window more than 14 days in the future (recaps look backward).
 */
export function extractBwbWeekWindow(
  text: string,
  now: Date = new Date(),
  ...
  // Shared post-parse validation — EVERY branch funnels through this so no
  // format can skip the ordering or 21-day mis-parse guards (architect
  // R125+137.36: the ISO branch originally returned early and accepted
  // year-long ranges).
  const finalize = (y1: number, m1: number, d1: number, y2: number, m2: number, d2: number) => {
    if (!valid(y1, m1, d1) || !valid(y2, m2, d2)) return null;
    const s = ymd(y1, m1, d1), e = ymd(y2, m2, d2);
    if (s >= e) return null;
    // Sanity: a "week" window should be short; reject > 21 days (mis-parse guard).
    if (Date.UTC(y2, m2 - 1, d2) - Date.UTC(y1, m1 - 1, d1) > 21 * 86400000) return null;
    return { weekStart: s, weekEnd: e };
  };

  // 1) ISO range: 2026-07-11 (to|through|–|-) 2026-07-18
  const iso = t.match(/(\d{4})-(\d{2})-(\d{2})\s*(?:to|through|thru|until|till|→|⇒|➔|->|[–—-])\s*(\d{4})-(\d{2})-(\d{2})/);
  if (iso) {
    return finalize(+iso[1], +iso[2], +iso[3], +iso[4], +iso[5], +iso[6]);
  }
  ...
  // Arrow separators included — Bob's real prod phrasing was "2026-07-12 → 2026-07-18".
  const sep = "(?:to|through|thru|until|till|→|⇒|➔|->|[–—-])";

  let sm = 0, sd = 0, em = 0, ed = 0, ySpec1 = 0, ySpec2 = 0;
  // 2) "july 11 through (the) (july )?18" — explicit years captured and HONORED
  // (architect R125+137.36: "July 11, 2025 through July 18, 2025" must pin
  // 2025, not silently resolve to the current year).
  const named = t.match(new RegExp(`\\b(${monthName})\\s+(\\d{1,2})(?:\\s*,?\\s*(\\d{4}))?\\s*${sep}\\s*(?:the\\s+)?(?:(${monthName})\\s+)?(\\d{1,2})(?:\\s*,?\\s*(\\d{4}))?\\b`));
  if (named) {
    sm = months[named[1]]; sd = +named[2]; ySpec1 = named[3] ? +named[3] : 0;
    em = named[4] ? months[named[4]] : sm; ed = +named[5]; ySpec2 = named[6] ? +named[6] : 0;
  } else {
    // 3) numeric "7/11 to 7/18", "7/11-18", or "7/11/2025 to 7/18/2025"
    const num = t.match(new RegExp(`\\b(\\d{1,2})\\/(\\d{1,2})(?:\\/(\\d{4}))?\\s*${sep}\\s*(?:(\\d{1,2})\\/)?(\\d{1,2})(?:\\/(\\d{4}))?\\b`));
    if (!num) return null;
    sm = +num[1]; sd = +num[2]; ySpec1 = num[3] ? +num[3] : 0;
    em = num[4] ? +num[4] : sm; ed = +num[5]; ySpec2 = num[6] ? +num[6] : 0;
  }
  // End-day smaller with no explicit end month ⇒ month rollover ("june 28 to 4")
  if (em === sm && ed < sd) em = sm === 12 ? 1 : sm + 1;

  let y1: number, y2: number;
  if (ySpec1 || ySpec2) {
    // Explicit year(s) stated — honor them verbatim, no inference/rollback.
    y1 = ySpec1 || ySpec2;
    y2 = ySpec2 || ySpec1;
    if (em < sm) {
      if (!ySpec2) y2 = y1 + 1;      // "dec 28, 2025 to jan 3" ⇒ end year +1
      else if (!ySpec1) y1 = y2 - 1; // "dec 28 to jan 3, 2026" ⇒ start year -1
    }
  } else {
    y1 = now.getUTCFullYear();
    y2 = em < sm ? y1 + 1 : y1; // Dec→Jan rollover within the window
    if (!valid(y1, sm, sd)) return null;
    // Recaps look backward: if the window starts >14 days in the future, assume last year.
    if (Date.UTC(y1, sm - 1, sd) > now.getTime() + 14 * 86400000) { y1 -= 1; y2 -= 1; }
  }
  return finalize(y1, sm, sd, y2, em, ed);
}

/**
 * Extract EXPLICITLY-STATED weight facts from delegation text (wrong-numbers
 * incident, R125+137.37): Bob's canonical prompt includes "Weight facts:
 * current 279 lbs, total lost 225lbs, start 504 lbs", but the .35 no-guessed-
 * args redirect dropped them, so the build backfilled STALE figures from the
 * persisted store and the recap narrated last week's weight.
 *
 * Contract: pure; LABELED figures only (current/total lost/start) — these are
 * supplied facts, never inferred from bare numbers in prose. Each figure is
 * range-checked (50–1000 lbs weight, 1–1000 lost); anything out of range is
 * dropped (fail toward the persisted-store backfill, never a garbage number —
 * findWeightViolations downstream keeps narration honest either way). Returns
 * null when no labeled figure parses.
 */
export function extractBwbWeightFacts(text: string): { currentWeight?: number; totalLost?: number; startWeight?: number } | null {
  const t = String(text || "").toLowerCase();
  const num = "(\\d{2,4}(?:\\.\\d+)?)";
  // Optional unit, then a clean boundary — the figure must NOT run into a
  // letter/digit ("504p resolution", "29th") or it isn't a weight figure.
  const lbs = "\\s*(?:lbs?|pounds)?(?![a-z0-9])";
  ...
  const inRange = (v: number | undefined, lo: number) => (typeof v === "number" && v >= lo && v <= 1000 ? v : undefined);

  // "current 279 lbs" / "currently 279" / "current weight 279" / "I'm at 279 lbs"
  const currentWeight = inRange(grab(new RegExp(`current(?:ly)?(?:\\s+weight)?(?:\\s+is|\\s*[:=])?\\s+${num}${lbs}`)), 50);
  // "total lost 225lbs" / "total loss 225" / "lost 225 lbs total" / "down 225 lbs total"
  const totalLost = inRange(
    grab(new RegExp(`total\\s+(?:lost|loss)(?:\\s+is|\\s*[:=])?\\s+${num}${lbs}`)) ??
      grab(new RegExp(`(?:lost|down)\\s+${num}${lbs}\\s+(?:total|so far|to date)`)),
    1,
  );
  // "start 504 lbs" / "starting weight 504" / "started at 504"
  const startWeight = inRange(grab(new RegExp(`start(?:ing|ed)?(?:\\s+weight)?(?:\\s+at|\\s+is|\\s*[:=])?\\s+${num}${lbs}`)), 50);

  if (currentWeight === undefined && totalLost === undefined && startWeight === undefined) return null;
  const out: { currentWeight?: number; totalLost?: number; startWeight?: number } = {};
  if (currentWeight !== undefined) out.currentWeight = currentWeight;
  if (totalLost !== undefined) out.totalLost = totalLost;
...
export async function buildVideoFromBrief(input: BuildVideoFromBriefInput): Promise<BuildVideoFromBriefResult> {
  ...
  const projectOwnershipError = await verifyVideoProjectOwnership(input);
  if (projectOwnershipError) return projectOwnershipError;
...
Return STRICT JSON (no markdown):
{
  ...
  try {
    started = await startVideoJob(startInput);
  } catch (e: any) {
