...
async function judgeResult(
  ...
  const primary = await callLlmJudge(response, task, "gpt-5.4");
  if (!primary) {
  ...
  if (primary.passed !== secondary.passed) {
    ...
    return {
      ...
      reason: `${tag}: judges disagree — gpt-5.4 said ${primary.passed ? "PASS" : "FAIL"} (${primary.reason.slice(0, 80)}); gemini-2.5-flash said ${secondary.passed ? "PASS" : "FAIL"} (${secondary.reason.slice(0, 80)}). Bob, manually review eval row.`.slice(0, 480),
    };
  }
