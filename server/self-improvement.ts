...
import { ALL_TENANTS, type TenantScope } from "./storage";
import fs from "fs";
...
export async function detectStagnation(category: string, tenantId: number): Promise<{ isStagnant: boolean; consecutiveFailures: number; repeatedHypotheses: string[]; recommendation: string }> {
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("detectStagnation: positive tenantId is required");
  }
...
export async function runSelfImprovementCycle(config: ExperimentConfig & { _signals?: EvolutionSignal[]; _stagnation?: Awaited<ReturnType<typeof detectStagnation>> }): Promise<ExperimentResult[]> {
  ...
  try {
    ...
    // Personas are platform records, so a bare persona ID is not tenant
    // authority. A tenant may tune only a persona assigned to its own desk.
    const personaResult = config.personaId ? await db.execute(sql`
      SELECT p.*
      FROM personas p
      WHERE p.id = ${config.personaId}
        AND EXISTS (
          SELECT 1 FROM agent_desks ad
          WHERE ad.persona_id = p.id AND ad.tenant_id = ${tenantId}
        )
      LIMIT 1
    `) : null;
    const personaRows = personaResult ? ((personaResult as any).rows || personaResult) : [];
    const persona = personaRows[0] || null;
    if (config.personaId && !persona) {
      throw new Error("persona does not belong to this tenant");
    }
...
async function runExperiment(
  ...
  try {
    const baselineResponse = await generateResponse(hyp.testPrompt, null, persona, config.tenantId!);
    const baselineReflection = await reflectOnResponse(hyp.testPrompt, baselineResponse, persona?.name, config.tenantId);
    const baselineScore = baselineReflection.scores.overall;

    const experimentResponse = await generateResponse(hyp.testPrompt, hyp.approach, persona, config.tenantId!);
    const experimentReflection = await reflectOnResponse(hyp.testPrompt, experimentResponse, persona?.name, config.tenantId);
    ...
    if (improvement >= 0.5) {
      ...
      const comparison = await compareResponses(hyp.testPrompt, baselineResponse, experimentResponse, config.tenantId!);
    ...
    if (finalStatus === "kept" && persona) {
      ...
      if (newSoul.length < 10000) {
        const updateResult = await db.execute(sql`
          UPDATE personas p SET soul = ${newSoul}
          WHERE p.id = ${persona.id}
            AND EXISTS (
              SELECT 1 FROM agent_desks ad
              WHERE ad.persona_id = p.id AND ad.tenant_id = ${config.tenantId}
            )
          RETURNING p.id
        `);
        const updatedRows = (updateResult as any).rows || updateResult;
        if (!updatedRows[0]) {
          throw new Error("persona update refused: tenant no longer owns persona");
        }
...
async function generateResponse(userMessage: string, modification: string | null, persona: any, tenantId: number): Promise<string> {
  ...
  const { client, actualModelId } = await getClientForModel(DEFAULT_CHAT_MODEL_ID, tenantId);
  const resp = await client.chat.completions.create({
    model: actualModelId,
    messages: [
...
async function compareResponses(question: string, responseA: string, responseB: string, tenantId: number): Promise<"A" | "B" | "tie"> {
  try {
...
export async function getExperimentHistory(limit = 20, category: string | undefined, tenantId: TenantScope): Promise<any[]> {
  const conditions = [];
  if (category) conditions.push(eq(experiments.category, category));
  if (tenantId !== ALL_TENANTS) {
    if (!Number.isInteger(tenantId) || tenantId <= 0) {
      throw new Error("getExperimentHistory: positive tenantId or explicit ALL_TENANTS scope is required");
    }
    conditions.push(eq(experiments.tenantId, tenantId));
  }
