  ...
  try {
    ...
    if (!concatRes.success || !concatRes.filePath) {
      ...
      if (state.concat_attempts >= 3) {
        ...
        } catch (_e) { logSilentCatch("video-job-runner:concat-incident", _e); }
      }
    ...
    // finished video. Fail-closed: back to retry-able state + repair incident.
    {
      ...
      try {
        const st = fs.statSync(concatRes.filePath);
        if (st.size < 4096) verifyErrs.push(`final file only ${st.size} bytes (< 4096 min for video)`);
        const fd = fs.openSync(concatRes.filePath, "r");
        try {
        ...
        if (!(concatRes.durationSeconds && concatRes.durationSeconds > 0)) verifyErrs.push("zero/unknown duration reported by concat");
      } catch (ve: any) {
      ...
      if (verifyErrs.length > 0) {
        ...
        } catch (_e) { logSilentCatch("video-job-runner:verify-incident", _e); }
    ...
    if (state.concat_attempts >= 3) {
      ...
      } catch (_e) { logSilentCatch("video-job-runner:concat-throw-incident", _e); }
    }
