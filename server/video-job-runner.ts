...
async function isCancelRequested(jobId: string): Promise<boolean | null> {
  try {
...
async function runChaptersInBackground(state: VideoJobState, chapters: ChapterSpec[]): Promise<void> {
  ...
  const tick = async (): Promise<void> => {
    ...
    const cancelAtTick = await isCancelRequested(state.job_id);
    ...
    while (queue.length > 0 && inFlight.size < MAX_PARALLEL_CHAPTERS) {
      ...
      const cancelPreDispatch = await isCancelRequested(state.job_id);
