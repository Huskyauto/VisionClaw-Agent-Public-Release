...
import { markInternalRenderStaging } from "./lib/internal-render-markers";
import { db } from "./db";
...
async function mirrorStateToDb(state: VideoJobState): Promise<void> {
  ...
  // job_id is the table's global primary key, so changing it to a compound key
  // would be a destructive migration. Keep that conflict identity, but require
  // tenant equality in DO UPDATE: even a caller-controlled/colliding job id can
  // never mutate another tenant's row. The timestamp clause also prevents
  // out-of-order fire-and-forget mirrors from regressing state.
  await db.insert(videoJobs).values(row).onConflictDoUpdate({
    ...
    where: and(
      eq(videoJobs.tenantId, row.tenantId),
      lt(videoJobs.updatedAt, row.updatedAt),
    ),
  });
}
...
async function renderOneChapter(state: VideoJobState, chapter: ChapterSpec, chapterIdx: number): Promise<void> {
  ...
  const renderPromise = produceVideo(markInternalRenderStaging({
  ...
  try {
    ...
    try {
      ...
      const ur = await uploadAndShare({
        ...
        tenantId: input.tenantId,
        generatedArtifact: { artifactKind: "video", logicalName: `${safeTitle}.mp4`, sourceRunKey: `video-job:${input.jobId}` },
      });
      if (!ur.success) {
        state.status = "ready_to_concat";
        state.last_concat_error = `Drive durability verification failed: ${ur.error || "unknown error"}`.slice(0, 500);
        writeStateAtomic(state);
        return { success: false, job_id: input.jobId, status: "ready_to_concat", message: state.last_concat_error, error: state.last_concat_error };
      }
      driveUrl = ur.viewUrl;
    } catch (error) {
      state.status = "ready_to_concat";
      state.last_concat_error = `Drive durability verification failed: ${error instanceof Error ? error.message : "unknown error"}`.slice(0, 500);
      writeStateAtomic(state);
      return { success: false, job_id: input.jobId, status: "ready_to_concat", message: state.last_concat_error, error: state.last_concat_error };
    }
