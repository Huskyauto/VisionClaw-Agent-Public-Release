...
async function getToTClient(tenantId: number): Promise<{ client: OpenAI; modelId: string }> {
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error(`treeOfThought requires a valid tenantId (got: ${JSON.stringify(tenantId)})`);
  }
  const result = await getClientForModel(HIGH_END_WORK_MODEL_ID, tenantId);
