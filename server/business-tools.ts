...
export async function createInvoice(params: any) {
  ...
  // Tenant-isolation fix (2026-07-25): customer_id is an LLM tool arg —
  // verify the caller's tenant owns the customer before storing the FK,
  // or an invoice can be attached to another tenant's customer. Fail closed.
  if (params.customer_id) {
    const own = await db.execute(sql`SELECT id FROM customers WHERE id = ${params.customer_id} AND tenant_id = ${tid} LIMIT 1`);
    if (((own as any).rows || own).length === 0) return { error: `customer_id ${params.customer_id} not found for this tenant` };
  }
  ...
  // Atomicity fix (2026-07-25, architect finding): header + line items +
  // customer revenue rollup commit together or not at all — a mid-sequence
  // failure must not leave partial financial state.
  const invoiceId = await db.transaction(async (tx) => {
    const res = await tx.execute(sql`
      ...
      RETURNING id
    `);
    const newId = (res as any).rows[0].id;
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await tx.execute(sql`
        ...
        VALUES (${newId}, ${item.description}, ${item.quantity || 1}, ${item.unit_price}, ${amount}, ${i + 1})
      `);
    }

    if (params.customer_id) {
      await tx.execute(sql`UPDATE customers SET total_revenue = total_revenue + ${total}, updated_at = CURRENT_TIMESTAMP WHERE id = ${params.customer_id} AND tenant_id = ${tid}`);
    }
    return newId;
  });
...
export async function logExpense(params: any) {
  ...
  // Tenant-isolation fix (2026-07-25): project_id is an LLM tool arg —
  // verify the caller's tenant owns the project before storing the FK. Fail closed.
  if (params.project_id) {
    const own = await db.execute(sql`SELECT id FROM projects WHERE id = ${params.project_id} AND tenant_id = ${tid} LIMIT 1`);
    if (((own as any).rows || own).length === 0) return { error: `project_id ${params.project_id} not found for this tenant` };
  }
...
export async function logInteraction(params: any) {
  ...
  // Tenant-isolation fix (2026-07-25): customer_id is an LLM tool arg — verify
  // the caller's tenant owns the customer before storing the FK (same pattern
  // as createInvoice/createContract). Fail closed.
  const own = await db.execute(sql`SELECT id FROM customers WHERE id = ${params.customer_id} AND tenant_id = ${tid} LIMIT 1`);
  if (((own as any).rows || own).length === 0) return { error: `customer_id ${params.customer_id} not found for this tenant` };
...
export async function createContract(params: any) {
  ...
  // Fail-closed ownership check: a caller-supplied customer_id must belong to
  // the caller's tenant before it is stored on the contract row.
  if (params.customer_id) {
    const owned = await db.execute(sql`SELECT id FROM customers WHERE id = ${params.customer_id} AND tenant_id = ${tid} LIMIT 1`);
    if ((owned as any).rows.length === 0) return { error: `customer ${params.customer_id} not found for this tenant` };
  }
...
export async function listContracts(params: any) {
  ...
  const res = await db.execute(sql`
    ...
    FROM contracts c LEFT JOIN customers cust ON cust.id = c.customer_id AND cust.tenant_id = c.tenant_id
