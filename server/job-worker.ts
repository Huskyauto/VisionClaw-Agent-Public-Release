...
/**
 * Return the persisted queue tenant, rejecting payload scopes that disagree
 * with it. Payloads are replayable input, not an authority for tenant scope.
 */
export function getResearchJobTenantId(
  job: Pick<AgentJob, "kind" | "tenantId" | "payload">,
): number {
  const tenantId = job.tenantId;
  if (tenantId === null || !Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error(`${job.kind} requires a positive persisted tenant scope`);
  }
  if (job.payload.tenantId != null && job.payload.tenantId !== tenantId) {
    throw new Error(`${job.kind} tenant scope mismatch`);
  }
  return tenantId;
}
...
registerJobHandler("research_code_proposal", async (job) => {
  const tenantId = getResearchJobTenantId(job);
  const { sessionId, model, programName, hypothesis, result, approach, score, mapping, personaId } = job.payload;
  ...
  // the durable job — the in-memory activeSessions Map won't survive restart,
...
// S5 — workspace-only consumer. The inbox CAS is the authority; the job
// payload is merely a pointer and can never widen tenant or source scope.
registerJobHandler("source_repair_handoff", async (job) => {
  const tenantId = getResearchJobTenantId(job);
  const handoffId = Number(job.payload.handoffId);
  if (!Number.isInteger(handoffId) || handoffId <= 0) throw new Error("source repair handoff missing inbox id");
  const { consumeRepairHandoff } = await import("./source-repair-handoff");
  return consumeRepairHandoff(handoffId, tenantId);
});

registerJobHandler("source_repair_verification", async (job) => {
  const tenantId = getResearchJobTenantId(job);
  const proposalId = Number(job.payload.proposalId);
  if (!Number.isInteger(proposalId) || proposalId <= 0) throw new Error("source repair verification missing proposal id");
  const { verifyProposalById } = await import("./proposal-verifier");
  return verifyProposalById(proposalId, tenantId);
});
...
registerJobHandler("astra_report_batch", async (job) => {
  if (
    job.tenantId == null ||
    job.payload.tenantId !== job.tenantId ||
    job.payload.input?.tenantId !== job.tenantId
  ) {
    throw new Error("astra_report_batch tenant scope mismatch");
  }
  const { advanceAstraBatch } = await import("./lib/astra-batch-finisher");
  const advanced = await advanceAstraBatch(job.payload.input, job.payload.batch);
  if (advanced.kind === "pending") {
    const deferred = await deferJob(
      job.id,
      job.attempts,
      job.tenantId,
      { ...job.payload, batch: advanced.record },
      60_000,
    );
    if (!deferred) throw new Error("astra_report_batch lost its lease while persisting provider state");
  ...
  const recorded = await recordCost({
    ...
    tokensIn: advanced.record.tokensIn ?? 0,
    tokensOut: advanced.record.tokensOut ?? 0,
    cachedTokensIn: advanced.record.cachedTokensIn ?? 0,
    operation: "batch_response",
    idempotencyKey: `${advanced.record.submissionKey}-cost`,
  });
  if (!recorded) throw new Error("astra_report_batch could not persist cost accounting");
  const continuation = job.payload.continuation;
  if (!continuation || continuation.tenantId !== job.tenantId) {
    throw new Error("astra_report_batch missing tenant-scoped report continuation");
  }
  const { finalizePreparedResearchReport, parseAstraReportDraft } = await import("./research-report-fulfillment");
  const parsed = advanced.usedAstra
    ? parseAstraReportDraft(
        advanced.text,
        continuation.sections.map((section: any) => section.heading),
        continuation.sections,
      )
    : null;
  const sections = parsed
    ? continuation.sections.map((section: any, index: number) => ({ ...section, body: parsed[index].body }))
    : continuation.sections;
  const fulfillment = await finalizePreparedResearchReport({ ...continuation, sections });
  if (!fulfillment.success) {
    throw new Error(`astra_report_batch final report assembly failed: ${fulfillment.error || "unknown"}`);
  }
  if (continuation.deliveryContext) {
    const { completeDelayedResearchReport } = await import("./webhookHandlers");
    await completeDelayedResearchReport({
      tenantId: job.tenantId,
      context: continuation.deliveryContext,
      intake: continuation.intake,
      fulfillment,
    });
  }
  return { ...fulfillment, usedAstra: advanced.usedAstra, batch: advanced.record };
});
