...
/** Preserve every non-terminal order; only terminal history is compacted. */
export function compactQueue<T extends { status: string; createdAt: string }>(items: T[], limit = QUEUE_HISTORY_LIMIT): T[] {
  const unresolved = items.filter(item => item.status !== "shipped" && item.status !== "rejected");
  if (unresolved.length >= limit) return unresolved;
  const terminal = items
    .filter(item => item.status === "shipped" || item.status === "rejected")
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
    .slice(0, limit - unresolved.length);
...
function writeQueueAtomic(q: QueueFile, mirror = true) {
  q.items = compactQueue(q.items);
  ensureDir();
  const tmp = `${queueFile()}.${process.pid}.${Date.now()}.tmp`;
  const json = JSON.stringify(q, null, 2);
  fs.writeFileSync(tmp, json, { mode: 0o600 });
  fs.renameSync(tmp, queueFile());
  // Durable mirror: prod FS is ephemeral (resets on every republish), so a
  // file-only queue silently loses paid pending orders on deploy. Ordered +
  // coalesced (see scheduleMirror) — fail-open (the file write above already
  // succeeded) but LOUD on failure. Test seam: no live mirror when the queue
  // path is overridden.
  if (mirror && !process.env.SERVICE_REVIEW_QUEUE_FILE) scheduleMirror(json);
}

// ───────────────────────────────────────────────────────────────────────
// Postgres durability layer (2026-08-14). The queue file is the working
// copy; a row in file_storage (admin tenant) is the crash/republish
// backup. Restore happens once at boot, ONLY when the file is absent —
// never over an existing (possibly newer) file, and never over a
// quarantined-corrupt state (readQueue throws before restore matters).
// ───────────────────────────────────────────────────────────────────────
const QUEUE_BACKUP_FILENAME = "service-review-queue-backup.json";
const QUEUE_BACKUP_TENANT = 1; // admin tenant — queue is platform-level state

// ── Ordered, coalescing mirror scheduler ───────────────────────────────
// Fire-and-forget mirrors from successive writes must NEVER land out of
// order (an older mirror overwriting a newer one would make boot restore
// resurrect a stale queue — architect HIGH, 2026-08-14). We keep only the
// LATEST pending JSON and drain writes on a single serialized chain, so
// the last file write is always the last DB write. Intermediate states
// are coalesced away — only the newest snapshot matters for restore.
let __latestMirrorJson: string | null = null;
let __mirrorDraining = false;

/** Injectable for tests — production writer upserts atomically via ON CONFLICT
 *  on the (tenant_id, filename) unique index. */
export let __mirrorWriter: (json: string) => Promise<void> = async (json: string) => {
  const { db } = await import("./db");
  const { fileStorage } = await import("@shared/schema");
  await db.insert(fileStorage).values({
    filename: QUEUE_BACKUP_FILENAME,
    tenantId: QUEUE_BACKUP_TENANT,
    originalName: QUEUE_BACKUP_FILENAME,
    mimeType: "application/json",
    size: Buffer.byteLength(json),
    data: json,
  }).onConflictDoUpdate({
    target: [fileStorage.tenantId, fileStorage.filename],
    // Matches the partial unique index file_storage_queue_backup_uidx.
    targetWhere: sql`filename = 'service-review-queue-backup.json'`,
    set: { size: Buffer.byteLength(json), data: json },
  });
};

/** Test seam: swap the DB writer for a recorder. Returns the previous writer. */
export function __setMirrorWriterForTest(w: (json: string) => Promise<void>): (json: string) => Promise<void> {
  const prev = __mirrorWriter;
  __mirrorWriter = w;
  return prev;
}

function scheduleMirror(json: string): void {
  __latestMirrorJson = json;
  if (__mirrorDraining) return; // drainer will pick up the newest snapshot
  __mirrorDraining = true;
  void (async () => {
    try {
      while (__latestMirrorJson !== null) {
        const next = __latestMirrorJson;
        __latestMirrorJson = null;
        try {
          await __mirrorWriter(next);
        } catch (err: any) {
          console.error(`[service-review] DB mirror FAILED (queue file is still intact on disk): ${err.message}`);
        }
      }
    } finally {
      __mirrorDraining = false;
    }
  })();
}

/** Test hook: drive the mirror scheduler directly (bypasses live-DB gating). */
export function __test_scheduleMirror(json: string): void {
  scheduleMirror(json);
}

/** Exposed for tests: resolves once no mirror write is pending or in flight. */
export async function __awaitMirrorIdleForTest(): Promise<void> {
  while (__mirrorDraining || __latestMirrorJson !== null) {
    await new Promise((r) => setTimeout(r, 5));
  }
}

/**
 * Boot-time restore: if the queue file is missing (fresh prod FS after a
 * republish, or a wiped workspace) and a DB backup exists, rebuild the file
 * from the backup BEFORE anything reads the queue. Validates shape strictly —
 * a malformed backup is refused (loud), never written as the live queue.
 */
export async function restoreQueueFromDbIfMissing(): Promise<boolean> {
  if (process.env.SERVICE_REVIEW_QUEUE_FILE) return false; // test seam: no live restore
  try {
    if (fs.existsSync(queueFile())) return false; // never clobber an existing file
    const { db } = await import("./db");
    const { fileStorage } = await import("@shared/schema");
    const { and, eq } = await import("drizzle-orm");
    const { desc } = await import("drizzle-orm");
    // Belt-and-braces: the (tenant_id, filename) unique index guarantees one
    // row, but order by newest id anyway so a legacy duplicate can never win.
    const [row] = await db.select().from(fileStorage)
      .where(and(eq(fileStorage.filename, QUEUE_BACKUP_FILENAME), eq(fileStorage.tenantId, QUEUE_BACKUP_TENANT)))
      .orderBy(desc(fileStorage.id))
      .limit(1);
    if (!row?.data) return false; // no backup yet — nothing to restore
    const parsed = JSON.parse(row.data);
    if (!parsed || parsed.version !== 1 || !Array.isArray(parsed.items)) {
      console.error("[service-review] DB backup has unexpected shape — REFUSING restore (manual inspection needed).");
      return false;
    }
    ensureDir();
    const tmp = `${queueFile()}.restore.${process.pid}.tmp`;
    fs.writeFileSync(tmp, row.data, { mode: 0o600 });
    fs.renameSync(tmp, queueFile());
    console.log(`[service-review] Restored review queue from DB backup (${parsed.items.length} items) — file was missing (fresh FS).`);
    return true;
  } catch (err: any) {
    console.error(`[service-review] DB restore check failed (queue starts empty if file is missing): ${err.message}`);
    return false;
  }
}
...
export async function addReviewItem(input: Omit<ReviewItem, "id" | "reviewToken" | "createdAt" | "status"> & { status?: ReviewStatus; durable?: boolean }): Promise<ReviewItem> {
  return withQueueLock(() => {
    ...
    if (existing) {
      ...
      if (input.durable) {
        return (async () => {
          await __awaitMirrorIdleForTest();
          await __mirrorWriter(JSON.stringify(q, null, 2));
          return existing;
        })() as any;
      }
      return existing;
    }
    const { durable, ...itemInput } = input;
    ...
    if (durable) {
      q.items = compactQueue(q.items);
      const json = JSON.stringify(q, null, 2);
      writeQueueAtomic(q, false);
      // Paid webhook callers opt into an awaitable, fail-closed mirror. Other
      // queue paths retain the historical coalesced best-effort behavior.
      return (async () => {
        await __awaitMirrorIdleForTest();
        await __mirrorWriter(json);
        return item;
      })() as any;
    }
    writeQueueAtomic(q);
...
/** Rebuild an attached PDF cache from the durable file_storage/object store. */
export async function restorePdfCacheFromDurableBytes(
  targetPath: string,
  bytes: Buffer,
  write: (target: string, data: Buffer) => Promise<void> = async (target, data) => {
    fs.mkdirSync(path.dirname(target), { recursive: true });
    fs.writeFileSync(target, data, { mode: 0o600 });
  },
): Promise<string> {
  if (bytes.subarray(0, 5).toString("ascii") !== "%PDF-") throw new Error("Durable artifact is not a PDF");
  await write(targetPath, bytes);
  return targetPath;
}

export async function rehydrateReviewArtifact(item: ReviewItem): Promise<string | null> {
  if (!item.artifactFilename && !item.artifactStorageKey && !item.fileName) return null;
  const abs = path.resolve(process.cwd(), item.filePath || path.join("uploads", item.artifactFilename || ""));
  const allowed = path.resolve(process.cwd(), "uploads");
  if (abs !== allowed && !abs.startsWith(`${allowed}${path.sep}`)) return null;
  if (fs.existsSync(abs)) {
    const bytes = fs.readFileSync(abs);
    if (bytes.subarray(0, 5).toString("ascii") === "%PDF-") return abs;
  }
  const { db } = await import("./db");
  const { fileStorage } = await import("@shared/schema");
  const { and, eq } = await import("drizzle-orm");
  const filename = item.artifactFilename || path.basename(abs);
  const [row] = await db.select().from(fileStorage).where(and(eq(fileStorage.tenantId, 1), eq(fileStorage.filename, filename))).limit(1);
  if (!row) return null;
  let bytes: Buffer;
  if (row.storageKey) {
    const { downloadTenantFile } = await import("./object-storage");
    bytes = await downloadTenantFile(1, row.storageKey);
  } else {
    bytes = Buffer.from(row.data || "", "base64");
  }
  try {
    return await restorePdfCacheFromDurableBytes(abs, bytes);
  } catch {
    return null;
  }
}

export async function updateReviewItem(id: string, patch: Partial<ReviewItem>, options?: { durable?: boolean }): Promise<ReviewItem | null> {
  return withQueueLock(() => {
    const q = readQueue();
    ...
    if (options?.durable) {
      ...
      return (async () => {
        await __awaitMirrorIdleForTest();
        await __mirrorWriter(json);
        return q.items.find(item => item.id === stableId) || null;
      })() as any;
    }
    writeQueueAtomic(q);
    return q.items.find(item => item.id === stableId) || null;
...
export function isAutoShipEligible(sku: string): { eligible: boolean; reason: string; stats: SkuStats; policy: AutoShipPolicy } {
  ...
  if (sku.startsWith("sample-agency-audit-")) {
    return { eligible: false, reason: 'Human-agency audits are always manual review', stats, policy: { ...policy, enabled: false } };
  }
