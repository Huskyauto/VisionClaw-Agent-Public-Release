...
// R125.1 — SESSION_SECRET strength gate (Meta AI review pickup). SESSION_SECRET
// keys session cookies AND the AES-256-GCM at-rest encryption (crypto.ts HKDF).
// A missing/short secret in prod means weak crypto — refuse to boot rather than
// run degraded. Dev warns only (local experimentation stays unblocked).
(function assertSessionSecretStrength() {
  const len = (process.env.SESSION_SECRET || "").length;
  if (len >= 32) return;
  const msg = `[boot] SESSION_SECRET is ${len === 0 ? "missing" : `only ${len} chars`} (minimum 32). Sessions and at-rest credential encryption require a strong secret.`;
...
const isolatedStaging = process.env.ISOLATED_STAGING === "1";
...
if (isolatedStaging) {
  app.use((req, res, next) => {
    const blockedPrefixes = [
      ...
      "/api/payments",
      "/api/store/checkout",
      "/api/public/archive-rescue/checkout",
      "/api/admin/income/payment-link",
    ];
    if (blockedPrefixes.some((prefix) => req.path.startsWith(prefix))) {
      return res.status(404).json({ error: "Not available in isolated staging" });
    }
    next();
  });
}
...
(async () => {
  if (!isolatedStaging) {
      ...
      if (databaseUrl) {
        ...
        if (replitDomains) {
          ...
          try {
            ...
            // Match by URL only. Never auto-create here: Stripe reveals a new
            // endpoint's signing secret once, and this process has no atomic
            // path to install it as STRIPE_WEBHOOK_SECRET.
            const existing = await stripe.webhookEndpoints.list({ limit: 100 });
            const match = existing.data.find((e: any) => e.url === desiredUrl);
            if (match) {
              const secretReady = !!process.env.STRIPE_WEBHOOK_SECRET;
              const level = match.status === 'enabled' && secretReady ? 'log' : 'error';
              console[level](
                `[stripe] Webhook endpoint found: ${match.url}; status=${match.status}; signing-secret=${secretReady ? 'configured (parity is verified per delivery)' : 'missing'}`,
              );
            } else {
              console.error(`[stripe] No webhook exists for ${desiredUrl}; configure it with its signing secret before accepting live events`);
            }
          } catch (whErr: any) {
            console.error(`[stripe] Webhook configuration check failed: ${whErr.message?.slice(0, 80)}`);
          }
        } else {
    ...
    console.log("[staging] Stripe/payment startup disabled");
  }
  ...
  if (!isolatedStaging) {
    ...
    console.log("[staging] Provider token refresh loops disabled");
  }
  ...
  // this middleware is a pass-through and the real handler takes over. The
  // placeholder must return 200 because Reserved VM promotion probes GET /.
  let appReady = false;
  app.use((req, res, next) => {
  ...
  // OIDC discovery is network-dependent and can consume the Reserved VM's
  // entire port-open deadline. Keep auth ahead of every real application route,
  // but perform it after the production socket and warm-up page are available.
  ...
  if (isProductionRuntime() && !isolatedStaging) {
    setTimeout(async () => {
      try {
  ...
  // Catalog file integrity check — every static product's files must exist on
  // disk (they live in the committed project-assets/catalog/ + project-assets/
  // dirs, so a publish must never lose them). A missing file used to 500 the
  // admin Service Orders page; lookupProduct is now fail-soft, but this makes
  // the breakage LOUD at boot instead of silent until a customer buys.
  try {
    const { verifyCatalogFiles } = await import("./product-catalog");
    const missing = verifyCatalogFiles();
    if (missing.length === 0) {
      console.log("[startup] product catalog integrity OK — all static product files present");
    } else {
      console.error(`[startup] PRODUCT CATALOG BROKEN — ${missing.length} missing file(s): ${missing.map(m => `${m.sku}:${m.filePath}`).join(", ")}. Affected SKUs cannot be purchased/delivered until restored (e.g. npx tsx scripts/regen-bundle-quickstart.ts for the bundle quickstart PDF).`);
    }
  } catch (e: any) {
    console.warn(`[startup] catalog integrity check skipped: ${e?.message?.slice(0, 120)}`);
  }
  ...
  // Recover stale generic video jobs from their tenant-scoped durable render
  // manifest. Recovery reuses only verified chapter files and atomically claims
  // each job, so a restart resumes work instead of leaving a phantom job.
  try {
    const { recoverStaleVideoJobs, armPeriodicRecoverySweeper } = await import("./video-job-runner");
    const r = await recoverStaleVideoJobs();
    if (r.recovered > 0) console.log(`[startup] video-jobs recovery: resumed ${r.recovered} stale job(s)`);
  ...
  if (isolatedStaging) {
    log("Isolated staging: email, schedulers, autonomous recovery, job workers, and maintenance loops disabled", "staging");
  } else {
  setTimeout(async () => {
    try {
  ...
  setInterval(async () => {
    try {
      ...
      if (result.resumed > 0 || result.restarted > 0 || result.recovered > 0) {
        log(
          `Periodic sweep reclaimed ${result.resumed} approved + ${result.restarted} stuck-executing + ${result.recovered} transient-failed plan(s)`,
          "plan-executor",
        );
      }
    } catch (e: any) {
  ...
  async function gracefulShutdown(signal: string) {
    ...
    try {
      const { stopClaudeRunnerBridge } = await import("./claude-runner");
      await stopClaudeRunnerBridge();
      log("Claude Runner bridge stopped", "shutdown");
    } catch (_silentErr) { logSilentCatch("server/index.ts", _silentErr); }
