...
export interface CatalogProduct {
  ...
  description: string;
  kind?: 'static' | 'service';
  serviceType?: 'research-report' | 'readiness-audit' | 'human-agency-audit';
...
export function verifyCatalogFiles(): { sku: string; filePath: string }[] {
  const missing: { sku: string; filePath: string }[] = [];
  for (const product of Object.values(CATALOG)) {
    if (product.kind === 'service') continue;
    const paths = [
      ...(product.primary ? [product.primary.filePath] : []),
      ...(product.additionalFiles || []).map(f => f.filePath).filter((p): p is string => !!p),
    ];
    for (const filePath of paths) {
      try {
        resolveAndCheck(filePath);
      } catch {
        missing.push({ sku: product.sku, filePath });
      }
    }
  }
  return missing;
