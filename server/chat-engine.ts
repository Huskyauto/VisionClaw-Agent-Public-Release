...
import { generateHypotheticalMemory, recordHydeShadow } from "./lib/hyde";
    ...
    if (userMessage) {
      try {
        ...
        fitMap = await scoreSkillFit({
          userMessage,
          skills: enabledSkills.map((s: any) => ({ name: s.name, description: s.description, promptContent: s.promptContent })),
    ...
    try {
      ...
      const queryEmbedding = hasAnyEmbeddings
        ? await (async () => {
            ...
            const withBudget = async <T>(p: Promise<T | null>, ms: number): Promise<T | null> => {
              if (ms <= 100) return null;
              let t: NodeJS.Timeout | undefined;
              try {
                return await Promise.race([
                  p,
                  new Promise<null>((_, r) => { t = setTimeout(() => r(null), ms); }),
                ]);
              } catch { return null; }
            ...
            try {
              if (MEMORY_HYDE_ENABLED) {
                ...
                const hypothetical = await generateHypotheticalMemory(userMessage);
                if (hypothetical) {
                  const hydeEmb = await withBudget(generateEmbedding(hypothetical), remaining());
                  if (hydeEmb) {
              ...
              return await withBudget(generateEmbedding(userMessage), remaining());
            } catch {
              return null;
            }
          })()
      ...
      // Shadow scoring (fire-and-forget, sampled at MEMORY_HYDE_SHADOW_RATE,
      // default 20%): re-rank memories with the raw query embedding and compare
      // the top-5 against the HyDE top-5. delta_top5=0 means both approaches
      // return identical results; delta_top5=5 means completely different sets.
      // Results are aggregated and logged every 50 comparisons by hyde.ts.
      if (_capturedHydeEmb) {
        const hydeEmbSnap = _capturedHydeEmb;
        const sampleRate = Number(process.env.MEMORY_HYDE_SHADOW_RATE ?? "0.2");
        if (Math.random() < sampleRate) {
          void (async () => {
            try {
              const rawEmb = await generateEmbedding(userMessage).catch(() => null);
              if (!rawEmb) return;
              const TOP_K = 5;
              const hydeTop = rankMemories(active, hydeEmbSnap, userMessage, RANKING_OPTS)
                .slice(0, TOP_K).map((m: any) => m.id);
              const rawSet = new Set(
                rankMemories(active, rawEmb, userMessage, RANKING_OPTS)
                  .slice(0, TOP_K).map((m: any) => m.id)
              );
              const delta = TOP_K - hydeTop.filter((id: any) => rawSet.has(id)).length;
              recordHydeShadow(delta);
            } catch (_hydeErr) { logSilentCatch("server/chat-engine.ts", _hydeErr); }
          })();
        }
      }
    } catch {
  ...
  try {
    ...
    if (hasAnyEmbeddings) {
      ...
      const withBudget = async <T>(p: Promise<T | null>, ms: number): Promise<T | null> => {
        ...
        let t: NodeJS.Timeout | undefined;
        try {
          return await Promise.race([p, new Promise<null>((_, r) => { t = setTimeout(() => r(null), ms); })]);
        } catch { return null; } finally { clearTimeout(t); }
      };
      if (process.env.MEMORY_HYDE_ENABLED !== "0") {
        const hypothetical = await generateHypotheticalMemory(userMessage);
        if (hypothetical) queryEmbedding = await withBudget(generateEmbedding(hypothetical), remaining());
      }
      if (!queryEmbedding) queryEmbedding = await withBudget(generateEmbedding(userMessage), remaining());
    }
...
// Defaults for non-admin tenants. processMessage resolves tenant-aware,
// env-overridable budgets per turn (mirrors the SSE route in routes.ts) —
// the admin/owner tenant gets a much larger budget so long multi-step
// investigations don't strand the final deliverable (PDF/report) at the cap.
...
export function resolveToolBudget(tenantId: number): { maxRounds: number; maxTotal: number; maxPerRound: number } {
  ...
  return isAdmin
    ? {
        ...
        maxTotal: parseIntCap(process.env.MAX_TOTAL_TOOL_CALLS_ADMIN, 40, 1, 200, "MAX_TOTAL_TOOL_CALLS_ADMIN"),
...
export async function processMessage(
  conversationId: number,
  content: string,
  opts?: { source?: string; enableTools?: boolean; blockedTools?: Set<string>; depth?: number; toolFilter?: string[]; leanContext?: boolean; pageContext?: any; radioChannel?: string }
...
async function _processMessageImpl(
  conversationId: number,
  content: string,
  opts: { source?: string; enableTools?: boolean; blockedTools?: Set<string>; depth?: number; toolFilter?: string[]; leanContext?: boolean; pageContext?: any; radioChannel?: string } | undefined,
  conv: any,
  tenantId: number,
): Promise<ChatEngineResult> {
  ...
  // model-rewritten prompt. The prompt-refiner (below) reassigns `content` to
  // jury output — a trust-boundary shift that MUST be re-validated, since the
  // safety-guard + intent-gate above only saw the ORIGINAL text. Returns a
  ...
  // it into a sharper, fully-specified prompt + strategy, THEN hand the refined
  // prompt to the main agent loop for final task completion. Mirrors the auto-
  ...
  // persisted above (transcript shows what Bob typed; only the model sees the
  // refined version). FAIL-OPEN: any failure/timeout/degenerate result leaves
    ...
    try {
      ...
      if (isRefinerEnabled()) {
        ...
        if (doRefine) {
          ...
          if (result.refined && result.refinedPrompt.trim().length > 0) {
            // Hand the upgraded prompt to the main agent loop and skip the
            // auto-ensemble answer path below (`!promptRefined`) so the jury
            // isn't charged twice — the main LLM does final completion.
            content = result.refinedPrompt;
            promptRefined = true;
            // Re-screen the model-rewritten prompt: reassigning `content` is a
            // trust-boundary shift, and the intent-gate above only saw the
            // ORIGINAL text. Refuse if the refined prompt trips a restricted
            // category (defense-in-depth; fail-open on classifier error).
            const refusal = await runIntentGateOrRefuse(content);
            if (refusal) return refusal;
      ...
      console.warn(`[prompt-refiner] integration error (using original prompt): ${e.message}`);
    }
  }
...
- Never send multi-section responses with "Summary:", "Planned capabilities:", etc.`;
  ...
  const contrastiveNote = (routedResult as any).contrastiveNote || "";
...
export { stripThinkTags, windowMessages, updateDailyLog };
