  ...
  if (depth === 0 && executedTools.length > 0 && !opts?.source?.startsWith("subagent:")) {
    ...
    if (incompleteOutcome) {
      ...
      try {
        ...
        const { client: completionClient, actualModelId: completionModel } =
          await getClientForModel(HIGH_END_WORK_MODEL_ID, tenantIdForScope);
        const completionResp = await completionClient.chat.completions.create({
        ...
        console.warn(`[completion-gate] Rebuild failed for ${agentName}: ${errorMessage(cgErr)}`);
      }
    }
  }
...
export { autoDetectThinkingLevel, detectIncompleteOutcome, parseInlineToolCalls, parseXmlToolCalls, resolveToolBudget, stripThinkTags, updateDailyLog, windowMessages };
