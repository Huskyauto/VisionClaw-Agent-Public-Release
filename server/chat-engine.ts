...
async function _processMessageImpl(
  ...
  let model = normalizeModelId(conv.model || "gpt-5.6-sol");
...
- Never send multi-section responses with "Summary:", "Planned capabilities:", etc.`;
  ...
  // GLM-5.3-Flash is evaluation-only: an owner must explicitly select it.
  // This narrow override reaches the real metered OpenRouter model without
  // enabling metered routing globally or changing any automatic model roster.
  const ownerEvaluationOverride =
  ...
  let { client: activeClient, actualModelId: activeModelId } = await getClientForModel(model, tenantId, {
    ...
    costExemptLane: upRouteActive,
    meteredOverride: ownerEvaluationOverride,
  });
