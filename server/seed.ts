...
async function ensureProductionIndexes() {
  ...
  // 'expired' (lazy TTL expiry in minerva-planner.listPlans) and 'revised',
  ...
  try {
      ...
      BEGIN
        IF NOT EXISTS (
          ...
          ALTER TABLE plans ADD CONSTRAINT plans_status_check CHECK (status = ANY (ARRAY[
            ...
            'revised'::text,'executing'::text,'completed'::text,'failed'::text,'expired'::text
          ]));
        END IF;
      END $$;
    `));
    ...
    console.log(`[seed] plans_status_check ('expired') + pending-plans partial index ensured`);
