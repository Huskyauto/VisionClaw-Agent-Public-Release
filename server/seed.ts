      ...
      await (db.insert(heartbeatTasks) as any).values([
        {
          ...
          createdBy: "system",
          tenantId: 1, // tenant-isolation audit 2026-07-31: heartbeat_tasks.tenant_id is NOT NULL with no default; system tasks belong to the admin tenant
...
- Be specific about use cases — do not recommend models without clear purpose`,
    ...
    if (settings) {
      ...
      const rawPin = process.env.ADMIN_PIN || "0000";
      // Whitespace-only ADMIN_PIN trims to "" — falling back to "0000" here
      // instead of hashing the empty string, which would recreate the exact
      // unloggable-in lockout this block exists to prevent.
      const defaultPin = rawPin.trim() || "0000";
      const { hashPin } = await import("./auth");
      const desired = hashPin(defaultPin);
      const corruptUntrimmed = rawPin === defaultPin ? null : hashPin(rawPin);
      ...
      if (!settings.accessPin) {
        const r: any = await db.execute(sql`
          UPDATE agent_settings SET access_pin = ${desired}
          WHERE id = 1 AND (access_pin IS NULL OR access_pin = '')
        `);
        if ((r.rowCount ?? 0) > 0) console.log("[seed] Admin PIN configured (peppered)");
      } else if (settings.accessPin === STALE_SEED_PIN_HASH && desired !== STALE_SEED_PIN_HASH) {
      ...
      } else if (corruptUntrimmed && settings.accessPin === corruptUntrimmed) {
        const r: any = await db.execute(sql`
          UPDATE agent_settings SET access_pin = ${desired}
          WHERE id = 1 AND access_pin = ${corruptUntrimmed}
        `);
