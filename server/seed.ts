...
async function cleanupRunawayDelegationTasks() {
  try {
    ...
    await db.execute(sql`
      UPDATE heartbeat_tasks SET cron_expression = '*/30 * * * *'
      WHERE name = 'Self-Reflection' AND cron_expression != '*/30 * * * *'
    `);
    await db.execute(sql`
...
const platformBriefingContent = `VISIONCLAW PLATFORM UPDATE BRIEFING — April 14, 2026

LATEST STATS (live from system):
- 195 enterprise tools (up from 152)
...
RECENT IMPROVEMENTS:
1. ARCHITECTURE PAGE (/architecture) — Live stats from system. Shows 195 tools, all 14 personas, real-time uptime, animated counters, 6 stat cards.

2. COMPARISON PAGE (/compare) — "9 Manual Steps. 14 Agents. Zero Effort." Shows how VisionClaw autonomously handles everything founders do manually with Claude in 9 steps.
...
KEY DEMO PAGES:
- /compare — "9 Manual Steps vs 14 Agents" comparison (public, no login)
