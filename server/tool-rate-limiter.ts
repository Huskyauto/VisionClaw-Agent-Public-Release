...
const EXPENSIVE_TOOLS: Record<string, RateLimitConfig> = {
  ...
  // R125+129 follow-up — evidence-docket assembly fans out to many tenant-scoped
  // DB queries + markdown/HTML render and is registered very_slow; throttle to
  // match deep_research so it can't be hammered from any persona surface.
  generate_evidence_docket: { maxPerMinute: 1, maxPerHour: 5, maxPerDay: 20 },
  // Simulation Sandbox (S4) — sandbox_run's model-swap corpora spend real LLM
  // budget ($5/run cap) and a run can take minutes; throttle hard so it can't
  // be hammered even by a trusted persona in a loop.
  sandbox_run: { maxPerMinute: 1, maxPerHour: 4, maxPerDay: 12 },
  // Verified Revenue Missions (S5a) — draft_experiment fans out a Gmail harvest
  // + an LLM ICP-filter/drafting pass. Owner-only + trusted-only already, but
  // throttle so a looping agent can't hammer the Gmail API or pile up
  // awaiting_approval packets faster than a human could ever review them.
  revenue_mission_draft_experiment: { maxPerMinute: 1, maxPerHour: 4, maxPerDay: 10 },
  ...
  scrapling_scrape:     { maxPerMinute: 5, maxPerHour: 30, maxPerDay: 100 },
  multi_source_collect: { maxPerMinute: 2, maxPerHour: 15, maxPerDay: 50 },
  ...
  // CMMC prospecting makes a bounded SAM.gov batch plus CAGE verifications;
  // rate-limit runs so a caller cannot multiply that fixed external budget.
  discover_cmmc_prospects: { maxPerMinute: 1, maxPerHour: 5, maxPerDay: 20 },
  lookup_sam_company_cage: { maxPerMinute: 1, maxPerHour: 10, maxPerDay: 40 },
  lookup_sam_exact_company: { maxPerMinute: 1, maxPerHour: 10, maxPerDay: 40 },
...
// The owner tenant runs several specialist agents concurrently. The standard
// 40/hour web bucket was shared across every conversation, so one research run
// could starve unrelated report work. Keep the per-minute pacing guard, while
// sizing the aggregate owner ceiling for multiple bounded turns.
const OWNER_RESEARCH_LIMITS: Record<string, RateLimitConfig> = {
  web_fetch:  { maxPerMinute: 8, maxPerHour: 160, maxPerDay: 600 },
  web_search: { maxPerMinute: 8, maxPerHour: 120, maxPerDay: 500 },
};

const DEFAULT_LIMIT: RateLimitConfig = { maxPerMinute: 10, maxPerHour: 60, maxPerDay: 200 };

function resolveRateLimitConfig(tenantId: number, toolName: string): RateLimitConfig {
  if (tenantId === ADMIN_TENANT_ID && OWNER_RESEARCH_LIMITS[toolName]) {
    return OWNER_RESEARCH_LIMITS[toolName];
  }
...
export function checkToolRateLimit(
  tenantId: number,
  toolName: string
): { allowed: boolean; reason?: string; retryAfterMs?: number; window?: RateLimitWindow } {
  cleanupStaleEntries();

  const config = resolveRateLimitConfig(tenantId, toolName);
...
// R125+138 — named alternate avenues per rate-limited tool. Each tool has its
// OWN per-tenant bucket AND a genuinely different external backend (Perplexity
// vs direct HTTP vs Firecrawl vs Rayobrowse vs Camofox), so spreading a big
// research batch across them is real parallel capacity, not limit evasion on
// one upstream. Front-loaded into the RATE LIMITED error so the agent learns
// the substitution at exactly the moment it needs it (buried doctrine loses to
// truncation — tool-description-routing lesson).
const RATE_LIMIT_ALTERNATES: Record<string, string> = {
  web_fetch: "firecrawl_scrape (separate limit, managed proxy), web_search (separate limit, Perplexity-backed), or stealth_browse/stealth_browse_camofox for hard pages",
  web_search: "web_fetch a known-good source directly, firecrawl_search, or deep_research for a big batched question",
  firecrawl_scrape: "web_fetch (separate limit, direct HTTP), or stealth_browse/stealth_browse_camofox",
  scrapling_scrape: "web_fetch or firecrawl_scrape when only the minute window is full; honor any domain Retry-After cooldown",
  multi_source_collect: "run fewer URLs and use the partial evidence already returned",
  firecrawl_crawl: "firecrawl_scrape individual key pages, or web_fetch them directly",
  browser: "web_fetch for static content, stealth_browse, or stealth_browse_camofox",
};
export function rateLimitAlternatesHint(toolName: string, window?: RateLimitWindow): string {
  // Hour/day are real SPEND CEILINGS — never steer the agent to sibling tools
  // to evade them (architect finding, 2026-08-08). Consolidate instead.
  if (window === "hour" || window === "day") {
    return " This is a hard usage ceiling — do NOT switch to sibling web tools just to keep the same lookup pattern going. Consolidate now: if this is CMMC prospecting, use discover_cmmc_prospects (the bounded official SAM.gov path). Otherwise stop research and finish, save, register, and deliver the best truthful report from evidence already collected; label any coverage gap instead of abandoning the deliverable.";
  }
  const alt = RATE_LIMIT_ALTERNATES[toolName];
  return alt
    ? ` Alternate avenues with SEPARATE limits: ${alt}. Spread the remaining lookups across them instead of waiting.`
    : " Use a different tool or approach instead.";
}

// PACE instead of KILL (Bob 2026-08-03, extended 2026-08-08 — Felix's research
// runs kept dying on web_fetch 8/8-per-minute via the GUARDED path, which
// lacked the pacing the dispatcher middleware already had). When only the
// per-MINUTE window is full (retryAfterMs small), wait once until a slot
// frees, then re-check and return that verdict. Hour/day caps still hard-deny
// — those are real spend ceilings, and a 5min-1hr sleep inside a tool call
// would wedge the executor. Bounded: single wait, ≤70s. Shared by BOTH
// enforcement points (guarded-tool-executor + tools middleware) so the
// behavior can't drift again.
export const PACE_MAX_WAIT_MS = 70_000;
export async function checkToolRateLimitPaced(
  tenantId: number,
  toolName: string
): Promise<{ allowed: boolean; reason?: string; retryAfterMs?: number; window?: RateLimitWindow; pacedMs?: number }> {
  let rl = checkToolRateLimit(tenantId, toolName);
  if (!rl.allowed && typeof rl.retryAfterMs === "number" && rl.retryAfterMs > 0 && rl.retryAfterMs <= PACE_MAX_WAIT_MS) {
    const waitMs = Math.min(rl.retryAfterMs + 250 + Math.floor(Math.random() * 500), PACE_MAX_WAIT_MS);
    console.warn(`[rate-limiter] PACING tool=${toolName} tenant=${tenantId} — minute window full, waiting ${waitMs}ms instead of failing (${rl.reason})`);
    await new Promise((r) => setTimeout(r, waitMs));
    rl = checkToolRateLimit(tenantId, toolName);
    return { ...rl, pacedMs: waitMs };
  }
  return rl;
}
...
export function getToolUsageStats(tenantId: number, toolName?: string): Record<string, { lastMinute: number; lastHour: number; lastDay: number; limit: RateLimitConfig }> {
  ...
  for (const [key, entries] of usageCache) {
    ...
    stats[tool] = {
      ...
      limit: resolveRateLimitConfig(tenantId, tool),
    };
  }
...
export function getRateLimitConfig(toolName: string, tenantId?: number): RateLimitConfig {
  return tenantId === undefined
    ? EXPENSIVE_TOOLS[toolName] || HIGHER_LIMIT_TOOLS[toolName] || DEFAULT_LIMIT
    : resolveRateLimitConfig(tenantId, toolName);
}
