...
export function rateLimitAlternatesHint(toolName: string, window?: RateLimitWindow): string {
  ...
  if (window === "hour" || window === "day") {
    return " This is a hard usage ceiling — do NOT switch to sibling web tools just to keep the same lookup pattern going. Consolidate the remaining work into fewer, richer calls (or resume later).";
  }
