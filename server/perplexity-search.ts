...
export async function perplexitySearch(query: string): Promise<PerplexityResult> {
  ...
  try {
    // 429/5xx-aware retry (Bob 2026-08-03 — orchestrated multi-page reviews
    // hit provider rate limits and every 429 became a dead step with no
    // retry). Up to 2 retries; honors Retry-After when present, capped at
    // 20s per wait; exponential fallback otherwise. Other 4xx fail fast.
    const MAX_RETRIES = 2;
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (resp.ok || !retryable || attempt === MAX_RETRIES) break;
      // Retry-After: delta-seconds OR HTTP-date form; jittered exponential fallback
      // (jitter desynchronizes parallel steps retrying in lockstep).
      const rawRetryAfter = resp.headers.get("retry-after");
      let retryAfterMs = 0;
      if (rawRetryAfter) {
        const asNum = Number(rawRetryAfter);
        if (Number.isFinite(asNum) && asNum > 0) retryAfterMs = asNum * 1000;
        else {
          const asDate = Date.parse(rawRetryAfter);
          if (Number.isFinite(asDate)) retryAfterMs = asDate - Date.now();
        }
      }
      const fallbackMs = 1500 * Math.pow(2, attempt) * (0.75 + Math.random() * 0.5);
      const waitMs = Math.min(Math.max(retryAfterMs > 0 ? retryAfterMs : fallbackMs, 250), MAX_RETRY_WAIT_MS);
      console.warn(`[perplexity] ${resp.status} on attempt ${attempt + 1} — retrying in ${waitMs}ms`);
