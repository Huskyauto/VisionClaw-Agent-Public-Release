...
export async function processNudge(
  ...
  try {
    ...
    await assertMemoryValueNotTombstoned({
      tenantId,
      source: "knowledge_nudges",
      parts: [fact],
    });
    const { redactPiiForStorage } = await import("./storage-helpers/pii-redaction-guard");
    const redactedFact = redactPiiForStorage(fact).redacted;
    await db.transaction(async (tx) => {
      const nudgeResult = await tx.execute(sql`
        INSERT INTO knowledge_nudges
          (tenant_id, fact, category, source, score, conversation_id)
        VALUES (${tenantId}, ${redactedFact}, 'nudge', 'proactive',
          ${score}, ${conversationId || null})
        RETURNING id
      `);
      const nudgeId = Number((((nudgeResult as any).rows || nudgeResult) as any[])[0]?.id);
      if (!Number.isInteger(nudgeId) || nudgeId <= 0) {
        throw new Error("knowledge nudge insert returned no id");
      }
      const memoryResult = await tx.execute(sql`
        INSERT INTO memory_entries
          (tenant_id, fact, category, source, status, persona_id)
        VALUES (${tenantId}, ${redactedFact}, 'nudge', 'proactive-nudge',
          'active', NULL)
        RETURNING id
      `);
      const memoryId = Number((((memoryResult as any).rows || memoryResult) as any[])[0]?.id);
      if (!Number.isInteger(memoryId) || memoryId <= 0) {
        throw new Error("nudge memory insert returned no id");
      }
      // The mirrored memory row is created from this exact nudge row in the
      // same transaction. It has no independent source, so erasure follows
      // the edge when the nudge is removed.
      await recordMemoryDerivation(tx, {
        tenantId,
        parentSource: "knowledge_nudges",
        parentId: nudgeId,
        childSource: "memory_entries",
        childId: memoryId,
        relationship: "nudge_mirror",
        solelyDerived: true,
      });
    });
