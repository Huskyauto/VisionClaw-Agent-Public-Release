...
export interface LedgerRunOptions {
  ...
  /** Model to use for orchestrator-internal calls. Default gpt-5.4 —
   *  token audit 2026-07-30 (Bob decision): gpt-4.1-mini is NOT in the
   *  mapReplitToOpenAI $0-lane set, so the old default billed the metered
   *  OpenAI key (~36k calls / ~$80 in 30d). gpt-5.4 IS modelfarm-supported
   *  ($0 lane); gpt-5.5 is in REPLIT_MODELFARM_UNSUPPORTED and would either
   *  be substituted (metered off) or leak to the metered OpenAI key
   *  (ALLOW_METERED_LLM=true). Deliberately NOT gpt-5-mini: reasoning-heavy
   *  minis can consume the full token budget on internal "reasoning_tokens"
   *  and return empty content, silently killing the loop
   *  (see scripts/test-ledger.ts). */
  model?: string;
...
export async function runLedgerLoop(opts: LedgerRunOptions): Promise<LedgerRunResult> {
  // Non-reasoning model by design — gpt-5-mini eats the budget as reasoning
  // tokens and returns empty content in this loop (see option doc above).
