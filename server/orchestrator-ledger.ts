...
export interface LedgerRunOptions {
   ...
   *  DELIBERATE exception to the platform-wide gpt-5-mini default: reasoning
   *  models like gpt-5-mini/o-series can consume the full token budget on
   *  internal "reasoning_tokens" and return empty content, which silently
   *  kills the loop (see scripts/test-ledger.ts). */
  model?: string;
