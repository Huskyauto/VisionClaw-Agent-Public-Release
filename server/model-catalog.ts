 ...
 * list and returns ranked rows tagged open/closed. No key configured resolves
 * to [] (a verified "nothing to rank"). R125+143 review fix: API/timeout/shape
 * FAILURES now return null (degraded) instead of [] so callers can distinguish
 * "authoritative empty" from "fetch broke" and report the degradation instead
 * of silently skipping ranking-driven sync. AA data is used internally for
 * ranking only, never surfaced to end users.
 */
export async function fetchArtificialAnalysisRanked(timeoutMs = 20_000): Promise<RankedExternalModel[] | null> {
  ...
  try {
    ...
    // Shape contract: top-level array OR { data: [...] }. Anything else is a
    // malformed payload → null (degraded), never a fake "empty ranking".
    if (!Array.isArray(json) && !Array.isArray(json?.data)) {
      throw new Error("Artificial Analysis payload shape unrecognized (neither array nor {data:[]})");
    }
    ...
    console.warn(`[model-catalog] Artificial Analysis ranking fetch DEGRADED (${err?.message || err}) — returning null so callers skip auto-add loudly instead of treating it as an empty ranking`);
    return null;
  } finally {
    clearTimeout(t);
  }
...
export async function runCatalogSync(opts: RunCatalogSyncOptions = {}): Promise<CatalogSyncResult> {
  // R125+143 — no more silent `?? 1`: model-discovery notes are admin-facing, so
  // ADMIN (1) is the correct home, but a caller omitting tenantId must be loud so
  // future/malformed callers can't silently misfile writes.
  if (opts.tenantId == null) {
    console.warn(`[model-catalog] runCatalogSync called without tenantId (source=${opts.source || "unknown"}) — defaulting memory_entries writes to ADMIN tenant 1`);
  }
