...
const pool = new Pool({
  ...
  // pg-pool awaits this verifier before handing a new connection to a caller.
  // An async "connect" listener is insufficient because EventEmitter does not
  // await listener promises.
  verify: (client: any, callback: (error?: Error) => void) => {
    client.query(
      "SELECT set_config('app.memory_tombstone_hmac_key', $1, false)",
      [process.env.MEMORY_TOMBSTONE_HMAC_KEY ?? ""],
    ).then(
      () => callback(),
      (error: unknown) => callback(
        error instanceof Error ? error : new Error("database session initialization failed"),
      ),
    );
  },
} as any);
