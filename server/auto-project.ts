...
function queryRows(result: unknown): QueryRow[] {
  if (Array.isArray(result)) return result as QueryRow[];
  if (result && typeof result === "object" && "rows" in result) {
    const rows = (result as { rows?: unknown }).rows;
    if (Array.isArray(rows)) return rows as QueryRow[];
  }
  return [];
}
...
export async function checkAndAutoCreateProject(
  ...
  try {
    return await db.transaction(async (tx) => {
      await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext(${`auto-project:${tenantId}:${conversationId}`}))`);
      ...
      `);
      const convRows = queryRows(convRes);
      const conv = convRows?.[0];
      if (!conv) return null;

      if (conv.project_id) {
        const ownedRes = await tx.execute(sql`
          SELECT id FROM projects WHERE id = ${conv.project_id} AND tenant_id = ${tenantId} LIMIT 1
        `);
        const ownedRows = queryRows(ownedRes);
        return ownedRows?.[0]?.id
      ...
      `);
      const linkRows = queryRows(linkRes);
      if (linkRows?.[0]?.project_id) {
        return { created: false, projectId: Number(linkRows[0].project_id) };
      }

      const msgRes = await tx.execute(sql`
        SELECT role, LEFT(content, 300) as content FROM messages
        WHERE conversation_id = ${conversationId}
        ORDER BY id ASC LIMIT 20
      `);
      const msgRows = queryRows(msgRes);
      const messageCount = msgRows.length;
      const userMessages = msgRows
        .filter((message) => message.role === "user")
        .map((message) => typeof message.content === "string" ? message.content : "");
      const aiMessages = msgRows
      ...
      const txResult = await tx.execute(sql`
        ...
        link_conv AS (
          UPDATE conversations SET project_id = (SELECT id FROM new_project) WHERE id = ${conversationId} AND tenant_id = ${tenantId}
        ),
        link_project AS (
      ...
      `);
      const txRows = queryRows(txResult);
      const rawProjectId = txRows?.[0]?.id;
...
5. Continue helping with their current request — don't interrupt the flow.`;
    ...
    console.error(`[auto-project] Error:`, err instanceof Error ? err.message : err);
    return null;
  }
}
