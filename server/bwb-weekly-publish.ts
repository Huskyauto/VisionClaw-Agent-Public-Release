...
> {
  ...
  try {
    const { recordBwbOversightShadow } = await import("./lib/bwb-oversight-shadow");
    await recordBwbOversightShadow({
      tenantId: params.tenantId,
      jobId: typeof (row.context as Partial<WeeklyPublishContext>)?.jobId === "string"
        ? (row.context as Partial<WeeklyPublishContext>).jobId!
        : undefined,
      event: "approval_decided",
      metadata: {
        approvalId: row.id,
        approved: params.approved,
        reviewLatencyMs: row.requestedAt
          ? Math.max(0, Date.now() - new Date(row.requestedAt).getTime())
          : undefined,
      },
    });
  } catch (error) {
    console.warn("[bwb-weekly-publish] oversight shadow skipped after durable decision:", error);
  }
