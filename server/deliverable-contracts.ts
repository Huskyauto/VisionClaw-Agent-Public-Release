...
export const DELIVERABLE_PIPELINES: Record<DeliverableFormat, DeliverablePipeline> = {
  video: {
    ...
    steps: [
      ...
      { tool: "grade_deliverable", required: false, purpose: "W3 vision grader — sample frames, audio LUFS, no >2s black, no meta-narration. Bounded auto-revise loop (revise + re-grade while the score improves, up to max_attempts) if <85.", inputsHint: "{deliverable_type:'video', file_path, content_excerpt:<full narration script text>}", wave: 4, dependsOn: [2] },
  ...
  audio: {
    ...
    steps: [
      ...
      { tool: "grade_deliverable", required: false, purpose: "W3 audio grader — LUFS in -23..-16, no end-cut, transcript matches script ≥95%.", inputsHint: "{deliverable_type:'audio', file_path, expected_spec:{transcript:<script>}, content_excerpt:<script>}", wave: 3, dependsOn: [1] },
  ...
  pdf: {
    ...
    steps: [
      ...
      { tool: "grade_deliverable", required: false, purpose: "W3 PDF grader — first/middle/last page render, no broken layout.", inputsHint: "{deliverable_type:'pdf', file_path, expected_spec:{min_pages, max_pages}, content_excerpt:<title + section outline/copy>}", wave: 3, dependsOn: [1] },
  ...
  slides: {
    ...
    steps: [
      ...
      { tool: "grade_deliverable", required: false, purpose: "W3 slides grader — vision LLM checks photo presence on required slides, on-brand colors, narration-matches-slide-content. Bounded auto-revise loop (revise + re-grade while the score improves, up to max_attempts) if <85.", inputsHint: "{deliverable_type:'slides', file_path, expected_spec:{slide_count, requires_photo_on:[...]}, content_excerpt:<slide titles + contents>}", wave: 3, dependsOn: [2] },
  ...
  html_app: {
    ...
    steps: [
      ...
      { tool: "grade_deliverable", required: false, purpose: "W3 HTML grader — re-run jsdom + per-app-type smoke assertion.", inputsHint: "{deliverable_type:'html_page', file_path, expected_spec:{smoke_assertion}, content_excerpt:<app topic + features description>}", wave: 3, dependsOn: [1] },
      { tool: "deliver_product", required: true, purpose: "Drive upload + branded email.", inputsHint: "{file_path, customer_email, product_name}", wave: 3, dependsOn: [1] },
...
Output ONLY JSON conforming to the schema.`;
  ...
  let next_step_instruction = firstStep
  ...
  // R125+137.81 — GOLD EXEMPLAR INJECTION (fail-open, returns "" when none/any
  // error). Appends this tenant's top-scoring prior productions of the same
  // format as few-shot exemplars so the producer writes against a proven-great
  // example instead of prose-only guidance.
  try {
    const { getExemplarGuidance } = await import("./lib/deliverable-exemplars");
    next_step_instruction += await getExemplarGuidance(opts?.tenantId, format);
  } catch (exemplarErr: any) {
    console.error(`[deliverable-contracts] exemplar injection failed (fail-open): ${exemplarErr?.message || String(exemplarErr)}`);
  }
...
Output ONLY JSON conforming to the schema.`;
  ...
  // Tighten the completion gate for dynamically-composed plans. Left alone a
  // custom plan can declare "done" on the workers' self-report: there is no
  // per-format rubric and the LLM-authored step list may omit any grading step.
  // Guarantee the plan ENDS with a REQUIRED grade_deliverable step (when that
  // tool exists in the live catalog) so the independent completion evaluator —
  // which grade_deliverable runs for non-rubric deliverables, judging the result
  // against these acceptanceNotes — gates the finish. Fail-OPEN: if the catalog
  // somehow lacks grade_deliverable we leave the plan as-is rather than invent a
  // tool.
  if (validNames.has("grade_deliverable") && !validated.some((s) => s.tool === "grade_deliverable")) {
    const idx = validated.length;
    validated.push({
      tool: "grade_deliverable",
      required: true,
      purpose: "Independent completion gate — confirm the result actually satisfies the acceptance criteria before declaring done.",
      inputsHint: `Pass deliverable_type (best fit), the produced file_path/file_url, acceptance_notes=${JSON.stringify(acceptanceNotes.slice(0, 160))}, and request=<the original user request>. For custom work an independent judge uses acceptance_notes as the contract — a trivial pass is no longer accepted.`,
      wave: idx + 1,
