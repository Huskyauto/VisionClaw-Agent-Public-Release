...
export async function setExplicitCommunicationMode(
  tenantId: number,
  mode: CommunicationMode,
): Promise<void> {
  if (!Number.isSafeInteger(tenantId) || tenantId <= 0) {
    throw new Error("A valid tenant is required to update communication mode");
  }
  if (mode !== "action_first" && mode !== "standard") {
    throw new Error("Invalid communication mode");
  }
  const patch = JSON.stringify({ communicationMode: mode });
  await db.execute(sql`
    INSERT INTO user_profiles (tenant_id, preferences)
    VALUES (${tenantId}, ${patch}::jsonb)
    ON CONFLICT (tenant_id) DO UPDATE SET
      preferences = COALESCE(user_profiles.preferences, '{}'::jsonb) || ${patch}::jsonb,
      updated_at = NOW()
  `);
}
...
Never infer medical, neurological, or mental-health conditions. Do not infer or change reserved explicit settings such as communicationMode.
...
Never infer medical, neurological, or mental-health conditions. Never emit or change the reserved communicationMode preference.`,
...
Decisions: ${profile.decisionPatterns || "not yet modeled"}`,
    ...
    try {
      ...
      const synthesizedPreferences =
        parsed.preferences && typeof parsed.preferences === "object" && !Array.isArray(parsed.preferences)
          ? parsed.preferences
          : {};
      const preservedPreferences = preserveExplicitCommunicationMode(
        profile.preferences,
        synthesizedPreferences,
      );
      const preferencesPatch = JSON.stringify(preservedPreferences);
      ...
      await db.execute(sql`
        UPDATE user_profiles SET
          ...
          preferences =
            (COALESCE(user_profiles.preferences, '{}'::jsonb) || ${preferencesPatch}::jsonb)
            || CASE
              WHEN user_profiles.preferences->>'communicationMode' IN ('action_first', 'standard')
                THEN jsonb_build_object('communicationMode', user_profiles.preferences->>'communicationMode')
              ELSE '{}'::jsonb
            END,
