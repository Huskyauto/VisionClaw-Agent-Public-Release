...
export const SECOND_OPINION_FALLBACK_MODEL_ID = "z-ai/glm-5.2";
...
const DEFAULT_TIMEOUT_MS = (() => {
  ...
  // Trimmed 90s → 60s: Fusion almost never lands a USEFUL answer between 60s and
  // 90s, so the extra 30s was pure dead-wait before the fail-open kicks in.
...
export type SecondOpinionFailureKind =
  | "fusion_daily_usd"
  | "cost_drift_latched"
  | "model_timeout"
  | "provider_overload"
...
export interface SecondOpinionFallbackDecision {
  modelId: string;
  reason: "fusion-pre-dispatch-failure";
}

export type SecondOpinionMode = "fusion" | "single_model_fallback";

export function buildExplicitFallbackOffer(mode: SecondOpinionMode = "fusion"): {
  explicitFallbackAvailable: boolean;
  fallbackModel?: string;
  fallbackRequiresExplicitAuthorization?: true;
} {
  return mode === "single_model_fallback"
    ? { explicitFallbackAvailable: false }
    : {
        explicitFallbackAvailable: true,
        fallbackModel: SECOND_OPINION_FALLBACK_MODEL_ID,
        fallbackRequiresExplicitAuthorization: true,
      };
}

export function resolveSecondOpinionRoute(mode: SecondOpinionMode = "fusion"): {
  modelId: string;
  routeReason: "fusion-direct" | "explicit-single-model-fallback";
  label: "Fusion" | "GLM 5.2 fallback";
} {
  return mode === "single_model_fallback"
    ? {
        modelId: SECOND_OPINION_FALLBACK_MODEL_ID,
        routeReason: "explicit-single-model-fallback",
        label: "GLM 5.2 fallback",
      }
    : {
        modelId: FUSION_MODEL_ID,
        routeReason: "fusion-direct",
        label: "Fusion",
      };
}

/**
 * Select the one bounded single-model fallback. Never fall back after ambiguous
 * completion or when doing so would bypass a budget/latch decision.
 */
export function selectSecondOpinionFallback(input: {
  failureKind: SecondOpinionFailureKind;
  providerWorkMayHaveStarted: boolean;
}): SecondOpinionFallbackDecision | null {
  if (input.providerWorkMayHaveStarted) return null;
  if (input.failureKind !== "wiring") return null;
  return {
    modelId: SECOND_OPINION_FALLBACK_MODEL_ID,
    reason: "fusion-pre-dispatch-failure",
  };
...
export interface SecondOpinionResult {
  ...
  routeReason?: string;
  failureKind?: SecondOpinionFailureKind;
  /** A separately metered single-model check is available, but was not silently started. */
  explicitFallbackAvailable?: boolean;
  fallbackModel?: string;
  fallbackRequiresExplicitAuthorization?: true;
...
export interface GetSecondOpinionOpts {
  ...
  /** Explicitly choose the server-pinned single-model fallback instead of Fusion. */
...
async function defaultReserve(args: {
  ...
  return db.transaction(async (tx) => {
    ...
    const ins: any = await tx.execute(sql`
      ...
      VALUES (${args.tenantId}, ${args.toolName}, ${args.modelId}, ${effectiveEstimate.toFixed(6)}, 0, 0, ${`${args.operation}:reserved`}, now())
      RETURNING id
    `);
...
export async function reserveFusionBudget(
  ...
  modelId: string = FUSION_MODEL_ID,
  ...
  try {
    const res = await reserve({ tenantId, estimateUsd, capUsd, toolName, operation, modelId });
    if (!res.ok) {
      return { ok: false, spentUsd: res.spentUsd, capUsd, remainingUsd: Math.max(0, capUsd - res.spentUsd), degraded: false };
    }
    if (!Number.isInteger(res.reservationId) || Number(res.reservationId) <= 0) {
      console.error(
        `[second-opinion] budget reserve reported success without a valid ledger row id (tenant=${tenantId} op=${operation}) — failing closed`,
      );
      return {
        ok: false,
        spentUsd: res.spentUsd,
        capUsd,
        remainingUsd: Math.max(0, capUsd - res.spentUsd),
        degraded: true,
      };
    }
    return {
...
export function settlementUpdatedExactlyOne(result: any): boolean {
  const rows = Array.isArray(result?.rows) ? result.rows : Array.isArray(result) ? result : [];
  const rowCount = Number(result?.rowCount ?? rows.length);
  return rowCount === 1 && rows.length === 1;
}

/** Settle a reservation row in place to the REAL spend and report reconciliation state. */
async function settleFusionReservation(
  ...
  try {
    const updated: any = await db.execute(sql`
      ...
      SET cost_usd = ${costUsd.toFixed(6)}, tokens_in = ${tokensIn}, tokens_out = ${tokensOut}, operation = ${operation}, model = ${modelId}
      WHERE id = ${reservationId}
      RETURNING id
    `);
    if (!settlementUpdatedExactlyOne(updated)) {
      console.error(
        `[second-opinion] reservation settlement updated ${Number(updated?.rowCount ?? updated?.rows?.length ?? 0)} rows; expected exactly 1 (reservation=${reservationId} op=${operation})`,
      );
      return false;
    }
    return true;
    ...
    logSilentCatch("server/second-opinion.ts", err);
    console.error("[second-opinion] Fusion reservation settlement requires reconciliation:", err);
    return false;
  }
}
...
export async function getSecondOpinion(opts: GetSecondOpinionOpts): Promise<SecondOpinionResult> {
  ...
  const route = resolveSecondOpinionRoute(opts.mode);
  if (question.length < 10) {
    return { ok: false, skipped: "invalid", failureKind: "invalid_request", error: "question must be ≥10 chars" };
  }
  ...
  try {
    const resolved = await getClientForModel(route.modelId, opts.tenantId, { costExemptLane: true });
    client = resolved.client;
    actualModelId = resolved.actualModelId;
  } catch (err) {
    logSilentCatch("server/second-opinion.ts", err);
    const decision =
      opts.mode !== "single_model_fallback"
        ? selectSecondOpinionFallback({ failureKind: "wiring", providerWorkMayHaveStarted: false })
        : null;
    return {
      ok: false,
      skipped: "wiring",
      ...
      error: `${route.label} client unavailable: ${(err as Error)?.message || err}`,
      ...(decision ? buildExplicitFallbackOffer(opts.mode) : { explicitFallbackAvailable: false }),
    };
  }

  const toolName = opts.auto ? "second_opinion_auto" : "second_opinion";
  const operationBase =
    opts.mode === "single_model_fallback" ? "second_opinion:fallback" : "second_opinion";
  const operation = opts.auto
    ? `second_opinion_auto:${opts.reason || "low-confidence"}`
    : operationBase;
  ...
  const budget = await reserveFusionBudget(
    opts.tenantId,
  ...
  try {
    ...
    if (raced === TIMED_OUT) {
      console.warn(`[second-opinion] ${route.label} call timed out after ${timeoutMs}ms (tenant=${opts.tenantId}${opts.auto ? " AUTO" : ""}) — failing open`);
      return {
        ok: false,
        skipped: "error",
        ...
        providerLane: "openrouter",
        routeReason: route.routeReason,
        error: `${route.label} call timed out after ${timeoutMs}ms`,
        model: actualModelId,
        latencyMs: Date.now() - t0,
        ...buildExplicitFallbackOffer(opts.mode),
      };
    }
    ...
    const settlementRecorded = await settleFusionReservation(
      budget.reservationId,
      realCost ?? FUSION_CALL_ESTIMATE_USD,
      tokensIn,
      tokensOut,
      operation,
      actualModelId,
    ...
    if (!settlementRecorded) {
      return {
        ...
        providerLane: "openrouter",
        routeReason: route.routeReason,
        error: `${route.label} answered, but its paid reservation could not be reconciled; result withheld until ledger integrity is restored`,
        model: actualModelId,
        costUsd: realCost,
        latencyMs,
        budget: {
    ...
    if (!answer || answer.trim().length === 0) {
      return { ok: false, skipped: "error", failureKind: "provider_error", requestedModel: route.modelId, actualModel: actualModelId, providerLane: "openrouter", routeReason: route.routeReason, error: `${route.label} returned an empty answer`, model: actualModelId, costUsd: realCost, latencyMs };
    }
    ...
    return {
      ...
      budget: {
        ...
        capUsd: budget.capUsd,
        remainingUsd: budget.remainingUsd,
        degraded: budget.degraded || !settlementRecorded,
      },
    };
  } catch (err) {
    if (timer) clearTimeout(timer);
    ...
    // Once create() has been invoked, any provider error is ambiguous completion:
    // paid work may already have occurred even if no response reached us. Keep the
    // reservation conservatively rather than reopening the daily cap.
    return {
      ok: false,
      skipped: "error",
      failureKind: aborted ? "model_timeout" : Number((err as any)?.status) === 529 ? "provider_overload" : "provider_error",
      requestedModel: route.modelId,
      actualModel: actualModelId,
      providerLane: "openrouter",
      routeReason: route.routeReason,
      error: aborted ? `${route.label} call timed out after ${timeoutMs}ms` : `${route.label} call failed: ${(err as Error)?.message || err}`,
      model: actualModelId,
      latencyMs: Date.now() - t0,
      ...buildExplicitFallbackOffer(opts.mode),
    };
