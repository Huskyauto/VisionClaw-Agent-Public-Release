...
export async function getSecondOpinion(opts: GetSecondOpinionOpts): Promise<SecondOpinionResult> {
  ...
  try {
    ...
    logSilentCatch("server/second-opinion.ts", err);
    return { ok: false, skipped: "wiring", failureKind: "wiring", requestedModel: FUSION_MODEL_ID, providerLane: "openrouter", routeReason: "fusion-direct", error: `Fusion client unavailable: ${(err as Error)?.message || err}` };
  }
  ...
  if (!budget.ok) {
    ...
    return {
      ...
      requestedModel: FUSION_MODEL_ID,
  ...
  try {
    ...
    if (raced === TIMED_OUT) {
      ...
      return { ok: false, skipped: "error", failureKind: "model_timeout", requestedModel: FUSION_MODEL_ID, actualModel: actualModelId, providerLane: "openrouter", routeReason: "fusion-direct", error: `Fusion call timed out after ${timeoutMs}ms`, model: actualModelId, latencyMs: Date.now() - t0 };
    }
    ...
    const settlementRecorded = await settleFusionReservation(budget.reservationId, realCost ?? FUSION_CALL_ESTIMATE_USD, tokensIn, tokensOut, operation);
    ...
    if (!answer || answer.trim().length === 0) {
      return { ok: false, skipped: "error", failureKind: "provider_error", requestedModel: FUSION_MODEL_ID, actualModel: actualModelId, providerLane: "openrouter", routeReason: "fusion-direct", error: "Fusion returned an empty answer", model: actualModelId, costUsd: realCost, latencyMs };
    }
