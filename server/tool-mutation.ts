...
/** Simulation-sandbox firewall support (S1): TRUE only when a tool is in the
 * explicit READ_ONLY_TOOLS allowlist. Unlike classifyToolRisk (which defaults
 * unknown tools to "read_only" — fail-open), this is a fail-closed lookup:
 * unknown/unregistered names return false. */
export function isExplicitlyReadOnly(toolName: string): boolean {
...
export function requestToolConfirmation(
  ...
  const armHITL = () => {
    ...
    // Architect MEDIUM fix — the auto-deny timer used to be armed with the
    // 120s default BEFORE the async WhatsApp config resolved, so connected
    // WhatsApp confirmations auto-denied at 2 min instead of the configured
    // window. Now the timer is armed AFTER the timeout is resolved (still
    // fail-closed: any failure in the async path arms the 120s default).
    (async () => {
...
export function recordMutation(entry: MutationAuditEntry): void {
  if (!Number.isInteger(entry.tenantId) || entry.tenantId <= 0) {
    throw new Error("recordMutation: positive tenantId is required");
  }
...
export function getRecentMutations(tenantId: number, limit: number = 20): MutationAuditEntry[] {
  return recentMutations.filter(entry => entry.tenantId === tenantId).slice(-limit);
}

export function getMutationStats(tenantId: number): {
  total: number;
  ...
  const tenantMutations = recentMutations.filter(entry => entry.tenantId === tenantId);
  for (const entry of tenantMutations) {
    byTool[entry.toolName] = (byTool[entry.toolName] || 0) + 1;
    byRisk[entry.riskLevel]++;
  }

  return { total: tenantMutations.length, byTool, byRisk };
}
