...
export async function decidePlan(args: {
  ...
  /** Tenant scope for every plan query in this decision. When provided (all
   *  tenant-facing routes), the CAS/probe/child lookups are constrained to
   *  this tenant so a cross-tenant planId can never be read or flipped.
   *  Internal system callers (internal-resolver) may omit it — they operate
   *  on rows they already own via server-generated ids. */
  tenantId?: number;
  ...
  const swap: any = await db.execute(sql`
    ...
    WHERE id = ${args.planId} AND status = 'awaiting_approval'
      ${args.tenantId != null ? sql`AND tenant_id = ${args.tenantId}` : sql``}
  ...
  if (!row) {
    ...
    const probe: any = await db.execute(sql`SELECT status, ceo_decision FROM plans WHERE id = ${args.planId} ${args.tenantId != null ? sql`AND tenant_id = ${args.tenantId}` : sql``}`);
    ...
    if (args.decision === "revise" && currentStatus === "revising") {
      const child: any = await db.execute(sql`
        SELECT id FROM plans WHERE parent_plan_id = ${args.planId} ${args.tenantId != null ? sql`AND tenant_id = ${args.tenantId}` : sql``} ORDER BY id DESC LIMIT 1
      `);
