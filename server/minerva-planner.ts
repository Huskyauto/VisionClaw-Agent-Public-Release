...
import { recoveryHash, canonicalRecoveryStep, recoveryApprovalActionHash, deriveStaleRecoveryBinding, canonicalRecoveryEvidence } from "./lib/plan-step-resilience";
...
export function validatePlanApprovalReadiness(
  ...
  if (repairAction) {
    try {
        if (repairAction.kind === "stale_plan_recovery") {
          if (!Number.isInteger(repairAction.originalPlanId) || !repairAction.incidentKey) {
            throw new Error("malformed stale recovery action");
          }
        } else if (repairAction.kind === "source_repair_handoff") validateSourceRepairAction(repairAction);
      else validateHeartbeatRepairAction(repairAction);
    } catch {
      return { ok: false, reason: STALE_OPERATIONAL_PLAN_REASON };
    }
      const expectedTool = repairAction.kind === "stale_plan_recovery"
        ? null : repairAction.kind === "source_repair_handoff" ? "source_repair_handoff" : "heartbeat_repair";
      if (!expectedTool) return { ok: true };
...
export async function decidePlan(args: {
  ...
  const staleProbe: any = await db.execute(sql`
    SELECT source FROM plans WHERE id = ${args.planId} AND tenant_id = ${args.tenantId}
    LIMIT 1
  `);
  if ((staleProbe.rows ?? staleProbe)[0]?.source === "stale-plan-recovery") {
    return decideStaleRecoveryPlanCore(args);
  }
  ...
  if (args.decision === "approve") {
    ...
    if (candidate) {
      ...
      if (candidate.plan_json?.repairAction) {
        ...
        if (candidate.plan_json.repairAction.kind === "stale_plan_recovery") {
          const action = candidate.plan_json.repairAction;
          if (!Number.isInteger(action.originalPlanId) || !action.incidentKey ||
              !/^[a-f0-9]{64}$/.test(String(action.originalFingerprint)) ||
              !/^[a-f0-9]{64}$/.test(String(action.stepHash)) ||
              !/^[a-f0-9]{64}$/.test(String(action.executionEvidenceHash))) {
            throw new Error("stale recovery action is malformed");
          }
          const parent: any = await db.execute(sql`
            SELECT version, plan_json, execution_log, tenant_id FROM plans
            WHERE id = ${action.originalPlanId} AND tenant_id = ${args.tenantId} LIMIT 1
          `);
          const p = (parent.rows ?? parent)[0];
          const childSteps = Array.isArray(candidate.plan_json?.steps) ? candidate.plan_json.steps : [];
          const parentSteps = Array.isArray(p?.plan_json?.steps) ? p.plan_json.steps : [];
          const start = Number(action.firstUnresolvedStepIndex);
          const suffixIds = new Set(parentSteps.slice(start).map((step: any) => Number(step?.n)));
          const expectedSuffixHash = recoveryHash(parentSteps.slice(start).map((step: any) => canonicalRecoveryStep({
            ...step,
            depends_on: Array.isArray(step?.depends_on)
              ? step.depends_on.filter((dependency: any) => suffixIds.has(Number(dependency))) : [],
          })));
          if (!p || Number(p.version) !== Number(action.originalVersion) ||
              recoveryHash(p.plan_json) !== action.originalFingerprint ||
              recoveryHash(canonicalRecoveryEvidence(p.execution_log).slice(-12000)) !== action.executionEvidenceHash ||
              expectedSuffixHash !== action.suffixHash ||
              recoveryHash(childSteps.map(canonicalRecoveryStep)) !== action.suffixHash ||
              recoveryApprovalActionHash(action) !== action.actionHash) {
            throw new Error("stale recovery binding drifted; approval refused");
          }
          approvalBinding = {
            type: "repair.approval_binding", at: new Date().toISOString(),
            stepId: action.firstUnresolvedStepId, version: action.originalVersion,
            kind: action.kind, actionHash: recoveryApprovalActionHash(action), actor: args.actor,
          };
        }
        if (candidate.plan_json.repairAction.kind !== "stale_plan_recovery") {
  ...
  if (args.decision === "reject") {
    if (row.plan_json?.repairAction?.kind === "stale_plan_recovery") {
      const action = row.plan_json.repairAction;
      await db.transaction(async (tx) => {
        await tx.execute(sql`
          UPDATE plans SET status = 'rejected', updated_at = now(),
            execution_log = COALESCE(execution_log, '[]'::jsonb) ||
              ${JSON.stringify([{ type: "execution.recovery_rejected", at: new Date().toISOString(),
                recoveryChildId: args.planId, incidentKey: action.incidentKey,
                reason: args.reason }])}::jsonb
          WHERE id = ${Number(action.originalPlanId)} AND tenant_id = ${args.tenantId}
            AND status = 'failed'
        `);
      });
    }
...
export async function decideStaleRecoveryPlanCore(args: {
  planId: number; decision: "approve" | "reject" | "revise"; reason: string; actor: string; tenantId: number;
  kick?: (planId: number) => void;
}): Promise<{ ok: true; status: string }> {
  if (args.decision === "revise") throw new Error("stale recovery actions cannot be revised; reject and create a new bound review");
  let outcome = "";
  let tenantId = args.tenantId;
  await db.transaction(async (tx) => {
    const locked: any = await tx.execute(sql`
      SELECT id, tenant_id, status, parent_plan_id, source, source_ref, plan_json, execution_log
      FROM plans WHERE id = ${args.planId} AND tenant_id = ${args.tenantId}
        AND source = 'stale-plan-recovery' FOR UPDATE
    `);
    const child = (locked.rows ?? locked)[0];
    if (!child) throw new Error(`Plan ${args.planId} not found`);
    const action = child.plan_json?.repairAction;
    const parentId = Number(action?.originalPlanId);
    const parentLock: any = await tx.execute(sql`
      SELECT id, tenant_id, status, version, plan_json, execution_log
      FROM plans WHERE id = ${parentId} AND tenant_id = ${args.tenantId} FOR UPDATE
    `);
    const parent = (parentLock.rows ?? parentLock)[0];
    const binding = parent ? deriveStaleRecoveryBinding({
      tenantId: args.tenantId, originalPlanId: parentId, version: Number(parent.version),
      planJson: parent.plan_json, executionLog: parent.execution_log,
    }) : null;
    const expectedAction = binding ? {
      version: 1, kind: "stale_plan_recovery", originalPlanId: parentId, parentPlanId: parentId,
      tenantId: args.tenantId, originalVersion: binding.originalVersion,
      originalFingerprint: binding.originalFingerprint, incidentKey: binding.incidentKey,
      firstUnresolvedStepIndex: binding.firstUnresolvedStepIndex, firstUnresolvedStepId: binding.firstUnresolvedStepId,
      stepHash: binding.stepHash, suffixHash: binding.suffixHash, executionEvidenceHash: binding.executionEvidenceHash,
       recoveryStepContract: parent.plan_json?.repairAction,
    } : null;
    const expectedHash = expectedAction ? recoveryApprovalActionHash(expectedAction) : "";
    if (!parent || !binding || child.parent_plan_id !== parentId ||
        child.source_ref !== binding.incidentKey ||
        recoveryApprovalActionHash(action) !== expectedHash ||
        action.actionHash !== expectedHash || Number(action.tenantId) !== args.tenantId ||
        child.status !== "awaiting_approval") {
      throw new Error("stale recovery binding drifted; decision refused");
    }
    tenantId = Number(child.tenant_id);
    if (args.decision === "reject") {
      await tx.execute(sql`
        UPDATE plans SET status='rejected', ceo_decision='rejected',
          ceo_decision_reason=${`[actor=${args.actor}] ${args.reason}`}, ceo_decided_at=now(),
          execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([{type:"execution.recovery_rejected",at:new Date().toISOString(),recoveryChildId:args.planId,incidentKey:binding.incidentKey,reason:args.reason}])}::jsonb
        WHERE id=${args.planId} AND tenant_id=${args.tenantId} AND status='awaiting_approval'
      `);
      await tx.execute(sql`
        UPDATE plans SET status='rejected', updated_at=now(),
          execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([{type:"execution.recovery_rejected",at:new Date().toISOString(),recoveryChildId:args.planId,incidentKey:binding.incidentKey,reason:args.reason}])}::jsonb
        WHERE id=${parentId} AND tenant_id=${args.tenantId} AND status='failed'
      `);
      outcome = "rejected";
      return;
    }
    await tx.execute(sql`
      UPDATE plans SET status='approved', ceo_decision='approved',
        ceo_decision_reason=${`[actor=${args.actor}] ${args.reason}`}, ceo_decided_at=now(),
        execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([{type:"repair.approval_binding",at:new Date().toISOString(),stepId:action.firstUnresolvedStepId,version:action.originalVersion,kind:action.kind,actionHash:expectedHash,actor:args.actor}])}::jsonb
      WHERE id=${args.planId} AND tenant_id=${args.tenantId} AND parent_plan_id=${parentId}
        AND source_ref=${binding.incidentKey} AND status='awaiting_approval'
    `);
    outcome = "approved";
  });
  if (outcome === "approved") {
    if (args.kick) args.kick(args.planId);
    else if (executorKickOverride) executorKickOverride(args.planId);
    else setImmediate(() => import("./plan-executor").then(({ executePlan }) => executePlan(args.planId)).catch(() => {}));
  }
...
async function expireStalePlans(tenantId: number): Promise<number> {
  try {
    const recoveryExpiry: any = await db.execute(sql`
      SELECT id, parent_plan_id FROM plans
      WHERE tenant_id = ${tenantId} AND source = 'stale-plan-recovery'
        AND status = 'awaiting_approval'
    ...
    const r: any = await db.execute(sql`
      ...
      WHERE tenant_id = ${tenantId} AND status = 'awaiting_approval'
        AND source <> 'stale-plan-recovery'
        AND created_at < now() - make_interval(days => ${PLAN_APPROVAL_TTL_DAYS})
    `);
    for (const child of (recoveryExpiry.rows ?? recoveryExpiry)) {
      await db.transaction(async (tx) => {
        const locked: any = await tx.execute(sql`
          SELECT id, parent_plan_id FROM plans
          WHERE id = ${Number(child.id)} AND tenant_id = ${tenantId}
            AND status = 'awaiting_approval' FOR UPDATE
        `);
        const lockedRow = (locked.rows ?? locked)[0];
        if (!lockedRow) return; // approval won the CAS
        const parentLock: any = await tx.execute(sql`
          SELECT id, status FROM plans
          WHERE id = ${Number(lockedRow.parent_plan_id)} AND tenant_id = ${tenantId}
          FOR UPDATE
        `);
        if (!(parentLock.rows ?? parentLock)[0]) return;
        await tx.execute(sql`
          UPDATE plans SET status = 'expired', updated_at = now()
          WHERE id = ${Number(child.id)} AND tenant_id = ${tenantId}
            AND status = 'awaiting_approval'
        ...
        await tx.execute(sql`
          UPDATE plans SET status = 'failed', updated_at = now(),
            execution_log = COALESCE(execution_log, '[]'::jsonb) ||
              ${JSON.stringify([{ type: "execution.recovery_blocker", at: new Date().toISOString(),
                recoveryChildId: Number(child.id), actionable: true,
                reason: "Recovery approval expired; create a new bound recovery review." }])}::jsonb
          WHERE id = ${Number(lockedRow.parent_plan_id)} AND tenant_id = ${tenantId}
            AND status = 'failed'
...
/** Narrow seam used by the queue and live race tests. Recovery expiry is
 * transactionally handled by expireStalePlans; this keeps the recovery API
 * explicit without exposing generic plan expiry. */
export async function expireStaleRecoveryPlanCore(tenantId: number): Promise<number> {
  return expireStalePlans(tenantId);
}
  ...
  const r: any = args.activity
    ? await db.execute(sql`
        ...
        WHERE tenant_id = ${tenantId}
         ...
         ORDER BY
             CASE
               WHEN status IN ('publish_required', 'handoff_pending', 'executing', 'approved') THEN 0
               WHEN status = 'failed' AND NOT EXISTS (
                 SELECT 1 FROM jsonb_array_elements(COALESCE(plans.execution_log, '[]'::jsonb)) AS activity_event
                 WHERE activity_event->>'type' = 'execution.recovery_blocked'
               ) THEN 1
               WHEN status = 'completed' AND NOT EXISTS (
                 SELECT 1 FROM jsonb_array_elements(COALESCE(plans.execution_log, '[]'::jsonb)) AS completed_event
                 WHERE completed_event->>'type' = 'execution.recovery_blocked'
               ) THEN 2
               ELSE 3
             END,
    ...
    : await db.execute(sql`
      ...
      `);
  const rows = r.rows ?? r;
  const ids = rows.map((row: any) => Number(row.id)).filter((id: number) => id > 0);
  const recoveryStatuses: any = ids.length ? await db.execute(sql`
    SELECT parent_plan_id, status FROM plans
    WHERE tenant_id = ${tenantId} AND source = 'stale-plan-recovery'
      AND parent_plan_id IN (${sql.join(ids.map((id: number) => sql`${id}`), sql`, `)})
    ORDER BY id DESC
  `) : { rows: [] };
  const childByParent = new Map<number, string>();
  for (const child of (recoveryStatuses.rows ?? recoveryStatuses)) {
    if (!childByParent.has(Number(child.parent_plan_id))) childByParent.set(Number(child.parent_plan_id), String(child.status));
  }
  return rows.map((row: any) => ({ ...row, recovery_child_status: childByParent.get(Number(row.id)) ?? null, ...derivePlanLifecycle({ ...row, recovery_child_status: childByParent.get(Number(row.id)) ?? null }) }));
}
...
export function derivePlanLifecycle(row: {
  ...
  recovery_child_status?: string | null;
}): {
  lifecycle: "awaiting_approval" | "approved" | "executing" | "recovery_approval_pending" | "recovery_executing" | "blocked" | "handoff_pending" | "publish_required" | "verified" | "archived";
  blockerReason: string | null;
  blockerClass?: "stale_recovery_refusal";
  nextAction: string;
  verified: boolean;
  ...
  const blockerReason = blockerEvent
    ...
    : null;
  const blockerClass = events.some((event: any) => event?.type === "execution.recovery_blocked")
    ? "stale_recovery_refusal" as const : undefined;
  ...
  const recoveryCompleted = events.some((event: any) => event?.type === "execution.recovery.completed");
  const recoveryRejected = events.some((event: any) => event?.type === "execution.recovery_rejected");
  const recoveryExpired = events.some((event: any) =>
    event?.type === "execution.recovery_blocker" &&
    String(event.reason || "").toLowerCase().includes("approval expired"),
  );
  let lifecycle: "awaiting_approval" | "approved" | "executing" | "recovery_approval_pending" | "recovery_executing" | "blocked" | "handoff_pending" | "publish_required" | "verified" | "archived";
  ...
  else if (recoveryCompleted || row.recovery_child_status === "completed") lifecycle = "verified";
  else if (row.recovery_child_status === "awaiting_approval") lifecycle = "recovery_approval_pending";
  else if (row.recovery_child_status === "approved" || row.recovery_child_status === "executing") lifecycle = "recovery_executing";
  else if (recoveryRejected || recoveryExpired || row.recovery_child_status === "rejected" || row.recovery_child_status === "expired") lifecycle = "blocked";
  ...
  const nextAction = lifecycle === "recovery_approval_pending" ? "Approve the exact recovery suffix on the child plan"
    : lifecycle === "recovery_executing" ? "Bound recovery suffix is executing"
    : blockerClass === "stale_recovery_refusal"
    ? "Recorded as a safety stop; review in plan history if needed."
    : lifecycle === "awaiting_approval" ? "Review and approve when ready"
  ...
  return {
    ...
    actionable: lifecycle !== "archived" && lifecycle !== "handoff_pending" && lifecycle !== "recovery_approval_pending" && blockerClass !== "stale_recovery_refusal",
  };
}
