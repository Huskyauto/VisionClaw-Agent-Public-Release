...
export function derivePlanLifecycle(row: {
  ...
  const blockerClass = events.some((event: any) => event?.type === "execution.recovery_blocked")
    ? "stale_recovery_refusal" as const
    : undefined;
  ...
  const nextAction = blockerClass === "stale_recovery_refusal"
  ...
  return {
    ...
    actionable: lifecycle !== "archived" && lifecycle !== "handoff_pending" && blockerClass !== "stale_recovery_refusal",
  };
}
