export const taskStateStore = new Map<string, TaskState>();

function taskStateKey(tenantId: number, conversationId: number): string {
...
export async function rehydrateTaskStateOnBoot(): Promise<{ rehydrated: number; skipped: number }> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!Number.isInteger(row.tenant_id) || row.tenant_id <= 0) { skipped++; continue; }
      const key = taskStateKey(row.tenant_id, convId);
      if (taskStateStore.has(key)) continue; // newer in-memory state wins
...
const INTENT_PATTERNS: Record<ConversationIntent, RegExp[]> = {
  ...
  operations: [
    ...
    // R125+13.15 Smart Leads wedge triggers (lead-enrichment track).
    // Deliberately EXCLUDES "lead research": that phrase also scores the
    // research intent and classifyIntent keeps the first equal-score intent,
    // so it would misroute. SOP + docs only advertise the phrases below.
    /\b(smart[\s-]?leads?|enrich (this|these|the|a|my)? ?leads?|lead[\s-]?(enrichment|dossier)s?)\b/i,
  ],
  delegation: [
...
export function getOrCreateTaskState(tenantId: number, conversationId: number, userMessage: string, source?: string): TaskState {
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("getOrCreateTaskState: positive tenantId is required");
  }
  const key = taskStateKey(tenantId, conversationId);
  const existing = taskStateStore.get(key);
  if (existing) {
  ...
  taskStateStore.set(key, state);
  return state;
}

export function updateTaskState(tenantId: number, conversationId: number, update: Partial<TaskState>): TaskState | null {
  const state = taskStateStore.get(taskStateKey(tenantId, conversationId));
  if (!state) return null;
  Object.assign(state, update, { updatedAt: Date.now() });
  return state;
}

export function logDecision(tenantId: number, conversationId: number, what: string, why: string): void {
  const state = taskStateStore.get(taskStateKey(tenantId, conversationId));
  if (!state) return;
...
export function recordToolExecution(tenantId: number, conversationId: number, toolName: string, success: boolean, identifiers?: Record<string, string>): void {
  const state = taskStateStore.get(taskStateKey(tenantId, conversationId));
  if (!state) return;
...
export function clearTaskState(tenantId: number, conversationId: number): void {
  taskStateStore.delete(taskStateKey(tenantId, conversationId));
}
...
export function buildBrainContext(tenantId: number, conversationId: number, userMessage: string, toolsUsed?: string[], source?: string): string | null {
  const state = getOrCreateTaskState(tenantId, conversationId, userMessage, source);
