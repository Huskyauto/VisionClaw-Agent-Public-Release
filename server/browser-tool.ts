...
import {
  ...
  resetVisionState,
  updateVisionState,
  type VisionMemoryEntry,
} from "./browser-vision-memory";
...
async function geminiVisionNarrate(screenshotBase64: string, pageUrl: string, action: string): Promise<string> {
  const VISION_MODELS = ["gpt-5.6-sol", "google/gemini-3.7-flash"];
...
async function captureAndNarrate(page: Page, tenantId: number | undefined, action: string) {
  ...
  const trustedTenantId = tenantId as number;
  const now = Date.now();
  const lastCall = narrationThrottleMap.get(trustedTenantId) || 0;
  if (now - lastCall < NARRATION_THROTTLE_MS) return;
  narrationThrottleMap.set(trustedTenantId, now);
  ...
  try {
    ...
    const driveResult = await uploadScreenshotToDrive(buf, filename, title, trustedTenantId);
    if (driveResult.error || !driveResult.driveUrl) {
      console.warn(`[browser-vision] Auto-capture was not made durable: ${driveResult.error || "Drive returned no view URL"}`);
      return;
    }
    const durableUrl = driveResult.driveUrl;
    emitBrowserLive(trustedTenantId, "screenshot", `${action}: ${title}`, {
      screenshotUrl: durableUrl,
    ...
    geminiVisionNarrate(base64, url, action).then((narration) => {
      if (narration) {
        emitBrowserLive(trustedTenantId, "analyzing", narration, {
          screenshotUrl: durableUrl,
          pageTitle: title,
          pageUrl: url,
...
export { loadBrowserConfig, saveBrowserConfig, autoConfigureFromEnv, getRayobrowseStatus, createProfile, updateProfile } from "./browser-config";
import { loadBrowserConfig, saveBrowserConfig } from "./browser-config";
export type { BrowserProfileConfig, BrowserConfig } from "./browser-config";
...
// ─── Request-level SSRF guard (closes the page.goto TOCTOU, 2026-07-08) ────
//
// The pre-navigation isUrlAllowedWithDns() check validates a URL BEFORE the
// remote Browserless session navigates — a DNS-rebinding host or a redirect
// chain could still steer the browser to a private/internal address AFTER
// the check (time-of-check/time-of-use). This guard intercepts every request
// the page issues, inside the CDP session itself:
//   • ALL requests (navigations + subresources): abort on literal private-IP
//     hosts, blocked hostnames, and non-http(s) schemes (data/blob/about pass).
//   • NAVIGATION requests (initial goto, every redirect hop, client-side
//     navigations, frames): full DNS-revalidated policy check, so a redirect
//     to a rebinding host is aborted at the hop where it appears.
// Verdicts are cached per-hostname for a short TTL to keep per-request DNS
// overhead off the hot path (the browser caches DNS on the same order anyway).
// Failure posture: navigation requests fail CLOSED (abort on guard error);
// subresource guard errors fail open (a broken image is not a security event).

const ssrfGuardedPages = new WeakSet<Page>();
const ssrfDnsVerdictCache = new Map<string, { allowed: boolean; expires: number }>();
const SSRF_DNS_VERDICT_TTL_MS = 30_000;
const SSRF_DNS_VERDICT_CACHE_MAX = 500;

async function isNavUrlAllowedCached(url: string, config: BrowserConfig): Promise<boolean> {
  const hostname = new URL(url).hostname.toLowerCase();
  const now = Date.now();
  const hit = ssrfDnsVerdictCache.get(hostname);
  if (hit && hit.expires > now) return hit.allowed;
  const allowed = await isUrlAllowedWithDns(url, config);
  if (ssrfDnsVerdictCache.size >= SSRF_DNS_VERDICT_CACHE_MAX) {
    for (const [k, v] of ssrfDnsVerdictCache) {
      if (v.expires <= now) ssrfDnsVerdictCache.delete(k);
    }
    if (ssrfDnsVerdictCache.size >= SSRF_DNS_VERDICT_CACHE_MAX) ssrfDnsVerdictCache.clear();
  }
  ssrfDnsVerdictCache.set(hostname, { allowed, expires: now + SSRF_DNS_VERDICT_TTL_MS });
  return allowed;
}

function hostnameIsLiteralPrivateIp(hostname: string): boolean {
  const bare = hostname.replace(/^\[|\]$/g, "");
  const normalized = normalizeIpv4(bare);
  if (normalized && isPrivateIpNormalized(normalized)) return true;
  return isPrivateIpNormalized(bare);
}

export async function attachSsrfRequestGuard(page: Page): Promise<void> {
  if (ssrfGuardedPages.has(page)) return;
  const config = loadBrowserConfig();
  if (config.ssrfPolicy.allowPrivateNetwork) {
  ...
  try {
    ...
    // Could not arm interception (page already closed / CDP hiccup). The
    // pre-navigation + post-action URL checks remain; log LOUD so this
    // never degrades silently.
    console.warn(`[browser] SSRF request guard could not attach: ${err?.message?.slice(0, 120)}`);
    return;
  }
  ssrfGuardedPages.add(page);
  page.on("request", (req) => {
    void (async () => {
      const abort = async () => { try { await req.abort("blockedbyclient"); } catch (_e) { logSilentCatch("server/browser-tool.ts", _e); } };
      const allow = async () => { try { await req.continue(); } catch (_e) { logSilentCatch("server/browser-tool.ts", _e); } };
      try {
        const url = req.url();
        if (/^(data|blob|about):/i.test(url)) return void (await allow());
        if (!/^https?:\/\//i.test(url)) return void (await abort());
        const hostname = new URL(url).hostname.toLowerCase();
        if (hostnameIsLiteralPrivateIp(hostname)) return void (await abort());
        if (config.ssrfPolicy.blockedHostnames?.some(b => hostname === b || hostname.endsWith(`.${b}`))) {
          return void (await abort());
        }
        if (req.isNavigationRequest()) {
          if (!(await isNavUrlAllowedCached(url, config))) {
            console.warn(`[browser] SSRF guard aborted navigation to ${hostname} (post-check re-validation)`);
            return void (await abort());
          }
        }
        return void (await allow());
      } catch (err: any) {
        logSilentCatch("server/browser-tool.ts", err);
        // Fail CLOSED for navigations, OPEN for subresources.
        let isNav = false;
        try { isNav = req.isNavigationRequest(); } catch (_e) { isNav = true; }
        if (isNav) await abort(); else await allow();
      }
    })();
  });
}
...
// Marks the async context of an INTERNAL frame-detach retry so recordAction
// sites (wherever they live — helpers included) skip quota accounting: one
// detached-frame incident = one logical action. ALS is per-async-context, so
// concurrent customer actions during the retry are unaffected.
const frameRetryAls = new AsyncLocalStorage<boolean>();

function recordAction(session: TenantBrowserSession): void {
  session.lastActivity = Date.now();
  if (frameRetryAls.getStore()) return; // internal retry: quota-neutral
  session.actionCount++;
...
export async function applyStealthToPage(page: Page, tenantId?: number): Promise<void> {
  try {
  ...
  // Awaited so attach order is deterministic (capture listener first, guard last).
  if (tenantId) await attachCaptureSafe(page, tenantId);
...
async function attachCaptureSafe(page: Page, tenantId: number): Promise<void> {
  try {
...
export async function getPageForSession(session: TenantBrowserSession, tabIndex?: number): Promise<Page> {
  ...
  if (tabIndex !== undefined) {
    ...
    // Pages can enter the context without going through our newPage path
    // (e.g. target=_blank popups opened by the site) — guard is idempotent.
    // Capture attaches first, guard LAST (same order as applyStealthToPage).
    await attachCaptureSafe(pages[tabIndex], session.tenantId);
    await attachSsrfRequestGuard(pages[tabIndex]);
    return pages[tabIndex];
  }
  if (pages.length === 0) {
    const page = await session.context.newPage();
    await applyStealthToPage(page, session.tenantId);
    return page;
  }
  await attachCaptureSafe(pages[0], session.tenantId);
  await attachSsrfRequestGuard(pages[0]);
  return pages[0];
}
...
async function uploadScreenshotToDrive(screenshotBuffer: Buffer, filename: string, pageTitle: string, tenantId?: number, parentFolderId?: string): Promise<{ driveUrl?: string; directDownloadUrl?: string; error?: string }> {
  try {
    if (!Number.isSafeInteger(tenantId) || (tenantId as number) <= 0) return {};
    const result = await uploadAndShare({
      ...
      generatedArtifact: { artifactKind: "browser_capture", logicalName: filename, sourceRunKey: `browser-capture:${filename}` },
    });
    if (result.success) {
      return {
    ...
    return { error: result.error || "Drive durability verification failed" };
  } catch (err: any) {
...
export async function takeScreenshot(index?: number, fullPage?: boolean, selector?: string, profileName?: string, tenantId?: number, returnBase64?: boolean): Promise<any> {
  if (!Number.isSafeInteger(tenantId) || (tenantId as number) <= 0) {
    return { success: false, error: "Generated browser captures require a trusted tenant ID for durable storage" };
  }
  let page: Page;
  ...
  let screenshotBase64Thumb: string | undefined;
  try {
    const thumbBuf = await page.screenshot({ type: "jpeg", quality: 50 }) as Buffer;
    screenshotBase64Thumb = `data:image/jpeg;base64,${thumbBuf.toString("base64")}`;
  } catch (thumbnailError) {
    console.warn("[browser-vision] Optional screenshot thumbnail unavailable", thumbnailError);
  }

  const driveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, pageTitle, tenantId);
  if (driveResult.error || !driveResult.driveUrl) {
    return { success: false, error: `Screenshot was captured but is not durable: ${driveResult.error}` };
  }
  emitBrowserLive(tenantId, "screenshot", `Screenshot: ${pageTitle}`, {
    screenshotUrl: driveResult.driveUrl,
    screenshotBase64: screenshotBase64Thumb,
    pageTitle,
    pageUrl: page.url(),
  });
  ...
  const result: any = {
    success: true,
    screenshotUrl: driveResult.driveUrl,
    screenshotMarkdown: `![Screenshot of ${pageTitle}](${driveResult.driveUrl})`,
  ...
  if (driveResult.error) result.driveError = driveResult.error;
...
export async function executeBrowserAction(params: BrowserAction): Promise<any> {
  ...
  // Internal one-shot frame-detach retry is quota-neutral: the original
  // admission already counted this logical action.
  if (tenantId && !(params as any).__frameRetried && !checkTenantRateLimit(tenantId, config)) {
  ...
  try {
    switch (params.action) {
      ...
      case "smart_browse": {
        ...
        const smartDriveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, title, tenantId, (params as any)._projectDriveFolderId);
        if (smartDriveResult.error || !smartDriveResult.driveUrl) {
          return { success: false, error: `Browser capture is not durable: ${smartDriveResult.error}` };
        }
        emitBrowserLive(tenantId, "screenshot", `Captured: ${title}`, { screenshotUrl: smartDriveResult.driveUrl, screenshotBase64: smartBase64Thumb, pageTitle: title, pageUrl: url });
        geminiVisionNarrate(screenshotBuffer.toString("base64"), url, `Browsed ${new URL(params.url).hostname}`).then((narration) => {
          if (narration) emitBrowserLive(tenantId, "analyzing", narration, { screenshotUrl: smartDriveResult.driveUrl, pageTitle: title, pageUrl: url, visionNarration: narration });
        });
        ...
        const smartResult: any = {
          ...
          url,
          title,
          screenshotUrl: smartDriveResult.driveUrl,
          screenshotMarkdown: `![Screenshot of ${title}](${smartDriveResult.driveUrl})`,
    ...
    // Transient frame-lifecycle errors: the page navigated / a frame was
    // replaced mid-read (e.g. "Attempted to use detached Frame '<id>'",
    // "Execution context was destroyed"). The browser session itself is
    // healthy — tearing it down (the generic path below) only makes the next
    // step slower. Instead: let the page settle briefly and retry the SAME
    // action once. Seen live 2026-08-09 during Felix's Eagle Management audit.
    const DETACHED_FRAME_RE = /detached Frame|Execution context was destroyed|Cannot find context with specified id|frame got detached|Node is detached from document/i;
    if (DETACHED_FRAME_RE.test(String(err?.message || ""))) {
      // Only READ-ONLY actions are safe to replay: mutating actions (click,
      // type, form_fill, navigate, vision_act…) may have fired their side
      // effect before the frame detached — replaying risks double-submit
      // (architect review 2026-08-09).
      const RETRYABLE_READONLY = new Set(["screenshot", "content", "snapshot", "read_page_md", "tabs", "health", "scroll_down", "scroll_up"]);
      const readOnly = RETRYABLE_READONLY.has(params.action as string);
      if (readOnly && !(params as any).__frameRetried) {
        console.warn(`[browser-tool] transient frame error on '${params.action}' — retrying once after settle: ${err.message}`);
        await new Promise((r) => setTimeout(r, 1500));
        // __frameRetried skips rate-limit re-admission at the top of this
        // function, and the ALS context makes every recordAction site inside
        // the retry a no-op for quota: one incident = one logical action,
        // concurrency-safe (no shared-array tail compensation).
        return frameRetryAls.run(true, () => executeBrowserAction({ ...(params as any), __frameRetried: true }));
      }
      // Verify the session is actually still alive before promising it is —
      // "Cannot find context" can also mean a dead CDP context.
      let sessionAlive = false;
      if (tenantId) {
        try {
          const key = sessionKey(tenantId, profileName || config.defaultProfile);
          const s = tenantSessions.get(key);
          if (s) {
            const p = await getPageForSession(s);
            await Promise.race([
              p.evaluate("1"),
              new Promise((_r, rej) => setTimeout(() => rej(new Error("health timeout")), 3000)),
            ]);
            sessionAlive = true;
          }
        } catch (_silentErr) { logSilentCatch("server/browser-tool.ts", _silentErr); }
      ...
      if (sessionAlive) {
        return {
          success: false,
          error: readOnly
            ? "The page changed while it was being read (a frame reloaded mid-action). The browser session is still open — take a fresh snapshot or re-navigate, then retry this step."
            : `The page changed while '${params.action}' was in flight, so the action's outcome is UNKNOWN — it may or may not have taken effect. Do NOT blindly repeat it: take a fresh snapshot first and verify the page state before deciding whether to redo it.`,
        };
      }
      // Session is dead — fall through to the generic teardown path below.
    }
...
(function() {
  ...
  document.querySelectorAll('div, section, aside, [role="dialog"], [role="alertdialog"], [class*="modal"], [class*="overlay"], [class*="popup"], [class*="banner"], [class*="cookie"], [class*="consent"]').forEach(function(el) {
    ...
    } catch (e) { console.warn("[browser-som] overlay inspection failed", e); }
  });
...
export async function injectSomAndScreenshot(
  ...
  const driveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, somResult.pageTitle, tenantId);
  if (driveResult.error || !driveResult.driveUrl) {
    return { success: false, error: `Vision scan capture is not durable: ${driveResult.error}` };
  }
  emitBrowserLive(tenantId, "screenshot", `Vision scan: ${somResult.pageTitle}`, { screenshotUrl: driveResult.driveUrl, screenshotBase64: somBase64Thumb, pageTitle: somResult.pageTitle, pageUrl: somResult.pageUrl || "" });
  ...
  const result: any = {
    ...
    scrollContext,
    screenshotUrl: driveResult.driveUrl,
    screenshotMarkdown: `![SoM annotated screenshot of ${somResult.pageTitle}](${driveResult.driveUrl})`,
    elementSummary,
