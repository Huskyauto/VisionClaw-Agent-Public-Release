...
async function applyStealthToPage(page: Page, tenantId?: number): Promise<void> {
  try {
...
function attachCaptureSafe(page: Page, tenantId: number): void {
  import("./lib/derived-api")
    .then(({ attachNetworkCapture }) => attachNetworkCapture(page, tenantId))
    .catch((e) => logSilentCatch("server/browser-tool.ts (derived-api capture attach)", e, "expected"));
}
...
async function getPageForSession(session: TenantBrowserSession, tabIndex?: number): Promise<Page> {
  ...
  if (tabIndex !== undefined) {
    ...
    attachCaptureSafe(pages[tabIndex], session.tenantId);
  ...
  attachCaptureSafe(pages[0], session.tenantId);
  return pages[0];
}
