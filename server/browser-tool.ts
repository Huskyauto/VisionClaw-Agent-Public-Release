...
async function geminiVisionNarrate(screenshotBase64: string, pageUrl: string, action: string): Promise<string> {
  const VISION_MODELS = ["gpt-5.4", "claude-opus-4-20250514", "gemini-3.1-pro-preview"];
...
function isPrivateIpNormalized(ip: string): boolean {
...
async function validatePageUrlAfterAction(page: Page, config: BrowserConfig): Promise<string | null> {
  ...
  if (currentUrl && currentUrl !== "about:blank" && !(await isUrlAllowedWithDns(currentUrl, config))) {
    try { await page.goBack({ timeout: 5000 }); } catch {}
...
async function getOrCreateSharedBrowser(profileName: string): Promise<Browser> {
  ...
  if (sharedBrowser) {
    try { await sharedBrowser.disconnect(); } catch {}
    sharedBrowser = null;
    sharedBrowserProfile = null;
  }
...
export async function takeScreenshot(index?: number, fullPage?: boolean, selector?: string, profileName?: string, tenantId?: number, returnBase64?: boolean): Promise<any> {
  ...
  for (const f of oldFiles) {
    try { fs.unlinkSync(path.join(screenshotDir, f)); } catch {}
  }
  ...
  const driveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, pageTitle, (params as any)._projectDriveFolderId);
  ...
  try {
    switch (params.action) {
      ...
      case "click": {
        ...
        await page.click(safe, { timeout: 5000, delay: Math.floor(Math.random() * 80) + 30 });
      ...
      case "type": {
        ...
        await humanDelay(100, 300);
        await page.click(safe, { timeout: 5000 });
        ...
        for (const field of params.fields) {
          try {
            ...
            if (fillType === "select") {
              ...
              await page.click(safe, { timeout: 5000 }).catch(() => {});
...
export async function injectSomAndScreenshot(
  ...
  const driveResult = await uploadScreenshotToDrive(screenshotBuffer, filename, somResult.pageTitle, (params as any)._projectDriveFolderId);
