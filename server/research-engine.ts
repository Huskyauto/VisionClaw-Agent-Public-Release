...
import { SCORING_SYSTEM_PROMPT } from "./research-scoring-prompt";
import {
  persistResearchDiscoveryAdapters,
  type ResearchDiscoveryAdapterSession,
} from "./research-discovery-adapters";
import type { ResearchDiscoveryObservation } from "./lib/research-discovery-shadow";
import { extractTriplesIntoGraph } from "./research-evidence-extraction";
...
export interface ActiveSession extends ResearchDiscoveryAdapterSession {
  sessionId: number;
  programId: number;
  tenantId: number;
  model: string;
...
export async function startResearchSession(params: {
  ...
  const { EXPERT_RESEARCH_PROGRAM_NAME, isExpertResearchEnabled } = await import("./research-expert-mode");
  if (program.name === EXPERT_RESEARCH_PROGRAM_NAME && !isExpertResearchEnabled()) {
    console.warn(`[research:expert] Refusing disabled expert program for tenant=${tenantId}`);
    return { sessionId: 0, error: "Expert research mode is disabled" };
  }
  ...
  if (program.persona_id) {
    const persona = await storage.getPersonaForTenant(program.persona_id, tenantId);
...
async function autoDepositFindings(sessionId: number, session: ActiveSession): Promise<void> {
  ...
  const personaId = programRow?.persona_id
    ? (await storage.getPersonaForTenant(programRow.persona_id, session.tenantId))?.id ?? null
    : null;
  ...
  await assertMemoryValueNotTombstoned({
    ...
    parts: [knowledgeTitle, knowledgeContent],
  });
  ...
  for (const f of findings) {
    ...
    await assertMemoryValueNotTombstoned({
      tenantId: session.tenantId,
      source: "agent_knowledge",
      parts: [findingTitle, findingContent],
    });
    await db.execute(sql`
      INSERT INTO agent_knowledge (tenant_id, persona_id, title, content, source, created_at)
  ...
  try {
    ...
    if (knowledgeId) {
      ...
      if (vec) await storeEmbeddingVec("agent_knowledge", knowledgeId, vec, {
        tenantId: session.tenantId,
        expectedKnowledgeContent: `${knowledgeTitle} ${knowledgeContent}`.slice(0, 6000),
      });
    }
...
async function endSession(sessionId: number, reason: string): Promise<void> {
  ...
  try {
    ...
    if (reason === "stopped_budget") {
      ...
      console.error(`[research] Session ${sessionId} summary generation failed; persisting explicit fallback`, err);
      summary = `Session ended (${reason}). ${session.keptCount} kept, ${session.discardedCount} discarded, ${session.crashedCount} crashed. Summary generation failed; see operator logs.`;
    }
...
async function runExperiment(session: ActiveSession): Promise<void> {
  ...
  const { isExpertResearchEnabled } = await import("./research-expert-mode");
  const expertCostMode = session.evalType === "cost" && isExpertResearchEnabled();
...
This program runs each hypothesis through a frozen 20-query benchmark and measures USD-per-query + a quality judge score 0-10. When Expert Evidence mode is active, a stratified 5-query screen runs first and qualifying custom prompts receive a no-prompt ablation.
You MUST include a CONFIG_JSON block in your RESULT with the exact configuration to test, like:
CONFIG_JSON: {"model": "gpt-5.4", "systemPrompt": "Be concise.", "temperature": 0.2}
Allowed model ids include: gpt-5.4, gpt-5, claude-sonnet-4-6, google/gemini-3.7-flash, deepseek/deepseek-v3.2.
...
INSIGHT: [One key insight for the next experiment]`;
  ...
  experimentId = expRows[0]?.id;
    ...
    const budgetClaim = await claimAutonomousBudget({
      tenantId: session.tenantId,
      estimatedUsd: expertCostMode ? 2.00 : 0.10,
...
Score this finding using the rubric in your instructions. Output your reasoning for each criterion on one line, then the final score on the last line as just a number.`;
    ...
    // Cost Optimizer branch: parse a CONFIG_JSON block and run the frozen
    // benchmark. Expert mode adds screen -> full -> prompt-ablation evidence.
    let numericMetricValue: number | null = null; let discoveryQuality: { passed: boolean; kind: "independent_evaluator" } | null = null;
    let metricDeltaPct: number | null = null;
    let expertBaselineEligible = !expertCostMode;
    let verificationStatus = expertCostMode ? "failed" : "unverified";
    let verificationDetails: string | null = expertCostMode
      ? JSON.stringify({ mode: "expert_cost_evidence_v1", reasonCodes: ["candidate_not_evaluated"] })
      : null;
    if (expertCostMode) score = Math.min(score, 4);
      ...
      try {
        ...
        if (cfgMatch) {
          ...
          if (cfg && typeof cfg.model === "string") {
            ...
            const {
              getFrozenCostEvalQueries,
              runCostEvalSuite,
              summarizeCostEvalForResearch,
            } = await import("./cost-eval-runner");
            let evalResult: Awaited<ReturnType<typeof runCostEvalSuite>>;
            if (expertCostMode) {
              const { runExpertCostEvaluation } = await import("./research-expert-mode");
              const { withTenantContext } = await import("./lib/tenant-context");
              const expert = await runExpertCostEvaluation({
                config: costConfig,
                baselineMetricValue: session.baselineMetricValue,
                fullQueries: getFrozenCostEvalQueries(),
                runSuite: (candidate, queries) => withTenantContext(
                  { tenantId: session.tenantId, source: "cron" },
                  () => runCostEvalSuite(candidate, {
                    queries,
              ...
              evalResult = expert.full || expert.screen;
              verificationStatus = expert.verdict.verificationStatus;
              expertBaselineEligible = expert.verdict.baselineEligible;
              verificationDetails = expert.verdict.details;
              metricDeltaPct = expert.verdict.metricDeltaPct;
              discoveryQuality = {
                passed: expert.verdict.keepEligible,
                kind: "independent_evaluator",
              };
              score = expert.verdict.keepEligible ? Math.max(score, 8) : Math.min(score, 4);
              result = `${result}\n\n--- EXPERT EVIDENCE SCREEN ---\n${summarizeCostEvalForResearch(expert.screen).result}\n\n--- EXPERT FULL BENCHMARK ---\n${expert.full ? summarizeCostEvalForResearch(expert.full).result : "Skipped: screening evidence did not meet the quality/completeness floor."}\n\n--- EXPERT PROMPT ABLATION ---\n${expert.ablation ? summarizeCostEvalForResearch(expert.ablation).result : "Not applicable or screening did not qualify."}\n\n--- EXPERT INTEGRITY VERDICT ---\n${expert.verdict.details}`;
              console.log(`[research:expert] session=${session.sessionId} exp=${session.experimentCount} status=${verificationStatus} full=${Boolean(expert.full)} ablation=${Boolean(expert.ablation)}`);
            } else {
              evalResult = await runCostEvalSuite(costConfig);
            }
            ...
            if (session.baselineMetricValue && session.baselineMetricValue > 0) {
              ...
              if (!expertCostMode) {
                discoveryQuality = evalResult.failureCount === 0 ? { passed: qualityOk, kind: "independent_evaluator" } : null;
              }
              const costImprovement = metricDeltaPct < -5; // at least 5% cheaper
              const costRegression = metricDeltaPct > 10;  // 10% more expensive = bad
              if (expertCostMode && discoveryQuality) {
                score = discoveryQuality.passed ? Math.max(score, 8) : Math.min(score, 4);
              } else if (qualityOk && costImprovement) score = Math.max(score, 8);
              ...
              if (!expertBaselineEligible) {
                throw new Error("incomplete expert evidence cannot establish a cost baseline");
              }
              const baselineWrite = await db.execute(sql`
                ...
                RETURNING baseline_metric_value
              `);
              const baselineRows = (baselineWrite as any).rows || baselineWrite;
              const persistedBaseline = Number(baselineRows[0]?.baseline_metric_value);
              if (!Number.isFinite(persistedBaseline) || persistedBaseline < 0) {
                throw new Error("cost baseline was not durably persisted");
              }
              session.baselineMetricValue = persistedBaseline;
              session.baselineLabel = "USD per query";
              metricValue = `${score} (baseline set: $${persistedBaseline.toFixed(6)}/q, q=${evalResult.judgeScoreAvg.toFixed(1)})`;
            }
          }
        }
      } catch (costErr: any) {
    ...
    if (verdict === "KEEP") {
      ...
      if (session.evidenceGraph) {
        extractTriplesIntoGraph({
          graph: session.evidenceGraph!,
          tenantId: session.tenantId,
    ...
    await persistResearchDiscoveryAdapters({ session, experimentId: experimentId!, hypothesis, approach, metric, score, numericMetricValue, qualityConstraintPassed: discoveryQuality?.passed ?? null, qualityEvidenceKind: discoveryQuality?.kind ?? "llm_judge" });
...
export async function resolvePersonaId(personaSlug: string, tenantId: number): Promise<number | null> {
  // Personas are platform-managed system records (no tenant_id in schema).
  // Bind resolution to a real tenant nonetheless; tenant_persona_names is only
  // a display-name override and must not be mistaken for ownership.
  const result = await db.execute(sql`
    SELECT p.id
    FROM personas p
    JOIN tenants t ON t.id = ${tenantId}
    WHERE p.name = ${personaSlug}
    LIMIT 1
  `);
  const rows = (result as any).rows || result;
...
async function injectKeepedFinding(
  ...
  try {
    await assertMemoryValueNotTombstoned({
      tenantId: session.tenantId,
      source: "agent_knowledge",
      parts: [safeInjectTitle, safeInjectContent],
    });
      ...
      try {
        ...
        if (embedding) {
          await storeEmbeddingVec("agent_knowledge", insertedId, embedding, {
            tenantId: session.tenantId,
            expectedKnowledgeContent: `${safeInjectTitle} ${safeInjectContent}`.slice(0, 6000),
          });
