...
export async function getResearchSessionStatus(sessionId: number, tenantId?: number) {
  ...
  const result = tenantId
    ? await db.execute(sql`SELECT * FROM research_sessions WHERE id = ${sessionId} AND tenant_id = ${tenantId}`)
    : await db.execute(sql`SELECT * FROM research_sessions WHERE id = ${sessionId}`);
  const rows = (result as any).rows || result;
  return rows[0] || null;
}
