  ...
  try {
    ...
    const result = await db.execute(sql`
      ...
      WHERE status = 'running'
        AND id NOT IN (${sql.raw(Array.from(activeSessions.keys()).join(",") || "0")})
    ...
    if (activeSessions.size === 0 && session.keptCount > 0) {
      setTimeout(async () => {
        try {
          ...
          console.log(`[research] All sessions complete — generating research digest...`);
          const digest = await generateResearchDigest(session.tenantId);
          if (digest.success) {
            console.log(`[research] Digest generated: ${digest.findingCount} findings, ${digest.proposalCount} proposals → ${digest.digestPath}`);
            if (digest.driveUrl) console.log(`[research] Digest uploaded to Drive: ${digest.driveUrl}`);
          }
        } catch (err: any) {
          console.warn(`[research] Auto-digest generation failed: ${err.message}`);
        }
      }, 10_000);
    }
  } finally {
...
async function injectKeepedFinding(
  ...
  if (score >= 7) {
    generateCodeProposal(session, programName, hypothesis, result, approach, score, mapping, personaId).catch(err => {
      console.error(`[research] Code proposal generation FAILED (score ${score}, prog="${programName}"): ${err.message}`);
      console.error(`[research] Code proposal stack: ${err.stack?.split("\n").slice(0, 3).join(" | ")}`);
    });
  }
}
...
export async function generateResearchDigest(tenantId: number = 1): Promise<{
  ...
  try {
    ...
    lines.push(`---`);
    lines.push(`_This digest is auto-generated by the VisionClaw Research Engine. To implement proposals, review them in the Research Dashboard or surface to the development agent._`);
