...
function ensureTable(): Promise<void> {
  if (!_tableEnsured) {
    _tableEnsured = (async () => {
      ...
      `);
      await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_knowledge_triples_tenant ON knowledge_triples(tenant_id)`);
      await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS uq_knowledge_triples_tenant_norm ON knowledge_triples(tenant_id, norm_key)`);
    })().catch((err) => {
  ...
  (async () => {
    try {
      ...
      for (const [key, t] of byKey.entries()) {
        await db.execute(sql`
          INSERT INTO knowledge_triples (tenant_id, subject, predicate, object, norm_key, source_session_id)
          VALUES (${tenantId}, ${t.subject}, ${t.predicate}, ${t.object}, ${key}, ${sessionId})
          ON CONFLICT (tenant_id, norm_key) DO UPDATE
            SET seen_count = knowledge_triples.seen_count + 1,
                created_at = now(),
      ...
      await db.execute(sql`DELETE FROM knowledge_triples WHERE tenant_id = ${tenantId} AND created_at < ${cutoff}`);
      await db.execute(sql`
        DELETE FROM knowledge_triples WHERE tenant_id = ${tenantId} AND id NOT IN (
          SELECT id FROM knowledge_triples WHERE tenant_id = ${tenantId}
...
export async function loadDurableTriples(
  ...
  try {
    ...
    db.execute(sql`
      UPDATE knowledge_triples SET hit_count = hit_count + 1, last_hit_at = now()
