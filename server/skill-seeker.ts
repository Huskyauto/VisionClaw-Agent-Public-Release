...
import { executeGuardedTool } from "./guarded-tool-executor";

import { logSilentCatch } from "./lib/silent-catch";
import { coerceGapTenantId } from "./lib/gap-tenant";
...
async function getAnalysisClient() {
  try {
    const { client } = await getClientForModel("gpt-5");
    return { client, model: "gpt-5" };
  } catch {
    try {
      const { client } = await getClientForModel("claude-sonnet-4-5");
      return { client, model: "claude-sonnet-4-5" };
    } catch {
      const { client } = await getClientForModel("gpt-5.6-sol");
      return { client, model: "gpt-5.6-sol" };
    }
  }
}
...
export async function detectGap(
  ...
  // Tenant-signal integrity (architect 2026-07-14): unknown/invalid tenant →
  // sentinel 0, never defaulted to admin tenant 1. See server/lib/gap-tenant.ts.
  tenantId = coerceGapTenantId(tenantId);
  // Tool Forge Phase 1: a repeat miss of an already-open gap is SIGNAL, not a
  // no-op — bump miss_count + last_seen_at so the "Unmet Capabilities" card can
  // rank gaps by demand. Atomic UPDATE (no read-modify-write race).
  const existing = await db.execute(
    sql`UPDATE capability_gaps SET miss_count = miss_count + 1, last_seen_at = now()
        WHERE gap_description = ${description} AND tenant_id = ${tenantId} AND status NOT IN ('resolved', 'safety_blocked')
        RETURNING *`
  );
  ...
  if (result.rows.length === 0) {
    // Lost an insert race — the concurrent inserter's row absorbs this miss too.
    const retry = await db.execute(
      sql`UPDATE capability_gaps SET miss_count = miss_count + 1, last_seen_at = now()
          WHERE gap_description = ${description} AND tenant_id = ${tenantId}
          RETURNING *`
    );
    ...
    if (analysis.tool_description) {
      ...
      if (toolCodeScan.safe) {
        try {
          ...
          const toolResult = await executeGuardedTool("create_tool", {
          ...
          if (toolResult?.error) {
            const approvalMsg = `NEEDS REVIEW: capability mutation requires explicit owner approval. ${toolResult.error}`;
            await db.execute(
              sql`UPDATE capability_gaps SET status = 'needs_review', resolution = ${approvalMsg} WHERE id = ${gapId} AND tenant_id = ${tenantId}`
            );
            result.action_taken = "needs_review";
            result.status = "needs_review";
            result.safety_report!.issues_found.push(approvalMsg);
            return result;
          }
          ...
          if (toolName) {
            if (toolImpl) {
              ...
              if (!implScan.safe) {
                ...
                try {
                  await executeGuardedTool("delete_custom_tool", { name: toolName }, {
                    tenantId: ADMIN_TENANT_ID,
                    personaName: "VisionClaw",
                    invokedVia: "system",
                  });
  ...
  if (analysis.approach === "skill" || analysis.approach === "both") {
    if (analysis.skill_content && analysis.skill_name) {
      ...
      if (skillScan.safe && !hasCriticalContent) {
        try {
          const skillResult = await executeGuardedTool("manage_skills", {
            command: "create",
          ...
          if (skillResult?.error) {
            const approvalMsg = `NEEDS REVIEW: capability mutation requires explicit owner approval. ${skillResult.error}`;
            await db.execute(
              sql`UPDATE capability_gaps SET status = 'needs_review', resolution = ${approvalMsg} WHERE id = ${gapId} AND tenant_id = ${tenantId}`
            );
            result.action_taken = "needs_review";
...
export async function listGaps(
  ...
  // Fail CLOSED on a missing/invalid tenant — an un-scoped read of
  // capability_gaps would be a cross-tenant disclosure footgun for any
  // future caller that forgets to pass tenantId.
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("listGaps requires a valid tenantId (tenant-scoped read only)");
  }
