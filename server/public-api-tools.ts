...
export async function readAllowedJsonResponse(
  resp: Response,
  allowedStatuses: ReadonlySet<number> = new Set(),
): Promise<any> {
  if (!resp.ok && !allowedStatuses.has(resp.status)) {
...
async function safeGetJson(
  ...
  for (let hop = 0; hop <= MAX_REDIRECT_HOPS; hop++) {
    ...
    try {
      ...
      return await readAllowedJsonResponse(resp, allowedStatuses);
    } finally {
      clearTimeout(timer);
    }
  }
...
// ───────────────────────── 4. Wikipedia summary ─────────────────────────────
export function wikipediaNotFoundResult(title: string): PublicApiOk {
  return {
    ok: true,
    ...
    data: {
      found: false,
      query: title,
      message: `No Wikipedia article found for "${title}"`,
    },
  };
}
...
export async function fetchWikipedia(params: Record<string, any>): Promise<PublicApiResult> {
  try {
    ...
    if (!data || data.type === "https://mediawiki.org/wiki/HyperSwitch/errors/not_found") {
      return wikipediaNotFoundResult(title);
    }
    return {
      ok: true,
      source: "en.wikipedia.org",
