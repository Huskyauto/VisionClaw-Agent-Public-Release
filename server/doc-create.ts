...
interface DocxOptions {
  ...
  headerText?: string;
  footerText?: string;
  /** Omit the usual platform footer for customer-owned documents. */
  includeFooter?: boolean;
  fileName?: string;
  folderLabel?: string;
  /** Set false when a caller must validate a complete document pair before any Drive upload. */
...
export async function createDocx(opts: DocxOptions): Promise<{
  ...
  try {
    ...
    for (const sec of opts.sections) {
      ...
      if (sec.signatureText && !sec.signatureAfterContent) {
        children.push(new Paragraph({
          children: [new TextRun({
            text: sec.signatureText,
            size: 34,
            font: "Segoe Script",
            italics: true,
            color: "16213e",
    ...
    const footerContent = opts.includeFooter === false ? undefined : [new Paragraph({
      children: [
    ...
    const doc = new Document({
      sections: [{
        ...
        footers: footerContent ? { default: new Footer({ children: footerContent }) } : undefined,
        children,
      }],
    });
    ...
    if (opts.uploadToDrive !== false) {
      try {
        const driveResult = await uploadAndShare({
          ...
          generatedArtifact: { artifactKind: "document", logicalName: `${opts.title}.docx`, sourceRunKey: `create-document:${slug}` },
        });
  ...
  try {
    ...
    try {
      const driveResult = await uploadAndShare({
        ...
        parentFolderId: (opts as any)._projectDriveFolderId || undefined,
        tenantId: opts.tenantId,
        generatedArtifact: { artifactKind: "spreadsheet", logicalName: `${opts.title}.xlsx`, sourceRunKey: `create-spreadsheet:${slug}` },
      });
      if (!driveResult.success) return { success: false, error: driveResult.error || "Drive durability verification failed" };
      if (driveResult?.viewUrl) {
        viewUrl = driveResult.viewUrl;
