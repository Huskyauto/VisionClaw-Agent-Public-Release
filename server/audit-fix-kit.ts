...
- Use ONLY facts present in the provided page content and audit findings.
- NEVER invent an address, phone number, email, opening hours, prices, staff names, awards, review counts, or founding year. If a fact is not in the provided content, OMIT it.
...
export async function generateAuditFixKit(params: {
  ...
  const modelId = await getModelForTierAsync("powerful", params.tenantId, { freeTierOnly: true }).catch(() => "gpt-5-mini");
  ...
  const llmSpecs: Array<{
  ...
  }> = [
    {
      ...
      user: `Write a complete llms.txt for this business. Format: start with "# ${params.company}", then a one-paragraph "> " summary, then sections (## Services, ## Key Pages, etc.) with markdown links to real pages ONLY if their paths appear in the page content — otherwise link just the homepage.\n\n${grounding}`,
      maxTokens: 900,
    ...
    },
    {
      ...
      user: `Write JSON-LD for this business: a LocalBusiness (or Organization if no physical-location evidence exists in the content) node with name, url, and description; include address/telephone/openingHours ONLY if they literally appear in the page content. If the page content contains obvious FAQ-style questions and answers, add a FAQPage node. Output valid JSON only.\n\n${grounding}`,
      maxTokens: 900,
