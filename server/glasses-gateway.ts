...
const GLASSES_DEFAULT_ALLOW_RAW: string[] = [
  ...
  // Research / web
  "web_search",
  "web_fetch", "scrapling_scrape",
  "deep_research",
  "research_digest",
  "lookup_sam_company_cage",
...
export function registerGlassesGateway(app: Express): void {
  ...
  app.post("/v1/glasses/execute", authGlasses, async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(`[glasses-gateway] tenant=${tenantId} tool=${name} ok=${!result?.error} dur=${durationMs}ms`);
      ...
      console.error(`[glasses-gateway] execute error tenant=${tenantId}:`, msg);
