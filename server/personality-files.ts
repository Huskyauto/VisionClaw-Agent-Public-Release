...
export async function ensurePersonalityFilesTable() {
  ...
  `);
  // Legacy migration (task #120): pre-existing DBs created this table with
  // tenant_id NOT NULL DEFAULT 1. Drop the admin-tenant default trapdoor so
  // every insert must name its tenant. Idempotent; loud on failure.
  try {
    await db.execute(sql`ALTER TABLE personality_files ALTER COLUMN tenant_id DROP DEFAULT`);
  } catch (e: any) {
    console.error("[personality-files] TENANT-ISOLATION migration failed (drop tenant_id default):", e?.message || e);
  }
}
