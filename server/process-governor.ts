...
type StaticCheck =
  ...
  | "time_of_day"
  | "failover_rate"
  | "framework_review_due"
  | "trust_score_update"
  | "proactive_action_budget"
  | "express_lane_health"
  | "environmental_signal_escalation"
  | "collective_intelligence_budget"
  | "earned_autonomy_audit";
...
type GovernanceCondition = ConditionOptions & (
  | { check: "subscription_activity"; operator?: "equals" | "greater_than"; value?: number }
  | { check: "task_failure_rate"; min_attempts?: number }
  | { check: ThresholdCheck; value?: number; threshold?: number; cap?: number }
  | { check: StaticCheck }
  | { check: string }
);
...
interface GovernanceRule {
  ...
  action: string;
  actionConfig: GovernanceActionConfig;
...
interface GovernanceRuleRow extends SqlRow {
  id: number;
  tenant_id: number;
  category: string;
  rule_name: string;
...
interface SubscriptionRow extends SqlRow {
  id: number;
  event_type: string;
  enabled: boolean;
  recent_events: string | number | null;
}

interface TaskFailureRow extends SqlRow {
  id: number;
  name: string;
...
interface DeskStatusRow extends SqlRow {
  persona_id: number;
...
interface CountRow extends SqlRow { cnt: string | number | null }
interface ProviderKeyRow extends SqlRow { provider: string; enabled: boolean | null }
interface AverageRow extends SqlRow { avg_ms: string | number | null }
interface FailingAgentsRow extends SqlRow { failing_agents: string | number | null }
interface MemoryWriterRow extends SqlRow { persona_id: number | null; writes: string | number }
interface OldestRow extends SqlRow { oldest: Date | string | null }
interface WorkloadRow extends SqlRow { persona_id: number; task_count: string | number }
interface FrameworkSummaryRow extends SqlRow { id: number; name: string; next_review_date: Date | string; last_reviewed?: Date | string | null }
interface LaneUsageRow extends SqlRow { lane_id: string; usage: string | number }
interface DeskRestartRow extends SqlRow { id: number; persona_id: number; name: string | null }
interface AlertRow extends SqlRow { id: number; category: string; item_name: string | null }
interface ChannelRow extends SqlRow { id: number }
interface SubscriptionStatusRow extends SqlRow { event_type: string; enabled: boolean }
interface RuleCategoryRow extends SqlRow { category: string; cnt: string | number }
interface LastRunRow extends SqlRow { created_at: Date | string }

export interface GovernanceActionRow extends SqlRow {
  rule_name: string;
  category: string;
...
export interface GovernanceRuleListRow extends SqlRow {
  id: number;
  category: string;
  rule_name: string;
  description: string;
...
export interface GovernanceActionHistoryRow extends GovernanceActionRow {
  id: number;
  tenant_id: number;
  rule_id: number;
...
interface FrameworkRow extends SqlRow {
  id: number;
  name: string;
  organization: string;
  version: string;
  ...
  rules_informed: unknown;
}

interface FrameworkReview {
  has_updates: boolean;
  new_version: string | null;
  new_principles: string[];
  outdated_principles: string[];
  suggested_new_rules: string[];
...
function parseCondition(value: unknown): GovernanceCondition {
  const parsed = parseJson(value);
  if (!isRecord(parsed) || typeof parsed.check !== "string") {
    return { check: "invalid_condition" };
  }
  const operator = parsed.operator === "equals" || parsed.operator === "greater_than" ? parsed.operator : undefined;
  const numeric = (key: string): number | undefined => typeof parsed[key] === "number" ? parsed[key] : undefined;
  return {
    check: parsed.check,
    operator,
    value: numeric("value"),
    min_attempts: numeric("min_attempts"),
    threshold: numeric("threshold"),
    cap: numeric("cap"),
  };
}

function parseActionConfig(value: unknown): GovernanceActionConfig {
  const parsed = parseJson(value);
  if (!isRecord(parsed)) return {};
  const routing = isRecord(parsed.routing)
    ? Object.fromEntries(Object.entries(parsed.routing).filter((entry): entry is [string, string] => typeof entry[1] === "string"))
    : undefined;
  return {
    notify_channel: typeof parsed.notify_channel === "string" ? parsed.notify_channel : undefined,
    routing,
    protected_personas: Array.isArray(parsed.protected_personas) ? parsed.protected_personas : undefined,
    exclude_kinds: Array.isArray(parsed.exclude_kinds) && parsed.exclude_kinds.every(v => typeof v === "string") ? parsed.exclude_kinds : undefined,
    reason: typeof parsed.reason === "string" ? parsed.reason : undefined,
    message: typeof parsed.message === "string" ? parsed.message : undefined,
    assign_to: typeof parsed.assign_to === "string" ? parsed.assign_to : undefined,
  };
}
...
function parseFrameworkReview(value: unknown): FrameworkReview | null {
  if (!isRecord(value) || typeof value.review_summary !== "string") return null;
  return {
    has_updates: value.has_updates === true,
    new_version: typeof value.new_version === "string" ? value.new_version : null,
    new_principles: stringArray(value.new_principles),
    outdated_principles: stringArray(value.outdated_principles),
    suggested_new_rules: stringArray(value.suggested_new_rules),
    review_summary: value.review_summary,
    next_review_months: typeof value.next_review_months === "number" ? value.next_review_months : 6,
  };
}
...
async function loadRules(tenantId: number): Promise<GovernanceRule[]> {
  const result = await db.execute<GovernanceRuleRow>(sql`
  ...
  try {
    switch (cond.check) {
      case "subscription_activity": {
        const subsResult = await db.execute<SubscriptionRow>(sql`
        ...
        const nonProtected = subs.filter(s => !PROTECTED_EVENTS.has(s.event_type));

        if (cond.operator === "equals" && cond.value === 0) {
          const dead = nonProtected.filter(s => s.enabled && parseInt(String(s.recent_events || "0")) === 0);
          if (dead.length > 0) {
            return { met: true, detail: `${dead.length} active subscription(s) with zero events in 30 days: ${dead.map(d => d.event_type).join(", ")}` };
          }
          const activitySubs = await checkBusinessActivity(tenantId);
          const noActivity = nonProtected.filter(s => {
            const check = activitySubs.find(a => a.eventType === s.event_type);
            return s.enabled && check && !check.hasActivity;
          });
          if (noActivity.length > 0) {
            return { met: true, detail: `${noActivity.length} subscription(s) with no business justification: ${noActivity.map(d => d.event_type).join(", ")}` };
          }
        }
        if (cond.operator === "greater_than" && cond.value === 0) {
          const activitySubs = await checkBusinessActivity(tenantId);
          const justified = activitySubs.filter(a => a.hasActivity);
          const disabled = nonProtected.filter(s => !s.enabled);
          const enableable = disabled.filter(s => justified.find(j => j.eventType === s.event_type));
          if (enableable.length > 0) {
            return { met: true, detail: `${enableable.length} disabled subscription(s) now have business activity: ${enableable.map(e => e.event_type).join(", ")}` };
          }
        }
      ...
      case "task_failure_rate": {
        const taskResult = await db.execute<TaskFailureRow>(sql`
          SELECT ht.id, ht.name,
            (SELECT COUNT(*) FROM heartbeat_logs hl WHERE hl.task_id = ht.id
              AND EXISTS (SELECT 1 FROM heartbeat_tasks scoped_ht WHERE scoped_ht.id = hl.task_id AND scoped_ht.tenant_id = ${tenantId})
              AND hl.status = 'success' AND hl.created_at > NOW() - INTERVAL '7 days') as successes,
            (SELECT COUNT(*) FROM heartbeat_logs hl WHERE hl.task_id = ht.id
              AND EXISTS (SELECT 1 FROM heartbeat_tasks scoped_ht WHERE scoped_ht.id = hl.task_id AND scoped_ht.tenant_id = ${tenantId})
              AND hl.status != 'success' AND hl.created_at > NOW() - INTERVAL '7 days') as failures
          FROM heartbeat_tasks ht WHERE ht.enabled = true AND ht.tenant_id = ${tenantId}
        `);
        const tasks = taskResult.rows;
        const failing = tasks.filter(t => {
          const s = parseInt(String(t.successes || "0"));
          const f = parseInt(String(t.failures || "0"));
          return (s + f) >= (cond.min_attempts || 5) && s === 0 && f > 0;
        });
        if (failing.length > 0) {
          return { met: true, detail: `${failing.length} task(s) with 100% failure: ${failing.map(f => `"${f.name}" (${f.failures} failures)`).join(", ")}` };
        }
      ...
      case "desk_status": {
        const deskResult = await db.execute<DeskStatusRow>(sql`
          SELECT ad.persona_id, p.name as persona_name, ad.status_note, ad.updated_at,
            (SELECT COUNT(*) FROM heartbeat_tasks WHERE persona_id = ad.persona_id AND tenant_id = ${tenantId} AND enabled = true) as pending_tasks
          FROM agent_desks ad LEFT JOIN personas p ON p.id = ad.persona_id
          WHERE ad.tenant_id = ${tenantId}
        `);
        const desks = deskResult.rows;
        const stalled = desks.filter(d => {
          const hours = (Date.now() - new Date(d.updated_at).getTime()) / (1000 * 60 * 60);
          return hours > (cond.value || 24) && parseInt(String(d.pending_tasks || "0")) > 0;
        });
        if (stalled.length > 0) {
          return { met: true, detail: `${stalled.length} agent(s) stalled 24h+ with pending work: ${stalled.map(s => s.persona_name).join(", ")}` };
        }
        return { met: false, detail: "No stalled agents detected" };
      }

      case "watchlist_alert": {
        const alertResult = await db.execute<CountRow>(sql`
          SELECT COUNT(*) as cnt FROM watchlist_alerts WHERE tenant_id = ${tenantId} AND acknowledged = false
        `);
        const cnt = parseInt(String(alertResult.rows[0]?.cnt || "0"));
        if (cnt > 0) {
      ...
      case "provider_health": {
        const keyResult = await db.execute<ProviderKeyRow>(sql`
          SELECT provider, enabled FROM provider_keys WHERE tenant_id = ${tenantId}
        `);
        const keys = keyResult.rows;
        const activeKeys = keys.filter(k => k.enabled !== false);
      ...
      case "response_time": {
        const perfResult = await db.execute<AverageRow>(sql`
          SELECT AVG(hl.duration_ms) as avg_ms
          FROM heartbeat_logs hl
          JOIN heartbeat_tasks ht ON ht.id = hl.task_id
          WHERE ht.tenant_id = ${tenantId}
            AND hl.created_at > NOW() - INTERVAL '1 hour'
            AND hl.duration_ms IS NOT NULL
            AND hl.status = 'success'
        `);
        const avgMs = parseFloat(String(perfResult.rows[0]?.avg_ms || "0"));
      ...
      case "autonomy_violation": {
        const violationResult = await db.execute<CountRow>(sql`
          SELECT COUNT(*) as cnt FROM autonomy_log
          WHERE tenant_id = ${tenantId} AND decision = 'blocked'
          AND created_at > NOW() - INTERVAL '24 hours'
        `);
        const violations = parseInt(String(violationResult.rows[0]?.cnt || "0"));
      ...
      case "cascading_failures": {
        const cascadeResult = await db.execute<FailingAgentsRow>(sql`
          SELECT COUNT(DISTINCT hl.persona_id) as failing_agents
          FROM heartbeat_logs hl
          JOIN heartbeat_tasks ht ON ht.id = hl.task_id
          WHERE ht.tenant_id = ${tenantId}
            AND hl.status != 'success'
            AND hl.created_at > NOW() - INTERVAL '30 minutes'
            AND hl.persona_id IS NOT NULL
        `);
        const failingAgents = parseInt(String(cascadeResult.rows[0]?.failing_agents || "0"));
      ...
      case "agent_scope_violations": {
        const scopeResult = await db.execute<CountRow>(sql`
          SELECT COUNT(*) as cnt FROM autonomy_log
          WHERE tenant_id = ${tenantId} AND decision = 'blocked' AND action_type NOT IN ('process_governance')
          AND created_at > NOW() - INTERVAL '24 hours'
        `);
        const scopeViolations = parseInt(String(scopeResult.rows[0]?.cnt || "0"));
      ...
      case "delegation_depth": {
        const depthResult = await db.execute<CountRow>(sql`
        ...
        `);
        const chainedDelegations = parseInt(String(depthResult.rows[0]?.cnt || "0"));
      ...
      case "memory_write_rate": {
        const memResult = await db.execute<MemoryWriterRow>(sql`
        ...
        `);
        const heavyWriters = memResult.rows;
      ...
      case "system_critical_state": {
        ...
        const keyResult = await db.execute<CountRow>(sql`
          SELECT COUNT(*) as cnt FROM provider_keys WHERE tenant_id = ${tenantId} AND enabled = true
        `);
        const activeKeys = parseInt(String(keyResult.rows[0]?.cnt || "0"));
      ...
      case "workload_balance": {
        const wlResult = await db.execute<WorkloadRow>(sql`
      ...
      case "framework_review_due": {
        const fwResult = await db.execute<FrameworkSummaryRow>(sql`
      ...
      case "express_lane_volume": {
        try {
          const result = await db.execute<LaneUsageRow>(sql`
          ...
          if (rows.length > 0) {
            return { met: true, detail: `${rows.length} lane(s) over daily cap: ${rows.map(r => `${r.lane_id}=${r.usage}`).join(", ")}` };
          }
    ...
    return { met: false, detail: `Error evaluating: ${errorMessage(err)}` };
  }
}
  ...
  switch (rule.action) {
    ...
    case "disable_task": {
      ...
      for (const t of tasks) {
        ...
        const f = parseInt(String(t.failures || "0"));
        if ((s + f) >= 5 && s === 0 && f > 0) {
          await db.execute(sql`UPDATE heartbeat_tasks SET enabled = false WHERE id = ${t.id} AND tenant_id = ${tenantId}`);
          disabled++;
    ...
    case "throttle_tasks": {
      ...
      if (spend.metrics.spend_percent >= 80) {
        ...
        `);
        const throttled = nonCritical.rowCount || 0;
        if (throttled > 0) {
          await postToChannel(tenantId, config.notify_channel || "#system-alerts",
    ...
    case "restart_agent": {
      ...
      const deskResult = await db.execute<DeskRestartRow>(sql`
      ...
      if (stalled.length > 0) {
        await postToChannel(tenantId, config.notify_channel || "#system-alerts",
          `[Governor] Reset ${stalled.length} stalled agent desk(s): ${stalled.map(s => s.name).join(", ")}`);
      }
    ...
    case "route_alert": {
      ...
      const alertResult = await db.execute<AlertRow>(sql`
        ...
        LEFT JOIN watchlist_items wi ON wi.id = wa.watchlist_item_id AND wi.tenant_id = ${tenantId}
    ...
    case "kill_switch": {
      ...
      `);
      const killed = killResult.rowCount || 0;
    ...
    case "cancel_pending_jobs": {
      ...
      try {
        ...
        return `cancel_pending_jobs failed: ${errorMessage(e).slice(0, 200)}`;
      }
    }
    ...
    case "review_frameworks": {
      const fwResult = await db.execute<FrameworkRow>(sql`
        SELECT * FROM governance_frameworks
        WHERE tenant_id = ${tenantId} AND status = 'active' AND next_review_date < NOW()
      `);
      const overdue = fwResult.rows;
      if (overdue.length === 0) return "No frameworks overdue for review";
      if (dryRun) return `Would review ${overdue.length} overdue framework(s): ${overdue.map(f => f.name).join(", ")}`;

      const results: string[] = [];
      for (const fw of overdue) {
        try {
          const principles = stringArray(fw.key_principles);
          const rulesInformed = stringArray(fw.rules_informed);
...
Respond in this exact JSON format (no markdown, no code blocks, just raw JSON):
{
  ...
  "next_review_months": 6
          ...
          let review: FrameworkReview | null;
          try {
            const jsonMatch = content.match(/\{[\s\S]*\}/);
            review = jsonMatch ? parseFrameworkReview(JSON.parse(jsonMatch[0])) : null;
          } catch { review = null; }
          ...
          await db.execute(sql`
            UPDATE governance_frameworks SET
              ...
              updated_at = NOW(),
              review_notes = ${`Auto-review ${new Date().toISOString().split("T")[0]}: Review failed — ${message}. Retry in 1 month.`}
            WHERE id = ${fw.id} AND tenant_id = ${tenantId}
          `);
          results.push(`${fw.name}: review FAILED — ${message}`);
        }
      }
...
async function postToChannel(tenantId: number, channelName: string, message: string) {
  try {
    const channelResult = await db.execute<ChannelRow>(sql`
    ...
    console.log(`[governor] Failed to post to ${channelName}: ${errorMessage(err)}`);
  }
}
  ...
  const checks: Array<{ eventType: string; desc: string; queryFn: () => Promise<number> }> = [
    {
      ...
      queryFn: async () => {
        const r = await db.execute<CountRow>(sql`SELECT COUNT(*) as cnt FROM tenants WHERE id = ${tenantId} AND stripe_customer_id IS NOT NULL AND plan != 'trial'`);
        return parseInt(String(r.rows[0]?.cnt || "0"));
      },
    },
    {
    ...
    },
    {
      ...
      queryFn: async () => {
        const r = await db.execute<CountRow>(sql`SELECT COUNT(*) as cnt FROM tenants WHERE id = ${tenantId} AND agentmail_inbox_id IS NOT NULL`);
        return parseInt(String(r.rows[0]?.cnt || "0"));
      },
    },
    {
      ...
      queryFn: async () => {
        const r = await db.execute<CountRow>(sql`SELECT COUNT(*) as cnt FROM heartbeat_tasks WHERE tenant_id = ${tenantId} AND enabled = true AND (name ILIKE '%content%' OR name ILIKE '%publish%' OR name ILIKE '%blog%' OR name ILIKE '%social%')`);
        return parseInt(String(r.rows[0]?.cnt || "0"));
      },
    },
  ];
...
export async function getGovernorStatus(tenantId: number = 1): Promise<{
  ...
  recentActions: GovernanceActionRow[];
  checks: ActivityCheckResult[];
}> {
  const subsResult = await db.execute<SubscriptionStatusRow>(sql`SELECT event_type, enabled FROM event_subscriptions WHERE tenant_id = ${tenantId}`);
  const subs = subsResult.rows;

  const rulesResult = await db.execute<RuleCategoryRow>(sql`SELECT category, COUNT(*) as cnt FROM governance_rules WHERE tenant_id = ${tenantId} AND enabled = true GROUP BY category`);
  const ruleCats = rulesResult.rows;
  const rulesByCategory: Record<string, number> = {};
  let totalRules = 0;
  for (const r of ruleCats) {
    rulesByCategory[r.category] = parseInt(String(r.cnt));
    totalRules += parseInt(String(r.cnt));
  }

  const actionsResult = await db.execute<GovernanceActionRow>(sql`
  ...
  `);
  const recentActions = actionsResult.rows;

  let lastRunAt: string | null = null;
  try {
    const logResult = await db.execute<LastRunRow>(sql`
...
export async function getRules(tenantId: number): Promise<GovernanceRuleListRow[]> {
  const result = await db.execute<GovernanceRuleListRow>(sql`
...
export async function getActionHistory(tenantId: number, limit: number = 50): Promise<GovernanceActionHistoryRow[]> {
  const result = await db.execute<GovernanceActionHistoryRow>(sql`
    SELECT ga.*, gr.description as rule_description
    FROM governance_actions ga
    LEFT JOIN governance_rules gr ON gr.id = ga.rule_id AND gr.tenant_id = ga.tenant_id
