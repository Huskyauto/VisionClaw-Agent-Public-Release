import { MODEL_REGISTRY, type ModelInfo, getClientForModel, markSubscriptionFailed, markProviderUnhealthy, getUnhealthyProviders, resetProviderHealth, LAST_RESORT_MODEL, RETIRED_MODEL_IDS, isSyntheticEvaluationModel } from "./providers";
...
export function findFallbackModel(
  ...
  // LAST-RESORT guard (Bob 2026-07-01): the super-expert escalation model is
  // NEVER a generic failover candidate — it is METERED and reachable ONLY via
  // resilientChatCompletion's explicit terminal rung (claim-before-spend,
  // daily-capped). Excluding it here covers every generic pool consumer
  // (pickNextModel, getClientWithFailover) in one place.
  // Registry entries such as "auto" are UI/routing selectors, not callable
  // provider model IDs. Letting one into this pool causes recovery itself to
  // fail with "Model 'auto' is not supported" after a real provider outage.
  const base = availableModels.filter(
    (m) =>
      m.id !== LAST_RESORT_MODEL &&
  ...
  const candidates = excludeProviders
    ? base.filter((m) => !excludeProviders.has(m.provider))
    : base;
