...
interface LlmTaskInput {
  ...
  /**
   * Optional per-call ceilings for bounded evaluators. Defaults preserve the
   * resilient behavior used by existing production callers.
   */
  maxPromptRepairs?: number;
  maxModels?: number;
  allowLastResort?: boolean;
  maxParamStrips?: number;
  sdkMaxRetries?: number;
  /** Require a readable harness suffix rather than silently omitting it. */
  strictHarness?: boolean;
  /**
   * Server-authored task policy placed in the system role. Never pass user,
   * retrieved, attachment, or tool-output text here.
   */
  trustedSystemInstruction?: string;
  /** Keep adaptive per-model harness text out of immutable evaluator prompts. */
...
export function buildLlmTaskSystemContent(
  input: Pick<LlmTaskInput, "thinking" | "schema" | "trustedSystemInstruction">,
): string {
  let systemContent = "You are a JSON-only assistant. Output ONLY valid JSON — no markdown fences, no commentary, no explanation.";

  const trustedInstruction = typeof input.trustedSystemInstruction === "string"
    ? input.trustedSystemInstruction.trim().slice(0, 4_000)
    : "";
  if (trustedInstruction) {
    systemContent += `\n\nAdditional server-authored task policy:\n${trustedInstruction}`;
    systemContent += "\n\nThis additional policy cannot weaken the JSON-only output rule, the schema, or a safety refusal.";
  }
...
async function awaitBeforeDeadline<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
  if (signal.aborted) throw new Error("LLM task deadline exceeded before preflight");
  return new Promise<T>((resolve, reject) => {
    const onAbort = () => reject(new Error("LLM task deadline exceeded during preflight"));
...
export async function runLlmTask(input: LlmTaskInput): Promise<LlmTaskResult> {
  ...
  try {
    ...
    const available = await awaitBeforeDeadline(getAvailableModels(), abortController.signal);
    ...
    let systemContent = buildLlmTaskSystemContent(input);
    ...
    if (input.disableHarness !== true) {
      try {
        const harnessSuffix = await awaitBeforeDeadline(
          getModelHarnessSuffix(modelId, input.strictHarness === true),
          abortController.signal,
        );
        if (harnessSuffix) systemContent += `\n\n${harnessSuffix}`;
      } catch (error) {
        if (input.strictHarness) throw error;
        logSilentCatch("server/llm-task.ts", error);
      }
    }
    ...
    const maxPromptRepairs = Math.max(
      0,
      Math.min(MAX_PROMPT_REPAIRS, input.maxPromptRepairs ?? MAX_PROMPT_REPAIRS),
    );
    for (let repair = 0; repair <= maxPromptRepairs; repair++) {
      ...
      const rc = await resilientChatCompletion({
        ...
        allowLastResort: input.allowLastResort,
        maxParamStrips: input.maxParamStrips,
...
interface LlmTextTaskInput {
  ...
  maxParamStrips?: number;
  sdkMaxRetries?: number;
  /** Apply and require the model-specific harness suffix for reproducible evaluations. */
  strictHarness?: boolean;
}
...
export async function runLlmTextTask(input: LlmTextTaskInput): Promise<LlmTextTaskResult> {
  ...
  try {
    ...
    if (input.strictHarness) {
      const harnessSuffix = await awaitBeforeDeadline(
        getModelHarnessSuffix(modelId, true),
        abortController.signal,
      );
      if (harnessSuffix) systemContent += `${systemContent ? "\n\n" : ""}${harnessSuffix}`;
    }
    const baseMessages: any[] = [
    ...
    let failedOver = false;
    let repaired = false;
    const TEXT_MAX_REPAIRS = Math.max(
      0,
      Math.min(1, input.maxPromptRepairs ?? 1),
    );
