...
export const heartbeatDelegateSchema = z.object({
  ...
  model: z.string().trim().min(1).max(100).optional().default("gpt-5.4"),
});
...
export function createCsrfMiddleware(
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Archive Rescue Admin status update: authenticated by the x-admin-pin
    // header (timingSafeEqual vs ADMIN_PIN + lockout throttle in
    // server/routes/archive-rescue.ts) — a per-request secret the attacker
    // can't make a victim's browser attach, so it's a stronger authenticator
    // than CSRF. The page has no session, so a session-bound CSRF token is
    // unobtainable. Anchored to exactly one id segment.
    if (/^\/api\/admin\/archive-rescue\/orders\/[^/]+\/status$/.test(req.path)) {
      return next();
    }
