...
const INJECTION_PATTERNS: Array<{
  ...
  { pattern: /(?:here\s+(?:are|is)|these\s+are|below\s+(?:are|is)|the\s+following\s+are)\s+(?:your\s+)?(?:new|updated|revised|real|actual|true|correct)\s+(?:instructions?|rules?|guidelines?|directives?)/i, type: "prompt_injection", name: "new_instructions_framing", severity: "high" },
  ...
  // Markdown / HTML image exfiltration: injected content embeds an image whose
  // URL carries a query string, so a rendering client silently GETs an external
  // URL and leaks whatever data was interpolated into the query. Medium (not
  // critical) — legitimate images can carry query strings, so this annotates +
  // raises the risk score without hard-blocking. (Fable 5 audit 2026-07-01.)
  { pattern: /!\[[^\]]*\]\(\s*https?:\/\/[^)\s]+\?[^)\s]+\)/i, type: "exfiltration", name: "markdown_image_exfil", severity: "medium" },
  { pattern: /<\s*img\b[^>]*\bsrc\s*=\s*["']https?:\/\/[^"']+\?[^"']+["']/i, type: "exfiltration", name: "html_image_exfil", severity: "medium" },
