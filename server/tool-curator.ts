...
// ─── Contrastive tool-choice (R125+ agentic-reasoning survey nugget) ───
/**
 * When two or three surfaced tools have overlapping hint matches for the
 * same message (e.g. web_search vs deep_research vs web_fetch), the model
 * often grabs the first plausible name. This builds a compact, deterministic
 * "compare before committing" note from the tools' useWhen/antiPatterns so
 * the model contrasts the candidates instead of pattern-matching on names.
 * $0 — no LLM call; pure hint-corpus lookup. Fires ONLY on genuine
 * ambiguity: ≥2 surfaced hinted tools whose hint scores are close.
 */
export function buildContrastiveNote(
  userMessage: string,
  surfacedToolNames: Iterable<string>,
): string {
  try {
    const surfaced = new Set(surfacedToolNames);
    const hintMatches = matchHintsToMessage(userMessage);
    const candidates = [...hintMatches.entries()]
      .filter(([name]) => surfaced.has(name))
      .sort((a, b) => b[1] - a[1]);
    if (candidates.length < 2) return "";
    const [topName, topScore] = candidates[0];
    const [, secondScore] = candidates[1];
    // Ambiguity gate: a meaningful top signal AND a runner-up within 60% of
    // it. A dominant winner (or weak noise matches) needs no comparison.
    if (topScore < 4 || secondScore < topScore * 0.6) return "";
    const contenders = candidates
      .slice(0, 3)
      .filter(([, s]) => s >= topScore * 0.6);
    ...
    for (const [name] of contenders) {
      ...
      if (!hint) continue;
      const use = (hint.useWhen || []).slice(0, 2).join("; ");
      const anti = (hint.antiPatterns || []).slice(0, 2).join("; ");
      lines.push(`- ${name}: use when ${use}.${anti ? ` NOT for: ${anti}.` : ""}`);
    }
    if (lines.length < 2) return "";
    const note =
      `\n\nTOOL CHOICE — this request matches several similar tools (${contenders.map(([n]) => n).join(", ")}). ` +
      `Briefly compare them against the user's actual need BEFORE committing to a call — do not pick by name familiarity:\n` +
      lines.join("\n");
    // Hard budget so prompt bloat is impossible even with verbose hints.
    return note.length > 1200 ? note.slice(0, 1200) : note;
  } catch {
    return "";
  }
}
