...
import { buildRecentUserRoutingContext, getMandatoryContextTools } from "./lib/tool-routing-context";
    ...
    try {
      ...
      try {
        const result = await runWithPreparationDeadline(
          () => getClientForModel(model, conv.tenantId, { requiresTools: true }),
        ...
        for (const [, tc] of Object.entries(toolCallBuffers)) {
          ...
          try {
            ...
            result = await executeGuardedTool(tc.name, parsedArgs, {
              ...
              authoritativeSourceLock: getMandatoryContextTools(
                buildRecentUserRoutingContext(apiMessages),
              ).some((tool) => tool === "discover_cmmc_prospects" || tool === "lookup_sam_exact_company" || tool === "lookup_sam_company_cage") ? "cmmc_sam" : undefined,
