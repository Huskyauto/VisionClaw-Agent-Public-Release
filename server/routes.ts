...
import { validate, stripeCheckoutSchema, presenterSessionSchema, contactFormSchema, triggerSchema, channelRouteSchema, personalityFileSchema, marketplaceInstallSchema, toggleSchema, scheduledPostCreateSchema, emptyBodySchema, createCsrfMiddleware, generateCsrfToken, getCsrfSessionKey } from "./validation";
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.use("/uploads", async (req: Request, res: Response, next: express.NextFunction) => {
    ...
    try {
      ...
      const [stored] = await db.select({ tenantId: fileStorage.tenantId })
        .from(fileStorage).where(eq(fileStorage.filename, baseName)).limit(1);
      if (stored && stored.tenantId && stored.tenantId !== authedTenantId) {
  ...
  const hitlPage = (status: "ok" | "error" | "confirm", title: string, body: string, opts?: { token?: string; decision?: "approve" | "deny"; cid?: string }): string => {
    ...
    if (status === "confirm" && opts?.token && opts?.decision) {
      ...
      const action = opts.decision === "approve" ? "/api/hitl/approve" : "/api/hitl/deny";
  ...
  app.post("/api/public/stripe/checkout", validate(stripeCheckoutSchema), async (req: Request, res: Response) => {
    try {
      ...
      // enterprise plan). Restrict to prices we have synced into
      // stripe.prices that are explicitly active AND whose parent product is
      // also active.
      {
        ...
        if (((allow as any).rows || allow).length === 0) {
          return res.status(400).json({ error: "Unknown or inactive priceId" });
        }
      }
    ...
    try {
      ...
      if (isApp) {
        ...
        if (host) {
          ...
          appPlayLink = `${proto}://${host}/uploads/delivery-${log.id}-${safeName}`;
        }
      }
      res.json({
        sessionId,
...
<p style="line-height:1.6;color:#888;font-size:13px;">This code expires in 15 minutes and can only be used once. If you didn't request it, you can ignore this email — no changes were made.</p>
  ...
  app.post("/api/conversations/:id/messages", async (req, res) => {
    ...
    const { prompt: basePrompt, injectedMemoryIds, citations: collectedCitations } = await buildSystemPrompt(
...
${buildFelixProtocol()}`
      ...
      round_loop: for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
        ...
        try {
          ...
          stream = await activeClient.chat.completions.create(createParams, { signal: streamAbortController.signal });
  ...
  app.get("/api/admin/ecosystem-health", authMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: e?.message || "ecosystem-health compute failed" });
    }
  });
