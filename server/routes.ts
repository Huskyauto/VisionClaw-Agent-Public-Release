...
import { authMiddleware, getSession, getTenantFromRequest, getTenantFromRequestAsync, isAdminRequest, requirePlatformAdmin, isPlatformAdmin, ADMIN_TENANT_ID, loadSessionsFromDb, revokeTenantSessions } from "./auth";
...
const PROMPT_CONTEXT_TIMEOUT_MS = parseTimeoutMs(process.env.PROMPT_CONTEXT_TIMEOUT_MS, 9_000);
const PROJECT_CONTEXT_TIMEOUT_MS = parseTimeoutMs(process.env.PROJECT_CONTEXT_TIMEOUT_MS, 5_000);
const ATTACHMENT_CONTEXT_TIMEOUT_MS = parseTimeoutMs(process.env.ATTACHMENT_CONTEXT_TIMEOUT_MS, 5_000);
const MODEL_CLIENT_INIT_TIMEOUT_MS = parseTimeoutMs(process.env.MODEL_CLIENT_INIT_TIMEOUT_MS, 5_000);
  ...
  try {
    ...
    for (const email of testEmails) {
      ...
      for (const row of rows) {
        ...
        await revokeTenantSessions(tid);
...
<p style="line-height:1.6;color:#888;font-size:13px;">This code expires in 15 minutes and can only be used once. If you didn't request it, you can ignore this email — no changes were made.</p>
  ...
  app.post("/api/conversations/:id/messages", async (req, res) => {
    ...
    const sendStatus = (phase: string) => {
      ...
      try { res.write(`data: ${JSON.stringify({ type: "status", phase })}\n\n`); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
    };
    ...
    const ROUTING_HARD_TIMEOUT_MS = parseTimeoutMs(process.env.ROUTING_HARD_TIMEOUT_MS, 12_000);
          ...
          for (const f of atts) {
            ...
            if (gdocMatch || f.type === "application/vnd.google-apps.document") {
              try {
                ...
                if (docId) {
                  const gdocText = await runAttachmentPreparation(
                    ...
                    async (signal) => {
                      ...
                      return gdocResp.ok ? gdocResp.text() : "";
                    },
                  );
                  if (gdocText) {
    ...
    try {
      ...
      try {
        ...
        if (streamAbortController.signal.aborted || primaryErr?.name === "AbortError") {
          throw primaryErr;
        }
        ...
        for (let attempt = 0; attempt < 5 && !resolved; attempt++) {
          ...
          try {
            ...
            if (streamAbortController.signal.aborted || fbErr?.name === "AbortError") {
              throw fbErr;
            }
        ...
        if (ladder.fits) {
          ...
          try {
            ...
            if (compactionResult.compacted && compactionResult.summary) {
              ...
              finalChatMessages = buildCompactedMessages(compactionResult.summary, toKeep, conversationId, compactionResult.archive);
