...
<p style="line-height:1.6;color:#888;font-size:13px;">This code expires in 15 minutes and can only be used once. If you didn't request it, you can ignore this email — no changes were made.</p>
  ...
  app.post("/api/conversations/:id/messages", async (req, res) => {
    ...
    let model = await selectTenantValidatedModel({
      requestedModel: normalizeModelId(conv.model || "gpt-5.6-sol"),
      tenantId,
    ...
    // self-guarded (falls back to gpt-5.6-sol on failure, never rejects); a
    ...
    const autoRouteRaw = (async (): Promise<AutoRouteDecision | null> => {
      ...
      try {
        ...
        console.error("[auto-router] Classification failed, using gpt-5.6-sol:", err);
        return { modelId: "gpt-5.6-sol", label: "GPT-5.6 Sol", reason: "Fallback", category: "general" };
      }
    })();
    ...
    const routingTimeoutP = new Promise<AutoRouteDecision | null>((resolve) => {
      routingTimer = setTimeout(() => {
        if (model === "auto") {
          console.warn(`[auto-router] Routing stage exceeded ${ROUTING_HARD_TIMEOUT_MS}ms — failing open to gpt-5.6-sol`);
          resolve({ modelId: "gpt-5.6-sol", label: "GPT-5.6 Sol", reason: "Routing timed out — fail-open default", category: "general" });
        } else {
          resolve(null);
        }
    ...
    try {
      ...
      const prompt = buildFastStartPrompt(persona, settings);
      promptBundle = {
        prompt,
        stablePrompt: prompt,
        dynamicPrompt: "",
