      ...
      round_loop: for (let round = 0; round <= maxToolRound; round++) {
              ...
              if (!inThinkBlock) {
                ...
                if (openIdx === -1) {
                  if (thinkBuffer.length > 10) {
                    ...
                    if (!outcomeEnabled) res.write(`data: ${JSON.stringify({ content: safe })}\n\n`);
                    thinkBuffer = thinkBuffer.slice(safe.length);
                  }
                  break;
                } else {
                  if (openIdx > 0) {
                    if (!outcomeEnabled) res.write(`data: ${JSON.stringify({ content: thinkBuffer.slice(0, openIdx) })}\n\n`);
                  }
            ...
            try {
              ...
              for await (const rChunk of recoveryStream) {
                ...
                if (rDelta) {
                  roundContent += rDelta;
                  fullResponse += rDelta;
                  if (!outcomeEnabled) res.write(`data: ${JSON.stringify({ content: rDelta })}\n\n`);
                }
              }
              if (roundContent) {
        ...
        if (isThinkingMode && thinkBuffer.length > 0) {
          if (inThinkBlock) {
            ...
            if (!outcomeEnabled) res.write(`data: ${JSON.stringify({ content: thinkBuffer })}\n\n`);
          }
        }
