      ...
      if (persona?.id === 2 && executedTools.length > 0) {
        try {
          ...
          if (incompleteOutcome) {
            ...
            const cgResult = await getClientForModel(HIGH_END_WORK_MODEL_ID, conv.tenantId);
            const cgClient = cgResult.client;
            const cgModel = cgResult.actualModelId;
          ...
          console.warn(`[completion-gate] SSE: Rebuild failed: ${errorMessage(cgErr)}`);
        }
      }
