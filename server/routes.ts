...
import { getRecencyTier } from "./lib/recency-tier";
...
import { DEFAULT_CHAT_MODEL_ID, DEFAULT_CHAT_MODEL_LABEL, HIGH_END_WORK_MODEL_ID } from "./chat-model-default";
import { getConversationForPlatformAdmin } from "./platform-admin-conversations";
import { seedDatabase } from "./seed";
import { ownerTenantId } from "./agentic/autonomous-budget";
// Legacy insert schemas live in their domain route modules.
...
import { isOwnerOnlyFinalDecisionModel } from "./model-registry";
import { splitSystemForCache } from "./anthropic-prompt-cache";
import { getClientForModel, getAvailableModels, getAvailableModelsForTenant, MODEL_REGISTRY, normalizeModelId, replitOpenai, getMaxOutputTokens, maskApiKey, markSubscriptionFailed, markProviderUnhealthy, getUnhealthyProviders, resetProviderHealth } from "./providers";
...
import { assessFelixOutcome } from "./felix-outcome-controller";
import { detectIncompleteOutcome, isAttachmentReviewRequest } from "./chat-response-validation";
import { createToolMarkupStreamFilter, stripXmlToolCallMarkup } from "./chat-tool-call-parser";
...
import { authMiddleware, getSession, getTenantFromRequest, getTenantFromRequestAsync, isAdminRequest, requirePlatformAdmin, isPlatformAdmin, ADMIN_TENANT_ID, loadSessionsFromDb, clearSessionsForTenant } from "./auth";
...
import { warnIfPartialBotConfig } from "./lib/bot-protection";
...
import { registerHumanAiSynergyRoutes } from "./routes/human-ai-synergy";
import { registerCommercialResearchFrontierRoutes } from "./routes/commercial-research-frontier";
import {
  MAX_STARTUP_UPLOAD_RESTORE_FILES,
  shouldRestoreStartupUpload,
} from "./lib/upload-restore-budget";
...
import { validate, stripeCheckoutSchema, presenterSessionSchema, contactFormSchema, triggerSchema, channelRouteSchema, personalityFileSchema, marketplaceInstallSchema, toggleSchema, scheduledPostCreateSchema, emptyBodySchema, messageFeedbackSchema, createCsrfMiddleware } from "./validation";
...
import { registerRepairHandoffRoute } from "./source-repair-handoff";
import { registerEventsRoutes } from "./routes/events";
import { registerGoalLedgerRoutes } from "./routes/goal-ledger";
import { registerSandboxRoutes } from "./routes/sandbox";
import { registerLoopPortfolioRoutes } from "./routes/loop-portfolio";
...
import { registerAuditRoutes, registerAuditAdminRoutes } from "./routes/audit";
import { registerIncomeAdminRoutes } from "./routes/income-admin";
import { registerCmmcAdminRoutes, registerCmmcPublicRoutes } from "./routes/cmmc";
...
import { registerAgentInsightsRoutes } from "./routes/agent-insights";
import { registerDreamGraphRoutes } from "./routes/dream-graph";
...
import { registerRecoveryManifestRoutes } from "./routes/recovery-manifests";
import { registerTenantBYOKRoutes } from "./routes/tenant-byok";
import { registerMemoryRoutes } from "./routes/memory";
import { registerResearchRoutes } from "./routes/research";
import { registerAdminRoutes } from "./routes/admin";
import { registerAuthPublicRoutes, registerAuthMonitoringRoutes } from "./routes/auth-extra";
import { registerTenantsPublicRoutes, registerTenantFilesRoutes, registerTemplatesRoutes } from "./routes/tenants";
import { registerArtifactRoutes } from "./routes/artifacts";
import { registerAdminExtraRoutes, registerAccountRoutes } from "./routes/admin-extra";
...
import { PreparationDeadlineExceededError, runWithPreparationDeadline } from "./lib/preparation-deadline";
import { selectTenantValidatedModel } from "./lib/tenant-model-access";
...
import { autoRouteModel, listRouteCategories, parseTimeoutMs } from "./auto-router";
...
import { SAFE_EXTENSIONS, UPLOADS_DIR, detectAndValidateUpload, extractTextFromFile, upload, uploadLarge, validateUploadedFile } from "./upload-handling";
...
fs.accessSync(UPLOADS_DIR, fs.constants.W_OK | fs.constants.X_OK);
...
(async function restoreUploadsFromDb() {
  try {
    ...
    ).limit(MAX_STARTUP_UPLOAD_RESTORE_FILES);
    let restored = 0;
    let restoredBytes = 0;
    ...
    for (const row of rows) {
      ...
      if (!fs.existsSync(fp)) {
        const decision = shouldRestoreStartupUpload(restoredBytes, row.data);
        if (!decision.restore) {
          skippedForBudget++;
          continue;
        }
        try {
          ...
          restored++;
          restoredBytes += decision.bytes;
        } catch (err) {
          console.warn(`[uploads] Failed to restore ${row.filename}: ${errorMessage(err)}`);
        }
      }
    }
    if (restored > 0) console.log(`[uploads] Restored ${restored} image(s), ${(restoredBytes / 1024 / 1024).toFixed(1)} MB from DB`);
    if (skippedForBudget > 0) console.log(`[uploads] Skipped ${skippedForBudget} image(s): startup restore budget reached`);
  } catch (err) {
    console.warn(`[uploads] DB-to-disk restore skipped: ${errorMessage(err)}`);
  }
})();
...
// First useful action must arrive promptly. Stream establishment is only a
// connection handshake; long generation belongs to the separate first-chunk
// and iteration limits below. Waiting two minutes here made a dead provider
// look like Felix was "working" even though no work had started.
const STREAM_CREATE_TIMEOUT_MS = parseTimeoutMs(process.env.STREAM_CREATE_TIMEOUT_MS, 12_000);
const CONTEXT_LOAD_TIMEOUT_MS = parseTimeoutMs(process.env.CONTEXT_LOAD_TIMEOUT_MS, 3_000);
const PROMPT_CONTEXT_TIMEOUT_MS = parseTimeoutMs(process.env.PROMPT_CONTEXT_TIMEOUT_MS, 6_000);
const PROJECT_CONTEXT_TIMEOUT_MS = parseTimeoutMs(process.env.PROJECT_CONTEXT_TIMEOUT_MS, 4_000);
const ATTACHMENT_CONTEXT_TIMEOUT_MS = parseTimeoutMs(process.env.ATTACHMENT_CONTEXT_TIMEOUT_MS, 4_000);
const MODEL_CLIENT_INIT_TIMEOUT_MS = parseTimeoutMs(process.env.MODEL_CLIENT_INIT_TIMEOUT_MS, 3_000);
const MAX_INLINE_IMAGE_BYTES = 8 * 1024 * 1024;

function buildFastStartPrompt(persona: any, settings: any): string {
  const name = persona?.name || settings?.platformName || "the agent";
  const role = persona?.role || "autonomous assistant";
  return `## FAST START MODE
You are ${name}, ${role}. Optional context was unavailable within the interactive response budget.

Begin useful work immediately. Use tools when they are needed, follow all server-enforced safety and approval rules, preserve tenant boundaries, never expose secrets, and be transparent if optional context would be needed to complete the request.`;
}
...
(async function ensureOrderLookupCodesTable() {
  try {
    ...
    `);
    // Existing single-store deployments predate tenant-bound challenges.
    // Backfill only the trusted storefront tenant, then require every new
    // challenge to retain its originating tenant.
    await db.execute(sql`ALTER TABLE order_lookup_codes ADD COLUMN IF NOT EXISTS tenant_id INTEGER`);
    await db.execute(sql`UPDATE order_lookup_codes SET tenant_id = ${ownerTenantId()} WHERE tenant_id IS NULL`);
    await db.execute(sql`ALTER TABLE order_lookup_codes ALTER COLUMN tenant_id SET NOT NULL`);
  } catch (err: unknown) {
    console.error("[order-lookup] failed to ensure table:", errorMessage(err));
  }
})();
...
async function pruneOrderLookupCodes() {
  try {
    ...
    console.warn(`[order-lookup] prune failed: ${errorMessage(err) || err}`);
  }
}
  ...
  try {
    ...
    for (const email of testEmails) {
      ...
      for (const row of rows) {
        ...
        await clearSessionsForTenant(tid);
...
async function backfillProjectDriveFolders() {
  try {
    ...
    `);
    // Legacy bootstrap columns used DEFAULT 1. All current writers supply
    // their resolved tenant explicitly, so remove the misattribution fallback.
    await db.execute(sql`ALTER TABLE felix_loop_runs ALTER COLUMN tenant_id DROP DEFAULT`);
    await db.execute(sql`ALTER TABLE felix_proposals ALTER COLUMN tenant_id DROP DEFAULT`);
    ...
    for (const p of projects) {
      try {
        ...
        console.warn(`[drive-backfill] Failed for project ${p.id} (${p.name}): ${errorMessage(err)}`);
      }
    }
    console.log(`[drive-backfill] Backfill complete: ${created}/${projects.length} project Drive folders created`);
  } catch (err: unknown) {
    console.warn(`[drive-backfill] Backfill failed: ${errorMessage(err)}`);
  }
}

export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  // The public storefront is owned by this trusted, server-configured tenant;
  // public callers must never select the tenant that order recovery queries.
  ...
  try {
    ...
    console.warn("[startup] Glasses gateway not mounted:", errorMessage(e)?.slice(0, 120));
  }
  ...
  try {
    ...
    console.warn("[startup] Tool registry audit skipped:", errorMessage(e)?.slice(0, 100));
  }

  import("./data-protection").then(dp => dp.ensureDataProtectionColumns()).catch(e => console.warn("[startup] data-protection columns:", errorMessage(e)));
  ...
  if (process.env.ISOLATED_STAGING !== "1") {
    ...
    console.log("[staging] Route-layer channels, backfills, heartbeat, and background engines disabled");
  }
  ...
  // Prometheus /metrics — token-gated (METRICS_TOKEN), fail-closed 404 when
  // unset. Mounted alongside /healthz (before auth/CSRF) because scrapers
  // authenticate with a Bearer token, not a session. Aggregate numbers only.
  try {
    const { registerMetricsEndpoint } = await import("./routes/metrics");
    registerMetricsEndpoint(app);
  } catch (e: unknown) {
    // Full error object (stack + cause), not message-only — boot-time mount
    // failures are exactly where the stack matters most.
    console.error("[startup] /metrics endpoint not mounted:", e);
  }
  ...
  app.post("/api/cron/weekly-maintenance", async (req: Request, res: Response) => {
    ...
    try {
      ...
      runWeeklyMaintenance().catch((e) => console.error("[cron] weekly maintenance error:", errorMessage(e)));
  ...
  app.get("/api/cron/repair-loop-health", async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(`[repair-loop-health] schema health probe failed: ${errorMessage(probeErr) || probeErr}`);
      }
  ...
  app.get("/api/cache/stats", async (req: Request, res: Response) => {
    ...
    // Task 124 — global system internals: platform admin only.
  ...
  app.get("/api/setup/status", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const adminAuthed = isPlatformAdmin(req); // Task 124 — env matrix is global
      const exposeMatrix = isFreshDeploy || adminAuthed;
      const typesafeJev = getTypeSafeJevStatus();

      if (!exposeMatrix) {
        const needsSetup = !hasDb;
        return res.json({ needsSetup, isFreshDeploy: false, typesafeJev });
      }
      ...
      const checks = {
        ...
        discord: hasDiscord,
        crypto: hasCoinbase,
        typesafeJev: typesafeJev.ready,
      };
      const needsSetup = !checks.database || !checks.aiProvider || isFreshDeploy;
      res.json({ needsSetup, isFreshDeploy, checks, typesafeJev });
    } catch (err: unknown) {
      res.status(500).json({ error: "Failed to check setup status" });
    }
  });

  app.get("/api/public/deliverable/:project/:file", (_req, res) => {
    // This legacy path exposed predictable on-disk project paths without
    // tenant ownership or an expiring capability. Deliveries now use signed
    // /uploads URLs, which bind the capability to an expiry and tenant.
    res.status(410).json({
      error: "Legacy public deliverable links are disabled. Use the signed delivery link instead.",
    });
  });
  ...
  // Routes-split Tier 1: login/status/csrf-token/password+email flows moved to
  // server/routes/auth-extra.ts. Registered HERE (pre-gate) so they stay ahead
  // of the /api authMiddleware gate below.
  registerAuthPublicRoutes(app, { loginLimiter, getTenantFromRequestAsync });
  registerCmmcPublicRoutes(app);
  ...
  const renderHitlConfirmPage = async (req: Request, res: Response) => {
    try {
      ...
      console.warn(`[hitl-link] GET error: ${errorMessage(err)}`);
  ...
  const handleHitlSubmit = async (req: Request, res: Response) => {
    try {
      ...
      console.warn(`[hitl-link] POST error: ${errorMessage(err)}`);
  ...
  const renderBwbConfirmPage = async (req: Request, res: Response) => {
    try {
      ...
      console.warn(`[bwb-link] GET error: ${errorMessage(err)}`);
      res.status(500).type("html").send(hitlPage("error", "Server error", "Something went wrong loading this approval page."));
    }
  };
  ...
  const handleBwbSubmit = async (req: Request, res: Response) => {
    try {
      ...
      console.warn(`[bwb-link] POST error: ${errorMessage(err)}`);
  ...
  // Routes-split Tier 1: tenants register/login/me/me-profile moved to
  // server/routes/tenants.ts. Registered HERE (pre-gate, original position).
  registerTenantsPublicRoutes(app, { loginLimiter, mutateLimiter, getTenantFromRequestAsync, ADMIN_TENANT_ID });
  ...
  app.get("/api/public/stats", authMiddleware, async (req, res) => {
    ...
    try {
      ...
      console.error("[public-stats] Error:", errorMessage(err));
      res.status(500).json({ error: "Failed to fetch stats" });
    }
  });

  app.get("/api/public/architecture", authMiddleware, async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    try {
      const { db } = await import("./db");
      const { sql: s } = await import("drizzle-orm");
      ...
      console.error("[architecture] Error:", errorMessage(err));
  ...
  app.get("/api/public/stripe/products", async (_req: Request, res: Response) => {
    try {
      ...
      console.error("[public-stripe] Products error:", errorMessage(err));
      res.json({ products: [] });
    }
  });
  ...
  app.post("/api/public/stripe/checkout", validate(stripeCheckoutSchema), async (req: Request, res: Response) => {
    try {
      ...
      console.error("[public-stripe] Checkout error:", errorMessage(err));
  ...
  app.get("/api/onedrive-health", async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ connected: false, error: errorMessage(e) });
    }
  });
  ...
  app.post("/api/admin-drive-upload", express.json({ limit: "10mb" }), async (req: Request, res: Response) => {
    try {
      ...
      if (session.tenantId !== ADMIN_TENANT_ID || !session.isAdmin) {
      ...
      // R125+137.23 — route through the peppered auth.ts comparator instead of
      // open-coding the public static salt (the last un-peppered PIN callsite).
      const { verifyAdminPinPlaintext } = await import("./auth");
      if (!verifyAdminPinPlaintext(String(pin || ""), adminPin)) {
  ...
  app.get("/api/presenter/:token/slide/:index", async (req: Request, res: Response) => {
    try {
      ...
      const sessionResult = await db.execute(sql`SELECT id, tenant_id FROM presenter_sessions WHERE token = ${token}`);
      if (sessionResult.rows.length === 0) return res.status(404).json({ error: "Session not found" });
      const sessionId = (sessionResult.rows[0] as any).id;
      const sessionTenantId = (sessionResult.rows[0] as any).tenant_id;

      const imgResult = await db.execute(sql`
        SELECT i.image_data, i.image_size
        FROM presenter_slide_images i
        INNER JOIN presenter_sessions s ON s.id = i.session_id
        WHERE i.session_id = ${sessionId} AND s.tenant_id = ${sessionTenantId}
          AND i.slide_index = ${slideIndex} AND i.quality = ${quality}
        LIMIT 1
      `);
      if (imgResult.rows.length === 0) {
        const fallback = await db.execute(sql`
          ...
          INNER JOIN presenter_sessions s ON s.id = i.session_id
          WHERE i.session_id = ${sessionId} AND s.tenant_id = ${sessionTenantId}
            AND i.slide_index = ${slideIndex}
          ORDER BY i.quality ASC
          LIMIT 1
        `);
  ...
  app.post("/api/presenter", express.json(), validate(presenterSessionSchema), async (req: Request, res: Response) => {
    try {
      ...
      // R-review MEDIUM hardening: body-supplied tenantId on this internal
      // (SESSION_SECRET-gated) endpoint must at least be a clean positive
      // integer — reject NaN/strings/objects fail-closed.
      const tenantIdNum = typeof tenantId === "number" && Number.isInteger(tenantId) && tenantId > 0 ? tenantId : null;
      if (!tenantIdNum) return res.status(400).json({ error: "tenantId is required (positive integer)" });
      const result = await db.execute(sql`INSERT INTO presenter_sessions (tenant_id, presentation_id, title, slides, embed_url, present_url, token) VALUES (${tenantIdNum}, ${presentationId}, ${title}, ${slidesJson}::jsonb, ${embedUrl || ""}, ${presentUrl || ""}, ${token}) RETURNING id, token`);
      const row = result.rows[0] as any;
  ...
  app.post("/api/presenter/:token/repair-images", express.json(), async (req: Request, res: Response) => {
    ...
    try {
      ...
      const sessionResult = await db.execute(sql`SELECT id, tenant_id, presentation_id, slides FROM presenter_sessions WHERE token = ${token}`);
      ...
      const existingImages = await db.execute(sql`
        SELECT i.slide_index
        FROM presenter_slide_images i
        INNER JOIN presenter_sessions s ON s.id = i.session_id
        WHERE i.session_id = ${sessionId} AND s.tenant_id = ${sessionTenantId}
          AND i.quality = 'full'
      ...
      for (let ti = 0; ti < pageIds.length && ti < totalSlides && ti < MAX_REPAIR_SLIDES; ti++) {
        ...
        try {
          ...
          if (imgBuf && imgBuf.length > 1000) {
            await db.execute(sql`
              INSERT INTO presenter_slide_images (session_id, slide_index, image_data, image_size, quality)
              SELECT s.id, ${ti}, ${imgBuf}, ${imgBuf.length}, 'full'
              FROM presenter_sessions s
              WHERE s.id = ${sessionId} AND s.tenant_id = ${sessionTenantId}
              ON CONFLICT (session_id, slide_index, quality) DO UPDATE
              SET image_data = EXCLUDED.image_data, image_size = EXCLUDED.image_size
              WHERE EXISTS (
                SELECT 1 FROM presenter_sessions owner
                WHERE owner.id = presenter_slide_images.session_id
                  AND owner.tenant_id = ${sessionTenantId}
              )
            `);
            downloaded++;
            if (sharp) {
              try {
                ...
                await db.execute(sql`
                  INSERT INTO presenter_slide_images (session_id, slide_index, image_data, image_size, quality)
                  SELECT s.id, ${ti}, ${thumbBuf}, ${thumbBuf.length}, 'thumb'
                  FROM presenter_sessions s
                  WHERE s.id = ${sessionId} AND s.tenant_id = ${sessionTenantId}
                  ON CONFLICT (session_id, slide_index, quality) DO UPDATE
                  SET image_data = EXCLUDED.image_data, image_size = EXCLUDED.image_size
      ...
      await db.execute(sql`
        UPDATE presenter_sessions SET slides = ${JSON.stringify(dbSlides)}::jsonb
        WHERE id = ${sessionId} AND tenant_id = ${sessionTenantId}
      `);
  ...
  app.get("/api/store/catalog", async (_req: Request, res: Response) => {
    try {
      ...
      console.error("[store] catalog error:", errorMessage(err));
  ...
  app.get("/api/store/order/:sessionId", async (req: Request, res: Response) => {
    try {
      ...
      const rows = await db
        ...
        .from(deliveryLogs)
        .where(and(
          eq(deliveryLogs.tenantId, storefrontTenantId),
          or(eq(deliveryLogs.orderId, sessionId), eq(deliveryLogs.stripePaymentId, sessionId)),
        ))
      ...
      console.error("[store] order lookup error:", errorMessage(err));
    ...
    setImmediate(async () => {
      try {
        ...
        const rows = await db
          ...
          .from(deliveryLogs)
          .where(and(
            sqlTag`lower(${deliveryLogs.customerEmail}) = ${rawEmail}`,
            eq(deliveryLogs.tenantId, storefrontTenantId),
          ))
          .limit(1);
        ...
        await db.execute(sql`
          INSERT INTO order_lookup_codes (email, tenant_id, code_hash, expires_at, attempts)
          VALUES (${rawEmail}, ${storefrontTenantId}, ${codeHash}, ${expiresAt.toISOString()}, 0)
...
<p style="line-height:1.6;color:#888;font-size:13px;">This code expires in 15 minutes and can only be used once. If you didn't request it, you can ignore this email — no changes were made.</p>
        ...
        try {
          ...
          console.error(`[store] order lookup code email failed for ${rawEmail}:`, errorMessage(sendErr));
        }
      } catch (err: unknown) {
        console.error("[store] order lookup (async) error:", errorMessage(err));
      }
    });
  });
  ...
  app.post("/api/store/verify-orders", orderVerifyLimiter, express.json(), async (req: Request, res: Response) => {
    ...
    const consumeRes: any = await db.execute(sql`
      ...
      WHERE email = ${rawEmail}
        AND tenant_id = ${storefrontTenantId}
    ...
    if (!consumed) {
      ...
      await db.execute(sql`
        WITH bumped AS (
          ...
          WHERE email = ${rawEmail} AND tenant_id = ${storefrontTenantId}
          RETURNING email, attempts
        )
        DELETE FROM order_lookup_codes
          WHERE tenant_id = ${storefrontTenantId} AND email IN (
          SELECT email FROM bumped WHERE attempts >= ${ORDER_LOOKUP_MAX_ATTEMPTS}
        )
           OR email IN (
             SELECT email FROM order_lookup_codes
             WHERE email = ${rawEmail} AND tenant_id = ${storefrontTenantId} AND expires_at <= NOW()
           )
      `);
    ...
    try {
      ...
      console.error("[store] order verify lookup error:", errorMessage(err));
  ...
  (async () => {
    try {
      ...
      `);
    } catch (e: unknown) { console.warn("[contact] Table init:", errorMessage(e)); }
  })();
  ...
  app.post("/api/public/contact", contactLimiter, validate(contactFormSchema), async (req: Request, res: Response) => {
    try {
      ...
      console.error("[contact] Submission error:", errorMessage(err));
  ...
  app.get("/uploads/:filename", async (req: Request, res: Response) => {
    ...
    let diskFallbackAuthorized = false;
    ...
    if (tenantId === undefined) {
      const bearer = req.headers.authorization?.replace("Bearer ", "") || "";
      const session = bearer ? await getSession(bearer) : null;
      if (!session) {
        return res.status(401).json({ error: "Authentication required" });
      }
  ...
  app.post("/api/vibevoice/transcribe", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      console.error("[vibevoice] Transcribe route error:", errorMessage(err));
  ...
  app.post("/api/upload-base64", authMiddleware, express.json({ limit: "50mb" }), async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // SECURITY (review fix): user uploads are a private vault backup — never
        // auto-create an anyone-with-link Drive share. Deliverables that must be
        // shared go through the explicit delivery path instead.
  ...
  app.post("/api/upload", authMiddleware, (req: Request, res: Response) => {
    ...
    upload.single("file")(req, res, async (err: any) => {
      if (err) {
        console.error("[upload] Multer error:", errorCode(err), errorMessage(err));
        if (errorCode(err) === "LIMIT_FILE_SIZE") {
          return res.status(413).json({ error: "File too large (max 50MB)" });
        }
        if (errorMessage(err)?.includes("File type not allowed")) {
          return res.status(400).json({ error: "File type not allowed" });
        }
        return res.status(400).json({ error: errorMessage(err) || "Upload failed" });
      }
      const file = req.file;
      if (!file) {
      ...
      try {
        ...
        const driveResult = await uploadAndShare({
          ...
          // SECURITY (review fix): private vault backup — no auto public share.
          share: false,
        });
  ...
  // Routes-split Tier 1: /api/tenant/files list/download/delete moved to
  // server/routes/tenants.ts.
  registerTenantFilesRoutes(app, { authMiddleware, mutateLimiter, getTenantFromRequest });

  app.post("/api/brand-logo", mutateLimiter, authMiddleware, (req: Request, res: Response) => {
    upload.single("file")(req, res, async (err: any) => {
      if (err) return res.status(400).json({ error: errorMessage(err) || "Upload failed" });
      const file = req.file;
      ...
      try {
        ...
        console.error("[brand-logo] Save failed:", errorMessage(e));
      }
      res.json({
  ...
  app.post("/api/telegram/connect", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: errorMessage(err) || "Failed to connect Telegram bot" });
    }
  });
  ...
  app.post("/api/telegram/disconnect", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: errorMessage(err) || "Failed to disconnect" });
    }
  });
  ...
  app.get("/api/triggers", async (req, res) => {
    ...
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    try { res.json(await listTriggers(tenantId)); } catch (err: unknown) { res.status(500).json({ error: errorMessage(err) }); }
  });
  ...
  app.post("/api/triggers", validate(triggerSchema), async (req, res) => {
    ...
    try {
      ...
      const trigger = await createTrigger({ name, description, personaId, tenantId });
      res.json(trigger);
    } catch (err: unknown) { res.status(500).json({ error: errorMessage(err) }); }
  });
  ...
  app.delete("/api/triggers/:id", async (req, res) => {
    ...
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    try { await deleteTrigger(parseInt(req.params.id as string), tenantId); res.json({ success: true }); }
    catch (err: unknown) { res.status(500).json({ error: errorMessage(err) }); }
  });
  ...
  app.post("/api/triggers/:id/toggle", validate(toggleSchema), async (req, res) => {
    ...
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    try { await toggleTrigger(parseInt(req.params.id as string), req.body.enabled, tenantId); res.json({ success: true }); }
    catch (err: unknown) { res.status(500).json({ error: errorMessage(err) }); }
  });
  ...
  app.get("/api/triggers/:id/events", async (req, res) => {
    ...
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    try { res.json(await getTriggerEvents(parseInt(req.params.id as string), tenantId)); }
    catch (err: unknown) { res.status(500).json({ error: errorMessage(err) }); }
  });
  ...
  app.get("/api/channel-routes", async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    try { res.json(await listChannelRoutes()); } catch (err: unknown) { res.status(500).json({ error: errorMessage(err) }); }
  });
  ...
  app.get("/api/delegation-events/:conversationId", authMiddleware, async (req: Request, res: Response) => {
    ...
    const conv = isPlatformAdmin(req)
      ? await getConversationForPlatformAdmin(req, conversationId)
      : await storage.getConversation(conversationId, tenantId);
    if (conv && conv.tenantId !== tenantId && !isPlatformAdmin(req)) {
  ...
  async function validateModelForTenant(modelId: string, tenantId: number, platformAdminVerified = false): Promise<boolean> {
    if (modelId === "auto") return true;
    if (isOwnerOnlyFinalDecisionModel(modelId) && !platformAdminVerified) return false;
  ...
  app.post("/api/messages/:id/feedback", validate(messageFeedbackSchema), async (req: Request, res: Response) => {
    try {
      ...
      const msgRow = await db.select({ conversationId: messages.conversationId })
        .from(messages).where(and(eq(messages.id, messageId), eq(messages.tenantId, tenantId))).limit(1);
      ...
      console.error("[message-feedback] error:", errorMessage(err) || err);
  ...
  app.post("/api/conversations/:id/messages", async (req, res) => {
    ...
    try {
      ...
      console.error("[rate-limit] gate error → fail-closed 503:", errorMessage(rateErr) || String(rateErr));
    ...
    if (conv.tenantId !== tenantId && !isPlatformAdmin(req)) {
      return res.status(403).json({ error: "Access denied" });
    }
    ...
    try {
      ...
      return res.status(429).json({ error: errorMessage(queueErr) || "Conversation is busy — please wait for the current message to finish" });
    }
    ...
    const { detectActionFirstModeCommand } = await import("./lib/action-first-mode");
    const requestedCommunicationMode = detectActionFirstModeCommand(String(content || ""));
    ...
    if (requestedCommunicationMode) {
      try {
        const { setExplicitCommunicationMode } = await import("./user-modeling");
        await setExplicitCommunicationMode(tenantId, requestedCommunicationMode);
        console.log(`[communication-mode] tenant=${tenantId} mode=${requestedCommunicationMode} source=explicit-chat-command`);
      } catch (modeErr: unknown) {
        console.error("[communication-mode] Explicit preference update failed:", modeErr);
        return res.status(500).json({ error: "Your message was saved, but communication mode could not be updated. Please try the command again." });
      }
    }
    ...
    // ---- EARLY SSE STREAM OPEN ----
    // Open the event stream BEFORE the heavy pre-flight (memory load, auto-route
    // classify, system-prompt build, project context). Previously headers weren't
    // flushed until after ALL of that ran, so the client sat on a bare
    // "Connecting to agent…" for the entire pre-flight (observed ~140s). Flushing
    // here makes the browser's fetch() resolve immediately and lets us stream
    // phase breadcrumbs while we prepare. The outer error handler already guards
    // every late failure with `if (!res.headersSent)` → SSE-error path, so this
    // is safe; the one pre-stream JSON error (unknown model) is converted to an
    // SSE error below.
    res.setHeader("Content-Type", "text/event-stream");
    res.setHeader("Cache-Control", "no-cache");
    res.setHeader("Connection", "keep-alive");
    ...
    // This controller exists before optional preparation so a dropped browser
    // cancels best-effort work as well as the provider stream opened later.
    const streamAbortController = new AbortController();
    const abortPreparationOnDisconnect = () => {
      streamAborted = true;
    ...
    req.once("close", abortPreparationOnDisconnect);
    req.once("aborted", abortPreparationOnDisconnect);
    res.once("close", abortPreparationOnDisconnect);
    res.once("error", abortPreparationOnDisconnect);
    try { res.flushHeaders(); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
    const sendStatus = (phase: string) => {
      if (streamAborted) return;
      const elapsedMs = Date.now() - preparationStartedAt;
      console.log(`[chat-preparation] phase=${phase} elapsedMs=${elapsedMs}`);
      try { res.write(`data: ${JSON.stringify({ type: "status", phase, elapsedMs })}\n\n`); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
    };
    ...
    if (streamAborted && globalKeepalive) {
      clearInterval(globalKeepalive);
      globalKeepalive = null;
    }
    sendStatus("connecting");

    // Owner web-chat invariant: a direct Built With Bob weekly-recap request is
    // execution, not planning prose. The route has already authenticated,
    // tenant-scoped, rate-limited, injection-scanned, persisted, and serialized
    // the request. Only a platform-admin owner request may borrow Felix's
    // protected builder identity; the helper independently verifies Felix and
    // supplies a semantic launch key so retries reuse one durable job.
    {
      const { launchBwbWeeklyRecapFromOwnerWebChat } = await import("./lib/bwb-web-chat-launch");
      const recapLaunch = await launchBwbWeeklyRecapFromOwnerWebChat(String(content || "").trim(), {
        ownerAuthorized: isPlatformAdmin(req),
        tenantId,
        sourcePersonaId: persona?.id ?? null,
        resolveFelix: async () => {
          const felix = await storage.getPersona(2).catch(() => null);
          return felix ? { id: felix.id, name: felix.name } : null;
        },
        enforcePolicy: async (felix) => {
          const { enforceToolPolicy } = await import("./safety/destructive-tool-policy");
          return enforceToolPolicy("bwb_weekly_build", {}, {
            tenantId,
            personaId: felix.id,
            personaName: felix.name,
            invokedVia: "owner-web-chat-recap",
            hasApproval: false,
          });
        },
        execute: async (_name, params) => {
          const { executeBwbWeeklyBuildFromOwnerWebChat } = await import("./lib/bwb-web-chat-tool-executor");
          return executeBwbWeeklyBuildFromOwnerWebChat(params);
        },
      });
      if (recapLaunch) {
        ...
        res.write(`data: ${JSON.stringify({ content: recapLaunch.response })}\n\n`);
        res.write(`data: ${JSON.stringify({ done: true, jobId: recapLaunch.jobId, watchProgressUrl: "/jobs" })}\n\n`);
        if (globalKeepalive) clearInterval(globalKeepalive);
        globalKeepalive = null;
        res.end();
        return;
      }
    ...
    // These reads are optional enrichment. A database pool stall must not hold
    // the interactive turn forever, and one failed read must not discard the
    // other context that arrived successfully. All three share a stage
    // deadline, so the slowest read determines the bounded wait rather than
    // multiplying it.
    const contextLoadDeadlineAt = Date.now() + CONTEXT_LOAD_TIMEOUT_MS;
    const runContextLoad = <T>(label: string, operation: () => Promise<T>, fallback: T): Promise<T> =>
      runWithPreparationDeadline(
        () => operation(),
        {
          label,
          timeoutMs: Math.max(1, contextLoadDeadlineAt - Date.now()),
          parentSignal: streamAbortController.signal,
        },
      ).catch((error: unknown) => {
        if (errorName(error) === "AbortError" || streamAbortController.signal.aborted) throw error;
        console.warn(`[chat-preparation] ${label} unavailable; continuing with safe fallback`);
        return fallback;
      });
    const [memResult, enabledSkills, knResult] = await Promise.all([
      runContextLoad("memory context", () => storage.getMemoryEntries(persona?.id, 100, 0, convTenantId), { data: [], total: 0, hasMore: false }),
      runContextLoad("enabled skills", () => storage.getEnabledSkillsWithPrompts(persona?.id), []),
      runContextLoad("knowledge context", () => storage.getKnowledge(persona?.id, 100, 0, convTenantId), { data: [], total: 0, hasMore: false }),
    ]);

    let model = await selectTenantValidatedModel({
      requestedModel: normalizeModelId(conv.model || DEFAULT_CHAT_MODEL_ID),
      tenantId,
      timeoutMs: MODEL_CLIENT_INIT_TIMEOUT_MS,
      parentSignal: streamAbortController.signal,
      validate: validateModelForTenant,
      platformAdminVerified: isPlatformAdmin(req),
    });
    ...
    // Parallelize the two pre-flight long poles. Auto-routing (an LLM classify
    // call) and system-prompt assembly are independent — neither consumes the
    // other's output — so run them concurrently instead of serially. autoRouteP is
    // self-guarded (falls back to the default chat model on failure, never rejects); a
    // buildSystemPrompt rejection propagates to the outer handler exactly as it
    // did before this change.
    sendStatus("routing");
    type AutoRouteDecision = { modelId: string; label: string; reason: string; category: string };
    // NOTE: the decision is RETURNED from the IIFE (not assigned to an outer
    // `let`) so it's captured via the Promise.all destructure below. Assigning
    // to an outer `let` from inside this closure would leave TS's control-flow
    // analysis blind to the assignment → the outer var narrows to `never`.
    const autoRouteRaw = (async (): Promise<AutoRouteDecision | null> => {
      if (model !== "auto") return null;
      try {
        // Reasoning Front Door (env-gated, fail-open): let the agent form an
        // explicit plan and pick the route itself; the category classifier is
        // demoted to a hint. Disabled or null → identical to today's path.
        let categoryOverride: string | undefined;
        // Env-gate BEFORE the dynamic import so the disabled path is byte-identical
        // to the legacy route — no module load, no new failure surface, just a string compare.
        if (process.env.REASONING_FRONT_DOOR === "1") {
          try {
            const fd = await import("./reasoning-front-door");
            const plan = await fd.runReasoningFrontDoor({
              userMessage: storedContent,
              tenantId: convTenantId,
              personaName: persona?.name,
            });
            if (fd.shouldTrustPlan(plan, listRouteCategories())) {
              categoryOverride = plan!.category;
            }
          } catch (_fdErr) { logSilentCatch("server/routes.ts", _fdErr); }
        }

        const decision = await autoRouteModel(storedContent, convTenantId, categoryOverride);
        console.log(`[auto-router] "${decision.category}" → ${decision.label} (${decision.reason})`);
        return decision;
      } catch (err) {
        console.error(`[auto-router] Classification failed, using ${DEFAULT_CHAT_MODEL_ID}:`, err);
        return { modelId: DEFAULT_CHAT_MODEL_ID, label: DEFAULT_CHAT_MODEL_LABEL, reason: "Fallback", category: "general" };
      }
    })();
    // Hard fail-open cap on the WHOLE routing stage. A hung provider anywhere
    // inside auto-routing (classifier LLM call, client construction, OAuth
    // refresh) used to hold the turn at "Routing to the best model…" for
    // minutes. After ROUTING_HARD_TIMEOUT_MS we proceed with the default model;
    // the late-resolving loser is discarded (model is assigned AFTER the race,
    // never from inside the closure, so it can't clobber the fallback).
    const ROUTING_HARD_TIMEOUT_MS = parseTimeoutMs(process.env.ROUTING_HARD_TIMEOUT_MS, 8_000);
    let routingTimer: NodeJS.Timeout | undefined;
    const routingTimeoutP = new Promise<AutoRouteDecision | null>((resolve) => {
      routingTimer = setTimeout(() => {
        if (model === "auto") {
          console.warn(`[auto-router] Routing stage exceeded ${ROUTING_HARD_TIMEOUT_MS}ms — failing open to ${DEFAULT_CHAT_MODEL_ID}`);
          resolve({ modelId: DEFAULT_CHAT_MODEL_ID, label: DEFAULT_CHAT_MODEL_LABEL, reason: "Routing timed out — fail-open default", category: "general" });
        } else {
          resolve(null);
        }
      }, ROUTING_HARD_TIMEOUT_MS);
    });
    const autoRouteP = Promise.race([autoRouteRaw, routingTimeoutP]).finally(() => clearTimeout(routingTimer));
    autoRouteRaw.catch(() => {}); // never rejects by construction, but never let a late loser become an unhandled rejection
    const buildPromptP = runWithPreparationDeadline(
      () => buildSystemPrompt(
      ...
      { label: "prompt context", timeoutMs: PROMPT_CONTEXT_TIMEOUT_MS, parentSignal: streamAbortController.signal },
    );
    const projectContextP = runWithPreparationDeadline(
      async () => {
        const { getConversationProjectContext } = await import("./chat-engine");
        return getConversationProjectContext(conversationId, conv);
      },
      { label: "project context", timeoutMs: PROJECT_CONTEXT_TIMEOUT_MS, parentSignal: streamAbortController.signal },
    ).then(
      (value) => ({ value }),
      (error: unknown) => ({ error }),
    );
    // Attach a no-op rejection handler immediately so Node never sees an
    // unhandled rejection during the `await autoRouteP` window below.  The
    // error still propagates normally when we `await buildPromptP` afterwards
    // — the .catch() only prevents the "unhandled" classification, it does not
    // swallow the rejection.  (Same pattern used for autoRouteRaw above.)
    buildPromptP.catch(() => {});
    // Await routing first (bounded at ROUTING_HARD_TIMEOUT_MS, default 12 s).
    // buildPromptP is already running concurrently — no wall-clock change.
    // Splitting the await means "Preparing the agent…" appears as soon as routing
    // resolves instead of waiting for the full prompt build. Previously, a hung
    // proactive-context LLM call inside buildSystemPrompt kept the turn visually
    // stuck on "Routing to the best model…" for 90–220 s even though routing
    // itself finished within 12 s.
    const autoRouteDecision = await autoRouteP;
    if (autoRouteDecision) model = autoRouteDecision.modelId;
    sendStatus("preparing");
    let promptBundle: Awaited<ReturnType<typeof buildSystemPrompt>>;
    try {
      promptBundle = await buildPromptP;
    } catch (promptErr) {
      if (!(promptErr instanceof PreparationDeadlineExceededError)) throw promptErr;
      console.warn(`[chat-preparation] ${promptErr.message}; starting with safe reduced context`);
      sendStatus("starting_without_optional_context");
      let prompt = buildFastStartPrompt(persona, settings);
      try {
        const { getOrCreateProfile } = await import("./user-modeling");
        const { renderActionFirstModeContext } = await import("./lib/action-first-mode");
        const profile = await runWithPreparationDeadline(
          () => getOrCreateProfile(tenantId),
          { label: "fast-start communication preference", timeoutMs: 300, parentSignal: streamAbortController.signal },
        );
        const mode = profile.preferences?.communicationMode === "action_first" ? "action_first" : "standard";
        const modeContext = renderActionFirstModeContext(mode);
        if (modeContext) prompt += `\n\n${modeContext}`;
      } catch (modeErr: unknown) {
        console.error("[chat-preparation] Fast-start communication preference unavailable:", modeErr);
      }
      promptBundle = {
        prompt,
        stablePrompt: prompt,
    ...
    const { prompt: basePrompt, stablePrompt: baseStablePrompt, injectedMemoryIds, citations: collectedCitations } = promptBundle;

    let systemPrompt = basePrompt;
    let resolvedProjectToolContext: ResolvedProjectToolContext | null = null;
    const projectOutcome = await projectContextP;
    if ("error" in projectOutcome) {
      const projectErr = projectOutcome.error;
      if (projectErr instanceof PreparationDeadlineExceededError) {
        console.warn(`[chat-preparation] ${projectErr.message}; continuing without project context`);
        sendStatus("starting_without_optional_context");
      } else if ((projectErr as Error).name === "AbortError") {
        throw projectErr;
      } else {
        logSilentCatch("server/routes.ts", projectErr);
      }
    } else if (projectOutcome.value) {
      systemPrompt += "\n\n" + projectOutcome.value.context;
      resolvedProjectToolContext = {
        projectId: projectOutcome.value.projectId,
        driveFolderId: projectOutcome.value.driveFolderId,
      };
    }
    ...
    if (!registeredModel) {
      // Headers already flushed (early SSE open) → emit an SSE error instead of a
      // JSON 400, then end the stream cleanly.
      try { res.write(`data: ${JSON.stringify({ type: "fatal_error", error: `Unknown model: ${model}. Update the model in conversation settings.` })}\n\n`); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
    ...
    storage.touchMemoryEntries(injectedMemoryIds, tenantId).catch(() => {});

    // File enrichment is optional context, never a reason to hold the first
    // useful agent action. One shared budget prevents a batch of documents or
    // a slow external document service from multiplying preparation latency.
    const attachmentContextDeadlineAt = Date.now() + ATTACHMENT_CONTEXT_TIMEOUT_MS;
    const runAttachmentPreparation = <T>(
      label: string,
      operation: (signal: AbortSignal) => Promise<T>,
    ) => runWithPreparationDeadline(operation, {
      label,
      timeoutMs: Math.max(1, attachmentContextDeadlineAt - Date.now()),
      parentSignal: streamAbortController.signal,
    });
    ...
    if (missingFiles.size > 0) {
      try {
        ...
        // Tenant-scope the restore: filenames are derived from user-supplied
        // attachment metadata, so an unscoped lookup would restore another
        // tenant's file content by referencing its filename. Bound to the
        // conversation's owning tenant.
        const stored = await runAttachmentPreparation(
          "attachment restore",
          () => fileDb.select().from(fileStorage).where(and(inArray(fileStorage.filename, [...missingFiles]), eq(fileStorage.tenantId, convTenantId))),
        );
    ...
    const fileTextCache = new Map<string, string>();
    {
      ...
      const attachmentTasks = new Map<string, Promise<void>>();
      const safeAttachmentLabel = (name: string | undefined) =>
        path.basename(name || "attachment").replace(/[\r\n\t]/g, " ").slice(0, 160);
      const resolveValidatedUploadPath = (url: string): string | null => {
        if (!url.startsWith("/uploads/")) return null;
        const candidatePath = path.join(UPLOADS_DIR, path.basename(url));
        if (!fs.existsSync(candidatePath)) return null;
        const realPath = fs.realpathSync(candidatePath);
        const uploadsReal = fs.realpathSync(UPLOADS_DIR);
        return realPath.startsWith(uploadsReal + path.sep) ? realPath : null;
      };
      const markAttachmentUnavailable = (url: string, name: string, kind: string) => {
        fileTextCache.set(
          url,
          `\n\n--- ${kind} content unavailable for ${safeAttachmentLabel(name)} (bounded preparation timed out, failed, or could not validate the local file; do not claim to have read it) ---`,
        );
      };
      for (const m of allMessages) {
        if (m.role === "assistant") continue;
        ...
        try {
          ...
          for (const f of atts) {
            if (fileTextCache.has(f.url) || attachmentTasks.has(f.url)) continue;
            const task = (async () => {
              const fExt = path.extname(f.name || "").toLowerCase();

              if (f.type === "application/pdf" && f.url.startsWith("/uploads/")) {
                const pdfPath = resolveValidatedUploadPath(f.url);
                if (pdfPath) {
                  try {
                    const { extractPdfText } = await runAttachmentPreparation(
                      "PDF parser loading",
                      () => import("./pdf-tool"),
                    );
                    const result = await runAttachmentPreparation(
                      `PDF extraction for ${f.name || "attachment"}`,
                    ...
                    if (result.success && result.text) {
                      ...
                      markAttachmentUnavailable(f.url, f.name, "PDF");
                    }
                  } catch (err: unknown) {
                    console.warn(`[attachment] PDF extraction unavailable for "${f.name}": ${errorMessage(err)?.slice(0, 100)}`);
                    markAttachmentUnavailable(f.url, f.name, "PDF");
                  }
                } else {
                  console.warn(`[attachment] PDF local file missing or outside upload root: "${safeAttachmentLabel(f.name)}"`);
                  markAttachmentUnavailable(f.url, f.name, "PDF");
                }
              }

              if ((DOC_TYPES.has(f.type) || DOC_EXTS.has(fExt)) && f.url.startsWith("/uploads/")) {
                const docPath = resolveValidatedUploadPath(f.url);
                if (docPath) {
                  try {
                    const text = await runAttachmentPreparation(
                      `document extraction for ${f.name || "attachment"}`,
                      () => extractTextFromFile(docPath, fExt || ".docx"),
                    );
                    if (text) {
                      const truncText = text.length > 8000 ? text.slice(0, 8000) + "\n...(truncated)" : text;
                      fileTextCache.set(f.url, `\n\n--- Content of ${f.name} ---\n${truncText}\n--- End of ${f.name} ---`);
                    } else {
                      markAttachmentUnavailable(f.url, f.name, "Document");
                    }
                  } catch (err: unknown) {
                    console.warn(`[attachment] document extraction unavailable for "${f.name}": ${errorMessage(err)?.slice(0, 100)}`);
                    markAttachmentUnavailable(f.url, f.name, "Document");
                  }
                } else {
                  console.warn(`[attachment] Document local file missing or outside upload root: "${safeAttachmentLabel(f.name)}"`);
                  markAttachmentUnavailable(f.url, f.name, "Document");
                }
              }
              ...
              if (gdocMatch || f.type === "application/vnd.google-apps.document") {
                try {
                  ...
                  if (!docId && f.url.startsWith("/uploads/")) {
                    const gdocPath = resolveValidatedUploadPath(f.url);
                    if (gdocPath) {
                      ...
                      console.warn(`[attachment] Google Doc pointer missing or outside upload root: "${safeAttachmentLabel(f.name)}"`);
                    }
                  }
                  if (docId) {
                    const gdocResult = await runAttachmentPreparation(
                      `Google Doc fetch for ${f.name || "attachment"}`,
                      async (signal) => {
                        const { getAccessToken } = await import("./google-drive");
                        const token = await getAccessToken();
                        if (!token) return { ok: false as const, reason: "Google Drive access token unavailable" };
                        const gdocResp = await fetch(`https://www.googleapis.com/drive/v3/files/${docId}/export?mimeType=text/plain`, {
                          headers: { Authorization: `Bearer ${token}` },
                          signal,
                        });
                        if (!gdocResp.ok) return { ok: false as const, reason: `Drive export HTTP ${gdocResp.status}` };
                        const text = await gdocResp.text();
                        return text
                          ? { ok: true as const, text }
                          : { ok: false as const, reason: "Drive export returned an empty document" };
                      },
                    );
                    if (gdocResult.ok) {
                      const truncText = gdocResult.text.length > 8000 ? gdocResult.text.slice(0, 8000) + "\n...(truncated)" : gdocResult.text;
                      fileTextCache.set(f.url, `\n\n--- Content of ${f.name} (Google Doc) ---\n${truncText}\n--- End of ${f.name} ---`);
                      console.log(`[attachment] Extracted Google Doc "${safeAttachmentLabel(f.name)}" (${gdocResult.text.length} chars)`);
                    } else {
                      console.warn(`[attachment] Google Doc unavailable for "${safeAttachmentLabel(f.name)}": ${gdocResult.reason}`);
                      markAttachmentUnavailable(f.url, f.name, "Google Doc");
                    }
                  } else {
                    console.warn(`[attachment] Google Doc id unavailable for "${safeAttachmentLabel(f.name)}"`);
                    markAttachmentUnavailable(f.url, f.name, "Google Doc");
                  }
                } catch (gdocErr: unknown) {
                  console.warn(`[attachment] Google Doc extraction unavailable for "${f.name}": ${errorMessage(gdocErr)?.slice(0, 100)}`);
                  markAttachmentUnavailable(f.url, f.name, "Google Doc");
                }
              }
            })();
            attachmentTasks.set(f.url, task);
          }
        } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
      }
      await Promise.all(attachmentTasks.values());
    }

    {
      ...
      if (lastMsg?.role === "user") {
        ...
        let gdocUrlMatch;
        const inlineGoogleDocTasks = new Map<string, Promise<void>>();
        while ((gdocUrlMatch = gdocUrlRegex.exec(lastMsg.content)) !== null) {
          const docId = gdocUrlMatch[1];
          const cacheKey = `gdoc:${docId}`;
          if (fileTextCache.has(cacheKey) || inlineGoogleDocTasks.has(cacheKey)) continue;
          const task = (async () => {
            try {
              const fetchedDoc = await runAttachmentPreparation(
                "inline Google Doc fetch",
                async (signal) => {
                  const { getAccessToken } = await import("./google-drive");
                  const token = await getAccessToken();
                  if (!token) return { ok: false as const, reason: "Google Drive access token unavailable" };
                  ...
                  if (!resp.ok) return { ok: false as const, reason: `Drive export HTTP ${resp.status}` };
                  const text = await resp.text();
                  if (!text) return { ok: false as const, reason: "Drive export returned an empty document" };
                  const metaResp = await fetch(`https://www.googleapis.com/drive/v3/files/${docId}?fields=name`, {
                    headers: { Authorization: `Bearer ${token}` },
                    signal,
                  }).catch((error: unknown) => {
                    console.warn(`[attachment] Google Doc metadata request failed: ${errorMessage(error)?.slice(0, 100)}`);
                    return null;
                  });
                  if (metaResp && !metaResp.ok) {
                    console.warn(`[attachment] Google Doc metadata unavailable: HTTP ${metaResp.status}`);
                  }
                  const docName = metaResp?.ok ? (await metaResp.json()).name || "Google Doc" : "Google Doc";
                  return { ok: true as const, text, docName };
                },
              );
              if (fetchedDoc.ok) {
                const truncText = fetchedDoc.text.length > 8000 ? fetchedDoc.text.slice(0, 8000) + "\n...(truncated)" : fetchedDoc.text;
                fileTextCache.set(cacheKey, `\n\n--- Content of "${fetchedDoc.docName}" (Google Doc) ---\n${truncText}\n--- End of "${fetchedDoc.docName}" ---`);
                console.log(`[attachment] Auto-fetched Google Doc "${fetchedDoc.docName}" from inline URL (${fetchedDoc.text.length} chars)`);
              } else {
                console.warn(`[attachment] Inline Google Doc unavailable: ${fetchedDoc.reason}`);
                fileTextCache.set(cacheKey, "\n\n--- Google Doc content unavailable during bounded preparation; do not claim to have read it ---");
              }
            } catch (e: unknown) {
              console.warn(`[attachment] Inline Google Doc fetch unavailable: ${errorMessage(e)?.slice(0, 100)}`);
              fileTextCache.set(cacheKey, "\n\n--- Google Doc content unavailable during bounded preparation; do not claim to have read it ---");
            }
          })();
          inlineGoogleDocTasks.set(cacheKey, task);
        }
        await Promise.all(inlineGoogleDocTasks.values());
      }
    }
            ...
            for (const img of imageAtts) {
              ...
              if (img.url.startsWith("/uploads/")) {
                ...
                try {
                  if (fs.existsSync(localPath)) {
                    ...
                    if (realPath.startsWith(uploadsReal + path.sep)) {
                      const imageBytes = fs.statSync(realPath).size;
                      if (imageBytes <= MAX_INLINE_IMAGE_BYTES) {
                        ...
                        console.warn(`[attachment] Image "${img.name || safeName}" is ${imageBytes} bytes; skipping inline payload above ${MAX_INLINE_IMAGE_BYTES}`);
                      }
                    }
                  }
                } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
                if (!resolved) {
                  ...
                  const restoredBytes = dbFile ? Buffer.byteLength(dbFile.data, "base64") : 0;
                  if (dbFile && restoredBytes <= MAX_INLINE_IMAGE_BYTES) {
                    imgUrl = `data:${dbFile.mimeType};base64,${dbFile.data}`;
                    resolved = true;
                  } else if (dbFile) {
                    console.warn(`[attachment] Restored image "${img.name || safeName}" is ${restoredBytes} bytes; skipping oversized inline payload`);
                  }
                }
                if (!resolved) {
    ...
    // SSE headers + keepalive are now opened EARLY (see "EARLY SSE STREAM OPEN"
    // above), so streamAborted / globalKeepalive are already declared and the
    // keepalive interval is already running by this point.
    ...
    try {
      ...
      try {
        const ownerExplicitPaidOverride =
          isPlatformAdmin(req) && isOwnerOnlyFinalDecisionModel(model);
        const result = await runWithPreparationDeadline(
          () => getClientForModel(model, conv.tenantId, {
            requiresTools: true,
            meteredOverride: ownerExplicitPaidOverride,
            explicitOwnerSelection: ownerExplicitPaidOverride,
            platformAdminVerified: ownerExplicitPaidOverride,
          }),
          { label: "model client initialization", timeoutMs: MODEL_CLIENT_INIT_TIMEOUT_MS, parentSignal: streamAbortController.signal },
        );
        activeClient = result.client;
        activeModelId = result.actualModelId;
      } catch (primaryErr: unknown) {
        if (streamAbortController.signal.aborted || errorName(primaryErr) === "AbortError") {
          throw primaryErr;
        }
        const available = await getAvailableModels();
        const excludedProviders = new Set<string>();
        const failedProv = MODEL_REGISTRY.find(m => m.id === model)?.provider;
        if (failedProv) { excludedProviders.add(failedProv); markProviderUnhealthy(failedProv, String(errorMessage(primaryErr) || "")); }
        ...
        for (let attempt = 0; attempt < 5 && !resolved; attempt++) {
          ...
          if (!fallback) break;
          try {
            const fbResult = await runWithPreparationDeadline(
              () => getClientForModel(fallback.id, conv.tenantId, { requiresTools: true }),
              { label: `fallback client initialization for ${fallback.id}`, timeoutMs: MODEL_CLIENT_INIT_TIMEOUT_MS, parentSignal: streamAbortController.signal },
            );
            activeClient = fbResult.client;
            activeModelId = fbResult.actualModelId;
            currentRegistryModelId = fallback.id;
            failoverInfo = { used: true, from: model, to: fallback.id, reason: errorMessage(primaryErr) };
            console.log(`[failover] Init ${attempt + 1}: ${model} → ${fallback.id} (${fallback.provider})`);
            res.write(`data: ${JSON.stringify({ type: "failover", from: model, to: fallback.id, reason: errorMessage(primaryErr) })}\n\n`);
            resolved = true;
          } catch (fbErr: unknown) {
            if (streamAbortController.signal.aborted || errorName(fbErr) === "AbortError") {
              throw fbErr;
            }
            markProviderUnhealthy(fallback.provider, String(errorMessage(fbErr) || ""));
      ...
      if (shouldCompact(chatMessages.length)) {
        ...
        if (ladder.fits) {
          if (ladder.layersFired.length > 0) {
            let archive: import("./compaction").CompactionArchiveRef;
            try {
              const { archiveMessages } = await import("./compaction");
              archive = await archiveMessages(conversationId, chatMessages as any, chatMessages as any, tenantId);
            } catch (archiveErr: unknown) {
              throw Object.assign(
                new Error("Context compaction could not safely preserve the full history. The conversation was left intact rather than discarding older turns."),
                { code: "CONTEXT_HISTORY_PRESERVATION_FAILED", cause: archiveErr },
              );
            }
            const { buildCompactionBookmarkMessage } = await import("./compaction");
            finalChatMessages = [buildCompactionBookmarkMessage(archive), ...ladder.messages] as any;
          ...
          try {
            ...
            if (compactionResult.compacted && compactionResult.summary) {
              ...
              finalChatMessages = buildCompactedMessages(compactionResult.summary, toKeep, conversationId, compactionResult.archive, tenantId);
            ...
            } else if (compactionResult.failure === "archive_failed") {
              throw Object.assign(
                new Error("Context compaction could not safely preserve the full history. The conversation was left intact rather than discarding older turns."),
                { code: "CONTEXT_HISTORY_PRESERVATION_FAILED" },
              );
            } else if (compactionResult.archive) {
              const { buildCompactionBookmarkMessage } = await import("./compaction");
              finalChatMessages = [buildCompactionBookmarkMessage(compactionResult.archive), ...ladder.messages] as any;
              console.warn(`[compaction] LLM summary unavailable; using the archived free-ladder result with recovery bookmark #${compactionResult.archive.id}`);
            } else {
              throw Object.assign(
                new Error("Context compaction did not produce a recoverable summary or archive reference. The conversation was left intact rather than discarding older turns."),
                { code: "CONTEXT_HISTORY_PRESERVATION_FAILED" },
              );
            }
          } catch (compErr) {
            if ((compErr as any)?.code === "CONTEXT_HISTORY_PRESERVATION_FAILED") {
              throw compErr;
            }
            console.error("[compaction] Error during LLM compaction:", compErr);
            throw Object.assign(
              new Error("Context compaction failed before a recoverable history bookmark was available. The conversation was left intact rather than discarding older turns."),
              { code: "CONTEXT_HISTORY_PRESERVATION_FAILED", cause: compErr },
            );
          }
        }
      }
      ...
      try {
        ...
        if (Number.isInteger(autoProj?.projectId) && (autoProj!.projectId as number) > 0) {
          // This is set for both the creator and a concurrent request that
          // waited, then observed the creator's final tenant-owned assignment.
          resolvedProjectToolContext = {
            projectId: autoProj!.projectId as number,
            driveFolderId: null,
          };
        }
        ...
        console.error(`[auto-project] Hook error:`, errorMessage(apErr));
      }

      const isFelix = persona?.id === 2;
      if (isFelix && useTools) {
        try {
...
${buildFelixProtocol()}`
      ...
      const outcomeEnabled = process.env.FELIX_OUTCOME_CONTROLLER_ENABLED === "1" && persona?.id === 2;
      let outcomeJudgeUsed = false;
      let outcomeContinuationUsed = false;
      const attachmentReviewGuardEnabled = isAttachmentReviewRequest(content, {
        attachmentCount: conversationAttachmentUrls.size,
        currentAttachmentCount: parsedAttachments.length,
      });
      const bufferTerminalOutput = outcomeEnabled || attachmentReviewGuardEnabled;
      const shouldBufferCurrentRoundOutput = () => bufferTerminalOutput;
      let attachmentRecoveryAttempts = 0;
      let mutatingToolDispatched = false;
      const workerModels = new Set<string>([currentRegistryModelId, activeModelId].filter(Boolean));
      ...
      round_loop: for (let round = 0; round <= maxToolRound; round++) {
        ...
        if (guard.action === "truncate") {
          ...
          let archive: import("./compaction").CompactionArchiveRef | undefined;

          if (droppedMessages.length > 0) {
            try {
              const { archiveMessages, extractAndSaveMemories } = await import("./compaction");
               archive = await archiveMessages(conversationId, droppedMessages as any, apiMessages as any, tenantId);
              ...
              console.error(`[context-guard] Archive save failed: ${errorMessage(archiveErr)}`);
            }

            if (!archive) {
              throw new Error("Context guard could not safely preserve the full history. The conversation was left intact rather than truncating older turns.");
            }

            try {
              const projIdResult = await db.execute(sql`SELECT project_id FROM conversations WHERE id = ${conversationId} AND tenant_id = ${tenantId} AND project_id IS NOT NULL`);
          ...
          apiMessages = truncateWithSummary(apiMessages, guard.truncateToMessages, archive);
        ...
        if (useTools && round < MAX_TOOL_ROUNDS) {
          ...
          // Contrastive tool-choice note — only on the first round (later
          // rounds already have committed context; re-injecting would nag).
          // Architect finding: keep the leading consecutive-system prefix
          // intact — insert AFTER the last leading system message, never as a
          // trailing system message behind user/assistant turns.
          if (round === 0 && routed.contrastiveNote) {
            let insertAt = 0;
            while (insertAt < apiMessages.length && apiMessages[insertAt].role === "system") insertAt++;
            apiMessages.splice(insertAt, 0, { role: "system", content: routed.contrastiveNote.trim() });
          }
        ...
        if (round === 0) sendStatus("starting");
        ...
        try {
          ...
          const errStatus = errorStatus(streamErr) || errorStatusCode(streamErr);
          const errMsg = String(errorMessage(streamErr) || "");
          ...
          if (isCreateTimeout || isRetryableError(streamErr)) {
            ...
            for (let attempt = 0; attempt < 5 && !streamResolved; attempt++) {
              ...
              try {
                const fbResult = await runWithPreparationDeadline(
                  () => getClientForModel(fallback.id, conv.tenantId, { requiresTools: useTools }),
                  { label: `stream failover client initialization for ${fallback.id}`, timeoutMs: MODEL_CLIENT_INIT_TIMEOUT_MS, parentSignal: streamAbortController.signal },
                );
                activeClient = fbResult.client;
                activeModelId = fbResult.actualModelId;
                currentRegistryModelId = fallback.id;
                ...
                const fbMsg = String(errorMessage(fbErr) || "");
                ...
                if ((errorStatus(fbErr) === 401 || errorStatus(fbErr) === 403 || errorStatus(fbErr) === 429) && conv.tenantId) {
                  markSubscriptionFailed(fbProvider, conv.tenantId, errorStatus(fbErr));
                }
              }
            }
            ...
            if (!streamResolved) {
              ...
              try {
                ...
                console.warn(`[rlm-recovery] All providers failed (${String(errorMessage(streamErr)).slice(0, 100)}). Engaging RLM fallback. promptChars=${flat.prompt.length}`);
              ...
              } catch (rlmErr: unknown) {
                console.warn(`[rlm-recovery] threw: ${String(errorMessage(rlmErr)).slice(0, 200)}`);
                throw streamErr;
              }
            }
          } else {
        ...
        let toolMarkupStreamFilter = createToolMarkupStreamFilter();
        ...
        toolMarkupStreamFilter = createToolMarkupStreamFilter();
        ...
        for await (const chunk of stream) {
          if ((chunk as any)?.model) workerModels.add(String((chunk as any).model));
          ...
          const streamSafeDelta = toolMarkupStreamFilter.push(rawDelta);
          let contentDelta = streamSafeDelta.replace(/<\/?tool_call>/g, "").replace(/<\/?function_calls?>/g, "").replace(/<invoke\s+name="[^"]*"\s*\/?>/g, "").replace(/<\/?antml:\w+>/g, "");
              ...
              if (!inThinkBlock) {
                ...
                if (openIdx === -1) {
                  if (thinkBuffer.length > 10) {
                    ...
                    if (!shouldBufferCurrentRoundOutput()) res.write(`data: ${JSON.stringify({ content: safe })}\n\n`);
                    thinkBuffer = thinkBuffer.slice(safe.length);
                  }
                  break;
                } else {
                  if (openIdx > 0) {
                    if (!shouldBufferCurrentRoundOutput()) res.write(`data: ${JSON.stringify({ content: thinkBuffer.slice(0, openIdx) })}\n\n`);
                  }
          ...
          } else if (!shouldBufferCurrentRoundOutput()) {
        ...
        if (streamAborted && !roundContent && !hasToolCalls) {
          if (hadSlowTool && round > 0 && executedTools.length > 0) {
            ...
            try {
              ...
              const recoveryStream: any = await createCompletionWithTimeout(
                (signal) => activeClient.chat.completions.create(recoveryParams, { signal }),
                streamAbortController.signal,
                STREAM_CREATE_TIMEOUT_MS,
                currentRegistryModelId,
              );
              const recoveryMarkupFilter = createToolMarkupStreamFilter();
              for await (const rChunk of recoveryStream) {
                if ((rChunk as any)?.model) workerModels.add(String((rChunk as any).model));
                const rDelta = rChunk.choices?.[0]?.delta?.content || "";
                if (rDelta) {
                  roundContent += rDelta;
                  const safeRecoveryDelta = recoveryMarkupFilter.push(rDelta);
                  fullResponse += safeRecoveryDelta;
                  if (safeRecoveryDelta && !shouldBufferCurrentRoundOutput()) {
                    res.write(`data: ${JSON.stringify({ content: safeRecoveryDelta })}\n\n`);
                  }
                }
              }
              const safeRecoveryTail = recoveryMarkupFilter.flush();
              fullResponse += safeRecoveryTail;
              if (safeRecoveryTail && !shouldBufferCurrentRoundOutput()) {
                res.write(`data: ${JSON.stringify({ content: safeRecoveryTail })}\n\n`);
              }
              if (roundContent) {
              ...
              console.error(`[sse-round] Recovery retry also failed: ${errorMessage(recoveryErr)?.slice(0, 200)}`);
            }
          }
        ...
        break escalation_retry; // success — no overflow, exit retry loop
        } catch (midStreamErr: unknown) {
          clearInterval(thinkingTimer);
          if (streamTimeoutTimer) { clearTimeout(streamTimeoutTimer); streamTimeoutTimer = null; }
          const midMsg = String(errorMessage(midStreamErr) || midStreamErr || "");
          ...
          if (isOverflow) {
            ...
            try {
              ...
              if (next) {
                try {
                  const escResult = await runWithPreparationDeadline(
                    () => getClientForModel(next.modelId, conv.tenantId, { requiresTools: useTools }),
                    {
                      label: `context escalation client initialization for ${next.modelId}`,
                      timeoutMs: MODEL_CLIENT_INIT_TIMEOUT_MS,
                      parentSignal: streamAbortController.signal,
                    },
                  );
                  ...
                  currentRegistryModelId = escResult.actualModelId;
                  _bigCtxTried.add(next.modelId);
                  createParams.model = activeModelId;
                  createParams.max_completion_tokens = getMaxOutputTokens(currentRegistryModelId);
                  const nextProvider = MODEL_REGISTRY.find(m => m.id === currentRegistryModelId)?.provider;
                  if (nextProvider && !PROVIDERS_SUPPORTING_TOOLS.has(nextProvider)) {
                    delete createParams.tools;
                    delete createParams.tool_choice;
                  }
                  const wasRemapped = currentRegistryModelId !== next.modelId;
                  failoverInfo = { used: true, from: failoverInfo.to || model, to: currentRegistryModelId, reason: wasRemapped ? `context-overflow request ${next.modelId} remapped by metered policy` : `context-overflow-escalation: ${next.contextWindow.toLocaleString()} ctx` };
                  res.write(`data: ${JSON.stringify({ type: "context_escalation", from: fromModel, to: currentRegistryModelId, contextWindow: wasRemapped ? undefined : next.contextWindow, rationale: wasRemapped ? `Requested ${next.modelId}; remapped by metered policy` : next.rationale, attempt: _escAttempt + 1 })}\n\n`);
                  // Soft-notify in chat as well so the user sees it without needing the SSE event channel.
                  const escNote = wasRemapped
                    ? `\n\n*[Context window exceeded — the requested ${next.modelId} lane was remapped by metered policy to ${currentRegistryModelId}; retrying before truncation...]*\n\n`
                    : `\n\n*[Context window exceeded — auto-escalating to ${currentRegistryModelId} (${(next.contextWindow / 1_000_000).toFixed(1)}M context) and resuming...]*\n\n`;
                  fullResponse += escNote;
                  res.write(`data: ${JSON.stringify({ content: escNote })}\n\n`);
                  console.log(`[sse-overflow-escalate] Round ${round} attempt ${_escAttempt + 1}: ${fromModel} → ${currentRegistryModelId}${wasRemapped ? ` (requested ${next.modelId}; metered remap)` : ` (${next.contextWindow.toLocaleString()} ctx)`}`);
                  ...
                  console.warn(`[sse-overflow-escalate] Failed to switch to ${next.modelId}: ${String(errorMessage(escErr)).slice(0, 200)}`);
                  // Fall through to chain-exhausted message below.
                }
              }
            } catch (escImportErr: unknown) {
              console.warn(`[sse-overflow-escalate] Escalator import failed: ${String(errorMessage(escImportErr)).slice(0, 200)}`);
            }
        ...
        const streamSafeTail = toolMarkupStreamFilter.flush();
        if (streamSafeTail) {
          const contentTail = streamSafeTail.replace(/<\/?tool_call>/g, "").replace(/<\/?function_calls?>/g, "").replace(/<invoke\s+name="[^"]*"\s*\/?>/g, "").replace(/<\/?antml:\w+>/g, "");
          fullResponse += contentTail;
          if (isThinkingMode) {
            thinkBuffer += contentTail;
          } else if (!shouldBufferCurrentRoundOutput()) {
            res.write(`data: ${JSON.stringify({ content: contentTail })}\n\n`);
            broadcastToConversation(conversationId, { type: "stream", content: contentTail });
          }
        }
        ...
        if (isThinkingMode && thinkBuffer.length > 0) {
          if (inThinkBlock) {
            ...
            if (!shouldBufferCurrentRoundOutput()) res.write(`data: ${JSON.stringify({ content: thinkBuffer })}\n\n`);
          }
        }
        ...
        if ((!hasToolCalls || toolCallCount === 0) && roundContent) {
          ...
          if (xmlParsed.length > 0) {
            ...
            roundContent = stripXmlToolCallMarkup(roundContent);
            fullResponse = stripXmlToolCallMarkup(fullResponse);
          }
        }
        if ((!hasToolCalls || toolCallCount === 0) && roundContent) {
        ...
        if (!hasToolCalls || toolCallCount === 0) {
          workerModels.add(currentRegistryModelId);
          let visibleTerminalCandidate = shouldBufferCurrentRoundOutput() ? stripThinkTags(roundContent).trim() : roundContent.trim();
          if (outcomeEnabled && !visibleTerminalCandidate) {
            // A reasoning-only response is not a user-visible answer. Remove the
            // hidden candidate before the existing bounded empty-response recovery.
            fullResponse = fullResponse.slice(0, roundResponseStart);
            roundContent = "";
          }
          const incompleteAttachmentReview = attachmentReviewGuardEnabled && roundContent
            ? detectIncompleteOutcome(content, visibleTerminalCandidate, executedTools, {
                attachmentCount: conversationAttachmentUrls.size,
                currentAttachmentCount: parsedAttachments.length,
              })
            : null;
          if (incompleteAttachmentReview && attachmentRecoveryAttempts < 2) {
            attachmentRecoveryAttempts++;
            maxToolRound++;
            fullResponse = fullResponse.slice(0, roundResponseStart);
            apiMessages.push({ role: "assistant", content: roundContent });
            apiMessages.push({
              role: "user",
              content: `SYSTEM COMPLETION RECOVERY: ${incompleteAttachmentReview.reason}. Continue now in this same turn. Use the available file-reading and jury/research tools needed to inspect the attachments, then deliver the complete requested analysis. Do not stop with another promise, status update, or raw tool-call markup.`,
            });
            useTools = true;
            console.warn(`[attachment-completion] SSE recovery ${attachmentRecoveryAttempts}/2 for persona ${persona?.name || persona?.id || "unknown"}: ${incompleteAttachmentReview.reason}`);
            continue;
          }
          if (incompleteAttachmentReview) {
            visibleTerminalCandidate = "I could not complete the attached-file review in this turn because the selected model repeatedly stopped before reading the files. No review was completed. Please retry with a different model; the incomplete response was blocked rather than presented as finished work.";
            roundContent = visibleTerminalCandidate;
            fullResponse = visibleTerminalCandidate;
            console.error(`[attachment-completion] Blocked repeated incomplete attachment response after ${attachmentRecoveryAttempts} recoveries for persona ${persona?.name || persona?.id || "unknown"}`);
          }
          if (outcomeEnabled && !outcomeJudgeUsed && roundContent) {
            outcomeJudgeUsed = true;
            const decision = await assessFelixOutcome({
              tenantId: conv.tenantId,
              userRequest: content,
              candidateResponse: visibleTerminalCandidate,
              tools: executedTools.map((tool) => ({ name: tool.name, output: tool.output, riskLevel: classifyToolRisk(tool.name, tool.input).riskLevel })),
              workerModels: [...workerModels],
              continuationUsed: outcomeContinuationUsed,
              mutatingToolDispatched,
              attachmentCount: conversationAttachmentUrls.size,
              currentAttachmentCount: parsedAttachments.length,
              availableToolNames: useTools
                ? (createParams.tools ?? []).map((tool: any) => tool?.function?.name).filter(Boolean)
                : [],
            });
            if (decision.shouldContinue && !outcomeContinuationUsed) {
              outcomeContinuationUsed = true;
              maxToolRound++;
              fullResponse = fullResponse.slice(0, roundResponseStart);
              apiMessages.push({ role: "assistant", content: roundContent });
              apiMessages.push({ role: "user", content: decision.continuationDirective! });
              useTools = decision.toolsMode === "preserve";
              console.warn(`[felix-outcome] SSE continuation verdict=${decision.verdict} tools=${decision.toolsMode}`);
              continue;
            }
            const visibleCandidate = visibleTerminalCandidate;
            if (visibleCandidate) {
              // Felix terminal candidates are intentionally not streamed until
              // the independent checkpoint accepts/degrades them.
              fullResponse = visibleCandidate;
              res.write(`data: ${JSON.stringify({ content: visibleCandidate })}\n\n`);
              broadcastToConversation(conversationId, { type: "stream", content: visibleCandidate });
            }
          } else if (outcomeEnabled && outcomeContinuationUsed && visibleTerminalCandidate) {
            // The single checkpoint was already spent on the prior candidate;
            // the bounded continuation is the accepted terminal synthesis.
            const visibleCandidate = visibleTerminalCandidate;
            fullResponse = visibleCandidate;
            res.write(`data: ${JSON.stringify({ content: visibleCandidate })}\n\n`);
            broadcastToConversation(conversationId, { type: "stream", content: visibleCandidate });
          } else if (shouldBufferCurrentRoundOutput() && visibleTerminalCandidate) {
            fullResponse = visibleTerminalCandidate;
            res.write(`data: ${JSON.stringify({ content: visibleTerminalCandidate })}\n\n`);
            broadcastToConversation(conversationId, { type: "stream", content: visibleTerminalCandidate });
          }
            ...
            try {
              ...
              for (const candidate of candidates.slice(0, 4)) {
                try {
                  ...
                  console.warn(`[sse-round] Empty-response failover candidate ${candidate.id} failed to bind: ${String(errorMessage(bindErr)).slice(0, 150)}`);
                }
              }
              if (bound) continue;
              console.error(`[sse-round] Round ${round}: model ${fromModel} returned EMPTY — no failover candidate bound.`);
            } catch (foErr: unknown) {
              console.error(`[sse-round] Empty-response failover failed: ${String(errorMessage(foErr)).slice(0, 200)}`);
            }
          }
          if (!roundContent) {
        ...
        // 70% budget nudge (once per turn): steer toward delegation + producing the
        // deliverable BEFORE the hard cap strands it (2026-08-09 executive-PDF incident).
        if (!budgetNudgeInjected && totalToolCalls + effectiveCount >= Math.ceil(MAX_TOTAL_TOOL_CALLS * 0.7)) {
          budgetNudgeInjected = true;
          console.log(`[tools] Budget nudge at ${totalToolCalls + effectiveCount}/${MAX_TOTAL_TOOL_CALLS} tool calls.`);
          apiMessages.push({ role: "user", content: `SYSTEM BUDGET WARNING: You have used ~70% of this turn's tool-call budget (${totalToolCalls + effectiveCount}/${MAX_TOTAL_TOOL_CALLS}). Course-correct NOW: (1) STOP open-ended research; (2) if research legs remain, hand them to sub-agents via delegate_task/orchestrate — each sub-agent has its OWN separate budget; (3) reserve your remaining calls for PRODUCING the promised deliverable (PDF/document/email) with what you already have. Do not let the deliverable get stranded at the cap.` });
        }
        ...
        for (const [, tc] of Object.entries(toolCallBuffers)) {
          ...
          stampResolvedProjectToolContext(parsedArgs, resolvedProjectToolContext);
          ...
          try {
            ...
            result = await executeGuardedTool(tc.name, parsedArgs, {
              ...
              authoritativeSourceLock: getCmmcAuthoritativeSourceLock(
                buildRecentUserRoutingContext(apiMessages),
              ),
              onDispatch: () => {
                if (toolRisk.isMutating) mutatingToolDispatched = true;
              },
            ...
            result = { error: errorMessage(err) || "Tool execution failed" };
          }
          ...
          if (PRES_TOOLS.has(tc.name) && result && !result.error) {
            try {
              ...
              console.warn(`[delivery-guarantee] Failed to save delivery: ${errorMessage(delErr)?.slice(0, 100)}`);
            }
          }
      ...
      if (fullResponse.length > 50 && !isThinkingMode && executedTools.length === 0 && !outcomeEnabled) {
        try {
          res.write(`data: ${JSON.stringify({ type: "reflection", status: "evaluating" })}\n\n`);
          const reflection = await reflectOnResponse(content, fullResponse, persona?.name, tenantId);
          ...
          if (reflection.shouldRefine) {
            ...
            const refined = await refineResponse(content, fullResponse, reflection, activeModelId, tenantId);
          ...
          console.log(`[self-reflection] Error: ${errorMessage(reflErr)}`);
        }
      }
      ...
      const cleanedFullResponse = stripXmlToolCallMarkup(fullResponse)
      ...
      if (Array.isArray(collectedCitations) && collectedCitations.length > 0 && (__asstMsg as any)?.id) {
        try { await storage.attachCitations((__asstMsg as any).id, collectedCitations, tenantId); }
      ...
      if (needsTitle) {
        try {
          ...
          const { client: titleClient, actualModelId: titleModel } =
            await getClientForModel(DEFAULT_CHAT_MODEL_ID, conv.tenantId ?? ADMIN_TENANT_ID);
          const titleResp = await titleClient.chat.completions.create({
            model: titleModel,
            messages: [
      ...
      if (suggestQuestions) {
        try {
          ...
          const { client: suggestionClient, actualModelId: suggestionModel } =
            await getClientForModel(DEFAULT_CHAT_MODEL_ID, conv.tenantId ?? ADMIN_TENANT_ID);
          const sqResp = await suggestionClient.chat.completions.create({
      ...
      const completedKeepalive = globalKeepalive;
      if (completedKeepalive) {
        try { clearInterval(completedKeepalive); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
      }
      try { res.write(`data: ${JSON.stringify({ done: true })}\n\n`); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
      ...
      const failedKeepalive = globalKeepalive;
      if (failedKeepalive) {
        try { clearInterval(failedKeepalive); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
      }
      const errMsg = errorMessage(err) || "Stream failed";
      const errStack = errorStack(err)?.slice(0, 500) || "";
  ...
  // Bounded, report-only Human-AI Synergy trial (owner/admin, tenant-scoped).
  registerHumanAiSynergyRoutes(app, {
    requireOwnerAdmin: requireAdmin,
  ...
  // Routes-split Tier 1: /api/auth/health pair moved to server/routes/auth-extra.ts.
  registerAuthMonitoringRoutes(app, { authMiddleware });
  ...
  // Routes-split Tier 1: tenants/fork + halt/resume-background + system-state +
  // risk-classes + ecosystem-health + decline-events + self-initiatives moved to
  // server/routes/admin-extra.ts. Registered here at the fork's original position
  // (no /api-wide middleware between here and the old block positions below).
  registerAdminExtraRoutes(app, { authMiddleware, requirePlatformAdmin, getTenantFromRequest });
  ...
  // R98.17 Cairo admin endpoints (halt/resume-background, system-state,
  // risk-classes, ecosystem-health, decline-events, self-initiatives) now
  // registered above via registerAdminExtraRoutes — see server/routes/admin-extra.ts.
  ...
  app.get("/api/scheduled-posts", authMiddleware, async (req: any, res) => {
    try {
      ...
      res.status(500).json({ error: errorMessage(e) || "list failed" });
    }
  });
  ...
  app.post("/api/scheduled-posts", authMiddleware, validate(scheduledPostCreateSchema), async (req: any, res) => {
    try {
      ...
      res.status(500).json({ error: errorMessage(e) || "schedule failed" });
    }
  });
  ...
  app.delete("/api/scheduled-posts/:id", authMiddleware, validate(emptyBodySchema), async (req: any, res) => {
    try {
      ...
      res.status(500).json({ error: errorMessage(e) || "cancel failed" });
    }
  });
  ...
  // Verified Revenue Missions — owner-only durable business experiments judged
  // by EXTERNAL evidence (replies/payments), never LLM forecasts. HITL: drafts
  // send nothing; approval is a precondition flag for the future send slice.
  const { revenueMissionsRouter } = await import("./routes/revenue-missions");
  app.use("/api/revenue-missions", authMiddleware, revenueMissionsRouter);
  ...
  app.post("/api/subagents/spawn", authMiddleware, async (req, res) => {
    ...
    try {
      ...
      const result = await spawnSubagent({ parentConversationId, task, label, agentId, model, thinkingLevel, runTimeoutSeconds, mode, tenantId: getTenantFromRequest(req) ?? ADMIN_TENANT_ID });
      res.json(result);
    } catch (err: unknown) {
      res.status(500).json({ error: errorMessage(err) });
    }
  ...
  try {
    ...
    console.warn("[routes] Twilio route registration skipped:", errorMessage(e));
  }
  ...
  registerProjectsRoutes(app, { getTenantFromRequest, authMiddleware, requirePlatformAdmin, upload, SAFE_EXTENSIONS, UPLOADS_DIR });
  ...
  app.patch("/api/skills/:id", async (req, res) => {
    ...
    try {
      ...
      if (body.enabled !== undefined) {
        import("./persona-sync").then(m => m.syncPersonaDocs()).catch(e => console.error("[persona-sync] Auto-sync after skill toggle failed:", errorMessage(e)));
      }
    } catch (error) {
      if (isManagedSkillSecurityError(error)) {
        return res.status(400).json({
  ...
  registerAgentInsightsRoutes(app, { authMiddleware, isPlatformAdmin });
  registerRecoveryManifestRoutes(app, { authMiddleware, getTenantFromRequest, requirePlatformAdmin });
  ...
  try {
    ...
    console.warn("[startup] Hyperagent routes not mounted:", errorMessage(e)?.slice(0, 120));
  }
  ...
  registerMemoryRoutes(app, { getTenantFromRequest, isAdminRequest, isPlatformAdmin, ADMIN_TENANT_ID, requirePlatformAdmin });
  ...
  app.post("/api/knowledge", async (req, res) => {
    ...
    import("./embeddings").then(({ generateEmbedding }) =>
      generateEmbedding(`${entry.title} ${entry.content}`).then((emb: number[] | null) => {
        if (emb) storage.updateKnowledgeEmbedding(entry.id, emb, tenantId).catch(() => {});
      }).catch(() => {})
    ).catch(() => {});
    res.json(entry);
  });
  ...
  app.get("/api/experiments", async (req, res) => {
    ...
    const category = req.query.category as string | undefined;
    const exps = await getExperimentHistory(limit, category, ALL_TENANTS);
  ...
  app.post("/api/backup/cloud", async (req, res) => {
    ...
    try {
      ...
      console.error("[backup] Manual backup failed:", errorMessage(err));
      res.status(500).json({ error: "Backup failed: " + errorMessage(err) });
    }
  });
  ...
  app.post("/api/backup/full", async (req, res) => {
    ...
    try {
      ...
      } catch (err: unknown) { results.cloudBackup = { error: errorMessage(err) }; }
      try {
        results.memoryBackup = await runMemoryBackupToGoogleDrive();
      } catch (err: unknown) { results.memoryBackup = { error: errorMessage(err) }; }
      try {
        ...
        const sanitized = String(errorMessage(err) || err).replace(/https:\/\/[^@\s]+@github\.com/gi, "https://[REDACTED]@github.com");
        results.githubPush = { error: sanitized };
      }
      res.json({ success: true, results });
    } catch (err: unknown) {
      res.status(500).json({ error: errorMessage(err), partialResults: results });
    }
  });
  ...
  app.get("/api/backup/status", async (req, res) => {
    ...
    try {
      ...
      const allLogs = await storage.getHeartbeatLogs(20, undefined, ALL_TENANTS);
  ...
  app.post("/api/email/reply", async (req, res) => {
    try {
      ...
      // 72h-review HIGH — message OWNERSHIP check. All tenants share one provider
      // inbox, so without this an authenticated tenant could reply into another
      // tenant's correspondence by submitting a known/guessed provider message id.
      // Fail closed: the provider message id must belong to a row this tenant owns.
      const owned = await db.execute(
        sql`SELECT 1 FROM inbox_messages WHERE message_id = ${String(messageId)} AND tenant_id = ${tenantId} LIMIT 1`
      );
      if ((((owned as any).rows || owned)?.length || 0) === 0) {
  ...
  // Routes-split Tier 1: account deletion lifecycle moved to server/routes/admin-extra.ts.
  registerAccountRoutes(app, { authMiddleware, getTenantFromRequest });
  ...
  await registerDreamGraphRoutes(app, { authMiddleware, getTenantFromRequest, isPlatformAdmin });
  ...
  app.get("/api/analytics", authMiddleware, async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error("[analytics] Error:", errorMessage(err));
  ...
  // Routes-split Tier 1: templates CRUD + start moved to server/routes/tenants.ts.
  registerTemplatesRoutes(app, { authMiddleware, getTenantFromRequest, requirePlatformAdmin });
  ...
  app.post("/api/treasury/forecast", async (req, res) => {
    ...
    try {
      ...
      res.json(result);
    } catch (e: unknown) { res.status(500).json({ error: errorMessage(e) || "Forecast failed" }); }
  });
  app.post("/api/treasury/portfolio", async (req, res) => {
    ...
    try {
      ...
      res.json(result);
    } catch (e: unknown) { res.status(500).json({ error: errorMessage(e) || "Portfolio analysis failed" }); }
  });
  ...
  registerCodeProposalsRoutes(app, { getTenantFromRequest, requirePlatformAdmin });
  registerRepairHandoffRoute(app);
  ...
  registerSandboxRoutes(app, { authMiddleware, getTenantFromRequest, isAdminRequest, ADMIN_TENANT_ID });
  registerLoopPortfolioRoutes(app, { authMiddleware, mutateLimiter, getTenantFromRequest, isAdminRequest, ADMIN_TENANT_ID });
  ...
  const { registerAdminHvacMissedCallRoutes } = await import("./routes/admin-hvac-missed-call");
  registerAdminHvacMissedCallRoutes(app, { authMiddleware, getTenantFromRequest, isAdminRequest });
  ...
  registerAuditAdminRoutes(app, { authMiddleware, getTenantFromRequest, isAdminRequest, ADMIN_TENANT_ID });
  registerIncomeAdminRoutes(app, { authMiddleware, getTenantFromRequest, isAdminRequest, ADMIN_TENANT_ID });
  registerCmmcAdminRoutes(app, { authMiddleware, getTenantFromRequest, isAdminRequest });
  ...
  registerConversationsRoutes(app, { getTenantFromRequest, getTenantFromRequestAsync, ADMIN_TENANT_ID, isPlatformAdmin, mutateLimiter, validateModelForTenant });
  ...
  registerPlatformConfigRoutes(app, { getTenantFromRequest, requirePlatformAdmin, isPlatformAdmin, ADMIN_TENANT_ID });
  registerTenantBYOKRoutes(app, { getTenantFromRequest });
  registerArtifactRoutes(app, { authMiddleware, mutateLimiter, getTenantFromRequest });
  ...
  app.get("/api/insights/usage", async (req, res) => {
    ...
    try {
      ...
      console.error("[api/insights/usage] failed:", errorMessage(e));
      res.status(500).json({ error: errorMessage(e) || "insights-failed" });
    }
  });
  ...
  app.post("/api/insights/:id/apply", async (req, res) => {
    ...
    try {
      const id = parseInt(req.params.id as string);
      const bodyParse = z.object({ actionTaken: z.string().max(2000).optional() }).safeParse(req.body ?? {});
      if (!bodyParse.success) return res.status(400).json({ error: "actionTaken must be a string (max 2000 chars)" });
      const actionTaken = bodyParse.data.actionTaken;
