...
export const LOOP_CONTRACTS: LoopContract[] = [
  {
    ...
    workflow: null,
    trigger: "Heartbeat task `commitment_drafter` (every 2h at :20 — token audit 2026-07-30).",
    objective:
  ...
  },
  {
    ...
    check:
      "mapJuryDecision routes ONLY a strict-majority FIX (2/3, 3/4, …) with adequate concordance and no escalation flag to repo_surgeon, which itself runs typecheck/test and lands-or-rolls-back; budget is claimed atomically before any spend (fails closed).",
    feedback:
    ...
    readiness: {
      ...
      reportOnlyFirst:
        "JURY_AUTOAPPLY unset by default — nothing enters the queue; only strict-majority FIX verdicts route to the repo surgeon, which typechecks/tests and lands-or-rolls-back.",
    },
  },
  {
  ...
  },
  {
    ...
    failMode: "closed",
    verifier: "deterministic",
    spend: "capped", // R125+143 — compile fan-out claims via claimAutonomousBudget before the first LLM call
    readiness: {
      runLog: "[agent-knowledge-refresh]",
      reportOnlyFirst:
        "Deterministic regeneration from the live registry; the LLM compile pass reserves against the autonomous daily cap (claimAutonomousBudget) before spending; the wiring audit gates the result and a broken sync surfaces non-zero.",
    },
  },
  {
