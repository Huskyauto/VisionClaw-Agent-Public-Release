...
export async function validateAgentResponse(
  ...
  if (opts?.verifyContracts && opts.tenantId) {
    ...
    if (claimed.length && !file) {
      const failureText = "no concrete file path or URL was returned by any tool";
      for (const type of claimed) {
        issues.push(`DELIVERABLE_VERIFICATION_UNVERIFIED: ${type}: ${failureText}`);
        contractFailures.push({ type, failures: [failureText] });
      }
    } else if (claimed.length && file) {
      ...
      for (const type of claimed) {
        try {
          ...
          // Every non-passed result is unverified. Unknown type identifiers,
          // URL-only files, and timeouts must never allow the persona to claim
          // that a customer-facing artifact was successfully delivered.
          const unverified = result.status !== "passed" || result.passed !== true;
          if (unverified) {
            const label = result.status === "failed" ? "DELIVERABLE_VERIFICATION_FAILED" : "DELIVERABLE_VERIFICATION_UNVERIFIED";
          ...
          const failureText = `contract verification error: ${(e as Error).message}`;
          console.error(`[supervisor] ${failureText} for ${type}`);
          issues.push(`DELIVERABLE_VERIFICATION_UNVERIFIED: ${type}: ${failureText}`);
          contractFailures.push({ type, failures: [failureText] });
        }
      }
  ...
  const FALLBACK_MAP: Record<string, { condition: (err: string) => boolean; suggestion: string }[]> = {
    ...
    "delegate_task": [
      ...
      },
      {
        ...
        suggestion: "Delegation hit a transient initialization error. Do NOT retry delegation — instead, execute this task yourself using your tools: check_system_status, recall_context, search_memory, project, check_api_keys, list_models, etc.",
      },
      {
        condition: () => true,
        suggestion: "Delegation failed. Do the work yourself directly using your available tools (check_system_status, recall_context, search_memory, project, check_api_keys, list_models, etc.) instead of trying to delegate again.",
      },
    ],
    "send_email": [
      {
