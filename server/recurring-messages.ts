...
export async function createScheduledMessage(input: {
  ...
  try {
    ...
    // Durable-store boundary: agent_knowledge writes MUST pass the PII/secret
    // redaction guard (post-edit-code-review HIGH, 2026-07-08). Redact the
    // OBJECT before stringify (never the JSON string) per storage-boundary rule.
    const safeTitle = redactPiiForStorage(input.title).redacted;
    ...
    const inserted: any = await db.execute(sql`
      ...
      VALUES (${safeTitle}, ${safeContent}, 'recurring_message', 4, ${tenantId}, 'cron_scheduler')
      RETURNING id
    `);
    return {
      success: true,
    ...
    try {
      ...
      if (p.expandViaPersona) {
        try {
          ...
          const result: any = await processMessage(convId, p.prompt, { tenantId: r.tenant_id, source: "scheduler" });
