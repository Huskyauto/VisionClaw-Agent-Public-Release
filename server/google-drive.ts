...
async function listAllDrivePermissions(fileId: string): Promise<any[]> {
  const permissions: any[] = [];
  let pageToken: string | undefined;
  for (let page = 0; page < 20; page++) {
    const query = new URLSearchParams({
      fields: "permissions(id,type,role,deleted),nextPageToken",
      pageSize: "100",
...
async function listMarkedCustomerDeliveryRoots(): Promise<any[]> {
  const roots: any[] = [];
...
export async function verifyDriveFilePublicAccess(fileId: string): Promise<{ verified: boolean; reason: string }> {
  ...
  try {
    ...
    return { verified: false, reason: `api-error-${String(err?.message || err).slice(0, 80)}` };
  }
}
...
export function evaluatePublicDrivePermissions(permissions: unknown): { verified: boolean; reason: string } {
  const activeAnyonePermissions = Array.isArray(permissions)
    ? permissions.filter((permission: any) => permission?.type === "anyone" && !permission?.deleted)
    : [];
  if (activeAnyonePermissions.some((permission: any) => permission?.role !== "reader")) {
    return { verified: false, reason: "broader-anyone-permission" };
  }
  return activeAnyonePermissions.some((permission: any) => permission?.role === "reader")
...
export async function uploadToDrive(params: {
  ...
  try {
    const rootFolderId = params.parentFolderId
    ...
    if (!response.ok) {
      return { success: false, error: result?.error?.message || result?.message || `Google Drive upload failed with HTTP ${response.status}` };
    }
    ...
    if (!result?.id || typeof result.id !== "string") {
      return { success: false, error: "Google Drive upload response did not include a valid file ID" };
    }
