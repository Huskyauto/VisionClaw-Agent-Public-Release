...
function resolveLocalDriveSource(filePath: string): string {
  const cwd = process.cwd();
  let candidate = filePath;
  let resolved: string;
  if (candidate.startsWith("/uploads/") || candidate.startsWith("uploads/")) {
    resolved = resolveUploadsChild(candidate.replace(/^\/?uploads\//, ""));
  } else {
    if (candidate.startsWith("/attached_assets/") || candidate.startsWith("/stress-test-output/")) {
      candidate = candidate.slice(1);
    }
    resolved = path.resolve(cwd, candidate);
  }
  const sourceRoot = isPathWithin(UPLOADS_DIR, resolved) ? UPLOADS_DIR : cwd;
  if (!isPathWithin(sourceRoot, resolved) || !fs.existsSync(resolved)) {
    throw new Error(`Local source is unavailable or outside allowed roots: ${filePath}`);
  }
  const sourceStat = fs.lstatSync(resolved);
  if (!sourceStat.isFile() || sourceStat.isSymbolicLink()) {
    throw new Error(`Local source must be a regular non-symlink file: ${filePath}`);
  }
  const realResolved = fs.realpathSync(resolved);
  if (!isPathWithin(sourceRoot, realResolved)) {
    throw new Error(`Local source resolves outside allowed roots: ${filePath}`);
  }
  return realResolved;
}
...
export async function uploadToDrive(params: {
  ...
  try {
    ...
    if (params.fileData) {
      ...
      fileBuffer = fs.readFileSync(resolveLocalDriveSource(params.filePath));
    } else {
...
export async function downloadFromDrive(params: {
  ...
  try {
    ...
    const cwd = process.cwd();
    const uploadsMatch = savePath.match(/^\/?uploads\/(.+)$/);
    const resolved = uploadsMatch
      ? resolveUploadsChild(uploadsMatch[1])
      : path.resolve(cwd, savePath);
    if (!uploadsMatch && !isPathWithin(cwd, resolved)) {
      return { success: false, error: `Path traversal blocked: ${savePath}` };
    }
    if (!fs.existsSync(path.dirname(resolved))) fs.mkdirSync(path.dirname(resolved), { recursive: true });
...
export async function uploadAndShare(params: {
  ...
  if (params.generatedArtifact) {
    ...
    try {
      if (params.fileData) {
        ...
        artifactBytes = fs.readFileSync(resolveLocalDriveSource(params.filePath));
      }
  ...
  if (!result.success || !result.fileId) {
    ...
    if (_demoMode && params.filePath && !params.generatedArtifact) {
      ...
      const dest = resolveUploadsChild(localName);
      try {
        const src = resolveLocalDriveSource(params.filePath);
        if (!isPathWithin(UPLOADS_DIR, dest)) throw new Error("Path traversal blocked");
  ...
  if (params.registerProjectFile !== false && params.projectId && result.fileId && result.shareableLink) {
    try {
      ...
      if (!fileSize && params.filePath) {
        try {
          fileSize = fs.statSync(resolveLocalDriveSource(params.filePath)).size;
