...
import { missionSpendCapUsdCents } from "./mission-budget-guard";
 ...
 */
/** Staleness window after which an orphaned 'launching' claim (no sequence ever
 * created) is considered dead and reclaimable. Must stay comfortably above the
 * longest plausible createSequence duration (a local DB insert, sub-second). */
export const STALE_LAUNCH_CLAIM_MINUTES = 15;

/** True when a row is wedged in 'launching' with NO sequence and a stale
 * updated_at — the signature of a launch that died after claiming (and whose
 * rollback also failed). Safe to reclaim: no sequence exists, so no send
 * happened. Pure. */
export function isStaleLaunchClaim(exp: any, now: Date = new Date()): boolean {
  if (!exp || exp.status !== "launching" || exp.sequence_id) return false;
  const updated = exp.updated_at ? new Date(exp.updated_at) : null;
  if (!updated || Number.isNaN(updated.getTime())) return false;
  return now.getTime() - updated.getTime() > STALE_LAUNCH_CLAIM_MINUTES * 60 * 1000;
}
...
export function assertSendAllowed(exp: any): void {
  ...
  if (exp.status !== "approved") {
    // Stale-claim recovery: an owner-approved row wedged in 'launching' with no
    // sequence (dead prior launch whose rollback failed) may retry — the CAS
    // below re-verifies the same conditions atomically before reclaiming.
    if (isStaleLaunchClaim(exp)) return;
...
export async function runApprovedExperiment(args: {
  ...
  // Pre-spawn budget gate (S6b, fail closed): PROVE headroom against the
  // experiment's clamped spend cap before the launch claim. Email sends are
  // zero-marginal-cost (estimate 0), but the ceiling must be PROVEN headroom,
  // not assumed; unreadable spend refuses.
  assertBudgetHeadroom({
    spendSoFarUsdCents: mission.spend_usd_cents,
    spendCapUsdCents: clampCap(exp.max_spend_usd_cents, HARD_CAPS.maxSpendUsdCents),
    estimatedStepCostUsdCents: 0,
    stepLabel: "outreach launch",
  });
  // MISSION-level hard cap (Grok item #3, fail closed): the experiment cap
  // above is a per-experiment snapshot; this proves headroom against the
  // mission row's max_cash_at_risk_usd (clamped by the platform ceiling,
  // fail-closed to 0 on null/unreadable) so a mission can never keep
  // launching experiments past its own budget.
  assertBudgetHeadroom({
    spendSoFarUsdCents: mission.spend_usd_cents,
    spendCapUsdCents: missionSpendCapUsdCents(mission),
    estimatedStepCostUsdCents: 0,
    stepLabel: "outreach launch (mission cap)",
  });
  ...
  const claim = await db.execute(sql`
    ...
    WHERE tenant_id = ${args.tenantId} AND id = ${args.experimentId}
      ...
      AND (
        status = 'approved'
        -- Stale-claim recovery: a prior launch that died after claiming (and
        -- whose rollback also failed) leaves 'launching' + sequence_id NULL
        -- forever. Reclaimable once demonstrably stale — no sequence was ever
        -- created, so re-launching cannot double-send.
        OR (status = 'launching' AND updated_at < CURRENT_TIMESTAMP - INTERVAL '15 minutes')
      )
    RETURNING id
  `);
  ...
  try {
    ...
    try {
      ...
      // Do NOT swallow: a failed rollback leaves the row wedged in
      // 'launching' + sequence_id NULL, which blocks all future retries
      // (the 'approved' CAS never matches again). Surface BOTH errors so
      // the operator/repair loop knows the row needs recovery — the stale-
      // claim recovery clause in the launch CAS below will also self-heal
      // it after the staleness window.
      const launchMsg = e instanceof Error ? e.message : String(e);
      console.error(`[mission-experiment-run] ROLLBACK FAILED for exp ${args.experimentId} (tenant ${args.tenantId}) — row may be wedged in 'launching': ${rollbackErr?.message ?? rollbackErr}; original launch error: ${launchMsg}`);
      throw new Error(`launch failed AND claim rollback failed (exp ${args.experimentId} may be wedged in 'launching'; will self-heal via stale-claim recovery): launch=${launchMsg}; rollback=${rollbackErr?.message ?? rollbackErr}`);
    }
    throw e;
  }

  // Persist sequence_id IMMEDIATELY — before any enrollment. This closes the
  // duplicate-send window: if the process dies during the enroll loop below,
  // the row already carries sequence_id, so (a) the idempotent early-return
  // resumes with the existing sequence, and (b) the stale-claim recovery CAS
  // (gated on sequence_id IS NULL) can never reclaim it and create a second
  // sequence. Failing toward under-send (partial enrollment, no dupes).
  // Kill-race guard (72h-review round 3 HIGH): this UPDATE is also the abort
  // checkpoint. If the owner cancelled/paused the experiment between the
  // launch claim and here (status no longer 'launching'), RETURNING yields 0
  // rows — we must NOT enroll anyone and must NOT resurrect the row to 'live'.
  const seqPersist = await db.execute(sql`
    UPDATE mission_experiments
    SET sequence_id = ${seq.sequenceId}, updated_at = CURRENT_TIMESTAMP
    WHERE tenant_id = ${args.tenantId} AND id = ${args.experimentId}
      AND status = 'launching' AND sequence_id IS NULL
    RETURNING id
  `);
  const seqPersistRows = (seqPersist as any).rows ?? seqPersist;
  if (!Array.isArray(seqPersistRows) || seqPersistRows.length === 0) {
    console.warn(`[mission-experiment-run] launch aborted for exp ${args.experimentId} (tenant ${args.tenantId}): status changed (kill/pause) before sequence persist — no enrollment performed`);
    throw new Error(`launch aborted: experiment ${args.experimentId} was cancelled/changed before enrollment (no contacts enrolled, no sends)`);
  }

  let enrolled = 0;
  const skipped: Array<{ email: string; reason: string }> = [];
  let abortedMidEnroll = false;
  for (const p of prospects) {
    // 72h-review HIGH: honor owner kill/pause DURING enrollment, not just
    // before persist and at the final transition — otherwise a cancel that
    // lands mid-loop still enrolls (and emails) the remaining prospects.
    const liveStatus = await db.execute(sql`
      SELECT status FROM mission_experiments
      WHERE tenant_id = ${args.tenantId} AND id = ${args.experimentId} LIMIT 1
    `);
    const liveRows = (liveStatus as any).rows ?? liveStatus;
    const currentStatus = liveRows?.[0]?.status;
    if (currentStatus !== "launching") {
      console.warn(`[mission-experiment-run] enrollment aborted for exp ${args.experimentId} (tenant ${args.tenantId}): status='${currentStatus}' mid-loop after ${enrolled} enrollments — stopping immediately`);
      abortedMidEnroll = true;
      break;
    }
  ...
  // Guarded final transition (72h-review round 3 HIGH): only flip to 'live'
  // if the row is still in 'launching'. If an owner kill/pause landed during
  // the enroll loop, do NOT resurrect the row — leave the cancelled state
  // authoritative (enrollments already sent are handled by pause/stop paths).
  const liveFlip = await db.execute(sql`
    UPDATE mission_experiments
    SET status = 'live', enrolled_count = ${enrolled},
        dry_run = false, updated_at = CURRENT_TIMESTAMP
    WHERE tenant_id = ${args.tenantId} AND id = ${args.experimentId}
      AND status = 'launching'
    RETURNING id
  `);
  const liveFlipRows = (liveFlip as any).rows ?? liveFlip;
  const wentLive = !abortedMidEnroll && Array.isArray(liveFlipRows) && liveFlipRows.length > 0;
  // Contacts that were ALREADY enrolled before an abort still count toward
  // leads_contacted (the emails are real), but a cancelled experiment must
  // NOT advance the mission stage to experiment_live (72h-review round 2).
  if (enrolled > 0) {
    await db.execute(sql`
  ...
  try {
    if (wentLive && canTransition(mission.stage, "experiment_live")) {
