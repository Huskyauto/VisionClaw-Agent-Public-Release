...
import type { ProductionReviewInput } from "./reasoned-review-production";
import {
  validateWorkWithTypeSafe,
  unavailableTypeSafeValidation,
  type TypeSafeValidationInput,
  type TypeSafeValidationReceipt,
} from "./typesafe-auto-validation";
...
export const DRAFTER_ALLOWLIST = new Set([
  "deepseek/deepseek-v4.1-flash",  // .00015/.00060
  "z-ai/glm-5.2",                  // metered OpenRouter route
...
function parseEnsembleConfigJson(json: string): EnsembleConfig {
  ...
  if (raw && typeof raw === "object") {
    ...
    if (candidate.enabled === false) cfg.enabled = false;
    if (Array.isArray(candidate.drafters)) {
      const ids: string[] = candidate.drafters
    ...
    if (typeof candidate.aggregator === "string" && AGGREGATOR_ALLOWLIST.has(candidate.aggregator.trim())) {
      cfg.aggregator = candidate.aggregator.trim();
    } else if (typeof candidate.aggregator === "string" && candidate.aggregator.trim()) {
      console.warn(`[deliverable-ensemble] override aggregator "${candidate.aggregator.trim()}" not in allowlist — keeping ${cfg.aggregator}`);
    }
  }
  return cfg;
}

/** Test-only parser hook; production callers cannot choose a filesystem path. */
export function _parseEnsembleConfigJsonForTest(json: string): EnsembleConfig {
  try {
    return parseEnsembleConfigJson(json);
  } catch {
    return { enabled: process.env.PREMIUM_ENSEMBLE_ENABLED !== "0", drafters: [...DEFAULT_DRAFTERS], aggregator: DEFAULT_AGGREGATOR };
  }
}

export function loadEnsembleConfig(): EnsembleConfig {
  const defaults: EnsembleConfig = {
    enabled: process.env.PREMIUM_ENSEMBLE_ENABLED !== "0",
    drafters: [...DEFAULT_DRAFTERS],
    aggregator: DEFAULT_AGGREGATOR,
  };
  try {
    if (!fs.existsSync(OVERRIDE_FILE)) return defaults;
    return parseEnsembleConfigJson(fs.readFileSync(OVERRIDE_FILE, "utf8"));
  } catch (e: unknown) {
    // Audited fail-open (2026-08-14): override file is optional; defaults are the safe path.
    console.warn(`[deliverable-ensemble] override file unreadable, using defaults: ${errorMessage(e)}`);
    logSilentCatch("server/lib/deliverable-ensemble.ts (override read)", e, "expected");
  }
  return defaults;
}
...
export interface EnsembleDraftArgs {
  ...
  /**
   * Optional production-shadow sidecar. The baseline remains the aggregator
   * result unless the separately enabled, narrow internal canary passes every
   * reviewer/margin gate.
   */
  reasonedReview?: {
  ...
  /** Test seam for the reasoned-review adapter. Production callers omit it. */
  _reasonedReviewFn?: (input: ProductionReviewInput) => Promise<{
    decisionUsed: "baseline" | "canary";
    selectedCandidateId: string;
  }>;
  /** Test seam. Production callers must not supply this. */
  _validationFn?: (input: TypeSafeValidationInput) => Promise<TypeSafeValidationReceipt>;
}
...
export interface EnsembleDraftResult {
  ...
  drafters: string[];
  aggregator: string;
  typesafeValidation?: TypeSafeValidationReceipt;
}
  ...
  try {
    ...
    settled.forEach((r, i) => {
      if (r.status === "fulfilled" && r.value && r.value.length >= 150) {
        ...
        const why = r.status === "rejected" ? errorMessage(r.reason) : "empty/too-short output";
    ...
    try {
      ...
      if (finalText && finalText.length >= 150) {
        ...
        if (args.reasonedReview) {
          // Dynamic import keeps the paid ensemble's existing tests and free
          // path independent from the database-backed evidence adapter.
          const reviewApi = await import("./reasoned-review-production");
          const reviewInput = {
            tenantId: args.tenantId,
            deliverableType: args.reasonedReview.deliverableType,
            rubric: args.reasonedReview.rubric,
            baselineCandidateId: "baseline",
            candidates: [
              { id: "baseline", content: finalText, generatorModel: cfg.aggregator },
              ...drafts.map((draft, index) => ({
                id: `draft-${index + 1}`,
                content: draft.text,
                generatorModel: draft.model,
              })),
            ],
            provenance: { source: args.reasonedReview.source || "premium-deliverable-ensemble", label },
            canaryEligible: args.reasonedReview.canaryEligible === true,
          };
          const runReview = args._reasonedReviewFn || reviewApi.reviewDeliverableCandidates;
          if (reviewApi.isReasonedReviewCanaryActive(reviewInput)) {
            const review = await runReview(reviewInput);
            if (review.decisionUsed === "canary") {
              const selected = reviewInput.candidates.find((candidate) => candidate.id === review.selectedCandidateId);
              if (selected) selectedText = selected.content;
            }
          } else {
            // Shadow is advisory and must not add latency to the customer path.
            void runReview(reviewInput).catch((error) => {
              console.warn(`[deliverable-ensemble] reasoned-review shadow failed for ${label}:`, error instanceof Error ? error.message : "unknown");
            });
          }
        }
        let typesafeValidation: TypeSafeValidationReceipt | undefined;
        const validate = process.env.NODE_ENV === "test" && args._validationFn
          ? args._validationFn
          : validateWorkWithTypeSafe;
        try {
          typesafeValidation = await validate({
            tenantId: args.tenantId,
            subjectKind: "deliverable",
            goal: args.user,
            output: selectedText,
            evidence: [
              `Independent drafts: ${drafts.map((draft) => draft.model).join(", ")}`,
              `Aggregator: ${cfg.aggregator}`,
              args.reasonedReview?.rubric ? `Review rubric: ${args.reasonedReview.rubric}` : "",
            ].filter(Boolean).join("\n"),
          });
        } catch (error) {
          console.warn(`[deliverable-ensemble] TypeSafe validation failed outside adapter for ${label} (non-fatal):`,
            error instanceof Error ? error.message.slice(0, 120) : "unknown");
          typesafeValidation = unavailableTypeSafeValidation("deliverable");
        }
        return {
          text: selectedText,
          mode: "ensemble",
          drafters: drafts.map((d) => d.model),
          aggregator: cfg.aggregator,
          typesafeValidation,
        };
      }
      ...
      // Audited fail-open (2026-08-14): 'partial ensemble never ships' contract — fall back to the free path.
      console.warn(`[deliverable-ensemble] aggregator ${cfg.aggregator} failed for ${label}: ${errorMessage(e)} — falling back to free single-model path`);
      logSilentCatch("server/lib/deliverable-ensemble.ts (aggregator)", e, "expected");
    }
