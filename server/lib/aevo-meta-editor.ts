...
export async function gatherEvidence(opts: {
  ...
  try {
    const r: any = await db.execute(sql`
      ...
      WHERE tenant_id = ${tenantId}
        AND kind = 'tool'
        AND name = 'lookup_output_skill'
        AND started_at >= ${since}
        AND (input::jsonb ->> 'topic') = ${targetId}
    `);
  ...
  try {
    const r: any = await db.execute(sql`
      ...
      WHERE tenant_id = ${tenantId}
        AND kind = 'tool'
        AND name = 'grade_deliverable'
        AND started_at >= ${since}
        AND output::text ILIKE '%nearMissDimension%'
    `);
  ...
  return { windowDays, lookupCount, deliveryFailureCount, nearMissCount, recentFailureNotes, summaryText, compaction };
}
