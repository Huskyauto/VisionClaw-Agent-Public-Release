...
import { claimRunRecoveryEventSequence, recordRecoveryCheckpointSafely } from "../recovery-manifests";
...
export async function withRootSpan<T>(
  opts: {
    ...
    summary?: string;
    metadata?: Record<string, unknown>;
    /** Optional trusted identifiers make trace boundaries inspectable after a
     * restart. No caller-provided cursor or execution instruction is accepted. */
    recoveryScope?: { runId?: number; conversationId?: number };
  },
  ...
  const recoveryScope = opts.recoveryScope ? { ...opts.recoveryScope, traceId } : null;
  const allocateRecoveryEventIndex = async (fallback: number): Promise<number> => {
    if (!recoveryScope?.runId) return fallback;
    // Scope ownership is enforced again by the checkpoint write. If a run was
    // deleted between these operations, report-only capture safely no-ops.
    try {
      return (await claimRunRecoveryEventSequence(tenantId, recoveryScope.runId)) ?? fallback;
    } catch {
      console.warn("[recovery-manifest] could not allocate trace event index; trace evidence may be skipped");
      return fallback;
    }
  };
  ...
  if (recoveryScope) {
    await recordRecoveryCheckpointSafely({
      ...
      eventIndex: await allocateRecoveryEventIndex(0),
      state: { status: "running", kind: "chat" },
      payload: { agentName: opts.agentName ?? null, kind: "chat" },
    });
  }
    ...
    try {
      ...
      if (recoveryScope) {
        await recordRecoveryCheckpointSafely({
          ...
          eventIndex: await allocateRecoveryEventIndex(1),
          state: { status: "completed", kind: "chat" },
          payload: { agentName: opts.agentName ?? null, kind: "chat" },
        });
      }
      return out;
      ...
      if (recoveryScope) {
        await recordRecoveryCheckpointSafely({
          tenantId,
          scope: recoveryScope,
          eventType: "trace.root.failed",
          eventIndex: await allocateRecoveryEventIndex(1),
          state: { status: "failed", kind: "chat" },
          payload: { agentName: opts.agentName ?? null, kind: "chat", error: e?.message?.slice(0, 500) },
        });
      }
      throw e;
    }
