...
export async function ingestNewIdeabrowser(opts: {
  ...
  // R125+137.27: never-throws contract — surface the missing config as a
  // structured error instead of tagging rows under an empty inbox id.
  if (!GMAIL_INBOX_ID) {
    summary.errors.push(
      "GMAIL_INGEST_ADDRESS env var is not set — ideabrowser ingest disabled (fail closed).",
    );
    return summary;
  }
