...
export function validateSellableFilePath(relPath: string): string {
  ...
  // 72h-review HIGH: prefix checks alone are not symlink-safe — a symlink
  // under an allowed root can point at secrets/server code. Canonicalize and
  // re-verify the REAL path stays inside an allowed root.
  const realAbs = fs.realpathSync(abs);
  const realRoot = fs.realpathSync(PROJECT_ROOT);
  if (!realAbs.startsWith(realRoot + path.sep)) {
    throw new Error(`File resolves outside project root (symlink escape): ${rel}`);
  }
  const realRel = path.relative(realRoot, realAbs).replace(/\\/g, "/");
  if (!ALLOWED_FILE_ROOTS.some(root => realRel.startsWith(root))) {
    throw new Error(`File resolves outside allowed roots (symlink escape): ${rel} -> ${realRel}`);
  }
  return rel;
}
...
export async function registerProduct(input: RegisterProductInput) {
  ...
  if (input.missionId != null) {
    ...
    const missionResult = await db.execute(sql`
      SELECT 1 FROM revenue_missions WHERE id = ${input.missionId} AND tenant_id = ${input.tenantId} LIMIT 1
    `);
    if (!(missionResult as any).rows?.[0]) {
      throw new Error(`Mission ${input.missionId} is not available to this tenant`);
    }
  }

  if (input.postPurchaseSequenceId != null) {
    if (!Number.isSafeInteger(input.postPurchaseSequenceId) || input.postPurchaseSequenceId <= 0) {
      throw new Error("postPurchaseSequenceId must be a positive integer");
    }
    const sequenceResult = await db.execute(sql`
      SELECT 1 FROM outreach_sequences WHERE id = ${input.postPurchaseSequenceId} AND tenant_id = ${input.tenantId} LIMIT 1
    `);
    if (!(sequenceResult as any).rows?.[0]) {
      throw new Error(`Outreach sequence ${input.postPurchaseSequenceId} is not available to this tenant`);
    }
  }
