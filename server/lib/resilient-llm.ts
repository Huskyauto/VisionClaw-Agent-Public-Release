...
export interface ResilientChatInput {
  ...
  maxModels?: number;
  /** Disable the metered emergency rung for strictly bounded evaluation runs. */
  allowLastResort?: boolean;
  /** Optional outer adaptation ceiling; provider-client adaptation remains intact. */
  maxParamStrips?: number;
  /** Per-request SDK retry ceiling for callers that need a hard request cap. */
...
export interface ResilientChatResult {
  ...
  usedModel: string;
  /** Provider-served model id after provider-specific routing/alias resolution. */
  servedModel: string;
  failoverUsed: boolean;
  steps: ResilientStep[];
}

async function awaitBeforeDeadline<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
  if (signal.aborted) throw new Error("Resilient LLM deadline exceeded before provider selection");
  return new Promise<T>((resolve, reject) => {
    const onAbort = () => reject(new Error("Resilient LLM deadline exceeded during provider selection"));
...
// Goal-brief reframing for the LAST-RESORT super-expert rung (Bob 2026-08-11).
// The expensive escalation model gets one shot after every cheaper model has
// already failed on the exact same prompt — so instead of replaying that prompt
// verbatim, prepend a system-level brief telling it to work the GOAL end-to-end
// (decide the approach, self-check where it backfires) rather than execute the
// literal task the cheaper models fumbled. Fail-open: if the params don't carry
// a well-formed messages array, pass them through untouched.
const GOAL_BRIEF_SYSTEM = [
  "ESCALATION BRIEF: You are the final, most capable model in a failover chain — every cheaper model has already failed to produce a correct answer to the request below.",
  "Treat the request as a GOAL to be achieved end-to-end, not a task to execute literally. Decide the best approach yourself; if the literal framing is part of why others failed, reframe it.",
  "Before finalizing, briefly self-check the most likely way your answer is wrong or backfires, and fix it. Return only the corrected final answer in the format the request asks for.",
].join(" ");

function withGoalBrief(baseParams: Record<string, any>): Record<string, any> {
  try {
    const msgs = baseParams?.messages;
    if (!Array.isArray(msgs) || msgs.length === 0) return baseParams;
    return { ...baseParams, messages: [{ role: "system", content: GOAL_BRIEF_SYSTEM }, ...msgs] };
  } catch {
    return baseParams; // fail-open — never let the brief break the last-resort attempt
  }
}
...
async function callWithParamAdaptation(
  ...
  steps: ResilientStep[],
  label: string,
  maxParamStrips = MAX_PARAM_STRIPS,
  sdkMaxRetries?: number,
): Promise<any> {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
        return await client.chat.completions.create(
          createParams,
          sdkMaxRetries === undefined ? { signal } : { signal, maxRetries: sdkMaxRetries },
        );
    } catch (apiErr: any) {
      if (signal.aborted) throw apiErr; // deadline wins — no strip-retry
      if (attempt >= maxParamStrips) throw apiErr;
...
export async function resilientChatCompletion(input: ResilientChatInput): Promise<ResilientChatResult> {
  ...
  const available = await awaitBeforeDeadline(getAvailableModels(), input.signal);
  ...
  for (let modelAttempt = 0; modelAttempt < maxModels; modelAttempt++) {
    ...
    try {
      ...
      // Non-failover-eligible init errors (caller-side/permanent) throw
      // directly — they must NOT fall through to the last-resort rung below.
      if (!shouldFailover(reason)) throw await attachFriendly(initErr);
    ...
    try {
      ...
      return { response, usedModel: currentModelId, servedModel: actualModelId, failoverUsed, steps };
    } catch (execErr: any) {
  ...
  // ── LAST-RESORT super-expert rung (Bob 2026-07-01) ─────────────────────────
  // Every generic candidate is exhausted. One explicit, metered attempt on the
  // super-expert model (claude-fable-5) — deliberately kept OUT of the generic
  // ladders/tier routing so it can never become a default. Bounded by a small
  // process-local daily cap (claimLastResortSlot claims BEFORE spend) and the
  // caller's abort signal; disabled via LAST_RESORT_MODEL_ENABLED=false.
  if (
    input.allowLastResort !== false &&
    !input.signal.aborted &&
    !triedModels.has(LAST_RESORT_MODEL) &&
    !(input.excludeModels || []).includes(LAST_RESORT_MODEL) &&
    claimLastResortSlot(label)
  ) {
    try {
      const { client, actualModelId } = await getClientForModel(LAST_RESORT_MODEL, input.tenantId, {
        requiresTools: input.requiresTools,
        meteredOverride: true, // deliberate: the whole point is a paid backup when free/sub lanes all failed
      });
      steps.push({ model: LAST_RESORT_MODEL, event: "failover", detail: "last-resort escalation" });
      console.log(JSON.stringify({ evt: "llm_failover", scope: label, phase: "last-resort", from: currentModelId, to: LAST_RESORT_MODEL }));
      const response = await callWithParamAdaptation(
        client,
        withGoalBrief(input.baseParams),
        actualModelId,
        LAST_RESORT_MODEL,
        input.signal,
        steps,
      ...
      steps.push({ model: LAST_RESORT_MODEL, event: "success" });
      console.log(`[${label}] recovered via LAST-RESORT escalation: ${primary} → ${LAST_RESORT_MODEL} (METERED)`);
      return {
        response,
        usedModel: LAST_RESORT_MODEL,
        servedModel: actualModelId,
        failoverUsed: true,
        steps,
      };
    } catch (lastResortErr: any) {
      // The super-expert failed too — surface the ORIGINAL exhaustion error
      // (more representative of the systemic failure) with the escalation noted.
      steps.push({ model: LAST_RESORT_MODEL, event: "route-error", detail: classifyError(lastResortErr).reason });
      console.warn(`[${label}] last-resort ${LAST_RESORT_MODEL} also failed: ${String(lastResortErr?.message || lastResortErr).slice(0, 200)}`);
      lastError = lastError || lastResortErr;
    }
  }
