...
export async function flushNow(opts: { throwOnError?: boolean } = {}): Promise<number> {
  if (flushing) {
    if (opts.throwOnError) throw new Error("memory queue flush is already in progress");
    return 0;
  }
  flushing = true;
  stats.flush_runs++;
  ...
  try {
    ...
    for (const item of drained) {
      try {
        ...
        pending.set(dedupKey(item.tenantId, item.fact), item);
        firstError ??= e;
      }
    }
  } finally {
  ...
  if (firstError) {
    scheduleFlush();
    if (opts.throwOnError) throw firstError;
  }
  return written;
}
