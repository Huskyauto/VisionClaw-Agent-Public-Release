...
// Per-stage deadline for the plan/revise/classify LLM calls (longer than the
// per-question timeout because these handle the full draft/claim set). Bounds
// every provider call so a hung backend can't stall the verifier tool path.
...
async function planVerifications(
  ...
  const r = await createWithTimeout(client, {
    model: actualModelId,
    messages: [
...
// ════════════════════════════════════════════════════════════════════════════
// Graduated claim-verdict taxonomy + sampling cadence
//
// Concept emulated (NOT copied) from the ARS "Claim Verification Protocol"
// (Phase E) reviewed 2026-06-30. Where verifyWithCoVe above produces a binary
// revise-or-not pass, this grades EACH sampled claim on a 5-level severity scale
// and computes a pass/fail GATE — so a harmless paraphrase is no longer treated
// the same as a fabrication. Two sharper ideas borrowed:
//   1. Graded severity: VERIFIED < MINOR_DISTORTION < MAJOR_DISTORTION, plus
//      UNVERIFIABLE (answer can't confirm) and UNVERIFIABLE_ACCESS (needs a
//      source we don't hold — noted but non-blocking).
//   2. Cost-aware sampling cadence: spot-check ~30% during drafting, exhaustive
//      (100%) only at the final gate. Sampling runs the expensive independent-
//      answer calls only on the sampled subset.
// Reuses planVerifications + answerIndependently above. Quality fails OPEN: any
// internal failure returns gate=PASS_WITH_NOTES + warning (never hard-FAIL the
// pipeline on a verifier crash), per VisionClaw completion-verification policy.
// ════════════════════════════════════════════════════════════════════════════

// Pure taxonomy/gate/sampling logic lives in ./claim-verdict (provider-free so
// it can be unit-tested without pulling ../providers' top-level setInterval).
// Re-export it so existing importers of cove-verifier keep working.
export type { ClaimVerdict, VerdictGate, ClaimGrade, ClaimVerdictReport } from "./claim-verdict";
export { computeVerdictGate, planSampleSize } from "./claim-verdict";

export interface GradeClaimsOptions {
  draft: string;
  topic?: string;
  tenantId: number;
  mode?: "draft" | "final";   // default "final"
  maxQuestions?: number;       // 1..15, default 12
  modelTier?: "fast" | "balanced" | "powerful";
  sampleRatio?: number;        // draft-mode fraction, default 0.3
  minSample?: number;          // draft-mode floor, default 5
}

/** One cheap classification call: maps (claim, question, answer) triples to verdicts. */
async function classifyVerdicts(
  actualModelId: string,
  client: any,
  items: { claim: string; question: string; answer: string }[],
  topic: string | undefined,
): Promise<{ verdict: ClaimVerdict; detail: string }[]> {
  const system = [
    "You are a claim-verification grader.",
    "For each item you get a CLAIM extracted from a draft, the VERIFICATION_QUESTION,",
    "and an INDEPENDENT_ANSWER produced WITHOUT seeing the draft.",
    "Assign exactly one verdict per claim:",
    "- VERIFIED: independent answer confirms the claim (exact or within rounding).",
    "- MINOR_DISTORTION: claim paraphrases/rounds but meaning is preserved.",
    "- MAJOR_DISTORTION: claim oversimplifies, exaggerates, or misrepresents the answer.",
    "- UNVERIFIABLE: the independent answer cannot confirm the claim (it is UNCERTAIN,",
    "  says it does not know, or does not address the claim).",
    "- UNVERIFIABLE_ACCESS: confirming would require a specific source/document not",
    "  available to general knowledge (private dataset, paywalled paper, internal figure).",
    'Output ONLY JSON: {"items":[{"verdict":"...","detail":"<=120 chars why"}]}',
    "with EXACTLY one entry per input item, IN ORDER.",
  ].join(" ");
  const lines = items.map((it, i) =>
    `ITEM ${i + 1}\nCLAIM: ${it.claim}\nVERIFICATION_QUESTION: ${it.question}\nINDEPENDENT_ANSWER: ${it.answer}`,
  );
  const user = [topic ? `TOPIC: ${topic}` : "", ...lines].filter(Boolean).join("\n\n");
  const r = await createWithTimeout(client, {
    model: actualModelId,
    messages: [
      { role: "system", content: system },
    ...
    temperature: 0.1,
    max_tokens: Math.min(2000, 120 + items.length * 60),
    response_format: { type: "json_object" },
  }, PER_STAGE_TIMEOUT_MS);
  const raw = r?.choices?.[0]?.message?.content || "";
  const parsed = parseJsonLoose<{ items?: { verdict?: string; detail?: string }[] }>(raw);
  const out: { verdict: ClaimVerdict; detail: string }[] = [];
  const arr = Array.isArray(parsed?.items) ? parsed!.items! : [];
  for (let i = 0; i < items.length; i++) {
    const v = normalizeVerdict(arr[i]?.verdict || "");
    // Unparseable single verdict → UNVERIFIABLE_ACCESS (non-blocking) so a parse
    // gap never manufactures a FAIL.
    out.push({ verdict: v || "UNVERIFIABLE_ACCESS", detail: String(arr[i]?.detail || (v ? "" : "verdict unparseable")) });
  }
  return out;
}

/**
 * Public entry point — grade a draft's factual claims on the 5-level taxonomy
 * with a cost-aware sampling cadence. Never throws (fail-open): on any internal
 * failure returns a report with gate=PASS_WITH_NOTES (or PASS when there is
 * simply nothing to verify) and `warning` set.
 */
export async function gradeClaims(opts: GradeClaimsOptions): Promise<ClaimVerdictReport> {
  const t0 = Date.now();
  const draft = sanitizeDraft(opts.draft);
  const mode: "draft" | "final" = opts.mode === "draft" ? "draft" : "final";
  const maxQuestions = clampQuestions(opts.maxQuestions ?? 12);
  const tier = opts.modelTier || "balanced";
  let modelId = "";
  const base: ClaimVerdictReport = {
    mode,
    totalClaims: 0,
    sampledClaims: 0,
    samplingRatio: 0,
  ...
  try {
    if (!draft || draft.trim().length < 80) {
      return { ...base, durationMs: Date.now() - t0, warning: "draft too short to grade (<80 chars)" };
    }
    modelId = await getModelForTierAsync(tier, opts.tenantId);
    const { client, actualModelId } = await getClientForModel(modelId, opts.tenantId);

    const { questions, claims } = await planVerifications(modelId, actualModelId, client, draft, opts.topic, maxQuestions);
    const totalClaims = claims.length;
    if (totalClaims === 0) {
      return { ...base, modelUsed: modelId, durationMs: Date.now() - t0, warning: "planner extracted 0 verifiable claims" };
    }

    const k = planSampleSize(totalClaims, mode, opts.sampleRatio, opts.minSample);
    const sampledIdx = strideSampleIndices(totalClaims, k);

    const answers = await Promise.allSettled(
      sampledIdx.map((i) => answerIndependently(modelId, actualModelId, client, questions[i], opts.topic)),
    );
    const items = sampledIdx.map((i, j) => {
      const a = answers[j];
      const answer = a.status === "fulfilled" && a.value ? a.value : "UNCERTAIN: verification call failed";
      return { claim: claims[i], question: questions[i], answer };
    });

    const verdicts = await classifyVerdicts(actualModelId, client, items, opts.topic);
    const counts = emptyVerdictCounts();
    const grades: ClaimGrade[] = items.map((it, j) => {
      const v = verdicts[j]?.verdict || "UNVERIFIABLE_ACCESS";
      counts[v] = (counts[v] || 0) + 1;
      return { claim: it.claim, question: it.question, independentAnswer: it.answer, verdict: v, detail: verdicts[j]?.detail || "" };
    });

    return {
      mode,
      totalClaims,
      sampledClaims: items.length,
      samplingRatio: totalClaims > 0 ? items.length / totalClaims : 0,
      grades,
      counts,
      gate: computeVerdictGate(counts),
      modelUsed: modelId,
      durationMs: Date.now() - t0,
    };
  } catch (e: any) {
    // Quality fails OPEN — never hard-FAIL the pipeline on a verifier crash.
    return { ...base, gate: "PASS_WITH_NOTES", modelUsed: modelId, durationMs: Date.now() - t0, warning: `gradeClaims failed: ${e?.message || String(e)}` };
  }
}
