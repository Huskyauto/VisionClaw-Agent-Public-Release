...
export async function settleMissionCapital(tenantId: number, missionId: number): Promise<{ settled: boolean; marginUsdCents?: number }> {
  ...
  return db.transaction(async (tx) => {
    ...
    // CONTRIBUTION margin (CPT 5.6 priority 4): revenue − refunds − spend −
    // Stripe payment fees − mission-attributed variable API cost. Fee/API-cost
    // sums run inside this txn; if unreadable they THROW, the txn rolls back
    // (CAS claim included) and reconcileUnsettledCapital retries later —
    // never settle on an overstated figure.
    const feesRes = await tx.execute(sql`
    ...
    `);
    const apiRes = await tx.execute(sql`
      SELECT CEIL(COALESCE(SUM(cost_usd::numeric), 0) * 100)::bigint AS cents
      FROM agent_cost_ledger
      WHERE tenant_id = ${tenantId} AND mission_id = ${missionId}
    `);
    const fees = Number(rows(feesRes)[0]?.fees);
    const apiCost = Number(rows(apiRes)[0]?.cents);
    if (!Number.isSafeInteger(fees) || fees < 0 || !Number.isSafeInteger(apiCost) || apiCost < 0) {
      throw new Error(`agent-capital: mission ${missionId} economics unreadable — aborting settlement (fail closed, reconciler will retry)`);
    }
    // Fail closed on corrupt rollups too: `Number(x) || 0` would coerce junk
    // to $0 and OVERSTATE margin (e.g. unreadable refunds vanish). Throw —
    // the txn (CAS claim included) rolls back and the reconciler retries.
    const { requireNonNegInt } = await import("./mission-economics");
    const margin =
      requireNonNegInt(m.revenue_usd_cents, "revenue_usd_cents", missionId)
      - requireNonNegInt(m.refunds_usd_cents, "refunds_usd_cents", missionId)
      - requireNonNegInt(m.spend_usd_cents, "spend_usd_cents", missionId)
      - fees - apiCost;
