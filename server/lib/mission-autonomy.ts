 ...
 * Margin = revenue − refunds − spend − extraCostsUsdCents (cents), from
 * EVIDENCE-derived rollups only. extraCostsUsdCents carries the honest-margin
 * components (Stripe fees + mission-attributed variable API cost — CPT 5.6
 * priority 4) so reinvestment runs on CONTRIBUTION margin, never the
 * overstated realized figure. Returns null when nothing may be reinvested
 * (no verified profit, malformed inputs incl. malformed extra costs, or
 * budget already at/above target). Fail closed.
 */
export function computeReinvestment(mission: {
...
}, extraCostsUsdCents: unknown = 0): { newBudgetUsd: number; reinvestedUsd: number } | null {
  ...
  // Fail closed: unreadable extra costs must refuse reinvestment, never read
  // as $0. Explicit null/boolean would coerce to a clean number via Number()
  // (value-cap type-confusion class) — reject them before coercion. Only an
  // OMITTED argument (undefined ⇒ default 0) means "legacy caller, no extra
  // costs".
  const extraCosts = (extraCostsUsdCents === null || typeof extraCostsUsdCents === "boolean")
    ? null
    : toInt(extraCostsUsdCents);
  if (revenue == null || refunds == null || spend == null || budgetUsd == null || extraCosts == null) return null;
  const marginCents = revenue - refunds - spend - extraCosts;
