...
export async function checkSystemStatus() {
  ...
  const [convR, settingsR, personaR, memStatsR, heartbeatR, tasksR, logsR, msgCountR, webServerR] = await Promise.all([
    probe(storage.getConversations(undefined, undefined, ALL_TENANTS)),
    probe(storage.getSettings()),
    probe(storage.getActivePersona()),
    probe(storage.getMemoryStats(undefined, ALL_TENANTS)),
    probe(getHeartbeatFns().then(h => h.isHeartbeatRunning())),
    probe(storage.getHeartbeatTasks(undefined, ALL_TENANTS)),
    probe(storage.getHeartbeatLogs(5, undefined, ALL_TENANTS)),
  ...
  // Speculative-prefetch observability (Task: surface whether the LOOP
  // plan-replay speed-up earns its keep). Counters are in-memory and
  // synchronous (no DB/network), so this is inherently bounded — but keep it
  // fail-open so a stats bug can never sink the status check.
  let speculativePrefetch: any = null;
  try {
    const { getSpeculationStats } = await import("../speculative-prefetch");
    const s = getSpeculationStats();
    speculativePrefetch = {
      ...s,
      // hits / prefetches — how often a prefetched result was actually
      // consumed. Null until at least one prefetch happens (avoid 0/0).
      hitRate: s.prefetches > 0 ? Math.round((s.hits / s.prefetches) * 1000) / 1000 : null,
      // Sum of prefetch-time tool latencies consumed by hits — the actual
      // wall-clock seconds this feature has saved (the keep/retire signal).
      savedSeconds: Math.round((s.savedMs / 1000) * 100) / 100,
      disabled: process.env.SPEC_PREFETCH_DISABLED === "1",
    };
  } catch (_silentErr) { logSilentCatch("server/lib/system-probes.ts", _silentErr); }
