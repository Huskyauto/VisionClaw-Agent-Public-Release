...
import { scanCustomerFacingText, reportQualityIncident } from "./outbound-quality-gate";
...
export async function runDueScheduledPosts(): Promise<RunResult> {
  ...
  for (const row of due) {
    ...
    try {
      ...
      // R125+138 app-wide quality gate — a public post with generation-failure
      // debris (placeholders, raw provider errors, AI refusal text) must never
      // publish. Content is deterministic per row, so this is terminal (no
      // retry) and files a self-repair incident. Fail-closed for quality.
      const contentScan = scanCustomerFacingText(row.content);
      // R125+139 — scanner crash (fail-open) is telemetried, not silent. The
      // post still publishes (fail-open by design for scanner infra bugs).
      if (contentScan.degraded) {
        reportQualityIncident({
          tenantId: row.tenant_id,
          signature: "outbound_quality_gate_degraded",
          title: `scheduled post ${row.id} published UNSCANNED — quality-gate scanner crashed (fail-open)`,
          error: "scanCustomerFacingText returned degraded:true",
          stage: "scheduled-post",
          candidateFiles: ["server/lib/outbound-quality-gate.ts", "server/lib/scheduled-post-runner.ts"],
          metadata: { postId: row.id, platforms: row.platforms },
        });
      }
      if (contentScan.blocked) {
        const detail = contentScan.reasons.join("; ").slice(0, 1500);
        reportQualityIncident({
          tenantId: row.tenant_id,
          signature: "scheduled_post_quality_gate_blocked",
          title: `scheduled post ${row.id} blocked by quality gate`,
          error: detail,
          stage: "scheduled-post",
          candidateFiles: ["server/lib/scheduled-post-runner.ts", "server/lib/outbound-quality-gate.ts"],
          metadata: { postId: row.id, platforms: row.platforms },
        });
        await db.execute(sql`
          UPDATE scheduled_posts
             SET status = 'failed',
                 last_error = ${`quality-gate: ${detail}`.slice(0, 2000)},
      ...
      if (allOk) {
        ...
        nextStatus = "failed";
        // R125+138 — terminal publish failure (all platforms, retries
        // exhausted) now reports itself to the self-repair loop instead of
        // dying silently in a DB column nobody reads.
        reportQualityIncident({
          tenantId: row.tenant_id,
          signature: "scheduled_post_publish_failed_terminal",
          title: `scheduled post ${row.id} failed on all platforms after ${attempts} attempts`,
          error: lastErrors.join(" | ").slice(0, 2000) || "all platforms failed (no error detail)",
          stage: "scheduled-post",
          candidateFiles: ["server/social-publisher.ts", "server/lib/scheduled-post-runner.ts"],
          metadata: { postId: row.id, platforms: row.platforms, attempts },
        });
      }
        ...
        await db.execute(sql`
          UPDATE scheduled_posts
             SET status = ${failClosedStatus},
                 ...
                 updated_at = NOW()
           WHERE id = ${row.id} AND tenant_id = ${row.tenant_id} AND status = 'publishing'
        `);
