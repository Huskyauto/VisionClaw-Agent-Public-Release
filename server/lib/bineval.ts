...
export interface EvaluatorABResult {
   ...
   * claim is wider separation (no ceiling effect). Positive `advantage` = BINEVAL
   * discriminated the change more sharply than the holistic judge.
   */
  discrimination: { holisticSpread: number; binevalSpread: number; advantage: number };
  notes: string;
}
