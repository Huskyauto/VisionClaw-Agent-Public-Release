...
export async function generateBinaryQuestions(
  ...
  try {
    ...
    for (let i = 0; i < arr.length && out.length < cap; i++) {
      ...
      out.push({
        id: `q${out.length + 1}`,
        dimension: typeof item.dimension === "string" && item.dimension.trim() ? item.dimension.trim() : "general",
...
export async function answerBinaryQuestions(
  ...
  try {
    ...
    for (const e of arr) {
      ...
      if (typeof item.verdict !== "boolean") continue;
      out.push({ id: item.id, verdict: item.verdict, evidence: typeof item.evidence === "string" ? item.evidence : undefined });
    }
    return out;
  } catch {
    return [];
...
type EvaluatorTargetCompletion = (
  params: {
    model: string;
...
export async function runEvaluatorTarget(
  doc: string,
  input: string,
  cfg: Pick<EvaluatorABConfig, "targetModel" | "tenantId">,
  completionFn?: EvaluatorTargetCompletion,
): Promise<string> {
  const complete =
    completionFn ??
    (await import("../providers")).createBackgroundCompletion;
  const comp = await complete({
...
export async function runEvaluatorAB(args: {
  ...
  for (const c of cases) {
    caseIdx++;
    try {
      const outBefore = await runEvaluatorTarget(docBefore, c.input, cfg);
      const outAfter = await runEvaluatorTarget(docAfter, c.input, cfg);

      // Holistic judge (the current reward fn) on both outputs.
      const [hb, ha] = await Promise.all([
        holisticGrade(c.input, outBefore, c.rubric, c.reference, {
          graderModel: cfg.graderModel,
          tenantId: cfg.tenantId,
        }),
        holisticGrade(c.input, outAfter, c.rubric, c.reference, {
          graderModel: cfg.graderModel,
          tenantId: cfg.tenantId,
        }),
      ]);
