...
async function safeShadowRecord(args: {
  tenantId: number;
  jobId: string;
  event: "run_started" | "phase_changed" | "build_completed" | "build_failed";
  phase?: string;
  metadata?: Record<string, unknown>;
}): Promise<void> {
  try {
    const { recordBwbOversightShadow } = await import("./bwb-oversight-shadow");
    await recordBwbOversightShadow(args);
  } catch (error) {
    logSilentCatch("server/lib/bwb-job-progress.ts:safeShadowRecord", error);
  }
}

/**
 * Build the live-row WHERE. Both job and tenant are mandatory: callers without a
 * valid tenant fail closed before constructing a DB query. `notDone`/`notFailed`
 * keep late writes from resurrecting a finished row.
 */
function scope(jobId: string, tenantId: number, opts?: { notDone?: boolean; notFailed?: boolean }) {
  const clauses = [
...
export async function createBwbJob(args: {
  ...
  try {
    if (!envJobId(args.jobId)) return false;
    if (!Number.isFinite(args.tenantId) || args.tenantId <= 0) return false;
    const now = new Date();
    // Launch creation is a control-plane prerequisite, unlike the advisory
    // progress writes below. Use PostgreSQL's statement timeout so a timeout
    // cancels and rolls back the INSERT instead of leaving a ghost active row
    // after the caller has already returned started:false.
...
export async function getBwbJob(jobId: string, tenantId: number): Promise<{ jobId: string; status: string } | null> {
  try {
    if (!envJobId(jobId) || !Number.isFinite(tenantId) || tenantId <= 0) return null;
    const rows = await raceDb(db
      .select({ jobId: videoJobs.jobId, status: videoJobs.status })
      .from(videoJobs)
      .where(scope(jobId, tenantId))
      .limit(1));
    return rows?.[0] || null;
  } catch (e) {
    logSilentCatch("server/lib/bwb-job-progress.ts:getBwbJob", e);
    return null;
  }
}

type BwbLaunchClaimDeps = {
  create: typeof createBwbJob;
  get: typeof getBwbJob;
};

export async function claimBwbLaunchJob(
  args: { launchKey: string; tenantId: number; title?: string },
  deps: BwbLaunchClaimDeps = { create: createBwbJob, get: getBwbJob },
): Promise<{ jobId: string; created: boolean } | null> {
  let jobId = `vj_chat_${crypto.createHash("sha256").update(args.launchKey).digest("hex").slice(0, 24)}`;
  for (let generation = 0; generation < 64; generation += 1) {
    if (await deps.create({ jobId, tenantId: args.tenantId, title: args.title })) {
      return { jobId, created: true };
    }
    const existing = await deps.get(jobId, args.tenantId);
    if (!existing) return null;
    if (existing.status !== "done" && existing.status !== "failed") {
      return { jobId, created: false };
    }
    jobId = `vj_retry_${crypto.createHash("sha256").update(`${args.launchKey}:${jobId}`).digest("hex").slice(0, 24)}`;
  }
  return null;
}

export async function findActiveBwbLaunchJob(
  launchKey: string,
  tenantId: number,
  deps: Pick<BwbLaunchClaimDeps, "get"> = { get: getBwbJob },
): Promise<{ jobId: string; status: string } | null> {
  let jobId = `vj_chat_${crypto.createHash("sha256").update(launchKey).digest("hex").slice(0, 24)}`;
  for (let generation = 0; generation < 64; generation += 1) {
    const existing = await deps.get(jobId, tenantId);
    if (!existing) return null;
    if (existing.status !== "done" && existing.status !== "failed") return existing;
    jobId = `vj_retry_${crypto.createHash("sha256").update(`${launchKey}:${jobId}`).digest("hex").slice(0, 24)}`;
  }
  return null;
}
...
export async function setBwbPhase(
  phase: string,
  extra?: { jobId?: string; tenantId?: number; title?: string; status?: string; totalChapters?: number; attempt?: number },
): Promise<void> {
  try {
    const jobId = envJobId(extra?.jobId);
    const tenantId = envTenantId(extra?.tenantId);
    ...
    const updated = await raceDb(db
      .update(videoJobs)
      .set(set)
      .where(scope(jobId, tenantId, { notDone: true, notFailed: true }))
      .returning({ jobId: videoJobs.jobId }));
    if (updated?.length) {
      await safeShadowRecord({
        tenantId,
        ...
        event: "phase_changed",
        phase,
        metadata: { totalChapters: extra?.totalChapters, attempt: extra?.attempt },
      });
    }
  } catch (e) {
...
export async function updateBwbChapters(
  ...
  extra?: { jobId?: string; tenantId?: number; phase?: string; totalChapters?: number },
): Promise<void> {
  try {
    const jobId = envJobId(extra?.jobId);
    const tenantId = envTenantId(extra?.tenantId);
...
export async function completeBwbJob(args: {
  ...
  try {
    ...
    const tenantId = envTenantId(args.tenantId);
    if (!jobId || !tenantId) return;
    const now = new Date();
    ...
    if (updated?.length) {
      await safeShadowRecord({
        ...
        metadata: { totalChapters: cur.length },
      });
    }
  } catch (e) {
 ...
 */
export async function bumpBwbHeartbeat(jobIdArg?: string, tenantIdArg?: number): Promise<void> {
  try {
    const jobId = envJobId(jobIdArg);
    const tenantId = envTenantId(tenantIdArg);
    if (!jobId || !tenantId) return;
    await raceDb(db
      .update(videoJobs)
 ...
 */
export async function failBwbJob(errorMessage: string, jobIdArg?: string, tenantIdArg?: number): Promise<void> {
  try {
    const jobId = envJobId(jobIdArg);
    const tenantId = envTenantId(tenantIdArg);
    if (!jobId || !tenantId) return;
    ...
    const updated = await raceDb(db
      ...
      .where(scope(jobId, tenantId, { notDone: true }))
      .returning({ jobId: videoJobs.jobId }));
    if (updated?.length) {
      await safeShadowRecord({
        tenantId,
        jobId,
        event: "build_failed",
        metadata: { failureCategory: "other" },
      });
    }
  } catch (e) {
