...
export async function createMission(input: CreateMissionInput): Promise<any> {
  ...
  // Fail-closed ownership check: a caller-supplied projectId must belong to the
  // caller's tenant before it can be linked into a mission row (cross-tenant
  // guessing defense).
  if (input.projectId != null) {
    const owned = await db.execute(sql`
      SELECT id FROM projects WHERE id = ${input.projectId} AND tenant_id = ${input.tenantId} LIMIT 1
    `);
    if (rows(owned).length === 0) {
      throw new Error(`project ${input.projectId} not found for tenant ${input.tenantId} (fail closed)`);
    }
  }
...
export async function setStage(tenantId: number, id: number, to: MissionStage, killedReason?: string): Promise<any | null> {
  ...
  // Post-mission scored retrospective (S6a): fire on TERMINAL transitions only.
  // Record-only ledger, never a gate — best-effort, fire-and-forget, and the
  // transition above has already committed. Call-time dynamic import keeps this
  // module free of a static dependency on the retrospective lib.
  if (updated && (to === "killed" || to === "scale_ready")) {
    import("./mission-retrospective")
      .then(({ recordRetrospective }) => recordRetrospective(tenantId, id, to))
      .catch((e) => console.warn("[silent-catch] server/lib/revenue-missions.ts retrospective (record-only, best-effort):", (e as any)?.message ?? e));
    // Capital-pool settlement (S7b): idempotent (capital_settled_at CAS +
    // advisory lock inside), realized margin only. Best-effort here — the
    // stage transition is already committed; a failed settlement retries on
    // any later settleMissionCapital call because the CAS never claimed.
    import("./agent-capital")
      .then(({ settleMissionCapital }) => settleMissionCapital(tenantId, id))
      .catch((e) => console.warn("[silent-catch] server/lib/revenue-missions.ts capital settlement (idempotent, retryable):", (e as any)?.message ?? e));
  }
  return updated;
}

// ── Pre-spawn budget gate (S6b) ─────────────────────────────────────────────

/**
 * Remaining budget for a mission against a clamped spend cap. Pure —
 * fail-closed: any non-clean number (NaN, Infinity, non-number) yields 0
 * headroom (value-cap type-confusion defense).
 */
export function budgetHeadroomUsdCents(spendSoFar: unknown, spendCapUsdCents: unknown): number {
  // null coerces to 0 via Number() — treat it as unreadable, not free headroom.
  if (spendSoFar == null || spendCapUsdCents == null) return 0;
  const spend = Number(spendSoFar);
  const cap = Number(spendCapUsdCents);
  if (!Number.isFinite(spend) || !Number.isFinite(cap)) return 0;
  return Math.max(0, Math.floor(cap) - Math.max(0, Math.floor(spend)));
}

/**
 * Fail-closed pre-spawn budget gate: an expensive step (outreach launch, paid
 * research, generation) must PROVE headroom for its estimated cost BEFORE it
 * fires. Throws on: unreadable spend/cap, unreadable estimate, or estimate
 * exceeding remaining headroom. Zero-cost steps still require headroom > 0.
 */
export function assertBudgetHeadroom(args: {
  spendSoFarUsdCents: unknown;
  spendCapUsdCents: unknown;
  estimatedStepCostUsdCents?: unknown;
  stepLabel?: string;
}): { remainingUsdCents: number } {
  const label = args.stepLabel ?? "step";
  // Number(null) === 0 — a null/missing spend must refuse, not read as $0 spent.
  if (args.spendSoFarUsdCents == null) {
    throw new Error(`mission spend is unreadable — refusing ${label} (fail closed)`);
  }
  const spend = Number(args.spendSoFarUsdCents);
  if (!Number.isFinite(spend)) {
    throw new Error(`mission spend is unreadable — refusing ${label} (fail closed)`);
  }
  const remaining = budgetHeadroomUsdCents(spend, args.spendCapUsdCents);
  if (remaining <= 0) {
    throw new Error(`spend cap reached: ${Math.floor(spend)} >= cap usd cents — refusing ${label} (fail closed)`);
  }
  const rawEst = args.estimatedStepCostUsdCents ?? 0;
  const est = Number(rawEst);
  if (!Number.isFinite(est) || est < 0) {
    throw new Error(`estimated cost for ${label} is unreadable — refusing (fail closed)`);
  }
  if (Math.ceil(est) > remaining) {
    throw new Error(`insufficient budget headroom for ${label}: estimated ${Math.ceil(est)} > remaining ${remaining} usd cents (refusing, not truncating)`);
  }
  return { remainingUsdCents: remaining };
}
...
export async function addEvidence(input: AddEvidenceInput): Promise<any> {
  ...
  // Fail-closed ownership check: a caller-supplied experimentId must belong to
  // the caller's tenant before it is written onto an evidence row.
  if (input.experimentId != null) {
    const exp = await getExperiment(input.tenantId, input.experimentId);
    if (!exp) throw new Error(`experiment ${input.experimentId} not found for tenant ${input.tenantId} (fail closed)`);
  }
  const res = await db.execute(sql`
  ...
  const counterSql: Record<string, ReturnType<typeof sql>> = {
    ...
    spend: sql`spend_usd_cents = spend_usd_cents + ${Math.max(0, input.amountUsdCents ?? 0)}`,
  };
  ...
  // Hard capital cap (budget guard): a spend row that pushes cumulative spend
  // to/over the mission cap auto-pauses the mission. Recording is fact-keeping
  // (money already spent) so a guard failure NEVER unwinds the evidence row —
  // it is logged loud instead.
  if (input.type === "spend") {
    try {
      const { enforceMissionBudgetAfterSpend } = await import("./mission-budget-guard");
      await enforceMissionBudgetAfterSpend(input.tenantId, input.missionId);
    } catch (e) {
      console.error(`[revenue-missions] budget enforcement failed after spend record (evidence kept): ${(e as any)?.message ?? e}`);
    }
  }
  return ev;
}
...
export async function createExperimentDraft(args: {
  ...
  if (!mission) throw new Error("mission not found");
  // Pre-launch validation gate (S7a, DeepSeek gap #1): no experiment may be
  // drafted unless the mission's deterministic preflight validation scores at
  // or above threshold. Fail-closed — sits ABOVE every insert/side-effect.
  const { assertMissionValidated, assertDemandProven } = await import("./mission-validation");
  await assertMissionValidated(args.tenantId, args.missionId);
  // Proof-of-premise gate (Grok item #1): once >=1 experiment has actually
  // sent, a NEW draft requires at least one real demand signal — no doubling
  // down on silence. Fail-closed, sits ABOVE every insert/side-effect.
  await assertDemandProven(args.tenantId, args.missionId);
  ...
  // Atomic cap enforcement (72h review, round-2 hardened): a bare
  // INSERT...SELECT count guard is NOT race-proof at READ COMMITTED (two
  // concurrent statements can read the same pre-insert count). Serialize
  // per-tenant draft creation with pg_advisory_xact_lock (same pattern as
  // autonomous-budget claim), then re-check the count inside the lock.
  const res = await db.transaction(async (tx) => {
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext('mission-experiment-draft'), ${args.tenantId})`);
    return tx.execute(sql`
      ...
      WHERE (
        SELECT count(*) FROM mission_experiments
        WHERE tenant_id = ${args.tenantId}
          AND status = ANY(${`{${ACTIVE_EXPERIMENT_STATUSES.join(",")}}`}::text[])
      ) < ${HARD_CAPS.maxConcurrentExperiments}
      RETURNING *
    `);
  });
  if (rows(res).length === 0) {
    throw new Error(
      `concurrent experiment cap exceeded at insert time (race-guarded): >= ${HARD_CAPS.maxConcurrentExperiments} active (refusing to draft)`,
    );
  }
