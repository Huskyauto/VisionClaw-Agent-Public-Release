...
// vision call, so agents opt in with `imageSummaries: true` for visual papers or
// operators can enable it with INGEST_PDF_IMAGE_SUMMARIES=1.
...
const IMAGE_SUMMARY_MODEL = "gpt-5.4";
...
export async function ingestPaper(opts: {
  ...
  // figures/diagrams alongside the prose. PDF-only. Explicit true enables it;
  // otherwise the
  // INGEST_PDF_IMAGE_SUMMARIES env flag controls the default.
  ...
  await db.transaction(async (tx) => {
    ...
    for (const row of pending) {
      ...
      // Durable-store boundary: external paper text is untrusted ingest —
      // redact secrets/SSN/CC before persistence (post-edit-code-review HIGH,
      // 2026-07-08).
      const safeTitle = redactPiiForStorage(row.title).redacted;
      const safeContent = redactPiiForStorage(row.content).redacted;
      ...
      const inserted = await tx.execute(sql`
        ...
        VALUES
          (${tenantId}, ${safeTitle}, ${safeContent}, 'paper', ${row.priority}, ${sourceLabel},
