...
export interface JuryTriageOptions {
  ...
  tenantId: number;
  invokedVia?: string;
  // Bob 2026-06-26 — owner explicit-jury paid override (admin tenant only).
  // Threaded straight through to executeMoA; gated there by ownerJuryCeiling().
  meteredOverride?: boolean;
  /** Test seams. Production callers must not supply these. */
  _executeMoAFn?: typeof executeMoA;
 ...
 */
export function _tallyVotes(votes: { verdict: JuryVerdict }[], quorumSize = votes.length): { verdict: JuryVerdict; majority: number } {
  const tally: Record<JuryVerdict, number> = { FIX: 0, ACCEPT: 0, REJECT: 0, ESCALATE: 0 };
  for (const v of votes) tally[v.verdict]++;
  const need = Math.floor(quorumSize / 2) + 1; // original jury quorum never shrinks on a failure
...
export async function juryTriage(opts: JuryTriageOptions): Promise<JuryDecision> {
  ...
  const runMoA = process.env.NODE_ENV === "test" && opts._executeMoAFn ? opts._executeMoAFn : executeMoA;
  const r = await runMoA({
    question,
    tenantId: opts.tenantId,
    invokedVia: opts.invokedVia || "jury-triage",
    pool: "frontier",
    meteredOverride: opts.meteredOverride,
  });
  ...
  const votes: JuryVote[] = r.proposers.map((p: any) => {
    if (!p.ok || !p.answer) {
      return {
        model: p.modelId,
        provider: p.provider,
        requestedModel: p.requestedModel || p.modelId,
        actualModel: p.actualModel,
        providerLane: p.providerLane,
  ...
  // Failed seats are evidence, never votes. Their absence does not reduce the
  // original strict-majority quorum, so a singleton cannot become "unanimous".
  const { verdict, majority } = _tallyVotes(
    votes.filter(v => v.ok),
    r.jurySeatCount ?? r.proposers.filter(p => p.role !== "steelman").length,
  );
  ...
  const validate = process.env.NODE_ENV === "test" && opts._validationFn
    ? opts._validationFn
    : validateWorkWithTypeSafe;
  try {
    decision.typesafeValidation = await validate({
      tenantId: opts.tenantId,
      subjectKind: "jury",
      goal: [issue, opts.context || ""].filter(Boolean).join("\n\n").slice(0, 4000),
      output: [
        `VERDICT: ${verdict}`,
        `MAJORITY: ${majority}`,
        `AGGREGATOR: ${(r.aggregated || "").slice(0, 3000)}`,
        fixProposal ? `FIX PROPOSAL:\n${fixProposal.slice(0, 2000)}` : "",
      ].filter(Boolean).join("\n"),
      evidence: votes.map((vote) =>
        `${vote.model}: ${vote.verdict} — ${vote.rationale.slice(0, 500)}`,
      ).join("\n").slice(0, 3000),
    });
  } catch (error) {
    console.warn("[jury-triage] TypeSafe validation failed outside adapter (non-fatal):", (error as Error)?.message?.slice(0, 120));
    decision.typesafeValidation = unavailableTypeSafeValidation("jury");
  }
  return decision;
}
