...
export function normalizeRetiredModelId(modelId: string): string {
  const direct = RETIRED_MODEL_REPLACEMENTS[modelId];
  if (direct) return direct;
  const separator = modelId.lastIndexOf("/");
  if (separator >= 0) {
    const retiredSuffix = modelId.slice(separator + 1);
    return RETIRED_MODEL_REPLACEMENTS[retiredSuffix] || modelId;
  }
  return modelId;
}
...
export function stripRejectedParam(createParams: any, apiErr: any): string | null {
  ...
  // Newer OpenAI reasoning-class models (gpt-5 family, o-series) REJECT the
  // legacy max_tokens and demand max_completion_tokens ("Unsupported
  // parameter: 'max_tokens' ... Use 'max_completion_tokens' instead"). Swap
  // FORWARD. Presence checks disambiguate direction (the error message
  // mentions both param names).
  if (
    /max_tokens/i.test(msg) &&
    createParams.max_tokens !== undefined &&
    createParams.max_completion_tokens === undefined
  ) {
    createParams.max_completion_tokens = createParams.max_tokens;
    delete createParams.max_tokens;
...
>(client: T, onEvent?: (e: ParamAdaptEvent) => void): T {
  ...
  (client.chat.completions as any).create = async function paramAdaptedCreate(
    ...
    // Retired model ids are rewritten at the universal client boundary so even
    // stale direct SDK callsites cannot execute them. Preserve caller ownership.
    const normalizedModel = normalizeRetiredModelId(String(params?.model || ""));
    let working = normalizedModel === params?.model
      ? params
