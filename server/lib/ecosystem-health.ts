...
export async function computeEcosystemHealth(tenantId: number): Promise<EcosystemHealth | null> {
  ...
  try {
    ...
    const totalFailures = tools.reduce((s, t) => s + t.failures, 0);
    const worstTools = tools
      .filter(t => t.calls >= DEFAULTS.toolReliabilityMinCalls && t.failures > 0)
      .sort((a, b) => a.successRate - b.successRate)
      .slice(0, DEFAULTS.toolReliabilityTopN);
    const topTools = tools
      .slice()
      .sort((a, b) => b.calls - a.calls)
      .slice(0, DEFAULTS.toolReliabilityTopN)
      .map(t => ({ toolName: t.toolName, calls: t.calls, successRate: t.successRate }));
    // Delegation handoff aggregate — cheap fold-in from trace spans.
    ...
    toolReliability = {
      ...
      delegation,
      breached: tools.some(t => t.calls >= DEFAULTS.toolReliabilityMinCalls && t.successRate < DEFAULTS.toolReliabilityMinRate),
      degraded: false,
    };
  } catch (_silentErr) {
    logSilentCatch("server/lib/ecosystem-health.ts", _silentErr);
