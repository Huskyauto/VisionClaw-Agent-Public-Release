...
export interface EcosystemHealth {
  ...
  // Copilot public-repo review (2026-07 triage) — per-tool success-rate rollup.
  // Source is the single-funnel tool_performance ledger (executeTool finally
  // block): success_count/fail_count per (tenant, tool). Surfaces the worst
  // reliability offenders (min-call floor so one-off failures don't dominate)
  // plus a cheap delegation-handoff aggregate from agent_trace_spans
  // kind='delegate'. Degraded-safe.
  toolReliability: {
    toolCount: number;
    totalCalls: number;
    totalFailures: number;
    overallSuccessRate: number; // 0..1, weighted by call volume
    ...
    minCalls: number;      // floor for a tool to be eligible for worstTools/breach
    threshold: number;     // per-tool success-rate floor
    delegation: { spans: number; ok: number; failed: number; successRate: number; windowDays: number };
    breached: boolean;
    degraded: boolean;
  };
  // Simulation Sandbox (S5) — replay-run rollup. Breaches when a run in the
  // last 7 days carries a CRITICAL verdict (a was-blocked → now-allowed flip
  // under a proposed config change) or the latest run failed. Degraded-safe.
  sandbox: {
    totalRuns: number;
    runs7d: number;
    runningCount: number;
    /** 'running' rows older than 30 min — orphaned by a killed driver, not real work. */
  ...
  // Autonomy Scoreboard (Kimi K3 #5) — how much genuinely self-directed work
  // the platform does, in one card: self-originated initiatives (intention
  // engine), condition-based wakes, episode playbooks distilled/reused,
  // outcome-measured actions feeding routing, and inbound A2A tasks accepted.
  // Telemetry only — never contributes to anyBreached (autonomy volume is a
  // capability reading, not a pathology). Degraded-safe.
  autonomy: {
    windowDays: number;
  ...
  // Tool Mis-Picks (R125+137.64) — record-only telemetry: within one turn,
  // tool A failed and a DIFFERENT tool B later succeeded. Surfaces the top
  // failed→succeeded pairs so doctrine/routing can be tuned. Never breaches.
  mispicks: import("./tool-mispick").MispickSummary;
...
const DEFAULTS = {
  ...
  unmetTopN: 5,
  // Per-tool reliability rollup — a tool needs at least this many recorded
  // calls before it can appear in worstTools or trip the breach flag, and
  // breaches when its lifetime success rate falls below the floor.
  toolReliabilityMinCalls: 20,
  toolReliabilityMinRate: 0.8,
  toolReliabilityTopN: 8,
  toolReliabilityDelegationWindowDays: 30,
};
...
export async function computeEcosystemHealth(tenantId: number): Promise<EcosystemHealth | null> {
  ...
  // 15) Per-tool success-rate rollup (Copilot public-repo review triage) —
  //     reads the single-funnel tool_performance ledger. Worst offenders are
  //     gated on a min-call floor so a one-off failure on a rarely-used tool
  //     doesn't dominate. Delegation aggregate from agent_trace_spans
  //     kind='delegate' (last N days). Degraded-safe.
  let toolReliability: EcosystemHealth["toolReliability"] = {
    toolCount: 0, totalCalls: 0, totalFailures: 0, overallSuccessRate: 1,
    worstTools: [], topTools: [],
    minCalls: DEFAULTS.toolReliabilityMinCalls, threshold: DEFAULTS.toolReliabilityMinRate,
    delegation: { spans: 0, ok: 0, failed: 0, successRate: 1, windowDays: DEFAULTS.toolReliabilityDelegationWindowDays },
    breached: false, degraded: false,
  };
  try {
    // Aggregates + breach detection computed in SQL — no full-table
    // materialization in Node on this 60s-polled admin endpoint.
    const aggRes = await withProbeTimeout(db.execute(sql`
      SELECT
        COUNT(*)::int AS tool_count,
        COALESCE(SUM(success_count + fail_count), 0)::bigint AS total_calls,
        COALESCE(SUM(fail_count), 0)::bigint AS total_failures,
        COUNT(*) FILTER (
          WHERE success_count + fail_count >= ${DEFAULTS.toolReliabilityMinCalls}
            AND success_count::float / (success_count + fail_count) < ${DEFAULTS.toolReliabilityMinRate}
        )::int AS breach_count
      FROM tool_performance
      WHERE tenant_id = ${tenantId}
        AND success_count + fail_count > 0
    `), "toolReliability.agg");
    const agg = (((aggRes as any).rows || aggRes) as any[])[0] || {};
    const toolCount = Number(agg.tool_count) || 0;
    const totalCalls = Number(agg.total_calls) || 0;
    const totalFailures = Number(agg.total_failures) || 0;
    const breachCount = Number(agg.breach_count) || 0;

    const mapRow = (r: any) => {
      const ok = Number(r.success_count) || 0;
      const fail = Number(r.fail_count) || 0;
      const calls = ok + fail;
      // Failure reasons can carry raw provider/debug text — strip anything
      // credential/PII-shaped and cap the length before it reaches the client.
      const rawReason = r.last_failure_reason ? String(r.last_failure_reason) : null;
      const safeReason = rawReason
        ? redactPiiForStorage(rawReason, { redactContactInfo: true }).redacted.slice(0, 200)
        : null;
      return {
        toolName: String(r.tool_name),
        calls,
        failures: fail,
        successRate: calls > 0 ? Math.round((ok / calls) * 1000) / 1000 : 1,
        lastFailureReason: safeReason,
        lastFailureAt: r.last_failure_at ? new Date(r.last_failure_at).toISOString() : null,
      };
    };

    const worstRes = await withProbeTimeout(db.execute(sql`
      SELECT tool_name, success_count, fail_count, last_failure_reason, last_failure_at
      FROM tool_performance
      WHERE tenant_id = ${tenantId}
        AND success_count + fail_count >= ${DEFAULTS.toolReliabilityMinCalls}
        AND fail_count > 0
      ORDER BY success_count::float / (success_count + fail_count) ASC
      LIMIT ${DEFAULTS.toolReliabilityTopN}
    `), "toolReliability.worst");
    const worstTools = (((worstRes as any).rows || worstRes) as any[]).map(mapRow);

    const topRes = await withProbeTimeout(db.execute(sql`
      ...
      LIMIT ${DEFAULTS.toolReliabilityTopN}
    `), "toolReliability.top");
    const topTools = (((topRes as any).rows || topRes) as any[]).map(r => {
      const ok = Number(r.success_count) || 0;
      const fail = Number(r.fail_count) || 0;
      const calls = ok + fail;
      return { toolName: String(r.tool_name), calls, successRate: calls > 0 ? Math.round((ok / calls) * 1000) / 1000 : 1 };
    });

    // Delegation handoff aggregate — cheap fold-in from trace spans. A failure
    // here marks the WHOLE probe degraded (honest partial-telemetry surfacing).
    let delegation = toolReliability.delegation;
    let delegationDegraded = false;
    try {
      const dRes = await withProbeTimeout(db.execute(sql`
        SELECT
          COUNT(*)::int AS spans,
          COUNT(*) FILTER (WHERE status = 'ok')::int AS ok,
          COUNT(*) FILTER (WHERE status IN ('error','declined'))::int AS failed
        FROM agent_trace_spans
        WHERE tenant_id = ${tenantId}
          AND kind = 'delegate'
          AND started_at > NOW() - (${DEFAULTS.toolReliabilityDelegationWindowDays}::int * INTERVAL '1 day')
      `), "toolReliability.delegation");
      const dr = (((dRes as any).rows || dRes) as any[])[0] || {};
      const spans = Number(dr.spans) || 0;
      const okD = Number(dr.ok) || 0;
      const failedD = Number(dr.failed) || 0;
      const settled = okD + failedD;
      delegation = {
        spans, ok: okD, failed: failedD,
        successRate: settled > 0 ? Math.round((okD / settled) * 1000) / 1000 : 1,
        windowDays: DEFAULTS.toolReliabilityDelegationWindowDays,
      };
    } catch (_silentErr) {
      logSilentCatch("server/lib/ecosystem-health.ts", _silentErr);
      delegationDegraded = true;
    }
    toolReliability = {
      toolCount,
      totalCalls,
      totalFailures,
      overallSuccessRate: totalCalls > 0 ? Math.round(((totalCalls - totalFailures) / totalCalls) * 1000) / 1000 : 1,
      worstTools,
      topTools,
      minCalls: DEFAULTS.toolReliabilityMinCalls,
      threshold: DEFAULTS.toolReliabilityMinRate,
      delegation,
      breached: breachCount > 0,
      degraded: delegationDegraded,
    };
  } catch (_silentErr) {
    logSilentCatch("server/lib/ecosystem-health.ts", _silentErr);
    toolReliability = { ...toolReliability, degraded: true };
  }

  // 16) Simulation Sandbox rollup (S5) — replay runs + latest verdict.
  //     A CRITICAL verdict in the last 7 days (block→allow flip under a
  //     proposed config) or a failed latest run breaches the card.
  let sandbox: EcosystemHealth["sandbox"] = {
    totalRuns: 0, runs7d: 0, runningCount: 0, staleRunningCount: 0, failed7d: 0, critical7d: 0,
    latest: null, breached: false, degraded: false,
  };
  try {
    const aggRes = await withProbeTimeout(db.execute(sql`
      SELECT
        ...
        COUNT(*) FILTER (WHERE status = 'running' AND started_at > NOW() - INTERVAL '30 minutes')::int AS running_count,
        COUNT(*) FILTER (WHERE status = 'running' AND started_at <= NOW() - INTERVAL '30 minutes')::int AS stale_running_count,
        COUNT(*) FILTER (WHERE status = 'failed' AND started_at > NOW() - INTERVAL '7 days')::int AS failed_7d,
        COUNT(*) FILTER (WHERE started_at > NOW() - INTERVAL '7 days' AND report->>'verdict' = 'CRITICAL')::int AS critical_7d
      FROM sandbox_runs
      WHERE tenant_id = ${tenantId}
    `), "sandbox.agg");
    const agg = (((aggRes as any).rows || aggRes) as any[])[0] || {};
    const latestRes = await withProbeTimeout(db.execute(sql`
      SELECT id, corpus, status, sample_size, report->>'verdict' AS verdict, started_at, completed_at
      FROM sandbox_runs
      WHERE tenant_id = ${tenantId}
      ORDER BY id DESC LIMIT 1
    `), "sandbox.latest");
    const latestRow = (((latestRes as any).rows || latestRes) as any[])[0];
    const latest = latestRow ? {
      runId: Number(latestRow.id),
      corpus: String(latestRow.corpus),
      status: String(latestRow.status),
      verdict: latestRow.verdict ? String(latestRow.verdict) : null,
      sampleSize: Number(latestRow.sample_size) || 0,
      startedAt: latestRow.started_at ? new Date(latestRow.started_at).toISOString() : null,
      completedAt: latestRow.completed_at ? new Date(latestRow.completed_at).toISOString() : null,
    } : null;
    const critical7d = Number(agg.critical_7d) || 0;
    sandbox = {
      totalRuns: Number(agg.total_runs) || 0,
      runs7d: Number(agg.runs_7d) || 0,
      runningCount: Number(agg.running_count) || 0,
      staleRunningCount: Number(agg.stale_running_count) || 0,
      failed7d: Number(agg.failed_7d) || 0,
      critical7d,
      latest,
      breached: critical7d > 0 || latest?.status === "failed",
      degraded: false,
    };
  } catch (_silentErr) {
    logSilentCatch("server/lib/ecosystem-health.ts", _silentErr);
    sandbox = { ...sandbox, degraded: true };
  }

  // Autonomy Scoreboard (Kimi K3 #5) — one bounded query per source, all
  // tenant-scoped, telemetry only. Any failure flags degraded, never breaches.
  const AUTONOMY_WINDOW_DAYS = 7;
  let autonomy: EcosystemHealth["autonomy"] = {
    windowDays: AUTONOMY_WINDOW_DAYS,
    initiatives: { total: 0, recent: 0, committed: 0 },
    conditionWakes: { activeConditional: 0, firedRecent: 0 },
    playbooks: { total: 0, distilledRecent: 0, hitsTotal: 0 },
    outcomes: { measuredRecent: 0, successRecent: 0 },
    a2aTasks: { recent: 0 },
    breached: false,
    degraded: false,
  };
  try {
    const cutoffExpr = sql`now() - make_interval(days => ${AUTONOMY_WINDOW_DAYS})`;
    const aRows: any = await withProbeTimeout(db.execute(sql`
      SELECT
        (SELECT count(*)::int FROM self_initiatives WHERE tenant_id = ${tenantId}) AS init_total,
        (SELECT count(*)::int FROM self_initiatives WHERE tenant_id = ${tenantId} AND created_at > ${cutoffExpr}) AS init_recent,
        (SELECT count(*)::int FROM self_initiatives WHERE tenant_id = ${tenantId} AND status IN ('committed','executing','completed')) AS init_committed,
        (SELECT count(*)::int FROM agent_wake_schedules WHERE tenant_id = ${tenantId} AND status = 'pending' AND trigger_event IS NOT NULL) AS wakes_conditional,
        (SELECT count(*)::int FROM agent_wake_schedules WHERE tenant_id = ${tenantId} AND trigger_event IS NOT NULL AND context ? 'triggeredByEvent' AND updated_at > ${cutoffExpr}) AS wakes_fired,
        (SELECT count(*)::int FROM episode_playbooks WHERE tenant_id = ${tenantId}) AS pb_total,
        (SELECT count(*)::int FROM episode_playbooks WHERE tenant_id = ${tenantId} AND created_at > ${cutoffExpr}) AS pb_recent,
        (SELECT COALESCE(sum(hit_count), 0)::int FROM episode_playbooks WHERE tenant_id = ${tenantId}) AS pb_hits,
        (SELECT count(*)::int FROM action_outcomes WHERE tenant_id = ${tenantId} AND outcome_status IN ('success','failure') AND action_timestamp > ${cutoffExpr}) AS out_measured,
        (SELECT count(*)::int FROM action_outcomes WHERE tenant_id = ${tenantId} AND outcome_status = 'success' AND action_timestamp > ${cutoffExpr}) AS out_success,
        (SELECT count(*)::int FROM conversations WHERE tenant_id = ${tenantId} AND title LIKE 'A2A: %' AND created_at > ${cutoffExpr}) AS a2a_recent
    `), "autonomy");
    const a = (((aRows as any).rows || aRows) as any[])[0] || {};
    autonomy = {
      windowDays: AUTONOMY_WINDOW_DAYS,
      initiatives: { total: Number(a.init_total) || 0, recent: Number(a.init_recent) || 0, committed: Number(a.init_committed) || 0 },
      conditionWakes: { activeConditional: Number(a.wakes_conditional) || 0, firedRecent: Number(a.wakes_fired) || 0 },
      playbooks: { total: Number(a.pb_total) || 0, distilledRecent: Number(a.pb_recent) || 0, hitsTotal: Number(a.pb_hits) || 0 },
      outcomes: { measuredRecent: Number(a.out_measured) || 0, successRecent: Number(a.out_success) || 0 },
      a2aTasks: { recent: Number(a.a2a_recent) || 0 },
      breached: false,
      degraded: false,
    };
  } catch (_silentErr) {
    logSilentCatch("server/lib/ecosystem-health.ts", _silentErr);
    autonomy = { ...autonomy, degraded: true };
  }

  // 18) Tool Mis-Picks (R125+137.64) — telemetry-only, never breaches.
  let mispicks: EcosystemHealth["mispicks"] = {
    windowDays: 30, totalMispicks: 0, distinctFailedTools: 0, topPairs: [], degraded: false,
  };
  try {
    const { summarizeMispicks } = await import("./tool-mispick");
    mispicks = await withProbeTimeout(summarizeMispicks(tenantId), "mispicks");
  } catch (_silentErr) {
    logSilentCatch("server/lib/ecosystem-health.ts", _silentErr);
    mispicks = { ...mispicks, degraded: true };
    probesDegraded.push("mispicks");
  }
