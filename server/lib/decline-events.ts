...
export async function recordDecline(opts: RecordDeclineOpts): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    if (opts.personaId != null) {
      if (!Number.isInteger(opts.personaId) || opts.personaId <= 0) {
        return { ok: false, error: "personaId must be a positive integer" };
      }
      const persona: any = await db.execute(sql`
        SELECT id FROM tenant_persona_names
        WHERE tenant_id = ${opts.tenantId} AND persona_id = ${opts.personaId}
        LIMIT 1
      `);
      if (((persona.rows ?? persona) as any[]).length === 0) {
        return { ok: false, error: `persona ${opts.personaId} not found for tenant ${opts.tenantId}` };
      }
    }
    if (opts.conversationId != null) {
      if (!Number.isInteger(opts.conversationId) || opts.conversationId <= 0) {
        return { ok: false, error: "conversationId must be a positive integer" };
      }
      const conversation: any = await db.execute(sql`
        SELECT id FROM conversations
        WHERE tenant_id = ${opts.tenantId} AND id = ${opts.conversationId}
        LIMIT 1
      `);
      if (((conversation.rows ?? conversation) as any[]).length === 0) {
        return { ok: false, error: `conversation ${opts.conversationId} not found for tenant ${opts.tenantId}` };
      }
    }
