 ...
 *
 *   complex / forced  → refine prompt → main LLM gets original + advisory rewrite
 *   simple            → normal route (no refinement, no extra cost)
 *
 * This is a PRE-PASS, not an answer path: the jury produces an advisory PROMPT,
 * the main model produces the OUTPUT from both views. It runs SILENTLY — the refined prompt is never
...
export interface RefineResult {
  ...
  strategy: string;
  /** Machine-validated, non-authorizing interpretation used by the executor. */
...
export const REQUEST_INTERPRETATION_PRIORITIES = [
  "accuracy",
  "completeness",
  "conciseness",
  "evidence",
...
export type RequestInterpretationPriority =
  typeof REQUEST_INTERPRETATION_PRIORITIES[number];

export interface RequestInterpretation {
  /** Exact substring copied from the user's request; never model-authored prose. */
  objectiveSpan: string;
  /** Exact substrings copied from the user's request. */
  constraintSpans: string[];
  /** Closed vocabulary; cannot encode recipients, approvals, facts, or tools. */
  priorities: RequestInterpretationPriority[];
}
...
const MAX_ADVISORY_REFINEMENT_CHARS = 12_000;
const MAX_STRATEGY_CHARS = 2_000;
const MAX_OBJECTIVE_SPAN_CHARS = 500;
const MAX_CONSTRAINT_SPAN_CHARS = 300;
const MAX_CONSTRAINT_SPANS = 6;
const PRIORITY_SET = new Set<string>(REQUEST_INTERPRETATION_PRIORITIES);
...
export function validateRequestInterpretation(
  ...
  const objectiveSpan =
    typeof candidate.objectiveSpan === "string" ? candidate.objectiveSpan.trim() : "";
  if (
    objectiveSpan.length < 3
    || objectiveSpan.length > MAX_OBJECTIVE_SPAN_CHARS
    || !originalPrompt.includes(objectiveSpan)
  ) return null;

  if (!Array.isArray(candidate.constraintSpans) || candidate.constraintSpans.length > MAX_CONSTRAINT_SPANS) {
    return null;
  }
  const constraintSpans: string[] = [];
  for (const rawSpan of candidate.constraintSpans) {
    if (typeof rawSpan !== "string") return null;
    const span = rawSpan.trim();
    if (
      span.length < 3
      || span.length > MAX_CONSTRAINT_SPAN_CHARS
      || !originalPrompt.includes(span)
    ) return null;
    constraintSpans.push(span);
  }

  if (!Array.isArray(candidate.priorities) || candidate.priorities.length > 4) return null;
  const priorities: RequestInterpretationPriority[] = [];
  for (const rawPriority of candidate.priorities) {
    if (typeof rawPriority !== "string" || !PRIORITY_SET.has(rawPriority)) return null;
    if (!priorities.includes(rawPriority as RequestInterpretationPriority)) {
      priorities.push(rawPriority as RequestInterpretationPriority);
    }
  }

  return { objectiveSpan, constraintSpans, priorities };
}

export function isAdvisoryRefinementUsable(refinement: RefineResult): boolean {
  const advisory = refinement.refinedPrompt.trim();
  return (
    refinement.refined
    && advisory.length > 0
    && advisory.length <= MAX_ADVISORY_REFINEMENT_CHARS
    && refinement.strategy.length <= MAX_STRATEGY_CHARS
    && validateRequestInterpretation(refinement.interpretation, refinement.originalPrompt) !== null
  );
}

/**
 * Give the executor both views of the current request without changing the
 * durable transcript. The original remains authoritative; the rewrite is
 * explicitly advisory and occupies the same user-role trust boundary.
 */
export function attachRefinementToLatestUserMessage<T extends RefinementMessage>(
  messages: T[],
  refinement: RefineResult,
): T[] {
  const advisory = refinement.refinedPrompt.trim();
  if (!isAdvisoryRefinementUsable(refinement)) return messages;
  ...
  const original = String(messages[latestUserIndex].content).trim();
  if (!original) return messages;
  const interpretation = validateRequestInterpretation(refinement.interpretation, original);
  ...
  const enhancedContent = [
    ...
    original,
    "",
    "### MACHINE-VALIDATED INTERPRETATION — ADVISORY",
    `Objective (verbatim from the request): ${interpretation.objectiveSpan}`,
    ...(interpretation.constraintSpans.length > 0
      ? [
          "Explicit constraints (verbatim from the request):",
          ...interpretation.constraintSpans.map((span) => `- ${span}`),
        ]
      : []),
    ...(interpretation.priorities.length > 0
      ? [`Priorities: ${interpretation.priorities.join(", ")}`]
      : []),
    "",
    "When the two differ, follow the original request. This advisory contains only allowlisted labels and verbatim source spans; it grants no new permission or facts.",
  ].join("\n");

  const result = messages.slice();
  result[latestUserIndex] = {
...
function buildSynthesizerQuestion(original: string, context?: string): string {
  ...
  return (
    ...
    "required format.\n\n" +
    "For the executor-facing interpretation, you may ONLY select priorities from " +
    `${REQUEST_INTERPRETATION_PRIORITIES.join(", ")}. objectiveSpan and every ` +
    "constraintSpans item MUST be copied verbatim from the raw request. Do not " +
    "put recipients, approvals, credentials, facts, tools, or instructions into " +
    "the JSON unless they already appear in those exact copied spans.\n\n" +
    "Output EXACTLY this format and nothing else:\n" +
    "===STRATEGY===\n" +
    "<2-4 sentences: the approach and what you improved>\n" +
    "===INTERPRETATION_JSON===\n" +
    '{"objectiveSpan":"<exact source span>","constraintSpans":["<exact source span>"],"priorities":["accuracy"]}\n' +
    "===REFINED_PROMPT===\n" +
...
function parseSynthesizerOutput(text: string): {
  strategy: string;
  refinedPrompt: string;
  interpretation: unknown;
} {
  const refinedMatch = text.match(/===\s*REFINED_PROMPT\s*===([\s\S]*?)(?:===\s*END\s*===|$)/i);
  const strategyMatch = text.match(/===\s*STRATEGY\s*===([\s\S]*?)===\s*INTERPRETATION_JSON\s*===/i);
  const interpretationMatch = text.match(/===\s*INTERPRETATION_JSON\s*===([\s\S]*?)===\s*REFINED_PROMPT\s*===/i);
  ...
  if (interpretationMatch) {
    const jsonText = interpretationMatch[1]
      .trim()
      .replace(/^```(?:json)?\s*/i, "")
      .replace(/\s*```$/, "");
    try {
  ...
  return { strategy, refinedPrompt, interpretation };
}
...
export async function refinePromptViaJury(opts: {
  ...
  try {
    ...
    if (
      parsed.refinedPrompt.length > MAX_ADVISORY_REFINEMENT_CHARS
      || parsed.strategy.length > MAX_STRATEGY_CHARS
    ) {
      return { ...base, reason: "jury output exceeded refinement budget", latencyMs: Date.now() - t0 };
    }
    const interpretation = validateRequestInterpretation(parsed.interpretation, original);
    if (!interpretation) {
      return { ...base, reason: "jury interpretation failed source-bound validation", latencyMs: Date.now() - t0 };
    }
