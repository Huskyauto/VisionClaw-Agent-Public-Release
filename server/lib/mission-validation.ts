...
// ─────────────────────────────────────────────────────────────────────────────
// Proof-of-premise gate (Grok 4.5 review item #1, 2026-07-25): the S7a packet
// validation above stops UNPREPARED missions from drafting; this gate stops
// DOUBLING DOWN ON SILENCE. Once a mission has actually SENT an experiment
// (sequence_id set), drafting ANOTHER experiment requires at least one real
// demand signal — a positive reply, booked call, checkout, agreed price, or
// payment. Deterministic, $0, fail closed.
// ─────────────────────────────────────────────────────────────────────────────

/** Evidence types that count as real demand (external buyer signal). */
export const DEMAND_EVIDENCE_TYPES = [
  "positive_reply",
  "call_booked",
  "checkout_started",
...
export interface DemandProofDecision {
  allowed: boolean;
  reason: string;
}

/**
 * Pure decision (exported for query-free tests): a mission that has already
 * sent >=1 experiment may only draft another if it has >=1 demand-evidence
 * row. Unreadable counts refuse (fail closed).
 */
export function decideDemandProof(sentExperimentCount: unknown, demandEvidenceCount: unknown): DemandProofDecision {
  // Number(null) === 0 — a missing count must refuse, not read as zero.
  if (sentExperimentCount == null || demandEvidenceCount == null) {
    return { allowed: false, reason: "experiment/evidence counts unreadable — refusing (fail closed)" };
  }
  const sent = Number(sentExperimentCount);
  const demand = Number(demandEvidenceCount);
  if (!Number.isFinite(sent) || !Number.isFinite(demand) || sent < 0 || demand < 0) {
    return { allowed: false, reason: "experiment/evidence counts unreadable — refusing (fail closed)" };
  }
  if (sent === 0) return { allowed: true, reason: "first experiment — packet validation gate applies" };
  if (demand >= 1) return { allowed: true, reason: `${demand} demand signal(s) recorded across ${sent} sent experiment(s)` };
  return {
    allowed: false,
    reason: `proof-of-premise gate: ${sent} experiment(s) already sent with ZERO demand signals (no positive replies, calls, checkouts, or payments) — refusing to double down. Record real demand evidence or revise the offer/mission first.`,
  };
}

/**
 * Fail-closed proof-of-premise gate. Called by createExperimentDraft AFTER
 * assertMissionValidated, BEFORE any insert.
 */
export async function assertDemandProven(tenantId: number, missionId: number): Promise<DemandProofDecision> {
  assertTenant(tenantId);
  ...
  const demandRes = await db.execute(sql`
    SELECT count(*)::int AS n FROM mission_evidence
    WHERE tenant_id = ${tenantId} AND mission_id = ${missionId}
      AND type = ANY(${`{${DEMAND_EVIDENCE_TYPES.join(",")}}`}::text[])
  `);
  const decision = decideDemandProof(rows(sentRes)[0]?.n, rows(demandRes)[0]?.n);
