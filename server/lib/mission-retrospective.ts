...
export async function recordRetrospective(tenantId: number, missionId: number, terminalStage: string): Promise<MissionRetrospective | null> {
  ...
  // Honest-margin breakdown (CPT 5.6 priority 4) — best-effort attach: an
  // unreadable economics ledger must not block the retrospective itself
  // (callers already treat this whole fn as best-effort), it just omits the
  // breakdown rather than fabricating $0 costs.
  try {
    const { getMissionEconomics } = await import("./mission-economics");
    (retro as any).economics = await getMissionEconomics(tenantId, missionId);
  } catch (e: any) {
    console.warn(`[mission-retrospective] economics unavailable for mission ${missionId} (breakdown omitted): ${e?.message}`);
  }
