...
export function buildCandidateEvidenceHash(evidence: unknown): string {
  return sha256(stableSerialize(evidence));
}

export function verifyJuryDecisionHash(input: {
  tenantId: number;
  skillId: number | null;
  seedHash: string;
  candidateHash: string;
  ...
  if (!input.providedHash || !/^[a-f0-9]{64}$/i.test(input.providedHash)) return false;
  try {
    const expected = buildJuryDecisionHash({
      tenantId: input.tenantId,
      skillId: input.skillId,
      seedHash: input.seedHash,
      candidateHash: input.candidateHash,
      policyVersion: input.policyVersion,
      evidenceHash: buildCandidateEvidenceHash(input.evidence),
      decision: input.decision,
    });
    const providedBuffer = Buffer.from(input.providedHash, "hex");
  ...
  return db.transaction(async (tx) => {
    ...
    try {
      juryDecisionHash = buildJuryDecisionHash({
        ...
        candidateHash: candidate.candidateHash,
        policyVersion: candidate.policyVersion,
        evidenceHash: buildCandidateEvidenceHash(candidate.evidence),
        decision,
      });
    } catch (error) {
      nextState = "failed";
