...
export interface SafetyReport {
  ...
  verdict: "CRITICAL" | "CHANGES" | "NO_CHANGE" | "INCONCLUSIVE";
}
...
export function buildSafetyReport(
  ...
  return {
    ...
    // CRITICAL always wins (even on a partial run). Otherwise a run with zero
    // successful replays, or where errors outnumber successes, must never
    // claim NO_CHANGE — that is an absence of evidence, not an all-clear.
    verdict: flips.block_to_allow > 0 ? "CRITICAL"
      : (outcomes.length === 0 || meta.errored > outcomes.length) ? "INCONCLUSIVE"
      : (flips.allow_to_block > 0 ? "CHANGES" : "NO_CHANGE"),
  };
}
...
export async function runSafetyReplay(opts: RunSafetyReplayOptions): Promise<RunSafetyReplayResult> {
  ...
  try {
    ...
    await pool.query(
      ...
      // Never silent: if the failed-status write itself fails, the row stays
      // 'running' forever — log loudly so the stale marker + operator can act.
      console.error(`[sandbox-replay] run=${runId} FAILED and the status update also failed (row may be stuck 'running'): ${updateErr?.message || updateErr}`);
    });
    throw err;
  }
}
...
export interface ModelSwapReport {
  ...
  stubbedToolCalls: number;
  verdict: "DRIFT" | "CHANGES" | "NO_CHANGE" | "INCONCLUSIVE";
}
...
export function buildModelSwapReport(
  ...
  // DRIFT always wins (real evidence of divergence, even on a partial run).
  // Otherwise: zero replays, zero graded items (e.g. embedding lane down), or
  // errors outnumbering successes = absence of evidence — INCONCLUSIVE, never
  // a confident NO_CHANGE.
  const verdict: ModelSwapReport["verdict"] =
    similarity.belowThreshold > 0 ? "DRIFT"
    : (outcomes.length === 0 || similarity.graded === 0 || meta.errored > outcomes.length) ? "INCONCLUSIVE"
    : (similarity.mean !== null && similarity.mean < 0.9) ? "CHANGES"
    : "NO_CHANGE";
  return {
    corpus,
    overrides,
