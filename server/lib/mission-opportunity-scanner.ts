...
import { readIdeaPriority } from "./idea-priority";
...
export function buildMissionPacket(project: {
  ...
  // One shared read contract with the scorer (metadata.priority.*) — see
  // server/lib/idea-priority.ts. Fail toward NOT proposing on any mismatch.
  const priority = readIdeaPriority(project?.metadata);
  if (!priority) return null;
  const { tier } = priority;
  if (tier !== "S" && tier !== "A") return null;
  const buyer = priority.buyerHypothesis;
  ...
  return {
    ...
    hypothesis: `Scored tier-${tier} idea (composite ${priority.composite || "?"}): ${rationale}`,
    idealCustomer: buyer,
...
// ── Retrospective feedback (S7c, DeepSeek gap #3) ───────────────────────────
// Past terminal-mission retrospectives adjust candidate RANKING (deterministic,
// $0, no LLM). Feedback is advisory re-ordering only — it never creates,
// blocks, or auto-approves anything (the validation gate + HITL do the
// blocking). Signals:
//  - price-band penalty: a candidate priced within ±50% of a KILLED mission
//    that scored < 40 repeats a disproven price point — rank it down;
//  - proven-pattern boost: a candidate priced within ±50% of a SCALE_READY
//    mission with positive realized margin reuses a proven price band.
export interface RetroSignal {
  terminalStage: string; // killed | scale_ready
  score: number; // retrospective 0-100
  priceUsd: number;
  realizedMarginUsdCents: number;
}

/** Pure re-rank adjustment for one candidate. Exported for query-free tests. */
export function retroAdjustment(candidatePriceUsd: number | undefined, signals: RetroSignal[]): number {
  const price = Number(candidatePriceUsd);
  if (!Number.isFinite(price) || price <= 0 || !Array.isArray(signals)) return 0;
  let adj = 0;
  for (const s of signals) {
    const sp = Number(s?.priceUsd);
    if (!Number.isFinite(sp) || sp <= 0) continue;
    const inBand = price >= sp * 0.5 && price <= sp * 1.5;
    if (!inBand) continue;
    if (s.terminalStage === "killed" && Number(s.score) < 40) adj -= 15;
    if (s.terminalStage === "scale_ready" && Number(s.realizedMarginUsdCents) > 0) adj += 10;
  }
  return Math.max(-30, Math.min(30, adj));
}

async function loadRetroSignals(tenantId: number): Promise<RetroSignal[]> {
  const res = await db.execute(sql`
    SELECT stage, price_usd, retrospective
    FROM revenue_missions
    WHERE tenant_id = ${tenantId} AND retrospective IS NOT NULL
    ORDER BY retrospective_at DESC
    LIMIT 25
  `);
  return rows(res)
    .map((r) => ({
      terminalStage: String(r.stage ?? ""),
      score: Number(r.retrospective?.score) || 0,
      priceUsd: Number(r.price_usd) || 0,
      realizedMarginUsdCents: Number(r.retrospective?.realizedMarginUsdCents) || 0,
    }))
    .filter((s) => s.terminalStage === "killed" || s.terminalStage === "scale_ready");
}
...
export async function scanAndProposeMission(tenantId: number): Promise<ScanResult> {
  ...
  // Capital-settlement reconciler (durable retry): sweep any terminal mission
  // whose fire-and-forget settlement was lost to a transient failure. The
  // settle CAS makes this idempotent; best-effort so a sweep failure never
  // blocks the scan itself.
  import("./agent-capital")
    .then(({ reconcileUnsettledCapital }) => reconcileUnsettledCapital(tenantId))
    .catch((e) => console.warn("[silent-catch] mission-opportunity-scanner capital reconcile (idempotent, retried weekly):", (e as any)?.message ?? e));
  ...
  const candidates = await db.execute(sql`
    ...
    WHERE p.tenant_id = ${tenantId}
      AND (p.metadata->'priority'->>'tier') IN ('S', 'A')
      AND (p.metadata->'priority'->>'composite') ~ '^[0-9]+$'
      AND 'idea-stage' = ANY(p.tags)
      AND NOT EXISTS (
        SELECT 1 FROM revenue_missions m
        WHERE m.tenant_id = ${tenantId} AND m.project_id = p.id
      )
    ORDER BY (p.metadata->'priority'->>'composite')::int DESC, p.id DESC
    LIMIT 5
  `);
  // Retrospective feedback (S7c): re-rank viable packets by composite score
  // adjusted with lessons from past terminal missions before proposing.
  const signals = await loadRetroSignals(tenantId);
  const viable = rows(candidates)
    .map((project) => ({ project, packet: buildMissionPacket(project) }))
    .filter((c): c is { project: any; packet: MissionPacket } => c.packet != null) // thin evidence — fail toward not proposing
    .map((c) => ({
      ...c,
      adjusted: (readIdeaPriority(c.project.metadata)?.composite || 0) + retroAdjustment(c.packet.priceUsd, signals),
    }))
    .sort((a, b) => b.adjusted - a.adjusted);
  for (const { project, packet } of viable) {
    const { createMission } = await import("./revenue-missions");
    const mission = await createMission({ tenantId, ...packet });
    return { proposed: true, missionId: Number(mission.id), reason: `proposed mission from project #${project.id} "${project.name}" (retro-adjusted rank over ${signals.length} past retrospectives)` };
  }
