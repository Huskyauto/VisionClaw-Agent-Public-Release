 ...
 *    persisted — only header NAMES. Captured bodies are credential-redacted
 *    before they are buffered or shown to the LLM.
...
/** Strip URL userinfo and credential-looking query-param values from a captured URL. */
  ...
  page.on("response", async (res: any) => {
    try {
      ...
      recordCapturedCall(tenantId, {
        ...
        postData: redactBodyText(req.postData()?.slice(0, MAX_BODY_BYTES)),
        responseBody: redactBodyText(body),
