...
import { confidenceFromEvidenceCount, type EvidenceConfidence } from "./evidence-confidence";
...
// The pinned dated id above WILL eventually be retired by Anthropic. When that
// happens a raw messages.create 404s — historically swallowed by allSettled and
// surfaced only as "+0 scores". resolveScorerModel() checks the pinned id
// against the live /models list once per process and, if it's gone, falls back
// to the newest live claude-sonnet-* (loud log, never silent). If the /models
// call itself fails (network), we keep the pinned id — a retired model then
// fails LOUD downstream via the zero-score guard, never quietly.
let resolvedScorerModel: string | null = null;

export async function resolveScorerModel(client: Anthropic): Promise<string> {
  if (resolvedScorerModel) return resolvedScorerModel;
  try {
    const live = await client.models.list({ limit: 100 });
    const ids = live.data.map((m) => m.id);
    if (ids.includes(SCORER_MODEL)) {
      resolvedScorerModel = SCORER_MODEL;
    } else {
      // Newest sonnet lineage model; /v1/models returns newest-first.
      const fallback = ids.find((id) => id.includes("sonnet"));
      if (!fallback) throw new Error(`no live claude-sonnet-* model found (live ids: ${ids.slice(0, 10).join(", ")})`);
      console.warn(`[ideabrowser-score] pinned scorer model ${SCORER_MODEL} is RETIRED — falling back to newest live sonnet: ${fallback}. Scores may drift vs historical portfolio; consider a re-baseline.`);
      resolvedScorerModel = fallback;
    }
  } catch (err) {
    console.warn(`[ideabrowser-score] could not verify scorer model against live /models (${err instanceof Error ? err.message : String(err)}); using pinned ${SCORER_MODEL}`);
    resolvedScorerModel = SCORER_MODEL;
  }
  return resolvedScorerModel;
}
...
export interface Score {
  ...
  composite: number;
  tier: "S" | "A" | "B" | "C" | "Park";
  /** Deterministic, from linked external evidence rows (never LLM self-report) — set by evidenceGateScore. */
...
// ── Evidence-gated rubric (Revenue Missions S5b) ────────────────────────────
// Principle (GPT 5.6 review, adopted): high market_signal / monetization
// scores must be EARNED by external evidence rows, never granted from prose.
// Without linked evidence, both axes are clamped to ≤ EVIDENCE_GATE.maxUngated
// and the composite/tier are recomputed. Pure — query-free-testable.

export const EVIDENCE_GATE = {
  maxUngated: 3, // "default to 3" per the rubric; anything higher requires evidence
} as const;

/** Clamp one score by its evidence count. evidenceCount<=0 (or junk) gates — fail closed. */
export function evidenceGateScore(score: Score, evidenceCount: unknown): Score {
  const n = Number(evidenceCount);
  const hasEvidence = Number.isSafeInteger(n) && n > 0;
  const evidence_confidence = confidenceFromEvidenceCount(evidenceCount);
  if (hasEvidence) return { ...score, evidence_confidence };
  const market = Math.min(score.market_signal, EVIDENCE_GATE.maxUngated);
  const monet = Math.min(score.monetization, EVIDENCE_GATE.maxUngated);
  if (market === score.market_signal && monet === score.monetization) return { ...score, evidence_confidence };
  const partial = { ...score, market_signal: market, monetization: monet, evidence_confidence };
  const composite = computeComposite(partial);
  return {
    ...partial,
    composite,
    tier: tierFor(composite),
    rationale: `${score.rationale} [evidence-gated: market/monetization capped at ${EVIDENCE_GATE.maxUngated} — no external evidence rows]`.slice(0, 340),
  };
}

/**
 * Evidence counts for a set of project ids: mission_evidence rows reached via
 * revenue_missions.project_id. Tenant-scoped on BOTH tables. Missing ids ⇒ 0.
 */
export async function loadEvidenceCounts(tenantId: number, projectIds: number[]): Promise<Map<number, number>> {
  const out = new Map<number, number>();
  if (!Number.isInteger(tenantId) || tenantId <= 0 || projectIds.length === 0) return out;
  const ids = projectIds.filter((id) => Number.isSafeInteger(id) && id > 0);
  if (ids.length === 0) return out;
  const literal = `{${ids.join(",")}}`;
  const res: any = await db.execute(sql`
    SELECT rm.project_id AS project_id, COUNT(me.id)::int AS n
    FROM revenue_missions rm
    JOIN mission_evidence me ON me.mission_id = rm.id AND me.tenant_id = ${tenantId}
    WHERE rm.tenant_id = ${tenantId} AND rm.project_id = ANY(${literal}::int[])
    GROUP BY rm.project_id
  `);
  for (const r of ((res?.rows ?? res) as any[])) out.set(Number(r.project_id), Number(r.n) || 0);
  return out;
}
  ...
  for (const s of scores) {
    const meta = {
      priority: {
        ...
        // Advisory Evidence Confidence (deterministic, from evidence rows) —
        // omitted for scores that never passed through evidenceGateScore.
        ...(s.evidence_confidence ? { evidence_confidence: s.evidence_confidence } : {}),
      },
    };
  ...
  for (let g = 0; g < batches.length; g += parallel) {
    ...
    for (const r of results) {
      if (r.status === "fulfilled") {
        try {
          // Evidence gate (S5b): high market/monetization scores require
          // external evidence rows; ungated ideas are clamped before persist.
          const counts = await loadEvidenceCounts(tenantId, r.value.map((s) => s.id));
          const gated = r.value.map((s) => evidenceGateScore(s, counts.get(s.id) ?? 0));
          await persistScores(gated, tenantId);
          summary.scored += gated.length;
          for (const s of gated) summary.tiers[s.tier] = (summary.tiers[s.tier] || 0) + 1;
        } catch (e: any) {
