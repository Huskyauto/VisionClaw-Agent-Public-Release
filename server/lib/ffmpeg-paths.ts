...
// SECURITY (review fix): scrub loader-hijack env vars (LD_PRELOAD, NODE_OPTIONS,
// etc.) before any child spawn, even for fixed internal commands.
// (Top-level import: inline require() throws under the tsx ESM loader.)
function scrubbedEnv(): NodeJS.ProcessEnv {
  try {
    return sanitizeSpawnEnv(process.env);
  } catch {
    return process.env;
  }
}
...
function tryWhich(name: string): string | null {
  try {
    const out = execSync(`which ${name} 2>/dev/null`, { encoding: "utf-8", timeout: 5000, env: scrubbedEnv() }).trim().split("\n")[0];
...
function execProbeOk(path: string): boolean {
  try {
    execSync(`${path} -version`, { encoding: "utf-8", timeout: 3000, stdio: ["ignore", "pipe", "ignore"], env: scrubbedEnv() });
    return true;
  } catch (_e) {
    logSilentCatch("server/lib/ffmpeg-paths.ts", _e);
    return false;
...
export function probeFfmpegCapabilities(): FfmpegCapabilities {
  ...
  let filtersOut = "";
  try {
    version = execSync(`"${ffmpegPath}" -hide_banner -version 2>&1 | head -1`, { encoding: "utf-8", timeout: 10000, env: scrubbedEnv() }).trim();
  } catch (e: any) { warnings.push(`ffmpeg -version failed: ${e?.message?.slice(0, 100)}`); }
  try {
    encodersOut = execSync(`"${ffmpegPath}" -hide_banner -encoders 2>&1`, { encoding: "utf-8", timeout: 10000, maxBuffer: 4 * 1024 * 1024, env: scrubbedEnv() });
  } catch (e: any) { warnings.push(`ffmpeg -encoders failed: ${e?.message?.slice(0, 100)}`); }
  try {
    filtersOut = execSync(`"${ffmpegPath}" -hide_banner -filters 2>&1`, { encoding: "utf-8", timeout: 10000, maxBuffer: 4 * 1024 * 1024, env: scrubbedEnv() });
