...
export async function createCompletionWithTimeout<T>(
  ...
  const timeoutP = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => {
      timedOut = true;
      localAc.abort();
      reject(new StreamCreateTimeoutError(timeoutMs, modelId));
    }, timeoutMs);
  });

  // HARD bound: race create() against the timeout so a provider that ignores
  // the abort signal still can't hold the turn open past timeoutMs. The losing
  // create() is aborted (best-effort) and its late settle is swallowed so it
  // can never surface as an unhandled rejection.
  const createP = createFn(localAc.signal);
  createP.catch(() => {});
