...
export async function draftSampleExperiment(args: {
  ...
  // Mission-scoped cost attribution (CPT 5.6 priority 4): the drafting LLM
  // call is a direct mission cost — stamp it in agent_cost_ledger.
  const { withMissionCostAttribution } = await import("../agentic/cost-ledger");
  const { prospects, variants } = await withMissionCostAttribution(
    args.missionId,
    () => icpFilterAndDraft({ tenantId: args.tenantId, mission, contacts, maxProspects }),
  );
