...
const EVENT_TYPES = [
  ...
  { name: "feature.shipped", category: "content", description: "A new platform feature/release shipped; triggers an auto-DRAFTED announcement" },
...
export async function emitEvent(params: {
  ...
  if (eventId) {
    ...
    // Condition-based wakes (Kimi K3 #1): pull matching sleeping agents' wake_at
    // to now so the next heartbeat tick fires them. Fire-and-forget + dynamic
    // import (wake-scheduler statically imports this module — avoid the cycle);
    // a matcher failure must never block the publisher.
    import("./agentic/wake-scheduler")
      .then(({ matchConditionWakes }) => matchConditionWakes(params.tenantId, params.type, params.data))
      .then((res) => {
        if (res.matched > 0) console.log(`[event-bus] event ${params.type} triggered ${res.matched} condition wake(s)`);
      })
      .catch((err) => {
        console.error(`[event-bus] condition-wake match failed for #${eventId}: ${err?.message || err}`);
      });
    try {
    ...
    // R125+67 — feature.shipped auto-DRAFTS an announcement (never publishes).
    // Fire-and-forget + dynamic import so a content-draft failure never blocks
    // the publisher's hot path and the bus carries no extra import weight.
    if (params.type === "feature.shipped") {
      import("./attention-handlers/feature-announce")
        .then(({ draftFeatureAnnouncement }) =>
          draftFeatureAnnouncement({
            tenantId: params.tenantId,
            eventId,
        ...
        .catch((err) => {
          console.error(`[event-bus] feature-announce draft failed for #${eventId}: ${err?.message || err}`);
        });
    }
  }
...
export async function getEventLog(tenantId: number, filters?: {
  ...
  if (filters?.status === "active") {
    const result = await db.execute(sql`
      ...
      WHERE tenant_id = ${tenantId} AND status IN ('pending', 'routed')
