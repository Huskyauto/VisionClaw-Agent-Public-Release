...
function buildPatchedCreate(
  ...
  return async function patchedCreate(params: any, options?: any) {
    ...
    const isProfundoFlat = providerLabel.startsWith("llm.profundo");
    const isAnthropic =
      !isProfundoFlat &&
...
export async function getClientForModel(modelId: string, tenantId?: number, options?: { requiresTools?: boolean; juryLane?: boolean; costExemptLane?: boolean; meteredOverride?: boolean }): Promise<{ client: OpenAI; actualModelId: string }> {
  ...
  // ── Z.AI direct lane (Bob 2026-08-14) — owner-funded metered exception, scoped
  // to ZAI_MODEL_MAP only (currently just glm-5.3, not on any flat lane yet).
  {
    const zai = tryZaiLane(modelId, tenantId);
    if (zai) return zai;
  }
