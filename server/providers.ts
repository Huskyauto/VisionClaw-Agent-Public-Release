...
import { normalizeRetiredModelId, wrapClientWithParamAdaptation } from "./lib/param-adaptation";
...
import {
  ...
  type UnionAlphaFixtureOutput,
  validateUnionAlphaCompletion,
} from "./lib/union-alpha-eval-core";
export {
  type ModelInfo,
  MODEL_REGISTRY,
  RETIRED_MODEL_IDS,
...
export function buildPatchedCreate(
  ...
  return async function patchedCreate(params: any, options?: any) {
    ...
    const mandatoryFableAccounting =
      isAnthropic && modelResolver(params) === "claude-fable-5-1";
    ...
    const requestOptions = mandatoryFableAccounting
      ? { ...(options || {}), timeout: 60_000, maxRetries: 0 }
      : options;
    const result: any = await origCreate(params, requestOptions);
    ...
    if (!isStream) {
      try {
        ...
        if (tokensIn || tokensOut) {
          const recordCost = recordCostOverride ?? (await import("./agentic/cost-ledger")).recordCost;
          ...
          if (mandatoryFableAccounting && recorded !== true) {
            throw new Error("mandatory Claude Fable 5.1 usage record was not persisted");
          }
        } else if (mandatoryFableAccounting) {
          throw new Error("missing mandatory Claude Fable 5.1 usage");
        }
      } catch (e: any) {
        ...
        if (mandatoryFableAccounting) throw e;
      }
      return result;
    }
    ...
    if (!result || typeof result[Symbol.asyncIterator] !== "function") {
      ...
      if (mandatoryFableAccounting) {
        throw new Error("malformed Claude Fable 5.1 stream");
      }
      return result;
    }
    const streamLedgerTarget: { current?: AsyncGenerator<any> } = {};
    const markStreamLedgerRecorded = (recorded: boolean) => {
      if (!streamLedgerTarget.current) return;
      Object.defineProperty(streamLedgerTarget.current, COST_LEDGER_RECORDED, {
        value: recorded,
        enumerable: false,
        configurable: true,
      });
    ...
    const wrapped = (async function* () {
      if (mandatoryFableAccounting) {
        const buffered: any[] = [];
        let mandatoryUsage: any = null;
        for await (const chunk of result as AsyncIterable<any>) {
          if (chunk?.usage) mandatoryUsage = chunk.usage;
          buffered.push(chunk);
        }
        const { tokensIn, tokensOut, cachedTokensIn, cacheWriteTokens } = extractUsageTokens(mandatoryUsage);
        if (!tokensIn && !tokensOut) {
          throw new Error("missing mandatory Claude Fable 5.1 usage");
        }
        const recordCost = recordCostOverride ?? (await import("./agentic/cost-ledger")).recordCost;
      ...
      try {
        ...
        try {
          if (lastUsage) {
            ...
            if (tokensIn || tokensOut) {
              const recordCost = recordCostOverride ?? (await import("./agentic/cost-ledger")).recordCost;
              ledgerRecorded = await recordCost({
                tenantId: tenantResolver(),
                toolName: providerLabel,
                model: modelResolver(params),
                tokensIn,
          ...
          markStreamLedgerRecorded(ledgerRecorded);
        } catch (e: any) {
          markStreamLedgerRecorded(false);
          console.warn(`[providers] streaming cost-track failed (${providerLabel}): ${e?.message}`);
        }
      }
    })();
    streamLedgerTarget.current = wrapped;
...
export function buildPatchedResponsesCreate(
  origCreate: Function,
  modelId: string,
  tenantId: number,
  providerLabel: string,
  recordCostOverride?: (entry: any) => Promise<boolean | void>,
) {
  return async function patchedResponsesCreate(params: any, options?: any) {
    params = { ...(params || {}) };
    const requestedModel = String(params.model || modelId);
    const normalizedModel = normalizeRetiredModelId(requestedModel);
    if (normalizedModel !== requestedModel || !params.model) {
      params.model = normalizedModel;
    }
    ...
    const accountingModel =
      modelId === "gpt-6-astra" && params?.service_tier === "flex"
        ? "gpt-6-astra:flex"
        : normalizedModel;
    const recordUsage = async (usage: any, operation: string) => {
      if (!usage) {
        if (modelId === "gpt-6-astra") throw new Error("GPT-6 Astra response omitted mandatory usage");
        return;
      }
      try {
        const { tokensIn, tokensOut, cachedTokensIn, cacheWriteTokens } = extractUsageTokens(usage);
        if (!tokensIn && !tokensOut) {
          if (modelId === "gpt-6-astra") throw new Error("GPT-6 Astra response reported zero mandatory usage");
          return;
        }
        const recorder = recordCostOverride || (await import("./agentic/cost-ledger")).recordCost;
        ...
        if (modelId === "gpt-6-astra" && recorded === false) {
          throw new Error("mandatory GPT-6 Astra usage record was not persisted");
        }
      } catch (e: any) {
        console.warn(`[providers] Responses API cost-track failed (${providerLabel}): ${e?.message}`);
        if (modelId === "gpt-6-astra") throw e;
      }
    };

    if (params?.stream !== true) {
      await recordUsage(result?.usage, "responses.create");
      return result;
    }
    ...
    const wrapped = (async function* () {
      ...
      try {
        for await (const event of result as AsyncIterable<any>) {
          if (
            /^response\.(completed|incomplete|failed)$/.test(String(event?.type || "")) &&
        ...
        await recordUsage(finalUsage, "responses.create.stream");
      }
    })();
    for (const key of Object.keys(result)) {
      if (!(key in wrapped)) {
...
function wrapClientWithCostTracking(client: OpenAI, modelId: string, tenantId: number, providerLabel: string): OpenAI {
  ...
  const patchedResponsesCreate = buildPatchedResponsesCreate(
    (client as any).responses.create.bind((client as any).responses),
    modelId,
    tenantId,
    providerLabel,
  );
  return new Proxy(client, {
    get(target, prop, receiver) {
      if (prop === "responses") {
        const responses = (target as any).responses;
        return new Proxy(responses, {
          get(responsesTarget, responsesProp) {
            if (responsesProp === "create") return patchedResponsesCreate;
            const value = responsesTarget[responsesProp];
            return typeof value === "function" ? value.bind(responsesTarget) : value;
          },
        });
      }
      if (prop !== "chat") {
...
export const PROVIDER_CONFIG: Record<string, { name: string; baseUrl: string; description: string }> = {
  ...
  openai: { name: "OpenAI", baseUrl: "https://api.openai.com/v1", description: "GPT-5, GPT-5.4, GPT-6 Luna" },
  ...
  profundo: { name: "Profundo AI", baseUrl: "https://api.profundoai.com/v1", description: "Flat-rate lane — GPT-5.6 Sol/Luna, Kimi K3, Claude Sonnet 5, Opus 4.8/5, and Fable 5; no token meter" },
...
const PROFUNDO_MODEL_MAP: Record<string, string> = {
  ...
  // Catalog narrowed by Profundo's 2026-09-07 retirement notice. Keep this map
  // fail-closed to the exact seven IDs the provider says remain available.
...
// Models outside the exact ZAI_MODEL_MAP must NOT be added here merely because
// another provider retired them; every direct Z.AI addition needs its own proof.
...
export const LEGACY_MODEL_ALIASES: Record<string, string> = {
  ...
  "gemini-3.5-flash": "google/gemini-3.7-flash",
};
...
export function normalizeModelId(modelId: string): string {
  return normalizeRetiredModelId(LEGACY_MODEL_ALIASES[modelId] || modelId);
}
...
export async function runUnionAlphaSyntheticFixture(
  fixtureId: string,
): Promise<UnionAlphaFixtureOutput> {
  if (process.env.UNION_ALPHA_SYNTHETIC_EVAL_ENABLED !== "1") {
    throw new Error("[union-alpha-eval] UNION_ALPHA_SYNTHETIC_EVAL_ENABLED must be exactly 1");
  }
  const fixture = UNION_ALPHA_SYNTHETIC_FIXTURES.find((candidate) => candidate.id === fixtureId);
  if (!fixture) {
    throw new Error(`[union-alpha-eval] unknown bundled fixture "${fixtureId}"`);
  }
  if (!isProviderHealthy("openrouter")) {
    throw new Error("[union-alpha-eval] OpenRouter is unavailable");
  }
  const key = (process.env.OPENROUTER_API_KEY || "").trim();
  if (!key.startsWith("sk-or-")) {
    throw new Error("[union-alpha-eval] OpenRouter credential is unavailable");
  }
  ...
  const client = wrapClientWithParamAdaptation(new OpenAI({
    ...
    defaultHeaders: {
      ...
      "X-Title": "VisionClaw Union Alpha Synthetic Evaluation",
    },
  }));
  const usesTool = fixture.kind === "tool";
  const completion = await client.chat.completions.create({
    model: "stealth/union-alpha",
    messages: [
      {
        role: "system",
        content: "You are being evaluated only on a synthetic fixture. Follow the fixture exactly and do not request external or private data.",
      },
      ...
      ? {
          tools: [{
            ...
            function: {
              ...
              description: "Synthetic dependency metadata lookup used only by this evaluation.",
              parameters: {
                type: "object",
  ...
  return validateUnionAlphaCompletion(fixture, {
...
async function getPinnedProviderLane(
  ...
  if (lane === "claude-runner" && tenantId !== (await import("./auth")).ADMIN_TENANT_ID) {
    throw new Error(
      `[providers] owner-only subscription lane "claude-runner" denied for tenant ${tenantId ?? "unknown"}`,
    );
  }
...
export async function getClientForModel(
  ...
  if (normalizedModelId !== modelId) {
    console.log(`[providers] Legacy/retired model alias: "${modelId}" → "${normalizedModelId}"`);
    modelId = normalizedModelId;
  }
  let model = MODEL_REGISTRY.find((m) => m.id === modelId);
  if (isSyntheticEvaluationModel(modelId)) {
    throw new Error(
      `[providers] ${modelId} is only available through its fixed synthetic evaluation runner`,
    );
  }
  const explicitOwnerOnlyModel = isOwnerOnlyFinalDecisionModel(modelId);
  const explicitOwnerModelAuthorized =
    explicitOwnerOnlyModel &&
    options?.explicitOwnerSelection === true &&
    options?.platformAdminVerified === true &&
    tenantId === (await import("./auth")).ADMIN_TENANT_ID;
  if (explicitOwnerOnlyModel && !explicitOwnerModelAuthorized) {
    const haveModelfarm = !!(
    ...
    if (!haveModelfarm) {
      throw new Error(
        `[providers] ${modelId} requires explicit admin selection; the free substitution lane is unavailable`,
      );
    }
    console.warn(
      `[providers] ${modelId} denied outside explicit admin selection → free modelfarm gpt-5.4`,
    );
    return { client: getReplit(), actualModelId: mapReplitToOpenAI("gpt-5.4") || "gpt-5.4" };
  }
  ...
  if (!model || model.provider === "replit") {
    const costTenant =
      typeof tenantId === "number" && Number.isInteger(tenantId) && tenantId > 0
        ? tenantId
        : (() => {
            const stack = new Error().stack?.split("\n").slice(2, 6).join(" | ") || "unknown caller";
            console.warn(`[providers] metered Replit-model fallback missing valid tenant context — billing ADMIN. caller: ${stack}`);
            return ADMIN_TENANT_ID_FALLBACK;
          })();
    // Profundo flat lane FIRST for replit-provider ids on the membership
    // (currently gpt-5.6-sol / gpt-5.6-luna):
    ...
    // the REAL model at $0 flat rate. gpt-5.4/gpt-5 are NOT in the map,
    ...
    const profundoEligible = !!PROFUNDO_MODEL_MAP[modelId];
    {
    ...
    // A model chosen because it belongs to Profundo's flat catalog must not
    // become a metered direct-key call merely because the provider is down.
    // Only an explicit per-call metered override may cross that cost boundary;
    // the global metered switch is insufficient for an automatic flat-lane pick.
    const forceZeroCostFallback = profundoEligible && !options?.meteredOverride;
    if (
      mapped &&
      (!meteredLlmEnabled() || forceZeroCostFallback) &&
      !options?.costExemptLane &&
      !options?.meteredOverride
    ) {
      const haveModelfarm = !!(
        replitKey &&
      ...
      console.warn(
        `[providers] $0 policy: unavailable flat/unsupported Replit model ${modelId} → free modelfarm ${freeId} ` +
    ...
    if (mapped && openaiEnvKey && openaiEnvKey.length > 5) {
      ...
      return { client: wrapClientWithCostTracking(getUserClient("openai", openaiEnvKey), mapped, costTenant, "llm.openai"), actualModelId: mapped };
    }

    const tenantKey = tenantId ? await storage.getTenantProviderKey(tenantId, "openai") : null;
    if (tenantKey?.api_key && mapped) {
      return { client: wrapClientWithCostTracking(getUserClient("openai", decryptApiKey(tenantKey.api_key)), mapped, costTenant, "llm.openai"), actualModelId: mapped };
    }
    ...
    if (openaiUserKey && openaiUserKey.enabled && openaiUserKey.apiKey && mapped) {
      return { client: wrapClientWithCostTracking(getUserClient("openai", decryptApiKey(openaiUserKey.apiKey)), mapped, costTenant, "llm.openai"), actualModelId: mapped };
    }
    ...
    if (mapped && anthropicIntegration) {
      ...
      return { client: wrapClientWithCostTracking(anthropicIntegration, "claude-sonnet-4-5", costTenant, "llm.anthropic"), actualModelId: "claude-sonnet-4-5" };
    }
    ...
    if (mapped && anthropicKey && anthropicKey.length > 5) {
      ...
      return { client: wrapClientWithCostTracking(getUserClient("anthropic", anthropicKey), "claude-sonnet-4-5", costTenant, "llm.anthropic"), actualModelId: "claude-sonnet-4-5" };
    }
  ...
  {
    const profundo = explicitOwnerModelAuthorized ? null : tryProfundoLane(modelId, tenantId);
    if (profundo) return profundo;
  }
...
export async function getModelForTierAsync(
  ...
  tenantId?: number,
  options?: { freeTierOnly?: boolean; allowMeteredFallback?: boolean },
): Promise<string> {
  ...
  // Pass 1: zero-marginal and subscription lanes. Provider order inside a tier
  // expresses task fit, but a metered integration can never leapfrog a healthy
  // flat/free lane merely because its API key is configured.
  for (const c of candidates) {
    ...
    if (c.provider === "replit") {
      ...
      if (mapped && isProviderHealthy("replit")) {
        logAutoRoute(tier, c.model, "replit modelfarm ($0 marginal)");
        return c.model;
      }
    }
    if (
      c.provider === "openference" &&
      hasOpenferenceKey() &&
      isProviderHealthy("openference")
    ) {
      logAutoRoute(tier, c.model, "Openference Auto Agent flat-rate plan");
      return c.model;
    }
    if (
      c.provider === "openrouter" &&
      c.model.endsWith(":free") &&
      (enabled.has("openrouter") || hasIntegrationFallback("openrouter")) &&
      isProviderHealthy("openrouter")
    ) {
      logAutoRoute(tier, c.model, "OpenRouter free lane");
      return c.model;
    }
  }
  ...
  if (freeTierOnly) {
    for (const c of candidates) {
      if (c.provider === "replit" && isProviderHealthy("replit")) {
      ...
      if (
        ...
        logAutoRoute(tier, c.model, "OpenRouter free lane (free-tier-only mode)");
        return c.model;
      }
    }
    throw new Error(`[auto-route] no healthy zero-marginal lane available for tier=${tier}`);
  }

  // Pass 2: metered lanes require all three authorizers: the global kill switch,
  // an explicit caller opt-in, and an authenticated tenant. Global credentials
  // are owner-only; other tenants must supply their own provider credential.
  // GPT-6 Luna is the bounded cheap fallback; Sol remains explicit-only.
  if (meteredLlmEnabled() && options?.allowMeteredFallback === true && tenantId) for (const c of candidates) {
    ...
    const tenantKey = c.provider === "replit" || tenantId === ADMIN_TENANT_ID_FALLBACK
      ? null
      : await storage.getTenantProviderKey(tenantId, c.provider);
    const tenantAuthorized =
      !!tenantKey?.api_key ||
      (tenantId === ADMIN_TENANT_ID_FALLBACK &&
        (enabled.has(c.provider) || hasIntegrationFallback(c.provider)));
    if (c.provider !== "replit" && tenantAuthorized && isProviderHealthy(c.provider)) {
      logAutoRoute(tier, c.model, `${c.provider} direct key (fallback — PAID API)`);
      return c.model;
    }
  }
  throw new Error(`[auto-route] no healthy authorized model lane available for tier=${tier}`);
}
...
export function getModelForTier(tier: "fast" | "balanced" | "powerful" | "reasoning"): string {
  ...
  // lane (gemini-2.5-flash); only heavier tiers may use metered Gemini 3.7 Flash.
  // The central metered-authorization guard still remaps unauthorized traffic to
  // a distinct free model, so naming 3.7 here never forces paid usage.
  const tierMap: Record<string, string> = {
    fast: "gemini-2.5-flash",
    balanced: "gemini-2.5-flash",
    powerful: hasOpenferenceKey() ? "openference/deepseek-v4-pro" : "google/gemini-3.7-flash",
    reasoning: hasOpenferenceKey() ? "openference/glm-5.3" : "google/gemini-3.7-flash",
  };
...
export async function getAvailableModels(): Promise<ModelInfo[]> {
  ...
  return MODEL_REGISTRY.filter(
    (m) =>
      ...
      !isOwnerOnlyFinalDecisionModel(m.id),
  );
}
...
export async function getAvailableModelsForTenant(tenantId: number, isAdmin: boolean): Promise<ModelInfo[]> {
  ...
  const models = MODEL_REGISTRY.filter(
    (m) =>
      ...
      !RETIRED_MODEL_IDS.has(m.id) &&
      !isSyntheticEvaluationModel(m.id) &&
      (!isOwnerOnlyFinalDecisionModel(m.id) || isAdmin),
  );
