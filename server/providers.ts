...
export const PROVIDER_CONFIG: Record<string, { name: string; baseUrl: string; description: string }> = {
  ...
  profundo: { name: "Profundo AI", baseUrl: "https://api.profundoai.com/v1", description: "Flat $20/mo unlimited — GLM 5.2, Kimi K3, Gemini 3.5 Flash/3.1 Pro, DeepSeek V4 Flash & more; no token meter" },
};
...
function getProfundoClient(): OpenAI | null {
  ...
  if (!clientCache.has(cacheKey)) {
    clientCache.set(cacheKey, new OpenAI({ apiKey: key, baseURL: PROVIDER_CONFIG.profundo.baseUrl }));
  }
...
function wrapProfundoFailureMarking(client: OpenAI): OpenAI {
  ...
  completions.create = (async (...args: Parameters<typeof origCreate>) => {
    try {
      ...
      if (status === 401 || status === 403 || status === 429 || (status >= 500 && status <= 599)) {
        markProviderUnhealthy("profundo", `HTTP ${status}: ${String(err?.message || "").slice(0, 60)}`);
      }
      throw err;
    }
...
// ── LAST-RESORT super-expert escalation (Bob 2026-07-01) ─────────────────────
// claude-fable-5 is deliberately NOT in tierModels / FRONTIER_PROPOSERS /
// model-tiers.json — it must NEVER be reachable via generic auto-routing (it
// caused 3× ~$20/day when defaulted 2026-06-11). The ONLY sanctioned path is
// resilientChatCompletion's terminal rung: after every other candidate model is
// genuinely exhausted, one explicit metered attempt, bounded by a small daily
// call cap. Disable entirely with LAST_RESORT_MODEL_ENABLED=false; tune the cap
// with LAST_RESORT_DAILY_MAX (default 10 calls/day, process-local).
...
/**
 * Atomically claim one last-resort call slot for today. Returns true (and
 * counts the call) only when the escalation lane is enabled AND under the
 * daily cap — callers must claim BEFORE spending. Process-local by design:
 * this is a coarse runaway brake, not an accounting ledger (real spend is
 * recorded by the cost ledger like every other call).
 */
...
export async function getClientForModel(modelId: string, tenantId?: number, options?: { requiresTools?: boolean; juryLane?: boolean; costExemptLane?: boolean; meteredOverride?: boolean }): Promise<{ client: OpenAI; actualModelId: string }> {
  ...
  if (!model || model.provider === "replit") {
    ...
    const REPLIT_MODELFARM_UNSUPPORTED = new Set(["gpt-5.5", "gpt-5.6-sol"]);
  ...
  // instead of the modelfarm downgrade. Supports tools via chat completions.
  {
    ...
    if (profundoId && isProviderHealthy("profundo")) {
      ...
      if (profundoClient) {
        ...
        return {
          client: wrapClientWithCostTracking(wrapProfundoFailureMarking(profundoClient), profundoId, profundoTenant, "llm.profundo"),
