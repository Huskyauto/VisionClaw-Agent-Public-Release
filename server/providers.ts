...
export type ProviderLane = "openference" | "profundo" | "claude-runner";
...
function getPinnedProviderLane(
  lane: ProviderLane,
  modelId: string,
  model: ModelInfo | undefined,
  tenantId: number | undefined,
  requiresTools: boolean | undefined,
): { client: OpenAI; actualModelId: string } {
  switch (lane) {
    case "openference": {
...
export async function getClientForModel(
  ...
  if (options?.providerLane) {
    return getPinnedProviderLane(options.providerLane, modelId, model, tenantId, options.requiresTools);
  }
  ...
  if (!model || model.provider === "replit") {
    ...
    if (mapped && REPLIT_MODELFARM_UNSUPPORTED.has(modelId) && options?.forbidMeteredFallback) {
      throw new Error(
...
export async function getModelForTierAsync(
  ...
  if (freeTierOnly) {
    for (const c of candidates) {
      ...
      if (c.provider === "openference" && hasOpenferenceKey()) {
