...
async function summarizeCluster(
  ...
  try {
    // Route through getClientForModel so the $0-cost policy substitutes the free
    // Replit modelfarm lane (gpt-5-mini is a modelfarm id; a raw new OpenAI() would
    // hit the metered api.openai.com endpoint and 404/leak). Use the returned
    // actualModelId — under the free lane the client swaps the underlying model.
...
export async function buildCommunitiesForTenant(
  ...
  for (const [cid, members] of eligible) {
    ...
    const summarized = await summarizeCluster(tentativeLabel, members, tripleSubset, tenantId);
