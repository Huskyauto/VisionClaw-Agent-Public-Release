...
//
// The detection uses OWNER_EMAIL, OWNER_ALERT_EMAIL, SITE_OWNER_EMAIL,
// SITE_CONTACT_EMAIL plus the literal huskyauto@gmail.com fallback (the
// canonical address per replit.md).
...
function ownerAddressSet(): Set<string> {
  ...
  const all = [...fromEnv, ...extras, "huskyauto@gmail.com"];
...
export async function flushIfDue(): Promise<{ flushed: boolean; count?: number; reason?: string }> {
  ...
  try {
    ...
    const recipient = process.env.OWNER_EMAIL || process.env.OWNER_ALERT_EMAIL || "huskyauto@gmail.com";
