...
- Max ${MAX_FACTS_PER_TURN} facts per extraction`,
      ...
      try {
        if (action.type === "create") {
          ...
          classifyMemoryCategory(action.fact, action.category, tenantId, personaId)
            .then(({ categoryId }) => {
              db.execute(sql`UPDATE memory_entries SET category_id = ${categoryId} WHERE id = ${entry.id} AND tenant_id = ${tenantId}`).catch(() => {});
            })
            .catch(() => {});
          stats.created++;
    ...
    extractTriples(userMessage, assistantResponse, tenantId, personaId, sourceMessageId).catch((e) =>
...
- Max 3 triples per extraction`,
  ...
  for (const t of triples) {
    ...
    await assertMemoryValueNotTombstoned({
      ...
      parts: [t.subject, t.predicate, t.object],
    });
    ...
    await db.transaction(async (tx) => {
      const inserted = await tx.execute(sql`
        INSERT INTO knowledge_triples
          (subject, predicate, object, confidence, source, valid_from, wing,
           room, tenant_id, persona_id)
        VALUES (${t.subject}, ${t.predicate}, ${t.object}, ${t.confidence},
                'conversation', ${validFrom}, ${t.wing || null},
                ${t.room || null}, ${tenantId}, ${personaId || null})
        RETURNING id
      `);
      const tripleId = Number((((inserted as any).rows || inserted) as any[])[0]?.id);
      if (!Number.isInteger(tripleId) || tripleId <= 0) {
        throw new Error("triple insert returned no id");
      }
      // A triple extracted from this concrete durable message has no
      // independent source. Do not write an edge when callers lack the
      // message id; guessing from text or conversation order is unsafe.
      if (Number.isInteger(sourceMessageId) && Number(sourceMessageId) > 0) {
        await recordMemoryDerivation(tx, {
          tenantId,
          parentSource: "messages",
          parentId: Number(sourceMessageId),
          childSource: "knowledge_triples",
          childId: tripleId,
          relationship: "triple_extracted",
          solelyDerived: true,
        });
      }
    });
...
export async function deduplicateMemories(personaId: number | null | undefined, tenantId: number): Promise<{ merged: number; removed: number }> {
  ...
  for (let i = 0; i < active.length; i++) {
    ...
    for (let j = i + 1; j < active.length; j++) {
      ...
      if (similarity > SIMILARITY_THRESHOLD) {
        ...
        await storage.updateMemoryEntry(remove.id, { status: "superseded" }, tenantId);
