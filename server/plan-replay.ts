...
export function recordPlanOutcome(
  ...
  const replayCacheId = (plan as any).__replayCacheId;
  const completedSteps = plan.steps?.filter((step) => step.status === "complete").length ?? 0;
  const totalSteps = plan.steps?.length ?? 0;
  void emitLoopOutcomeBestEffort({
    tenantId: plan.tenantId,
    eventKey: `orchestration-plan:${plan.id}:outcome`,
    loopKind: "plan-replay",
    policyVersion: replayCacheId ? "replayed-v1" : "fresh-v1",
    sourceType: "orchestration_plan",
    sourceId: String(plan.id),
    taskClass: requestClass,
    mode: replayCacheId ? "replay" : "online",
    quality: totalSteps > 0 ? completedSteps / totalSteps : 0,
    costUsd: 0,
    latencyMs: plan.completedAt ? Math.max(0, plan.completedAt - plan.createdAt) : 0,
    safetyPassed: true,
    ...
    evidence: {
      status: plan.status,
      replayCacheId: replayCacheId ?? null,
      completedSteps,
      totalSteps,
    },
    occurredAt: new Date(plan.completedAt ?? Date.now()).toISOString(),
  });
