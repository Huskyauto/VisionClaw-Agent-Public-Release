...
interface PendingReuse {
  reviewId: number;
  tenantId: number; // authoritative tenant from the review INSERT context
...
/**
 * Bind a JavaScript string array as one opaque Postgres parameter. The pg
 * driver, rather than application string concatenation, performs text[] encoding
 * so quotes, backslashes, commas, braces, and empty strings retain their exact
 * values and can never become SQL syntax.
 */
export function parameterizedTextArray(values: string[]): SQL {
  return drizzleSql`${drizzleSql.param(values)}::text[]`;
}
  ...
  try {
    ...
    const res = await db.execute(sql`
      ...
      VALUES (
        ...
        ${reuseEligible},
        ${parameterizedTextArray(surfaced)},
        false
      )
      RETURNING id
    `);
...
export async function markReviewReused(
  ...
  try {
    ...
    await db.execute(sql`
      ...
      WHERE id = ${pending.reviewId}
        AND tenant_id = ${pending.tenantId}
        AND reused = false
    `);
    pendingReuseByConversation.delete(conversationId as number);
  } catch (_e) {
