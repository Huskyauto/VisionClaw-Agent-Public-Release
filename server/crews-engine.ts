...
async function executeAgentTask(
  ...
  const model = agent.config?.model || "anthropic/claude-sonnet-4-5";
  ...
  try {
    ...
    // R125+144 — radio block rides in the USER message (unprivileged, already
    // delimiter-wrapped + sanitized by the mention-injection lib), never the
    // system prompt.
    const radioSuffix = radioBlock ? `\n\n${radioBlock}` : "";
    const response = await anthropic.messages.create({
      model: model.replace("anthropic/", ""),
      max_tokens: maxTokens,
      system: interpolatedPrompt,
      messages: [{ role: "user", content: `Execute this task and provide your output. Be thorough and precise.\n\nTask: ${interpolateInputs(task.description, inputs)}\n\nExpected output format: ${task.expectedOutput}${radioSuffix}` }],
    });
...
async function executeSequential(
  ...
  const radio = await initCrewRadio(crew, tenantId);
  ...
  for (const task of tasks) {
    ...
    const radioBlock = await injectCrewRadio(radio, tenantId);

    try {
      const result = await executeAgentTask(agent, task, contextOutputs, inputs, tenantId, radioBlock);
      taskOutputs.push({
        taskId: task.id,
        taskName: task.name,
      ...
      await broadcastCrewDiscovery(radio, tenantId, task, agent.name, result.output);
...
${tasks.map((t, i) => `${i + 1}. ${t.name || "Task " + t.id}: ${t.description.slice(0, 200)}`).join("\n")}`;
  ...
  for (const task of tasks) {
    ...
    try {
      const result = await executeAgentTask(selectedAgent, task, contextOutputs, inputs, tenantId, radioBlock);
      taskOutputs.push({
        taskId: task.id,
        taskName: task.name,
        output: result.output,
      ...
      await broadcastCrewDiscovery(radio, tenantId, task, selectedAgent.name, result.output);
      totalTokenUsage.prompt += result.tokenUsage.prompt;
      totalTokenUsage.completion += result.tokenUsage.completion;
...
// R125+144 — AgentRadio passive awareness for crews (arXiv:2607.28430).
// A per-crew radio channel lets each task see fresh discoveries posted DURING
// the run (by concurrent runs of the same crew, operators, or other agents),
// not just the staged contextOutputs handoff. All three helpers fail open.
interface CrewRadio {
  channelName: string;
  lastId: number;
  enabled: boolean;
...
async function initCrewRadio(crew: Crew, tenantId: number): Promise<CrewRadio> {
  const channelName = `crew-${crew.id}-radio`;
  try {
    const { mentionInjectionEnabled, snapshotRadioHighWater } = await import("./lib/mention-injection");
    if (!mentionInjectionEnabled()) return { channelName, lastId: Number.MAX_SAFE_INTEGER, enabled: false };
    const lastId = await snapshotRadioHighWater(tenantId, channelName);
    return { channelName, lastId, enabled: lastId !== Number.MAX_SAFE_INTEGER };
  } catch {
    return { channelName, lastId: Number.MAX_SAFE_INTEGER, enabled: false };
  }
}

/** Returns the untrusted radio block ("" when nothing new). The block goes
 * into the task's USER message, never the system prompt — the prose framing
 * is not a security boundary, so peer content must stay unprivileged. */
async function injectCrewRadio(radio: CrewRadio, tenantId: number): Promise<string> {
  if (!radio.enabled) return "";
  try {
    const { fetchRadioMessages } = await import("./lib/mention-injection");
    const fresh = await fetchRadioMessages({ tenantId, channelName: radio.channelName, afterId: radio.lastId });
    radio.lastId = Math.max(radio.lastId, fresh.lastId);
    return fresh.count > 0 ? fresh.block : "";
  } catch (err: any) {
    console.warn(`[crews-engine] radio inject failed (fail-open): ${err?.message}`);
    return "";
  }
}

async function broadcastCrewDiscovery(radio: CrewRadio, tenantId: number, task: CrewTask, agentName: string, output: string): Promise<void> {
  if (!radio.enabled) return;
  try {
    const { postDiscovery } = await import("./lib/mention-injection");
    const ownId = await postDiscovery({
      tenantId,
      channelName: radio.channelName,
      content: `[${agentName}] finished "${task.name || "Task " + task.id}": ${output.slice(0, 600)}`,
      metadata: { taskId: task.id },
    });
    // Advance past EXACTLY our own post (self-echo guard). Never snapshot the
    // global high-water here — that would swallow concurrent siblings' posts
    // that landed between our post and the snapshot.
    if (ownId !== null && ownId > radio.lastId) radio.lastId = ownId;
  } catch (err: any) {
    console.warn(`[crews-engine] radio broadcast failed (fail-open): ${err?.message}`);
  }
}
...
export async function addFlowStep(params: {
  ...
  if (!params.name?.trim()) return { success: false, error: "name required" };
  // Tenant-isolation fix (2026-07-25): crewId arrives from untrusted
  // boundaries (req.body spread, LLM tool args) — verify the caller's tenant
  // owns the crew before wiring it into a flow step, or a tenant can plant
  // another tenant's crew and kick it off when the flow runs. Fail closed.
  if (params.crewId != null) {
    const crew = await getCrew(params.crewId, params.tenantId);
    if (!crew) return { success: false, error: "Crew not found" };
  }
...
async function executeFlowStep(
  ...
  if (step.actionType === "llm_call") {
    ...
    const response = await anthropic.messages.create({
      model: step.actionConfig.model || "claude-sonnet-4-5",
