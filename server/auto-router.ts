...
const TASK_CATEGORIES: Record<string, { models: string[]; description: string }> = {
  ...
  "coding": {
    ...
    description: "Code generation, debugging, refactoring, technical architecture (Opus 4.7 leads for high-end / self-healing recovery code)",
  },
  "reasoning": {
    models: [
      "gpt-5.4",
...
export async function autoRouteModel(userMessage: string, tenantId?: number): Promise<RouteDecision> {
  ...
  if (throttlePremium) {
    const filtered = available.filter(m => !m.id.includes("opus") && !m.id.includes("gpt-5.4"));
