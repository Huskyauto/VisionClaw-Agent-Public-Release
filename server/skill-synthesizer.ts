...
export async function listSkillCandidates(opts?: { personaId?: number; status?: string; limit?: number }): Promise<SkillCandidate[]> {
...
export async function promoteSkillCandidate(id: number): Promise<{ success: boolean; error?: string }> {
  try {
    const row: any = await db.execute(sql`SELECT id, title, content, persona_id, tenant_id FROM agent_knowledge WHERE id = ${id} AND category = 'skill_candidate' LIMIT 1`);
...
export async function rejectSkillCandidate(id: number, reason?: string): Promise<{ success: boolean; error?: string }> {
  try {
    await db.execute(sql`
      UPDATE agent_knowledge
      SET category = 'skill_rejected',
          ...
          updated_at = NOW()
      WHERE id = ${id} AND category = 'skill_candidate'
    `);
    return { success: true };
  } catch (e: any) {
    return { success: false, error: e.message };
...
export async function getApprovedSkillsForPersona(personaId: number): Promise<string> {
  const rows: any = await db.execute(sql`
