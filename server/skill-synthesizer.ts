...
import { redactPiiForStorage, redactObjectForStorage } from "./storage-helpers/pii-redaction-guard";
...
export async function synthesizeSkill(input: SynthesizeInput): Promise<{ success: boolean; candidate?: SkillCandidate; error?: string }> {
  try {
    ...
    // (post-edit-code-review HIGH, 2026-07-08). Object-level redaction keeps
    // the persisted blob valid JSON.
    const safeTitle = redactPiiForStorage(synth.title).redacted;
    const safePayload = redactObjectForStorage(payload).redacted;
    const safeContent = JSON.stringify(safePayload);
    await assertMemoryValueNotTombstoned({
      tenantId,
      source: "agent_knowledge",
      parts: [safeTitle, safeContent],
    });
...
export async function promoteSkillCandidate(id: number, tenantId: number): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    const r = row.rows[0];
    // Content safety scan at the promotion chokepoint (OpenClaw borrow,
    // R125+137.22): a promoted skill becomes future procedure/prompt
    // content, so dangerous capability patterns (shell exec, env harvest,
    // prompt-injection directives, guard-disable text) fail promotion
    // CLOSED. Operator override path: reject + reauthor, or promote via the
    // registry pipeline after manual review — this API stays strict.
    try {
      const { scanSkillContentSafety } = await import("./lib/skill-code-safety");
      const scan = scanSkillContentSafety(`${r.title || ""}\n${r.content || ""}`);
      if (!scan.safe) {
        const blocked = scan.findings.filter(f => f.severity === "block").map(f => f.pattern).join(", ");
        return { success: false, error: `Safety scan blocked promotion (patterns: ${blocked}). Review the candidate content; reauthor without the flagged capability text.` };
      }
    } catch (scanErr: any) {
      // The scan itself failing is a fail-CLOSED condition for a safety gate.
      return { success: false, error: `Safety scan unavailable (${scanErr?.message || scanErr}) — promotion refused (fail closed)` };
    }
    let payload: any = {};
    try { payload = JSON.parse(r.content); } catch (_silentErr) { logSilentCatch("server/skill-synthesizer.ts", _silentErr); }
...
export async function nudgeMemory(input: { fact: string; category?: string; personaId?: number; tenantId?: number; source?: string }): Promise<{ success: boolean; memoryId?: number; error?: string }> {
  ...
  // Durable-store boundary: a self-nudge fact is free text that can quote the
  // user / untrusted content — redact secrets/SSN/CC before persistence so
  // memory_entries never retains credential-shaped material (post-edit-code-
  // review HIGH, 2026-07-09). category/source are controlled enums but pass
  // through the same guard cheaply for consistency.
  const safeFact = redactPiiForStorage(input.fact).redacted;
  const safeCategory = redactPiiForStorage(input.category || "preference").redacted;
  const safeSource = redactPiiForStorage(input.source || "self_nudge").redacted;
  try {
    await assertMemoryValueNotTombstoned({
