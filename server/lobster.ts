...
interface PausedState {
  ...
  // Preserve the original launcher's tenant/persona across the pause→resume
  // boundary. Without this the resume path passed undefined → executeStep
  // defaulted to ADMIN_TENANT_ID + persona "system" (trusted), silently
  // escalating a non-admin launcher's remaining steps to admin scope.
...
async function executeStep(
  ...
  if (step.tool) {
    try {
      ...
      // SECURITY (architect HIGH): `_approvedByGate` and `_conversationId` are also
      // AUTHORIZATION / identity signals and must never be trusted from workflow step
      // args. A step setting `_approvedByGate: true` could otherwise self-approve a
      // requiresApproval tool (e.g. `exec`) and bypass the policy gate; a spoofed
      // `_conversationId` could redirect tenant-scoped writes. Strip both (mirrors
      // plan-executor). Autonomous lobster steps can NEVER self-approve — approval is
      // always false here so requiresApproval tools fail closed.
      delete (args as Record<string, any>)._approvedByGate;
      delete (args as Record<string, any>)._conversationId;

      // SECURITY (R125+65 — architect Cluster A HIGH): raw executeTool() skips the
      // AHB destructive-tool policy. Enforce it directly, fail-CLOSED (no HITL await
      // — that would deadlock an autonomous workflow). internal/autonomous (no
      // invokerTenantId, incl. the resume path) and the admin/owner tenant run as
      // the trusted "system" persona; any real non-admin invoker gets no trusted
      // name → trusted-only + approval-required tools fail closed.
      try {
        const { enforceToolPolicy } = await import("./safety/destructive-tool-policy");
        const policyPersonaName =
          invokerTenantId === undefined || invokerTenantId === ADMIN_TENANT_ID
            ? "system"
            : undefined;
        const pol = await enforceToolPolicy(step.tool, args, {
          tenantId: typeof (args as any)._tenantId === "number" ? (args as any)._tenantId : ADMIN_TENANT_ID,
          personaId: typeof invokerPersonaId === "number" ? invokerPersonaId : null,
          personaName: policyPersonaName,
          invokedVia: "lobster",
          hasApproval: false,
        });
        if (pol.action === "block") {
          return { id: step.id, status: "failed", error: `Blocked by destructive-tool policy: ${pol.reason}`, durationMs: Date.now() - start };
        }
      } catch (_polErr: any) {
        return { id: step.id, status: "failed", error: `Destructive-tool policy check failed for '${step.tool}'. Refusing to execute.`, durationMs: Date.now() - start };
      }
  ...
  for (let i = 0; i < workflow.steps.length; i++) {
    ...
    if (result.status === "approval_pending") {
      ...
      const pausedState: PausedState = {
        ...
        invokerTenantId: typeof params.tenantId === "number" ? params.tenantId : undefined,
        invokerPersonaId: typeof params.personaId === "number" ? params.personaId : undefined,
      };
...
async function resumeWorkflow(
  ...
  for (let i = state.pendingStepIndex + 1; i < state.workflow.steps.length; i++) {
    ...
    const result = await executeStep(step, state.completedSteps, state.resolvedArgs, globalEnv, defaultTimeout, maxStdoutBytes, state.invokerPersonaId, state.invokerTenantId);
    ...
    if (result.status === "approval_pending") {
      ...
      const newState: PausedState = {
        ...
        approvalItems: result.json || [],
        approvalStdinData: result.stdout,
        invokerTenantId: state.invokerTenantId,
        invokerPersonaId: state.invokerPersonaId,
      };
