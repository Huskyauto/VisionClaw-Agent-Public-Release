...
export async function ensureDataProtectionColumns(): Promise<void> {
  try {
    ...
    // Legacy migration (task #120): pre-existing DBs created memory_categories with
    // tenant_id INTEGER DEFAULT 1 (nullable). Drop the admin-tenant trapdoor and
    // enforce NOT NULL once no NULL rows remain. Loud on failure — do not let the
    // outer catch make this invisible.
    try {
      await db.execute(sql`ALTER TABLE memory_categories ALTER COLUMN tenant_id DROP DEFAULT`);
      const mcNulls: any = await db.execute(sql`SELECT count(*)::int AS n FROM memory_categories WHERE tenant_id IS NULL`);
      const mcNullCount = Number((mcNulls.rows?.[0] ?? mcNulls[0])?.n ?? 0);
      if (mcNullCount === 0) {
        await db.execute(sql`ALTER TABLE memory_categories ALTER COLUMN tenant_id SET NOT NULL`);
      } else {
        console.error(`[data-protection] TENANT-ISOLATION: memory_categories has ${mcNullCount} NULL tenant_id row(s) — NOT NULL not applied; backfill required`);
      }
    } catch (mcErr: any) {
      console.error("[data-protection] TENANT-ISOLATION migration failed (memory_categories tenant_id):", mcErr?.message || mcErr);
    }
    ...
    // Legacy column backfill only — new DBs get tenant_id NOT NULL (no default) from
    // shared/schema.ts. DEFAULT 1 removed (task: drop the hidden admin-tenant trapdoor);
    // keep DEFAULT 1 here ONLY for the one-time legacy add so pre-existing rows backfill,
    // then strip the default immediately so future inserts must pass tenant_id.
    await db.execute(sql`ALTER TABLE agent_knowledge ADD COLUMN IF NOT EXISTS tenant_id INTEGER DEFAULT 1`);
    try {
      await db.execute(sql`ALTER TABLE agent_knowledge ALTER COLUMN tenant_id DROP DEFAULT`);
      const akNulls: any = await db.execute(sql`SELECT count(*)::int AS n FROM agent_knowledge WHERE tenant_id IS NULL`);
      const akNullCount = Number((akNulls.rows?.[0] ?? akNulls[0])?.n ?? 0);
      if (akNullCount === 0) {
        await db.execute(sql`ALTER TABLE agent_knowledge ALTER COLUMN tenant_id SET NOT NULL`);
      } else {
        console.error(`[data-protection] TENANT-ISOLATION: agent_knowledge has ${akNullCount} NULL tenant_id row(s) — NOT NULL not applied; backfill required`);
      }
    } catch (akErr: any) {
      console.error("[data-protection] TENANT-ISOLATION migration failed (agent_knowledge tenant_id):", akErr?.message || akErr);
    }
...
export async function softDeleteConversation(
  ...
  try {
    ...
    // Tenant-scoped atomic write (72h review): the pre-read above is advisory
    // only — the UPDATE itself must carry the tenant predicate or a race can
    // cross the tenant boundary.
    const upd = await db.execute(
      sql`UPDATE conversations SET deleted_at = NOW(), deleted_by = ${deletedBy} WHERE id = ${conversationId} AND tenant_id = ${tenantId} AND deleted_at IS NULL RETURNING id`
    );
...
export async function permanentlyPurgeSoftDeleted(): Promise<{ purged: number }> {
  try {
    ...
    for (const conv of rows) {
      ...
      // Defense-in-depth (architect, post-72h review): scope child deletes to
      // the conversation's own tenant so an FK anomaly can never let a purge
      // reach another tenant's rows.
      await db.execute(sql`DELETE FROM messages WHERE conversation_id = ${conv.id} AND tenant_id = ${conv.tenant_id}`);
      await db.execute(sql`DELETE FROM compaction_archives WHERE conversation_id = ${conv.id} AND tenant_id = ${conv.tenant_id}`);
      await db.execute(sql`DELETE FROM conversations WHERE id = ${conv.id} AND tenant_id = ${conv.tenant_id}`);
    }
...
export async function verifyMessageSaved(
  ...
  // Fail CLOSED: without a valid tenant this becomes a cross-tenant existence
  // oracle. Require an explicit positive tenantId and scope the SELECT to it.
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    console.error(`[data-protection] verifyMessageSaved called without valid tenantId (conv ${conversationId})`);
    return false;
  }
  try {
    const result = await db.execute(
      sql`SELECT id FROM messages WHERE conversation_id = ${conversationId} AND role = ${role} AND tenant_id = ${tenantId} ORDER BY created_at DESC LIMIT 1`
    );
    const rows = (result as any).rows || result;
...
export async function backupConversationToDrive(
  ...
  try {
    ...
    const msgResult = await db.execute(
      sql`SELECT role, content, created_at FROM messages WHERE conversation_id = ${conversationId} AND tenant_id = ${tenantId} ORDER BY created_at ASC`
    );
    ...
    const archiveResult = await db.execute(
      sql`SELECT content, archived_at FROM compaction_archives WHERE conversation_id = ${conversationId} AND tenant_id = ${tenantId} ORDER BY archived_at ASC`
    );
  ...
  try {
    const result = await db.execute(
      sql`SELECT id, title, deleted_at, deleted_by, 
          (SELECT COUNT(*)::int FROM messages WHERE conversation_id = conversations.id AND tenant_id = ${tenantId}) as message_count
          FROM conversations 
