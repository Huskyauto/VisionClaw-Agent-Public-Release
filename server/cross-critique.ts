...
const PANEL: Array<{ id: string; lens: string; model: string }> = [
  { id: "claude", lens: "ux",        model: "claude-opus-4-7" },
  { id: "openai", lens: "technical", model: "gpt-5.4" },
...
interface Finding {
  ...
  rebuttalSurvival: number;    // 1-10, model self-scores how well it survives a rebuttal
  evidence?: string;
  lens: string;
...
const SCHEMA = {
  ...
  properties: {
    findings: {
      ...
      items: {
        type: "object",
        required: ["finding", "severity", "rebuttalSurvival"],
        properties: {
          finding: { type: "string" },
          severity: { type: "string" },
          rebuttalSurvival: { type: "integer" },
...
For each counter-argument, score "rebuttalSurvival" 1-10 — how likely the argument survives a smart rebuttal (10 = airtight, 1 = easily dismissed). Return 3-7 findings, ranked highest survival first.

Target under review:
${target}
${context ? `\nAdditional context:\n${context}` : ""}

Return ONLY JSON: {"findings":[{"finding":"...","severity":"low|medium|high|critical","rebuttalSurvival":1-10,"evidence":"..."}]}`;
  ...
  const findings: Finding[] = (r.json.findings as any[]).slice(0, 8).map((f) => ({
    ...
    rebuttalSurvival: Math.max(1, Math.min(10, Number(f.rebuttalSurvival) || 5)),
