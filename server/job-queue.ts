...
import { classifyFailure, retryPolicyFor, type FailureClass } from "./lib/failure-taxonomy";
import { ALL_TENANTS, type TenantScope } from "./storage-helpers/tenant-scope";
...
export async function enqueueUniqueJob(
  ...
  dedupeKey: string,
  payload: Record<string, any>,
  opts: EnqueueOpts & { tenantId: number },
): Promise<number> {
  if (!Number.isInteger(opts.tenantId) || opts.tenantId <= 0 || !dedupeKey.trim()) {
    throw new Error("enqueueUniqueJob requires a positive tenantId and dedupeKey");
  }
  const nextRunAt = new Date(Date.now() + (opts.delayMs ?? 0));
  const result: any = await db.transaction(async (tx: any) => {
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext(${`${kind}:${opts.tenantId}:${dedupeKey}`}))`);
    return tx.execute(sql`
      WITH existing AS (
        ...
        INSERT INTO agent_jobs (kind, payload, tenant_id, persona_id, max_attempts, next_run_at, parent_job_id)
        SELECT ${kind}, ${JSON.stringify({ ...payload, dedupeKey })}::jsonb, ${opts.tenantId},
               ${opts.personaId ?? null}, ${opts.maxAttempts ?? 100}, ${nextRunAt}, ${opts.parentJobId ?? null}
...
export async function claimDueJobs(
  ...
  const result: any = await db.execute(sql`
    ...
    RETURNING id, kind, payload, tenant_id, persona_id, status, attempts, max_attempts,
              lease_until, next_run_at, parent_job_id, result, error, failure_class, rollback_note,
...
/**
 * Mark a job as failed, with FAILURE-CLASS-AWARE retry handling.
 *
 * The raw error is classified (server/lib/failure-taxonomy.ts) into a
 * FailureClass that decides the retry behavior:
 *   - DETERMINISTIC classes (validation_error, guardrail_blocked, not_found,
 *     deterministic_input) are NON-retryable → straight to failed_terminal even
 *     if retry budget remains, because retrying just repeats the same bad call
 *     (wasted money + loops). This is the core reliability fix.
 *   - rate_limit retries with a LONGER backoff (3×) so we don't hammer a
 *     throttled upstream.
 *   - everything else (incl. the "unknown" fallback) keeps the original
 *     exponential schedule, so unclassified errors are a behavioral no-op.
 *
 * The resolved class is persisted to `failure_class` for the dead-letter view
 * and failure clustering. Mutating handlers may pass `rollbackNote` describing
 * the compensating action taken (or still required) so the operator inbox can
 * see the recovery state of a half-applied job.
 *
...
export async function failJob(
  ...
  attempts: number,
  errorMsg: string,
  opts: { rollbackNote?: string; failureClass?: FailureClass } = {},
): Promise<void> {
  ...
  const failureClass = opts.failureClass ?? classifyFailure(errorMsg);
  const policy = retryPolicyFor(failureClass);
  const rollbackNote = opts.rollbackNote ?? null;

  // Non-retryable (deterministic) failures dead-letter immediately; retryable
  // classes still respect the attempt budget.
  const goTerminal = !policy.retryable || attempts >= maxAttempts;

  if (goTerminal) {
    const terminalTag = !policy.retryable && attempts < maxAttempts
      ? ` [non-retryable: ${failureClass}]`
      : "";
    await db.execute(sql`
      UPDATE agent_jobs
      SET status = 'failed_terminal',
          error = ${errorMsg + terminalTag},
          failure_class = ${failureClass},
          rollback_note = COALESCE(${rollbackNote}, rollback_note),
          completed_at = NOW(),
          lease_until = NULL
      WHERE id = ${id}
        AND status = 'running'
  ...
  // Class-aware exponential backoff: base 30s · 4^(n-1), then the class
  // multiplier (rate_limit stretches it), capped at 1h.
  const base = Math.min(30_000 * Math.pow(4, attempts - 1), 60 * 60 * 1000);
  const backoffMs = Math.min(base * policy.backoffMultiplier, 60 * 60 * 1000);
...
/**
 * Operator: manually retry a failed or cancelled job (resets attempts).
 * Returns whether a retry was actually scheduled, so callers do not report a
 * successful retry for completed, running, pending, or unknown jobs.
 */
export async function retryJob(id: number, tenantId: number): Promise<boolean> {
  if (!Number.isInteger(tenantId) || tenantId <= 0) {
    throw new Error("retryJob: tenantId must be a positive integer");
  }
  const result: any = await db.execute(sql`
    UPDATE agent_jobs
    SET status = 'pending',
    ...
    WHERE id = ${id}
      AND tenant_id = ${tenantId}
      AND status IN ('failed','failed_terminal','cancelled')
    RETURNING id
  `);
  const rows = (result.rows || result) as any[];
...
/**
 * Admin operator lookup used solely to resolve a job's immutable tenant scope
 * before a platform-admin route retries it. Tenant-facing routes must never
 * use this helper to select another tenant's job.
 */
export async function getJobTenantForAdmin(id: number): Promise<{ tenantId: number | null } | null> {
  if (!Number.isInteger(id) || id <= 0) return null;
  const [row]: any = await db.execute(sql`
    SELECT tenant_id FROM agent_jobs WHERE id = ${id}
  `).then((r: any) => r.rows || r);
  return row ? { tenantId: row.tenant_id ?? null } : null;
}
...
/** Operator: aggregate counts for the dashboard header under explicit scope. */
export async function getJobStats(scope: TenantScope): Promise<Record<JobStatus, number> & { total: number }> {
  if (
    scope !== ALL_TENANTS &&
    (typeof scope !== "number" || !Number.isInteger(scope) || scope <= 0)
  ) {
    throw new Error("getJobStats requires a positive tenantId or ALL_TENANTS");
  }
  const tenantFilter = scope === ALL_TENANTS
    ? sql``
    : sql`AND tenant_id = ${scope}`;
  const result: any = await db.execute(sql`
...
function rowToJob(r: any): AgentJob {
  return {
    ...
    error: r.error ?? null,
    failureClass: r.failure_class ?? null,
    rollbackNote: r.rollback_note ?? null,
