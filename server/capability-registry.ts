...
const AGENTS: CapabilityRecord[] = [
  ...
  { kind: "agent", name: "Echo",            category: "research",    description: "UX researcher — voice of the user; grounds product decisions in real user evidence.",  codePath: "server/seed.ts", codeSymbol: "DEFAULT_PERSONAS[Echo]" },
  { kind: "agent", name: "Hermes",          category: "marketing",   description: "Growth hacker; acquisition channels and experiments (Felix approves budget/public).",  codePath: "server/seed.ts", codeSymbol: "DEFAULT_PERSONAS[Hermes]" },
  { kind: "agent", name: "Robert",          category: "wellness",    description: "Wellness Coach ([Your Product]); empathy + mindfulness tools for stress eating.",     codePath: "server/seed.ts", codeSymbol: "personas table row id=16 (DB-seeded, not in DEFAULT_PERSONAS)" },
];
...
const FULFILLMENT_PATHS: CapabilityRecord[] = [
  { kind: "fulfillment", name: "youtube_transcript_fallback", category: "research", description: "Deterministic YouTube transcript acquisition: one fixed Firecrawl Show-transcript action, English captions once, then yt-dlp audio plus ElevenLabs Scribe once. Accepts only explicit transcript-panel content, writes it to a local file, and returns a compact receipt. Runbook: .agents/skills/youtube-transcript-fallback/SKILL.md.", codePath: ".agents/skills/youtube-transcript-fallback/SKILL.md", codeSymbol: "extractToFile" },
  ...
  { kind: "fulfillment", name: "human_ai_synergy_trial", category: "measurement", description: "WHAT: Owner-only, report-only deterministic three-arm observational measurement across human-alone, AI-alone, and human-plus-AI conditions. WHEN: An owner needs bounded task evidence and both baseline lift calculations. NOT-WHEN: Never makes model calls, makes hiring/IQ or causal claims, or takes autonomous action. Route: server/routes/human-ai-synergy.ts; core: server/lib/human-ai-synergy-core.ts; page: client/src/pages/admin-human-ai-synergy-trial.tsx.", codePath: "server/routes/human-ai-synergy.ts", codeSymbol: "registerHumanAiSynergyRoutes", metadata: { ownerOnly: true, reportOnly: true } },
...
const WEDGES: CapabilityRecord[] = [
  ...
  { kind: "fulfillment", name: "wedge_smart_leads",            category: "wedge", description: "R125+13.15 — Smart Leads Zero Research wedge ($99/$249/$499 per month; Bob's pick 2026-08-14, highest composite 27/S in the Isenberg portfolio). Automated lead enrichment: raw lead → SSRF-jailed research sweep → 3-proposer MoA ensemble draft (gpt-5.6-sol, claude-sonnet-5, gemini-3.1-pro-preview) → claude-opus-5 final-touch aggregation → 1-page sales dossier via deliver_product. SOP: data/output-skills/wedge-smart-leads-sop.md. Tracker: project tagged 'wedge:smart-leads'.", codePath: "data/output-skills/wedge-smart-leads-sop.md", codeSymbol: "lookup_output_skill:wedge-smart-leads-sop" },
...
// =============================================================================
// R125+62 — AUTONOMOUS-LOOP CONTRACT SYSTEM. Governance infrastructure (NOT
// agent-callable tools): a typed contract for every long-running autonomous
// loop (objective/check/feedback/stop/escalate/authority/failMode) plus the
// Loop Doctor audit that fails CI if any live workflow has no contract.
// Registered here so Minerva's planner + the orphan-skill audit know the loop
// fleet exists and is governed. The per-loop knowledge lives in agent_knowledge
// (walkLoopContracts in scripts/agent-knowledge-refresh.ts) for search_knowledge
// / recall_capabilities retrieval; this is the high-level "it exists" pointer.
// =============================================================================
const LOOP_ENGINEERING: CapabilityRecord[] = [
  { kind: "fulfillment", name: "loop_engineering_patterns", category: "self_repair", description: "R125+137.80 — Loop Engineering prompt patterns: seven goal-based iterative-fix loops (Type Error Killer, Test Gap Closer, Dead Code Sweeper, Dependency Updater, Pattern Migrator, Lint Fixer, Doc Gap Filler) plus a template for composing new ones. Every loop = Goal + Rules (one unit at a time, explicit do-NOTs) + Verify-after-each-step (undo if the count went up) + Exit-when. Use when grinding down a countable defect class or building anything that benefits from fix-one → verify → repeat. Retrievable via search_knowledge (skill:loop-engineering). Pairs with the 2-failed-corrections rule — log and move on after 2 failed attempts on the same item.", codePath: ".agents/skills/loop-engineering/SKILL.md", codeSymbol: "loop-engineering skill" },
];

const GOLD_EXEMPLARS: CapabilityRecord[] = [
  { kind: "fulfillment", name: "gold_exemplar_library", category: "quality", description: "R125+137.81 — Gold Exemplar Library: the platform learns from its own best work. When grade_deliverable scores a deliverable ≥90, pass content_excerpt (the production TEXT — narration script, doc outline+copy, slide contents) and it is auto-captured into deliverable_exemplars (tenant-scoped, sha256-deduped, cap 5 auto_grade per tenant+format; owner_marked rows are permanent). At plan time classifyDeliverable auto-injects the top-2 exemplars as few-shot structure/style reference (delimited as untrusted data, never instructions). Agents' ONE job: ALWAYS pass content_excerpt when calling grade_deliverable so top work gets captured — retrieval/injection is automatic.", codePath: "server/lib/deliverable-exemplars.ts", codeSymbol: "maybeStoreExemplar + getExemplarGuidance" },
];

const INTERNAL_ADMIN_INFRASTRUCTURE: CapabilityRecord[] = [
  { kind: "fulfillment", name: "portable_skill_behavior_bundle", category: "skill_governance", description: "Owner/admin internal infrastructure for signing, verifying, and importing learned skill behavior bundles. Bundles are tenant-bound, expiring, compatibility-checked, safety-scanned, and imported only into review quarantine; foreign jury approval is provenance-only and never authorizes promotion.", codePath: "server/lib/skill-behavior-bundle.ts", codeSymbol: "createPortableSkillBehaviorBundle + verifyPortableSkillBehaviorBundle + importPortableSkillBehaviorBundle" },
  { kind: "fulfillment", name: "artifact_authority_receipts", category: "loop_governance", description: "Server-owned deterministic verification receipts bind an accepted claim to its tenant, subject, exact source bytes, verifier ID/version, verdict, reasons, and evidence digest. Research code proposals must carry a current accepted receipt before atomic apply; status strings, agent agreement, and stale verification are never authority.", codePath: "server/lib/artifact-authority.ts", codeSymbol: "issueArtifactAuthorityReceipt + validateArtifactAuthorityReceipt" },
];

const LOOP_GOVERNANCE: CapabilityRecord[] = [
  { kind: "fulfillment", name: "governed_loop_portfolio", category: "loop_governance", description: "R131+sec — tenant-scoped append-only outcome graph for research, plan replay, and Skill Optimizer telemetry; computes convergence and advisory allocation. Owner-only Dream-RSI historical replay is exact-opt-in, bounded, report-only, and has no model, tool, promotion, allocation, evaluator, limit, or authority-changing path.", codePath: "server/routes/loop-portfolio.ts", codeSymbol: "registerLoopPortfolioRoutes" },
  { kind: "fulfillment", name: "loop_contract_registry", category: "loop_governance", description: "R125+62 — Typed LoopContract registry for every autonomous loop in the platform (8 loops: 6 workflow-backed — agentic-ci-self-healer, auto-git-push, jury-queue-drainer, skill-optimizer-nightly, tenant-isolation-audit, agent-knowledge-refresh; + 2 in-process — heartbeat-scheduled-tasks, bwb-weekly-render). Each contract declares objective / check / feedback / stop[] / escalate[] / authority / failMode / verifier (generator≠evaluator independence) / spend (token-cap ceiling); every mutating or destructive loop is failMode 'closed', no loop may grade its own output (verifier 'self') or spend uncapped (spend 'unbounded') — VisionClaw invariants. Source of truth that the Loop Doctor, the agent-knowledge walker, and this registry all read — no loop runs uncontracted.", codePath: "server/loop-contracts.ts", codeSymbol: "LOOP_CONTRACTS + getLoopContract + auditLoopCoverage" },
  { kind: "fulfillment", name: "loop_doctor_audit", category: "loop_governance", description: "R125+62 — Loop Doctor: audits every contract for completeness (non-empty stop + escalate, mutating⇒failMode closed, verifier≠self [generator/evaluator split], spend≠unbounded [token cap], source file exists) AND derives the live .replit workflow set + IN_PROCESS_LOOPS, ERRORing on any uncontracted loop so a new loop can't ship as stale-doc theater. Exit 0/1/2/3, --json. Harnessed as weekly-maintenance Pass 12 and covered by tests/unit/loop-contracts.test.ts.", codePath: "scripts/loop-contract-audit.ts", codeSymbol: "loop-contract-audit (Loop Doctor)" },
];

const MODEL_ROUTING: CapabilityRecord[] = [
  { kind: "fulfillment", name: "cost_aware_model_routing", category: "llm", description: "Routes task tiers through healthy zero-marginal lanes first: Profundo flat-rate, Openference flat-rate, Replit model farm, and verified OpenRouter free models. GPT-6 Luna is the first bounded metered fallback only when ALLOW_METERED_LLM authorizes spend; GPT-6 Sol and premium models require deliberate escalation. Tenant access, provider quarantine, daily ceilings, cost attribution, and provider-diverse review remain authoritative.", codePath: "server/providers.ts + server/model-registry.ts + server/agentic/cost-ledger.ts", codeSymbol: "getModelForTierAsync + getClientForModel + estimateCostUsd" },
];
