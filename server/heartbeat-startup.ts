...
export function shouldPreserveOverdueRunAtStartup(taskType: string, productionRuntime: boolean): boolean {
  return (taskType === "ideabrowser_dev_sync" && !productionRuntime)
    || (taskType === "ideabrowser_ingest" && productionRuntime);
}
  ...
  try {
    ...
    for (const t of allTasks) {
      ...
      if (t.type === "delegation" || (t.runOnce && t.parentTaskId)) {
        await storage.updateHeartbeatTask(t.id, { enabled: false }, t.tenantId);
        cleaned++;
    ...
    for (const t of allTasks) {
      ...
      if (t.model && t.model.startsWith("claude-")) {
        await db.execute(sql`UPDATE heartbeat_tasks SET model = 'gpt-5.4' WHERE id = ${t.id}`);
        console.log(`[heartbeat] Startup fix: "${t.name}" model normalized from ${t.model} to gpt-5.4 ($0 lane)`);
      }
      ...
      if (isOverdue && shouldPreserveOverdueRunAtStartup(t.type, isProductionRuntime())) {
        console.log(`[heartbeat] Startup catch-up: "${t.name}" remains due so today's IdeaBrowser folder sync is not skipped`);
        continue;
      }
