...
export const TOOL_POLICIES: Record<string, ToolPolicy> = {
  typesafe_jev_advisory: { name: "typesafe_jev_advisory", risk: "safe", riskClass: "LOW", requiresStructuredArgs: true, trustedPersonasOnly: true, extraAllowedPersonas: ["Felix", "Minerva"] },
  // Bounded, tenant-scoped, read-only public procurement research. Explicitly
  // registered rather than relying on the default-safe fallback.
  discover_cmmc_prospects:    { name: "discover_cmmc_prospects", risk: "safe", riskClass: "LOW", requiresStructuredArgs: true },
  lookup_sam_company_cage:    { name: "lookup_sam_company_cage", risk: "safe", riskClass: "LOW", requiresStructuredArgs: true },
  lookup_sam_exact_company:   { name: "lookup_sam_exact_company", risk: "safe", riskClass: "LOW", requiresStructuredArgs: true },
  ...
  // Simulation Sandbox (S4). sandbox_run spends real LLM budget on the
  // model-swap corpora (atomic $5/run claim, fail-closed) and replays the live
  // intent gate — trusted personas + structured args only. sandbox_report is a
  // tenant-scoped read-only fetch of a stored report.
  sandbox_run:                 { name: "sandbox_run",                 risk: "sensitive",   riskClass: "HIGH",   requiresStructuredArgs: true, trustedPersonasOnly: true },
  sandbox_report:              { name: "sandbox_report",              risk: "safe",        riskClass: "LOW",    requiresStructuredArgs: false },
  // sandbox_promote spends ~5x a normal LLM call (3-frontier jury) and writes
  // durable improvement proposals — trusted personas + structured args only.
  sandbox_promote:             { name: "sandbox_promote",             risk: "sensitive",   riskClass: "HIGH",   requiresStructuredArgs: true, trustedPersonasOnly: true },
  // Verified Revenue Missions (S5a). Owner-only at the handler (tenant 1 gate,
  // fail-closed). create writes a durable mission; draft_experiment harvests
  // the owner's Gmail graph (PII) + drafts outreach copy — sensitive +
  // trusted-only + structured args. It SENDS NOTHING (awaiting_approval packet;
  // launch fail-closes on approved_by_owner_at — approval is HITL-only, not a
  // tool). list/status are owner-scoped reads.
  revenue_mission_create:           { name: "revenue_mission_create",           risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true },
  revenue_mission_list:             { name: "revenue_mission_list",             risk: "safe",      riskClass: "LOW" },
  mission_portfolio_review:         { name: "mission_portfolio_review",         risk: "safe",      riskClass: "LOW" },
  revenue_mission_status:           { name: "revenue_mission_status",           risk: "safe",      riskClass: "LOW",    requiresStructuredArgs: true },
  revenue_mission_draft_experiment: { name: "revenue_mission_draft_experiment", risk: "sensitive", riskClass: "HIGH",   requiresStructuredArgs: true, trustedPersonasOnly: true },
  product_listing_create:           { name: "product_listing_create",           risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true },
  product_listing_list:             { name: "product_listing_list",             risk: "safe",      riskClass: "LOW" },
  create_payment_link:              { name: "create_payment_link",              risk: "sensitive", riskClass: "HIGH",   requiresStructuredArgs: true, trustedPersonasOnly: true, requiresApproval: true },
  // DFY audit Fix Kit generator — network-touching product output (SSRF-jailed
  // internally); generates files only, delivery stays HITL via deliver_product.
  generate_audit_fix_kit:           { name: "generate_audit_fix_kit",           risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true, trustedPersonasOnly: true },
  ...
  // generate_evidence_docket fans out through the SAME delivery pipeline as
  // deliver_product (writes to Drive + emails the tenant owner/reviewer), and
  // bundles cross-cutting audit rows (security checks, jury, delivery). Same
  // mass-comms + external-exfil class → sensitive HIGH with structured args so
  // the gate inspects emailTo before any send. No requiresApproval (no HITL
  // deadlock — mirrors deliver_product).
  generate_evidence_docket:    { name: "generate_evidence_docket",    risk: "sensitive",   riskClass: "HIGH",     requiresStructuredArgs: true },
  ...
  // R125+83 — aeo_score: pure mechanical Markdown analysis (no LLM, no network,
  // no DB, no tenant data). Registered explicitly so it never rides the default.
  aeo_score:                      { name: "aeo_score", risk: "safe", riskClass: "LOW", requiresStructuredArgs: true },
  ...
  scrapling_scrape:               { name: "scrapling_scrape", risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true, trustedPersonasOnly: true },
  multi_source_collect:           { name: "multi_source_collect", risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true, trustedPersonasOnly: true },
  ...
  query_project_relationships:    { name: "query_project_relationships", risk: "safe", riskClass: "LOW" },
  ...
  derived_api_capture_list:       { name: "derived_api_capture_list", risk: "safe", riskClass: "LOW", requiresStructuredArgs: false },
  derived_api_derive:             { name: "derived_api_derive", risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true },
  derived_api_replay:             { name: "derived_api_replay", risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true },
  derived_api_list:               { name: "derived_api_list", risk: "safe", riskClass: "LOW", requiresStructuredArgs: false },
  derived_api_delete:             { name: "derived_api_delete", risk: "safe", riskClass: "LOW", requiresStructuredArgs: false },
  ...
  update_invoice_status:          { name: "update_invoice_status", risk: "sensitive", riskClass: "MEDIUM", requiresStructuredArgs: true, requiresApproval: true },
...
/**
 * Resolve the structural risk LEVEL ("safe" | "sensitive" | "destructive") for a
 * tool. Explicit TOOL_POLICIES entry wins; an UNREGISTERED tool falls back to
 * name inference — a suspicious name (delete_/exec_sql/transfer_/…) resolves
 * `destructive` (fail-closed) rather than the bare `safe` default. Use this
 * (not `TOOL_POLICIES[name]?.risk`, which is undefined for unlisted tools) wherever
 * a caller needs the effective risk of an arbitrary tool name.
 */
export function getEffectiveToolRisk(toolName: string): ToolRiskLevel {
  return TOOL_POLICIES[toolName]?.risk ?? inferRiskFromName(toolName);
}
...
export async function enforceToolPolicy(
  ...
  // Value cap. SECURITY (Fable-5 C3 hardening): the cap must not be bypassable
  // by TYPE CONFUSION. The prior check only fired for `typeof val === "number"`,
  // so a jailbroken model could smuggle `amount: "600000"` (numeric string) or
  // `NaN`/`Infinity` past the cap — the executor / provider SDK then coerces the
  // string back to a number and spends it, or a non-finite value slips through
  // because `NaN > max` and `-1 > max` are both false. A value-capped tool MUST
  // receive a CLEAN, finite, non-negative number within [0, max]; anything else
  // (missing, non-numeric, non-coercible, non-finite, negative) fails CLOSED.
  if (policy.maxValue) {
    const raw = getByPath(args, policy.maxValue.argPath);
    // Coerce numeric strings ("600000") to their real numeric value so the cap
    // sees what the executor will actually spend; everything else → NaN → block.
    const num =
      typeof raw === "number"
        ? raw
    ...
    const shown =
      ...
      : raw === null ? "null"
      : typeof raw === "object" ? JSON.stringify(raw).slice(0, 60)
      : String(raw).slice(0, 60);
    if (!Number.isFinite(num)) {
      return block(`tool "${toolName}" arg "${policy.maxValue.argPath}" must be a finite number (got ${shown}); value-capped tools reject non-numeric/non-finite args`);
    }
    if (num < 0) {
      return block(`tool "${toolName}" arg "${policy.maxValue.argPath}"=${num} must be non-negative`);
    }
    if (num > policy.maxValue.max) {
      return block(`tool "${toolName}" arg "${policy.maxValue.argPath}"=${num} exceeds policy max ${policy.maxValue.max}${policy.maxValue.unit ? ` ${policy.maxValue.unit}` : ""}`);
    }
  }
