...
export interface IntentGateContext {
  ...
  refusalCopy?: string;
  /**
   * Simulation Sandbox (S2): when false, the gate runs evaluate-only —
   * no security_intent_checks INSERT, no decline_event, no cache write.
   * Default true (production behavior unchanged). Only platform replay
   * code sets this; it is never derived from user/LLM input.
   */
  auditWrites?: boolean;
}
...
const FALLBACK_PATTERNS: Array<{ category: string; rx: RegExp }> = [
  ...
  // R125+138 (Hermes/Echo onboarding): these three categories previously had
  // CATEGORY_HINTS coverage only (or none) — meaning in NO_INTENT_GATE_LLM /
  // classifier-down mode, moderate personas could never accumulate the 2nd
  // signal for them. One fallback pattern per category, per security-hardening.
  { category: "mass_email_unapproved", rx: /\b(email|send|blast|mail|message)\b.{0,80}\b(all|every)\s+(?:the\s+)?(customers?|users?|subscribers?|contacts?|address(?:es)?|inbox(?:es)?|lists?)\b/i },
  { category: "public_post_unapproved", rx: /\b(post|posts|publish(?:es)?|tweet|announce|share)\b.{0,80}\b(without|skip(?:ping)?|before|bypass(?:ing)?)\b.{0,40}\b(review|approval|approv\w+|waiting)/i },
  { category: "tenant_isolation_bypass", rx: /\b(bypass|disable|skip|ignore|break|cross)\w*\b.{0,40}\btenant\b|\banother tenant'?s\b.{0,80}\b(data|records?|customers?|users?|interviews?|transcripts?)\b/i },
];
...
export async function runIntentGate(content: string, ctx: IntentGateContext): Promise<IntentGateResult> {
  ...
  // Sandbox replay (auditWrites:false) skips cache READS too — a replay must
  // never reuse a production cache entry (fidelity), just as it never writes one.
  const key = cacheKey(ctx.personaId, mode, ctx.restrictedCategories, content);
  if (ctx.auditWrites !== false) {
  ...
  const CATEGORY_HINTS: Record<string, RegExp> = {
    ...
    // R125+138: previously missing — personas declaring this category got zero
    // hint-tier coverage for it.
    public_post_unapproved: /\b(post|posts|publish(?:es)?|tweet|announce|share)\b.{0,80}\b(without|skip(?:ping)?|before|bypass(?:ing)?)\b.{0,40}\b(review|approval|approv\w+|waiting)/i,
  };
  ...
  // Evaluate-only mode (Simulation Sandbox replay): return the verdict with
  // ZERO writes — no cache entry (a replay under override categories must not
  // pollute the shared cache), no audit row, no decline event.
  if (ctx.auditWrites === false) {
    return result;
  }
