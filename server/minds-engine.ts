...
async function checkDependenciesMet(ticket: MindTicket, tenantId: number): Promise<boolean> {
  ...
  const dependencyIds = normalizeIntegerArray(ticket.dependsOn);
  if (dependencyIds.length === 0) return false;

  const res = await db.execute(sql`
    SELECT id, status FROM mind_tickets
    WHERE id = ANY(${dependencyIds}::int[])
...
export async function emitEvent(params: {
  ...
  const res = await db.execute(sql`
    ...
    SELECT m.id, m.tenant_id, ${params.eventType.slice(0, 100)}, ${(params.source || "system").slice(0, 200)}, ${JSON.stringify(params.payload || {})}::jsonb
    FROM minds m
    WHERE m.id = ${params.mindId} AND m.tenant_id = ${params.tenantId}
    RETURNING *
  `);
  const rows = (res as any).rows || res;
...
export async function markEventsHandled(eventIds: number[], tenantId: number): Promise<number> {
  const normalizedEventIds = normalizeIntegerArray(eventIds);
  if (!normalizedEventIds.length) return 0;
  const res = await db.execute(sql`
    UPDATE mind_events SET handled = TRUE, handled_at = NOW()
    WHERE id = ANY(${normalizedEventIds}::int[]) AND tenant_id = ${tenantId}
  `);
...
4. Your assessment of whether the task is complete`;
  ...
  // R125+144 — AgentRadio passive awareness: per-mind radio channel. Workers
  // launched later see earlier siblings' discoveries at launch time (below),
  // and RUNNING workers receive fresh discoveries between tool rounds via the
  // radioChannel hook in chat-engine. Fail-open throughout.
  const radioChannel = `mind-${ticket.mind_id}-radio`;
  let launchRadioContext = "";
  try {
    const { fetchRadioMessages, mentionInjectionEnabled } = await import("./lib/mention-injection");
    if (mentionInjectionEnabled()) {
      // recent:true = latest N discoveries rendered chronologically — never the
      // oldest-six-forever trap on a long-lived per-mind channel.
      const recent = await fetchRadioMessages({ tenantId, channelName: radioChannel, afterId: 0, recent: true });
      if (recent.block) launchRadioContext = `\n\n## Teammate radio (untrusted peer updates)\n${recent.block}`;
    }
  } catch (err: any) {
    console.warn(`[minds] radio launch-context fetch failed (fail-open): ${err?.message}`);
  }

  const { launchAutonomousConversation } = await import("./agent-manager");
  const runResult = await launchAutonomousConversation({
    tenantId,
    task: taskPrompt + launchRadioContext,
  ...
  const checkInterval = setInterval(async () => {
    try {
      ...
      if (!run || run.status === "completed" || run.status === "failed" || run.status === "timeout") {
        ...
        // R125+144 — broadcast the discovery on the mind's radio channel so
        // sibling workers still running pick it up between tool rounds.
        try {
          const { postDiscovery } = await import("./lib/mention-injection");
          await postDiscovery({
            tenantId,
            channelName: `mind-${mindId}-radio`,
            content: `Ticket #${ticketId} finished (${run?.status || "unknown"}): ${resultText.slice(0, 800)}`,
            metadata: { ticketId, runId },
          });
        } catch (err: any) {
          console.warn(`[minds] radio postDiscovery failed (fail-open): ${err?.message}`);
        }
      }
    } catch (err: any) {
