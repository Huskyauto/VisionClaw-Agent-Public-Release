...
import { EXPERT_RESEARCH_PROGRAM_NAME } from "./research-expert-mode";
...
export async function seedNightlyAutoresearch() {
  ...
  await db.transaction(async (tx) => {
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext('seed-expert-research-program'), 1)`);
    const expertObjective = "Find lower-cost model and prompt configurations for VisionClaw without sacrificing answer quality. Every experiment must propose one exact CONFIG_JSON candidate. The runtime will screen it on a stratified benchmark subset, run the full frozen benchmark only when screening passes, and remove any custom system prompt for an ablation.";
    const expertConstraints = "Use only registered provider model IDs. Change the smallest possible number of configuration variables per experiment. Never modify the evaluator, benchmark, safety policy, authorization, or spend limits. Treat verified benchmark output as authoritative over model prose.";
    const expertMetrics = "USD per successful query; independent quality judge average; benchmark completion; prompt-ablation contribution";
    const expertExisting = await tx.execute(sql`
    SELECT id FROM research_programs
    WHERE tenant_id = 1 AND name = ${EXPERT_RESEARCH_PROGRAM_NAME}
    LIMIT 1
  `);
    const expertRows = (expertExisting as any).rows || expertExisting;
    let expertProgramId: number | undefined = expertRows[0]?.id;
    if (expertProgramId) {
      await tx.execute(sql`
      UPDATE research_programs
      SET objective = ${expertObjective},
          constraints = ${expertConstraints},
          metrics = ${expertMetrics},
          exploration_strategy = 'conservative',
          max_experiments_per_session = 2,
          eval_type = 'cost',
          is_active = true,
          updated_at = NOW()
      WHERE id = ${expertProgramId} AND tenant_id = 1
    `);
    } else {
      const expertInserted = await tx.execute(sql`
      INSERT INTO research_programs (
        tenant_id, persona_id, name, objective, constraints, metrics,
        exploration_strategy, model, max_experiments_per_session, eval_type, is_active
      )
      VALUES (
        1, 3, ${EXPERT_RESEARCH_PROGRAM_NAME}, ${expertObjective}, ${expertConstraints},
        ${expertMetrics}, 'conservative', 'gemini-3-flash-preview', 2, 'cost', true
      )
      RETURNING id
    `);
      const insertedRows = (expertInserted as any).rows || expertInserted;
      expertProgramId = insertedRows[0]?.id;
    }

    if (expertProgramId) {
      const expertScheduleName = "Research: Weekly Expert Evidence";
      const expertSchedule = await tx.execute(sql`
      SELECT id FROM research_schedules
      WHERE tenant_id = 1 AND name = ${expertScheduleName}
      LIMIT 1
    `);
      const expertScheduleRows = (expertSchedule as any).rows || expertSchedule;
      const nextRun = new Date();
      nextRun.setHours(3, 15, 0, 0);
      const daysUntilMonday = (1 - nextRun.getDay() + 7) % 7;
      nextRun.setDate(nextRun.getDate() + daysUntilMonday);
      if (nextRun.getTime() <= Date.now()) nextRun.setDate(nextRun.getDate() + 7);
      if (expertScheduleRows[0]?.id) {
        await tx.execute(sql`
        UPDATE research_schedules
        SET program_id = ${expertProgramId},
            cron_expression = '15 3 * * 1',
            timezone = 'America/Chicago',
            is_enabled = true
        WHERE id = ${expertScheduleRows[0].id} AND tenant_id = 1
      `);
      } else {
        await tx.execute(sql`
        INSERT INTO research_schedules (
          tenant_id, program_id, name, cron_expression, timezone, is_enabled, run_all, next_run_at
        )
        VALUES (
          1, ${expertProgramId}, ${expertScheduleName}, '15 3 * * 1',
          'America/Chicago', true, false, ${nextRun}
        )
      `);
      }
    }
