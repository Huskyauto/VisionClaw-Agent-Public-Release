...
import { getGmailDirectAccessToken, loadGmailDirectRefreshToken } from "./lib/gmail-direct-token";
import { encodeMimeSubject } from "./lib/outbound-email-preflight";
import {
  clearGoogleConnectorTokenCache,
  fetchGoogleApiJson,
  getGoogleConnectorToken,
  type GoogleApiFetchDeps,
  type GoogleApiRetryContext,
} from "./lib/google-connector-client";
...
} from "./google-slides-foundations";
import {
...
} from "./google-gmail-delivery-verification";
export {
  assertOwnerGmailConnectorProfile,
  assertOwnerGmailReportDestination,
...
const DIRECT_OWNER_REPORT_GMAIL_SCOPES = new Set([GMAIL_READONLY_SCOPE, GMAIL_SEND_SCOPE]);
export function clearGoogleTokenCache(connectorName?: string) {
  clearGoogleConnectorTokenCache(connectorName);
}

function gmailScopes(scope: string | undefined): Set<string> {
  return new Set(String(scope || "").split(/\s+/).filter(Boolean));
}

function hasOnlyDirectOwnerReportGmailScopes(scope: string | undefined): boolean {
  const scopes = gmailScopes(scope);
  return [...scopes]
    .filter((value) => value.startsWith("https://www.googleapis.com/auth/gmail.") || value === GMAIL_FULL_ACCESS_SCOPE)
    .every((value) => DIRECT_OWNER_REPORT_GMAIL_SCOPES.has(value));
}

function hasDirectGmailReadScope(scope: string | undefined): boolean {
  const scopes = gmailScopes(scope);
  return hasOnlyDirectOwnerReportGmailScopes(scope) && scopes.has(GMAIL_READONLY_SCOPE);
}

export function hasDirectGmailSendAndReadScopes(scope: string | undefined): boolean {
  const scopes = gmailScopes(scope);
  return hasOnlyDirectOwnerReportGmailScopes(scope)
    && scopes.has(GMAIL_READONLY_SCOPE)
    && scopes.has(GMAIL_SEND_SCOPE);
}

async function getGmailConnectorToken(tenantId: number): Promise<string> {
  const adminTenantId = Number(process.env.ADMIN_TENANT_ID) || 1;
  if (tenantId !== adminTenantId) {
    throw new Error(`Direct Gmail connector delivery is restricted to the admin tenant (${adminTenantId}).`);
  }
  const token = await getGoogleConnectorToken("google-mail", { required: true });
  if (!token) {
    throw new Error("Gmail connector is unavailable. Reconnect Google Gmail in Integrations.");
  }
  return token;
}
...
export async function getGoogleToken(tenantId: number, service?: string, op: "read" | "write" = "write"): Promise<string> {
  // The installed google-mail connector is the canonical admin Gmail
  // transport. Prefer it over the legacy direct OAuth grant so a stale
  // gmail-direct token cannot silently shadow a freshly reauthorized Gmail
  // connection.
  if (service === "gmail" && tenantId === (Number(process.env.ADMIN_TENANT_ID) || 1)) {
    const connectorToken = await getGoogleConnectorToken("google-mail");
  ...
  // Legacy direct Gmail OAuth remains a fallback for existing admin inbox
  // ingestion and custom OAuth deployments. Owner delivery below uses the
  // connector explicitly, so this fallback cannot shadow a reauthorized
  // google-mail connection.
  if (service === "gmail") {
    const directConfig = await loadGmailDirectRefreshToken();
    const hasRequiredScope = op === "read"
      ? hasDirectGmailReadScope(directConfig?.scope)
      : hasDirectGmailSendAndReadScopes(directConfig?.scope);
    if (hasRequiredScope) {
  ...
  if (connectorName) {
    const token = await getGoogleConnectorToken(connectorName);
...
export async function googleApiFetch(
  ...
  const getFreshToken = deps.getFreshToken || (retryContext
    ? () => retryContext.connectorName
      ? getGoogleConnectorToken(retryContext.connectorName, { required: true })
      : getGoogleToken(retryContext.tenantId, retryContext.service)
    : undefined);
  return fetchGoogleApiJson(token, url, init, retryContext, { ...deps, getFreshToken });
}

const gFetch = googleApiFetch;
...
export async function gmailSend(tenantId: number, to: string, subject: string, body: string, cc?: string, bcc?: string): Promise<any> {
  ...
  return sendGmailMessageWithToken(token, { to, subject, body, cc, bcc, contentType: "text/html" });
}

function mimeHeader(value: string, field: string): string {
  const sanitized = String(value || "").replace(/[\r\n]+/g, " ").trim();
  if (!sanitized) throw new Error(`Gmail ${field} cannot be empty`);
  return sanitized;
}

async function sendGmailMessageWithToken(
  token: string,
  params: {
    to: string;
    subject: string;
  ...
  retryContext?: { tenantId: number; service?: string; connectorName?: string },
): Promise<any> {
  let rawEmail =
    `To: ${mimeHeader(params.to, "recipient")}\n` +
    `Subject: ${encodeMimeSubject(mimeHeader(params.subject, "subject"))}\n` +
    `Content-Type: ${params.contentType}; charset=utf-8\nMIME-Version: 1.0\n`;
  if (params.cc) rawEmail += `Cc: ${mimeHeader(params.cc, "cc recipient")}\n`;
  if (params.bcc) rawEmail += `Bcc: ${mimeHeader(params.bcc, "bcc recipient")}\n`;
  if (params.messageIdHeader) rawEmail += `Message-ID: ${mimeHeader(params.messageIdHeader, "message ID")}\n`;
  rawEmail += `\n${params.body}`;
...
/**
 * Owner-report transport with proof stronger than a send response: the Gmail
 * account used for the report must show that exact message in its Inbox.
 *
 * This uses the reauthorized google-mail connector for both send and Inbox
 * verification. Keeping the transport on one connection avoids the previous
 * split-brain failure where connector send worked but a stale direct OAuth
 * grant blocked the owner-report path.
 */
export async function gmailSendAndVerifyInbox(
  tenantId: number,
  to: string,
  subject: string,
  text: string,
...
): Promise<{ messageId: string; threadId?: string; inboxVerified: boolean; verificationAttempts: number }> {
  const ownerRecipient = assertOwnerGmailReportDestination(tenantId, to);
  const token = await getGmailConnectorToken(tenantId);
  const retryContext = { tenantId, service: "gmail", connectorName: "google-mail" };
  ...
  assertOwnerGmailConnectorProfile(profile, ownerRecipient);

  const stableMessageKey = String(options.stableMessageKey || "").replace(/[^a-zA-Z0-9._-]/g, "");
  const messageIdHeader = stableMessageKey ? `<${stableMessageKey}@visionclaw.local>` : undefined;
  let sent: any = null;
  const messageId = await resolveExistingOrSentGmailMessageId({
    ...
    findByStableMessageKey: async () => {
      ...
      const params = new URLSearchParams({ q: `rfc822msgid:${messageIdHeader}`, maxResults: "1" });
      const listed = await gFetch(token, `${GOOGLE_API}/gmail/v1/users/me/messages?${params}`, undefined, retryContext);
      return typeof listed?.messages?.[0]?.id === "string" ? listed.messages[0].id : null;
    },
    send: async () => {
      sent = await sendGmailMessageWithToken(token, {
        to: ownerRecipient,
        subject,
        body: text,
      ...
      if (options.onMessageSent) await options.onMessageSent(id);
      return id;
    },
  });

  const verification = await verifyGmailInboxDelivery(
    (id) => gFetch(token, `${GOOGLE_API}/gmail/v1/users/me/messages/${encodeURIComponent(id)}?format=metadata&metadataHeaders=To`, undefined, retryContext),
    messageId,
    ownerRecipient,
  );
  return {
    ...
    inboxVerified: verification.verified,
    verificationAttempts: verification.attempts,
  };
}

/**
 * Recovery-only owner Gmail lookup. Unlike gmailSendAndVerifyInbox, this
 * function has no send callback and therefore cannot create a duplicate.
 */
export async function gmailFindAndVerifyInbox(
  tenantId: number,
  to: string,
  options: {
    existingMessageId?: string | null;
...
): Promise<{ messageId: string | null; inboxVerified: boolean; verificationAttempts: number }> {
  const ownerRecipient = assertOwnerGmailReportDestination(tenantId, to);
  const token = await getGmailConnectorToken(tenantId);
  const retryContext = { tenantId, service: "gmail", connectorName: "google-mail" };
  const profile = await gFetch(
  ...
  let messageId = String(options.existingMessageId || "").trim();
  if (!messageId) {
    const stableMessageKey = String(options.stableMessageKey || "").replace(/[^a-zA-Z0-9._-]/g, "");
    if (!stableMessageKey) {
      throw new Error("Gmail owner-report reconciliation requires a stored message ID or stable message key.");
    }
    const messageIdHeader = `<${stableMessageKey}@visionclaw.local>`;
    const params = new URLSearchParams({ q: `rfc822msgid:${messageIdHeader}`, maxResults: "1" });
    const listed = await gFetch(token, `${GOOGLE_API}/gmail/v1/users/me/messages?${params}`, undefined, retryContext);
    messageId = typeof listed?.messages?.[0]?.id === "string" ? listed.messages[0].id : "";
  }
  if (!messageId) {
    return { messageId: null, inboxVerified: false, verificationAttempts: 0 };
  }
...
export async function slidesCreate(tenantId: number, options: SlidesCreateOptions): Promise<any> {
  ...
  try {
    ...
    if (pdfResp.ok) {
      ...
      const pdfUpload = await uploadAndShare({
        ...
        tenantId,
        generatedArtifact: { artifactKind: "presentation_pdf", logicalName: `${safeTitle}.pdf`, sourceRunKey: `slides-export:${presentationId}:pdf` },
      });
      if (!pdfUpload.success) throw new Error(pdfUpload.error || "Presentation PDF durability verification failed");
      if (pdfUpload.success && pdfUpload.fileId) {
        pdfDriveUrl = pdfUpload.viewUrl || pdfDriveUrl;
    ...
    if (pptxResp.ok) {
      ...
      const safeTitle = options.title.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 80);
      const pptxUpload = await uploadAndShare({
        ...
        parentFolderId: options._projectDriveFolderId || undefined,
        tenantId,
        generatedArtifact: { artifactKind: "presentation_pptx", logicalName: `${safeTitle}.pptx`, sourceRunKey: `slides-export:${presentationId}:pptx` },
      });
      if (!pptxUpload.success) throw new Error(pptxUpload.error || "Presentation PPTX durability verification failed");
      if (pptxUpload.success && pptxUpload.fileId) {
        pptxDriveUrl = pptxUpload.viewUrl || pptxDriveUrl;
        console.log(`[slides] PPTX exported & uploaded to Drive: ${pptxDriveUrl}`);
      }
    }
  } catch (exportErr: any) {
    return { success: false, error: `Presentation export is not durable: ${exportErr.message?.slice(0, 200)}` };
  }
