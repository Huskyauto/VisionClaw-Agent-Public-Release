...
function headerValues(message: any, name: string): string[] {
  const headers = message?.payload?.headers;
  if (!Array.isArray(headers)) return [];
  return headers
    .filter((header: any) => String(header?.name || "").toLowerCase() === name.toLowerCase())
    .map((header: any) => String(header?.value || ""));
}
 ...
 * only API acceptance.
 */
export function isGmailInboxDeliveryConfirmed(message: any, expectedRecipient: string): boolean {
...
export async function getGoogleToken(tenantId: number, service?: string, op: "read" | "write" = "write"): Promise<string> {
  ...
  // Direct Gmail OAuth is used only when the stored consent covers the
  // requested operation. Existing read-only grants keep serving inbox reads;
  // sending remains on the connector until the owner upgrades consent to
...
async function sendGmailMessageWithToken(
  ...
  let rawEmail =
    ...
    `Subject: ${mimeHeader(params.subject, "subject")}\n` +
 ...
 *
 * This deliberately requires the direct OAuth token to carry both gmail.readonly
 * and gmail.send. The Replit connector can send, but its current grant cannot
 * read the Inbox, so it cannot provide this proof.
 */
export async function gmailSendAndVerifyInbox(
  ...
  const directConfig = await loadGmailDirectRefreshToken();
  if (String(directConfig?.email_address || "").trim().toLowerCase() !== ownerRecipient) {
    throw new Error("Gmail owner-report delivery is unverified: the direct OAuth token is not bound to the configured owner inbox.");
  }
  if (!hasDirectGmailSendAndReadScopes(directConfig?.scope)) {
    throw new Error(
      "Gmail owner-report delivery is unverified: direct OAuth must be re-authorized with gmail.readonly and gmail.send before this report can be sent.",
    );
  }
  const token = await getGmailDirectAccessToken();
  if (!token) {
    throw new Error("Gmail owner-report delivery is unverified: direct OAuth token is unavailable or expired.");
  }
  ...
  const messageId = await resolveExistingOrSentGmailMessageId({
    ...
    findByStableMessageKey: async () => {
      ...
      const listed = await gFetch(token, `${GOOGLE_API}/gmail/v1/users/me/messages?${params}`);
  ...
  const verification = await verifyGmailInboxDelivery(
    (id) => gFetch(token, `${GOOGLE_API}/gmail/v1/users/me/messages/${encodeURIComponent(id)}?format=metadata&metadataHeaders=To`),
    messageId,
    ownerRecipient,
  );
  return {
