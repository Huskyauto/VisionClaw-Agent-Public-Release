...
const VISIONCLAW_LOGO_URL = "https://lh3.googleusercontent.com/d/19n3MgI-qj4wN_4atXEAewZ-UmYgbRwaG";
...
export async function getGoogleToken(tenantId: number, service?: string): Promise<string> {
...
export async function slidesCreate(tenantId: number, options: SlidesCreateOptions): Promise<any> {
  ...
  deferredImageRequests.push({
    createImage: {
      objectId: qrImgId,
      url: "https://lh3.googleusercontent.com/d/1vYDgZ1Fkr23PXN7iZvN02i7JMRw0ALWZ",
      elementProperties: {
        pageObjectId: qrSid,
        ...
        for (let ti = 0; ti < pageIds.length; ti++) {
          ...
          try {
            ...
            if (imgBuf && imgBuf.length > 1000) {
              ...
              try { fs.writeFileSync(path.join(slideDir, fileName), imgBuf); } catch {}
