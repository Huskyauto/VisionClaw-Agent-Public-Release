...
export interface EvidenceDocketOptions {
  ...
  /** Per-section row cap (bounds the scan). Clamped to [1, 200]. Defaults to
   * the declared proof level's cap (L2 → 50, matching prior behavior). */
  maxRows?: number;
  /** Declared per-run proof level (L1–L5). ADVISORY: tunes docket strictness
   * (row caps + which missing evidence counts as "proof debt"). Never blocks
   * execution or weakens a safety gate. Default L2 = today's behavior. */
...
export interface EvidenceDocket {
  ...
  /** Declared per-run rigor (PROOF_LOOPS L1–L5) + the obligations it imposes. */
  proofLevel: {
    level: ProofLevel;
    name: string;
    memoryEffect: string;
  };
  /** Unmet proof obligations at the declared level. Non-empty ⇒ verdict REVIEW. */
  proofDebt: string[];
...
export function assembleDocket(
  ...
  // Declared per-run rigor (PROOF_LOOPS L1–L5). ADVISORY: at the declared
  // level, missing evidence becomes explicit "proof debt" line items and flips
  // the verdict to REVIEW. Default L2 imposes no obligations beyond the
  // pre-existing failure/block checks, so omitting the knob changes nothing.
  const policy = resolveProofLevelPolicy(opts.proofLevel ?? DEFAULT_PROOF_LEVEL);
  const proofDebt = computeProofDebt(policy, {
    verificationCount: verifications.length,
    failedVerificationCount: failed,
    hasReplayPointer: !!opts.runId,
    juryRowCount: jury.length,
  });
  ...
  return {
    ...
    proofLevel: {
      level: policy.level,
      name: policy.name,
      memoryEffect: policy.memoryEffect,
    },
    proofDebt,
    summary,
...
export function renderDocketMarkdown(d: EvidenceDocket): string {
  ...
  L.push(`## Proof level`);
  L.push(`**${d.proofLevel.level} — ${d.proofLevel.name}.** ${d.proofLevel.memoryEffect}`);
  if (d.proofDebt.length) {
    L.push("");
    L.push(`### Proof debt (unmet obligations at ${d.proofLevel.level})`);
    for (const p of d.proofDebt) L.push(`- ⚠ ${p}`);
  } else {
    L.push(`_All obligations at this level are satisfied._`);
  }
  L.push("");
...
<section><h2>Proof level</h2>
<p><span class="pill">${esc(d.proofLevel.level)}</span> <strong>${esc(d.proofLevel.name)}</strong> — ${esc(d.proofLevel.memoryEffect)}</p>
${d.proofDebt.length ? `<h3>Proof debt (unmet obligations at ${esc(d.proofLevel.level)})</h3><ul>${d.proofDebt.map((p) => `<li class="fail">${esc(p)}</li>`).join("")}</ul>` : `<p class="empty">All obligations at this level are satisfied.</p>`}
</section>
...
export async function gatherDocketData(opts: EvidenceDocketOptions): Promise<RawDocketData> {
  ...
  // Row cap: explicit maxRows wins; else the declared proof level's cap
  // (L2 default → 50, identical to prior behavior). Always clamped [1, 200].
  const levelCap = resolveProofLevelPolicy(opts.proofLevel ?? DEFAULT_PROOF_LEVEL).docketMaxRows;
  const maxRows = Math.min(Math.max(opts.maxRows ?? levelCap, 1), 200);
