...
export async function handleVoiceMessage(req: Request, res: Response) {
  ...
  try {
    ...
    // R125+143 — was `conv.tenantId ?? ADMIN_TENANT_ID` (silent admin-tenant write
    // fallback). The conversation was fetched tenant-scoped with the authenticated
    // tenant, so that IS the authoritative owner; a null/mismatched conv.tenantId is
    // a data anomaly worth a loud log, never a silent re-route to tenant 1.
    const voiceTenantId = authenticatedTenantId;
    if (conv.tenantId != null && conv.tenantId !== authenticatedTenantId) {
      console.error(`[voice] SECURITY: conversation ${conversationId} tenant_id=${conv.tenantId} != authenticated tenant ${authenticatedTenantId} — writing under authenticated tenant`);
    } else if (conv.tenantId == null) {
      console.error(`[voice] conversation ${conversationId} has NULL tenant_id (legacy row?) — writing transcript under authenticated tenant ${authenticatedTenantId}`);
    }
    ...
    const model = conv.model || DEFAULT_CHAT_MODEL_ID;
    ...
    storage.touchMemoryEntries(injectedMemoryIds, voiceTenantId).catch(() => {});
    ...
    // Architect MEDIUM closed (2026-08-01): pass the tenant so tenant provider
    // keys / subscription routing apply to voice, matching the chat path.
    const { client, actualModelId } = await getClientForModel(model, voiceTenantId);
    ...
    if (needsTitle) {
      try {
        const { client: titleClient, actualModelId: titleModel } =
          await getClientForModel(DEFAULT_CHAT_MODEL_ID, voiceTenantId);
...
export async function handleTextToSpeech(req: Request, res: Response) {
  ...
  try {
    if (streamed) {
      ...
      // Emit each chunk to the client as soon as it finishes synthesis, maintaining
      // order via a cursor. Previously the server awaited ALL chunks before sending
      // any — this meant the listener heard nothing until the last sentence was
      // synthesised. The pattern mirrors handleVoiceMessage's tryEmitInOrder.
      const readyMap = new Map<number, { buffer: Buffer; format: "mp3" | "pcm" } | null>();
      let nextEmit = 0;
      let completedCount = 0;
      let slotSemaphore = 0;

      let allDoneResolve: (() => void) | null = null;
      const allDone = new Promise<void>(r => { allDoneResolve = r; });

      function tryEmitInOrder() {
        while (readyMap.has(nextEmit)) {
          const result = readyMap.get(nextEmit)!;
          readyMap.delete(nextEmit);
          if (result) {
            res.write(`data: ${JSON.stringify({ type: "voice_audio_chunk", index: nextEmit, format: result.format })}\n\n`);
            sendAudioSSE(res, result.buffer, result.format);
            res.write(`data: ${JSON.stringify({ type: "voice_chunk_end", index: nextEmit })}\n\n`);
          }
          nextEmit++;
        }
        // Resolve once every chunk is synthesised and emitted (including failed/null ones).
        if (completedCount === chunks.length && nextEmit === chunks.length) {
          allDoneResolve?.();
        }
      }

      const pendingTTS = chunks.map((chunkText, idx) =>
        (async () => {
          ...
          try {
            ...
            readyMap.set(idx, result ? { buffer: result.buffer, format: result.format } : null);
          } finally {
            slotSemaphore--;
            completedCount++;
            tryEmitInOrder();
