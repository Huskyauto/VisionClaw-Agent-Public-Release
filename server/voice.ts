...
async function ttsElevenLabs(text: string, voiceId?: string): Promise<{ buffer: Buffer; format: "pcm" }> {
  ...
  const response = await fetch(`${ELEVENLABS_BASE}/v1/text-to-speech/${effectiveVoiceId}`, {
    method: "POST",
    headers: {
      "xi-api-key": getElevenLabsKey(),
...
async function synthesizeSpeech(text: string, voiceId?: string): Promise<{ buffer: Buffer; format: "mp3" | "pcm"; usedProvider: TTSProvider }> {
  ...
  for (const p of fallbackOrder) {
    ...
    try {
      switch (p) {
        case "openai":
          return { ...(await ttsOpenAI(text)), usedProvider: "openai" };
        case "elevenlabs":
...
export async function handleVoiceMessage(req: Request, res: Response) {
  ...
  try {
    ...
    const voiceTenantId = conv.tenantId ?? 1;
    ...
    const { prompt: systemPrompt, injectedMemoryIds } = await buildSystemPrompt(
      persona, memResult.data, settings, enabledSkills, knResult.data, false, "off", transcript
    );
    ...
    if (needsTitle) {
      try {
        ...
        storage.updateConversation(conversationId, { title: transcript.slice(0, 50) }).catch(() => {});
      }
    }
