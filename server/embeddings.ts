...
export async function storeEmbeddingVec(
  ...
  id: number,
  embedding: number[],
  options?: { expectedKnowledgeContent?: string },
): Promise<boolean> {
  ...
  if (table === "agent_knowledge" && expectedKnowledgeContent !== undefined) {
    const result = await db.execute(sql`
      UPDATE agent_knowledge
         SET embedding_vec = ${sql.raw(`'${vec}'::vector`)},
             embedding = ${JSON.stringify(embedding)}::jsonb
       WHERE id = ${id}
         AND LEFT(title || ' ' || content, 6000) = ${expectedKnowledgeContent}
      RETURNING id
    `);
