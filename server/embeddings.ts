...
async function bm25SearchKnowledge(
  ...
  const rows = await db.execute(sql`
    WITH q AS (SELECT websearch_to_tsquery('english', ${query}) AS tsq)
    SELECT k.id, k.title, k.content, k.category, k.priority, k.access_count,
...
export async function vectorSearchKnowledge(
  ...
  const [bm25Results, vectorResults] = await Promise.all([
    ...
    (async () => {
      ...
      const vec = vecLiteral(queryEmbedding);
      const rows = await db.execute(sql`
        SELECT id, title, content, category, priority, access_count,
               1 - (embedding_vec <=> ${sql.raw(`'${vec}'::vector`)}) AS similarity
  ...
  const catalog = new Map<number, { id: number; title: string; content: string; category: string; priority: number; accessCount: number; inVec: boolean; inBm: boolean }>();
  for (const r of vectorResults) {
    catalog.set(r.id, { id: r.id, title: r.title, content: r.content, category: r.category, priority: r.priority, accessCount: (r as any).accessCount ?? 0, inVec: true, inBm: false });
  }
  for (const r of bm25Results) {
    const prev = catalog.get(r.id);
    if (prev) { prev.inBm = true; prev.accessCount = Math.max(prev.accessCount, (r as any).accessCount ?? 0); }
    else catalog.set(r.id, { id: r.id, title: r.title, content: r.content, category: r.category, priority: r.priority, accessCount: (r as any).accessCount ?? 0, inVec: false, inBm: true });
  }
