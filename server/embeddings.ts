...
export async function generateEmbedding(
  ...
  try {
    const cleaned = normalizeEmbeddingInput(text);
    if (!cleaned) return null;

    const cacheKey = embeddingCacheKey(cleaned);
...
async function bm25SearchKnowledge(
  query: string,
  opts: { personaId?: number; tenantId: number; topK?: number; includeGlobal?: boolean },
): Promise<{ id: number; title: string; content: string; category: string; source: string; priority: number; rank: number }[]> {
  const { personaId, tenantId, topK = 10, includeGlobal = false } = opts;
  const personaFilter = personaId != null
    ? (includeGlobal
        ? sql`AND (persona_id = ${personaId} OR persona_id IS NULL)`
        : sql`AND persona_id = ${personaId}`)
    : sql``;
  ...
  const rows = await db.execute(sql`
    WITH q AS (SELECT websearch_to_tsquery('english', ${query}) AS tsq)
    SELECT k.id, k.title, k.content, k.category, k.source, k.priority, k.access_count,
...
export async function vectorSearchKnowledge(
  query: string,
  opts: { personaId?: number; tenantId: number; topK?: number; threshold?: number; includeGlobal?: boolean },
): Promise<{ id: number; title: string; content: string; category: string; source: string; similarity: number; priority?: number; retrieval?: "vector" | "bm25" | "hybrid" }[]> {
  ...
  const { personaId, tenantId, topK = 10, threshold = 0.3, includeGlobal = false } = opts;
  const oversample = Math.max(topK * 3, 15);
  ...
  // Error≠empty (architect 2026-08-11): a broken tier must not masquerade as
  // "no results" — track errors so the fallback path can log a LOUD degraded
  // signal (retrieval stays fail-open; the signal is for operators).
  let bm25Error: string | null = null;
  let vectorError: string | null = null;
  const [bm25Results, vectorResults] = await Promise.all([
    bm25SearchKnowledge(query, { personaId, tenantId, topK: oversample, includeGlobal }).catch((e) => {
      bm25Error = e?.message ?? String(e);
    ...
    (async () => {
      ...
      const vec = vecLiteral(queryEmbedding);
      const rows = await db.execute(sql`
        SELECT id, title, content, category, source, priority, access_count,
               1 - (embedding_vec <=> ${sql.raw(`'${vec}'::vector`)}) AS similarity
      ...
      return [] as { id: number; title: string; content: string; category: string; source: string; priority: number; similarity: number }[];
    }),
  ]);

  // If both retrievers struck out, fall back to the JS keyword tier so callers
  // still get *something* back (preserves prior behavior). Fallback results are
  // genuine returned knowledge — bump their usage too.
  if (bm25Results.length === 0 && vectorResults.length === 0) {
    if (bm25Error || vectorError) {
      // Degraded, not empty — say so loudly (once per call). A permanently
      // missing column (42703 "access_count does not exist") lands here.
      console.error(
        `[hybrid] DEGRADED knowledge retrieval — falling back to JS keyword tier. ` +
        `bm25: ${bm25Error ?? "empty"}; vector: ${vectorError ?? "empty"}`,
      );
    }
    const fallback = await keywordSearchKnowledge(query, opts);
    bumpKnowledgeUsage(tenantId, fallback.map((r) => r.id));
    return fallback;
  }

  // Build the catalog of unique candidates and compute RRF.
  const catalog = new Map<number, { id: number; title: string; content: string; category: string; source: string; priority: number; accessCount: number; inVec: boolean; inBm: boolean }>();
  for (const r of vectorResults) {
    catalog.set(r.id, { id: r.id, title: r.title, content: r.content, category: r.category, source: r.source, priority: r.priority, accessCount: (r as any).accessCount ?? 0, inVec: true, inBm: false });
  }
  for (const r of bm25Results) {
    const prev = catalog.get(r.id);
    if (prev) { prev.inBm = true; prev.accessCount = Math.max(prev.accessCount, (r as any).accessCount ?? 0); }
    else catalog.set(r.id, { id: r.id, title: r.title, content: r.content, category: r.category, source: r.source, priority: r.priority, accessCount: (r as any).accessCount ?? 0, inVec: false, inBm: true });
  }

  const fused = rrfMerge<{ id: number }>([vectorResults, bm25Results]);
  // Usage-aware TIE-BREAKER (2026-08-11, policy per architect review): relevance
  // (RRF score) ALWAYS dominates; access frequency only decides between rows with
  // an identical fused score (common under RRF when rows share rank patterns).
  // A multiplier — even a capped one — can reorder adjacent RRF ranks (~1-2%
  // apart), so frequency is deliberately NOT blended into the score.
  // Note: later stages (cohereRerank / lost-in-the-middle reorder) may reorder
  // rows by RELEVANCE — that is fine; the invariant is "usage never boosts
  // relevance", not "final order preserves usage tie-breaks".
  const fusedSorted = Array.from(catalog.values())
    .map((row) => ({ ...row, score: fused.get(row.id) ?? 0 }))
    .sort((a, b) => (b.score - a.score) || (b.accessCount - a.accessCount));
  ...
  // Usage instrumentation: fire-and-forget bump for the rows actually returned.
  bumpKnowledgeUsage(tenantId, ranked.map((r: any) => r.id));
...
/**
 * Central usage marker for agent_knowledge rows that were actually RETURNED to
 * a caller (2026-08-11). Fire-and-forget: never blocks or fails the search
 * path. Tenant-scoped as defense-in-depth (candidate ids already come from
 * tenant-filtered queries). Call this from every path that hands knowledge
 * rows to an agent — hybrid, fallback, and keyword tiers alike — so the
 * frequency signal covers all retrievals, not just the happy path.
 */
export function bumpKnowledgeUsage(tenantId: number, ids: number[]): void {
  const clean = ids.filter((n) => Number.isSafeInteger(n) && n > 0);
  if (!tenantId || clean.length === 0) return;
  void db.execute(sql`
    UPDATE agent_knowledge
    SET access_count = access_count + 1, last_accessed = NOW()
    WHERE id IN (${sql.join(clean.map((n) => sql`${n}`), sql`, `)})
      AND tenant_id = ${tenantId}
  `).catch((e) => {
    // 42703 = undefined column, 42P01 = undefined table: permanent schema drift
    // (this deployment never got the boot DDL) — error-level so it's an operator
    // signal, not an infinitely-repeating warn (architect finding 2026-08-11).
    const code = e?.code ?? e?.cause?.code;
    if (code === "42703" || code === "42P01") {
      if (!_bumpSchemaErrorLogged) {
        _bumpSchemaErrorLogged = true;
        console.error(`[bumpKnowledgeUsage] ❌ PERMANENT schema error (${code}): ${e?.message} — agent_knowledge usage columns missing on this deployment; run the seed boot DDL.`);
      }
    } else {
      console.warn(`[bumpKnowledgeUsage] non-fatal: ${e?.message}`);
    }
  });
}
let _bumpSchemaErrorLogged = false;
...
async function keywordSearchKnowledge(
  query: string,
  opts: { personaId?: number; tenantId?: number; topK?: number; threshold?: number } = {},
): Promise<{ id: number; title: string; content: string; category: string; source: string; similarity: number }[]> {
  ...
  return knowledge.data
    .map((k) => ({ id: k.id, title: k.title, content: k.content, category: k.category, source: k.source, similarity: keywordSimilarity(query, `${k.title} ${k.content}`) }))
...
export async function storeEmbeddingVec(
  table: "memory_entries" | "agent_knowledge",
  id: number,
  embedding: number[],
  options?: { tenantId?: number; expectedKnowledgeContent?: string },
): Promise<boolean> {
  if (!_pgvectorReady) return false;
  ...
  const expectedKnowledgeContent = options?.expectedKnowledgeContent;
  if (table === "agent_knowledge") {
    const tenantId = options?.tenantId;
    if (!Number.isInteger(tenantId) || Number(tenantId) <= 0) {
      throw new Error("storeEmbeddingVec: positive tenantId required for agent_knowledge");
    }
    const result = await db.execute(sql`
      UPDATE agent_knowledge
         SET embedding_vec = ${sql.raw(`'${vec}'::vector`)},
             embedding = ${JSON.stringify(embedding)}::jsonb
       WHERE id = ${id}
         AND tenant_id = ${tenantId}
         ${expectedKnowledgeContent !== undefined
           ? sql`AND LEFT(title || ' ' || content, 6000) = ${expectedKnowledgeContent}`
           : sql``}
      RETURNING id
    `);
  ...
  if (tenantId !== undefined) {
    if (!Number.isInteger(tenantId) || Number(tenantId) <= 0) {
      throw new Error("storeEmbeddingVec: positive tenantId required for memory_entries");
    }
    const result = await db.execute(sql`
...
async function backfillMissingKnowledgeEmbeddings(): Promise<void> {
  const rows = await db.execute(sql`
    SELECT id, tenant_id, title, content FROM agent_knowledge
    ...
    try {
      ...
      if (emb) {
        await storeEmbeddingVec("agent_knowledge", entry.id, emb, {
          tenantId: Number(entry.tenant_id),
          expectedKnowledgeContent: text,
        });
        count++;
      }
