...
export const MAINTENANCE_SCRIPTS: Record<string, { bin: string; args: string[]; timeoutMs: number; label: string }> = {
  ...
  "wedge-lifecycle":    { bin: "tsx", args: ["scripts/wedge-lifecycle-update.ts"], timeoutMs: 5 * 60 * 1000, label: "Wedge Lifecycle Update" },
  "plan-prewarm":       { bin: "tsx", args: ["scripts/plan-prewarm.ts"],           timeoutMs: 5 * 60 * 1000, label: "Plan Pre-Warm" },
};
...
export async function seedSystemHeartbeatTasks(): Promise<void> {
  try {
    ...
    // Task 160 (Bob 2026-08-07): DEV-workspace-only IdeaBrowser folder sync.
    // Prod is the pipeline's canonical home, but dev and prod DBs are separate,
    // so the workspace app's Projects → IdeaBrowser folder froze (Jul 20 → Aug 7)
    // until a manual backfill. This daily dev-side task re-runs the idempotent
    // ingest+score against the dev DB and mirrors prod's Weekly Run cards from
    // the weekly-scenario delivery email (see server/lib/ideabrowser-dev-sync.ts).
    // Seeded DEV-only; the prod deploy skips the type entirely (heartbeat gates).
    const hasDevSync = allTasksForSeed.some(t => t.type === "ideabrowser_dev_sync");
    if (!isProductionRuntime() && !hasDevSync) {
      await storage.createHeartbeatTask({
        name: "IdeaBrowser Dev Folder Sync (daily)",
        description: "Dev-workspace-only daily sync so the app's IdeaBrowser folder never freezes: ingests new Idea-of-the-Day emails into the DEV DB (dedupe on Gmail message id), scores unscored ideas (tier:* badges), and mirrors prod 'Weekly Run' cards from the weekly-scenario delivery email (dedupe on run name). Gmail token failures are logged loudly and skipped — never silent.",
        type: "ideabrowser_dev_sync",
        cronExpression: "0 12 * * *",
        enabled: true,
        promptContent: "Run the dev-side IdeaBrowser ingest + score + weekly-card email sync.",
        model: "none",
        personaId: null,
        createdBy: "system",
        runOnce: false,
      ...
      console.log("[heartbeat] Seeded ideabrowser_dev_sync task (daily 12:00 UTC) — dev-workspace-only");
    }
    ...
    const MAINT_SEED: Array<{ key: string; name: string; cron: string; desc: string }> = [
      ...
      { key: "wedge-lifecycle",    name: "Wedge Lifecycle Update (Mon)",  cron: "45 12 * * 1", desc: "Weekly wedge-card refresh — reads every wedge project's real signals (waitlist, inbox, content, paid orders), moves stage tags through validation/traction/scale/stalled/parked, updates active/on-hold/archived status, and posts a dated progress note visible on the Projects card. $0, no LLM." },
      ...
      { key: "plan-prewarm",       name: "Plan Pre-Warm (nightly)",       cron: "40 8 * * *", desc: "Sleep-time compute (agentic-reasoning survey, 2026-08): nightly refresh of proven near-expiry plan-replay cache entries (hit_count ≥ 2, day 23–30 of the 30-day TTL) so recurring requests keep skipping the paid planner LLM call. $0, no LLM — copies the row in-SQL with a fresh created_at; refreshed copies restart at 0 hits so unused plans still age out." },
    ];
    ...
    if (isProductionRuntime()) for (const m of MAINT_SEED) {
      ...
      if (!existing) {
        ...
        await storage.updateHeartbeatTask(existing.id, { enabled: true, description: restoredDesc || m.desc, nextRunAt: new Date() } as any, ADMIN_TENANT_ID);
        ...
        if (nextAt > Date.now() + 60 * 60 * 1000) {
          await storage.updateHeartbeatTask(existing.id, { nextRunAt: new Date() } as any, ADMIN_TENANT_ID);
    ...
    if (!hasTokenAudit) {
      await storage.createHeartbeatTask({
        ...
        enabled: true,
        promptContent: "Run a read-only token usage audit: 1) list heartbeat_tasks whose model is not on an approved zero-marginal lane (Profundo Luna/Sol, Replit gpt-5.4/gpt-5, Openference, or a verified OpenRouter :free model) or whose cron runs hourly or more often; 2) summarize the last 30 days of agent_cost_ledger spend grouped by model, and compare to the prior 30 days; 3) email the owner a short ranked summary of any token-waste findings with recommended (not applied) fixes. Make NO configuration changes.",
        model: "gpt-5.4",
        personaId: null,
        createdBy: "system",
        runOnce: false,
