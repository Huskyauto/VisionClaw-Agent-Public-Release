...
export async function seedSystemHeartbeatTasks(): Promise<void> {
  try {
    ...
    const MAINT_SEED: Array<{ key: string; name: string; cron: string; desc: string }> = [
      ...
      { key: "wedge-lifecycle",    name: "Wedge Lifecycle Update (Mon)",  cron: "45 12 * * 1", desc: "Weekly wedge-card refresh — reads each wedge project's real signals (waitlist signups, content shipped, paid orders), advances/demotes the stage:* tag (validation→traction on signal, →stalled after 21 quiet days, recovery on new signal), and appends a dated progress note to the card so the Projects UI stops showing frozen day-one snapshots. $0, no LLM. Bob 2026-08-08." },
