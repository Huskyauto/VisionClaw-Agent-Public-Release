...
const HUMAN_AGENCY_SKUS = new Set(['sample-agency-audit-starter', 'sample-agency-audit-full']);
const productIsHumanAgencySku = (sku: string) => HUMAN_AGENCY_SKUS.has(String(sku));

export function sanitizeFilePath(rawPath: string | undefined, fileName: string): string {
  const candidate = rawPath || `uploads/${fileName}`;
  const resolved = path.resolve(process.cwd(), candidate);
  // Separator-safe containment: a bare startsWith(ALLOWED_UPLOAD_DIR) also
  // matches sibling dirs like uploads_evil/ (prefix confusion). path.relative
  // is the authoritative check — anything escaping the root yields '..' or an
  // absolute path; '' means the root dir itself (not a file, block too).
  const rel = path.relative(ALLOWED_UPLOAD_DIR, resolved);
  ...
  // Return the normalized in-root path (never the raw candidate) so downstream
  // consumers (delivery-pipeline, drive upload) only ever see uploads/<rel>.
  return path.join('uploads', rel);
}

const pendingDeliveries = new Set<string>();

export async function completeDelayedResearchReport(params: {
  tenantId: number;
  context: {
    sessionId: string;
    sku: string;
  ...
  fulfillment: import('./research-report-fulfillment').FulfillmentResult;
}): Promise<void> {
  const { fulfillment, context, intake } = params;
  if (!fulfillment.success || !fulfillment.filePath || !fulfillment.fileName) {
    throw new Error(`Delayed report fulfillment is not reviewable: ${fulfillment.error || 'missing PDF'}`);
  }
  const {
    addReviewItem,
    runQualityChecks,
    isAutoShipEligible,
    autoDisableForBrokenShip,
    updateReviewItem,
  } = await import('./service-review-queue');
  const qa = runQualityChecks({
    filePath: fulfillment.filePath,
    pageCount: fulfillment.pages,
    depth: intake.depth === 'deep' ? 'deep' : 'standard',
  ...
  const eligibility = isAutoShipEligible(context.sku);
  if (eligibility.eligible && qa.passed) {
    try {
      const result = await deliverDigitalProduct({
        ...
        stripePaymentId: context.paymentKey,
        sendEmail: true,
        missionId: context.missionId,
        metadata: { sku: context.sku, reviewItemId: queued.id, source: 'auto-ship' },
      });
      if (!result.success) throw new Error(result.error || 'unknown delivery failure');
      await enrollPostPurchaseSequence({
        sku: context.sku,
        customerEmail: context.customerEmail,
        customerName: context.customerName,
        productName: context.productName,
      ...
      if (result.linkVerified === false) {
        await autoDisableForBrokenShip(context.sku, result.deliveryId);
        throw new Error('delivery link verification failed');
      }
      return;
    } catch (error: any) {
      await updateReviewItem(queued.id, {
        status: 'failed',
        reviewedAt: new Date().toISOString(),
        rejectedReason: `Auto-ship delivery failed: ${error?.message || String(error)}`,
      });
      await autoDisableForBrokenShip(context.sku);
      throw error;
    }
  }
  ...
  await sendCustomerHoldingNotice({
    customerEmail: context.customerEmail,
    customerName: context.customerName,
    productName: context.productName,
    intake,
...
export async function dispatchVerifiedStripeEvent(
  sync: { processEvent(event: unknown): Promise<unknown> },
  event: unknown,
): Promise<void> {
...
export class WebhookHandlers {
  static async processWebhook(payload: Buffer, signature: string): Promise<void> {
    ...
    let claim: ClaimResult = { status: 'fresh', claimToken: null };
    if (event?.id) {
    ...
    await runWithWebhookClaimLease('stripe', event?.id || '', claim.claimToken, async (assertOwnership) => {
    ...
    if (event?.id) {
      await markWebhookEventCompleted('stripe', event.id, claim.claimToken);
    }
  }
  ...
  static async handleDeliveryEvents(event: any): Promise<void> {
    if (
      ...
      // Fail CLOSED on missing payment_status (72h-review round 3 MEDIUM):
      // a session with no payment_status is NOT proof of payment — defer.
      const paid =
  ...
  static async handleCheckoutCompleted(session: any): Promise<void> {
    try {
      ...
      // Public storefront buyers are consumers, not platform tenants. Derive
      // the delivery tenant from server-owned platform configuration, never
      // from app-writable Stripe metadata.
      const { ownerTenantId: resolveOwnerTenant } = await import('./agentic/autonomous-budget');
      const deliveryTenantId = resolveOwnerTenant();
    ...
    try {
      const existing = await getDeliveryByStripePayment(paymentKey, deliveryTenantId);
      if (existing) {
      ...
      if (bundleSku) {
        if (productIsHumanAgencySku(bundleSku) && process.env.AI_TASK_FIT_AUDIT_ENABLED !== '1') {
          console.warn(`[stripe-delivery] Human-agency audit feature disabled; refusing session ${session.id}`);
          return;
        }
        ...
        if (product.kind === 'service' && product.serviceType === 'human-agency-audit') {
          // Human-agency audits are deliberately manual-first: payment creates
          // one pending queue item, never a generated draft, autonomous report,
          // notification-driven delivery, or auto-ship.
          ...
          for (const field of product.intakeFields || []) {
            const value = metadata[`intake_${field.key}`];
            if (typeof value === 'string' && value.trim()) {
              intake[field.key] = value.trim().slice(0, Math.min(field.maxLength || 500, 500));
            }
          }
          const queued = await addReviewItem({
            ...
            customerName: String(customerName || customerEmail || 'Customer').slice(0, 200),
            intake,
            filePath: '',
            fileName: '',
            qa: { passed: false, issues: ['Final PDF must be attached by an operator before approval.'] },
            status: 'pending',
            payment: `Stripe ${String(session.amount_total || '')}`,
            durable: true,
          });
          console.log(`[stripe-delivery] Human-agency audit queued for manual fulfillment: ${queued.id}`);
          return;
        }
        ...
        if (product.kind === 'service' && (product.serviceType === 'research-report' || product.serviceType === 'readiness-audit')) {
          ...
          if (isAudit) {
            ...
            intake = {
              ...
              crm: metadata.intake_crm || undefined,
          ...
          try {
            ...
            const PLATFORM_OWNER_TENANT_ID = deliveryTenantId;
            // Task #153 — fulfillment cost attribution. If this checkout was
            // launched by a revenue mission (mission_id in Stripe metadata),
            // run generation inside the mission-cost ALS scope so every LLM
            // call recordCost fires lands in agent_cost_ledger with mission_id
            // and counts against the mission's contribution margin.
            const { missionIdFromStripeMetadata: missionIdFromMeta, getMission: getMissionForAttribution, STRIPE_EVIDENCE_STAGES: ATTRIBUTION_STAGES } = await import('./lib/revenue-missions');
            // Provenance guard (architect, post-72h review): Stripe metadata is
            // app-writable — an arbitrary mission_id must NOT be able to attach
            // cost attribution / review-queue linkage to a mission it doesn't
            // own. Mirror recordMissionEvidence: the mission must exist in the
            // owner tenant AND be in a post-approval (live) stage; otherwise
            // attribution is dropped (fulfillment itself is unaffected).
            let fulfillmentMissionId = missionIdFromMeta(metadata);
            if (fulfillmentMissionId != null) {
              const attributionMission = await getMissionForAttribution(PLATFORM_OWNER_TENANT_ID, fulfillmentMissionId);
              if (!attributionMission || !(ATTRIBUTION_STAGES as readonly string[]).includes(attributionMission.stage)) {
                console.warn(`[stripe-delivery] metadata mission_id=${fulfillmentMissionId} ${!attributionMission ? 'not found for owner tenant' : `in stage '${attributionMission.stage}' (not post-approval)`} — dropping cost attribution`);
                fulfillmentMissionId = null;
              }
            }
            const attributed = async <T>(fn: () => Promise<T>): Promise<T> => {
              if (fulfillmentMissionId == null) return fn();
              const { withMissionCostAttribution } = await import('./agentic/cost-ledger');
              return withMissionCostAttribution(fulfillmentMissionId, fn);
            };
            ...
            if (isAudit) {
              ...
              fulfillment = await attributed(() => fulfillReadinessAudit({
                intake,
                customerEmail,
                orderId: session.id,
              ...
              fulfillment = await attributed(() => fulfillResearchReport({
                intake: {
                  ...intake,
                  delayTolerant: process.env.ASTRA_BATCH_ENABLED === '1',
                } as any,
                customerEmail,
                orderId: session.id,
                tenantId: PLATFORM_OWNER_TENANT_ID,
                deliveryContext: {
                  ...
                  paymentKey,
                  missionId: fulfillmentMissionId ?? undefined,
                },
              }));
            }
            if (fulfillment.pending && fulfillment.jobId) {
              console.log(`[stripe-delivery] Service fulfillment queued durably for ${session.id} as agent_job #${fulfillment.jobId}`);
              return;
            }
            if (!fulfillment.success || !fulfillment.filePath || !fulfillment.fileName) {
            ...
            // DFY Fix Kit: done-for-you audit orders also get the generated
            // remediation package (llms.txt, JSON-LD, meta tags, robots rules,
            // punch-list README) attached to the SAME review item. Fail-soft:
            // a kit failure never blocks the audit PDF — it lands in qa.issues
            // so the order holds for Bob's manual review instead.
            let fixKitFiles: { fileName: string; filePath: string }[] | undefined;
            const auditTierMeta = String(metadata.tier || '');
            if (isAudit && auditTierMeta === 'done-for-you') {
              try {
                const website = String(intake.website || '').trim();
                if (!website) {
                  qa.issues.push('Fix Kit skipped: no website in intake — generate manually via the admin audit page once the customer replies with their URL.');
                  qa.passed = false;
                } else {
                  const { generateAuditFixKit } = await import('./audit-fix-kit');
                  const kit = await attributed(() => generateAuditFixKit({
                    website,
                    company: String(intake.company || 'Customer'),
                    industry: intake.industry ? String(intake.industry) : undefined,
                    tenantId: PLATFORM_OWNER_TENANT_ID,
                    orderId: session.id,
                  }));
                  if (kit.success && kit.filePath && kit.fileName) {
                    fixKitFiles = [{ fileName: kit.fileName, filePath: kit.filePath }];
                    if (kit.issues.length > 0) {
                      qa.issues.push(...kit.issues.map((i) => `Fix Kit: ${i}`));
                      qa.passed = false; // degraded kit ⇒ always hold for Bob
                    }
                  } else {
                    qa.issues.push(`Fix Kit generation FAILED: ${kit.error || 'unknown'} — DFY order held for manual remediation.`);
                    qa.passed = false;
                  }
                }
              } catch (kitErr: any) {
                qa.issues.push(`Fix Kit generation threw: ${kitErr?.message?.slice(0, 200) || 'unknown'} — DFY order held for manual remediation.`);
                qa.passed = false;
              }
            }

            // Decide: auto-ship (graduated SKU + QA passed) or hold for review.
            // Done-for-you audit orders are HARD manual-only: the $1,997 tier
            // always goes through Bob's review queue regardless of SKU
            // auto-ship graduation or QA outcome (architect HIGH, 2026-07-27).
            const isDfyManualOnly = isAudit && auditTierMeta === 'done-for-you';
            const eligibility = isAutoShipEligible(bundleSku);
            const canAuto = eligibility.eligible && qa.passed && !isDfyManualOnly;
            ...
            const queued = await addReviewItem({
              ...
              modelUsed: fulfillment.modelUsed,
              pages: fulfillment.pages,
              additionalFiles: fixKitFiles,
              tier: auditTierMeta || undefined,
              missionId: fulfillmentMissionId ?? undefined,
            });
            ...
            if (canAuto) {
              ...
              try {
                const result = await deliverDigitalProduct({
                  ...
                  sendEmail: true,
                  additionalFiles: fixKitFiles,
                  // Task #153 — delivery runs inside the mission-cost scope too.
                  missionId: fulfillmentMissionId ?? undefined,
                  metadata: { sku: bundleSku, reviewItemId: queued.id, source: 'auto-ship' },
                  // Self-serve $497 audit → upsell the $1,997 done-for-you tier.
                  upsell: isAudit
                    ? (await import('./lib/audit-upsell')).auditDfyUpsell({ sku: bundleSku, productName, tierHint: auditTierMeta })
                    : undefined,
                });
                if (result.success) {
  ...
  static async recordMissionEvidence(kind: 'payment' | 'refund', obj: any): Promise<void> {
    ...
    for (const item of items) {
      ...
      if (ev) recordedAny = true;
    }
    void recordedAny; // retained for logging context; side effects below are
    // individually idempotent (never gated on it), so a transient fee/wake
    // failure on the FIRST delivery is retried on Stripe's replay instead of
    // being permanently lost behind a blanket "all duplicates" early-return.
    // ── Payment-fee capture (CPT 5.6 priority 4) — record the Stripe fee from
    // the charge's balance transaction as mission evidence so contribution
    // margin is honest. Best-effort: a fee-lookup failure NEVER blocks/poisons
    // the webhook (payment evidence is already recorded; Stripe gets its 200).
    // Type 'other' deliberately — 'spend' would distort the budget-cap and
    // auto-pause semantics that gate on cash-at-risk.
    if (kind === 'payment') {
      try {
        await WebhookHandlers.recordMissionPaymentFee(tenantId, missionId, obj);
      } catch (e) {
        console.warn('[mission-evidence] payment-fee capture failed (payment already recorded, non-fatal):', (e as any)?.message ?? e);
      }
    }
    ...
    if (kind === 'payment') {
      try {
        await WebhookHandlers.applyMissionAutonomyHooks(tenantId, missionId, typeof obj?.id === 'string' ? obj.id : undefined);
      } catch (e) {
  ...
  /**
   * Fetch the balance transaction behind a paid payment_intent and record its
   * Stripe fee as mission evidence (type 'other', raw.kind='payment_fee',
   * deduped on the txn_ id via the evidence unique index). Fail closed at the
   * parse level (no evidence beats corrupted counters); callers treat any
   * throw as best-effort.
   */
  static async recordMissionPaymentFee(tenantId: number, missionId: number, paymentIntent: any): Promise<void> {
    const chargeId = typeof paymentIntent?.latest_charge === 'string'
      ? paymentIntent.latest_charge
      : (typeof paymentIntent?.latest_charge?.id === 'string' ? paymentIntent.latest_charge.id : null);
    if (!chargeId) {
      console.warn(`[mission-evidence] payment ${paymentIntent?.id ?? '?'} has no latest_charge — fee not recorded`);
      return;
    }
    // Inline balance_transaction object (rare) or retrieve by id. The Stripe
    // client is only constructed when a retrieval is actually needed — an
    // inline object records the fee with zero network (and works without keys).
    let bt: any = paymentIntent?.latest_charge?.balance_transaction;
    if (!bt || typeof bt === 'string') {
      const stripe = await getStripeSync();
    ...
    const { paymentFeeItemFromBalanceTransaction } = await import('./lib/mission-economics');
    const item = paymentFeeItemFromBalanceTransaction(bt);
    if (!item) {
      console.warn(`[mission-evidence] balance transaction for charge ${chargeId} unparseable — fee not recorded (fail closed)`);
      return;
    }
    const { addEvidence } = await import('./lib/revenue-missions');
    const ev = await addEvidence({
      tenantId,
      missionId,
      type: 'other',
      summary: `Stripe payment fee: ${(item.feeUsdCents / 100).toFixed(2)} USD (${item.externalRef})`,
      source: 'stripe',
      externalRef: item.externalRef,
      amountUsdCents: item.feeUsdCents,
      raw: { kind: 'payment_fee', balanceTransactionId: item.externalRef, chargeId, paymentIntentId: paymentIntent?.id, feeUsdCents: item.feeUsdCents, livemode: paymentIntent?.livemode === true },
    });
    console.log(`[mission-evidence] ${ev ? 'recorded' : 'duplicate (skipped)'} payment fee ${item.externalRef} → mission ${missionId} (${item.feeUsdCents}c)`);
  }
   ...
   */
  static async applyMissionAutonomyHooks(tenantId: number, missionId: number, paymentRef?: string): Promise<void> {
    const { getMission, addEvidence } = await import('./lib/revenue-missions');
    ...
    if (autonomyAllows(mission, 'auto_fulfillment_kickoff')) {
      // Idempotent per payment: a Stripe retry of the SAME payment must not
      // stack duplicate fulfillment wakes, but a transient wake failure on the
      // first delivery IS retried (the existence check finds nothing).
      // Atomicity (architect finding): a bare read-then-insert is racy under
      // concurrent deliveries — check + insert run inside one transaction
      // serialized by a per-(tenant, paymentRef) advisory xact lock.
      const goal = `Revenue mission #${missionId} ("${mission.name}") received a verified Stripe payment. Draft a fulfillment plan for the offer "${mission.offer}" — deliverable scope, margin check, delivery route via deliver_product. Do NOT send anything external without the standard delivery pipeline.`;
      if (paymentRef) {
        const { db } = await import('./db');
        const { sql } = await import('drizzle-orm');
        const wakeContext = JSON.stringify({ missionId, trigger: 'mission_payment', autonomyLevel: mission.autonomy_level, paymentRef });
        const res: any = await db.transaction(async (tx) => {
          await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext('mission-payment-wake'), hashtext(${`${tenantId}:${paymentRef}`}))`);
          const existing: any = await tx.execute(sql`
            SELECT id FROM agent_wake_schedules
            WHERE tenant_id = ${tenantId} AND created_by = 'mission-autonomy'
              AND context->>'paymentRef' = ${paymentRef}
            LIMIT 1
          `);
          if (((existing?.rows ?? existing) as any[]).length > 0) return null;
          return tx.execute(sql`
            INSERT INTO agent_wake_schedules
              (tenant_id, kind, goal, context, wake_at, max_attempts, created_by)
            VALUES
              (${tenantId}, 'follow_up', ${goal}, ${wakeContext}::jsonb,
               ${new Date(Date.now() + 60_000)}, 1, 'mission-autonomy')
            RETURNING id
          `);
        });
        const inserted = res ? ((res?.rows ?? res) as any[]) : [];
        if (inserted.length > 0) {
          console.log(`[mission-autonomy] level ${mission.autonomy_level}: fulfillment wake #${inserted[0].id} scheduled for mission ${missionId}`);
        }
      } else {
        // Legacy caller without a payment ref: unguarded (pre-existing behavior).
    ...
    if (autonomyAllows(mission, 'reinvest_realized_margin')) {
      // At-most-once per payment (replay guard): claim a durable marker row
      // via the evidence unique index BEFORE applying. A duplicate claim
      // (Stripe retry) skips reinvestment entirely — the budget must never
      // ratchet on replays of the same payment. Claim-then-apply fails toward
      // NOT reinvesting (bounded, safe) if the process dies between the two.
      if (paymentRef) {
        const claim = await addEvidence({
          tenantId,
          missionId,
          type: 'other',
          summary: `Reinvestment claim for payment ${paymentRef}`,
          source: 'internal',
          externalRef: `reinvest_${paymentRef}`,
          raw: { kind: 'reinvest_claim', paymentRef },
        });
        if (!claim) {
          console.log(`[mission-autonomy] reinvestment already claimed for payment ${paymentRef} — skipping (replay)`);
          return;
        }
      }
      // Honest margin (CPT 5.6 priority 4): reinvest from CONTRIBUTION margin —
      // subtract Stripe fees + mission-attributed variable API cost. Fail
      // closed: if either sum is unreadable, refuse to reinvest this round.
      let extraCostsUsdCents: number | null = null;
      try {
        const { sumPaymentFeesUsdCents, sumVariableApiCostUsdCents } = await import('./lib/mission-economics');
        ...
        extraCostsUsdCents = fees + apiCost;
      } catch (e) {
        console.warn(`[mission-autonomy] economics unreadable — refusing reinvestment for mission ${missionId} (fail closed):`, (e as any)?.message ?? e);
      }
      const plan = extraCostsUsdCents == null ? null : computeReinvestment(mission, extraCostsUsdCents);
      if (plan) {
        ...
        const res: any = await db.execute(sql`
          ...
          WHERE tenant_id = ${tenantId} AND id = ${missionId}
            ...
            AND ${plan.newBudgetUsd}::int <= ${REINVEST_RULES.maxBudgetUsd}::int
          RETURNING id
        `);
  ...
  static async handlePaymentSucceeded(paymentIntent: any): Promise<void> {
    try {
      ...
      // This legacy path is likewise platform-owned. Do not let Stripe
      // metadata choose a delivery tenant.
      const { ownerTenantId: resolveOwnerTenant } = await import('./agentic/autonomous-budget');
      const deliveryTenantId = resolveOwnerTenant();
