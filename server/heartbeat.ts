...
async function executeTaskInner(task: HeartbeatTask, start: number, persona: Persona | null, personaLabel: string) {
  ...
  if (task.type === "cloud_backup") {
    ...
    try {
      // R63: Race the backup against a 90s ceiling so a hung Drive auth cascade
      // fails fast and frees the heartbeat slot instead of hanging the full 600s.
      // Real backups complete in well under a minute; anything longer is a hang.
      const BACKUP_HARD_TIMEOUT_MS = 90 * 1000;
      const summary = await Promise.race([
        runBackupToGoogleDrive(),
        new Promise<string>((_, reject) =>
          setTimeout(() => reject(new Error(`Cloud backup aborted: Drive auth/upload exceeded ${BACKUP_HARD_TIMEOUT_MS / 1000}s (likely a token-refresh hang).`)), BACKUP_HARD_TIMEOUT_MS)
        ),
      ]);
      let gitStatus = "";
      try {
