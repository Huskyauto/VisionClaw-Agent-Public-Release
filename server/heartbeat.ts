...
async function executeTaskInner(task: HeartbeatTask, start: number, persona: Persona | null, personaLabel: string) {
  ...
  if (task.type === "ideabrowser_dev_sync") {
    ...
    try {
      ...
      const summary = `ingested=${ing.newlyStored} newProjects=${ing.createdProjectIds.length} fetched=${ing.fetched}${ing.errors.length ? ` ingestErrors=${ing.errors.length}` : ""}${scoreSummary}${weeklySummary}`;
      if (task.runOnce) {
        await storage.updateHeartbeatTask(task.id, { enabled: false });
        await storage.markHeartbeatTaskRun(task.id, new Date());
      } else {
