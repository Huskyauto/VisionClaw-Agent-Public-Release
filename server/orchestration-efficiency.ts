...
export interface OrchestrationEfficiencyRecord {
  ...
  // arXiv:2606.24937 — process-quality / trajectory grading. Optional: only
  // callsites with a finished trajectory in scope (CEO plan execution) populate
  // these; everything else leaves them null.
    ...
    await db.execute(sql`
      INSERT INTO orchestration_efficiency
        (tenant_id, request_class, label, predicted_duration_ms, predicted_cost_usd,
         ...
         process_quality, completion_rate, failure_rate, redundancy_rate, steps)
      VALUES (
        ${rec.tenantId},
        ...
        ${rec.triviality ?? null},
        ${rec.processQuality ?? null},
        ${rec.completionRate ?? null},
        ${rec.failureRate ?? null},
        ${rec.redundancyRate ?? null},
        ${rec.steps ?? null}
      )
    `);
  } catch (_silentErr) {
...
export interface EfficiencySummary {
  ...
  threshold: number;
  breached: boolean;
  // arXiv:2606.24937 — process-quality / trajectory grading ("evaluation beyond
  // final task success"). Aggregated over the subset of recent records that
  // carried a trajectory score (CEO plan executions).
  processQuality: {
    /** how many of the recent records carried a process-quality score. */
    sampleSize: number;
    /** median 0..1 composite — higher = cleaner execution paths. */
    medianQuality: number;
    /** median completed/steps. */
    medianCompletionRate: number;
    /** count of runs whose process quality fell below the quality threshold. */
...
const QUALITY_THRESHOLD = 0.7; // median process quality below this (median) = trajectories are wasteful
    ...
    const res = await db.execute(sql`
      WITH recent AS (
        SELECT predicted_duration_ms, actual_duration_ms,
               ...
               process_quality, completion_rate
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE process_quality IS NOT NULL)::int AS pq_sample,
        COALESCE(percentile_cont(0.5) WITHIN GROUP (
          ORDER BY process_quality) FILTER (WHERE process_quality IS NOT NULL), 0) AS pq_median,
        COALESCE(percentile_cont(0.5) WITHIN GROUP (
          ORDER BY completion_rate) FILTER (WHERE completion_rate IS NOT NULL), 0) AS cr_median,
        COALESCE(SUM(CASE WHEN process_quality IS NOT NULL AND process_quality < ${QUALITY_THRESHOLD} THEN 1 ELSE 0 END), 0)::int AS pq_low
      FROM recent
    `);
    const row = (((res as any).rows || res) as any[])[0] || {};
    const sample = Number(row.sample) || 0;
    const gap = Number(row.gap) || 0;
    const pqSample = Number(row.pq_sample) || 0;
    const pqMedian = Math.round((Number(row.pq_median) || 0) * 100) / 100;
    const gapBreached = sample >= 10 && gap > GAP_THRESHOLD;
    // only flag a process-quality breach once there's a meaningful sample.
    const pqBreached = pqSample >= 10 && pqMedian < QUALITY_THRESHOLD;
    return {
      sampleSize: sample,
      ...
      threshold: GAP_THRESHOLD,
      // card-level / overall breach = EITHER the prediction-gap illusion OR a
      // wasteful-trajectory breach, so neither can hide behind the other.
      breached: gapBreached || pqBreached,
      processQuality: {
        sampleSize: pqSample,
        medianQuality: pqMedian,
        medianCompletionRate: Math.round((Number(row.cr_median) || 0) * 100) / 100,
        lowQualityCount: Number(row.pq_low) || 0,
        threshold: QUALITY_THRESHOLD,
        breached: pqBreached,
      },
    };
