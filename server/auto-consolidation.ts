...
async function runREMPhase(tenantId: number, deepResult: DreamConsolidationResult): Promise<{ wrote: boolean; diary?: string; error?: string }> {
  try {
    const memResult = await db.execute(sql`
      SELECT id, fact, category FROM memory_entries
      ...
      LIMIT 30
    `);
    const rows = (((memResult as any)?.rows) || []) as { id: number; fact: string; category: string }[];
    ...
    const { client, actualModelId } = await getClientForModel("gpt-5.4", tenantId, {});
    ...
    await db.transaction(async (tx) => {
      const inserted = await tx.execute(sql`
        ...
        RETURNING id
      `);
      const diaryId = Number((((inserted as any).rows || inserted) as any[])[0]?.id);
      if (!Number.isInteger(diaryId) || diaryId <= 0) {
        throw new Error("REM diary insert returned no memory ID");
      }
      // The diary is generated exclusively from this selected durable batch.
      // Record only the ids actually supplied to the synthesizer; never infer
      // provenance from a tenant-wide query or from the generated text.
