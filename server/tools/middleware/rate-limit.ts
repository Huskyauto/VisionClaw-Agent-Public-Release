...
export async function enforceRateLimitGate(
  ...
  try {
    ...
    if (rlTenant && !(params as any)._rateLimitChecked) {
      const { checkToolRateLimitPaced, recordToolUsage, isExpensiveTool } = await import("../../tool-rate-limiter");
      if (isExpensiveTool(name)) {
        // PACE instead of KILL (Bob 2026-08-03) — pacing now lives in the
        // shared checkToolRateLimitPaced (tool-rate-limiter.ts) so this path
        // and guarded-tool-executor can't drift apart again (2026-08-08:
        // Felix's research runs were dying on the guarded path, which lacked
        // the pacing this middleware had). Minute-window-full waits once
        // (≤70s); hour/day ceilings still hard-deny.
        const rl = await checkToolRateLimitPaced(rlTenant, name);
        if (!rl.allowed) {
          console.warn(`[executeTool] RATE LIMITED tool=${name} tenant=${rlTenant} reason=${rl.reason}`);
          return { error: `RATE LIMITED: ${rl.reason}${(await import("../../tool-rate-limiter")).rateLimitAlternatesHint(name, rl.window)}` };
        }
    ...
    const HARDCODED_EXPENSIVE = new Set([
      ...
      // 2026-08-09 architect parity sweep — these limiter-classified tools were
      // missing from the backstop (would have failed OPEN on limiter outage):
      "generate_evidence_docket", "sandbox_run", "revenue_mission_draft_experiment",
      "jury_triage", "bwb_weekly_build",
    ]);
