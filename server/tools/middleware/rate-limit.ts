  ...
  try {
    ...
    if (rlTenant && !(params as any)._rateLimitChecked) {
      ...
      if (isExpensiveTool(name)) {
        let rl = checkToolRateLimit(rlTenant, name);
        // PACE instead of KILL (Bob 2026-08-03 — Felix's multi-page website
        // reviews kept dying mid-run on the per-MINUTE window). When only the
        // minute window is full (retryAfterMs small), wait once until a slot
        // frees, then re-check and proceed. Hour/day caps still hard-deny —
        // those are real spend ceilings, and a 5min-1hr sleep inside a tool
        // call would wedge the step executor. Bounded: single wait, ≤70s.
        const PACE_MAX_WAIT_MS = 70_000;
        if (!rl.allowed && typeof rl.retryAfterMs === "number" && rl.retryAfterMs > 0 && rl.retryAfterMs <= PACE_MAX_WAIT_MS) {
          ...
          console.warn(`[executeTool] PACING tool=${name} tenant=${rlTenant} — minute window full, waiting ${waitMs}ms instead of failing (${rl.reason})`);
          await new Promise((r) => setTimeout(r, waitMs));
          rl = checkToolRateLimit(rlTenant, name);
        }
        if (!rl.allowed) {
