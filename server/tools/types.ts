...
export interface ToolContext {
  ...
  /**
   * Action Ledger S4 — ADVISORY cancellation signal. Present only when the
   * dispatch entered through `executeToolWithTimeout` (which owns the
   * AbortController); fires when the outer timeout race has already rejected,
   * so long-running handlers / fetch clients can stop wasted work. NEVER an
   * authz input, and no retry decision may key off it (S5 owns retries).
   * Platform-threaded via ALS (server/lib/tool-abort-context.ts) — never
   * sourced from a caller-supplied param.
   */
