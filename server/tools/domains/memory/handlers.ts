  ...
  try {
    switch (action) {
      case "store": {
        ...
        const newVersion = await memDb.transaction(async (tx) => {
          const existing = await tx.execute(memSql`SELECT id, version
            FROM graph_memory
            WHERE tenant_id = ${tenantId} AND path = ${path}
              AND (persona_id = ${personaId}
                OR (persona_id IS NULL AND ${personaId} IS NULL))
            ORDER BY version DESC LIMIT 1`);
          const parentId = Number((((existing as any).rows || existing) as any[])[0]?.id);
          const currentVersion = Number((((existing as any).rows || existing) as any[])[0]?.version || 0);
          const nextVersion = currentVersion + 1;
          const inserted = await tx.execute(memSql`
            INSERT INTO graph_memory
              (tenant_id, persona_id, path, content, trigger_condition, version,
               parent_path, created_at, updated_at)
            VALUES (${tenantId}, ${personaId}, ${path}, ${content},
              ${trigger_condition || null}, ${nextVersion}, ${parentPath},
              NOW(), NOW())
            RETURNING id
          `);
          const childId = Number((((inserted as any).rows || inserted) as any[])[0]?.id);
          if (!Number.isInteger(childId) || childId <= 0) {
            throw new Error("graph memory insert returned no id");
          }
          if (Number.isInteger(parentId) && parentId > 0) {
            const { recordMemoryDerivation } = await import("../../../memory/forgetting-store");
            // A graph revision is independently readable and authored by the
            // caller, so retain provenance but do not authorize erasure
            // cascading through it.
            await recordMemoryDerivation(tx, {
              tenantId,
              parentSource: "graph_memory",
              parentId,
              childSource: "graph_memory",
              childId,
              relationship: "graph_revision",
              solelyDerived: false,
            });
          }
          return nextVersion;
