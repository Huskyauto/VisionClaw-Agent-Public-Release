...
async function logExperimentHandler(
  ...
  // Tenant-isolation audit 2026-07-31: fail CLOSED when the dispatcher did not
  // stamp a tenant — never write an experiment row with an undefined tenant.
...
async function getExperimentsHandler(
  ...
  // R125+143 review fix — fail closed on missing tenant context (mirrors the
  // adjacent write/run handlers): getExperimentHistory returns UNFILTERED rows
  // when tenantId is absent, which would leak cross-tenant experiments.
  if (ctx.tenantId == null) {
    return { error: "tenant context missing — refusing unscoped experiment read" };
  }
