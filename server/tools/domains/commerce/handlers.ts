...
async function createPaymentLinkHandler(
  ...
  try {
    ...
    if (row.mission_id) {
      ...
      const mission = await getMission(gate.tenantId, row.mission_id);
      if (!mission) {
        return { error: `SKU '${sku}' has a mission link outside the owner tenant; repair the catalog link before creating a payment link` };
      }
    ...
    const link = await stripe.paymentLinks.create({
      ...
      metadata: {
        bundle_sku: sku,
        ...(missionId ? { mission_id: String(missionId) } : {}),
      },
