...
async function assertMemoryValueNotTombstoned(
  input: { tenantId: number; source: "memory_entries"; parts: unknown[] },
): Promise<void> {
  const tombstones = await import("../../../memory/tombstones");
  await tombstones.assertMemoryValueNotTombstoned(input);
}
