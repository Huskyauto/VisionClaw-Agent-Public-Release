...
async function validateManagedSkillPrompt(input: {
  name: unknown;
  description: unknown;
  ...
  } = await import("../../../lib/managed-skill-safety");
  try {
    assertManagedSkillPromptSafe(input as {
      name: string;
      description: string;
...
async function managedSkillPersistenceError(error: unknown): Promise<ToolResult | null> {
  const { isManagedSkillSecurityError } = await import("../../../lib/managed-skill-safety");
  if (!isManagedSkillSecurityError(error)) return null;
  return { error: `Skill content rejected by security scan (${error.patterns.join(", ")}).` };
}
...
async function manageSkillsHandler(
  ...
  switch (params.command) {
    ...
    case "create": {
      ...
      const securityError = await validateManagedSkillPrompt({
        name: params.name,
      ...
      try {
        ...
        const securityError = await managedSkillPersistenceError(error);
        if (securityError) return securityError;
        throw error;
      }
    ...
    case "update": {
      ...
      if (
        ...
        || params.promptContent !== undefined
      ) {
        const existingSkills = await storage.getSkills();
        const existing = existingSkills.find((skill: any) => skill.id === params.id);
        if (!existing) return { error: `Skill ${params.id} not found` };
        const securityError = await validateManagedSkillPrompt({
          name: params.name ?? existing.name,
          description: params.description ?? existing.description,
          promptContent: params.promptContent ?? existing.promptContent,
        });
        if (securityError) return securityError;
      }
      let updated: any;
      try {
        updated = await storage.updateSkill(params.id, updates);
      } catch (error) {
        const securityError = await managedSkillPersistenceError(error);
        if (securityError) return securityError;
        throw error;
    ...
    case "enable": {
      ...
      let enabled: any;
      try {
        enabled = await storage.updateSkill(params.id, { enabled: true });
      } catch (error) {
        const securityError = await managedSkillPersistenceError(error);
        if (securityError) return securityError;
        throw error;
