...
async function critiqueResponseHandler(
  ...
  return critiqueToolForAgent(params.content, params.context, ctx.tenantId);
}
...
async function gradeDeliverableHandler(
  ...
  try {
    ...
    // Outcome ledger (Grok item #2, fire-and-forget, fail-open): record EVERY
    // grade event — pass AND fail — so the nightly outcome-promotion scan can
    // turn recurring failure patterns into skill-optimizer eval seeds.
    try {
      if (typeof res.score === "number") {
        const { recordGradeOutcome } = await import("../../../lib/outcome-promotion");
        void recordGradeOutcome({
          tenantId: tid,
          format: String(res.graderFormat || params.deliverable_type || ""),
          score: res.score,
          passed: !!res.ok,
          critique: res.critique ? String(res.critique) : undefined,
          requestExcerpt: params.request ? String(params.request) : undefined,
        });
      }
    } catch (_outcomeErr) {
      const { logSilentCatch } = await import("../../../lib/silent-catch");
      logSilentCatch("server/tools/domains/quality/handlers.ts", _outcomeErr);
    }
