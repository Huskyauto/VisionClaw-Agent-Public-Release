...
async function mpegProduceHandler(
  ...
  const { inheritInternalRenderStaging } = await import("../../../lib/internal-render-markers");
  const result = await produceVideo(inheritInternalRenderStaging(params, {
...
async function mpegProduceParallelHandler(
  ...
  const { inheritInternalRenderStaging } = await import("../../../lib/internal-render-markers");
  const result = await produceVideoParallel(inheritInternalRenderStaging(params, {
    title: params.title || "Untitled Video",
...
3. Slide 1 is a hook (grab attention in the first 2 seconds). Open a specific curiosity gap, tension, or useful promise in that hook; do not reveal its full answer until the later payoff. Slide ${targetSlides} is the payoff${cta ? " + the call to action above" : ""}.
4. Every slide must advance the story or understanding with new evidence, an example, a contrast, a reveal, or the next necessary step. Do not repeat the setup in different words or add decorative filler just to occupy time.
5. Build a retention arc: hook → curiosity/question → escalating value or story → clear payoff${cta ? " → the requested CTA, connected naturally to the value just delivered" : ". End on the payoff; do not invent a follow, subscribe, comment, or other engagement CTA"}. Never use forced engagement bait.
6. Pick the BEST OpenAI TTS voice for this tone+audience: alloy (neutral/balanced), echo (warm male), fable (storyteller male, British-leaning), onyx (deep authoritative male), nova (bright friendly female), shimmer (soft warm female).${voicePref ? ` User requested: "${voicePref}" — honor it unless clearly mismatched.` : ""}
7. Pick a transition_type that matches the tone: "fade" for most, "dissolve" for soft/emotional, "wipeleft" or "slideleft" for upbeat/dynamic, "circleopen" for reveal moments. ken_burns should usually be true for documentary/professional/cinematic feel; false only for flat-illustrated styles.
