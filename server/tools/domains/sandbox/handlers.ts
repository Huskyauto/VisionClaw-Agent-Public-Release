...
/**
 * Sandbox is admin-tenant only this round (shared/models/sandbox.ts). The
 * admin HTTP routes enforce ADMIN_TENANT_ID + isAdminRequest; the tool path
 * must enforce the same tenant boundary or a trusted persona operating in a
 * non-admin tenant context could run sandbox operations outside scope.
 * Trust seam: tenantId comes from platform-stamped ctx, never params.
 */
const ADMIN_TENANT_ID = 1;
function requireAdminTenant(ctx: ToolContext): { tenantId: number } | { error: string } {
  if (!ctx.tenantId) return { error: "Tenant context required" };
  const tenantId = Number(ctx.tenantId);
  if (tenantId !== ADMIN_TENANT_ID) {
    return { error: "Sandbox tools are admin-tenant only" };
  }
  return { tenantId };
}
...
async function sandboxRunHandler(
  ...
  const adminCheck = requireAdminTenant(ctx);
