...
export const listPlansDefinition: ToolDefinition = {
  ...
  function: {
    name: "list_plans",
    description: "Use when Bob asks \"what is Minerva planning\" or \"what is Felix deciding\", before approving any plan to see related history, or when auditing what plans came through this week. Returns plans for this tenant with status (awaiting_approval | approved | executing | rejected | revising | revised | completed | failed | expired). Filter by status to surface decision queues — `awaiting_approval` is the actionable queue.",
    parameters: {
      type: "object",
      properties: {
        status: { type: "string", enum: ["awaiting_approval", "approved", "executing", "rejected", "revising", "revised", "completed", "failed", "expired"], description: "Optional: filter by plan status. `awaiting_approval` = pending Felix decision; `executing` = plan-executor is running it; `revising` = superseded by a child revision plan; `revised` = the child revision plan itself; `expired` = sat undecided past the 14-day approval TTL and was auto-expired (re-propose if still relevant)." },
