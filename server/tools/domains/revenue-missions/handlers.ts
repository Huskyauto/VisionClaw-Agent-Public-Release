...
async function revenueMissionStatusHandler(
  ...
  try {
    ...
    const capUsdCents = Math.max(0, Number(mission.max_cash_at_risk_usd) || 0) * 100;
    const spendUsdCents = Number(mission.spend_usd_cents) || 0;
    // Honest-margin ledger (CPT 5.6 priority 4) — best-effort: surface the
    // full economic breakdown; if unreadable, report the error string instead
    // of silently showing $0 costs.
    let economics: any = null;
    try {
      const econ = await import("../../../lib/mission-economics");
      economics = await econ.getMissionEconomics(gate.tenantId, missionId);
    } catch (e: any) {
      economics = { error: `economics unavailable: ${e?.message}` };
    }
    return {
      mission,
      economics,
      budget: {
        capUsdCents,
        spendUsdCents,
        remainingUsdCents: lib.budgetHeadroomUsdCents(spendUsdCents, capUsdCents),
      },
      retrospective: mission.retrospective ?? null,
      doneChecks,
