...
async function storeTriple(params: Record<string, any>, tenantId: number) {
  ...
  await assertMemoryValueNotTombstoned({
    ...
    parts: [params.subject, params.predicate, params.object],
  });
...
function isTrustedExactOperationalSkill(
  item: { title: string; category: string; source: string },
  query: string,
): boolean {
  const exactQuery = query.trim();
  return /^skill:[a-z0-9-]+$/.test(exactQuery)
    && item.title === exactQuery
    && item.category === "agent_skill"
    && item.source === "agent_skill";
}

function knowledgeSearchResult(
  item: { id: number; title: string; category: string; content: string; source: string; priority?: number; similarity?: number },
  query: string,
) {
  const trustedExactSkill = isTrustedExactOperationalSkill(item, query);
  return {
    id: item.id,
    title: item.title,
    category: item.category,
    source: item.source,
    // A trusted exact runbook is intentionally returned in full so its safety
    // gates are not lost to the normal snippet budget. All other knowledge
    // stays excerpted and is reference material, not operational instruction.
    content: trustedExactSkill ? item.content : item.content.slice(0, 500),
...
async function searchKnowledge(query: string, tenantId: number) {
  ...
  try {
    ...
    // includeGlobal (2026-08-11): platform-global rows (persona_id NULL — e.g.
    // nightly Agent Knowledge Refresh briefs) were invisible to persona-scoped
    // semantic search while the keyword path DID include them. Align the two.
    const results = await vectorSearchKnowledge(query, { personaId: persona?.id, tenantId, topK: 25, includeGlobal: true });
    if (results.length > 0) {
      return {
        count: results.length,
        searchType: "semantic",
        results: results.map((k) => knowledgeSearchResult(k, query)),
      };
    }
  ...
  const matches = knResult.data
    ...
    .slice(0, 10);
  // Keyword-tier results are genuine returned knowledge — record usage here too
  // so the frequency signal isn't blind to the fallback path.
  try {
    const { bumpKnowledgeUsage } = await import("../../../embeddings");
    bumpKnowledgeUsage(tenantId, matches.map((k) => k.id));
  } catch (_silentErr) { const { logSilentCatch } = await import("../../../lib/silent-catch"); logSilentCatch("server/tools/domains/knowledge/handlers.ts", _silentErr); }
  return {
    count: matches.length,
    searchType: "keyword",
    results: matches.map((k) => knowledgeSearchResult(k, query)),
  };
}
