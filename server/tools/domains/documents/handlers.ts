...
/**
 * R125+138 app-wide self-healing — post-create PDF verification. A "successful"
 * create that produced a corrupt/empty file (overlayFS truncation, renderer
 * crash mid-write) must not be handed back to the agent as deliverable. Runs
 * the existing contract verifier (magic bytes + min size) and, on failure,
 * files a self-repair incident and converts the result to an error. Verifier
 * infra errors fail OPEN (a checker outage must never sink a legit create).
 */
async function verifyCreatedPdf(result: ToolResult, ctx: ToolContext, toolName: string): Promise<ToolResult> {
  try {
    const r: any = result;
    const filePath: string | undefined = r?.path || r?.filePath || r?.file_path;
    if (!r || r.error || r.success === false || !filePath || !/\.pdf$/i.test(filePath)) return result;
    const tenantId = ctx.tenantId ?? 1; // audit/incident attribution only — never an authz grant
    const { verifyDeliverable } = await import("../../../deliverable-verifier");
    const v = await verifyDeliverable({ tenantId, deliverableType: "pdf_document", filePath });
    if (v.status === "failed") {
      const detail = v.failures.join("; ").slice(0, 1000);
      const { reportQualityIncident } = await import("../../../lib/outbound-quality-gate");
      reportQualityIncident({
        tenantId,
        signature: "created_pdf_failed_verification",
        title: `${toolName} produced an invalid PDF: ${filePath}`,
        error: detail,
        stage: "document-create",
        candidateFiles: ["server/pdf-create.ts", "server/tools/domains/documents/handlers.ts"],
        metadata: { toolName, filePath },
      });
      return { error: `The generated PDF failed verification (${detail}). It was NOT delivered — regenerate it. The self-repair system has been notified.` } as ToolResult;
    }
  } catch (e: any) {
    console.warn(`[documents] pdf verification error (failing open): ${e?.message}`);
  }
  return result;
}
...
async function createPdfHandler(
  ...
  return verifyCreatedPdf(await createPdf({
    title: params.title,
...
async function createStyledReportHandler(
  ...
  return verifyCreatedPdf(await generateStyledPdf({
    title: params.title,
    ...
    includePlatformBranding: params.includePlatformBranding,
    fileName: params.fileName,
    folderLabel: params.folderLabel || "deliverables",
    tenantId: ctx.tenantId,
    _projectDriveFolderId: params._projectDriveFolderId,
  } as any), ctx, "create_styled_report");
}
