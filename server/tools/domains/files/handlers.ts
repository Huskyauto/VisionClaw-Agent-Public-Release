      ...
      try {
        ...
        if (pfRows.length > 0) {
          ...
          if (pf.filePath) {
            ...
            if (localCandidate.startsWith("/home/runner/workspace") && fs.existsSync(localCandidate) && !fs.statSync(localCandidate).isDirectory()) {
          ...
          if (!found && pf.fileUrl) {
            ...
            if (driveId) {
              ...
              if (dl.success && dl.path) {
                const recovered = path.resolve(process.cwd(), dl.path);
                const uploadsRoot = path.resolve("/home/runner/workspace", "uploads") + path.sep;
                // Confine the recovered file to the uploads root before trusting it.
                if (recovered.startsWith(uploadsRoot) && fs.existsSync(recovered)) {
                  absPath = recovered;
                  found = true;
