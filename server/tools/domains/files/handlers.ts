...
const UPLOADS_ROOT_WITH_SEP = UPLOADS_ROOT + path.sep;
...
export async function readFileHandler(
  ...
  if (!isPathInsideRoot(absPath, WORKSPACE_ROOT)) return { error: "Access denied: path outside workspace" };
  // Set ONLY when a tenant-scoped project_files recovery succeeded — lets the
  // uploads ownership gate below accept this exact recovered path (there is
  // no file_storage row for a Drive-recovered project file).
  let projectRecoveredPath: string | null = null;
  let projectRecoveryFailure: string | null = null;
  if (!fs.existsSync(absPath)) {
    const basename = path.basename(safeParts);
    const searchDirs = ["attached_assets", "uploads", "data", "client/public", UPLOADS_ROOT];
    let found = false;
    ...
    if (!found) {
      ...
      // PROJECT-FILES FALLBACK — documents "saved to a project" live as
      // project_files metadata (local path + Drive URL), which the
      // file_storage lookup above cannot see. On ephemeral prod FS the local
      // copy vanishes across restarts, so recover from the Drive copy.
      try {
        const { db } = await import("../../../db");
        const { projectFiles, projects } = await import("@shared/schema");
        const { eq, desc, or, and } = await import("drizzle-orm");
        const pfRows = await db.select({
          ...
          .where(and(
            or(eq(projectFiles.fileName, basename), eq(projectFiles.filePath, safeParts)),
            ctx.tenantId ? eq(projects.tenantId, ctx.tenantId) : eq(projects.tenantId, -1),
          ))
          .orderBy(desc(projectFiles.createdAt))
          .limit(1);
        if (pfRows.length > 0) {
          const pf = pfRows[0];
          // Try the registered local path first.
          if (pf.filePath) {
            const localCandidate = path.resolve("/home/runner/workspace", pf.filePath.replace(/\.\./g, "").replace(/^\/+/, ""));
            if (isPathInsideRoot(localCandidate, WORKSPACE_ROOT) && fs.existsSync(localCandidate) && !fs.statSync(localCandidate).isDirectory()) {
              absPath = localCandidate;
              found = true;
            }
          }
          // Otherwise recover from the Drive copy (durable in prod).
          if (!found && pf.fileUrl) {
            const { downloadFromDrive, extractDriveFileId } = await import("../../../google-drive");
            // file_url is historically a Drive URL, but durable artifact
            // recovery can store the provider's raw file ID. Both are durable
            // identities; neither depends on the vanished local cache.
            const driveId = extractDriveFileId(pf.fileUrl);
            if (driveId) {
              const dl = await downloadFromDrive({ fileId: driveId, savePath: `uploads/project-recovered-${driveId}-${basename}` });
              if (dl.success && dl.path) {
                const recovered = path.join(UPLOADS_ROOT, path.basename(dl.path));
                // Confine the recovered file to the uploads root before trusting it.
                if (isPathInsideRoot(recovered, UPLOADS_ROOT) && fs.existsSync(recovered)) {
                  absPath = recovered;
                  found = true;
                  // Authorization came from the tenant-scoped project_files
                  // query above — mark this exact path so the uploads
                  // ownership gate accepts it (no file_storage row exists).
                  projectRecoveredPath = recovered;
                }
              } else {
                projectRecoveryFailure = dl.error || "Drive returned no readable file content";
              }
            } else {
              projectRecoveryFailure = "project file has an invalid or unsupported Drive identity";
            }
          }
          if (!found) {
            return {
              error: projectRecoveryFailure
                ? `File '${safeParts}' is registered on project #${pf.projectId}, but Drive recovery failed: ${projectRecoveryFailure}`
                : `File '${safeParts}' is registered on project #${pf.projectId} but its content is not recoverable here (no local copy, no readable Drive copy). Use the project tool: project {command:"get", id:${pf.projectId}} to see its files/notes, or google_drive search for "${pf.fileName}".`,
            };
          }
        }
      } catch (_silentErr) {
        projectRecoveryFailure = _silentErr instanceof Error ? _silentErr.message : String(_silentErr);
        logSilentCatch("server/tools/domains/files/handlers.ts", _silentErr);
      }
      if (!found) {
        return {
          error: projectRecoveryFailure
            ? `File not found: ${safeParts}. Project/Drive recovery failed: ${projectRecoveryFailure}`
            : `File not found: ${safeParts} (also checked attached_assets/, uploads/, data/, database, and project files). If this document was saved to a project, its text may be a project NOTE — use project {command:"get", id:<projectId>} to read notes, or google_drive search by name. For the latest platform features document use path "data/VisionClaw-Comprehensive-Features.txt".`,
        };
      }
    }
  }
  ...
  const uploadsRoots = [path.resolve("/home/runner/workspace", "uploads") + path.sep, UPLOADS_ROOT_WITH_SEP];
  // projectRecoveredPath: this exact path was just recovered via a
  // tenant-scoped project_files→projects join for THIS tenant, so ownership
  // is already proven (a Drive-recovered project file has no file_storage row).
  if (uploadsRoots.some(root => absPath.startsWith(root)) && ctx.tenantId !== 1 && absPath !== projectRecoveredPath) {
    try {
      if (typeof ctx.tenantId !== "number") {
  ...
  try {
    ...
    // The configured uploads directory is a legitimate search root — it is
    ...
    if (!isPathInsideRoot(realPath, WORKSPACE_ROOT) && !isPathInsideRoot(realPath, UPLOADS_ROOT)) {
...
export async function writeFileHandler(
  ...
  try {
    if (fs.existsSync(absPath)) {
      ...
      if (lst.isSymbolicLink()) return { error: "Access denied: target is a symlink" };
      const realPath = fs.realpathSync(absPath);
      if (!isPathInsideRoot(realPath, WORKSPACE_ROOT)) return { error: "Access denied: realpath escapes workspace" };
    }
  } catch (_silentErr) { logSilentCatch("server/tools/domains/files/handlers.ts", _silentErr); }
  ...
  try {
    let probe = dir;
    while (isPathInsideRoot(probe, WORKSPACE_ROOT) && probe !== WORKSPACE_ROOT) {
  ...
  try {
    ...
    const driveResult = await uploadAndShare({
      ...
      generatedArtifact: {
        ...
        sourceRunKey: `write-file:${ctx.projectId || "tenant"}:${safeParts}`,
        metadata: { producer: "write_file" },
      },
    });
    if (driveResult.success) {
      ...
      driveUploadError = driveResult.error || "Google Drive durability verification failed";
    }
  } catch (driveErr: any) {
    ...
    if (driveErrMsg.startsWith("Pre-upload secret scan BLOCKED") || driveErrMsg.startsWith("Pre-upload secret scan unavailable")) {
      ...
      driveUploadError = driveErrMsg;
    }
  }
  const secretScanBlockReason: string | undefined = (params as any).__secretScanBlockReason;
  if (!driveUrl) {
    const message = secretScanBlockReason || driveUploadError || "Google Drive upload failed or unavailable";
    return { error: `File was written locally but is not durable: ${message}`, path: safeParts, size: stat.size, upload_error: message };
  }
