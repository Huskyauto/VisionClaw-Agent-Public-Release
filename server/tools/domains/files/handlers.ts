    ...
    if (!found) {
      ...
      try {
        ...
        if (pfRows.length > 0) {
          ...
          if (!found && pf.fileUrl) {
            const idMatch = pf.fileUrl.match(/\/d\/([A-Za-z0-9_-]{20,})|[?&]id=([A-Za-z0-9_-]{20,})|\/file\/d\/([A-Za-z0-9_-]{20,})/);
            const driveId = idMatch ? (idMatch[1] || idMatch[2] || idMatch[3]) : null;
            if (driveId) {
          ...
          if (!found) {
            return { error: `File '${safeParts}' is registered on project #${pf.projectId} but its content is not recoverable here (no local copy, no readable Drive copy). Use the project tool: project {command:"get", id:${pf.projectId}} to see its files/notes, or google_drive search for "${pf.fileName}".` };
          }
        }
      } catch (_silentErr) { logSilentCatch("server/tools/domains/files/handlers.ts", _silentErr); }
      if (!found) {
        return { error: `File not found: ${safeParts} (also checked attached_assets/, uploads/, data/, database, and project files). If this document was saved to a project, its text may be a project NOTE — use project {command:"get", id:<projectId>} to read notes, or google_drive search by name. For the latest platform features document use path "data/VisionClaw-Comprehensive-Features.txt".` };
      }
    }
  }
