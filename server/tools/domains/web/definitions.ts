...
export const scraplingScrapeDefinition: ToolDefinition = {
  type: "function",
  function: {
    name: "scrapling_scrape",
    description: "Extract one public HTTPS page with Scrapling; use as a separately paced lane when web_fetch or Firecrawl is blocked.",
    parameters: {
      type: "object",
      properties: {
        url: { type: "string", description: "Public HTTPS URL to extract" },
        mode: { type: "string", enum: ["static"], description: "Governed pinned static extraction mode" },
      },
      required: ["url"],
    },
  },
...
export const multiSourceCollectDefinition: ToolDefinition = {
  type: "function",
  function: {
    name: "multi_source_collect",
    description: "Collect one public URL through staggered web, Firecrawl, and Scrapling lanes, then deduplicate partial results.",
    parameters: {
      type: "object",
      properties: {
        url: { type: "string", description: "Public HTTPS URL to collect" },
        lanes: { type: "array", maxItems: 3, items: { type: "string", enum: ["web_fetch", "firecrawl_scrape", "scrapling_scrape"] } },
        scraplingMode: { type: "string", enum: ["static"] },
        staggerMs: { type: "number", description: "Server-clamped delay between lanes, 250-10000ms" },
      },
      required: ["url"],
    },
  },
