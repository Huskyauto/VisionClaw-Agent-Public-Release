...
export async function webFetch(url: string) {
  ...
  let upstreamLimit: { status: number; retryAfter?: string; retryAfterMs?: number } | undefined;
  const captureLimit = (resp: Response) => {
    const retryAfter = resp.headers.get("retry-after") || undefined;
    if (resp.status !== 429 && !retryAfter) return;
    const seconds = retryAfter === undefined ? NaN : Number(retryAfter);
    const at = retryAfter === undefined ? NaN : Date.parse(retryAfter);
    upstreamLimit = {
      status: resp.status,
      retryAfter,
      retryAfterMs: Number.isFinite(seconds) && seconds >= 0
        ? seconds * 1000
        : Number.isFinite(at) ? Math.max(0, at - Date.now()) : undefined,
    };
  };
  ...
  try {
    ...
    const html = await readResponseTextBounded(resp, 5 * 1024 * 1024);
  ...
  try {
    ...
    const text = await readResponseTextBounded(resp, 5 * 1024 * 1024);
  ...
  try {
    ...
    return {
      success: false,
      url,
      status: upstreamLimit?.status,
      retryAfter: upstreamLimit?.retryAfter,
      retryAfterMs: upstreamLimit?.retryAfterMs,
      error: `All extraction methods failed. Last: ${basicErr.message}`,
...
export async function webSearch(query: string) {
  ...
  if (isPerplexityAvailable()) {
    // Cached (30m TTL, query-keyed) — orchestrated multi-page reviews repeat
    // identical searches across steps/personas; the direct path previously
    // paid a fresh provider call (and a rate-limit slot) every time while
    // only parallel_research used the cache (Bob 2026-08-03 rate-limit fix).
    // Results depend only on the query, so the shared global slot is fine.
    const { cachedPerplexitySearch } = await import("../../../agentic/cached-tools");
    const result = await cachedPerplexitySearch(query);
...
async function scraplingScrapeHandler(params: Record<string, any>, ctx: ToolContext): Promise<ToolResult> {
  if (!ctx.tenantId) return { error: "Tenant context required for scrapling_scrape" };
  const { claimWebDomainUsage } = await import("../../../lib/web-domain-budget");
  const domainClaim = await claimWebDomainUsage(ctx.tenantId, params.url);
  if (!domainClaim.allowed) {
    return {
      success: false,
      domainCooldown: Boolean(domainClaim.retryAfterMs),
      retryAfterMs: domainClaim.retryAfterMs,
      error: domainClaim.reason,
    };
  }
  const { scraplingScrape } = await import("../../../scrapling-client");
  const { wrapExternalContent } = await import("../../../external-content-security");
  const result = await scraplingScrape(params.url, { mode: params.mode });
  if (!result.ok || !result.text) {
    if ((result.status === 429 || result.retryAfterMs) && result.retryAfterMs) {
      const { reportDomainCooldown } = await import("../../../multi-source-collector");
      await reportDomainCooldown(ctx.tenantId, params.url, result.retryAfterMs);
    }
    return { success: false, engine: "scrapling", status: result.status, retryAfter: result.retryAfter, retryAfterMs: result.retryAfterMs, error: result.error };
  }
  const { wrapped, suspicious } = wrapExternalContent(result.text.slice(0, 12_000), "web_fetch", { url: result.finalUrl || params.url });
  return {
    success: true, engine: "scrapling", mode: result.mode, status: result.status, url: result.finalUrl || params.url,
    title: result.title, content: wrapped, truncated: result.text.length > 12_000,
    suspiciousPatterns: suspicious.length, elapsedMs: result.elapsedMs,
  };
}

async function multiSourceCollectHandler(params: Record<string, any>, ctx: ToolContext): Promise<ToolResult> {
  if (!ctx.tenantId) return { error: "Tenant context required for multi_source_collect" };
  const { collectFromMultipleSources } = await import("../../../multi-source-collector");
  return collectFromMultipleSources(params.url, ctx.tenantId, {
    lanes: params.lanes,
    mode: params.scraplingMode,
...
export const webDomainTools: RegisteredTool[] = [
  ...
  defineTool(scraplingScrapeDefinition, scraplingScrapeHandler),
  defineTool(multiSourceCollectDefinition, multiSourceCollectHandler),
