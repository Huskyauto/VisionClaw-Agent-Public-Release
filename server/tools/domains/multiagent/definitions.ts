...
export const ensembleQueryDefinition: ToolDefinition = {
  ...
  function: {
    name: "ensemble_query",
    description: "Mixture-of-Agents (MoA): when you face a hard reasoning, factual, or judgment question and want the most reliable answer, run the same question through diverse models in parallel and have a strong aggregator (Claude Opus 5, fallback GPT-5.6 Sol) synthesize the best combined answer. PREFER proposer_pool='premium' for hard reasoning (3 cheapest frontier-class drafters — DeepSeek V4 Pro 0813 / GLM-5.2 / Gemini 3.7 Flash — then aggregator redraft; near-frontier quality at a fraction of the cost). NOTE: the owner-metered override (with its daily $ ceiling) applies ONLY to the owner/admin tenant; other tenants use paid proposer ids only when the global metered flag permits, else they remap to free lanes. Use SPARINGLY — slower than a normal call. Best for: ambiguous research questions, multi-step reasoning where you're unsure, factual claims you want cross-checked, code or design decisions with multiple valid approaches. Do NOT use for: simple chat, tool routing, anything where one good answer is obviously enough. R125+1 — `proposer_pool` selects the proposer set: 'premium' (recommended), 'frontier' (default — 4 top-tier proposers: Claude Opus 5, GPT-5.6 Sol, Gemini 3.5 Flash, DeepSeek V4 Pro 0813), 'cheap' (5 lineage-diverse OpenRouter), 'mixed' (4 frontier + 3 cheap), or R125+13.18 'polarity' (4 frontier models, each running a DIFFERENT reasoning-tradition system prompt — Munger inversion / Taleb tail-risk / Kahneman bias-audit / Meadows systems-loops — forces genuinely different reasoning paths). R125+13.18 — `restate_gate` runs a fast pre-round where each proposer reframes the question; if restatements diverge, response sets `questionAmbiguous=true`. `dissent_quota` watches κ and, if proposers agree above 70%, spawns 2 extra steelman proposers to argue the opposing view (anti-groupthink).",
    parameters: {
      type: "object",
      properties: {
        ...
        proposer_pool: { type: "string", enum: ["premium", "frontier", "cheap", "mixed", "polarity"], description: "PREFER 'premium' for hard reasoning/high-stakes drafting (Bob's 2026-08-13 best-of-the-best program): the 3 cheapest frontier-class models draft in parallel, then the aggregator redrafts — near-frontier quality at a fraction of the cost. 'frontier' uses the top-tier proposer set (4 hardcoded; a valid data/model-tiers.json override replaces it); 'cheap' uses 5 lineage-diverse OpenRouter cheap models; 'mixed' uses the frontier set + 3 cheap; 'polarity' uses 4 frontier models each running a different reasoning-tradition system prompt (Munger inversion / Taleb tail-risk / Kahneman bias-audit / Meadows systems-loops) — use for strategic decisions wanting maximally different reasoning paths." },
