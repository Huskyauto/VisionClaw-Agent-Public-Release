...
async function ensembleQueryHandler(
  ...
  const validatedPool = (poolRaw === "frontier" || poolRaw === "cheap" || poolRaw === "mixed" || poolRaw === "polarity" || poolRaw === "premium")
    ? poolRaw as "frontier" | "cheap" | "mixed" | "polarity" | "premium"
    : undefined;
  ...
  return {
    ...
    proposers: r.proposers.map(p => ({ model: p.modelId, provider: p.provider, requestedModel: p.requestedModel, actualModel: p.actualModel, providerLane: p.providerLane, routeReason: p.routeReason, failureKind: p.failureKind, retryAfterMs: p.retryAfterMs, ok: p.ok, latencyMs: p.latencyMs, error: p.error, answerLen: p.answer?.length || 0, label: p.label, role: p.role })),
...
async function juryTriageHandler(
  ...
  return {
    ...
    votes: d.votes.map(v => ({ model: v.model, provider: v.provider, requestedModel: v.requestedModel, actualModel: v.actualModel, providerLane: v.providerLane, routeReason: v.routeReason, failureKind: v.failureKind, retryAfterMs: v.retryAfterMs, verdict: v.verdict, rationale: v.rationale, ok: v.ok })),
    fixProposal: d.fixProposal,
    aggregatorAnswer: d.aggregatorAnswer,
    totalLatencyMs: d.totalLatencyMs,
    loggedAs: d.loggedAs,
    typesafeValidation: d.typesafeValidation,
  };
}
...
async function secondOpinionHandler(
  ...
  return await getSecondOpinion({
    ...
    tenantId: ctx.tenantId,
    invokedVia: params._invokedVia || "tool",
    mode: params.mode === "single_model_fallback" ? "single_model_fallback" : "fusion",
  });
}
