...
export async function lookupSamCompanyCageHandler(params: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    const tierLabel = typeof params.tierLabel === "string" ? params.tierLabel.trim().slice(0, 100) : "";
    const cmmc = getCmmcProspectingService();
    const flightKey = `${ctx.tenantId}:${includeInactive}:${tierLabel}:${JSON.stringify(companyNames)}`;
