...
async function writeScratchpadHandler(
  ...
  // stamp a tenant — never write a scratchpad row with an undefined tenant.
  if (!ctx.tenantId) return { success: false, message: "Tenant context required" };
...
async function readScratchpadHandler(
  ...
  // Fail CLOSED when the dispatcher did not stamp a tenant (mirrors the write
  // handler) — an undefined tenant must never silently return empty results,
  // as that can mask a dispatcher identity-propagation regression.
  if (!ctx.tenantId) return { success: false, message: "Tenant context required" };
