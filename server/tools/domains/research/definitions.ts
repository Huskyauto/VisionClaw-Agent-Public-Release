...
export const deepResearchDefinition: ToolDefinition = {
  ...
  function: {
    name: "deep_research",
    description: "Research multiple web sources and return normalized URLs plus a citation audit. Use for comprehensive, source-backed investigation.",
    parameters: {
      type: "object",
      properties: {
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        rootModel: {
          type: "string",
          description: "Optional override for the root model. Default 'gpt-5.6-sol'. Avoid plain 'gpt-5' (routes to Anthropic which lacks chat.completions).",
        },
        subModel: {
          type: "string",
          description: "Optional override for the sub model. Default 'gpt-5.4'. Should be cheaper/smaller than rootModel.",
        },
      },
...
export const ingestPaperDefinition: ToolDefinition = {
  ...
  function: {
    name: "ingest_paper",
    description: "Ingest a paper for citation. For PDFs with figures, charts, diagrams, or tables, pass image_summaries=true for capped multimodal indexing.",
    parameters: {
      type: "object",
      properties: {
        ...
        image_summaries: { type: "boolean", description: "PDF only. Set true for papers whose figures, diagrams, charts, or tables carry meaning. Renders up to 12 pages and embeds substantive visual summaries into the same knowledge store. Defaults to the INGEST_PDF_IMAGE_SUMMARIES env flag, which is off when unset." },
      },
      required: ["file_path"],
    },
  },
