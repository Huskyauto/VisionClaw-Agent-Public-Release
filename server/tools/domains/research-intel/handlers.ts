...
async function saveEvidenceHandler(
  ...
  return af.saveEvidence({ ...params, tenantId: ctx.tenantId } as any);
}
...
async function queryEvidenceHandler(
  ...
  if (!ctx.tenantId) return { error: "Tenant context required" };
  const af = await import("../../../agentic-features");
  return af.queryEvidence({ ...params, tenantId: ctx.tenantId } as any);
}

async function queryProjectRelationshipsHandler(
  params: Record<string, any>,
  ctx: ToolContext,
): Promise<ToolResult> {
  if (!ctx.tenantId || !ctx.projectId) {
    return { error: "Tenant and project context required for query_project_relationships" };
  }
  const af = await import("../../../agentic-features");
  return af.queryProjectRelationships({
    ...params,
...
async function synthesizeResearchHandler(
  ...
  if (!ctx.tenantId) return { error: "Tenant context required" };
  const af = await import("../../../agentic-features");
  return af.synthesizeResearch({ ...params, tenantId: ctx.tenantId } as any);
}
...
export const researchIntelDomainTools: RegisteredTool[] = [
  ...
  defineTool(queryProjectRelationshipsDefinition, queryProjectRelationshipsHandler),
