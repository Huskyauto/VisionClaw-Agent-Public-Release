  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        retrievalMode: { type: "string", enum: ["baseline", "trial"], description: "Use baseline retrieval, or request the configured provenance trial. Trial remains off unless the owner enables it." },
      },
      required: [],
    },
  },
...
export const queryProjectRelationshipsDefinition: ToolDefinition = {
  type: "function",
  function: {
    name: "query_project_relationships",
    description: "Find source-backed relationships inside the current project. Returns only verified evidence links with cited support; never infers people, affiliations, or facts.",
    parameters: {
      type: "object",
      properties: {
        query: { type: "string", description: "Optional text to match against project research claims and questions" },
        theme: { type: "string", description: "Optional research theme/category filter" },
        minConfidence: { type: "number", description: "Optional minimum confidence score from 0 to 100" },
        limit: { type: "number", description: "Maximum source evidence rows to inspect (default 20, max 100)" },
      },
      required: [],
    },
  },
