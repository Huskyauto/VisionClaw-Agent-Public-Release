...
const TRUST_SIGNAL_KEYS = [
  // Never accept an authoritative tenant from model-provided tool arguments.
  // Migrated handlers must source it only from ToolContext.
  "tenantId",
  "_tenantId",
  "_personaId",
  "_conversationId",
  ...
  "_projectId",
  "_allowedPaths",
  // Speculative-prefetch recursion guard (server/speculative-prefetch.ts) —
  // internal dispatcher metadata only; handlers must never see it (architect
  // finding: caller-controlled control-channel leak otherwise).
...
export function stripTrustSignals(params: Record<string, any>): Record<string, any> {
  ...
  // Server-owned capabilities are private non-enumerable Symbols, so object
  // spread intentionally omits them. Preserve those opaque capabilities across
  // this dispatcher clone without knowing or trusting any particular Symbol:
  // their owning module alone decides whether a copied key grants authority.
  for (const key of Object.getOwnPropertySymbols(params)) {
