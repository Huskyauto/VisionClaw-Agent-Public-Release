...
export function startClaudeRunnerBridge(): Promise<boolean> {
  return serializeBridgeLifecycle(async () => {
    if (bridgeRunning) return true;
    if (!isClaudeRunnerSubscriptionConfigured()) {
      console.log("[claude-runner] CLAUDE_CODE_OAUTH_TOKEN is not configured; subscription bridge disabled (refusing ANTHROPIC_API_KEY metered fallback)");
      return false;
    }
