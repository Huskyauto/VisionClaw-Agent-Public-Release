...
const ENTITLEMENT_PROBE_TIMEOUT_MS = 30_000;
const CLAUDE_SUBSCRIPTION_TOKEN_PATTERN = /^sk-ant-oat\d+-[A-Za-z0-9_-]{20,}$/;

let bridgeRunning = false;
let bridgeHealthy = false;
let activeSubscriptionToken: string | null = null;
let bridgeServer: ReturnType<typeof createServer> | null = null;
let bridgeLifecycleTail: Promise<void> = Promise.resolve();
let totalRequests = 0;
let totalErrors = 0;
...
const ENV_ALLOWLIST = new Set([
  ...
  // CLAUDE_CODE_OAUTH_TOKEN = Claude Pro/Max subscription token (minted via
...
export function buildSafeEnv(): Record<string, string> {
  ...
  if (isClaudeRunnerSubscriptionConfigured(env)) {
    delete env.ANTHROPIC_API_KEY;
  }
  return env;
}

export function buildClaudeSubscriptionEnv(token: string): Record<string, string> {
  const env = buildAllowlistedEnv(process.env);
...
function getClaudeRunnerSubscriptionToken(
  env: Readonly<Record<string, string | undefined>> = process.env,
): string | null {
  const token = env.CLAUDE_CODE_OAUTH_TOKEN?.trim() || "";
  return CLAUDE_SUBSCRIPTION_TOKEN_PATTERN.test(token) ? token : null;
}

/**
 * Claude Runner is a subscription lane, not an Anthropic API-key convenience
 * wrapper. Keep this check separate from buildSafeEnv(): the SDK can still use
 * the API key for explicitly metered paths, but this bridge must never label
 * that spend as a $0 subscription request.
 */
export function isClaudeRunnerSubscriptionConfigured(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  return getClaudeRunnerSubscriptionToken(env) !== null;
}
...
export function mapModelId(model: string): string {
  ...
  // claude-haiku-4-5 — plus the family aliases opus/sonnet/haiku.
...
function startQuery(prompt: string, model: string, systemPrompt: string | undefined, requestId: string) {
  if (!activeSubscriptionToken) {
    throw new Error("Claude subscription is not active");
  }
  ...
  return query({
    ...
    options: {
      ...
      env: buildClaudeSubscriptionEnv(activeSubscriptionToken),
      cwd: process.cwd(),
...
function abortAllLiveQueries(reason: string): void {
...
async function handleNonStreamingResponse(
  ...
  const terminalSuccess = resultMsg?.subtype === "success" && resultMsg?.is_error !== true;
  if (!resultText || !terminalSuccess) {
    recordFailure();
    if (!res.headersSent && !res.writableEnded) {
      res.writeHead(502, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: { message: resultMsg ? `Claude query failed (${resultMsg.subtype || "error"})` : "Claude query ended without a successful terminal result", type: "server_error" } }));
    }
    return;
  }
...
async function handleStreamingResponse(
  ...
  if (sawErrorResult || !sawText || !sawSuccessfulResult) {
    recordFailure();
    const message = sawErrorResult
      ? "Claude query failed"
      : !sawText
        ? "Claude query produced no output"
        : "Claude query ended without a successful terminal result";
    if (!headersSent && !res.headersSent && !res.writableEnded) {
      res.writeHead(502, { "Content-Type": "application/json" });
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === "GET" && req.url === "/v1/models") {
    ...
    res.end(JSON.stringify({
      object: "list",
      data: [
        { id: "claude-opus-5", object: "model", owned_by: "claude-runner" },
        ...
        { id: "claude-opus-4-5", object: "model", owned_by: "claude-runner" },
        { id: "claude-sonnet-4-5", object: "model", owned_by: "claude-runner" },
      ],
    }));
    return;
  }
  ...
  const model = parsed.model ?? "claude-opus-5";
...
function attemptListen(): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    server.once("error", (err: any) => {
      if (bridgeServer === server) {
        ...
        abortAllLiveQueries("bridge_server_error");
        if (server.listening) server.close();
      }
      if (err.code === "EADDRINUSE") {
        console.error(`[claude-runner] Port ${BRIDGE_PORT} is already occupied; bridge disabled without disturbing its owner`);
      } else {
...
function serializeBridgeLifecycle<T>(operation: () => Promise<T>): Promise<T> {
  const run = bridgeLifecycleTail.then(operation, operation);
  bridgeLifecycleTail = run.then(
    () => undefined,
    () => undefined,
  );
  return run;
...
async function verifyClaudeCliAvailable(token: string): Promise<boolean> {
  try {
    const { execSync } = await import("node:child_process");
    const version = execSync("npx claude --version 2>/dev/null", {
      timeout: 10_000,
      env: buildClaudeSubscriptionEnv(token),
    }).toString().trim();
...
async function verifyClaudeSubscriptionEntitlement(token: string): Promise<boolean> {
  const abort = new AbortController();
  const timeout = setTimeout(() => abort.abort(), ENTITLEMENT_PROBE_TIMEOUT_MS);
  try {
    const probe = query({
      prompt: "Reply with OK.",
      options: {
        ...buildQueryOptions("claude-haiku-4-5", undefined),
        maxTurns: 1,
        env: buildClaudeSubscriptionEnv(token),
        cwd: process.cwd(),
        abortController: abort,
    ...
    for await (const message of probe as AsyncIterable<any>) {
...
type ClaudeRunnerStartupDependencies = {
  verifyCli?: (token: string) => Promise<boolean>;
  verifyEntitlement?: (token: string) => Promise<boolean>;
  listen?: () => Promise<boolean>;
};

export function startClaudeRunnerBridge(
  dependencies: ClaudeRunnerStartupDependencies = {},
): Promise<boolean> {
  return serializeBridgeLifecycle(async () => {
    if (bridgeRunning) return true;
    const subscriptionToken = getClaudeRunnerSubscriptionToken();
    if (!subscriptionToken) {
      console.log("[claude-runner] CLAUDE_CODE_OAUTH_TOKEN is missing or invalid; subscription bridge disabled (refusing ANTHROPIC_API_KEY metered fallback)");
      return false;
    }

    const verifyCli = dependencies.verifyCli ?? verifyClaudeCliAvailable;
    if (!await verifyCli(subscriptionToken)) {
      return false;
    }

    const verifyEntitlement = dependencies.verifyEntitlement ?? verifyClaudeSubscriptionEntitlement;
    if (!await verifyEntitlement(subscriptionToken)) {
      console.log("[claude-runner] Claude subscription entitlement probe failed; bridge disabled");
      return false;
    }

    const listen = dependencies.listen ?? attemptListen;
    activeSubscriptionToken = subscriptionToken;
    const listening = await listen();
    if (!listening) activeSubscriptionToken = null;
    return listening;
  });
}
...
export async function stopClaudeRunnerBridge(): Promise<void> {
  await serializeBridgeLifecycle(async () => {
    abortAllLiveQueries("bridge_shutdown");
    if (bridgeServer) {
