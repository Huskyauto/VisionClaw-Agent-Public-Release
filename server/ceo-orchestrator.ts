...
[
  ...
  const fallbackModels = [model, "claude-opus-4-7", "gpt-5.4", "claude-opus-4-6", "gpt-4.1", "claude-sonnet-4-20250514"];
  let client: any;
  ...
  const buildParams = (modelId: string, base: Record<string, any>) => {
    const isOpus47Plus = /^claude-opus-4-7/.test(modelId) || /claude-opus-4-7/.test(actualModel || "");
    if (isOpus47Plus) {
  ...
  try {
    ...
    for (const retryModel of ["claude-opus-4-7", "claude-opus-4-6", "gpt-4.1", "claude-sonnet-4-20250514"]) {
      try {
