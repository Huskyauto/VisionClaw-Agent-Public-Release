...
// Task 104 girth split — persona routing/classification + plan synthesis live in
// ceo-persona-routing.ts / ceo-plan-synthesis.ts; re-exported so importers are unchanged.
import {
  classifyStepMode,
  compressWarRoomEntry,
  matchPersona,
  getRoleGuidance,
} from "./ceo-persona-routing";
export {
  classifyStepMode,
  compressWarRoomEntry,
  getRoleGuidanceForDelegation,
  isCasualChat,
  isComplexRequest,
} from "./ceo-persona-routing";
export { synthesizeResults } from "./ceo-plan-synthesis";
import { assessBudgetPhase } from "./agentic/goal-contract";
import { extractTrustedDeliverableLinks } from "./lib/orchestration-deliverable-links";
/**
...
export interface OrchestrationStep {
  ...
  model?: string;
  /** tool names ACTUALLY invoked during this step (from processMessage's
   *  toolsUsed), as opposed to toolChain which is only the INTENDED surface.
   *  Consumed by the completion evaluator's consequential-actions risk gate
   *  (Sol #3): a degraded judge on a run that fired non-safe tools must return
   *  completed_unverified, never a clean "done". */
  toolsUsed?: string[];
  /** Deliverable URLs extracted only from approved producer tool outputs. */
...
Use this orchestration pattern as a guide:\n${formatCrossWorkflowForPrompt(classification.crossDepartment)}\nAdapt the steps to the specific objective but follow the agent assignments and parallel/sequential structure.`;
  ...
  try {
    ...
    if (hit) {
      ...
      // Speculative read-only prefetch (arXiv:2607.25816 adaptation): the
      // replayed plan names its tools upfront, so pre-warm the safe zero-arg
      // subset while the first step's LLM turn is still streaming.
      // Fire-and-forget, fail-open, kill switch SPEC_PREFETCH_DISABLED=1.
      try {
        const { prefetchForReplayPlan } = await import("./speculative-prefetch");
        prefetchForReplayPlan(replayedPlan.steps, tenantId);
      } catch (_silentErr) { logSilentCatch("server/ceo-orchestrator.ts", _silentErr); }
      return replayedPlan;
    }
  } catch (_silentErr) { logSilentCatch("server/ceo-orchestrator.ts", _silentErr); }

  // Episode playbooks (Kimi K3 #2): step-level trajectories from similar past
  // successful runs, injected as ADVISORY planner context. Fail-open.
  let playbookHint = "";
  try {
    const { retrieveEpisodePlaybooks, formatPlaybooksForPlanner } = await import("./episode-playbooks");
    const hits = await retrieveEpisodePlaybooks(objective, tenantId);
    playbookHint = formatPlaybooksForPlanner(hits);
    if (hits.length > 0) console.log(`[ceo] Episode playbooks: ${hits.length} advisory hit(s) (best similarity ${hits[0].similarity.toFixed(3)})`);
  } catch (_silentErr) { logSilentCatch("server/ceo-orchestrator.ts", _silentErr); }

  // Falsified routes (Argus, arXiv:2608.05144): similar past FAILED plans,
  // injected as an ADVISORY warning so the planner avoids re-planning a route
  // the system already proved wrong. Fail-open.
  let falsifiedHint = "";
  try {
    const { retrieveFalsifiedRoutes, formatFalsifiedRoutesForPlanner } = await import("./falsified-routes");
    const fHits = await retrieveFalsifiedRoutes(objective, tenantId);
    falsifiedHint = formatFalsifiedRoutesForPlanner(fHits);
    if (fHits.length > 0) console.log(`[ceo] Falsified routes: ${fHits.length} advisory warning(s) (best similarity ${fHits[0].similarity.toFixed(3)})`);
  } catch (_silentErr) { logSilentCatch("server/ceo-orchestrator.ts", _silentErr); }
...
${crossWorkflowHint}${commitmentHint}${situationContext}${playbookHint}${falsifiedHint}
Objective: ${objective}
...
[
  ...
  const fallbackModels = [model, "gpt-5.6-sol", "gpt-5.4", "google/gemini-3.7-flash", "gpt-5", "claude-sonnet-4-5"];
  let client: any;
  ...
  try {
    ...
    for (const retryModel of ["gpt-5.6-sol", "google/gemini-3.7-flash", "gpt-5", "claude-sonnet-4-5"]) {
      try {
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      steps = parsed.map((t: any) => ({
        ...
        assignedPersona: matchPersona(t.required_skill_type || "general", tenantId),
  ...
  for (const step of steps) {
    ...
    // Pairing invariant (2026-08-04 incident): create_pdf must NEVER travel
    // alone. Lean orchestrated steps mount EXACTLY this chain (no router), so
    // a chain with only the low-level create_pdf leaves the premium
    // create_styled_report uncallable and the persona ships a downgraded
    // deliverable. If any PDF capability is in the chain, both tiers ride.
    if (step.toolChain.includes("create_pdf") && !step.toolChain.includes("create_styled_report")) {
      step.toolChain.push("create_styled_report");
    }
...
async function _executePlanImpl(
  ...
  const { buildCeoExecutionAttemptId } = await import("./delegation-supervisor");
  ...
  while (round < MAX_ROUNDS) {
    ...
    const executeStep = async (
      ...
      executionAttempt = buildCeoExecutionAttemptId("initial"),
    ) => {
      ...
      // any step writes its heartbeat_log row.
      //
      // Failure posture (review fix): the DB-backed checkPace may soft-fail
      // open (availability — a transient DB blip must not halt orchestration;
      // spend is separately bounded by the autonomous-budget claim), but the
      // IN-MEMORY reservation cap is enforced unconditionally, so a pacing-
      // store outage can never permit unbounded parallel dispatch.
      ...
      try {
        ...
        if (inFlight >= personaCap) {
          const reason = `In-flight reservations at cap: ${inFlight} >= ${personaCap}`;
        ...
        try {
          ...
          if (!pace.allowed || pace.used + inFlight >= personaCap) {
            const reason = pace.reason || `Window usage would exceed cap: ${pace.used}+${inFlight} >= ${personaCap}`;
            console.warn(`[ceo] Pace cap hit on step ${step.taskId} → ${step.assignedPersona}: ${reason}`);
          ...
          console.warn(`[ceo] DB pace check failed (in-memory cap still enforced):`, dbErr?.message || dbErr);
        }
      } catch (paceErr: any) {
        console.warn(`[ceo] Pace gate unavailable (module load failed):`, paceErr?.message || paceErr);
      }
...
- Writing a "content brief" or "outline" as text when you were asked to CREATE the actual deliverable is a FAILURE. Use the tool.`;
      ...
      try {
        ...
        await reportDelegationAttempt({
          tenantId: plan.tenantId,
          requesterIdentity: "ceo-orchestrator",
          targetAgent: step.assignedPersona || "VisionClaw",
          supervisionMode: "tiered",
          source: "internal",
          delegationId: `ceo:${plan.id}:${step.taskId}`,
...
Return your output as JSON: {"content": "your full output text here", "keyFindings": ["bullet1", "bullet2", ...], "deliverables": ["any URLs or file references"]}`,
        ...
        if (!step.leanMode) {
          ...
          const childConv = await storage.createConversation({
            ...
            model: plan.modelId || "gpt-5.6-sol",
          ...
          const stepPromise = processMessage(
            childConv.id,
            taskPrompt,
            { tenantId: plan.tenantId, enableTools: true, depth: stepDepth, toolFilter: useToolFilter, leanContext: useLeanContext }
          );
          ...
          try {
            ...
            if (timeoutErr.message?.includes("timed out")) {
              ...
              if (settled.ok && settled.result) {
                ...
                await reportDelegationAttempt({
                  tenantId: plan.tenantId,
                  requesterIdentity: "ceo-orchestrator:timeout-retry",
                  targetAgent: targetPersona?.name || step.assignedPersona || "VisionClaw",
                  supervisionMode: "tiered",
                  source: "internal",
                  delegationId: `ceo:${plan.id}:${step.taskId}`,
                  attemptId: `${executionAttempt}:timeout-retry`,
                });
                ...
                result = await Promise.race([
                  processMessage(
                    timeoutRetryConv.id,
                    shortenedPrompt,
                    { tenantId: plan.tenantId, enableTools: true, depth: stepDepth, toolFilter: step.toolChain, leanContext: true }
                  ),
                  new Promise((_, reject) =>
          ...
          // Record the tools ACTUALLY invoked (names) for the completion
          // evaluator's consequential-actions gate. processMessage returns
          // toolsUsed as {name,...} objects on some paths and plain name
          // strings on others — normalize both. Best-effort; never throws.
          try {
            const rawTools: any[] = Array.isArray(result?.toolsUsed) ? result.toolsUsed : [];
            const names = rawTools
              .map((t: any) => (typeof t === "string" ? t : t?.name))
              .filter((n: any): n is string => typeof n === "string" && n.trim().length > 0);
            if (names.length > 0) step.toolsUsed = [...new Set(names)];
          } catch (_silentErr) { logSilentCatch("server/ceo-orchestrator.ts", _silentErr); }
          ...
          if (needsEscalation) {
            ...
            await reportDelegationAttempt({
              tenantId: plan.tenantId,
              requesterIdentity: "ceo-orchestrator:escalation",
              targetAgent: targetPersona?.name || step.assignedPersona || "VisionClaw",
              supervisionMode: "tiered",
              source: "internal",
              delegationId: `ceo:${plan.id}:${step.taskId}`,
              attemptId: `${executionAttempt}:escalation`,
            });
            ...
            // BOUNDED escalation (Bob 2026-08-03 — "CEO Orchestrating" hung 500+s
            // twice on PDF-flowchart plans). This was the ONLY processMessage call
            // in the step executor with no timeout race; a stuck tool chain here
            // wedged the entire plan indefinitely. Escalation is a best-effort
            // improvement pass, so on timeout we KEEP the lean result and move on
            // rather than failing the step. processMessage cannot be aborted
            // (signal-less), so like the other races the original may still finish
            // in the background — we log if it does, and never re-fire tools.
            const ESCALATION_TIMEOUT_MS = STEP_TIMEOUT_MS;
            const escalationPromise = processMessage(
              retryConv.id,
              taskPrompt,
              { tenantId: plan.tenantId, enableTools: true, depth: stepDepth }
            );
            let escalationResult: any = null;
            let escalationTimer: NodeJS.Timeout | undefined;
            try {
              escalationResult = await Promise.race([
                escalationPromise,
                new Promise((_, reject) => {
                  escalationTimer = setTimeout(() => reject(new Error(`escalation timed out after ${ESCALATION_TIMEOUT_MS / 1000}s`)), ESCALATION_TIMEOUT_MS);
                }),
              ]);
              clearTimeout(escalationTimer);
            } catch (escErr: any) {
              clearTimeout(escalationTimer);
              console.warn(`[ceo] Step ${step.taskId} full-context escalation ${escErr?.message?.includes("timed out") ? "TIMED OUT" : `failed (${escErr?.message ?? escErr})`} — keeping lean result (${resultText.length} chars)`);
              storage.updateConversation(retryConv.id, { title: `[ABANDONED] ${retryConv.title}` }, retryConv.tenantId ?? ADMIN_TENANT_ID).catch(() => {});
              escalationPromise
                .then((r: any) => console.warn(`[ceo] Step ${step.taskId} escalation ORIGINAL CALL completed AFTER abandonment (length ${JSON.stringify(r ?? "").length}) — result discarded, no re-fire`))
                .catch(() => { /* original errored — nothing to discard */ });
            }

            if (escalationResult) {
              const retryText = escalationResult?.response || JSON.stringify(escalationResult);
              if (retryText.length > resultText.length) {
                result = escalationResult;
                resultText = retryText;
                console.log(`[ceo] Step ${step.taskId} escalation succeeded — full context produced ${retryText.length} chars`);
              } else {
          ...
          try {
            ...
            if (result?.toolsUsed?.length) {
              ...
              for (const tool of result.toolsUsed) {
                for (const link of extractTrustedDeliverableLinks(tool)) foundLinks.add(link);
              }
            }
            ...
            if (foundLinks.size > 0) {
              ...
              step.deliverableLinks = allLinks;
              console.log(`[ceo] Recorded ${allLinks.length} provenance-backed deliverable link(s) for step ${step.taskId}`);
            }
          } catch (e: any) {
        ...
        if (retryAttempt < 1) {
          ...
          try {
            await executeStep(step, buildCeoExecutionAttemptId("self-correction", 1));
            ...
            if (backupAgent && !alreadyTriedBackup) {
              ...
              try {
                await executeStep(step, buildCeoExecutionAttemptId("backup", 1, backupAgent));
                step.description = originalDesc;
  ...
  // Falsified routes (Argus, arXiv:2608.05144): retain FAILED plans so the
  // planner is warned before re-planning the same approach on a similar goal.
  // Fire-and-forget + fail-open — never slows or blocks the failure path.
  if (plan.status === "failed") {
    try {
      const { recordFalsifiedRoute } = await import("./falsified-routes");
      const failedSteps = plan.steps.filter(s => s.status === "failed");
      const reason = failedSteps.map(s => s.error).filter(Boolean).slice(0, 3).join("; ") || "plan failed (steps incomplete)";
      recordFalsifiedRoute({
        tenantId: plan.tenantId,
        objective: plan.objective,
        steps: plan.steps.map(s => ({ description: s.description, status: s.status, error: s.error })),
        reason,
  ...
  try {
    ...
    const { scoreTrajectory, countRedundantToolChains } = await import("./lib/trajectory-quality");
    const predTimeSec = (plan as any).__predictedTimeSeconds;
    const predCost = (plan as any).__predictedCostUsd;
    // arXiv:2606.24937 (Roitman, ch.20 + 29.2.6) — grade HOW WELL the plan
    // executed, not just whether it finished. Cheap, no-LLM, from the finished
    // step list already in scope.
    const traj = scoreTrajectory({
      steps: plan.steps.length,
      completed: completedCount,
      failed: failedCount,
      redundantSteps: countRedundantToolChains(plan.steps.map(s => s.toolChain)),
    });
    void recordOrchestrationEfficiency({
      ...
      heavyLoopUsed: true,
      guardVerdict: "worth",
      processQuality: traj.processQuality,
      completionRate: traj.completionRate,
      failureRate: traj.failureRate,
      redundancyRate: traj.redundancyRate,
      steps: plan.steps.length,
    });
  } catch (_silentErr) { logSilentCatch("server/ceo-orchestrator.ts", _silentErr); }
  ...
  import("./auto-skillify").then(({ autoSkillCapture }) => {
    autoSkillCapture({
      ...
      steps: plan.steps.map(s => ({
        ...
        toolsUsed: (s.toolsUsed && s.toolsUsed.length > 0) ? s.toolsUsed : (s.toolChain || []),
        status: s.status,
        leanMode: s.leanMode,
      })),
