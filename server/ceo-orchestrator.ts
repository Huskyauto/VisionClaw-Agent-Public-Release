...
${crossWorkflowHint}${situationContext}
Objective: ${objective}
...
[
  ...
  const fallbackModels = [model, "gpt-5.4", "claude-opus-4-6", "gpt-4.1", "claude-sonnet-4-20250514"];
  let client: any;
  ...
  try {
    ...
    for (const retryModel of ["claude-opus-4-6", "gpt-4.1", "claude-sonnet-4-20250514"]) {
      try {
        const retryResult = await getClientForModel(retryModel, tenantId);
        response = await retryResult.client.chat.completions.create({
  ...
  for (const step of steps) {
    ...
    if (/drive|upload|save|file/i.test(descLower)) extras.push("google_drive");
    if (/code|script|execute/i.test(descLower)) extras.push("execute_code");
...
export async function executePlan(
  ...
  import("./agent-activity").then(({ updateLiveStatus }) => {
    updateLiveStatus(2, "Felix", "orchestrating", plan.objective.slice(0, 100));
    for (const step of plan.steps) {
      const pid = cachedPersonas.find(p => p.name === step.assignedPersona)?.id;
      if (pid) updateLiveStatus(pid, step.assignedPersona, "queued", step.description.slice(0, 80));
    }
  }).catch(() => {});
  ...
  while (round < MAX_ROUNDS) {
    ...
    const executeStep = async (step: OrchestrationStep) => {
      ...
      import("./agent-activity").then(({ updateLiveStatus }) => {
        const pid = cachedPersonas.find(p => p.name === step.assignedPersona)?.id;
        if (pid) updateLiveStatus(pid, step.assignedPersona, "active", step.description.slice(0, 80));
      }).catch(() => {});
...
${contextFromDeps ? `\nINPUT FROM PREVIOUS STEPS:${contextFromDeps}` : ""}
...
Return your output as JSON: {"content": "your full output text here", "keyFindings": ["bullet1", "bullet2", ...], "deliverables": ["any URLs or file references"]}`,
          ...
          try {
            ...
            if (timeoutErr.message?.includes("timed out")) {
              ...
              if (settled.ok && settled.result) {
                ...
                console.warn(`[ceo] Step ${step.taskId} original call did not settle — marking abandoned and retrying with lean mode`);
                storage.updateConversation(childConv.id, { title: `[ABANDONED] ${childConv.title}` }).catch(() => {});
        ...
        import("./agent-activity").then(({ updateLiveStatus }) => {
          const pid = cachedPersonas.find(p => p.name === step.assignedPersona)?.id;
          if (pid) updateLiveStatus(pid, step.assignedPersona, "idle");
        }).catch(() => {});
        ...
        if (retryAttempt < 1) {
          ...
          const selfCorrectionPrefix = isTimeout
            ...
            : `SELF-CORRECTION RETRY: The previous attempt at this task failed with error: "${(err.message || "Unknown error").slice(0, 200)}". Diagnose what went wrong and try a DIFFERENT approach. If a tool failed, use introspect_tools to check its schema, then retry with corrected parameters. If the approach itself is wrong, try an alternative tool or method. Do NOT repeat the same failing approach.\n\nOriginal task: `;
          ...
          try {
            ...
            if (backupAgent && !alreadyTriedBackup) {
              ...
              step.startedAt = undefined;
              step.completedAt = undefined;
              step.description = `BACKUP AGENT TAKEOVER: ${originalAgent} failed this task twice. You (${backupAgent}) are the backup. The error was: "${(retryErr.message || "").slice(0, 150)}". Take a fresh approach and complete this task.\n\nOriginal task: ${originalDesc}`;
              ...
              import("./agent-activity").then(({ updateLiveStatus }) => {
                const pid = cachedPersonas.find(p => p.name === step.assignedPersona)?.id;
                if (pid) updateLiveStatus(pid, step.assignedPersona, "error", retryErr.message?.slice(0, 60));
              }).catch(() => {});
            }
          }
        } else {
          ...
          import("./agent-activity").then(({ updateLiveStatus }) => {
            const pid = cachedPersonas.find(p => p.name === step.assignedPersona)?.id;
            if (pid) updateLiveStatus(pid, step.assignedPersona, "error", err.message?.slice(0, 60));
          }).catch(() => {});
        }
      }
    };
