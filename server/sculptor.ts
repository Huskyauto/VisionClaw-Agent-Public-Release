...
export async function createAgentSession(params: SculptorSessionParams): Promise<{
  ...
  // Tenant-isolation audit 2026-07-31: a caller-supplied parentSessionId must
  // belong to the SAME tenant before we link it — otherwise a forged id could
  // create a cross-tenant session chain.
  if (params.parentSessionId != null) {
    const parent = await db.execute(sql`
      SELECT id FROM sculptor_sessions WHERE id = ${params.parentSessionId} AND tenant_id = ${params.tenantId} LIMIT 1
    `);
    if (!(((parent as any).rows || parent)[0])) {
      return { success: false, error: `parentSessionId ${params.parentSessionId} not found in this tenant` };
    }
  }
