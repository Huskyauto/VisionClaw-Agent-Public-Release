...
export async function sessionsSend(params: {
  ...
  const delegationId = `intersession:${convId}:${randomUUID()}`;

  try {
    const { reportDelegationAttempt } = await import("./delegation-supervisor");
    await reportDelegationAttempt({
      tenantId: params.tenantId,
      requesterIdentity: sourceLabel,
      targetAgent: `session:${convId}`,
    ...
    const primaryResult = await withTimeout(
      processMessage(convId, taggedMessage, { tenantId: params.tenantId, source: `inter_session:${sourceLabel}` }),
      timeoutMs
    );
    ...
    if (sourceConvId) {
      for (let turn = 0; turn < MAX_PING_PONG_TURNS; turn++) {
        ...
        await reportDelegationAttempt({
          tenantId: params.tenantId,
          requesterIdentity: `session:${convId}`,
          targetAgent: `session:${sourceConvId}`,
          supervisionMode: "tiered",
          source: "internal",
          delegationId,
          attemptId: `reply:${turn + 1}`,
        });
        const followUpResult = await withTimeout(
          processMessage(sourceConvId, followUpMsg, { tenantId: params.tenantId, source: `inter_session_reply:${conv.title || convId}` }),
          timeoutMs
        );
        ...
        await reportDelegationAttempt({
          tenantId: params.tenantId,
          requesterIdentity: `session:${sourceConvId}`,
          targetAgent: `session:${convId}`,
          supervisionMode: "tiered",
          source: "internal",
          delegationId,
          attemptId: `bounce:${turn + 1}`,
        });
        const bounceResult = await withTimeout(
          processMessage(convId, bounceMsg, { tenantId: params.tenantId, source: `inter_session_pingpong:${sourceLabel}` }),
          timeoutMs
        );
