...
import { ssrfSafeUrl } from "./lib/ssrf-jail";   // R98.14 +sec-2 — shared jail (was inline-duplicated)

const FETCH_TIMEOUT_MS = 15000;
const MAX_BYTES = 2 * 1024 * 1024;

async function fetchTextWithCap(url: URL): Promise<{ ok: true; body: string; contentType: string; bytes: number } | { ok: false; reason: string }> {
  ...
  try {
    ...
    const res = await fetch(url.toString(), { signal: ctrl.signal, redirect: "error", headers: { "User-Agent": "VisionClaw-Reference-Learner/1.0 (+https://visionclaw.replit.app)" } });
...
export async function learnFromReference(input: LearnFromReferenceInput): Promise<LearnFromReferenceResult> {
  ...
  if (yt) {
    ...
    const fetched = await fetchTextWithCap(url);
    if (fetched.ok) {
    ...
    if (thumbnailUrl) {
      ...
      if (thumbSafe.ok) {
        try {
          const tRes = await fetch(thumbSafe.url.toString(), { signal: AbortSignal.timeout(8000), redirect: "error" });
          if (tRes.ok) {
