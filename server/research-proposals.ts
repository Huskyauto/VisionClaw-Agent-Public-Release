...
Produce a concrete code proposal to implement this finding in VisionClaw.`,
  ...
  if (newProposalId && validationResult.valid) {
    try {
      ...
      fireAndForgetVerify(newProposalId, session.tenantId);
    } catch (e) {
...
export async function replayHighValueFindings(opts: {
  ...
  for (const row of rows) {
    ...
    const stubSession: ActiveSession = {
      ...
      previousResults: [], discoveryObservations: [], metaNStrategyLayers: [],
      timer: null, experimentInFlight: false,
      evidenceGraph: null,
    };
...
export async function safeApplyProposal(proposalId: number, tenantId: number): Promise<{
  ...
  const authorityReceipt = extractArtifactAuthorityReceipt(proposal.verification_details);
  if (!authorityReceipt) {
    return {
      success: false,
      stage: "artifact_authority",
      error: "Apply blocked: verification has no valid artifact-authority receipt; re-run verification",
      reverted: false,
    };
  }
  const authorityValidation = validateArtifactAuthorityReceipt(authorityReceipt, {
    ...
    subject: {
      ...
      sourceSha256: artifactAuthoritySha256Text(originalContent),
  ...
  if (!authorityValidation.valid) {
    return {
      success: false,
      stage: "artifact_authority",
      error: `Apply blocked: artifact-authority receipt rejected (${authorityValidation.reason})`,
      reverted: false,
    };
  }
  ...
  try {
    ...
    try {
      ...
      const driveResult = await uploadAndShare({
        ...
        tenantId,
        generatedArtifact: { artifactKind: "research_digest", logicalName: `VisionClaw_Research_Digest_${dateStr}.md`, sourceRunKey: `research-digest:${dateStr}` },
      });
      if (driveResult.success && driveResult.viewUrl) {
        ...
        return { success: false, proposalCount: proposals.length, findingCount: findings.length, error: driveResult.error || "Research digest durability verification failed" };
      }
    } catch (driveErr: any) {
      return { success: false, proposalCount: proposals.length, findingCount: findings.length, error: `Research digest is not durable: ${driveErr.message}` };
    }
    ...
    try {
      ...
      await assertMemoryValueNotTombstoned({
        ...
        parts: [safeDigestTitle, safeDigest],
      });
      await db.execute(sql`
