...
export function validateProductIntake(product: CatalogProduct, intake: Record<string, unknown> | undefined): { error?: string; metadata: Record<string, string> } {
  const metadata: Record<string, string> = {};
  if (product.kind !== "service") return { metadata };
  if (!intake || typeof intake !== "object" || Array.isArray(intake)) return { error: "intake required for this product", metadata };
  for (const field of product.intakeFields || []) {
    const raw = intake[field.key];
    if ((raw == null || raw === "") && !field.required) continue;
    if (field.type === "select" && (typeof raw !== "string" || !(field.options || []).some(option => option.value === raw))) {
      return { error: `intake.${field.key} must match a declared option`, metadata };
    }
    const value = raw == null ? "" : String(raw).trim();
    if (field.required && !value) return { error: `intake.${field.key} required`, metadata };
    if (value) metadata[`intake_${field.key}`] = value.slice(0, Math.min(field.maxLength || 500, 500));
  }
  return { metadata };
}
  ...
  app.post("/api/store/checkout", async (req: Request, res: Response) => {
    try {
      ...
      if (product.kind === "service") {
        const intakeResult = validateProductIntake(product, intake);
        if (intakeResult.error) return res.status(400).json({ error: intakeResult.error });
        Object.assign(intakeMetadata, intakeResult.metadata);
      }
