  ...
  app.post("/api/webhooks/agentmail", async (req: Request, res: Response) => {
    try {
      ...
      if (!webhookSecret) {
        ...
        // SECURITY (review fix): HMAC must cover the exact bytes the provider
        // signed. The global express.json parser captures them on req.rawBody;
        // JSON.stringify(req.body) is NOT byte-stable and is only a dev-era
        // fallback if rawBody is somehow absent.
        const rawBody = (req as any).rawBody;
        const body: Buffer | string = Buffer.isBuffer(rawBody)
          ? rawBody
