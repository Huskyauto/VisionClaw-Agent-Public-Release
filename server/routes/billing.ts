...
/**
 * Mirrors the webhook's paid-state gate. A completed Checkout Session can still
 * be unpaid for delayed payment methods, so completion alone is never
 * entitlement evidence.
 */
export function isAuthoritativePaidCheckout(session: {
  status?: string | null;
...
export function registerBillingRoutes(app: Express, helpers: BillingHelpers) {
  ...
  app.post("/api/subscribe/activate", mutateLimiter, async (req: Request, res: Response) => {
    try {
      ...
      if (!isAuthoritativePaidCheckout(session)) {
