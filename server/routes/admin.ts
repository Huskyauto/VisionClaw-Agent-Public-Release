...
type AdminHelpers = {
  ...
  improvementEvidenceReport?: (query: { limit: number; skillId?: number }) => Promise<unknown>;
  capabilityMaturityReport?: (query: { limit: number }) => Promise<unknown>;
  paperExecutionCapsuleCompiler?: (manifest: unknown) => unknown | Promise<unknown>;
};
...
export function registerAdminRoutes(app: Express, helpers: AdminHelpers) {
  ...
  app.get("/api/admin/improvement-evidence", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) {
        return res.status(403).json({ error: "Admin access required" });
      }
      const reportModule = await import("../lib/improvement-evidence-report");
      const query = reportModule.parseImprovementEvidenceQuery(
        req.query as Record<string, unknown>,
      );
      res.json(await (improvementEvidenceReport ?? reportModule.buildImprovementEvidencePayload)(query));
    } catch (err: any) {
      if (/^(?:limit|skillId) must /.test(String(err?.message || ""))) {
        return res.status(400).json({ error: err.message });
      }
      console.error("[improvement-evidence] route failed:", err);
      res.status(500).json({ error: "Improvement evidence report failed" });
    }
  });

  app.get("/api/admin/capability-maturity", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) {
        return res.status(403).json({ error: "Admin access required" });
      }
      const reportModule = await import("../lib/capability-maturity-report");
      const query = reportModule.parseCapabilityMaturityQuery(
        req.query as Record<string, unknown>,
      );
      res.json(await (capabilityMaturityReport ?? reportModule.buildCapabilityMaturityPayload)(query));
    } catch (err: any) {
      if (/^limit must /.test(String(err?.message || ""))) {
        return res.status(400).json({ error: err.message });
      }
      console.error("[capability-maturity] route failed:", err);
      res.status(500).json({ error: "Capability maturity report failed" });
    }
  });

  app.post("/api/admin/paper-execution-capsule", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) {
        return res.status(403).json({ error: "Admin access required" });
      }
      const capsuleModule = await import("../lib/paper-execution-capsule");
      const compile = paperExecutionCapsuleCompiler ??
        capsuleModule.compilePaperExecutionCapsule;
      res.json(await compile(req.body));
    } catch (err: any) {
      const capsuleModule = await import("../lib/paper-execution-capsule");
      if (err instanceof capsuleModule.PaperExecutionCapsuleValidationError) {
        return res.status(400).json({ error: err.message });
      }
      console.error("[paper-execution-capsule] route failed:", err);
      res.status(500).json({ error: "Paper execution capsule compilation failed" });
    }
  });
  ...
  app.get("/api/admin/service-orders/policy", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      for (const sku of listSkus()) {
        // lookupProduct verifies static-product files on disk and THROWS on
        // a missing one. One broken static entry must not 500 the whole
        // policy list (this page only cares about service products anyway).
        let p: ReturnType<typeof lookupProduct>;
        try {
          p = lookupProduct(sku);
        } catch (skuErr: any) {
          console.warn(`[service-orders/policy] Skipping broken catalog entry ${sku}: ${skuErr?.message}`);
          continue;
        }
  ...
  app.post("/api/admin/service-orders/policy/:sku", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      if (product.serviceType === "human-agency-audit" && enabled === true) {
        return res.status(400).json({ error: "Human-agency audits are always manual review" });
      }
  ...
  app.get("/api/admin/service-orders/:id", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) return res.status(403).json({ error: "Admin access required" });
      const { getReviewItem, rehydrateReviewArtifact } = await import("../service-review-queue");
      const item = getReviewItem((req.params.id as string));
      if (!item) return res.status(404).json({ error: "Not found" });
      if (item.sku.startsWith("sample-agency-audit-")) await rehydrateReviewArtifact(item);
  ...
  // Review-queue primary files are PDFs (audit/report) or ZIPs (DFY Fix Kit).
  const reviewItemMime = (fileName: string): string =>
    /\.zip$/i.test(fileName || "") ? "application/zip" : "application/pdf";

  // Manual-only Human Agency audits accept a final operator PDF. The queue
  // remains the source of workflow state; file_storage/object storage is the
  // durable artifact copy, while uploads/ is only the preview/delivery cache.
  app.post("/api/admin/service-orders/:id/attach-pdf", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) return res.status(403).json({ error: "Admin access required" });
      const { getReviewItem, updateReviewItem } = await import("../service-review-queue");
      const { confinedRegularFilePath } = await import("../lib/confined-file-cleanup");
      const item = getReviewItem(req.params.id as string);
      if (!item) return res.status(404).json({ error: "Not found" });
      const previousArtifact = { filePath: item.filePath, artifactFilename: item.artifactFilename, artifactStorageKey: item.artifactStorageKey };
      if (!item.sku.startsWith("sample-agency-audit-")) return res.status(400).json({ error: "PDF attachment is only for human-agency audits" });
      if (item.status !== "pending") return res.status(409).json({ error: "Only pending orders can receive an attachment" });
      const data = typeof req.body?.data === "string" ? req.body.data : "";
      const originalName = typeof req.body?.fileName === "string" ? req.body.fileName : "final-audit.pdf";
      const mimeType = typeof req.body?.mimeType === "string" ? req.body.mimeType.toLowerCase() : "";
      if (mimeType !== "application/pdf") return res.status(415).json({ error: "Only application/pdf is accepted" });
      if (!/\.pdf$/i.test(originalName) || originalName.length > 180) return res.status(400).json({ error: "A bounded .pdf filename is required" });
      if (!/^[A-Za-z0-9._ -]+\.pdf$/i.test(originalName)) return res.status(400).json({ error: "Unsafe filename" });
      if (!/^[A-Za-z0-9+/]*={0,2}$/.test(data) || data.length > 20_000_000) return res.status(413).json({ error: "PDF exceeds the 15 MB limit" });
      const bytes = Buffer.from(data, "base64");
      if (bytes.length === 0 || bytes.length > 15 * 1024 * 1024) return res.status(413).json({ error: "PDF exceeds the 15 MB limit" });
      if (bytes.subarray(0, 5).toString("ascii") !== "%PDF-") return res.status(415).json({ error: "PDF signature is invalid" });
      const fs = await import("fs/promises");
      const path = await import("path");
      const crypto = await import("crypto");
      const uploads = path.resolve(process.cwd(), "uploads");
      const storedName = `human-agency-${item.id}-${crypto.randomBytes(8).toString("hex")}.pdf`;
      const filePath = path.join(uploads, storedName);
      if (path.dirname(filePath) !== uploads) return res.status(400).json({ error: "Invalid artifact path" });
      await fs.mkdir(uploads, { recursive: true });
      await fs.writeFile(filePath, bytes, { mode: 0o600 });
      // Validate report safety before any durable publication.
      const { extractPdfTextBounded } = await import("../upload-handling");
      const { validateAuditOutput } = await import("../lib/ai-task-fit-audit");
      try {
        const text = await extractPdfTextBounded(filePath);
        const safety = validateAuditOutput(text);
        if (!safety.valid) {
          await fs.unlink(filePath).catch(() => {});
          return res.status(422).json({ error: "PDF contains prohibited audit language", violations: safety.violations });
        }
      } catch {
        await fs.unlink(filePath).catch(() => {});
        return res.status(422).json({ error: "PDF text could not be safely inspected" });
      }
      let storageKey: string | null = null;
      try {
        const stored = await import("../object-storage");
        storageKey = (await stored.uploadTenantFile(tenantId, "service-review", storedName, bytes)).storageKey;
      } catch (err: any) {
        console.warn(`[service-review] Object storage unavailable for ${item.id}; retaining DB artifact copy: ${err?.message}`);
      }
      const { fileStorage } = await import("@shared/schema");
      try {
        await db.insert(fileStorage).values({
          filename: storedName, originalName, mimeType: "application/pdf", size: bytes.length,
          data: storageKey ? "" : bytes.toString("base64"), storageKey, tenantId,
        });
      } catch (persistErr: any) {
        await fs.unlink(filePath).catch((cleanupErr: any) =>
          console.error(`[service-review] local artifact cleanup failed for ${storedName}: ${cleanupErr?.message}`));
        if (storageKey) {
          try { await (await import("../object-storage")).deleteTenantFile(tenantId, storageKey); }
          catch (cleanupErr: any) { console.error(`[service-review] durable object cleanup failed for ${storageKey}: ${cleanupErr?.message}`); }
        }
        throw persistErr;
      }
      let updated;
      try {
        updated = await updateReviewItem(item.id, {
          filePath: `uploads/${storedName}`, fileName: originalName,
          qa: { passed: true, issues: [], fileSizeBytes: bytes.length },
          artifactStorageKey: storageKey || undefined,
          artifactFilename: storedName,
        }, { durable: true });
      } catch (publishErr: any) {
        await fs.unlink(filePath).catch((e: any) => console.error(`[service-review] new local cleanup failed: ${e?.message}`));
        try {
          await db.delete(fileStorage).where(and(eq(fileStorage.tenantId, tenantId), eq(fileStorage.filename, storedName)));
        } catch (e: any) { console.error(`[service-review] new DB artifact cleanup failed: ${e?.message}`); }
        if (storageKey) {
          try { await (await import("../object-storage")).deleteTenantFile(tenantId, storageKey); }
          catch (e: any) { console.error(`[service-review] new object artifact cleanup failed: ${e?.message}`); }
        }
        throw publishErr;
      }
      // New artifact is now queue-published durably; retire the old cache/copy
      // only after that point. Cleanup is best effort and never loses the new
      // artifact.
      if (previousArtifact.filePath && previousArtifact.filePath !== `uploads/${storedName}`) {
        const oldAbs = confinedRegularFilePath(path.resolve(process.cwd(), previousArtifact.filePath), path.resolve(process.cwd(), "uploads"));
        if (oldAbs) await fs.unlink(oldAbs).catch((e: any) => console.warn(`[service-review] old artifact cleanup failed: ${e?.message}`));
        else console.error(`[service-review] refusing unsafe old local artifact cleanup path: ${previousArtifact.filePath}`);
      }
      if (previousArtifact.artifactStorageKey && previousArtifact.artifactStorageKey !== storageKey) {
        try { await (await import("../object-storage")).deleteTenantFile(tenantId, previousArtifact.artifactStorageKey); }
        catch (e: any) { console.error(`[service-review] old object artifact cleanup failed: ${e?.message}`); }
      }
      if (previousArtifact.artifactFilename && previousArtifact.artifactFilename !== storedName) {
        try {
          await db.delete(fileStorage).where(and(eq(fileStorage.tenantId, tenantId), eq(fileStorage.filename, previousArtifact.artifactFilename)));
        } catch (e: any) { console.error(`[service-review] old DB artifact cleanup failed: ${e?.message}`); }
      }
      const { reviewToken: _t, ...safe } = updated || item;
      return res.json({ ok: true, item: safe });
    } catch (err: any) {
      console.error("[service-review] PDF attachment failed:", err?.message);
      return res.status(500).json({ error: "PDF attachment failed" });
    }
  });
  ...
  app.get("/api/admin/service-orders/:id/file", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      const mime = reviewItemMime(item.fileName);
      res.setHeader("Content-Type", mime);
      res.setHeader("Content-Disposition", `${mime === "application/pdf" ? "inline" : "attachment"}; filename="${item.fileName.replace(/["\r\n]/g, "")}"`);
  ...
  app.post("/api/admin/service-orders/:id/approve", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) return res.status(403).json({ error: "Admin access required" });
      const { getReviewItem, updateReviewItem, rehydrateReviewArtifact } = await import("../service-review-queue");
      const item = getReviewItem((req.params.id as string));
      if (!item) return res.status(404).json({ error: "Not found" });
      ...
      if (item.sku.startsWith("sample-agency-audit-")) {
        await rehydrateReviewArtifact(item);
        if (!item.filePath || !item.fileName || !item.fileName.toLowerCase().endsWith(".pdf")) {
          return res.status(400).json({ error: "Attach a valid PDF before approval" });
        }
        const fs = await import("fs");
        const path = await import("path");
        const abs = path.isAbsolute(item.filePath) ? path.resolve(item.filePath) : path.resolve(process.cwd(), item.filePath);
        const allowed = path.resolve(process.cwd(), "uploads");
        if (abs !== allowed && !abs.startsWith(`${allowed}${path.sep}`) || !fs.existsSync(abs)) {
          return res.status(400).json({ error: "Attached PDF is missing or outside uploads" });
        }
        const fd = fs.openSync(abs, "r");
        const sig = Buffer.alloc(5);
        fs.readSync(fd, sig, 0, 5, 0);
        fs.closeSync(fd);
        if (sig.toString("ascii") !== "%PDF-") return res.status(400).json({ error: "Attached file is not a valid PDF" });
      }
      ...
      const { auditDfyUpsell } = await import("../lib/audit-upsell");
      ...
      const result = await deliverDigitalProduct({
        ...
        mimeType: reviewItemMime(item.fileName),
        ...
        // Task #153 — manual-approve deliveries carry the mission linkage
        // captured at enqueue time, so delivery-time instrumented spend is
        // attributed to the mission just like the auto-ship path.
        missionId: item.missionId,
        // $497 self-serve audits carry the $1,997 done-for-you upgrade CTA.
        // item.tier (checkout metadata) is authoritative; additionalFiles
        // (DFY Fix Kit) is a belt-and-suspenders suppressor for old items.
        upsell: item.additionalFiles?.length ? undefined : auditDfyUpsell({ sku: item.sku, productName: item.productName, tierHint: item.tier }),
      });
      if (!result.success) {
  ...
  // Prevents concurrent duplicate Fix Kit generations (see route below).
  const dfyUpgradeInFlight = new Set<string>();

  // Run the $1,997 Done-For-You upgrade from an existing audit order: reuse
  // the original intake (website/company/industry) to generate the Fix Kit,
  // then enqueue it as a NEW pending review item so Bob proofs + ships it
  // through the same HITL rail (DFY is hard manual-only — never auto-ships).
  app.post("/api/admin/service-orders/:id/dfy-upgrade", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (tenantId !== ADMIN_TENANT_ID || !isAdminRequest(req)) return res.status(403).json({ error: "Admin access required" });
      const { getReviewItem, addReviewItem, findReviewItemBySessionId } = await import("../service-review-queue");
      const item = getReviewItem(req.params.id as string);
      if (!item) return res.status(404).json({ error: "Not found" });
      if ((item.tier || "").toLowerCase() === "done-for-you" || item.sessionId.endsWith(":dfy-upgrade")) {
        return res.status(400).json({ error: "This order is already the done-for-you tier" });
      }
      const website = String(item.intake?.website || "").trim();
      if (!website) return res.status(400).json({ error: "Original order has no website in its intake — can't build the Fix Kit" });
      const upgradeSessionId = `${item.sessionId}:dfy-upgrade`;
      const existing = findReviewItemBySessionId(upgradeSessionId);
      if (existing && existing.status !== "rejected" && existing.status !== "failed") {
        return res.status(409).json({ error: `Upgrade already ${existing.status} (order ${existing.id})` });
      }
      // In-flight idempotency lock: the queue item only lands AFTER the
      // multi-minute kit generation, so without this a double-click launches
      // two full (expensive) generations that both pass the precheck above.
      if (dfyUpgradeInFlight.has(upgradeSessionId)) {
        return res.status(409).json({ error: "Upgrade generation already in progress for this order" });
      }
      dfyUpgradeInFlight.add(upgradeSessionId);
      const company = String(item.intake?.company || item.customerName || "Customer");
      // Kit generation takes a few minutes (LLM + site fetch) — run in the
      // background and surface the result as a new queue item either way.
      setImmediate(async () => {
        try {
          const { generateAuditFixKit } = await import("../audit-fix-kit");
          // Attach customer-stated facts from a submitted DFY intake form, if
          // one matches this customer's website/email (fail-soft — the kit
          // still generates without them, just leaner).
          let intakeFacts: Record<string, string> | undefined;
          try {
            const { findSubmittedDfyIntake, intakeFactsFromForm } = await import("../dfy-intake");
            const form = await findSubmittedDfyIntake({ tenantId: ADMIN_TENANT_ID, website, customerEmail: item.customerEmail });
            if (form) {
              intakeFacts = intakeFactsFromForm(form);
              console.log(`[dfy-upgrade] Using ${Object.keys(intakeFacts).length} customer answers from intake form #${form.id}`);
            }
          } catch (e: any) { console.warn("[dfy-upgrade] intake lookup failed (continuing without):", e?.message); }
          // Task #153 — DFY-upgrade generation belongs to the same mission as
          // the original order (if any): wrap in mission cost attribution.
          const runKit = () => generateAuditFixKit({
            website,
            company,
            industry: item.intake?.industry ? String(item.intake.industry) : undefined,
            tenantId: ADMIN_TENANT_ID,
            orderId: upgradeSessionId,
            intakeFacts,
          });
          let kit: Awaited<ReturnType<typeof generateAuditFixKit>>;
          if (typeof item.missionId === "number" && Number.isInteger(item.missionId) && item.missionId > 0) {
            const { withMissionCostAttribution } = await import("../agentic/cost-ledger");
            kit = await withMissionCostAttribution(item.missionId, runKit);
          } else {
            kit = await runKit();
          }
          ...
          await addReviewItem({
            ...
            // Inherit the offline-payment marker (admin-created originals) so
            // the upgrade card also shows how this customer pays.
            payment: item.payment,
            // Task #153 — the upgrade inherits the original order's mission.
            missionId: item.missionId,
          });
          console.log(`[dfy-upgrade] Kit ${ok ? "ready for review" : "FAILED"} for ${upgradeSessionId} (${website})`);
        } catch (bgErr: any) {
          console.error(`[dfy-upgrade] Crashed for ${upgradeSessionId}: ${bgErr?.message}`);
          try {
            await addReviewItem({
              sessionId: upgradeSessionId,
              sku: item.sku,
            ...
            // Double failure: kit generation crashed AND the failure record
            // couldn't be queued. The admin saw started:true — make this loud
            // (silent-failure review 2026-07-27).
            console.error(`[dfy-upgrade] UNRECOVERABLE: crash record for ${upgradeSessionId} could not be queued (${recoveryErr?.message}) — no card will appear in the review queue. Original crash: ${bgErr?.message}`);
            logSilentCatch("server/routes/admin.ts", recoveryErr);
          }
        } finally {
          dfyUpgradeInFlight.delete(upgradeSessionId);
        }
      });
      res.json({ ok: true, started: true, upgradeSessionId });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });
