  ...
  app.get("/api/plans", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      if (!Number.isInteger(tenantId) || (tenantId as number) <= 0) {
        return res.status(401).json({ error: "Valid tenant context required" });
      }
      const { listPlans } = await import("../minerva-planner");
      const plans = await listPlans({
        tenantId: tenantId as number,
        status: req.query.status as string | undefined,
        activity: req.query.activity === "true",
  ...
  app.post("/api/plans/:id/decide", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      const result = await decidePlan({
        ...
        reason: reason.trim(),
        actor,
        tenantId, // tenant-isolation audit 2026-07-31: scope CAS/probe/child lookups
      });
      res.json(result);
