...
import { validate, stripeCheckoutSchema } from "../validation";
...
export function registerStripeCheckoutRoutes(app: Express, helpers: Helpers) {
  ...
  app.post("/api/stripe/checkout", validate(stripeCheckoutSchema), async (req: Request, res: Response) => {
    try {
      ...
      // Never log the raw Stripe error object — it can carry request headers/
      // body fragments. Message + safe metadata only.
      console.error(
        `[stripe] Checkout error: ${err?.message || "unknown"} (type=${err?.type || "n/a"} status=${err?.statusCode ?? "n/a"} requestId=${err?.requestId || "n/a"})`,
      );
