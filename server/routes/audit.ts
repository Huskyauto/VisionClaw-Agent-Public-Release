...
export function registerAuditAdminRoutes(app: Express, helpers: {
  ...
  app.post("/api/admin/audit/full", authMiddleware, async (req: Request, res: Response) => {
    ...
    const { company, website, industry, notes } = parsed.data;
    const jobId = crypto.randomUUID();
    const job: FullAuditJob = { id: jobId, status: "running", startedAt: Date.now(), company, website };
    FULL_AUDIT_JOBS.set(jobId, job);
    saveFullAuditJobs();
    const customerEmail = ownerEmail() || "operator@visionclaw";
    fulfillReadinessAudit({
      intake: { company, website, industry: industry || undefined, notes: notes || undefined },
      customerEmail,
