...
import { db } from "../db";
import { proposedSkills, abRuns } from "@shared/schema";
...
export function registerHyperagentRoutes(
  ...
  app.post("/api/proposed-skills/:id/accept", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const promoted = await storage.createSkill({
        name: cleanName,
      ...
      if (isManagedSkillSecurityError(e)) {
        return res.status(400).json({
          error: `Skill content rejected by security scan (${e.patterns.join(", ")}).`,
        });
      }
