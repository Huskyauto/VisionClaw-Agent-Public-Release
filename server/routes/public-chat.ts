...
import { buildRecentUserRoutingContext, getCmmcAuthoritativeSourceLock } from "../lib/tool-routing-context";
  ...
  app.post("/api/public-chat/:token/conversations/:convId/messages", publicChatMessageLimiter, async (req, res) => {
    try {
      ...
      const { prompt: systemPrompt } = await buildSystemPrompt(
        ...
        false, "off", userContent, convTenantId, undefined, undefined, "public",
      );
