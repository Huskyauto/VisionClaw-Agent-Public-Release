...
import {
  authorizePlatformAdminSessionReset,
...
export function registerPlatformConfigRoutes(app: Express, helpers: PlatformConfigHelpers) {
  const { getTenantFromRequest, requirePlatformAdmin, isPlatformAdmin, ADMIN_TENANT_ID } = helpers;
  ...
  app.get("/api/settings", async (req, res) => {
    ...
    if (!s) {
      ...
      return res.json({
        ...
        defaultModel: isAuthed ? DEFAULT_CHAT_MODEL_ID : null,
  ...
  app.put("/api/settings", async (req, res) => {
    ...
    const defaults = {
      ...
      defaultModel: DEFAULT_CHAT_MODEL_ID,
      thinkingEnabled: false,
    };
    ...
    if (parsed.data.accessPin !== undefined) {
      ...
      const resetAuthority = authorizePlatformAdminSessionReset(
        req,
        res,
        "Platform access PIN changed",
      );
      if (!resetAuthority) return;
      await clearAllSessionsAsPlatformAdmin(resetAuthority);
    }
  ...
  app.get("/api/models", async (req, res) => {
    ...
    const available = await getAvailableModelsForTenant(tenantId, isPlatformAdmin(req));
