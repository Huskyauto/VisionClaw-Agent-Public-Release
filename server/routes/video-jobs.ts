...
export function toClientRow(row: any, tenantId: number) {
  ...
  const safeSpec = (spec && typeof spec === "object" && !Array.isArray(spec))
    ? {
      ...
      voiceProvider: typeof spec.voiceProvider === "string" ? spec.voiceProvider : undefined,
      strictVoice: spec.strictVoice === true ? true : undefined,
      resolution: typeof spec.resolution === "string" ? spec.resolution : undefined,
      fps: typeof spec.fps === "number" ? spec.fps : undefined,
      autoFinalize: spec.autoFinalize === true ? true : undefined,
