import express, { type Express, type Request, type Response } from "express";
import type multer from "multer";
import path from "path";
import crypto from "crypto";
import fsPromises from "fs/promises";
import { db } from "../db";
import { sql } from "drizzle-orm";
import { storage } from "../storage";
import { logSilentCatch } from "../lib/silent-catch";
import {
  listFrontierRevenueConcepts,
  listFrontierRevenueJuryRuns,
  readFrontierRevenueConcept,
  readFrontierRevenueJuryRun,
} from "../lib/frontier-revenue-discovery";
import {
  getRevenueWorkspaceByProjectName,
  getRevenueWorkspaceIdea,
  type RevenueWorkspaceKey,
} from "../lib/revenue-idea-catalog";
import { ADMIN_TENANT_ID } from "../tenant-constants";
import { getProjectCompass, ProjectCompassConflictError, replaceProjectCompass } from "../project-compass";

type ProjectsHelpers = {
  getTenantFromRequest: (req: Request) => number | null;
  authMiddleware: any;
  requirePlatformAdmin: (req: Request, res: Response) => boolean;
  upload: multer.Multer;
  SAFE_EXTENSIONS: Record<string, string>;
  UPLOADS_DIR: string;
  frontierConceptRoot?: string;
  frontierJuryRunRoot?: string;
};

/**
 * Round 60: Projects routes extracted from server/routes.ts (~337 LOC removed
 * from the monolith). Pure move — no behavior changes. The /api/projects/:id
 * conversations-union query was previously fixed for snake_case `project_id`.
 */
export function registerProjectsRoutes(app: Express, helpers: ProjectsHelpers) {
  const { getTenantFromRequest, authMiddleware, requirePlatformAdmin, upload, SAFE_EXTENSIONS, UPLOADS_DIR } = helpers;
  const frontierConceptRoot = helpers.frontierConceptRoot || path.join(process.cwd(), "data", "money-opportunities");
  const frontierJuryRunRoot = helpers.frontierJuryRunRoot || path.join(process.cwd(), "data", "moa-prompt-tests");

  function requireFrontierConceptOwner(req: Request, res: Response): boolean {
    return requirePlatformAdmin(req, res);
  }

  app.get("/api/frontier-revenue/discoveries", authMiddleware, async (req, res) => {
    if (!requireFrontierConceptOwner(req, res)) return;
    try {
      res.set("Cache-Control", "no-cache, no-store, must-revalidate");
      res.json(await listFrontierRevenueConcepts(frontierConceptRoot));
    } catch (error) {
      console.error("[frontier-concepts] list failed:", error);
      res.status(500).json({ error: "Could not load frontier concepts" });
    }
  });

  app.get("/api/frontier-revenue/runs", authMiddleware, async (req, res) => {
    if (!requireFrontierConceptOwner(req, res)) return;
    try {
      res.set("Cache-Control", "no-cache, no-store, must-revalidate");
      res.json(await listFrontierRevenueJuryRuns(frontierJuryRunRoot));
    } catch (error) {
      console.error("[frontier-revenue] jury run list failed:", error);
      res.status(500).json({ error: "Could not load frontier jury research" });
    }
  });

  app.get("/api/frontier-revenue/runs/:runId", authMiddleware, async (req, res) => {
    if (!requireFrontierConceptOwner(req, res)) return;
    res.set("Cache-Control", "no-cache, no-store, must-revalidate");
    try {
      const run = await readFrontierRevenueJuryRun(frontierJuryRunRoot, req.params.runId);
      if (!run) return res.status(404).json({ error: "Frontier jury research not found" });
      const { rawRun: _rawRun, ...detail } = run;
      res.json({
        ...detail,
        sourceUrl: `/api/frontier-revenue/runs/${encodeURIComponent(run.id)}/file`,
      });
    } catch (error: any) {
      if (error?.code === "ENOENT") return res.status(404).json({ error: "Frontier jury research not found" });
      console.error("[frontier-revenue] jury run detail failed:", error);
      res.status(500).json({ error: "Could not load this frontier jury research" });
    }
  });

  app.get("/api/frontier-revenue/runs/:runId/file", authMiddleware, async (req, res) => {
    if (!requireFrontierConceptOwner(req, res)) return;
    res.set("Cache-Control", "no-cache, no-store, must-revalidate");
    try {
      const run = await readFrontierRevenueJuryRun(frontierJuryRunRoot, req.params.runId);
      if (!run) return res.status(404).json({ error: "Frontier jury research not found" });
      res.type("application/json");
      res.setHeader("Content-Disposition", `inline; filename="${run.id}.json"`);
      res.send(JSON.stringify(run.rawRun, null, 2) + "\n");
    } catch (error: any) {
      if (error?.code === "ENOENT") return res.status(404).json({ error: "Frontier jury research not found" });
      console.error("[frontier-revenue] jury run file failed:", error);
      res.status(500).json({ error: "Could not load frontier jury research file" });
    }
  });

  app.get("/api/frontier-revenue/discoveries/:conceptId", authMiddleware, async (req, res) => {
    if (!requireFrontierConceptOwner(req, res)) return;
    try {
      const concept = await readFrontierRevenueConcept(frontierConceptRoot, req.params.conceptId);
      if (!concept) return res.status(404).json({ error: "Frontier concept not found" });
      const baseUrl = `/api/frontier-revenue/discoveries/${encodeURIComponent(concept.id)}/files`;
      res.json({
        ...concept,
        urls: {
          concept: `${baseUrl}/concept`,
          brief: `${baseUrl}/brief`,
          sourceRun: `${baseUrl}/source`,
        },
      });
    } catch (error) {
      console.error("[frontier-concepts] detail failed:", error);
      res.status(500).json({ error: "Could not load this frontier concept" });
    }
  });

  app.get("/api/frontier-revenue/discoveries/:conceptId/files/:kind", authMiddleware, async (req, res) => {
    if (!requireFrontierConceptOwner(req, res)) return;
    const kind = req.params.kind;
    if (kind !== "concept" && kind !== "brief" && kind !== "source") {
      return res.status(400).json({ error: "Unknown frontier concept file" });
    }
    try {
      const concept = await readFrontierRevenueConcept(frontierConceptRoot, req.params.conceptId);
      if (!concept) return res.status(404).json({ error: "Frontier concept not found" });
      const relativePath = kind === "concept"
        ? concept.files.concept
        : kind === "brief"
          ? concept.files.brief
          : concept.files.sourceRun;
      const root = await fsPromises.realpath(path.resolve(frontierConceptRoot));
      const filePath = path.resolve(root, relativePath);
      if (!filePath.startsWith(`${root}${path.sep}`)) {
        return res.status(400).json({ error: "Unsafe frontier concept file path" });
      }
      const canonicalFilePath = await fsPromises.realpath(filePath);
      if (!canonicalFilePath.startsWith(`${root}${path.sep}`)) {
        return res.status(400).json({ error: "Unsafe frontier concept file path" });
      }
      const contents = await fsPromises.readFile(canonicalFilePath);
      res.type(kind === "brief" ? "text/markdown" : "application/json");
      res.setHeader("Content-Disposition", `inline; filename="${kind === "brief" ? "README.md" : kind === "source" ? "run.json" : "concept.json"}"`);
      res.send(contents);
    } catch (error: any) {
      if (error?.code === "ENOENT") return res.status(404).json({ error: "Frontier concept file not found" });
      console.error("[frontier-concepts] file failed:", error);
      res.status(500).json({ error: "Could not load frontier concept file" });
    }
  });

  app.get("/api/projects", async (req, res) => {
    try {
      res.set("Cache-Control", "no-cache, no-store, must-revalidate");
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      console.log(`[projects] Fetching projects for tenant ${tenantId}`);
      const result = await db.execute(sql`
        SELECT p.*,
          (SELECT COUNT(*) FROM project_files WHERE project_id = p.id) as file_count,
          (SELECT COUNT(*) FROM project_notes WHERE project_id = p.id) as note_count,
          (SELECT COUNT(*) FROM project_conversations WHERE project_id = p.id) as conversation_count
        FROM projects p WHERE p.tenant_id = ${tenantId} ORDER BY p.updated_at DESC
      `);
      const rows = (result as any).rows || result;
      console.log(`[projects] Found ${Array.isArray(rows) ? rows.length : 0} projects for tenant ${tenantId}`);
      res.json(rows);
    } catch (e: any) {
      console.error("[projects] Error fetching projects:", e);
      res.status(500).json({ error: "Failed to fetch projects" });
    }
  });

  app.get("/api/projects/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      if (isNaN(id)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const pResult = await db.execute(sql`SELECT * FROM projects WHERE id = ${id} AND tenant_id = ${tenantId}`);
      const pRows = (pResult as any).rows || pResult;
      const project = Array.isArray(pRows) ? pRows[0] : pRows;
      if (!project) return res.status(404).json({ error: "Not found" });
      // R74.13h: defense-in-depth — even though the parent project row is
      // already tenant-checked above, also constrain dependent reads to the
      // same tenant so any stale/malformed cross-tenant FK can't leak data.
      const files = await db.execute(sql`
        SELECT pf.* FROM project_files pf
        JOIN projects p ON p.id = pf.project_id
        WHERE pf.project_id = ${id} AND p.tenant_id = ${tenantId}
        ORDER BY pf.created_at DESC
      `);
      const notes = await db.execute(sql`
        SELECT pn.* FROM project_notes pn
        JOIN projects p ON p.id = pn.project_id
        WHERE pn.project_id = ${id} AND p.tenant_id = ${tenantId}
        ORDER BY pn.created_at DESC
      `);
      const convs = await db.execute(sql`
        SELECT conversation_id, title, created_at FROM (
          SELECT pc.conversation_id, c.title, c.created_at
          FROM project_conversations pc
          JOIN conversations c ON c.id = pc.conversation_id
          WHERE pc.project_id = ${id} AND c.tenant_id = ${tenantId}
          UNION
          SELECT c.id as conversation_id, c.title, c.created_at
          FROM conversations c
          WHERE c.project_id = ${id} AND c.tenant_id = ${tenantId}
        ) combined ORDER BY created_at DESC
      `);
      const compass = await getProjectCompass(id, tenantId);
      res.json({ project, compass, files: (files as any).rows || files, notes: (notes as any).rows || notes, conversations: (convs as any).rows || convs });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  app.get("/api/projects/:id/revenue-ideas", authMiddleware, async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    try {
      const projectId = parseInt(req.params.id as string);
      if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });

      const projectResult = await db.execute(sql`
        SELECT id, name
        FROM projects
        WHERE id = ${projectId} AND tenant_id = ${tenantId}
        LIMIT 1
      `);
      const projectRows = (projectResult as any).rows || projectResult;
      const project = projectRows[0];
      if (!project) return res.status(404).json({ error: "Project not found" });

      const workspace = getRevenueWorkspaceByProjectName(String(project.name));
      if (!workspace) return res.status(404).json({ error: "No curated revenue ideas for this project" });

      const linkedNames = workspace.ideas
        .map((idea) => idea.linkedProjectName)
        .filter((name): name is string => Boolean(name));
      const linkedResult = linkedNames.length > 0
        ? await db.execute(sql`
            SELECT id, name
            FROM projects
            WHERE tenant_id = ${tenantId}
              AND name IN (${sql.join(linkedNames.map((name) => sql`${name}`), sql`, `)})
          `)
        : { rows: [] };
      const linkedRows = (linkedResult as any).rows || linkedResult;
      const linkedByName = new Map(linkedRows.map((row: any) => [String(row.name), Number(row.id)]));
      const workspaceKey = workspace.key as RevenueWorkspaceKey;

      res.set("Cache-Control", "no-cache, no-store, must-revalidate");
      res.json({
        workspaceKey,
        projectId,
        ownerOnly: workspace.ownerOnly,
        ideas: workspace.ideas.map((idea) => ({
          ...idea,
          linkedProjectId: workspaceKey === "customer-revenue-launchpad"
            ? projectId
            : (idea.linkedProjectName ? linkedByName.get(idea.linkedProjectName) || null : null),
          briefUrl: `/api/projects/${projectId}/revenue-ideas/${encodeURIComponent(idea.slug)}/brief`,
        })),
      });
    } catch (e: any) {
      console.error("[projects] Revenue ideas error:", e);
      res.status(500).json({ error: "Failed to fetch revenue ideas" });
    }
  });

  app.get("/api/projects/:id/revenue-ideas/:slug/brief", authMiddleware, async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    try {
      const projectId = parseInt(req.params.id as string);
      if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });

      const projectResult = await db.execute(sql`
        SELECT id, name
        FROM projects
        WHERE id = ${projectId} AND tenant_id = ${tenantId}
        LIMIT 1
      `);
      const projectRows = (projectResult as any).rows || projectResult;
      const project = projectRows[0];
      if (!project) return res.status(404).json({ error: "Project not found" });
      const workspace = getRevenueWorkspaceByProjectName(String(project.name));
      if (!workspace) return res.status(404).json({ error: "No curated revenue ideas for this project" });
      const idea = getRevenueWorkspaceIdea(workspace.key, String(req.params.slug));
      if (!idea) return res.status(404).json({ error: "Revenue idea not found" });

      const root = await fsPromises.realpath(process.cwd());
      const filePath = path.resolve(root, idea.briefPath);
      if (!filePath.startsWith(`${root}${path.sep}`)) {
        return res.status(400).json({ error: "Unsafe revenue brief path" });
      }
      const canonicalFilePath = await fsPromises.realpath(filePath);
      if (!canonicalFilePath.startsWith(`${root}${path.sep}`)) {
        return res.status(400).json({ error: "Unsafe revenue brief path" });
      }
      const stat = await fsPromises.stat(canonicalFilePath);
      if (!stat.isFile()) return res.status(404).json({ error: "Revenue brief not found" });
      const contents = await fsPromises.readFile(canonicalFilePath);
      res.type("text/markdown");
      res.setHeader("Content-Disposition", `inline; filename="${path.basename(idea.briefPath)}"`);
      res.send(contents);
    } catch (e: any) {
      if (e?.code === "ENOENT") return res.status(404).json({ error: "Revenue brief not found" });
      console.error("[projects] Revenue brief error:", e);
      res.status(500).json({ error: "Failed to load revenue brief" });
    }
  });

  app.post("/api/projects/:id/revenue-ideas/:slug/conversation", authMiddleware, async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    try {
      const parentProjectId = parseInt(req.params.id as string);
      if (isNaN(parentProjectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      if (tenantId !== ADMIN_TENANT_ID) return res.status(403).json({ error: "Owner workspace required" });

      const parentResult = await db.execute(sql`
        SELECT id, name
        FROM projects
        WHERE id = ${parentProjectId} AND tenant_id = ${tenantId}
        LIMIT 1
      `);
      const parentRows = (parentResult as any).rows || parentResult;
      const parent = parentRows[0];
      if (!parent) return res.status(404).json({ error: "Project not found" });
      const workspace = getRevenueWorkspaceByProjectName(String(parent.name));
      if (!workspace) return res.status(404).json({ error: "No curated revenue ideas for this project" });
      const idea = getRevenueWorkspaceIdea(workspace.key, String(req.params.slug));
      if (!idea) return res.status(404).json({ error: "Revenue idea not found" });

      let targetProjectId = parentProjectId;
      if (idea.linkedProjectName) {
        const childResult = await db.execute(sql`
          SELECT id
          FROM projects
          WHERE tenant_id = ${tenantId} AND name = ${idea.linkedProjectName}
          ORDER BY id
        `);
        const childRows = (childResult as any).rows || childResult;
        if (childRows.length !== 1) {
          return res.status(409).json({ error: "Fable idea project is missing or duplicated" });
        }
        targetProjectId = Number(childRows[0].id);
      }

      const conversationKey = `revenue-idea:${workspace.key}:${idea.slug}`;
      const conversationTitle = `Revenue idea [${workspace.key}:${idea.slug}] — ${idea.title}`;
      const activePersona = await storage.getActivePersona();
      const result = await db.transaction(async (tx) => {
        await tx.execute(sql`
          SELECT pg_advisory_xact_lock(hashtext(${`${tenantId}:${targetProjectId}:${conversationKey}`}))
        `);
        const existingResult = await tx.execute(sql`
          SELECT id
          FROM conversations
          WHERE tenant_id = ${tenantId}
            AND project_id = ${targetProjectId}
            AND title = ${conversationTitle}
            AND deleted_at IS NULL
          ORDER BY id
        `);
        const existingRows = (existingResult as any).rows || existingResult;
        if (existingRows.length > 1) {
          throw new Error(`Revenue idea conversation integrity failure for ${conversationKey}`);
        }
        if (existingRows[0]?.id) {
          return { id: Number(existingRows[0].id), created: false };
        }

        const createdResult = await tx.execute(sql`
          INSERT INTO conversations (title, model, thinking, thinking_level, persona_id, tenant_id, project_id)
          VALUES (
            ${conversationTitle},
            'deepseek/deepseek-v3.2',
            true,
            'auto',
            ${activePersona?.id ?? null},
            ${tenantId},
            ${targetProjectId}
          )
          RETURNING id
        `);
        const createdRows = (createdResult as any).rows || createdResult;
        const conversationId = Number(createdRows[0]?.id);
        if (!conversationId) throw new Error(`Revenue idea conversation insert returned no id for ${conversationKey}`);
        await tx.execute(sql`
          INSERT INTO project_conversations (project_id, conversation_id)
          VALUES (${targetProjectId}, ${conversationId})
        `);
        await tx.execute(sql`
          UPDATE projects SET updated_at = CURRENT_TIMESTAMP WHERE id = ${targetProjectId} AND tenant_id = ${tenantId}
        `);
        return { id: conversationId, created: true };
      });

      res.set("Cache-Control", "no-cache, no-store, must-revalidate");
      res.status(result.created ? 201 : 200).json({
        id: result.id,
        projectId: targetProjectId,
        created: result.created,
        conversationKey,
      });
    } catch (e: any) {
      console.error("[projects] Revenue idea conversation error:", e);
      res.status(500).json({ error: "Failed to open revenue idea workspace" });
    }
  });

  app.post("/api/projects", async (req, res) => {
    try {
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const { name, description, customerName, customerEmail, tags, status } = req.body;
      if (!name?.trim()) return res.status(400).json({ error: "name required" });
      const tagArray = Array.isArray(tags) ? tags.map((t: string) => String(t).slice(0, 100)) : [];
      const tagLiteral = `{${tagArray.map((t: string) => `"${t.replace(/"/g, '\\"')}"`).join(",")}}`;
      const result = await db.execute(sql`
        INSERT INTO projects (name, description, status, customer_name, customer_email, tags, tenant_id)
        VALUES (${name.trim()}, ${description || ''}, ${status || 'active'}, ${customerName || null}, ${customerEmail || null}, ${tagLiteral}::text[], ${tenantId})
        RETURNING *
      `);
      const rows = (result as any).rows || result;
      const project = Array.isArray(rows) ? rows[0] : rows;

      try {
        const tenant = await storage.getTenant(tenantId);
        const tenantName = tenant?.name || `tenant-${tenantId}`;
        const { ensureProjectFolder } = await import("../google-drive");
        const folder = await ensureProjectFolder(project.id, name.trim(), tenantId, tenantName);
        project.drive_folder_id = folder.id;
        project.drive_folder_url = folder.url;
        console.log(`[projects] Auto-created Drive folder for project ${project.id}: ${folder.url}`);
      } catch (driveErr) {
        console.log(`[projects] Drive folder creation skipped: ${(driveErr as Error).message}`);
      }

      res.json(project);
    } catch (e: any) { console.error("[projects] Create error:", e.message); res.status(500).json({ error: "Failed to create project" }); }
  });

  app.patch("/api/projects/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      if (isNaN(id)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const { name, description, status, customerName, customerEmail, primaryConversationId } = req.body;
      const chunks = [sql`UPDATE projects SET updated_at = CURRENT_TIMESTAMP`];
      if (name !== undefined) chunks.push(sql`, name = ${name}`);
      if (description !== undefined) chunks.push(sql`, description = ${description}`);
      if (status !== undefined) chunks.push(sql`, status = ${status}`);
      if (customerName !== undefined) chunks.push(sql`, customer_name = ${customerName}`);
      if (customerEmail !== undefined) chunks.push(sql`, customer_email = ${customerEmail}`);
      if (primaryConversationId !== undefined) {
        if (primaryConversationId === null) {
          chunks.push(sql`, primary_conversation_id = NULL`);
        } else {
          const convCheck = await db.execute(sql`
            SELECT 1 FROM conversations c
            WHERE c.id = ${primaryConversationId} AND c.tenant_id = ${tenantId}
            AND (
              EXISTS (SELECT 1 FROM project_conversations pc WHERE pc.project_id = ${id} AND pc.conversation_id = ${primaryConversationId})
              OR c.project_id = ${id}
            )
          `);
          const convRows = (convCheck as any).rows || convCheck;
          if (!convRows || (Array.isArray(convRows) && convRows.length === 0)) {
            return res.status(400).json({ error: "Conversation not found or not linked to this project" });
          }
          chunks.push(sql`, primary_conversation_id = ${primaryConversationId}`);
        }
      }
      chunks.push(sql` WHERE id = ${id} AND tenant_id = ${tenantId} RETURNING *`);
      const result = await db.execute(sql.join(chunks, sql.raw("")));
      const rows = (result as any).rows || result;
      const updated = Array.isArray(rows) ? rows[0] : rows;
      if (!updated) return res.status(404).json({ error: "Project not found" });
      res.json(updated);
    } catch (e: any) { res.status(500).json({ error: "Failed to update project" }); }
  });

  // Lifecycle decision: only a proven platform admin may move an IdeaBrowser
  // opportunity into the income-product portfolio. The global /api CSRF
  // middleware covers this mutating browser route.
  app.post("/api/projects/:id/transition-income-product", authMiddleware, async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    const projectId = parseInt(req.params.id as string, 10);
    if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    try {
      const { transitionBusinessOpportunityToIncomeProduct } = await import("../lib/ideabrowser-opportunity-lifecycle");
      const project = await transitionBusinessOpportunityToIncomeProduct(tenantId, projectId);
      if (!project) return res.status(404).json({ error: "Business opportunity not found" });
      res.json(project);
    } catch (e: any) {
      console.error("[projects] Income product transition failed:", e?.message || e);
      res.status(500).json({ error: "Failed to transition business opportunity" });
    }
  });

  app.get("/api/projects/:id/compass", async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      if (isNaN(id)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const compass = await getProjectCompass(id, tenantId);
      if (!compass) return res.status(404).json({ error: "Project not found" });
      res.json(compass);
    } catch (e: any) {
      res.status(500).json({ error: e.message || "Failed to fetch Project Compass" });
    }
  });

  app.put("/api/projects/:id/compass", async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      if (isNaN(id)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const compass = await replaceProjectCompass(id, tenantId, req.body?.revision, req.body?.entries);
      if (!compass) return res.status(404).json({ error: "Project not found" });
      res.json(compass);
    } catch (e: any) {
      if (e instanceof ProjectCompassConflictError) {
        return res.status(409).json({ error: e.message });
      }
      res.status(400).json({ error: e.message || "Invalid Project Compass" });
    }
  });

  app.delete("/api/projects/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      if (isNaN(id)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      await db.execute(sql`DELETE FROM projects WHERE id = ${id} AND tenant_id = ${tenantId}`);
      res.json({ deleted: true });
    } catch (e: any) { res.status(500).json({ error: "Failed to delete project" }); }
  });

  app.post("/api/projects/:id/notes", authMiddleware, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      if (isNaN(id)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const projCheck = await db.execute(sql`SELECT id FROM projects WHERE id = ${id} AND tenant_id = ${tenantId}`);
      const projRows = (projCheck as any).rows || projCheck;
      if (!Array.isArray(projRows) || projRows.length === 0) return res.status(404).json({ error: "Project not found" });
      if (!req.body.note?.trim()) return res.status(400).json({ error: "Note content required" });
      const result = await db.execute(sql`
        INSERT INTO project_notes (project_id, note, author)
        VALUES (${id}, ${req.body.note}, ${req.body.author || 'user'})
        RETURNING *
      `);
      const rows = (result as any).rows || result;
      res.json(Array.isArray(rows) ? rows[0] : rows);
    } catch (e: any) { res.status(500).json({ error: "Failed to add note" }); }
  });

  app.post("/api/projects/:id/files-base64", authMiddleware, express.json({ limit: "50mb" }), async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const projCheck = await db.execute(sql`SELECT id, name, drive_folder_id FROM projects WHERE id = ${projectId} AND tenant_id = ${tenantId}`);
      const projRows = (projCheck as any).rows || projCheck;
      if (!Array.isArray(projRows) || projRows.length === 0) return res.status(404).json({ error: "Project not found" });
      const projectName = projRows[0].name || `Project ${projectId}`;
      const projectDriveFolderId = projRows[0].drive_folder_id || null;
      const { files: fileList } = req.body;
      if (!fileList || !Array.isArray(fileList) || fileList.length === 0) return res.status(400).json({ error: "No files provided" });
      const results: any[] = [];
      for (const f of fileList) {
        const fileBuffer = Buffer.from(f.data, "base64");
        const ext = SAFE_EXTENSIONS[f.mimeType] || path.extname(f.fileName) || ".bin";
        const uniqueName = `${Date.now()}-${crypto.randomBytes(6).toString("hex")}${ext}`;
        const diskPath = path.join(UPLOADS_DIR, uniqueName);
        await fsPromises.writeFile(diskPath, fileBuffer);
        let fileUrl: string | null = null;
        try {
          const { uploadAndShare } = await import("../google-drive");
          const driveResult = await uploadAndShare({ filePath: diskPath, fileName: f.fileName, mimeType: f.mimeType, folderLabel: `Projects/${projectName}`, description: `Project file: ${f.fileName}`, parentFolderId: projectDriveFolderId || undefined, share: true });
          if ((driveResult as any).shareableLink) fileUrl = (driveResult as any).shareableLink;
          else if (driveResult.viewUrl) fileUrl = driveResult.viewUrl;
        } catch (driveErr: any) {
          console.log(`[projects] Drive upload skipped for ${f.fileName}: ${driveErr.message}`);
        }
        const downloadPath = `/api/projects/${projectId}/files/download/${uniqueName}`;
        const result = await db.execute(sql`
          INSERT INTO project_files (project_id, file_name, file_path, file_url, file_type, file_size)
          VALUES (${projectId}, ${f.fileName}, ${"uploads/" + uniqueName}, ${fileUrl || downloadPath}, ${f.mimeType || "application/octet-stream"}, ${fileBuffer.length})
          RETURNING *
        `);
        const rows = (result as any).rows || result;
        results.push(Array.isArray(rows) ? rows[0] : rows);
      }
      await db.execute(sql`UPDATE projects SET updated_at = CURRENT_TIMESTAMP WHERE id = ${projectId}`);
      res.json({ uploaded: results.length, files: results });
    } catch (e: any) {
      console.error("[projects] File upload error:", e.message || e);
      res.status(500).json({ error: e.message || "Failed to upload files" });
    }
  });

  app.post("/api/projects/:id/files", (req, res, next) => {
    upload.array("files", 20)(req, res, (err: any) => {
      if (err) {
        if (err.code === "LIMIT_FILE_SIZE") {
          return res.status(413).json({ error: "File too large. Maximum size is 50 MB per file." });
        }
        if (err.code === "LIMIT_FILE_COUNT") {
          return res.status(400).json({ error: "Too many files. Maximum is 20 files per upload." });
        }
        return res.status(400).json({ error: err.message || "Upload failed" });
      }
      next();
    });
  }, async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const projCheck = await db.execute(sql`SELECT id, name, drive_folder_id FROM projects WHERE id = ${projectId} AND tenant_id = ${tenantId}`);
      const projRows = (projCheck as any).rows || projCheck;
      if (!Array.isArray(projRows) || projRows.length === 0) {
        return res.status(404).json({ error: "Project not found" });
      }
      const projectName = projRows[0].name || `Project ${projectId}`;
      const projectDriveFolderId = projRows[0].drive_folder_id || null;
      const files = req.files as Express.Multer.File[];
      if (!files || files.length === 0) return res.status(400).json({ error: "No files uploaded" });
      const results: any[] = [];
      for (const file of files) {
        const filePath = `uploads/${file.filename}`;
        let fileUrl: string | null = null;
        try {
          const { uploadAndShare } = await import("../google-drive");
          const driveResult = await uploadAndShare({
            filePath,
            fileName: file.originalname,
            mimeType: file.mimetype,
            folderLabel: `Projects/${projectName}`,
            description: `Project file: ${file.originalname}`,
            parentFolderId: projectDriveFolderId || undefined,
            share: true,
          });
          if ((driveResult as any).shareableLink) {
            fileUrl = (driveResult as any).shareableLink;
          } else if (driveResult.viewUrl) {
            fileUrl = driveResult.viewUrl;
          }
        } catch (driveErr: any) {
          console.log(`[projects] Drive upload skipped for ${file.originalname}: ${driveErr.message}`);
        }
        const downloadPath = `/api/projects/${projectId}/files/download/${file.filename}`;
        const result = await db.execute(sql`
          INSERT INTO project_files (project_id, file_name, file_path, file_url, file_type, file_size)
          VALUES (${projectId}, ${file.originalname}, ${filePath}, ${fileUrl || downloadPath}, ${file.mimetype}, ${file.size})
          RETURNING *
        `);
        const rows = (result as any).rows || result;
        results.push(Array.isArray(rows) ? rows[0] : rows);
      }
      await db.execute(sql`UPDATE projects SET updated_at = CURRENT_TIMESTAMP WHERE id = ${projectId}`);
      res.json({ uploaded: results.length, files: results });
    } catch (e: any) {
      console.error("[projects] File upload error:", e.message || e);
      res.status(500).json({ error: e.message || "Failed to upload files" });
    }
  });

  app.get("/api/projects/:id/files", async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const projCheck = await db.execute(sql`SELECT id FROM projects WHERE id = ${projectId} AND tenant_id = ${tenantId}`);
      const projRows = (projCheck as any).rows || projCheck;
      if (!Array.isArray(projRows) || projRows.length === 0) {
        return res.status(404).json({ error: "Project not found" });
      }
      const result = await db.execute(sql`SELECT * FROM project_files WHERE project_id = ${projectId} ORDER BY created_at DESC`);
      const rows = (result as any).rows || result;
      res.json(Array.isArray(rows) ? rows : []);
    } catch (e: any) {
      res.status(500).json({ error: "Failed to get files" });
    }
  });

  app.get("/api/projects/:id/files/download/:filename", async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const projCheck = await db.execute(sql`SELECT id FROM projects WHERE id = ${projectId} AND tenant_id = ${tenantId}`);
      const projRows = (projCheck as any).rows || projCheck;
      if (!Array.isArray(projRows) || projRows.length === 0) {
        return res.status(404).json({ error: "Project not found" });
      }
      const filename = path.basename(req.params.filename as string);
      if (filename !== req.params.filename || filename.includes("..") || filename.includes("/") || filename.includes("\\")) {
        return res.status(400).json({ error: "Invalid filename" });
      }
      const fileRecord = await db.execute(sql`SELECT file_name, file_type, file_path FROM project_files WHERE project_id = ${projectId} AND file_path = ${"uploads/" + filename} LIMIT 1`);
      const fileRow = (fileRecord as any).rows?.[0];
      if (!fileRow) return res.status(404).json({ error: "File not found in project" });
      const filePath = path.join(process.cwd(), "uploads", filename);
      const fs = await import("fs");
      if (!fs.existsSync(filePath)) return res.status(404).json({ error: "File not found on disk" });
      const originalName = fileRow.file_name || filename;
      const mimeType = fileRow.file_type || "application/octet-stream";
      res.setHeader("Content-Disposition", `attachment; filename="${originalName}"`);
      res.setHeader("Content-Type", mimeType);
      const fileStream = fs.createReadStream(filePath);
      fileStream.pipe(res);
    } catch (e: any) {
      res.status(500).json({ error: "Download failed" });
    }
  });

  app.delete("/api/projects/:id/files/:fileId", async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      const fileId = parseInt(req.params.fileId as string);
      if (isNaN(projectId) || isNaN(fileId)) return res.status(400).json({ error: "Invalid ID" });
      const tenantId = getTenantFromRequest(req);
      if (!tenantId) return res.status(401).json({ error: "Authentication required" });
      const projCheck = await db.execute(sql`SELECT id FROM projects WHERE id = ${projectId} AND tenant_id = ${tenantId}`);
      const projRows = (projCheck as any).rows || projCheck;
      if (!Array.isArray(projRows) || projRows.length === 0) {
        return res.status(404).json({ error: "Project not found" });
      }
      await db.execute(sql`DELETE FROM project_files WHERE id = ${fileId} AND project_id = ${projectId}`);
      res.json({ deleted: true });
    } catch (e: any) {
      res.status(500).json({ error: "Failed to delete file" });
    }
  });
}
