...
import { validate, heartbeatDelegateSchema } from "../validation";
...
export function registerHeartbeatRoutes(app: Express, helpers: HeartbeatHelpers) {
  ...
  app.post("/api/heartbeat/tasks/:id/approve", async (req, res) => {
    ...
    const task = await storage.getHeartbeatTask(taskId, tenantId);
    if (!task) return res.status(404).json({ error: "Task not found" });
  ...
  app.get("/api/heartbeat/logs", async (req, res) => {
    ...
    const personaId = req.query.personaId ? parseInt(req.query.personaId as string) : undefined;
    res.json(await storage.getHeartbeatLogs(limit, personaId, ALL_TENANTS));
  });

  app.get("/api/heartbeat/status", async (req, res) => {
    if (!requirePlatformAdmin(req, res)) return;
    const [tasks, recentLogs, personas] = await Promise.all([
      storage.getHeartbeatTasks(undefined, ALL_TENANTS),
      storage.getHeartbeatLogs(5, undefined, ALL_TENANTS),
      storage.getPersonas(),
    ]);
  ...
  app.post("/api/heartbeat/delegate", validate(heartbeatDelegateSchema), async (req, res) => {
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Unauthorized" });
