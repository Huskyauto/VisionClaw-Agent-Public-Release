...
type TemplatesHelpers = {
  authMiddleware: any;
  getTenantFromRequest: (req: Request) => number | null;
  // Task 124 — conversation_templates is a GLOBAL table (no tenant_id), so
  // mutation requires PLATFORM admin (tenant #1), not just a flagged tenant admin.
  ...
  app.post("/api/templates/:id/start", async (req: Request, res: Response) => {
    try {
      ...
      const conv = await storage.createConversation({
        title: template.name,
        model: template.model || settings?.defaultModel || DEFAULT_CHAT_MODEL_ID,
        thinking: true,
        thinkingLevel: "auto",
