...
import { storage, memoryEntrySafeCols, ALL_TENANTS } from "../storage";
...
import { createForgettingCoordinator } from "../memory/forgetting-coordinator";
import { createForgettingStore } from "../memory/forgetting-store";
import {
  MEMORY_LIFECYCLE_SOURCES,
  type MemoryLifecycleSource,
} from "../memory/forgetting-policy";

type MemoryHelpers = {
  getTenantFromRequest: (req: Request) => number | null;
  isAdminRequest: (req: Request) => boolean;
  // Task 124 — cross-tenant scope bypasses use PLATFORM admin only.
...
export function registerMemoryRoutes(app: Express, helpers: MemoryHelpers) {
  const { getTenantFromRequest, isAdminRequest, isPlatformAdmin, ADMIN_TENANT_ID, requirePlatformAdmin } = helpers;

  const actorIdForRequest = (req: Request): number | null => {
    const request = req as any;
    const raw = request.user?.id
      ?? request.user?.claims?.user_id
      ?? request.user?.claims?.id
      ?? request.session?.userId
      ?? request._validatedSession?.userId;
    if (typeof raw === "number" && Number.isInteger(raw) && raw > 0) return raw;
    if (typeof raw === "string" && /^[1-9]\d*$/.test(raw)) {
      const parsed = Number(raw);
      if (Number.isSafeInteger(parsed)) return parsed;
    }
    // Replit's authenticated subject is text while the additive erasure
    // table's legacy actor column is integer.  Derive a stable, non-secret
    // surrogate from the server-verified subject; never accept an actor from
    // the request body.
    const subject = request.user?.claims?.sub;
    if (typeof subject !== "string" || subject.length === 0) return null;
    const derived = createHash("sha256").update(subject, "utf8").digest().readUInt32BE(0) & 0x7fffffff;
    return derived || 1;
  };

  const policyForRequest = async (tenantId: number) => {
    const store = createForgettingStore();
    const policy = await store.loadActivePolicy?.(tenantId);
    return { store, policy };
  };
  ...
  app.patch("/api/memory/:id", async (req, res) => {
    ...
    const existing = await storage.getMemoryEntry(memId, tenantId);
    if (!existing) return res.status(404).json({ error: "Not found" });
    if (existing.tenantId !== tenantId && !isPlatformAdmin(req)) {
      return res.status(403).json({ error: "Access denied" });
    }
    const parsed = insertMemoryEntrySchema.partial().safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
    const entry = await storage.updateMemoryEntry(memId, parsed.data, tenantId);
  ...
  app.delete("/api/memory/:id", async (req, res) => {
    ...
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    const memId = parseInt(req.params.id as string);
    const platformWide = isPlatformAdmin(req);
    const existing = await storage.getMemoryEntry(memId, platformWide ? undefined : tenantId);
    if (!existing) return res.status(404).json({ error: "Not found" });
    if (existing.tenantId !== tenantId && !isPlatformAdmin(req)) {
      return res.status(403).json({ error: "Access denied" });
    }
    // R98.22+sec — pass an explicit tenant scope to the storage delete. A
    // platform admin's cross-tenant maintenance operation uses the existing
    // server-owned sentinel; there is no implicit unscoped fallback.
    await storage.deleteMemoryEntry(memId, platformWide ? ALL_TENANTS : tenantId);
    res.status(204).send();
  });
  ...
  app.get("/api/memory/:id/links", async (req, res) => {
    ...
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    try {
      const scope = isPlatformAdmin(req) ? undefined : tenantId;
      ...
      if (sourceEntry.tenantId !== tenantId && !isPlatformAdmin(req)) {
        return res.status(403).json({ error: "Access denied" });
      }
      ...
      const linkedIds = await getLinkedMemories(
        ...
        isPlatformAdmin(req) ? null : tenantId,
      );
  ...
  app.post("/api/memory/backfill-embeddings", async (req, res) => {
    ...
    try {
      ...
      const knowledgeTenantById = new Map(
        knowledgeWithout.map((k) => [k.id, Number(k.tenantId)]),
      );
      const kCount = await generateAndStoreEmbeddings(
        knowledgeWithout.map((k) => ({ id: k.id, text: `${k.title} ${k.content}` })),
        (id, emb) => {
          const rowTenantId = knowledgeTenantById.get(id);
          if (!Number.isInteger(rowTenantId) || Number(rowTenantId) <= 0) {
            throw new Error(`knowledge row ${id} has no valid tenantId`);
          }
          return storage.updateKnowledgeEmbedding(id, emb, Number(rowTenantId));
        },
      );
  ...
  app.post("/api/memory/forgetting/report", async (req, res) => {
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    if (!isAdminRequest(req)) return res.status(403).json({ error: "Admin only" });
    try {
      const loaded = await policyForRequest(tenantId);
      const requested = Array.isArray(req.body?.sources) ? req.body.sources : undefined;
      const sources = requested?.filter((s: unknown): s is MemoryLifecycleSource =>
        typeof s === "string" && (MEMORY_LIFECYCLE_SOURCES as readonly string[]).includes(s));
      if (requested && sources?.length !== requested.length) {
        return res.status(400).json({ error: "Invalid memory source" });
      }
      const result = await createForgettingCoordinator({
        store: loaded.store,
        policy: loaded.policy ?? undefined,
        env: { MEMORY_FORGETTING_ENABLED: process.env.MEMORY_FORGETTING_ENABLED },
        tombstoneKey: process.env.MEMORY_TOMBSTONE_HMAC_KEY,
        authorize: ({ operation, tenantId: requestedTenant }) => {
          if (requestedTenant !== tenantId || !isAdminRequest(req)) return false;
          return operation === "report" || isPlatformAdmin(req) || requestedTenant === tenantId;
        },
      }).report({
        tenantId,
        sources,
      ...
      res.status(400).json({ error: e?.message || "forgetting report failed" });
    }
  });

  app.post("/api/memory/forgetting/run", async (req, res) => {
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    if (!requirePlatformAdmin(req, res)) return;
    try {
      ...
      res.status(400).json({ error: e?.message || "forgetting run failed" });
    }
  });

  app.post("/api/memory/forgetting/erase", async (req, res) => {
    const tenantId = getTenantFromRequest(req);
    if (!tenantId) return res.status(401).json({ error: "Authentication required" });
    let durableStore: ReturnType<typeof createForgettingStore> | null = null;
    let durableRequestStarted = false;
    let durableRequestKey = "";
    try {
      const actorId = actorIdForRequest(req);
      if (!actorId) return res.status(403).json({ error: "Authenticated actor identity required" });
      ...
      if (!requestKey.trim()) return res.status(400).json({ error: "requestKey is required" });
      // This is the authenticated self-service endpoint.  Compliance and
      // offboarding requests are created only by their internal/admin
      // server flows, never by a caller-controlled body field.
      if (req.body?.mode !== undefined && req.body.mode !== "explicit") {
        return res.status(400).json({ error: "Invalid erasure mode" });
      }
      const mode = "explicit" as const;
      if (items.length > 500 || items.some((item: any) =>
        !item || typeof item.source !== "string" ||
        !(MEMORY_LIFECYCLE_SOURCES as readonly string[]).includes(item.source) ||
        (typeof item.sourceId !== "string" && typeof item.sourceId !== "number") ||
        String(item.sourceId).length === 0
      )) {
        return res.status(400).json({ error: "Invalid erasure item" });
      }
      const store = createForgettingStore();
      durableStore = store;
      durableRequestKey = requestKey;
      const durable = await store.beginErasureRequest?.({
        tenantId,
        requestKey,
        mode,
        reason,
      ...
      durableRequestStarted = !!durable;
      if (durable && !durable.execute) {
        return res.status(durable.status === "running" ? 202 : 200).json({
          tenantId,
          requestKey,
          erased: durable.erasedCount,
          idempotent: 1,
          skipped: 0,
          errors: durable.error ? [durable.error] : [],
      ...
      const result = await createForgettingCoordinator({
        ...
        tombstoneKey: process.env.MEMORY_TOMBSTONE_HMAC_KEY,
        authorize: ({ operation, tenantId: requestedTenant }) =>
          requestedTenant === tenantId && (operation === "report" || requestedTenant === tenantId),
      }).explicitErase({
        tenantId,
        requestKey,
        reason,
      ...
      await store.completeErasureRequest?.({
        tenantId,
        requestKey,
      ...
      if (durableStore && durableRequestStarted) {
        await durableStore.completeErasureRequest?.({
          tenantId,
          requestKey: durableRequestKey,
          status: "failed",
          erasedCount: 0,
          error: e?.message || "explicit erasure failed",
        }).catch((finalizeError: unknown) => {
          console.error("[memory-forgetting] durable request finalization failed", {
            tenantId,
            requestKey: durableRequestKey,
      ...
      res.status(400).json({ error: e?.message || "explicit erasure failed" });
    }
  });
}
