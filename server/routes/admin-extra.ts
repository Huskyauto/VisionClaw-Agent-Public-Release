  ...
  app.delete("/api/account", authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      const { randomUUID } = await import("node:crypto");
       const { prepareOffboardingErasure, recordAccountErasureReceipt } = await import("../memory/forgetting-store");
      const requestKey = `offboarding-${randomUUID()}`;

      // Keep the lifecycle evidence and all account deletes in one transaction.
       // Tenant deletion intentionally cascades tenant-scoped lifecycle rows.
       // The value-free, HMAC-keyed platform receipt is inserted before the
       // DELETE so it survives those cascades (the sole global compliance
       // exception; it has no tenant_id/FK or memory content).
      await database.transaction(async (tx: any) => {
         const erasure = await prepareOffboardingErasure(tx, tenantId, requestKey);
         await recordAccountErasureReceipt(tx, {
           tenantId,
           requestKey,
           itemCount: erasure.itemCount,
           erasedCount: erasure.erasedCount,
           status: "completed",
         });
        await tx.execute(s`DELETE FROM messages WHERE conversation_id IN (SELECT id FROM conversations WHERE tenant_id = ${tenantId})`);
        await tx.execute(s`DELETE FROM conversations WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM memory_entries WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM daily_notes WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM agent_knowledge WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM custom_tools WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM experiments WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM file_storage WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM tenant_persona_names WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM heartbeat_tasks WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM tenant_provider_keys WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM auth_sessions WHERE tenant_id = ${tenantId}`);
        await tx.execute(s`DELETE FROM tenants WHERE id = ${tenantId}`);
      });
