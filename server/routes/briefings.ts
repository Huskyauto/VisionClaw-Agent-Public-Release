  ...
  app.get("/api/briefing", async (req, res) => {
    ...
    try {
      ...
      const [logs, convResult, personas, memStats] = await Promise.all([
        storage.getHeartbeatLogs(50, undefined, ALL_TENANTS),
...
Keep it concise — this is a morning briefing, not a novel. Use bullet points. Be direct and actionable.`;
      ...
      let model = settings?.defaultModel || "gpt-5";
      if (model === "auto") {
        model = availableModels.find((m: any) => m.id === "gpt-5" || m.id === "gpt-5.6-sol")?.id || "gpt-5";
      }
  ...
  app.get("/api/activity/pulse", async (req, res) => {
    ...
    try {
      const [recentLogs, personas] = await Promise.all([
        storage.getHeartbeatLogs(15, undefined, ALL_TENANTS),
        storage.getPersonas(),
      ]);
