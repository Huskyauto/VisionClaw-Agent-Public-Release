...
async function dispatchToPersona(opts: {
  ...
  try {
    ...
    const conv = await storage.createConversation({
      ...
      // Keep Slack aligned with the platform-wide default rather than allowing
      // the schema fallback to persist an obsolete or channel-specific model.
      model: DEFAULT_CHAT_MODEL_ID,
    } as any);
    const { processMessage } = await import('../chat-engine');
    const reply = await processMessage(conv.id, prompt, { tenantId, source: 'slack', enableTools: true });
