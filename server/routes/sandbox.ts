...
export function registerSandboxRoutes(app: Express, helpers: SandboxHelpers) {
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT id, corpus, status, overrides, sample_size, report, error, started_at, completed_at,
                (status = 'running' AND started_at < now() - interval '30 minutes') AS stale
  ...
  // Improvement list — jury-vetted upgrade proposals promoted from sandbox
  // runs (sandbox_promote tool). Read-only list; NOTHING here auto-applies.
  app.get("/api/admin/sandbox/improvements", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = requireSandboxAdmin(req, res);
      if (tenantId === null) return;
      const result = await pool.query(
        `SELECT id, run_id, title, summary, proposal, jury_verdict, jury_votes, status, created_at, decided_at
         FROM sandbox_improvements WHERE tenant_id = $1
         ORDER BY CASE status WHEN 'approved' THEN 0 WHEN 'escalated' THEN 1 WHEN 'jury_pending' THEN 2 ELSE 3 END, id DESC
         LIMIT 100`,
        [tenantId],
      );
      res.json({ improvements: result.rows });
    } catch (err: any) {
      console.error("[sandbox routes] improvements list failed:", err);
      res.status(500).json({ error: err?.message || "failed to list improvements" });
    }
  });

  // Human/agent decision on a proposal: mark implemented (adopted into the
  // platform) or rejected (declined). Only these two transitions are allowed
  // from this surface — jury verdicts are never overwritten.
  app.patch("/api/admin/sandbox/improvements/:id", authMiddleware, async (req: Request, res: Response) => {
    try {
      const tenantId = requireSandboxAdmin(req, res);
      if (tenantId === null) return;
      ...
      const bodyParse = z.object({ status: z.enum(["implemented", "rejected"]) }).safeParse(req.body ?? {});
      if (!bodyParse.success) {
        return res.status(400).json({ error: "status must be 'implemented' or 'rejected'" });
      }
      const status = bodyParse.data.status;
      const result = await pool.query(
        `UPDATE sandbox_improvements SET status = $1, decided_at = COALESCE(decided_at, now())
         WHERE id = $2 AND tenant_id = $3
         RETURNING id, status`,
        [status, id, tenantId],
      );
      if (!result.rows[0]) return res.status(404).json({ error: `no improvement ${id} for this tenant` });
      res.json({ improvement: result.rows[0] });
    } catch (err: any) {
      console.error("[sandbox routes] improvement update failed:", err);
      res.status(500).json({ error: err?.message || "failed to update improvement" });
    }
  });
