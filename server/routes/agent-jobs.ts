...
import {
  listJobs,
  getJobStats,
  getJobTenantForAdmin,
  retryJob,
  cancelJob,
} from "../job-queue";
import { getRegisteredKinds } from "../job-worker";
import { ALL_TENANTS } from "../storage-helpers/tenant-scope";
...
export function registerAgentJobsRoutes(app: Express, deps: Deps) {
  ...
  app.post("/api/admin/agent-jobs/:id/retry", authMiddleware, async (req: Request, res: Response) => {
    ...
    try {
      ...
      // This route is already platform-admin-gated above. Resolve scope from
      // the durable job row rather than trusting a client tenant selector.
      const job = await getJobTenantForAdmin(id);
      const jobTenantId = job?.tenantId;
      if (!Number.isInteger(jobTenantId) || (jobTenantId as number) <= 0) {
        return res.status(409).json({ error: "Job is not retryable", id });
      }
      const retried = await retryJob(id, jobTenantId as number);
      if (!retried) {
        return res.status(409).json({ error: "Job is not retryable", id });
      }
