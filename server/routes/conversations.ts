...
import { selectHermesAdminPilotModel } from "../lib/hermes-model-pilot";
import { getModelForTierAsync, normalizeModelId } from "../providers";
...
type ConversationsHelpers = {
  ...
  // Task 124 — unscoped (cross-tenant) conversation access is PLATFORM admin only.
  isPlatformAdmin: (req: Request) => boolean;
  mutateLimiter: RequestHandler;
  validateModelForTenant: (modelId: string, tenantId: number, platformAdminVerified?: boolean) => Promise<boolean>;
};
...
export function registerConversationsRoutes(app: Express, helpers: ConversationsHelpers) {
  ...
  app.post("/api/conversations", async (req: Request, res: Response) => {
    ...
    const pilotModel = selectHermesAdminPilotModel({
      tenantId,
      adminTenantId: ADMIN_TENANT_ID,
      personaName: activePersona?.name,
      explicitModelSelected: typeof parsed.data.model === "string" && parsed.data.model.trim().length > 0,
    });
    let requestedModel = normalizeModelId(
      parsed.data.model ||
      pilotModel ||
      personaReasoningConfig.preferredModel ||
    ...
    const explicitModelSelected =
      typeof parsed.data.model === "string" && parsed.data.model.trim().length > 0;
    const modelAllowed =
      (!isOwnerOnlyFinalDecisionModel(requestedModel) || explicitModelSelected) &&
      await validateModelForTenant(requestedModel, tenantId, isPlatformAdmin(req));
  ...
  app.get("/api/conversations/:id", async (req: Request, res: Response) => {
    ...
    const conv = isPlatformAdmin(req)
      ? await getConversationForPlatformAdmin(req, id)
  ...
  app.patch("/api/conversations/:id", mutateLimiter, async (req: Request, res: Response) => {
    ...
    if (
      ...
      conv.title.startsWith("Revenue idea [") &&
      updateData.title !== conv.title
    ) {
      return res.status(400).json({ error: "Revenue idea workspace titles are fixed for reliable reuse" });
    }
    if (updateData.model) {
      updateData.model = normalizeModelId(updateData.model);
      const modelAllowed = await validateModelForTenant(updateData.model, tenantId, isPlatformAdmin(req));
      if (!modelAllowed) {
