        ...
        try {
          ...
          for (const entry of created) {
            try {
              ...
              if (embedding) await storage.updateKnowledgeEmbedding(entry.id, embedding, tenantId);
    ...
    try {
      ...
      setImmediate(async () => {
        try {
          const { generateEmbedding } = await import("../embeddings");
          for (const entry of created) {
            try { const emb = await generateEmbedding(`${entry.title} ${entry.content}`); if (emb) await storage.updateKnowledgeEmbedding(entry.id, emb, tenantId); } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
          }
        } catch (_silentErr) { logSilentCatch("server/routes.ts", _silentErr); }
      });
    } catch (err: any) { res.status(500).json({ error: err.message }); }
