...
async function runHeartbeatRepairStep(plan: PlanRow, step: any): Promise<StepResult> {
  const startedIso = nowIso();
  const action = validateHeartbeatRepairAction(plan.plan_json?.repairAction);
  ...
  if (!binding || plan.source !== "agentic-engine.auto-apply") {
    throw new Error("heartbeat repair missing immutable approval binding");
  }
  const evidence = await executeHeartbeatRepair(plan.tenant_id, action);
  return {
...
async function runSourceRepairHandoffStep(
  ...
  const action = validateSourceRepairAction(plan.plan_json?.repairAction);
  const binding = (Array.isArray(plan.execution_log) ? plan.execution_log : []).find((event: any) =>
    event?.type === "repair.approval_binding" && event.stepId === step.n &&
    event.kind === action.kind && event.actionHash === JSON.stringify(action));
  if (!binding || plan.source !== "agentic-engine.auto-apply") throw new Error("source repair missing immutable approval binding");
  if (String(action.findingId) !== String(plan.source_ref || "")) {
  ...
  const found: any = await db.execute(sql`
    ...
    FROM ai_insights WHERE id = ${Number(plan.source_ref || 0)} AND tenant_id = ${plan.tenant_id}
  `);
...
export async function executePlan(planId: number): Promise<void> {
  ...
  try {
    ...
    const claim: any = await db.execute(sql`
      ...
      RETURNING id, tenant_id, objective, source, source_ref, plan_json, execution_log, planner_persona_id
    `);
    ...
    const runStepSafe = async (step: any): Promise<StepResult> => {
      try {
        if (step?.tool === "heartbeat_repair") return await runHeartbeatRepairStep(row, step);
        if (step?.tool === "source_repair_handoff") return await runSourceRepairHandoffStep(row, step);
    ...
    while (pending.length > 0) {
      ...
      if (failedThisWave.length > 0) {
        ...
        if (!boundRepairFailure && replanCount < MAX_REPLANS) {
          replanCount++;
    ...
    const finalStatus = allOk && sourceHandoffPlan ? "handoff_pending" : (allOk ? "completed" : "failed");
    const terminalTenantId = claimedTenantId;
    await finalizePlanExecution({
      commitTerminalState: async () => {
        const committed: any = await db.execute(sql`
          UPDATE plans
    ...
    await emitEvent({
      type: allOk && sourceHandoffPlan ? "execution.handoff_pending" : (allOk ? "plan.completed" : "plan.failed"),
      source: "plan-executor",
      tenantId: row.tenant_id,
      data: {
        planId,
