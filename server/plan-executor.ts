...
import { finalizePlanExecution } from "./lib/plan-finalization";
import {
...
import {
  ...
  recoveryApprovalActionHash,
  canonicalRecoveryEvidence,
  deriveRecoveryReplaySafety,
...
import { executeHeartbeatRepair } from "./agentic/heartbeat-repair-adapter";
import { hashRepairAction, validateHeartbeatRepairAction } from "./agentic/heartbeat-repair-contract";
import { dispatchRepairHandoff, validateSourceRepairAction } from "./source-repair-handoff";
...
async function runHeartbeatRepairStep(
  plan: PlanRow,
  step: any,
  execute: typeof executeHeartbeatRepair = executeHeartbeatRepair,
): Promise<StepResult> {
  const startedIso = nowIso();
  const recoveryEnvelope = plan.plan_json?.repairAction?.kind === "stale_plan_recovery"
    ? plan.plan_json.repairAction
    : null;
  const action = validateHeartbeatRepairAction(
    recoveryEnvelope?.recoveryStepContract ?? plan.plan_json?.repairAction,
  );
  const binding = (Array.isArray(plan.execution_log) ? plan.execution_log : []).find((event: any) =>
    recoveryEnvelope
      ? event?.type === "repair.approval_binding" &&
        event.kind === "stale_plan_recovery" &&
        event.actionHash === recoveryApprovalActionHash(recoveryEnvelope)
      : event?.type === "repair.approval_binding" &&
        event.stepId === step.n &&
        event.version === action.version &&
        event.kind === action.kind &&
        event.actionHash === hashRepairAction({
          version: action.version, kind: action.kind, taskId: action.taskId,
          expectedBefore: action.expectedBefore, desiredAfter: action.desiredAfter,
          verifier: action.verifier,
        }),
  );
  if (!binding || (!recoveryEnvelope && plan.source !== "agentic-engine.auto-apply")) {
    throw new Error("heartbeat repair missing immutable approval binding");
  }
  const evidence = await execute(plan.tenant_id, action);
  return {
    step: step.n, agent: step.agent, started_at: startedIso, ended_at: nowIso(),
    durationMs: 0, success: true, summary: "Verified heartbeat repair",
    output: { verified: true, before: evidence.before, after: evidence.after },
  };
}

async function runSourceRepairHandoffStep(
  plan: PlanRow,
  step: any,
  dispatch: typeof dispatchRepairHandoff = dispatchRepairHandoff,
): Promise<StepResult> {
  const recoveryEnvelope = plan.plan_json?.repairAction?.kind === "stale_plan_recovery"
    ? plan.plan_json.repairAction
    : null;
  const action = validateSourceRepairAction(
    recoveryEnvelope?.recoveryStepContract ?? plan.plan_json?.repairAction,
  );
  const binding = (Array.isArray(plan.execution_log) ? plan.execution_log : []).find((event: any) =>
    recoveryEnvelope
      ? event?.type === "repair.approval_binding" &&
        event.kind === "stale_plan_recovery" &&
        event.actionHash === recoveryApprovalActionHash(recoveryEnvelope)
      : event?.type === "repair.approval_binding" && event.stepId === step.n &&
        event.kind === action.kind && event.actionHash === JSON.stringify(action));
  if (!binding || (!recoveryEnvelope && plan.source !== "agentic-engine.auto-apply")) {
    throw new Error("source repair missing immutable approval binding");
  }
  if (!recoveryEnvelope && String(action.findingId) !== String(plan.source_ref || "")) {
    throw new Error("source repair finding does not match plan source");
  }
  const found: any = await db.execute(sql`
    SELECT id, tenant_id, title, summary, details, data_snapshot
    FROM ai_insights WHERE id = ${Number(action.findingId)} AND tenant_id = ${plan.tenant_id}
  `);
  const row = (found.rows ?? found)[0];
  if (!row) throw new Error("source finding no longer exists for tenant");
  const snapshot = typeof row.data_snapshot === "string"
    ? row.data_snapshot.slice(0, 20_000)
    : JSON.stringify(row.data_snapshot ?? "").slice(0, 20_000);
  let snapshotValue: any;
  try { snapshotValue = JSON.parse(snapshot); } catch { throw new Error("source finding evidence snapshot is invalid"); }
  const currentVersion = typeof snapshotValue?.evidenceVersion === "string" ? snapshotValue.evidenceVersion : "";
  const currentHash = createHash("sha256").update(snapshot).digest("hex");
  if (currentVersion !== action.evidenceVersion || currentHash !== action.evidenceHash) {
    throw new Error("source finding evidence drifted since approval");
  }
  const result = await dispatch({
    tenantId: plan.tenant_id, sourceFindingId: action.findingId,
    sourceEvidenceVersion: action.evidenceVersion, sourceEvidenceHash: action.evidenceHash,
    findingIntent: `Source finding ${action.findingId} requires workspace investigation under evidence ${action.evidenceVersion}.`,
    evidence: snapshot,
  });
  if (!result.accepted) throw new Error(result.blocker || "source repair handoff blocked");
  return { step: step.n, agent: step.agent, started_at: nowIso(), ended_at: nowIso(),
    durationMs: 0, success: true, summary: "Source finding handed to development workspace",
    output: { handoff: "accepted", handoffPending: true, handoffId: result.handoffId, idempotent: result.idempotent } };
}

/** Focused contract seam for integration tests; production dispatch uses runStepSafe. */
export async function executeSourceRepairHandoffStepForTest(
  plan: {
    id: number; tenant_id: number; source?: string; source_ref?: string | null; plan_json: any; execution_log: any;
  },
  step: any,
  dispatch: typeof dispatchRepairHandoff = dispatchRepairHandoff,
): Promise<StepResult> {
  return runSourceRepairHandoffStep(plan as PlanRow, step, dispatch);
}

export async function executeHeartbeatRepairStepForTest(plan: {
  id: number; tenant_id: number; source?: string; plan_json: any; execution_log: any;
}, step: any): Promise<StepResult> {
  return runHeartbeatRepairStep(plan as PlanRow, step);
}
...
export interface PlanExecutionDeps {
  executeHeartbeatRepair?: typeof executeHeartbeatRepair;
  dispatchRepairHandoff?: typeof dispatchRepairHandoff;
}

export async function executePlan(planId: number, deps: PlanExecutionDeps = {}): Promise<void> {
  ...
  let claimedRow: PlanRow | undefined;
  try {
    // CAS: only proceed if plan is currently 'approved'. Recovery path
    // (resumeStuckPlans) flips 'executing' → 'approved' before re-firing.
    const sourceProbe: any = await db.execute(sql`SELECT source FROM plans WHERE id=${planId} LIMIT 1`);
    const claim: any = (sourceProbe.rows ?? sourceProbe)[0]?.source === "stale-plan-recovery"
      ? { rows: [await claimStaleRecoveryChildCore(db, planId)].filter(Boolean) }
      : await db.execute(sql`
      UPDATE plans
      SET status = 'executing', updated_at = CURRENT_TIMESTAMP
      WHERE id = ${planId} AND status = 'approved'
      RETURNING id, tenant_id, objective, source, source_ref, plan_json, execution_log, planner_persona_id, version, parent_plan_id
    `);
    ...
    claimedRow = row;
    const staleRecovery = row.source === "stale-plan-recovery" &&
      row.plan_json?.repairAction?.kind === "stale_plan_recovery";
    if (staleRecovery) {
      const action = row.plan_json.repairAction;
      const parentResult: any = await db.execute(sql`
        SELECT id, tenant_id, version, plan_json, execution_log, status
        FROM plans WHERE id = ${Number(action.originalPlanId)}
          AND tenant_id = ${claimedTenantId} LIMIT 1
      `);
      const parent = (parentResult.rows ?? parentResult)[0];
      const parentFingerprint = parent ? recoveryHash(parent.plan_json) : "";
      const parentEvidence = parent ? recoveryHash(canonicalRecoveryEvidence(parent.execution_log).slice(-12000)) : "";
      if (!parent || Number(parent.version) !== Number(action.originalVersion) ||
          parentFingerprint !== action.originalFingerprint ||
          parentEvidence !== action.executionEvidenceHash ||
          Number(parent.tenant_id) !== claimedTenantId) {
        throw new Error("stale recovery binding drifted before execution; refusing child dispatch");
      }
      const firstRecoveryStep = Array.isArray(row.plan_json?.steps) ? row.plan_json.steps[0] : null;
      const approvalBinding = (Array.isArray(row.execution_log) ? row.execution_log : [])
        .find((event: any) => event?.type === "repair.approval_binding" &&
          event.kind === "stale_plan_recovery" &&
          event.actionHash === recoveryApprovalActionHash(action));
      const parentSteps = Array.isArray(parent.plan_json?.steps) ? parent.plan_json.steps : [];
      const unresolvedSuffix = parentSteps.slice(Number(action.firstUnresolvedStepIndex));
      const unsafeRecoveryStep = unresolvedSuffix.find((step: any) =>
        !deriveRecoveryReplaySafety(step, parent.execution_log, parent.plan_json).allowed);
      if (!firstRecoveryStep || !approvalBinding ||
          recoveryHash(canonicalRecoveryStep(firstRecoveryStep)) !== action.stepHash ||
          recoveryHash((row.plan_json.steps as any[]).map(canonicalRecoveryStep)) !== action.suffixHash ||
          Number(action.parentPlanId) !== Number(action.originalPlanId) ||
          recoveryApprovalActionHash(action) !== action.actionHash ||
          unsafeRecoveryStep) {
        throw new Error("stale recovery action is not immutably approved or first step drifted");
      }
      await appendExecutionLog(planId, claimedTenantId, {
        type: "execution.recovery_executing", at: nowIso(),
        originalPlanId: Number(action.originalPlanId), incidentKey: action.incidentKey,
      });
    }
    ...
    const runStepSafe = async (step: any): Promise<StepResult> => {
      try {
        if (step?.tool === "heartbeat_repair") {
          return await runHeartbeatRepairStep(row, step, deps.executeHeartbeatRepair);
        }
        if (step?.tool === "source_repair_handoff") {
          return await runSourceRepairHandoffStep(row, step, deps.dispatchRepairHandoff);
        }
    ...
    while (pending.length > 0) {
      ...
      if (failedThisWave.length > 0) {
        ...
        const boundRepairFailure = terminal.step?.tool === "heartbeat_repair" ||
          terminal.step?.tool === "source_repair_handoff";
        if (!staleRecovery && !boundRepairFailure && replanCount < MAX_REPLANS) {
          replanCount++;
      ...
      // A stale-recovery child is an exact owner-approved suffix. Neither a
      // failure nor a low heuristic score may replace it with model-authored
      // steps; the child must finish exactly or terminalize with one blocker.
      if (degradingDecision.trigger && !staleRecovery) {
    ...
    // A source handoff is successful transport, not repair completion. Keep
    // Bob's publish boundary visible on the original approved plan.
    const sourceHandoffPlan = row.plan_json?.repairAction?.kind === "source_repair_handoff";
    const recoveredSourceHandoff = staleRecovery &&
      row.plan_json?.repairAction?.recoveryStepContract?.kind === "source_repair_handoff";
    let finalStatus = allOk && sourceHandoffPlan ? "handoff_pending" : (allOk ? "completed" : "failed");
    const terminalTenantId = claimedTenantId;
    let recoveryBindingDrift = false;
    let recoveryTerminalResult: Awaited<ReturnType<typeof terminalizeStaleRecoveryChildCore>> | undefined;
    await finalizePlanExecution({
      commitTerminalState: async () => {
        const commit = async (executor: any) => {
          if (staleRecovery) {
            recoveryTerminalResult = await terminalizeStaleRecoveryChildCore(executor, planId, {
              success: allOk,
              reason: terminalFailure?.error || "Recovery suffix failed.",
              terminalStatus: allOk && recoveredSourceHandoff ? "handoff_pending" : "completed",
            });
            if (!["completed", "handoff_pending", "already_terminal"].includes(recoveryTerminalResult.outcome)) {
              recoveryBindingDrift = true;
              finalStatus = "failed";
            } else if (recoveryTerminalResult.outcome === "already_terminal") {
              finalStatus = ["completed", "handoff_pending"].includes(recoveryTerminalResult.childStatus)
                ? recoveryTerminalResult.childStatus : "failed";
            } else {
              finalStatus = recoveryTerminalResult.childStatus;
            }
            return;
          }
          const committed: any = await executor.execute(sql`
          UPDATE plans
          SET status = ${finalStatus},
              updated_at = CURRENT_TIMESTAMP,
              execution_log = COALESCE(execution_log, '[]'::jsonb) || ${JSON.stringify([{
                type: sourceHandoffPlan && allOk ? "execution.handoff_pending" : `execution.${finalStatus}`,
                at: nowIso(),
                ...
                ...(sourceHandoffPlan ? {
                  handoffIdentity: row.plan_json?.repairAction ? {
                    findingId: row.plan_json.repairAction.findingId,
                    evidenceVersion: row.plan_json.repairAction.evidenceVersion,
                    evidenceHash: row.plan_json.repairAction.evidenceHash,
                  } : null,
                  handoffId: results.find(r => r.output?.handoffId)?.output?.handoffId ?? null,
                } : {}),
              }])}::jsonb
          WHERE id = ${planId} AND tenant_id = ${claimedTenantId} AND status = 'executing'
          RETURNING id
          `);
          if (!(committed.rows ?? committed)[0]) {
          throw new Error(`terminal state commit did not update executing plan #${planId}`);
          }
        };
        if (staleRecovery) await db.transaction((tx) => commit(tx));
        else await commit(db);
      },
      appendTerminalAdvisory: () => appendPlanGoalState(planId, terminalTenantId, {
        planId,
        objective: row.objective,
        phase: "final",
        steps,
    ...
    if (staleRecovery && recoveryTerminalResult) {
      const recoverySucceeded = recoveryTerminalResult.outcome === "completed" ||
        recoveryTerminalResult.outcome === "handoff_pending" ||
        (recoveryTerminalResult.outcome === "already_terminal" &&
          ["completed", "handoff_pending"].includes(recoveryTerminalResult.childStatus) &&
          recoveryTerminalResult.childStatus === recoveryTerminalResult.parentStatus);
      finalStatus = recoverySucceeded ? recoveryTerminalResult.childStatus : "failed";
      recoveryBindingDrift = !recoverySucceeded;
    }
    await emitEvent({
      type: staleRecovery
        ? (recoveryBindingDrift ? "plan.failed" :
          finalStatus === "handoff_pending" ? "execution.handoff_pending" : "plan.completed")
        : (allOk && !recoveryBindingDrift && sourceHandoffPlan ? "execution.handoff_pending" : (allOk && !recoveryBindingDrift ? "plan.completed" : "plan.failed")),
      source: "plan-executor",
      tenantId: row.tenant_id,
      data: {
        planId,
        ...
        totalSteps: results.length,
        replanned,
         failedStep: allOk && !recoveryBindingDrift ? null : terminalFailure?.step ?? null,
         failedReason: allOk && !recoveryBindingDrift ? null : terminalFailure?.error ?? "stale recovery binding drifted",
      },
    });
    ...
    if (claimedTenantId !== undefined) {
      try {
        if (claimedRow?.source === "stale-plan-recovery" && claimedRow.plan_json?.repairAction?.originalPlanId) {
          const action = claimedRow.plan_json.repairAction;
          await db.transaction(async (tx) => {
            const blocker = { type: "execution.recovery_blocker", at: nowIso(), recoveryChildId: planId,
              actionable: true, reason: String(err?.message || "Recovery child failed before terminal evidence.") };
            await tx.execute(sql`
              UPDATE plans SET status = 'failed', updated_at = now(),
                execution_log = COALESCE(execution_log, '[]'::jsonb) || ${JSON.stringify([blocker])}::jsonb
              WHERE id = ${planId} AND tenant_id = ${claimedTenantId} AND status = 'executing'
            `);
            await tx.execute(sql`
              UPDATE plans SET status = 'failed', updated_at = now(),
                execution_log = COALESCE(execution_log, '[]'::jsonb) || ${JSON.stringify([blocker])}::jsonb
              WHERE id = ${Number(action.originalPlanId)} AND tenant_id = ${claimedTenantId}
                AND status IN ('failed','executing')
                AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log, '[]'::jsonb)) e
                  WHERE e->>'type' = 'execution.recovery_blocker'
                    AND e->>'recoveryChildId' = ${String(planId)})
            `);
          });
        } else {
        await db.execute(sql`
...
export async function claimStaleRecoveryChildCore(executor: { execute: (q: any) => Promise<any> }, planId: number): Promise<any | null> {
  const result: any = await executor.execute(sql`
    UPDATE plans SET status='executing', updated_at=now()
    WHERE id=${planId} AND source='stale-plan-recovery' AND status='approved'
    RETURNING id, tenant_id, objective, source, source_ref, plan_json, execution_log, planner_persona_id, version, parent_plan_id
  `);
  return (result.rows ?? result)[0] ?? null;
}

export async function terminalizeStaleRecoveryChildCore(
  tx: { execute: (q: any) => Promise<any> },
  childId: number,
  outcome: { success: boolean; reason?: string; terminalStatus?: "completed" | "handoff_pending" },
): Promise<{ outcome: "completed" | "handoff_pending" | "failed_execution" | "failed_drift" | "failed_orphan" | "already_terminal"; childStatus: string; parentStatus: string | null; reason: string }> {
  const childResult: any = await tx.execute(sql`
    SELECT id, tenant_id, parent_plan_id, source_ref, status, plan_json, execution_log FROM plans
    WHERE id=${childId} AND source='stale-plan-recovery' FOR UPDATE
  `);
  const child = (childResult.rows ?? childResult)[0];
  if (!child) throw new Error("stale recovery child not found");
  const action = child.plan_json?.repairAction;
  const parentResult: any = await tx.execute(sql`
    SELECT id, status, version, plan_json, execution_log FROM plans
    WHERE id=${Number(action?.originalPlanId)} AND tenant_id=${Number(child.tenant_id)} FOR UPDATE
  `);
  const parent = (parentResult.rows ?? parentResult)[0];
  const parentEvents = Array.isArray(parent?.execution_log) ? parent.execution_log : [];
  const childEvents = Array.isArray(child.execution_log) ? child.execution_log : [];
  const coherentComplete = ["completed", "handoff_pending"].includes(child.status) &&
    child.status === parent?.status;
  const coherentFailed = child.status === "failed" && parent?.status === "failed" &&
    parentEvents.some((e: any) => e?.type === "execution.recovery_blocker" && Number(e.recoveryChildId) === childId);
  const coherentRejected = child.status === "rejected" && parent?.status === "rejected";
  if (coherentComplete || coherentFailed || coherentRejected) {
    return {
      outcome: "already_terminal", childStatus: child.status, parentStatus: parent.status,
      reason: "Recovery child and parent already form a coherent terminal outcome.",
    };
  }
  if (child.status === "completed" || child.status === "handoff_pending" ||
      child.status === "failed" || child.status === "rejected" ||
      (parent && parent.status !== "failed")) {
    const blocker = { type: "execution.recovery_blocker", at: nowIso(), recoveryChildId: childId,
      actionable: true, reason: "Recovery child and parent terminal states were inconsistent; reconciled safely." };
    const childWrite: any = await tx.execute(sql`UPDATE plans SET status='failed',updated_at=now(),
      execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([blocker])}::jsonb
      WHERE id=${childId} AND tenant_id=${Number(child.tenant_id)} AND status <> 'failed' RETURNING id`);
    if (parent) {
      const parentWrite: any = await tx.execute(sql`UPDATE plans SET status='failed',updated_at=now(),
        execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([blocker])}::jsonb
        WHERE id=${Number(parent.id)} AND tenant_id=${Number(child.tenant_id)} AND status <> 'failed' RETURNING id`);
      if (!(parentWrite.rows ?? parentWrite)[0] && parent.status !== "failed") throw new Error("recovery parent reconciliation update failed");
    }
    if (!(childWrite.rows ?? childWrite)[0] && child.status !== "failed") throw new Error("recovery child reconciliation update failed");
    return { outcome: "failed_drift", childStatus: "failed", parentStatus: parent ? "failed" : null, reason: blocker.reason };
  }
  const binding = parent ? deriveStaleRecoveryBinding({
    tenantId: Number(child.tenant_id), originalPlanId: Number(parent.id),
    version: Number(parent.version), planJson: parent.plan_json, executionLog: parent.execution_log,
  }) : null;
  const childSteps = Array.isArray(child.plan_json?.steps) ? child.plan_json.steps : [];
  const suffixHash = recoveryHash(childSteps.map(canonicalRecoveryStep));
  const approvalEvent = (Array.isArray(child.execution_log) ? child.execution_log : []).find((e: any) =>
    e?.type === "repair.approval_binding" && e?.actionHash);
  const validBinding = Boolean(parent && action &&
    Number(child.parent_plan_id) === Number(action.originalPlanId) &&
    Number(action.parentPlanId) === Number(child.parent_plan_id) &&
    Number(action.tenantId) === Number(child.tenant_id) &&
    Number(action.originalVersion) === Number(parent.version) &&
    child.source_ref === action.incidentKey && binding &&
    binding.incidentKey === action.incidentKey &&
    binding.originalFingerprint === action.originalFingerprint &&
    binding.executionEvidenceHash === action.executionEvidenceHash &&
    binding.firstUnresolvedStepIndex === Number(action.firstUnresolvedStepIndex) &&
    binding.firstUnresolvedStepId === Number(action.firstUnresolvedStepId) &&
    binding.stepHash === action.stepHash && binding.suffixHash === action.suffixHash &&
    suffixHash === action.suffixHash &&
    approvalEvent?.actionHash === action.actionHash &&
    recoveryApprovalActionHash(action) === action.actionHash);
  if (!validBinding) {
    const drift = { type: "execution.recovery_blocker", at: nowIso(), recoveryChildId: childId,
      actionable: true, reason: !parent ? "Recovery child is orphaned." : `Recovery binding drifted before terminalization (parent=${Boolean(parent)} tenant=${Number(action?.tenantId)===Number(child.tenant_id)} source=${child.source_ref===action?.incidentKey} fp=${Boolean(binding && binding.originalFingerprint===action?.originalFingerprint)} suffix=${Boolean(binding && binding.suffixHash===action?.suffixHash)} evidence=${Boolean(binding && binding.executionEvidenceHash===action?.executionEvidenceHash)} hash=${Boolean(action && recoveryApprovalActionHash(action)===action.actionHash)}).` };
    await tx.execute(sql`UPDATE plans SET status='failed',updated_at=now(),
      execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([drift])}::jsonb
      WHERE id=${childId} AND tenant_id=${Number(child.tenant_id)} AND status IN ('approved','executing')`);
    if (parent) await tx.execute(sql`UPDATE plans SET status='failed',updated_at=now(),
      execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([drift])}::jsonb
      WHERE id=${Number(parent.id)} AND tenant_id=${Number(child.tenant_id)} AND status='failed'
        AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log,'[]'::jsonb)) e
          WHERE e->>'type'='execution.recovery_blocker' AND e->>'recoveryChildId'=${String(childId)})`);
    return { outcome: parent ? "failed_drift" : "failed_orphan", childStatus: "failed", parentStatus: parent ? "failed" : null, reason: drift.reason };
  }
  const successStatus = outcome.terminalStatus ?? "completed";
  const event = outcome.success
    ? { type: successStatus === "handoff_pending" ? "execution.handoff_pending" : "execution.recovery.completed",
        at: nowIso(), recoveryChildId: childId, incidentKey: action?.incidentKey }
    : { type: "execution.recovery_blocker", at: nowIso(), recoveryChildId: childId, incidentKey: action?.incidentKey, actionable: true, reason: outcome.reason || "Recovery suffix failed." };
  await tx.execute(sql`
    UPDATE plans SET status=${outcome.success ? successStatus : "failed"}, updated_at=now(),
      execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([event])}::jsonb
    WHERE id=${childId} AND tenant_id=${Number(child.tenant_id)} AND status IN ('approved','executing')
  `);
  if (!parent) return { outcome: "failed_orphan", childStatus: "failed", parentStatus: null, reason: "Recovery child parent is missing." };
  const parentEvent = outcome.success
    ? event
    : { ...event, reason: outcome.reason || "Recovery suffix failed." };
  const parentUpdate: any = await tx.execute(sql`
    UPDATE plans SET status=${outcome.success ? successStatus : "failed"}, updated_at=now(),
      execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([parentEvent])}::jsonb
    WHERE id=${Number(parent.id)} AND tenant_id=${Number(child.tenant_id)}
      AND status='failed' RETURNING id
  `);
  if (!(parentUpdate.rows ?? parentUpdate)[0]) return { outcome: "already_terminal", childStatus: outcome.success ? successStatus : "failed", parentStatus: parent.status, reason: "Parent was already terminal." };
  return { outcome: outcome.success ? successStatus : "failed_execution", childStatus: outcome.success ? successStatus : "failed", parentStatus: outcome.success ? successStatus : "failed", reason: outcome.reason || (outcome.success ? "Recovery completed." : "Recovery suffix failed.") };
}
...
export async function resumeStuckPlans(): Promise<{ resumed: number; restarted: number; recovered: number }> {
  await drainHistoricalRecoveryBlockedPlans();
  await reconcileStaleRecoveryChildren();
  const staleCandidates: any = await db.execute(sql`
    SELECT id, tenant_id, status, plan_json, execution_log, version
    FROM plans
    WHERE status = 'executing' AND source <> 'stale-plan-recovery'
  ...
  for (const candidate of (staleCandidates.rows ?? staleCandidates) as any[]) {
    ...
    if (!recoverable) {
      ...
      const binding = deriveStaleRecoveryBinding({
        tenantId: Number(candidate.tenant_id), originalPlanId: Number(candidate.id),
        version: Number(candidate.version || 1), planJson: candidate.plan_json,
        executionLog: candidate.execution_log,
      });
      const reason = recoveryExhausted
        ? "Automatic stale recovery exhausted; exact unresolved suffix requires owner approval."
        : "Automatic stale recovery refused because replay may repeat a structured tool side effect.";
      await createStaleRecoveryChildCore(db, candidate, reason);
      continue;
      await db.transaction(async (tx) => {
        // Advisory lock makes the read/insert/event sequence one winner even on
        // deployments whose legacy schema predates the conditional unique index.
        if (binding) await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext(${binding.incidentKey}))`);
        const existing = binding ? await tx.execute(sql`
          SELECT id FROM plans WHERE tenant_id = ${candidate.tenant_id}
            AND source = 'stale-plan-recovery' AND source_ref = ${binding.incidentKey} LIMIT 1
        `) : { rows: [] };
        const existingRow = (existing as any).rows?.[0];
        if (existingRow?.id) {
          // The first sweep committed the child and linkage event. Reuse is a
          // no-op: never append a second card/event for the same incident.
          return;
        }
        if (!binding) {
          await tx.execute(sql`
            UPDATE plans SET status = 'failed', updated_at = CURRENT_TIMESTAMP,
              execution_log = COALESCE(execution_log, '[]'::jsonb) || ${JSON.stringify([{
                type: "execution.recovery_blocker", at: nowIso(),
                reason: `${reason} Recovery evidence or step shape is malformed; create a new review.`,
                actionable: true,
              }])}::jsonb
            WHERE id = ${candidate.id} AND tenant_id = ${candidate.tenant_id} AND status = 'executing'
          `);
          return;
        }
        const steps = candidate.plan_json.steps as any[];
        const first = binding.firstUnresolvedStepIndex;
        const suffixNs = new Set(steps.slice(first).map((step: any) => Number(step?.n)));
        const suffix = steps.slice(first).map((step: any) => ({
          ...step,
          depends_on: Array.isArray(step.depends_on)
            ? step.depends_on.filter((dep: any) => suffixNs.has(Number(dep))) : [],
          // Never carry model/caller authorization claims into a repair child.
          args: step.args && typeof step.args === "object" ? Object.fromEntries(
            Object.entries(step.args).filter(([key]) => !key.startsWith("_")),
          ) : undefined,
        }));
        const action = {
          version: 1, kind: "stale_plan_recovery", originalPlanId: Number(candidate.id),
          tenantId: Number(candidate.tenant_id), originalVersion: binding.originalVersion,
          originalFingerprint: binding.originalFingerprint, incidentKey: binding.incidentKey,
          firstUnresolvedStepIndex: binding.firstUnresolvedStepIndex,
          firstUnresolvedStepId: binding.firstUnresolvedStepId,
          stepHash: binding.stepHash, executionEvidenceHash: binding.executionEvidenceHash,
          suffixHash: binding.suffixHash, parentPlanId: Number(candidate.id),
        };
        (action as any).actionHash = recoveryApprovalActionHash(action as any);
        const childPlan = {
          objective: `Bound recovery for plan #${candidate.id}: ${reason}`,
          context: { recovery: true, originalPlanId: Number(candidate.id) },
          steps: suffix, repairAction: action,
          total_estimated_minutes: suffix.reduce((n, s) => n + Number(s.estimated_minutes || 0), 0),
          total_estimated_cost_usd: suffix.reduce((n, s) => n + Number(s.estimated_cost_usd || 0), 0),
          risks: ["Exact unresolved suffix only; no generic replanning."], success_criteria: ["Bound suffix completes"],
          unknowns: [],
        };
        const inserted: any = await tx.execute(sql`
          ...
          VALUES (${candidate.tenant_id}, ${childPlan.objective}, 'stale-plan-recovery',
            ${binding.incidentKey}, 'awaiting_approval', ${JSON.stringify(childPlan)}::jsonb,
            15, 1, ${candidate.id}) RETURNING id
        `);
        const childId = Number((inserted.rows ?? inserted)[0]?.id);
        if (!childId) throw new Error("stale recovery child insert returned no id");
        const event = {
          type: "execution.recovery_approval_created", at: nowIso(),
          recoveryChildId: childId, incidentKey: binding.incidentKey,
          originalPlanId: Number(candidate.id), firstUnresolvedStepIndex: first,
        };
        await tx.execute(sql`
          UPDATE plans SET status = 'failed', updated_at = CURRENT_TIMESTAMP,
            execution_log = COALESCE(execution_log, '[]'::jsonb) || ${JSON.stringify([event])}::jsonb
...
export async function backfillHistoricalRecoveryBlockedPlansCore(
  executor: { transaction?: (fn: (tx: any) => Promise<unknown>) => Promise<unknown>; execute: (q: any) => Promise<any> },
  tenantId?: number,
  batch = 25,
): Promise<{ processed: number; hasMore: boolean; lastId?: number }> {
  const rows: any = await executor.execute(sql`
    SELECT id, tenant_id, version, plan_json, execution_log
    FROM plans
    WHERE status='failed' AND source <> 'stale-plan-recovery'
      AND (${tenantId ?? null}::integer IS NULL OR tenant_id=${tenantId ?? null})
      AND EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log,'[]'::jsonb)) e
        WHERE e->>'type'='execution.recovery_blocked')
      AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log,'[]'::jsonb)) e
        WHERE e->>'type'='execution.recovery_blocker')
      AND NOT EXISTS (SELECT 1 FROM plans c WHERE c.tenant_id=plans.tenant_id
        AND c.source='stale-plan-recovery' AND c.parent_plan_id=plans.id)
    ORDER BY id LIMIT ${Math.min(25, Math.max(1, batch))}
  `);
  let count = 0;
  let lastId: number | undefined;
  for (const row of (rows.rows ?? rows)) {
    await createStaleRecoveryChildCore(executor, row, "Historical recovery blocker requires owner approval.");
    count++;
    lastId = Number(row.id);
  }
  return { processed: count, hasMore: (rows.rows ?? rows).length >= Math.min(25, Math.max(1, batch)), lastId };
}

const HISTORICAL_BACKFILL_PAGES_PER_TICK = 4;
let historicalBackfillContinuationScheduled = false;
type HistoricalBackfillScheduler = (task: () => Promise<void>, delayMs: number) => void;
export async function drainHistoricalRecoveryBlockedPlansCore(
  executor: Parameters<typeof backfillHistoricalRecoveryBlockedPlansCore>[0],
  maxPages = HISTORICAL_BACKFILL_PAGES_PER_TICK,
  batch = 25,
): Promise<{ processed: number; hasMore: boolean; pages: number }> {
  let processed = 0;
  let hasMore = false;
  let pages = 0;
  const boundedPages = Math.min(20, Math.max(1, maxPages));
  for (; pages < boundedPages; pages++) {
    const result = await backfillHistoricalRecoveryBlockedPlansCore(executor, undefined, batch);
  ...
  return { processed, hasMore, pages };
}

export async function runHistoricalRecoveryBackfillWorker(args: {
  executor?: Parameters<typeof backfillHistoricalRecoveryBlockedPlansCore>[0];
  schedule?: HistoricalBackfillScheduler;
  maxPages?: number;
  batch?: number;
} = {}): Promise<{ processed: number; hasMore: boolean; pages: number }> {
  const executor = args.executor ?? db;
  const schedule = args.schedule ?? ((task, delayMs) => {
    setTimeout(() => { void task(); }, delayMs);
  });
  const scheduleNext = (delayMs: number) => {
    if (historicalBackfillContinuationScheduled) return;
    historicalBackfillContinuationScheduled = true;
    schedule(async () => {
      historicalBackfillContinuationScheduled = false;
      try {
        await runHistoricalRecoveryBackfillWorker(args);
      } catch (error: any) {
        console.warn(`[plan-executor] historical recovery backfill continuation failed: ${error?.message ?? error}`);
      }
    }, delayMs);
  };
  try {
    const result = await drainHistoricalRecoveryBlockedPlansCore(
      executor,
      args.maxPages ?? HISTORICAL_BACKFILL_PAGES_PER_TICK,
      args.batch ?? 25,
    );
    if (result.hasMore) scheduleNext(1_000);
    return result;
  } catch (error) {
    scheduleNext(30_000);
    throw error;
...
async function drainHistoricalRecoveryBlockedPlans(): Promise<void> {
  await runHistoricalRecoveryBackfillWorker();
}

/** Recovery children never recurse into another recovery child. A crashed child
 * is terminalized under its immutable binding and its parent receives one
 * actionable blocker; uncertain structured dispatch is never retried. */
async function reconcileStaleRecoveryChildren(): Promise<number> {
  const result: any = await db.execute(sql`
    SELECT id, tenant_id, parent_plan_id FROM plans
    WHERE source = 'stale-plan-recovery' AND status = 'executing'
      AND updated_at < NOW() - (${STALE_EXECUTING_MIN} * INTERVAL '1 minute')
    LIMIT 50
  `);
  let reconciled = 0;
  for (const candidate of (result.rows ?? result)) {
    await db.transaction(async (tx) => {
      const locked: any = await tx.execute(sql`
        SELECT id, tenant_id, parent_plan_id, plan_json, execution_log
        FROM plans WHERE id = ${Number(candidate.id)} AND tenant_id = ${Number(candidate.tenant_id)}
          AND source = 'stale-plan-recovery' AND status = 'executing' FOR UPDATE
      `);
      const child = (locked.rows ?? locked)[0];
      if (!child) return;
      const parentLock: any = await tx.execute(sql`
        SELECT id, tenant_id, version, plan_json, execution_log FROM plans WHERE id = ${Number(child.parent_plan_id)}
          AND tenant_id = ${Number(child.tenant_id)} FOR UPDATE
      `);
      const parent = (parentLock.rows ?? parentLock)[0];
      const action = child.plan_json?.repairAction;
      const binding = parent ? deriveStaleRecoveryBinding({
        tenantId: Number(child.tenant_id), originalPlanId: Number(parent.id),
        version: Number(parent.version), planJson: parent.plan_json, executionLog: parent.execution_log,
      }) : null;
      const valid = Boolean(parent && action &&
        Number(child.parent_plan_id) === Number(action.originalPlanId) &&
        Number(action.tenantId) === Number(child.tenant_id) &&
        child.source_ref === action.incidentKey && binding &&
        binding.incidentKey === action.incidentKey &&
        binding.originalFingerprint === action.originalFingerprint &&
        binding.executionEvidenceHash === action.executionEvidenceHash &&
        binding.suffixHash === action.suffixHash &&
        recoveryApprovalActionHash(action) === action.actionHash);
      const event = {
        type: "execution.recovery_blocker", at: nowIso(), recoveryChildId: Number(child.id),
        actionable: true, reason: "Recovery child became stale before terminal evidence; exact bound suffix was not retried.",
      };
      if (!parent) event.reason = "Recovery child is orphaned; parent plan no longer exists for this tenant.";
      else if (!valid) event.reason = "Recovery child binding is corrupt or stale; create a new bound recovery review.";
      const childUpdate: any = await tx.execute(sql`
        UPDATE plans SET status = 'failed', updated_at = now(),
          execution_log = COALESCE(execution_log, '[]'::jsonb) || ${JSON.stringify([event])}::jsonb
        WHERE id = ${Number(child.id)} AND tenant_id = ${Number(child.tenant_id)}
          AND status = 'executing' RETURNING id
      `);
      if (!(childUpdate.rows ?? childUpdate)[0]) return;
      await tx.execute(sql`
        UPDATE plans SET status = 'failed', updated_at = now(),
          execution_log = COALESCE(execution_log, '[]'::jsonb) ||
            ${JSON.stringify([event])}::jsonb
        WHERE id = ${Number(child.parent_plan_id)} AND tenant_id = ${Number(child.tenant_id)}
          AND status IN ('failed','executing')
          AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log, '[]'::jsonb)) e
            WHERE e->>'type' = 'execution.recovery_blocker'
              AND e->>'recoveryChildId' = ${String(child.id)})
      `);
      reconciled++;
    });
  }
...
/** Production recovery-child creation seam. The sweep and integration tests
 * call this exact transaction; advisory lock + source identity are the
 * database concurrency boundary. */
export async function createStaleRecoveryChildCore(
  executor: { transaction?: (fn: (tx: any) => Promise<unknown>) => Promise<unknown>; execute: (q: any) => Promise<any> },
  candidate: any,
  reason = "Automatic stale recovery refused; exact unresolved suffix requires owner approval.",
): Promise<number | null> {
  const run = async (tx: any): Promise<number | null> => {
    const binding = deriveStaleRecoveryBinding({
      tenantId: Number(candidate.tenant_id), originalPlanId: Number(candidate.id),
      version: Number(candidate.version || 1), planJson: candidate.plan_json, executionLog: candidate.execution_log,
    });
    const originalSteps = Array.isArray(candidate.plan_json?.steps) ? candidate.plan_json.steps : [];
    const unresolvedSuffix = binding
      ? originalSteps.slice(binding.firstUnresolvedStepIndex)
      : [];
    const unsafe = unresolvedSuffix
      .map((step: any) => deriveRecoveryReplaySafety(step, candidate.execution_log, candidate.plan_json))
      .find((result: { allowed: boolean }) => !result.allowed);
    if (unsafe) {
      await tx.execute(sql`UPDATE plans SET status='failed',updated_at=now(),
        execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([{type:"execution.recovery_blocker",at:nowIso(),actionable:true,reason:unsafe.reason}])}::jsonb
        WHERE id=${candidate.id} AND tenant_id=${candidate.tenant_id}
          AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log,'[]'::jsonb)) e WHERE e->>'type'='execution.recovery_blocker' AND e->>'reason'=${unsafe.reason})`);
      return null;
    }
    if (!binding) {
      await tx.execute(sql`UPDATE plans SET status='failed', updated_at=now(),
        execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([{type:"execution.recovery_blocker",at:nowIso(),actionable:true,reason:"Malformed stale plan; create a new bound recovery review."}])}::jsonb
        WHERE id=${candidate.id} AND tenant_id=${candidate.tenant_id}
          AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(COALESCE(execution_log,'[]'::jsonb)) e WHERE e->>'type'='execution.recovery_blocker' AND e->>'reason'='Malformed stale plan; create a new bound recovery review.')`);
      return null;
    }
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext(${binding.incidentKey}))`);
    const prior: any = await tx.execute(sql`SELECT id FROM plans WHERE tenant_id=${candidate.tenant_id}
      AND source='stale-plan-recovery' AND source_ref=${binding.incidentKey} LIMIT 1`);
    const existing = (prior.rows ?? prior)[0];
    if (existing?.id) return Number(existing.id);
    const steps = candidate.plan_json.steps as any[];
    const ids = new Set(steps.slice(binding.firstUnresolvedStepIndex).map((s: any) => Number(s?.n)));
    const suffix = steps.slice(binding.firstUnresolvedStepIndex).map((step: any) => ({
      ...step,
      depends_on: Array.isArray(step.depends_on) ? step.depends_on.filter((d: any) => ids.has(Number(d))) : [],
      args: step.args && typeof step.args === "object" ? Object.fromEntries(Object.entries(step.args).filter(([k]) => !String(k).startsWith("_"))) : undefined,
    }));
    const action: any = {
      version: 1, kind: "stale_plan_recovery", originalPlanId: Number(candidate.id),
      parentPlanId: Number(candidate.id), tenantId: Number(candidate.tenant_id),
      originalVersion: binding.originalVersion, originalFingerprint: binding.originalFingerprint,
      incidentKey: binding.incidentKey, firstUnresolvedStepIndex: binding.firstUnresolvedStepIndex,
      firstUnresolvedStepId: binding.firstUnresolvedStepId, stepHash: binding.stepHash,
      suffixHash: binding.suffixHash, executionEvidenceHash: binding.executionEvidenceHash,
      recoveryStepContract: candidate.plan_json?.repairAction,
    };
    action.actionHash = recoveryApprovalActionHash(action);
    const child: any = await tx.execute(sql`INSERT INTO plans
      (tenant_id,objective,source,source_ref,status,plan_json,planner_persona_id,version,parent_plan_id)
      VALUES (${candidate.tenant_id},${`Bound recovery for plan #${candidate.id}: ${reason}`},
        'stale-plan-recovery',${binding.incidentKey},'awaiting_approval',
        ${JSON.stringify({ objective: "Bound stale recovery", steps: suffix, repairAction: action })}::jsonb,
        15,1,${candidate.id}) RETURNING id`);
    const childId = Number((child.rows ?? child)[0]?.id);
    if (!childId) throw new Error("recovery child insert returned no id");
    await tx.execute(sql`UPDATE plans SET status='failed',updated_at=now(),
      execution_log=COALESCE(execution_log,'[]'::jsonb)||${JSON.stringify([{type:"execution.recovery_approval_created",at:nowIso(),recoveryChildId:childId,incidentKey:binding.incidentKey}])}::jsonb
      WHERE id=${candidate.id} AND tenant_id=${candidate.tenant_id} AND status='executing'`);
    return childId;
  };
  if (executor.transaction) return executor.transaction(run) as Promise<number | null>;
  return run(executor);
}
