...
} from "./lib/anti-sycophancy-evaluator";
import { persistSnapshotsIndividually } from "./lib/evaluator-snapshot-persistence";
...
export async function evalDailySpend(tenantId: number): Promise<EvaluatorResult> {
  ...
  try {
    ...
    const agentResult = await db.execute(sql`
      ...
      FROM messages m JOIN conversations c ON m.conversation_id = c.id AND c.tenant_id = ${tenantId}
...
export async function evalAntiSycophancy(tenantId: number): Promise<EvaluatorResult | null> {
  if (resolveAntiSycophancyMode(process.env.ANTI_SYCOPHANCY_EVAL_MODE) !== "shadow") return null;
  try {
    const result = await db.execute(sql`
    ...
    `);
    const rows = (result as any).rows || result;
    const turns: AntiSycophancyTurn[] = rows.map((row: any) => ({
      conversationId: Number(row.conversation_id),
      role: row.role === "assistant" ? "assistant" : "user",
      content: String(row.content || ""),
    }));
    const assessment = assessAntiSycophancy(turns);
    return {
    ...
    logSilentCatch("server/evaluators.ts", _silentErr);
    return {
      evaluator: "anti_sycophancy",
      status: "warning",
      metrics: { mode: "shadow", degraded: true, reason: "query-failed" },
      timestamp: Date.now(),
    };
  }
}
...
export async function runAllEvaluators(tenantId: number): Promise<EvaluatorResult[]> {
  const evaluatorPromises: Array<Promise<EvaluatorResult> | Promise<EvaluatorResult | null>> = [
  ...
  if (resolveAntiSycophancyMode(process.env.ANTI_SYCOPHANCY_EVAL_MODE) === "shadow") {
    evaluatorPromises.push(evalAntiSycophancy(tenantId));
  }
  const results = await Promise.allSettled(evaluatorPromises);

  const evaluatorResults = results
    .filter((r): r is PromiseFulfilledResult<EvaluatorResult | null> => r.status === "fulfilled")
    .map(r => r.value)
    .filter((value): value is EvaluatorResult => value !== null);

  const failedSnapshots = await persistSnapshotsIndividually(
    evaluatorResults,
    async (evalResult) => {
    ...
    (_silentErr) => logSilentCatch("server/evaluators.ts", _silentErr),
  );
  if (failedSnapshots.length > 0) {
    evaluatorResults.push({
      evaluator: "evaluator_snapshot_persistence",
      status: "warning",
      metrics: {
        degraded: true,
        failedCount: failedSnapshots.length,
        failedEvaluators: failedSnapshots,
      },
      timestamp: Date.now(),
    });
  }
