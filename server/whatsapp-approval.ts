...
export function handleWhatsAppApprovalCommand(text: string, fromJid: string, tenantId?: number): boolean {
  ...
  if (!approvalPhoneJid) {
    for (const [key, phone] of tenantApprovalPhones) {
      ...
      if (isApprovalSender(fromJid, tid)) {
        ...
        // Architect HIGH fix — cross-tenant approval guard: this fallback loop
        // authenticates the sender for tenant `tid`, so it may ONLY resolve a
        // confirmation belonging to that same tenant. Without this, a valid
        // approver for tenant A could approve tenant B's pending dangerous
        // action on a short-ID collision. Admin key (tid undefined) may only
        // resolve confirmations with no tenant or the admin tenant (1).
        const found = pending.find((p) =>
          formatShortId(p.id) === shortId &&
          (tid != null ? p.tenantId === tid : (p.tenantId == null || p.tenantId === 1)),
        );
        if (found) {
          const resolved = resolveToolConfirmation(found.id, approved, tid ?? found.tenantId ?? undefined);
          if (resolved) {
  ...
  const pending = getPendingConfirmations();
  // Same cross-tenant guard as the fallback loop above: only resolve a
  // confirmation that belongs to the tenant this sender is authorized for.
  const found = pending.find((p) =>
    formatShortId(p.id) === shortId &&
    (p.tenantId == null || p.tenantId === checkTenantId),
  );
  if (found) {
    const resolved = resolveToolConfirmation(found.id, approved, checkTenantId);
    if (resolved) {
