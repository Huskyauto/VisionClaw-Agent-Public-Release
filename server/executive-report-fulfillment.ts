...
export async function fulfillExecutiveReport(params: {
  ...
  const fileName = `executive-opportunity-report-${safeName}-${Date.now()}.pdf`;
  ...
  try {
    ...
    const coverStats = [
      intake.industry
        ...
        : { label: "Scope", value: "Website + Revenue" },
      intake.crm
    ...
    const styled = await generateStyledPdf({
      title: `${intake.company || "Executive"} Opportunity Report`,
      subtitle: "Website Positioning, Profitability Levers, and Growth Recommendations",
      companyLines: [
        `Prepared for ${intake.principalName || intake.company || params.customerEmail}`,
      ...
      footerLines: ["Executive Opportunity Report — prepared by the VisionClaw Agent platform"],
  ...
  if (!finalRelPath) {
    const pdfResult = await createPdf({
      title: `Executive Opportunity Report${intake.company ? `: ${intake.company.slice(0, 60)}` : ""}`,
