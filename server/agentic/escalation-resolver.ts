 ...
 *      test|guard|safety surface is forced AWAY from auto-fix; a FIX without a
 *      strict majority or with low fix-concordance escalates instead of guessing.
 *   3. Routing → action:
 *        · 'surface'        (ACCEPT/REJECT majority, or guard-working-as-intended)
 *                           → CLOSE the incident (ledger-only, fully reversible).
 *        · 'repo_surgeon'   (strict-MAJORITY, safe-surface FIX) → dispatchIncidentRemedy,
 ...
 *   · Code is ONLY ever modified via the jury-MAJORITY-FIX → repo_surgeon path,
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (decision.routedTo === "repo_surgeon") {
        // Majority, safe-surface FIX. File edits are pointless/harmful on the
        ...
        if (isProd()) {
          if (!dryRun) {
            await updateLedger(row.id, tenantId, {
              ...
              actionDetail: { decidedBy: "jury", note: "majority FIX but prod runtime — code fix deferred to a human (dev-only).", juryVerdict: jury.verdict },
              escalated: true,
            });
          }
          result.keptForHuman++;
