...
/**
 * Purge persisted checkpoint rows for a job-key PREFIX (tenant-scoped).
 *
 * Built for the "start provably clean" case: a prior run persisted stage
 * artifacts under this ISO week's key(s) that must NOT be resumed (e.g. a
 * wrong-window BWB weekly run whose transcription/planning checkpoints hold
 * the previous week's clips). Deleting by prefix covers BOTH the pure
 * `bwb-weekly-<isoWeek>` key and any window-suffixed variants.
 *
 * Deliberately conservative: exact tenant match + prefix LIKE with escaped
 * LIKE metacharacters, returns the deleted-row count so callers can log it.
 */
export async function purgeCheckpoints(opts: {
  tenantId: number;
  jobKeyPrefix: string;
}): Promise<number> {
  if (!opts.jobKeyPrefix || opts.jobKeyPrefix.trim().length < 8) {
    // Refuse a dangerously-broad purge (empty/short prefix would sweep
    // unrelated jobs for the tenant). 8 chars comfortably covers real
    // prefixes like "bwb-weekly-".
    throw new Error(`purgeCheckpoints: refusing over-broad jobKeyPrefix ${JSON.stringify(opts.jobKeyPrefix)} (min 8 chars)`);
  }
  const { ensurePipelineStageArtifactsTable } = await import("./pipeline-checkpoint-table");
  await ensurePipelineStageArtifactsTable();
  const { db } = await import("../db");
  const { sql } = await import("drizzle-orm");
  const escaped = opts.jobKeyPrefix.replace(/([\\%_])/g, "\\$1");
  const res = await db.execute(sql`
    DELETE FROM pipeline_stage_artifacts
    WHERE tenant_id = ${opts.tenantId} AND job_key LIKE ${escaped + "%"}
  `);
  const count = Number((res as any).rowCount ?? (res as any).count ?? 0);
