...
export async function decideApproval(params: {
  ...
  // Phase 1 — commitment-draft cards: reflect the decision onto the
  // commitments row (approved_sent / dismissed). Fire-and-forget quality
  // path; a hook failure must never break the decision itself.
  if (row && (row.context as any)?.type === "commitment_draft") {
    try {
      const { applyDraftDecision } = await import("../commitment-drafter");
      await applyDraftDecision({ tenantId: row.tenantId, context: row.context }, params.approved);
    } catch (err) {
      console.warn(`[approvals] commitment-draft decision hook failed for approval ${row.id}:`, err);
    }
  }
