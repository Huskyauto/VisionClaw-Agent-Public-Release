...
export interface CloserDeps {
  ...
  reverify: (id: number, tenantId: number) => Promise<{ status: string; details?: string }>;
  /**
...
function defaultDeps(): CloserDeps {
  return {
    ...
    async reverify(id, tenantId) {
      const { verifyProposalById } = await import("../proposal-verifier");
      const v = await verifyProposalById(id, tenantId);
...
export async function runAutonomousCodeCloser(opts: {
  ...
  for (const c of candidates) {
    try {
      ...
      try {
        fresh = await deps.reverify(c.id, tenantId);
      } catch (e: any) {
