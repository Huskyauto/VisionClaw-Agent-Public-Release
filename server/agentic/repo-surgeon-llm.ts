...
const FIX_PROPOSAL_SCHEMA = {
  ...
  properties: {
    ...
    specTestConflict: {
      type: "boolean",
      description: "TRUE if the FAILING TEST itself contradicts the documented spec, house pattern, or clear code intent (the test is wrong, not the code). Always pair with cannotFix=true and explain the conflict in diagnosis. NEVER modify correct code to satisfy a wrong test.",
    },
...
- SPEC-VS-TEST AUTHORITY RULE: if the FAILING TEST itself contradicts the documented spec, an established house pattern, or the clear intent of the code (i.e. the TEST is wrong, not the code), do NOT modify correct code to make the wrong test pass. Set cannotFix=true AND specTestConflict=true, and explain the conflict in diagnosis (cite the spec/pattern the test violates). Surfacing the conflict is the correct output.
...
export async function proposeFix(
  ...
  const model = tier2 ? "google/gemini-3.7-flash" : "gpt-5.6-sol";
  ...
  return {
    ...
    cannotFix: j.cannotFix === true || j.specTestConflict === true,
    specTestConflict: j.specTestConflict === true,
