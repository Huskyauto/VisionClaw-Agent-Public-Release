...
async function executeFix(plan: FixPlan, opts: { tenantId: number; runId: number | null }): Promise<{ ok: boolean; output?: any; error?: string }> {
  ...
  const callTool = async (name: string, args: any) => {
    ...
    // Fail CLOSED at the tenant boundary (post-edit review 2026-07-10,
    // reversing R63.17's opt-in): opts.tenantId is typed non-optional, so a
    // missing/invalid tenant here is a bug — surface it as an error rather
    // than silently executing against the admin tenant.
    if (!Number.isInteger(opts.tenantId) || opts.tenantId <= 0) {
      throw new Error(`[self-heal] refusing tool execution without a valid tenantId (got ${String(opts.tenantId)})`);
    }
...
export async function attemptSelfHeal(params: {
  ...
  if (runId) {
    await appendStep(runId, tenantId, {
