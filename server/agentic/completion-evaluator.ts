...
export interface CompletionEvidence {
  ...
  workerModels?: string[];
  /**
   * Sol #3 — risk-gated fail-open. true ⇒ this run performed CONSEQUENTIAL
   * actions (sensitive/destructive tools per the destructive-tool policy: money,
   * mass comms, deletion, prod mutation). When the judge is degraded, such a run
   * must NOT silently fall open to a clean "done" — it becomes
   * "completed_unverified" so callers surface it honestly. Omitted/false keeps
   * the prior fail-open behavior (read-only work: a judge blip never blocks it).
   */
...
export interface CompletionVerdict {
  /**
   * "completed_unverified" (Sol #3): the run finished within budget but the
   * independent judge was unavailable AND the evidence flags consequential
   * (sensitive/destructive) actions — completion is claimed, not verified.
   * Callers must present it as unverified, never as a clean "done".
   */
...
export async function evaluateCompletion(
  ...
  if (budget.exceeded) {
    ...
    if (evidence.consequentialActions === true) {
      // Sol #3 — risk gate on the fail-open path. Consequential work (money,
      // deletion, mass comms, prod mutation) with no independent verification
      // must not be reported as a clean "done". Budget passed, so the work is
      // not blocked — but the verdict is honest about the missing verification.
      verdict = "completed_unverified";
      stopConditionMet = true;
      invariantsIntact = true;
      unmetCriteria = ["independent verification unavailable (judge degraded) for a run that performed consequential actions"];
      reason = "Completion judge unavailable and this run performed consequential (sensitive/destructive) actions — passed deterministic budget checks but completion is NOT independently verified; surface as unverified.";
    } else {
      verdict = "done";
      stopConditionMet = true;
      invariantsIntact = true;
