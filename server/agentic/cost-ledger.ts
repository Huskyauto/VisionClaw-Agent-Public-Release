...
const MODEL_COST_PER_1K: Record<string, { in: number; out: number }> = {
  ...
  // R125+137.3 — platform default since the Sol migration. Without this entry the
  // unknown-gpt fallback bills it at $5/$5 per M (2x in / 3x-low out vs flagship class).
  // Flagship-class estimate (matches gpt-5.4) pending a verified live rate.
  ...
  // Gemini 3.7 Flash (2026-08-13) — escalation-only pick via OpenRouter (registry
  // overlay entry; NOT in any default routing per Bob's keep-defaults-free directive).
  // Intro OpenRouter rate $0.75/M in $3.75/M out until 2026-12-31; from 2027-01-01
  // Google's published rate becomes $1.50/$7.50 per M — bump this entry then.
  ...
  "deepseek/deepseek-v4-flash-0731": { in: 0.00009, out: 0.00018 },
  // DeepSeek V4 Pro official build (0813, stealth-shipped 2026-08-13) — frontier
  // proposer #4. Live OpenRouter rate $0.435/M in $0.87/M out verified 2026-08-13.
  // NOTE: the OLD un-dated id deepseek/deepseek-v4-pro now costs ~2.7x more
  // ($1.168/$2.336) — never revert to it.
  "deepseek/deepseek-v4-pro-0813": { in: 0.000435, out: 0.00087 },
  ...
  "moonshotai/kimi-k2.6": { in: 0.0005795, out: 0.00244 },
  ...
  "z-ai/glm-5.2": { in: 0.0004746, out: 0.0014916 },
  // R125+136 — explicit entry: unknown non-gpt/claude ids fall back to $0, so metered
  // GLM-5.1 usage would ledger as free. Live OpenRouter rate verified 2026-07-11.
  "z-ai/glm-5.1": { in: 0.000952, out: 0.002992 },
...
// OWNER_JURY_METERED=false disables the paid override entirely.
...
// Default ON (Bob 2026-06-26) — explicit opt-out only.
