...
export function mapJuryDecision(jury: any): ClassificationResult {
  ...
  // Empty/missing votes array = legacy entry shape → assume the default 3-seat
  // jury, so a strict majority (>=2) is still required to act either way.
  const total = Array.isArray(jury?.votes) && jury.votes.length > 0 ? jury.votes.length : 3;
  const conf = juryConfidence(majority, total);
  const verdict = jury?.verdict;
  const shouldEscalate = jury?.shouldEscalate === true;
  // Owner policy (2026-07-28): auto-acting on a FIX requires a STRICT MAJORITY
  // of the jury (2-of-3, 3-of-4, 3-of-5, …) — majority approve ⇒ apply,
  // majority disapprove ⇒ deny/close. The escalation flag and the fix-direction
  // concordance guard still override a majority; downstream repo-surgeon
  // gates (typecheck/tests/rollback, sensitive-surface HITL, prod refusal)
  // are unchanged.
  const strictMajority = total > 0 && majority >= Math.floor(total / 2) + 1;
  ...
  if (verdict === "FIX") {
    ...
    // changes. Escalate (never auto-fix) on ANY of: no strict majority, the
    // jury's own escalation flag (κ<0.5 / single-proposer), or low fix-direction
    // concordance (the coupled-verifier Goodhart guard).
    if (!strictMajority || shouldEscalate || lowConcordance) {
      const why = !strictMajority
        ? `no strict majority (${majority}/${total})`
        : shouldEscalate
    ...
    return {
      classification: "code_defect",
      confidence: conf,
      reason: `Jury MAJORITY voted FIX (${majority}/${total}): ${(jury?.aggregatorAnswer || "").slice(0, 300)}`,
      routedTo: "repo_surgeon",
      classifiedBy: "jury",
      safetyBlockedAutofix: false,
    };
  ...
  if ((verdict === "ACCEPT" || verdict === "REJECT") && total > 0 && majority >= Math.floor(total / 2) + 1 && !shouldEscalate) {
    return {
      classification: "safety_guard",
      confidence: conf,
