  ...
  try {
    ...
    async function worker() {
      while (true) {
        ...
        try {
          await appendStep(run.id, params.tenantId, { at: new Date().toISOString(), step: `branch:${branch.id}`, status: "started" });
          const value = await branch.fn(branch.input);
          const durationMs = Date.now() - start;
          results[idx] = { id: branch.id, success: true, value, durationMs };
          await appendStep(run.id, params.tenantId, { at: new Date().toISOString(), step: `branch:${branch.id}`, status: "completed", durationMs });
        } catch (err: any) {
          const durationMs = Date.now() - start;
          results[idx] = { id: branch.id, success: false, error: err?.message || String(err), durationMs };
          await appendStep(run.id, params.tenantId, { at: new Date().toISOString(), step: `branch:${branch.id}`, status: "failed", detail: err?.message, durationMs });
        }
      }
    }
    ...
    await completeRun(run.id, params.tenantId, {
    ...
    await failRun(run.id, params.tenantId, `runParallel orchestration error: ${err?.message || String(err)}`);
    throw err;
  }
}
...
export async function runSupervisor<TOutput>(params: {
  ...
  // Snapshots per-tenant cumulative agent_cost_ledger.cost_usd at the top of each turn
  ...
  // Cap is approximate — it counts ALL tenant ledger spend since the run started
  // (parallel runs by the same tenant inflate it). Acceptable for circuit-
  // breaker purposes; do not use as a billing meter. A configured budget fails
  // toward honesty: if the ledger cannot be checked, no additional work runs.
  ...
  const log = async (msg: string, detail?: any) => {
    await appendStep(run.id, params.tenantId, { at: new Date().toISOString(), step: msg, status: "completed", detail });
  };
  ...
  try {
    for (let turn = 0; turn < maxTurns + coachExtraTurns; turn++) {
      ...
      if (elapsedMs > maxWallClockMs) {
        const wallContract = renderFailureContract({
          reason: "timeout",
          failed: {
            what: "the run",
            why: `it hit the wall-clock cap (${(elapsedMs / 1000).toFixed(1)}s > ${(maxWallClockMs / 1000).toFixed(0)}s) before finishing`,
          },
          nextStep: "Re-run with a smaller scope, or split the request so each part fits the time budget.",
          nextStepOwner: "agent",
        });
        const finalAnswer = wallContract.text;
        await log(`turn:${turn}:wallclock_abort`, { elapsedMs, maxWallClockMs });
        await completeRun(run.id, params.tenantId, { finalAnswer, failureContract: wallContract.meta, turns: turn, history, abortedReason: "wallclock_cap" });
        return { runId: run.id, finalAnswer, history };
      }

      // R110.14 — per-loop USD budget cap (Barry Zhang: iter+budget+wallclock).
      // Snapshot the authoritative central ledger since `startedAt`. A caller
      // that opts into a hard budget must receive an honest halt when its
      // evidence is unavailable; allowing another turn would silently defeat
      // the cap.
      if (params.maxLoopUsdBudget !== undefined) {
        ...
        // never trips, defeating the whole circuit breaker. Fail closed rather
        // than pretending the cap was checked.
        const tid = params.tenantId;
        if (typeof tid !== "number" || !Number.isFinite(tid) || tid <= 0) {
          snapshotOk = false;
          console.error(`[executor] budget_cap configured but tenantId is invalid (${String(tid)}) — halting before another turn`);
        } else {
          try {
            const { db } = await import("../db");
            const { sql: sqlTpl } = await import("drizzle-orm");
            const startedIso = new Date(startedAt).toISOString();
            const r: any = await db.execute(sqlTpl`SELECT COALESCE(SUM(NULLIF(cost_usd, '')::numeric), 0)::float AS total FROM agent_cost_ledger WHERE tenant_id = ${tid} AND created_at >= ${startedIso}::timestamp`);
            spentUsd = Number(((r as any).rows ?? r)?.[0]?.total || 0);
          } catch (budgetErr: any) {
            snapshotOk = false;
            console.error(`[executor] budget_cap snapshot FAILED — halting before another turn: ${budgetErr?.message || String(budgetErr)}`);
          }
        }
        if (!snapshotOk) {
          const budgetContract = renderFailureContract({
            reason: "budget_halt",
            failed: {
              what: "the run",
              why: "its configured budget could not be verified from the authoritative cost ledger before another turn",
            },
            nextStep: "Restore the ledger connection and retry, or remove the configured cap deliberately.",
            nextStepOwner: "owner",
          });
          const finalAnswer = budgetContract.text;
          await log(`turn:${turn}:budget_unverified_abort`, { maxLoopUsdBudget: params.maxLoopUsdBudget });
          await completeRun(run.id, params.tenantId, { finalAnswer, failureContract: budgetContract.meta, turns: turn, history, abortedReason: "budget_unverified" });
          return { runId: run.id, finalAnswer, history };
        }
        if (spentUsd > params.maxLoopUsdBudget) {
          const budgetContract = renderFailureContract({
            reason: "budget_halt",
            failed: {
              what: "the run",
              why: `it hit the per-loop USD budget cap ($${spentUsd.toFixed(4)} spent > $${params.maxLoopUsdBudget.toFixed(2)} cap) before finishing`,
            },
            nextStep: "Raise the loop budget for this request, or narrow it so it completes within the cap.",
            nextStepOwner: "owner",
          });
          const finalAnswer = budgetContract.text;
          ...
          await completeRun(run.id, params.tenantId, { finalAnswer, failureContract: budgetContract.meta, turns: turn, history, abortedReason: "budget_cap" });
          return { runId: run.id, finalAnswer, history };
        }
      }
      ...
      if (decision.action === "finish") {
        ...
        await completeRun(run.id, params.tenantId, { finalAnswer, turns: turn, history });
        return { runId: run.id, finalAnswer, history };
      }
      ...
      try {
        ...
        if (heal.healed && heal.shouldRetry) {
          ...
          if (next >= maxConsecutiveFailures) {
            const lastError = heal.error || specErr?.message || "unknown";
            const contract = renderFailureContract({
              reason: "consecutive_failure_cap",
              failed: {
                what: `the "${specialist.name}" step`,
                why: `it failed ${next} times in a row (cap=${maxConsecutiveFailures}) and self-heal could not recover. Last error: ${lastError}`,
              },
              nextStep:
                "Re-run with a sharper, narrower instruction (fresh context beats more retries past 2-3 same-fix attempts), or split the request into smaller pieces.",
              nextStepOwner: "agent",
            });
            const finalAnswer = contract.text;
            await log(`turn:${turn}:circuit_break`, { specialist: specialist.name, consecutiveFailures: next, lastError });
            await completeRun(run.id, params.tenantId, { finalAnswer, failureContract: contract.meta, turns: turn + 1, history, abortedReason: "consecutive_failure_cap", failedSpecialist: specialist.name });
            return { runId: run.id, finalAnswer, history };
          }
      ...
      if (stuckEnabled) {
        ...
        if (signal.isStuck) {
          ...
          const stuckContract = renderFailureContract({
            reason: "consecutive_failure_cap",
            headline: "Stopped — the run stopped making progress",
            failed: {
              what: "the run",
              why: `it was detected as stuck (${signal.reason}). Last output: ${JSON.stringify(output).slice(0, 300)}`,
            },
            nextStep: "Re-run with a sharper, more specific instruction — fresh context beats more turns once a loop is stuck.",
            nextStepOwner: "agent",
          });
          const finalAnswer = stuckContract.text;
          await completeRun(run.id, params.tenantId, { finalAnswer, failureContract: stuckContract.meta, turns: turn + 1, history, stuck: signal });
          return { runId: run.id, finalAnswer, history };
        }
      }
    ...
    const maxTurnsContract = renderFailureContract({
      reason: "consecutive_failure_cap",
      headline: `Stopped — reached the turn limit (${maxTurns}) without finishing`,
      failed: {
        what: "the run",
        why: `it used all ${maxTurns} allowed turns without reaching a finish. Last output: ${JSON.stringify(history[history.length - 1]?.output ?? null).slice(0, 500)}`,
      },
      nextStep: "Re-run with a narrower goal, or break the request into smaller steps so each completes within the turn budget.",
      nextStepOwner: "agent",
    });
    const finalAnswer = maxTurnsContract.text;
    await completeRun(run.id, params.tenantId, { finalAnswer, failureContract: maxTurnsContract.meta, turns: maxTurns, history, truncated: true });
    return { runId: run.id, finalAnswer, history };
  } catch (err: any) {
