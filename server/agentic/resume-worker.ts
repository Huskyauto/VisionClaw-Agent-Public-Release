...
export async function resumePendingApprovalRuns(): Promise<SweepResult> {
  ...
  try {
    const rows = await db.execute(sql`
      SELECT
        ...
        r.state->>'resumeClaimedAt' AS resume_claimed_at,
        a.id          AS approval_id,
  ...
  for (const row of candidates) {
    ...
    const resumeClaimedAt: string | null = row.resume_claimed_at ?? null;
    if (resumeClaimedAt) {
      // A live process retains the continuation until it settles. A process
      // restart loses that map entry, at which point this durable claim is
      // visible and can be terminally recovered instead of stranding a run.
      if (continuations.has(runId)) continue;
      try {
        const recoveryStep = {
          at: new Date().toISOString(),
          step: "resume:claim_recovered",
          status: "failed",
          detail: { reason: "The process that claimed this approved run did not retain its continuation." },
        };
        const recovered: any = await db.execute(sql`
          UPDATE agent_runs
          SET status = 'failed',
              error = 'Resume claim recovered after worker restart; re-trigger from the caller.',
              state = COALESCE(state, '{}'::jsonb) - 'resumeClaimedAt',
              steps = COALESCE(steps, '[]'::jsonb) || ${JSON.stringify([recoveryStep])}::jsonb,
              completed_at = NOW(),
              updated_at = NOW()
          WHERE id = ${runId}
            AND tenant_id = ${tenantId}
            AND state->>'resumeClaimedAt' = ${resumeClaimedAt}
          RETURNING id
        `);
        if (((recovered.rows || recovered || []) as any[]).length > 0) result.orphaned++;
      } catch (err: any) {
        console.error(`[resume-worker] failed recovering claimed run ${runId}:`, err.message);
        result.errors++;
      }
      continue;
    }
    ...
    const resumeStep = approvalStatus === "approved"
      ? {
        at: new Date().toISOString(),
    ...
    try {
        ...
        SET state = (COALESCE(state, '{}'::jsonb) - 'pendingApprovalId') ||
                    ${JSON.stringify(
                      approvalStatus === "approved"
                        ? { resumedAt: claimedAt, resumeClaimedAt: claimedAt, lastApprovalId: approvalId, lastApprovalDecision: approvalStatus }
                        : { resumedAt: claimedAt, lastApprovalId: approvalId, lastApprovalDecision: approvalStatus },
                    )}::jsonb,
            steps = COALESCE(steps, '[]'::jsonb) || ${JSON.stringify([resumeStep])}::jsonb,
      ...
      if (approvalStatus === "approved") {
        ...
        if (entry) {
          void (async () => {
            try {
              await entry.fn({ approved: true, note, decidedBy, approvalId });
              // The resolver only hands the decision back to the supervisor;
              // its resumed work happens after this promise resolves. Keep the
              // durable marker and in-process continuation until completeRun
              // or failRun clears the marker. A process restart loses the map,
              // making the marker eligible for terminal recovery on the next
              // sweep instead of leaving a running row stranded.
              result.resumed++;
            } catch (err: any) {
              console.error(`[resume-worker] continuation for run ${runId} threw:`, err.message);
              try {
                await failRun(runId, tenantId, `Resume continuation failed: ${err.message?.slice(0, 400) || "unknown"}`);
              } catch (failErr: any) {
                console.error(`[resume-worker] failed to persist continuation failure for run ${runId}:`, failErr.message);
                result.errors++;
              }
              continuations.delete(runId);
            }
          ...
          try {
            await failRun(runId, tenantId, `Resume orphaned: approval ${approvalId} was approved but the original process is gone. Re-trigger from the caller.`);
            result.orphaned++;
          } catch (e: any) {
