...
function applyMandatoryContextPolicy(tools: ToolDefinition[], mandatoryTools: string[]): ToolDefinition[] {
  const exactLookup = mandatoryTools.includes("lookup_sam_exact_company");
  const batchLookup = mandatoryTools.includes("lookup_sam_company_cage");
  const rosterDiscovery = mandatoryTools.includes("discover_cmmc_prospects");
  if (!exactLookup && !batchLookup && !rosterDiscovery) return tools;
  return tools.filter((tool) => {
    if (CMMC_FORBIDDEN_FALLBACKS.has(tool.function.name)) return false;
    if (exactLookup && ["discover_cmmc_prospects", "lookup_sam_company_cage"].includes(tool.function.name)) return false;
    if (batchLookup && ["discover_cmmc_prospects", "lookup_sam_exact_company"].includes(tool.function.name)) return false;
    if (rosterDiscovery && ["lookup_sam_company_cage", "lookup_sam_exact_company"].includes(tool.function.name)) return false;
    return true;
  });
}
...
const PERSONA_TOOL_POLICIES: Record<string, { allowed: string[]; blocked: string[]; priority: string[] }> = {
  ...
  "imported subagent (researcher)": {
    ...
    allowed: ["memory", "knowledge", "notes", "web", "research", "charts", "ai", "diff", "evidence", "scraping", "competitorIntel", "leadEnrichment"],
    ...
    priority: ["research", "web", "evidence", "scraping", "competitorIntel", "knowledge"],
  },
  "marketing": {
  ...
  "sales": {
    allowed: ["memory", "knowledge", "notes", "web", "research", "email", "marketing", "charts", "files", "delivery", "pdf", "workspace", "ai", "crm", "leadEnrichment", "outreachSequencing", "invoicing"],
  ...
  "researcher": {
    allowed: ["memory", "knowledge", "notes", "web", "research", "charts", "files", "ai", "diff", "evidence", "scraping", "competitorIntel", "leadEnrichment"],
...
const ALWAYS_INCLUDE = new Set([
  ...
  "jury_triage",
  // Optional bounded typed classifier. Policy filtering keeps this visible only
  // to Felix and the explicitly granted Minerva persona.
  "typesafe_jev_advisory",
  ...
  "generate_design_doc",
  // R125+ (Aug 2026, Bob's directive) — introspect_tools is the rung-zero of the
  // TOOL ESCALATION LADDER: the routed menu is a curated ~25-tool subset of the
  // full 400+ registry, and executeTool runs ANY registered tool by name even if
  // it wasn't surfaced this turn. So the catalog-search tool itself must surface
  // EVERY turn — "the tool I need isn't on my menu" resolves to introspect_tools
  // search → call the found tool by name, instead of stalling or quitting.
  // (skill_seeker/create_tool remain the last rung when NO registered tool fits.)
  "introspect_tools",
]);
...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  research: ["research", "investigate", "source-backed", "prospect research", "cmmc", "sam.gov", "federal contractor", "federal-contractors", "cage code"],
...
export async function routeTools(
...
): Promise<{ tools: ToolDefinition[]; matchedCategories: string[]; totalAvailable: number; contrastiveNote?: string; curatorSignals?: { hintBoosts: number; semanticPicks: number; perfReranked: boolean; deprecatedDropped: number } }> {
  const maxTools = opts?.maxTools ?? 25;
  const userMsg = getLatestUserRoutingMessage(messages);
  const routingContext = buildRecentUserRoutingContext(messages);
  const mandatoryContextTools = getMandatoryContextTools(routingContext);
  const dedicatedCmmcRoute = mandatoryContextTools.some((name) =>
    name === "discover_cmmc_prospects" || name === "lookup_sam_company_cage" || name === "lookup_sam_exact_company"
  );
  ...
  if (!userMsg || userMsg.length < 3) {
    if (opts?.personaRole) {
      ...
      if (policy) {
        ...
        for (const toolName of mandatoryContextTools) priorityTools.add(toolName);
        ...
        let filtered = allTools.filter(t => priorityTools.has(t.function.name));
        if (filtered.length >= 5) {
          // R125+137.24 — hard cap (was: whole categories added, no slice-back).
          const protectedTools = new Set([...ALWAYS_INCLUDE, ...mandatoryContextTools]);
          const capped = enforceToolCap(filtered, maxTools, protectedTools, priorityTools);
          if (capped.trimmed > 0) {
            console.log(`[tool-router] Persona pre-filter hard cap: trimmed ${capped.trimmed} overflow tools (${filtered.length}→${capped.tools.length}, max ${maxTools})`);
            filtered = capped.tools;
          }
          console.log(`[tool-router] Persona pre-filter (${opts.personaRole}): ${filtered.length}/${totalAvailable} tools`);
          return {
            tools: applyMandatoryContextPolicy(filtered, mandatoryContextTools),
            matchedCategories: [...policy.priority, "persona_filtered"],
            totalAvailable,
          };
        }
      }
    }
    return { tools: applyMandatoryContextPolicy(allTools, mandatoryContextTools), matchedCategories: ["all"], totalAvailable };
  }
  ...
  for (const toolName of mandatoryContextTools) {
  ...
  if (dedicatedCmmcRoute) {
    // CMMC discovery is a host-locked SAM workflow. A 429 must not make the
    // model improvise through browser/stealth tools (which may be unconfigured
    // and cannot replace authoritative CAGE evidence).
    filtered = applyMandatoryContextPolicy(filtered, mandatoryContextTools);
  }

  // R125+137.24 — hard cap enforcement (ChatGPT 5.6 review finding, verified):
  // the category loop above adds WHOLE categories and only breaks after the
  // selection already crossed maxTools; the related-category expansion and
  // semantic fallback can push further. Trim back to maxTools by priority
  // (Set insertion order: ALWAYS_INCLUDE → categories in score order →
  // related → semantic), never trimming the ALWAYS_INCLUDE safe set. The
  // "return allTools" escape hatches above are intentionally NOT capped —
  // they signal "routing produced no usable signal, give the model everything".
  let capTrimmed = 0;
  {
    const protectedTools = new Set([...ALWAYS_INCLUDE, ...mandatoryContextTools]);
    const capped = enforceToolCap(filtered, maxTools, protectedTools, selectedToolNames);
    if (capped.trimmed > 0) {
      capTrimmed = capped.trimmed;
      filtered = capped.tools;
    }
  }
  ...
  // Contrastive tool-choice (agentic-reasoning survey, 2026-08): when several
  // surfaced tools have overlapping hint matches (web_search vs deep_research,
  // create_pdf vs create_styled_report, …), emit a deterministic $0 comparison
  // note the caller can append to the system prompt. Fail-open: never blocks
  // routing.
  let contrastiveNote = "";
  try {
    contrastiveNote = buildContrastiveNote(userMsg, filtered.map(t => t.function.name));
    if (contrastiveNote) console.log(`[tool-router] contrastive note injected (${contrastiveNote.length} chars)`);
  } catch (err) {
    console.warn("[tool-router] contrastive note failed:", (err as Error).message);
  }

  console.log(`[tool-router] "${userMsg.slice(0, 60)}..." → ${matchedCategories.join(",")} (${filtered.length}/${totalAvailable} tools, hints:${hintBoosts}, sem:${semanticPicks}, rerank:${perfReranked}, dep:${deprecatedDropped}, capTrim:${capTrimmed}, ~${estimateSchemaTokens(filtered)} schema tok)`);

  return {
    tools: filtered,
    matchedCategories,
    totalAvailable,
    contrastiveNote: contrastiveNote || undefined,
