...
function applyMandatoryContextPolicy(tools: ToolDefinition[], mandatoryTools: string[]): ToolDefinition[] {
  const directLookup = mandatoryTools.includes("lookup_sam_exact_company")
    || mandatoryTools.includes("lookup_sam_company_cage");
  const rosterDiscovery = mandatoryTools.includes("discover_cmmc_prospects");
  if (!directLookup && !rosterDiscovery) return tools;
  return tools.filter((tool) => {
    if (CMMC_FORBIDDEN_FALLBACKS.has(tool.function.name)) return false;
    if (directLookup && ["discover_cmmc_prospects", "lookup_sam_company_cage"].includes(tool.function.name)) return false;
...
export async function routeTools(
    ...
    if (opts?.personaRole) {
      ...
      if (policy) {
        ...
        if (filtered.length >= 5) {
          ...
          const capped = enforceToolCap(filtered, maxTools, ALWAYS_INCLUDE, priorityTools);
          if (capped.trimmed > 0) {
  ...
  let capTrimmed = 0;
  {
    const capped = enforceToolCap(filtered, maxTools, ALWAYS_INCLUDE, selectedToolNames);
    if (capped.trimmed > 0) {
