...
registerTool("scrapling_scrape", { categories: ["research", "web"], speed: "slow", isProductOutput: false, isNetworkTool: true });
registerTool("multi_source_collect", { categories: ["research", "web"], speed: "very_slow", isProductOutput: false, isNetworkTool: true });
...
registerTool("typesafe_jev_advisory", { categories: ["reasoning", "ai"], speed: "normal", isProductOutput: false, isNetworkTool: true });
...
registerTool("aeo_score", { categories: ["marketing", "content", "research"], speed: "fast", isProductOutput: false, isNetworkTool: false });
...
registerTool("discover_cmmc_prospects", { categories: ["research", "leadEnrichment"], speed: "slow", isProductOutput: false, isNetworkTool: true });
registerTool("lookup_sam_company_cage", { categories: ["research", "leadEnrichment"], speed: "slow", isProductOutput: false, isNetworkTool: true });
registerTool("lookup_sam_exact_company", { categories: ["research", "leadEnrichment"], speed: "slow", isProductOutput: false, isNetworkTool: true });
...
// R125+52.x — create_styled_report MUST live in the "pdf" category alongside
// create_pdf: report/deliverable keywords route to "pdf", and when the premium
// tool sat only in "docs" it was never mounted in delivery sessions (Felix
// could only see the low-level create_pdf). Keep "docs" too for document-y
// phrasings.
registerTool("create_styled_report", { categories: ["pdf", "docs"], speed: "normal", isProductOutput: true, isNetworkTool: false });
...
registerTool("generate_evidence_docket", { categories: ["delivery", "validation"], speed: "very_slow", isProductOutput: false, isNetworkTool: true });
...
registerTool("bwb_weekly_build", { categories: ["media", "video", "social"], speed: "very_slow", isProductOutput: false, isNetworkTool: true });
...
// Simulation Sandbox (S4) — what-if replay of historical corpora under override
// bundles inside the side-effect firewall. sandbox_run spends LLM budget on the
// model-swap corpora (capped $5/run); sandbox_report is a tenant-scoped read.
// Derived-API recipes ("watch once, skip the browser") — passive capture of
// browser XHR/fetch traffic → one $0-lane derive call → SSRF-pinned replay.
// capture_list/list are tenant-scoped reads; derive spends one free-lane LLM
// call; replay is an outbound pinned fetch; delete is tenant-scoped.
registerTool("derived_api_capture_list", { categories: ["web", "data"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("derived_api_derive", { categories: ["web", "data"], speed: "slow", isProductOutput: false, isNetworkTool: true });
registerTool("derived_api_replay", { categories: ["web", "data"], speed: "fast", isProductOutput: false, isNetworkTool: true });
registerTool("derived_api_list", { categories: ["web", "data"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("derived_api_delete", { categories: ["web", "data"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("sandbox_run", { categories: ["evaluation", "system"], speed: "very_slow", isProductOutput: false, isNetworkTool: true });
registerTool("sandbox_report", { categories: ["evaluation", "system"], speed: "fast", isProductOutput: false, isNetworkTool: false });
// sandbox_promote — promotes a tested sandbox upgrade into the jury-vetted
// Improvement list (NOT auto-apply); jury vote is an LLM call (network).
registerTool("sandbox_promote", { categories: ["evaluation", "system"], speed: "slow", isProductOutput: false, isNetworkTool: true });
// Verified Revenue Missions (S5a) — owner-only mission spine tools. Approve/kill
// are HITL-only (admin UI), deliberately NOT tools. draft_experiment reads Gmail
// + drafts via LLM (network, slow) but sends NOTHING (awaiting_approval packet).
registerTool("revenue_mission_create", { categories: ["business", "planning"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("revenue_mission_list", { categories: ["business"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("revenue_mission_status", { categories: ["business"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("revenue_mission_draft_experiment", { categories: ["business", "planning"], speed: "slow", isProductOutput: false, isNetworkTool: true });
registerTool("mission_portfolio_review", { categories: ["business"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("product_listing_create", { categories: ["business"], speed: "fast", isProductOutput: false, isNetworkTool: false });
// R125+13.x — DFY Fix Kit: audited-site remediation package (llms.txt, JSON-LD,
// meta tags, robots rules, punch-list) zipped under uploads/. Network: fetches
// the customer site via the SSRF-jailed audit fetcher.
registerTool("generate_audit_fix_kit", { categories: ["business"], speed: "slow", isProductOutput: true, isNetworkTool: true });
registerTool("product_listing_list", { categories: ["business"], speed: "fast", isProductOutput: false, isNetworkTool: false });
registerTool("create_payment_link", { categories: ["business"], speed: "normal", isProductOutput: false, isNetworkTool: true });
...
registerTool("query_project_relationships", { categories: ["research"], speed: "normal", isProductOutput: false, isNetworkTool: false });
