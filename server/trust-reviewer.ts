...
import { isReservedKnowledgeCategory } from "./lib/reserved-knowledge-categories";
...
const AUTO_APPROVE_PATTERNS: Array<{ tools: Set<string>; condition: (args: Record<string, unknown>) => boolean; label: string }> = [
  {
    ...
    condition: (args) => {
      // Reserved agent_knowledge categories carry trust/routing semantics
      // (e.g. messaging_pairing → cross-tenant SMS hijack). The storage
      // chokepoint fails closed regardless, but the review layer must not
      // greenlight them either (defense-in-depth, post-edit review 2026-07-10).
      if (isReservedKnowledgeCategory((args as any)?.category)) return false;
