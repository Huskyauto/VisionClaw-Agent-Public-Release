...
import {
  ...
  type UnreadChannel,
  type UnreadChannelSqlRow,
} from "./lib/channel-row-contracts";
...
export async function getUnreadCount(tenantId: number, personaId: number): Promise<UnreadChannel[]> {
  const result = await db.execute<UnreadChannelSqlRow>(sql`
    ...
    ORDER BY count DESC
  `);
  return normalizeUnreadChannelRows(result.rows);
}
...
export async function subscribeToChannel(tenantId: number, channelId: number, personaId: number, priority?: string): Promise<void> {
  // Fail-closed ownership check: the caller-supplied channelId must belong to
  // the caller's tenant before a subscription row is written (cross-tenant
  // channel-id guessing defense).
  const owned = await db.execute(sql`
    SELECT id FROM agent_channels WHERE id = ${channelId} AND tenant_id = ${tenantId} LIMIT 1
  `);
  if (((owned as any).rows || owned).length === 0) {
    throw new Error(`channel ${channelId} not found for tenant ${tenantId} (fail closed)`);
  }
  await db.execute(sql`
