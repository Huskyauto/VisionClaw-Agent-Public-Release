...
export async function getDecryptedPassword(id: number, tenantId: number): Promise<string | null> {
  ...
  await db.execute(sql`UPDATE credential_vault SET last_used_at = NOW() WHERE id = ${id} AND tenant_id = ${tenantId}`);
...
export async function getLoginCredentials(siteUrl: string, tenantId: number): Promise<{ username: string; password: string } | null> {
  ...
  await db.execute(sql`UPDATE credential_vault SET last_used_at = NOW() WHERE id = ${cred.id} AND tenant_id = ${tenantId}`);
  return {
    username: cred.username,
