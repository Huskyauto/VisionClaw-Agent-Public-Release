...
import { getSession, ADMIN_TENANT_ID } from "./auth";
...
type WebhookStatusSession = { tenantId: number; isAdmin: boolean } | null;

/**
 * Webhook status is process-global operational data, not tenant data. A
 * tenant-local administrator must not gain access merely by holding isAdmin.
 */
export function canReadWebhookStatus(session: WebhookStatusSession): boolean {
  return session?.tenantId === ADMIN_TENANT_ID && session.isAdmin === true;
}
...
export function registerWebhookRoutes(app: Express) {
    ...
    setImmediate(async () => {
      try {
        ...
        const conv = await storage.createConversation({
          tenantId: ADMIN_TENANT_ID,
          title: `[Hook] ${hookName}`,
          model: hookModel || settings?.defaultModel || DEFAULT_CHAT_MODEL_ID,
          thinking: settings?.thinkingEnabled ?? false,
          personaId: persona?.id || null,
        });
  ...
  app.get("/api/hooks/status", async (req: Request, res: Response) => {
    ...
    const sessionData = session ? await getSession(session) : null;
    const authed = (webhookConfig.enabled && token && token === webhookConfig.token) ||
                   canReadWebhookStatus(sessionData);
