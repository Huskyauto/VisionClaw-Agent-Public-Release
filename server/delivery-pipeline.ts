...
function publishOneFileToOwnServer(file: { fileName: string; filePath?: string; fileData?: Buffer }, deliveryId: number): string | null {
  try {
    ...
    // required"} on every play/download attempt. 90-day TTL is long enough for
    // re-shares without forcing re-delivery, short enough to age out leaks.
    ...
    try {
      ...
      const signed = signUploadUrl(publicName, ADMIN_TENANT, NINETY_DAYS_MS);
