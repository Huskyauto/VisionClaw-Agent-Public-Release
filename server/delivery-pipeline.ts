...
async function attemptUpload(req: DeliveryRequest, deliveryId: number): Promise<{
  ...
  // Verify the exact file-level URLs that may appear in the customer email.
  // A reachable folder is not proof that an individual file is shared, and
  // verifying only the primary would leave bundled file links unchecked.
  const customerFileLinks = [
    uploadResult.directDownloadLink,
    uploadResult.shareableLink,
    ...bundleResults.flatMap(({ uploadResult: bundle }) =>
      bundle?.success ? [bundle.directDownloadLink, bundle.shareableLink] : [],
    ),
  ].filter((url): url is string => typeof url === "string" && url.length > 0);
  let linkVerified = customerFileLinks.length > 0;
  if (linkVerified) {
    await sleep(1500);
    for (const fileLink of customerFileLinks) {
      let verified = await verifyShareLink(fileLink);
      if (!verified) {
        await sleep(3000);
        verified = await verifyShareLink(fileLink);
      }
      if (!verified) {
        linkVerified = false;
        break;
...
async function deliverDigitalProductInner(req: DeliveryRequest): Promise<DeliveryResult> {
  if (req.idempotencyKey) {
    ...
    if (prior) {
      // Video-job recovery obtains an exclusive, expiring job lease before it
      // reaches this point. A non-complete record therefore belongs to a dead
      // prior attempt, not a concurrent sender; release it for a fresh retry.
      await db.update(deliveryLogs).set({ idempotencyKey: null }).where(and(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!result.linkVerified) {
        lastError = "Link verification failed after all upload attempts; refusing to email an inaccessible artifact";
      ...
      if (req.sendEmail !== false && req.customerEmail && isEmailConfigured()) {
        try {
          ...
          console.error(`[delivery] #${deliveryId} Email failed; delivery remains retryable: ${emailErr.message}`);
          await updateDeliveryLog(deliveryId, { status: "email_failed", errorMessage: `Email failed: ${emailErr.message}` });
          // Silent-failure review 2026-07-27: the file is safely uploaded but the
          // customer never got the link — that must be LOUD, not a log line.
          // Alert the owner so they can resend manually; keep delivery completed
          // (the artifact exists; emailSent:false is returned to the caller).
          sendAdminAlert(deliveryId, `Customer email FAILED after successful upload — resend manually. Error: ${emailErr.message}`, req).catch(() => {});
          throw emailErr;
        }
      }
