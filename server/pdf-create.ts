...
<style>
  ...
  try {
    const driveResult = await uploadAndShare({ filePath, fileName: `${title}.pdf`, mimeType: "application/pdf", folderLabel, share: true, parentFolderId: params._projectDriveFolderId || undefined });
...
function buildStyledHtml(opts: StyledPdfOptions): string {
  let qrDataUri = "";
  try {
    const qrFile = process.env.SITE_QR_CODE_FILE || "data/qr-code.png";
...
export async function generateStyledPdf(opts: StyledPdfOptions): Promise<{
  ...
  try {
    ...
    ensureOutputDir();
    fs.writeFileSync(filePath, pdfBuffer);
    await persistToDb(filename, `${opts.title}.pdf`, new Uint8Array(pdfBuffer));
    ...
    if (opts.uploadToDrive !== false) {
      try {
        const driveResult = await uploadAndShare({
          ...
          share: true,
          parentFolderId: opts._projectDriveFolderId || undefined,
        });
