...
import { UPLOADS_DIR, isPathWithin, resolveUploadsChild } from "./uploads-path";
...
function safePath(filePath: string): string {
  ...
  if (!isPathWithin(WORKSPACE_ROOT, resolved)) throw new Error("Path escapes workspace");
  return resolved;
}

function resolveUploadPath(filePath: string): string {
  if (filePath.startsWith("/uploads/") || filePath.startsWith("uploads/")) {
    return resolveUploadsChild(filePath.replace(/^\/?uploads\//, ""));
  }
  if (path.isAbsolute(filePath)) {
    if (isPathWithin(UPLOADS_DIR, filePath) || isPathWithin(WORKSPACE_ROOT, filePath)) {
      return path.resolve(filePath);
    }
    throw new Error("Path escapes allowed PDF roots");
  }
  const inUploads = resolveUploadsChild(filePath);
  if (fs.existsSync(inUploads)) return inUploads;
  const direct = path.resolve(WORKSPACE_ROOT, filePath);
  if (isPathWithin(WORKSPACE_ROOT, direct)) return direct;
  return inUploads;
}
...
export async function createPdf(params: CreatePdfParams): Promise<{ success: boolean; path?: string; url?: string; pages?: number; fields?: number; error?: string }> {
  try {
    ...
    if (!Number.isSafeInteger(params.tenantId) || (params.tenantId as number) <= 0) {
      return { success: false, error: "Generated PDF requires a trusted tenant ID for durable storage" };
    }
    const filename = params.outputPath || `pdf_${Date.now()}.pdf`;
    const outputPath = resolveUploadsChild(path.basename(filename));
    ...
    const relativePath = `uploads/${baseName}`;
    ...
    try {
      const driveResult = await uploadAndShare({
        ...
        generatedArtifact: { artifactKind: "pdf", logicalName: displayName, sourceRunKey: `create-pdf:${baseName}` },
      });
      if (driveResult.success) {
        ...
        return { success: false, error: driveResult.error || "Drive durability verification failed" };
      }
    } catch (driveErr: any) {
      return { success: false, error: `Drive durability verification failed: ${driveErr.message}` };
    }
...
export async function fillPdf(params: FillPdfParams): Promise<{ success: boolean; path?: string; url?: string; filledFields?: string[]; error?: string }> {
  try {
    ensureOutputDir();
    const inputPath = resolveUploadPath(params.inputPath);
...
<style>
  ...
  await persistToDb(filename, `${title}.pdf`, new Uint8Array(pdfBuffer), trustedTenantId);

  let driveUrl: string | null = null;
  try {
    const driveResult = await uploadAndShare({ filePath, fileName: `${title}.pdf`, mimeType: "application/pdf", folderLabel, share: true, tenantId: trustedTenantId, generatedArtifact: { artifactKind: "pdf", logicalName: `${title}.pdf`, sourceRunKey: `html-pdf:${filename}` } });
    if (!driveResult.success) return { success: false, error: driveResult.error || "Drive durability verification failed" };
    if (driveResult.viewUrl) {
      driveUrl = driveResult.viewUrl;
      console.log(`[pdf] Uploaded to Drive: ${driveUrl}`);
    }
  } catch (err: any) {
    return { success: false, error: `Drive durability verification failed: ${err.message}` };
  }
...
export interface PdfSection {
  ...
  content?: string;
  signature?: string;
  /** Render the signature after supporting content and tables (opt-in). */
  signatureAfterContent?: boolean;
  /** Multiple body paragraphs (rendered as successive <p> blocks after `content`). */
  paragraphs?: string[];
  bullets?: string[];
  subsections?: Array<{ title: string; content?: string; bullets?: string[]; paragraphs?: string[] }>;
...
export interface StyledPdfOptions {
  ...
  tenantId?: number;
  /** Disable all platform promotion for customer-owned attestations. */
...
function renderSection(sec: PdfSection): string {
  ...
  if (sec.signature && !sec.signatureAfterContent) {
    html += `<div class="signature">${escHtml(sec.signature)}</div>`;
  }

  if (sec.paragraphs && sec.paragraphs.length > 0) {
    html += sec.paragraphs.map(p => `<p>${escHtml(p)}</p>`).join("");
  }
...
export function buildStyledHtml(opts: StyledPdfOptions): string {
  const includePlatformBranding = opts.includePlatformBranding !== false;
  let qrDataUri = "";
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .signature { font-family: 'Segoe Script', 'Brush Script MT', cursive; font-size: 24px; font-style: italic; color: #16213e; border-bottom: 1px solid #16213e; display: inline-block; min-width: 220px; padding: 6px 24px 2px 4px; margin: 8px 0 2px; }
...
<div class="cover">
  ...
  ${companyHtml}
  ${includePlatformBranding && process.env.SITE_OWNER_PHONE ? `<div class="company">Ph. ${process.env.SITE_OWNER_PHONE}</div>` : ""}
  ${statsHtml}
</div>
${sectionsHtml}
${footerHtml}
${includePlatformBranding ? `<div class="platform-url" style="page-break-before:always;text-align:center;padding:60px 40px 40px;">
...
export async function generateStyledPdf(opts: StyledPdfOptions): Promise<{
  ...
  try {
    ...
    // Fail-closed render-integrity gate (architect, post-72h review): an HTTP-200
    // body is NOT proof of a valid PDF — a proxy/upstream error page would
    // otherwise ship to a paying customer. Require the PDF magic bytes AND a
    // successful parse with at least one page before persisting/succeeding.
    if (pdfBuffer.length < 1024 || !pdfBuffer.subarray(0, 5).equals(Buffer.from("%PDF-"))) {
      console.error(`[pdf] Browserless returned 200 but body is not a PDF (${pdfBuffer.length} bytes, head=${JSON.stringify(pdfBuffer.subarray(0, 12).toString("latin1"))})`);
      return { success: false, error: "PDF render returned invalid (non-PDF) output" };
    }
    try {
      const parsed = await PDFDocument.load(new Uint8Array(pdfBuffer), { updateMetadata: false });
      if (parsed.getPageCount() < 1) {
        return { success: false, error: "PDF render returned a zero-page document" };
      }
    } catch (parseErr: any) {
      console.error(`[pdf] rendered PDF failed to parse: ${parseErr?.message || parseErr}`);
      return { success: false, error: "PDF render returned a corrupt document" };
    }
    ...
    if (opts.uploadToDrive !== false) {
      try {
        const driveResult = await uploadAndShare({
          ...
          generatedArtifact: { artifactKind: "report", logicalName: `${opts.title}.pdf`, sourceRunKey: `styled-report:${filename}` },
        });
        if (!driveResult?.success) return { success: false, error: driveResult?.error || "Drive durability verification failed" };
        if (driveResult?.viewUrl) {
