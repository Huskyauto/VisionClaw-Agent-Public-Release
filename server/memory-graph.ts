...
- Return max ${maxCategories} relevant and 2 anticipated`
    ...
    const parsed = JSON.parse(content);
    const llmRelevant: string[] = Array.isArray(parsed.relevant) ? parsed.relevant : [];
    const llmAnticipated: string[] = Array.isArray(parsed.anticipated) ? parsed.anticipated : [];
    const matchedIds = resolveIds([...llmRelevant, ...llmAnticipated]);

    // Task 147 — report-only shadow record (fire-and-forget, never blocks or
    // alters the live LLM-driven decision below).
    try {
