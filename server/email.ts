...
async function sendEmailInternal(params: {
  ...
  // Task #161 — the R98.25 bounce gate, R125+138 quality gate, and R95
  // redaction gate now live in ONE shared preflight so every transport
  // (AgentMail here, Gmail-direct in the delivery pipeline, any future
  // channel) runs the identical safeguards. See
  // server/lib/outbound-email-preflight.ts for the gate order + rationale.
  const { preflightOutboundEmail } = await import("./lib/outbound-email-preflight");
  const gated = await preflightOutboundEmail({
    to: params.to,
    cc: params.cc,
    bcc: params.bcc,
  ...
  if (gated.html) sendParams.html = gated.html;
