...
async function sendEmailInternal(params: {
  ...
  if (!params._bypassOwnerDigest) {
    try {
      ...
      flushIfDue().catch((e) => console.warn(`[email] digest flush error: ${e?.message}`, e?.stack || e));
  ...
  // R125+138 — App-wide self-healing quality gate. Any email headed to a
  // NON-owner recipient (customer/prospect/public) is scanned for generation-
  // failure debris (section placeholders, raw provider "Error: 400 ..." text,
  // AI refusal leakage, unfilled templates). One central chokepoint covers
  // outreach, newsletters, fulfillment, and every future callsite. Owner-only
  // emails are exempt — Bob's alert/digest emails legitimately quote errors.
  try {
    const { resolveOwnerEmails } = await import("./lib/owner-email");
    const ownerSet = new Set(resolveOwnerEmails().map((s) => s.toLowerCase()));
    // Only actual delivery targets (to/cc/bcc) count — replyTo is a header,
    // not a recipient; including it would wrongly gate owner-only emails that
    // carry an external reply-to address (architect finding).
    ...
    if (hasExternalRecipient) {
      const { scanCustomerFacingText, reportQualityIncident } = await import("./lib/outbound-quality-gate");
      const scan = scanCustomerFacingText([params.subject || "", bodyText, params.html || ""].join("\n"));
      // R125+139 — the scanner crashing (fail-open) must be visible beyond
      // logs: file a self-repair incident so "shipped UNSCANNED" shows up in
      // dashboards. Non-blocking; the email still sends (fail-open by design).
      if (scan.degraded) {
        reportQualityIncident({
          ...
          candidateFiles: ["server/lib/outbound-quality-gate.ts", "server/email.ts"],
      ...
      if (scan.blocked) {
        ...
        reportQualityIncident({
          ...
          error: detail,
          stage: "outbound-email",
          candidateFiles: ["server/email.ts", "server/lib/outbound-quality-gate.ts"],
    ...
    console.warn(`[email] quality gate error (failing open): ${e?.message}`); // infra bug ≠ block
  }
  ...
  // Action Ledger S4 — when this send runs under a ledgered tool dispatch
  // (ALS context set only by the S2 middleware after a successful prepare
  // write; unforgeable), stamp the attempt's idempotency key as an outbound
  // header. The sent-mail verify probe (action-ledger-probes.ts) later scans
  // the provider's message list for this header — positive provider-side
  // proof of commit, mirroring Stripe's event.request.idempotency_key.
  // Absent context (digest flush, scripts, routes) sends are unchanged.
  try {
    const { getCurrentLedgerAttempt } = await import("./lib/action-ledger-context");
    const ledgerAttempt = getCurrentLedgerAttempt();
    if (ledgerAttempt?.idempotencyKey) {
      sendParams.headers = { "X-VC-Ledger-Key": ledgerAttempt.idempotencyKey };
    }
  } catch (e: any) {
    // Advisory stamp only — never block a send on ledger plumbing.
    console.warn(`[email] ledger-key header stamp skipped: ${e?.message || e}`);
  }
...
export async function replyToEmail(params: {
  ...
  // R125+138 — replies go to external correspondents by definition, so they
  // get the same app-wide quality gate as sendEmail (architect finding: this
  // path bypassed sendEmailInternal entirely). Blocked errors propagate; gate
  // infra errors fail open.
  try {
    const { scanCustomerFacingText, reportQualityIncident } = await import("./lib/outbound-quality-gate");
    const scan = scanCustomerFacingText([params.text || "", params.html || ""].join("\n"));
    // R125+139 — degraded (scanner crash, fail-open) must be visible in the
    // self-repair loop, not only in logs. Non-blocking.
    if (scan.degraded) {
      reportQualityIncident({
        tenantId: 1,
        signature: "outbound_quality_gate_degraded",
        title: `email reply shipped UNSCANNED — quality-gate scanner crashed (fail-open)`,
        error: "scanCustomerFacingText returned degraded:true",
        stage: "outbound-email",
        candidateFiles: ["server/lib/outbound-quality-gate.ts", "server/email.ts"],
        metadata: { messageId: params.messageId, kind: "reply" },
      });
    }
    if (scan.blocked) {
      const detail = scan.reasons.join("; ").slice(0, 1000);
      reportQualityIncident({
        tenantId: 1,
        signature: "email_quality_gate_blocked",
        title: `email reply blocked by quality gate (message ${params.messageId})`,
        error: detail,
        stage: "outbound-email",
        candidateFiles: ["server/email.ts", "server/lib/outbound-quality-gate.ts"],
        metadata: { messageId: params.messageId, kind: "reply" },
      });
      throw new Error(`Email reply refused by outbound quality gate (broken/error content must never reach a customer): ${detail}`);
    }
  } catch (e: any) {
    if (e?.message?.includes("outbound quality gate")) throw e;
    console.warn(`[email] reply quality gate error (failing open): ${e?.message}`);
  }

  // R125+143 (architect HIGH closed) — replies also get the R95 outbound
  // secret-redaction gate that sendEmail applies. The quality scan above only
  // catches broken/generated debris; enforceOutbound is the egress safety
  // control (credential-shaped content, redaction-blocked payloads). Fail
  // CLOSED — a gate refusal aborts the reply.
  const { enforceOutbound } = await import("./lib/outbound-redaction");
  const textGate = enforceOutbound(params.text || "", { surface: "email:reply:text" });
  if (!textGate.ok) throw new Error(`Email reply refused by R95 (text): ${textGate.error}`);
  const replyParams: any = {
    text: textGate.payload,
  };
  if (params.html) {
    const htmlGate = enforceOutbound(params.html, { surface: "email:reply:html" });
    if (!htmlGate.ok) throw new Error(`Email reply refused by R95 (html): ${htmlGate.error}`);
    replyParams.html = htmlGate.payload;
  }
