...
async function decompose(goal: string, context: string | undefined, tenantId: number): Promise<PlanStep[]> {
...
[{"id":0,"action":"verb phrase","description":"details","tool":"tool_name","toolArgs":{},"dependsOn":[]},...]`;
  ...
  const { client, actualModelId } = await getClientForModel(HIGH_END_WORK_MODEL_ID, tenantId);
...
async function skillAwareRefine(
  ...
  try {
    ...
    if (!Number.isInteger(tenantId) || (tenantId as number) <= 0) {
      throw new Error("task planner refinement requires a valid tenantId");
    }
    const { client, actualModelId } = await getClientForModel(HIGH_END_WORK_MODEL_ID, tenantId);
    const resp = await client.chat.completions.create({
      model: actualModelId,
...
async function replanFromFailure(
  ...
  try {
    ...
    if (!Number.isInteger(plan.invokerTenantId) || (plan.invokerTenantId as number) <= 0) {
      throw new Error("task planner re-planning requires a valid tenantId");
    }
    const { client, actualModelId } = await getClientForModel(HIGH_END_WORK_MODEL_ID, plan.invokerTenantId);
    const resp = await client.chat.completions.create({
      model: actualModelId,
      messages: [
...
async function executeStep(step: PlanStep, plan: ExecutionPlan): Promise<void> {
  ...
  if (step.tool && step.tool !== "none") {
    ...
    try {
      ...
      // SECURITY (architect HIGH): `_approvedByGate` and `_conversationId` are
      // ALSO authorization/routing signals that must NEVER be trusted from
      // LLM/planner-authored step args. A step setting `_approvedByGate: true`
      // could otherwise self-approve a requiresApproval/destructive tool below;
      // a spoofed `_conversationId` could redirect tenant-scoped writes. Strip
      // both (mirrors server/plan-executor.ts + server/lobster.ts so all three
      // autonomous step executors fail closed identically).
      delete (args as Record<string, any>)._approvedByGate;
      delete (args as Record<string, any>)._conversationId;

      // SECURITY (R125+65 — architect Cluster A HIGH): the raw executeTool()
      // dispatcher does NOT run the AHB destructive-tool policy (trusted-persona /
      // structured-args / approval / value-cap gates). That gate lives in
      // executeGuardedTool — which we deliberately AVOID here because its HITL
      // confirmation await would deadlock an autonomous plan. So enforce the
      // structural policy gate directly, fail-CLOSED. Persona resolution mirrors
      // the tenant model: internal/autonomous (no invokerTenantId) and the
      // admin/owner tenant run as the trusted "system" persona; any real non-admin
      // invoker gets no trusted name → trusted-only + approval-required tools fail
      // closed, closing the plan-step policy bypass.
      let policyBlockReason: string | null = null;
      try {
        const { enforceToolPolicy } = await import("./safety/destructive-tool-policy");
        const policyPersonaName =
          plan.invokerTenantId === undefined || plan.invokerTenantId === ADMIN_TENANT_ID
            ? "system"
            : undefined;
        const pol = await enforceToolPolicy(step.tool, args, {
          tenantId: typeof args._tenantId === "number" ? args._tenantId : ADMIN_TENANT_ID,
          personaId: typeof plan.invokerPersonaId === "number" ? plan.invokerPersonaId : null,
          personaName: policyPersonaName,
          invokedVia: "planner",
          hasApproval: false,
        });
        if (pol.action === "block") policyBlockReason = `Blocked by destructive-tool policy: ${pol.reason}`;
      } catch (_polErr: any) {
        policyBlockReason = `Destructive-tool policy check failed for '${step.tool}'. Refusing to execute.`;
      }

      if (policyBlockReason) {
        step.error = policyBlockReason;
        step.status = "failed";
        step.result = { error: policyBlockReason };
      } else {
    ...
    try {
      ...
      if (!Number.isInteger(plan.invokerTenantId) || (plan.invokerTenantId as number) <= 0) {
        throw new Error("task planner step completion requires a valid tenantId");
      }
      const { client, actualModelId } = await getClientForModel(HIGH_END_WORK_MODEL_ID, plan.invokerTenantId);
      const resp = await client.chat.completions.create({
        model: actualModelId,
...
export async function planAndExecute(
  ...
  if (!Number.isInteger(invokerTenantId) || (invokerTenantId as number) <= 0) {
    throw new Error("planAndExecute requires a valid authenticated tenantId");
  }
  const tenantId = invokerTenantId as number;
  ...
  onProgress?.(plan);
  plan.steps = await decompose(goal, context, tenantId);
  ...
  const refined = await skillAwareRefine(goal, plan.steps, context, tenantId);
