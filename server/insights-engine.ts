...
const PRICING_PER_M_TOKENS: Array<[string, { in: number; out: number }]> = [
  ...
  // R125+137.3 — platform default model since the Sol migration; without this entry
  // pricingFor() falls through to {0,0} and usage insights silently report $0 for the
  // majority of conversations. Flagship-class estimate (matches gpt-5.4) pending a
  // verified live rate; metered flagship lane applies when the ChatGPT OAuth sub is off.
  ["gpt-5.6-sol", { in: 5, out: 30 }],
  ["gpt-5-mini", { in: 0.25, out: 2 }],
  // -mini must sit ABOVE the generic gpt-4.1 prefix (first-match-wins) — caught
  // by the prefix-shadowing guard in tests/cost/model-pricing-drift.test.ts.
  ["gpt-4.1-mini", { in: 0.4, out: 1.6 }],
  ["gpt-4.1", { in: 2, out: 8 }],
  // Task 72 — gpt-5 upgrade ($1.25/$10 per M). Prefix table is first-match-wins:
  // this MUST stay below the longer gpt-5.4 / gpt-5.6-sol / gpt-5-mini entries.
  ["gpt-5", { in: 1.25, out: 10 }],
  ["o4-mini", { in: 1.1, out: 4.4 }],
  ["claude-sonnet-4", { in: 3, out: 15 }],
  // ultra:free must sit ABOVE the generic "nvidia/nemotron-3" matching (first-match-wins)
  ["nvidia/nemotron-3-ultra-550b-a55b:free", { in: 0, out: 0 }],
  ["nvidia/nemotron-3-super", { in: 0.3, out: 0.6 }],
  ["x-ai/grok-4.20-multi-agent", { in: 1.25, out: 2.5 }],
  ["x-ai/grok-4.6", { in: 2, out: 6 }],
  ["x-ai/grok-4.5", { in: 2, out: 6 }],
  // Gemini 3.7 Flash — escalation-only lane (2026-08-13); synced with
  // cost-ledger/resource-predictor ($0.75/$3.75 per M; doubles 2027-01-01).
  // Must sit ABOVE the generic "google/gemma" entry region is irrelevant
  // (different prefix), but keep near other google/ entries for clarity.
  ["google/gemini-3.7-flash", { in: 0.75, out: 3.75 }],
  ["deepseek/deepseek-v4-pro-0813", { in: 0.435, out: 0.87 }],
  // -0731 snapshot must sit ABOVE the generic v4-flash prefix (first-match-wins);
  // rates synced with cost-ledger/resource-predictor ($0.09/$0.18 per M).
  ["deepseek/deepseek-v4-flash-0731", { in: 0.09, out: 0.18 }],
  ["deepseek/deepseek-v4-flash", { in: 0.14, out: 0.28 }],
  ["deepseek/deepseek-v3.2", { in: 0.269, out: 0.4 }],
  ["deepseek/deepseek-r1", { in: 0.7, out: 2.5 }],
  // glm-5.2 must sit ABOVE the generic "z-ai/glm-5" prefix (first-match-wins);
  // rates synced with cost-ledger/resource-predictor 2026-08-01 price refresh.
  ["z-ai/glm-5.2", { in: 0.4746, out: 1.4916 }],
  ["z-ai/glm-5.1", { in: 0.952, out: 2.992 }],
  ...
  // kimi-k3 must sit ABOVE the generic "moonshotai/kimi" prefix (first-match-wins)
  ["moonshotai/kimi-k3", { in: 3.0, out: 15.0 }],
  // kimi-k2.6 also ABOVE the generic prefix; synced 2026-08-01 price refresh.
  ["moonshotai/kimi-k2.6", { in: 0.5795, out: 2.44 }],
  ["moonshotai/kimi", { in: 0.5, out: 1.5 }],
  ["meta/muse-spark-1.1", { in: 1.25, out: 4.25 }],
