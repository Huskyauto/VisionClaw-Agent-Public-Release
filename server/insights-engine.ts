...
const PRICING_PER_M_TOKENS: Array<[string, { in: number; out: number }]> = [
  ...
  // Platform default model; explicit pricing prevents a {0,0} fallback.
  ...
  // Gemini 3.7 Flash — escalation-only lane.
  ["google/gemini-3.7-flash", { in: 0.75, out: 3.75 }],
  ["deepseek/deepseek-v4-pro-0813", { in: 1.1154, out: 3.3462 }],
  // -0731 snapshot must sit ABOVE the generic v4-flash prefix (first-match-wins);
  // rates synced with cost-ledger/resource-predictor ($0.065/$0.18 per M).
  ["deepseek/deepseek-v4-flash-0731", { in: 0.065, out: 0.18 }],
  ...
  // rates synced with cost-ledger/resource-predictor.
  ["z-ai/glm-5.2", { in: 0.966, out: 3.036 }],
  ["z-ai/glm-5.3-flash", { in: 0.075, out: 0.25 }],
  ...
  // kimi-k2.6 also sits above the generic prefix.
