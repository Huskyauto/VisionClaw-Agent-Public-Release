...
async function processEntry(entry: RegistryEntry, dryRun: boolean): Promise<EntryResult> {
  ...
  // (arXiv:2606.27226) against the current holistic scalar judge on the SAME
  // before/after outputs. This NEVER changes the apply decision (measure-first,
  // per the acceptance-gate non-regression principle) — it only records whether
  // the binary decomposition discriminates the upgrade more sharply than the
  // opaque scalar reward. Fully fail-open: a measurement error can't disturb the
  // nightly run. Disable with SKILL_OPT_EVAL=off.
