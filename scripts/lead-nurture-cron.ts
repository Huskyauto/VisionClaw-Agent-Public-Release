...
// R125+13.15 (architect finding closed): tenant must be explicit — matches the
// wire-wedges.ts contract. --tenant-id=N or NURTURE_TENANT_ID env; legacy
// single-tenant default 1 only with ALLOW_DEFAULT_TENANT=1.
function resolveTenantId(): number {
  ...
  const fromEnv = Number(process.env.NURTURE_TENANT_ID || NaN);
  ...
  console.error("[nurture] tenant required: pass --tenant-id=N or NURTURE_TENANT_ID env, or set ALLOW_DEFAULT_TENANT=1 for the legacy single-tenant default.");
  process.exit(2);
}
    ...
    staleRes = await db.execute(sql`
      ...
      WHERE l.tenant_id = ${TENANT_ID}
        ...
        AND NOT EXISTS (
          ...
          WHERE d.lead_id = l.id AND d.tenant_id = ${TENANT_ID} AND d.status = 'pending_review'
        )
...
function draftFor(wedge: string, name: string): { subject: string; body: string } {
  ...
  switch (wedge) {
    ...
    case "smart-leads":
      return {
        subject: "Want 5 of your leads researched free?",
        body: `Hi ${first},\n\nThanks for the Smart Leads signup. Quick offer while we onboard design partners: send me 5 raw leads (just the company domain or a contact email) and I'll send back full research dossiers — company snapshot, buying signals, and a suggested opener — free, no strings.\n\nIf they're good, we talk about the monthly tier. If not, you keep the dossiers.\n\n—Bob`,
      };
    default:
      return {
