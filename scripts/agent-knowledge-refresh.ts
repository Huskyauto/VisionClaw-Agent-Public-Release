...
import { LOOP_CONTRACTS } from "../server/loop-contracts";
...
actually receive.`,
  ...
  {
    title: "platform_briefing:R125+62:autonomous_loop_contracts",
    category: "platform_briefing",
    priority: 1,
    content: `R125+62 — Autonomous-loop contract system (how every background loop is governed).

WHAT: the platform runs a fleet of long-running autonomous loops (CI self-healer,
auto git push, jury-queue drainer, nightly skill optimizer, tenant-isolation audit,
BWB weekly render, this knowledge refresh, the heartbeat scheduled-task runner).
Each one now has a typed LoopContract in server/loop-contracts.ts declaring its
objective, check (how it verifies the last pass worked), feedback, stop[] conditions,
escalate[] (when it hands control to a human), authority, failMode, verifier
(independent | self | deterministic — who is allowed to say "no", the generator≠evaluator
split), and spend (capped | bounded | none | unbounded — the token-cap ceiling).

INVARIANTS: every contract MUST have non-empty stop[] (no unbounded loops) and
non-empty escalate[] (no silent forever-runs); every mutating or destructive loop
MUST be failMode "closed"; no loop may grade its OWN output (verifier "self" FAILS
the audit — the generator/evaluator split) or spend without a ceiling (spend
"unbounded" FAILS). The Loop Doctor (scripts/loop-contract-audit.ts, run as
weekly-maintenance Pass 12) audits all of this AND derives the live .replit workflow
set, ERRORing if any running loop has no contract — a new loop cannot ship ungoverned.

HOW AGENTS USE THIS: to learn what a specific loop does, when it stops, or when it
escalates, retrieve its entry via search_knowledge / recall_capabilities — each loop
is indexed under "loop-contract:<id>" (e.g. "loop-contract:jury-queue-drainer").
Do NOT design around or bypass a loop's escalate path; if a loop should behave
differently, edit its contract in server/loop-contracts.ts (the single source of
truth that the Loop Doctor, this knowledge index, and the capability registry all
read) — never fork its behavior silently.`,
  },
  {
...
// R125+62 — index the autonomous-loop contracts so every persona can retrieve,
// via search_knowledge / recall_capabilities, exactly what each long-running
// loop does, how it verifies itself, when it stops, and when it escalates to a
// human. Imports the typed LOOP_CONTRACTS registry directly (not a regex parse)
// so this can never drift from the source of truth — add a loop, it's indexed.
function walkLoopContracts(): Indexable[] {
  return LOOP_CONTRACTS.map((c) => {
    const content = [
      `Autonomous loop: ${c.name} (id: ${c.id})`,
      `Source: ${c.source}`,
      `Workflow: ${c.workflow ?? "(in-process / heartbeat-driven, no .replit workflow)"}`,
      `Trigger: ${c.trigger}`,
      `Authority: ${c.authority} · Fail mode: ${c.failMode}`,
      `Evaluator: ${c.verifier} (who is allowed to say "no" — generator≠evaluator) · Spend: ${c.spend} (token-cap posture)`,
      ``,
      `Objective (what it accomplishes): ${c.objective}`,
      `Check (how it verifies the last attempt worked): ${c.check}`,
      `Feedback (what it does with what it learned): ${c.feedback}`,
      ``,
      `Stops when:`,
      ...c.stop.map((s) => `  • ${s}`),
      ...
      ...c.escalate.map((s) => `  • ${s}`),
      ``,
      `Governance: contract lives in server/loop-contracts.ts; the Loop Doctor`,
      `(scripts/loop-contract-audit.ts, weekly-maintenance Pass 12) fails CI if`,
      `this loop ever ships without a complete contract. Do NOT bypass a loop's`,
      `escalate path — mutating/destructive loops fail CLOSED by invariant.`,
    ].join("\n");
    return {
      title: `loop-contract:${c.id}`,
...
function walkInternalCapabilities(): Indexable[] {
  return [{
    title: "capability:portable_skill_behavior_bundle",
    category: "internal_capability",
    priority: 1,
    content: [
      "Owner/admin internal capability: portable learned skill behavior bundles.",
      "Source: server/lib/skill-behavior-bundle.ts",
      "Exports: createPortableSkillBehaviorBundle, verifyPortableSkillBehaviorBundle, importPortableSkillBehaviorBundle.",
      "Purpose: signed, tenant-bound, expiring, compatibility-checked learned behavior transfer.",
      "Safety: destination managed-skill scan; review-only quarantine; foreign jury approval remains provenance and never authorizes promotion.",
      "This is not a persona tool, route, or public customer capability.",
    ].join("\n"),
  }];
}

async function getActiveTenantIds(): Promise<number[]> {
  const result = await db.execute(sql`SELECT id FROM tenants WHERE is_active = true`);
  const tenantIds = ((result as any).rows || result)
    .map((row: any) => Number(row.id))
    .filter((id: number) => Number.isSafeInteger(id) && id > 0);
  if (tenantIds.length === 0) {
    throw new Error("No active tenants available for the platform knowledge catalog");
  }
  return tenantIds;
}

async function upsertIndexables(
  items: Indexable[],
  label: string,
  tenantIds: number[] = [ADMIN_TENANT_ID],
): Promise<{ inserted: number; updated: number; changed: Indexable[] }> {
  let inserted = 0;
  let updated = 0;
  const changed: Indexable[] = [];
  const canonicalTenantId = tenantIds.includes(ADMIN_TENANT_ID) ? ADMIN_TENANT_ID : tenantIds[0];
  for (const tenantId of tenantIds) {
    for (const b of items) {
      ...
      if (rows.length > 0) {
        const id = rows[0].id;
        const contentChanged = rows[0].content !== b.content;
        const embeddingReset = contentChanged
          ? sql`, embedding = NULL, embedding_vec = NULL`
          : sql``;
        ...
        `);
        updated++;
        if (contentChanged && tenantId === canonicalTenantId) changed.push(b);
      } else {
        await db.execute(sql`
          INSERT INTO agent_knowledge (title, content, category, priority, persona_id, tenant_id, source, created_at, updated_at)
          VALUES (${b.title}, ${b.content}, ${b.category}, ${b.priority}, NULL, ${tenantId}, ${label}, NOW(), NOW())
        `);
        inserted++;
        if (tenantId === canonicalTenantId) changed.push(b);
      }
    }
  }
...
async function upsertBriefs(tenantIds: number[]): Promise<{ inserted: number; updated: number }> {
  let inserted = 0;
  let updated = 0;
  for (const tenantId of tenantIds) {
    for (const b of BRIEFS) {
      ...
      if (rows.length > 0) {
        ...
        await db.execute(sql`
          INSERT INTO agent_knowledge (title, content, category, priority, persona_id, tenant_id, source, created_at, updated_at)
          VALUES (${b.title}, ${b.content}, ${b.category}, ${b.priority}, NULL, ${tenantId}, 'platform_briefing', NOW(), NOW())
        `);
        inserted++;
      }
    }
...
async function main() {
  ...
  const t0 = Date.now();
  let activeTenantIds: number[];
  try {
    activeTenantIds = await getActiveTenantIds();
  } catch (e: any) {
    console.error("[agent-knowledge-refresh] active tenant lookup FAILED:", e.message);
    process.exit(2);
  }
  console.log(`[agent-knowledge-refresh] platform catalog target: ${activeTenantIds.length} active tenant(s)`);
  ...
  let upsertResult;
  try {
    upsertResult = await upsertBriefs(activeTenantIds);
  } catch (e: any) {
  ...
  const changedSources: Indexable[] = [];
  try {
    const rlog = parseReleaseLog();
    const r1 = await upsertIndexables(rlog, "release_log", activeTenantIds);
    changedSources.push(...r1.changed);
  ...
  try {
    ...
    const r2 = await upsertIndexables(aSkills, "agent_skill", activeTenantIds);
    changedSources.push(...r2.changed);
  ...
  try {
    ...
    const r3 = await upsertIndexables(oSkills, "output_skill", activeTenantIds);
    changedSources.push(...r3.changed);
  ...
  try {
    const loops = walkLoopContracts();
    const r4 = await upsertIndexables(loops, "loop_contract", activeTenantIds);
    changedSources.push(...r4.changed);
    console.log(`[agent-knowledge-refresh] loop-contracts: inserted=${r4.inserted}, updated=${r4.updated} (of ${loops.length} loops)`);
  } catch (e: any) {
    console.error(`[agent-knowledge-refresh] loop-contract indexing FAILED (non-fatal):`, e.message);
  }
  try {
    const internalCapabilities = walkInternalCapabilities();
    const r5 = await upsertIndexables(internalCapabilities, "internal_capability", activeTenantIds);
    changedSources.push(...r5.changed);
    console.log(`[agent-knowledge-refresh] internal-capabilities: inserted=${r5.inserted}, updated=${r5.updated} (of ${internalCapabilities.length} capabilities)`);
  } catch (e: any) {
    console.error(`[agent-knowledge-refresh] internal-capability indexing FAILED (non-fatal):`, e.message);
  }

  // ────────────────────────────────────────────────────────────────────
  // Compile-on-ingest (LLM Wiki borrow, 2026-07-30 — see
  // .agents/memory/karpathy-llm-wiki-verdict.md). CHANGED sources only are
  // decomposed into concept entries that UPDATE existing compiled_concept
  // rows (update-not-append), so knowledge revises instead of piling up.
  // Advisory + fail-open: any failure logs and moves on. Spend-bounded:
  // one fast-tier call per source, max KNOWLEDGE_COMPILE_MAX_SOURCES/run
  // (drift catches up across runs). Kill switch: KNOWLEDGE_COMPILE_DISABLED=1.
  // ────────────────────────────────────────────────────────────────────
  try {
    const {
      isCompileDisabled, buildCompilePrompt, parseCompileResponse, conceptTitle,
      clampMaxSources, COMPILE_SOURCE, COMPILE_CATEGORY, CONCEPT_TITLE_PREFIX,
      COMPLETION_MAX_TOKENS,
    } = await import("../server/lib/knowledge-compile");
    if (isCompileDisabled()) {
      console.log(`[agent-knowledge-refresh] compile: SKIPPED (KNOWLEDGE_COMPILE_DISABLED=1)`);
    } else if (changedSources.length === 0) {
      console.log(`[agent-knowledge-refresh] compile: no changed sources this run`);
    } else {
      const maxSources = clampMaxSources(process.env.KNOWLEDGE_COMPILE_MAX_SOURCES);
      let batch = changedSources.slice(0, maxSources);
      // R125+143 review fix — atomic claim-before-spend for the compile fan-out
      // (memory: autonomous-loop-claim-before-spend / loop-contract-spend-proof).
      // A fixed work-set bounds ONE run, but overlapping workflow runs could
      // spend outside the autonomous daily cap without a reservation.
      const { claimAutonomousBudget } = await import("../server/agentic/autonomous-budget");
      const compileClaim = await claimAutonomousBudget({
        tenantId: ADMIN_TENANT_ID,
        estimatedUsd: Math.max(0.01, batch.length * 0.02), // conservative per-source ceiling on the fast tier
        label: "agent-knowledge-refresh:compile",
      });
      if (!compileClaim.ok) {
        console.warn(`[agent-knowledge-refresh] compile: BLOCKED by autonomous budget (${compileClaim.reason || "cap reached"}) — skipping LLM compilation this run (deterministic sync already complete).`);
        batch = [];
      }
      const { getClientForModel, getModelForTier } = await import("../server/providers");
      const modelId = getModelForTier("fast");
      let compiledRows = 0, degraded = 0;
      for (const src of batch) {
        try {
          // Existing-concept context: recent compiled rows + token-overlap with the source title.
          const existingRes = await db.execute(sql`
            SELECT title, content FROM agent_knowledge
             WHERE tenant_id = ${ADMIN_TENANT_ID} AND persona_id IS NULL
               AND source = ${COMPILE_SOURCE}
             ORDER BY updated_at DESC
             LIMIT 15
          `);
          const existing = (((existingRes as any).rows || existingRes) as Array<{ title: string; content: string }>)
            .map((r) => ({ key: r.title.replace(CONCEPT_TITLE_PREFIX, ""), content: r.content }));
          ...
          const timeout = new Promise<never>((_, reject) => {
            timer = setTimeout(() => reject(new Error("compile call timeout after 90s")), 90_000);
          });
          let raw = "";
          try {
            const comp: any = await Promise.race([call, timeout]);
            raw = comp?.choices?.[0]?.message?.content ?? "";
          } finally {
            if (timer) clearTimeout(timer);
            Promise.resolve(call).catch(() => {}); // swallow late settle
          }
          const concepts = parseCompileResponse(raw);
          if (concepts === null) {
            degraded++;
            console.error(`[agent-knowledge-refresh] compile: unparseable output for ${src.title} (degraded, not "no concepts")`);
            continue;
          }
          if (concepts.length) {
            const res = await upsertIndexables(
              concepts.map((c) => ({
                title: conceptTitle(c.key),
                category: COMPILE_CATEGORY,
                priority: 2,
                content: `${c.title}\n\n${c.summary}\n\n(last compiled from: ${src.title})`,
              })),
              COMPILE_SOURCE,
            );
            compiledRows += res.inserted + res.updated;
          }
        } catch (e: any) {
          degraded++;
          console.error(`[agent-knowledge-refresh] compile FAILED for ${src.title} (non-fatal):`, e.message);
        }
      }
      console.log(`[agent-knowledge-refresh] compile: ${batch.length}/${changedSources.length} changed sources → ${compiledRows} concept rows upserted, ${degraded} degraded`);
    }
  } catch (e: any) {
    console.error(`[agent-knowledge-refresh] compile step FAILED (non-fatal):`, e.message);
  }

  // Embedding backfill for the sources THIS script manages. upsertIndexables
  // writes content but not embedding_vec, and the startup backfill in
  // server/embeddings.ts only covers source='autoresearch' — so release-log /
  // skill / loop-contract rows were invisible to vectorSearchKnowledge
  // (recall_capabilities) until this step. Bounded per run; non-fatal.
  try {
    const { initPgVector, generateEmbedding, storeEmbeddingVec } = await import("../server/embeddings");
    await initPgVector(); // storeEmbeddingVec silently no-ops unless pgvector is initialized in THIS process
    const activeTenantSql = sql.join(activeTenantIds.map((tenantId) => sql`${tenantId}`), sql`, `);
    const missing = await db.execute(sql`
      SELECT id, title, content FROM agent_knowledge
      WHERE embedding_vec IS NULL
        AND tenant_id IN (${activeTenantSql}) AND persona_id IS NULL
        AND source IN ('release_log', 'agent_skill', 'output_skill', 'loop_contract', 'internal_capability', 'platform_briefing', 'knowledge_compile')
      ORDER BY id DESC
      LIMIT 120
    `);
    ...
    for (const row of rows) {
      const text = `${row.title} ${row.content}`.slice(0, 6000);
      const emb = await generateEmbedding(text);
      if (emb) {
        const stored = await storeEmbeddingVec("agent_knowledge", row.id, emb, { expectedKnowledgeContent: text });
        if (stored) embedded++;
      }
      await new Promise(r => setTimeout(r, 150));
    }
    const still = await db.execute(sql`
      SELECT count(*)::int AS n FROM agent_knowledge
      WHERE embedding_vec IS NULL
        AND tenant_id IN (${activeTenantSql}) AND persona_id IS NULL
        AND source IN ('release_log', 'agent_skill', 'output_skill', 'loop_contract', 'internal_capability', 'platform_briefing', 'knowledge_compile')
    `);
    const remaining = (((still as any).rows || still)[0])?.n ?? -1;
    console.log(`[agent-knowledge-refresh] embedding backfill: ${embedded}/${rows.length} rows embedded, ${remaining} still missing (release_log/skills/loop-contracts)`);
  } catch (e: any) {
    console.error(`[agent-knowledge-refresh] embedding backfill FAILED (non-fatal):`, e.message);
  }
