...
async function upsertIndexables(items: Indexable[], label: string): Promise<{ inserted: number; updated: number; changed: Indexable[] }> {
  let inserted = 0;
  let updated = 0;
  ...
  for (const b of items) {
    ...
    if (rows.length > 0) {
      ...
      `);
      updated++;
      if (contentChanged) changed.push(b);
    } else {
      await db.execute(sql`
...
async function main() {
  ...
  try {
    ...
    const r4 = await upsertIndexables(loops, "loop_contract");
  ...
  try {
    ...
    const missing = await db.execute(sql`
      ...
      WHERE embedding_vec IS NULL
        AND tenant_id = ${ADMIN_TENANT_ID} AND persona_id IS NULL
    ...
    for (const row of rows) {
      ...
      if (emb) {
        await storeEmbeddingVec("agent_knowledge", row.id, emb);
        embedded++;
      }
