...
const HEADLINE_STATS = {
  tools: "391 (+ 4 MCP memory tools, external surface)",
  skills: "32 (.agents/) + 62 (db) + 38 (output-skills/) = 132 reference surfaces",
  personas: "16",
  capabilities: "126",
  tables: "198",
  indexes: "581",
  ...
  try {
    ...
    const sections: any[] = [
      ...
      },
      {
        ...
        content:
          "Every skill currently registered in the skills table (62 entries). A separate count of 31 lives on disk under .agents/skills/ — these are agent operating runbooks, not user-runnable AI skills. Both numbers are canonical; surfaces should mention all three as 31 (.agents) + 62 (DB) + 34 (output-skills) = 127 reference surfaces.",
