...
import { createStableVideoSnapshot, validateRenderedVideo } from "./lib/video-output-validation";
...
async function main(): Promise<number> {
  ...
  if (!scriptPath || !existsSyncEIO(scriptPath)) die("SCRIPT env var must point to an existing script JSON");
  ...
  if (process.env.DELIVER === "true") {
    ...
    if (script.thumbnailPrompt) {
      ...
      try {
        // best-effort thumbnail inside this try/catch — a transient EIO here is
        // caught below and the render still completes. eio-safe: best-effort thumb.
        if (!fs.existsSync(tp)) {
          ...
          writeFileSyncEIO(tp, Buffer.from(m[2], "base64"));
        }
        thumbnailPath = tp;
      } catch (e: any) {
    ...
    let videoSnapshot: ReturnType<typeof createStableVideoSnapshot>;
    try {
      videoSnapshot = createStableVideoSnapshot(outMp4);
    } catch (e: any) {
      die(`refusing to deliver unsafe GitHub render ${outMp4}: ${e?.message || e}`);
    }
    const videoValidation = validateRenderedVideo(videoSnapshot.filePath);
    if (!videoValidation.ok) {
      videoSnapshot.cleanup();
      die(`refusing to deliver invalid GitHub render ${outMp4}: ${videoValidation.reason}`);
    }
    console.log(`[gh-render] render output passed pre-delivery validation: ${videoValidation.reason}`);
    ...
    try {
      delivery = await deliverDigitalProduct({
        ...
        productName: `Built With Bob — ${script.title}`, filePath: videoSnapshot.filePath, fileName: path.basename(outMp4),
        // During a weekly orchestrated run, the orchestrator owns the approval
        // email after it verifies this durable Drive receipt. Standalone renders
        // retain their normal delivery email.
        mimeType: "video/mp4", sendEmail: !process.env.BWB_ORCHESTRATOR_RUN_ID,
        emailSubject: `${script.videoId} ready (GitHub render) — ${script.title}`,
        emailBody: `Rendered via the GitHub Actions farm (${chapters} parallel containers).\nPlay/Download links below.`,
        idempotencyKey: `bwb-weekly:${process.env.BWB_JOB_KEY || script.videoId}:delivery`,
      });
    } finally {
      videoSnapshot.cleanup();
    }
    ...
    try {
      ...
      const sidecar = {
        ...
        orchestratorRunId: process.env.BWB_ORCHESTRATOR_RUN_ID || null,
        renderedVia: "github-actions",
        builtAt: new Date().toISOString(),
      };
      writeFileSyncEIO(resultPath, JSON.stringify(sidecar, null, 2));
    ...
    if (!delivery.success) {
      console.error(`[gh-render] delivery failed for #${delivery.deliveryId}; refusing to report render success`);
      return 4;
    }
  } else {
