...
const STEP_EXECUTORS = ["task-planner.ts", "lobster.ts", "plan-executor.ts", "heartbeat.ts"];
...
const EXECUTETOOL_ALLOWLIST = new Set<string>([
  ...
  "heartbeat.ts",               // R125+137.36 — BWB recap self-heal redirect calls the HARDCODED tool bwb_weekly_build only, enforceToolPolicy-gated first (hasApproval literal false), trust derived from the RESOLVED persona after chain-of-command + pace cap
  ...
  "lib/bwb-web-chat-tool-executor.ts", // owner-authenticated BWB shortcut; this single-purpose seam hardcodes bwb_weekly_build and accepts no caller-supplied tool name; routes.ts enforces policy first
