...
const DEFAULT_LOCKED_FRONTIER = ["claude-opus-5", "gpt-5.6-sol", "gemini-3.5-flash", "deepseek/deepseek-v4-pro-0813"];
...
async function runLiveDiscovery(protectedIds: string[]): Promise<{ raw: any[]; rankingChanges: { promoted: string[]; retired: string[] } }> {
  ...
  try {
    ...
    try {
      ...
      if (ranked === null) {
        log(`ranking-driven auto-add SKIPPED — Artificial Analysis fetch DEGRADED (API/timeout/shape failure, not an empty ranking); existing ranking-driven models left untouched.`);
