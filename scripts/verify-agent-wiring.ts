 ...
 *   16 STALE CURATED DOC (doc-only PERSONA_DOCS tools_doc names an unknown tool-like token)
 *
 * Bitmask: dead(1) | drift(2) | orphan_skill(4) | schema_gap(8) | stale_curated_doc(16).
 * Multiple concurrent failures OR'd together.
 */
...
const MAIN_AGENT_ONLY_SKILLS = new Set<string>([
  // diagram-design: full-fidelity reference lives on disk for the main agent;
  // personas get the condensed global "Diagram Design" DB skill row (id 63).
  "diagram-design",
  // deep-learning-math-triage: main-agent mathematical claim router/reference;
  // invoked during paper and model-theory reviews, not exposed as a persona tool.
  "deep-learning-math-triage",
  // delegation-contract: main-agent subagent governance; customer personas do not spawn Replit subagents.
  "delegation-contract",
  // scientific-paper-15-steps: main-agent operational writing method (PLOS 15-step);
  // used when authoring research-grade papers/reports, not a persona tool surface.
  "scientific-paper-15-steps",
  ...
  "dependency-upgrade",
  "feature-contract",
  "guides-vs-forces-explainer", // main-agent-only communication skill: guides-vs-forces framing for explainer content (R125+152)
  "independent-review-brief", // main-agent-only export-safe context packet for independent AI review
  "income-opportunity-engine", // main-agent-only evidence-to-opportunity funnel; hands selected offers to revenue-program-builder
  "load-test-gate",
  "loop-integrity-policy", // main-agent-only governance skill: evaluator-integrity contract for self-improving/auto-apply loops (R125+154)
  "low-cost-release", // main-agent-only operational skill: tiered release ladder + anti-waste rules for GitHub/mirror pushes
  "module-girth-guard", // main-agent-only operational skill: god-file alarm + safe-split playbook (fires from preflight-file-girth gate, not persona-invoked)
  "new-persona-onboarding",
  "new-tool-registration",
  "owner-notification",
  "ponytail",
  "post-edit-code-review",
  "underdog-vs-empire-post", // main-agent-only operational skill: viral anti-incumbent post format + run log (Bob 2026-08-06)
  "plan-replay-drift-watch", // main-agent-only tripwire skill: watch plan-replay cache for negative transfer, MemHarness-informed (Bob 2026-08-07)
  "token-usage-audit", // main-agent-only operational skill: 3-phase HITL config-level token-waste audit (Bob 2026-07-30)
  "pre-build-design-gate", // main-agent-only operational skill: 8-question pre-build stress test (cost, state, failure, dedup, limits, kill switch, dispersal, capability delta)
  "post-edit-pipeline",
  "production-verification",
  "public-mirror-push",
  "release-cutting",
  "replit-md-maintenance",
  "revenue-program-builder", // main-agent-only offer-to-fulfillment runbook
  "schema-migration",
  "security-hardening",
  "silent-failure-hunter",
  "tdd",
  "typesafe-ai", // official TypeSafe engineering skill; guides the main agent when designing or updating Jev-backed features
  "visionclaw-config-and-flags",
  "visionclaw-debugging-playbook",
  "visionclaw-domain-reference",
  "visionclaw-engineering-atlas",
  "visionclaw-failure-archaeology",
  "visionclaw-proof-and-analysis-toolkit",
  "visionclaw-research-frontier",
  "visionclaw-research-methodology",
  "visionclaw-tools-layer-split-campaign",
...
async function main(): Promise<void> {
  ...
  // Per-persona overrides for wide-surface-BY-DESIGN personas. Rationale
  // (Bob 2026-07-22): tools must stay VISIBLE in doctrine so agents never
  // falsely claim "I don't have a tool for that" — so executor/engineer
  // personas legitimately mention more. Thresholds sit just above each
  // persona's audited baseline so any NEW sprawl still trips the warning;
  // everyone else keeps the strict default of 30.
  const SPRAWL_WARN_OVERRIDES: Record<number, number> = {
    1: 36, // VisionClaw — router persona, doctrine enumerates the routing map (baseline 32)
    2: 72, // Felix — the executor: widest surface by design (baseline 67)
    3: 42, // Forge — engineering persona holds the code/CI/repair families (baseline 38)
    16: 37, // Robert — support persona with wellness+delivery families (baseline 33)
  };
  const personaToolCounts: { id: number; name: string; count: number; threshold: number }[] = [];
  for (const p of personas) {
    const loop = perPersonaOperatingLoop(p.operating_loop || "");
    const tdoc = perPersonaToolsDoc(p.tools_doc || "");
    ...
    personaToolCounts.push({
      id: p.id,
      name: p.name,
      count: mentioned.size,
      threshold: SPRAWL_WARN_OVERRIDES[p.id] ?? PERSONA_TOOL_SPRAWL_WARN,
    });
  }
  const sprawlWarn = personaToolCounts.filter((p) => p.count > p.threshold);
  ...
  try {
    ...
    const schemaFiles = [
      ...
      path.join(process.cwd(), "shared/models/self-repair.ts"),
  ...
  try {
    ...
    // so schema-pair checks keep seeing tools after their definitions migrate.
    const { readToolSourcesConcatenated } = await import("./lib/tool-source-files");
    const toolsSrc = readToolSourcesConcatenated();
  ...
  // ──────────────────────────────────────────────────────────────────────
  // Check 7: STALE CURATED DOC (HARD FAIL) — Task 116 (2026-08-01).
  // Since Task 115, PERSONA_DOCS[*].tools_doc literals in
  // server/seed-persona-prompts.ts are documentation-only (persona-sync is
  // the single canonical writer of personas.tools_doc), but they are still
  // parsed by scripts/generate-public-docs.ts for the public mirror. With no
  // writer keeping them honest they can silently drift from the live tool
  // surface. This check flags any snake_case token in a curated tools_doc /
  // tools_doc_addendum literal that is neither a live registered tool nor an
  // explicitly allowlisted non-tool token. When a tool is renamed/removed,
  // its curated mention becomes unknown and trips this check.
  // ──────────────────────────────────────────────────────────────────────
  // Shared scan logic + non-tool allowlist live in
  // scripts/lib/curated-doc-staleness.ts (also used by the static
  // scripts/verify-curated-tools-doc.ts gate inside build-public-mirror.sh
  // and pinned by tests/tools/curated-doc-staleness.test.ts).
  const { scanCuratedDocText } = await import("./lib/curated-doc-staleness");
  const registeredToolSet = new Set(registeredTools);
  const staleCuratedDocs: { id: number; token: string }[] = [];
  for (const [idStr, docs] of Object.entries(PERSONA_DOCS)) {
    const text = `${(docs as any).tools_doc || ""}\n${(docs as any).tools_doc_addendum || ""}`;
    for (const token of scanCuratedDocText(text, registeredToolSet)) {
      staleCuratedDocs.push({ id: Number(idStr), token });
    }
  }
  ...
  console.log(`Sprawl warns:    ${sprawlWarn.length} personas exceed their sprawl threshold (default ${PERSONA_TOOL_SPRAWL_WARN}, per-persona overrides for wide-surface personas)`);
  ...
  console.log(`Stale curated:   ${staleCuratedDocs.length} (unknown tool-like tokens in doc-only PERSONA_DOCS tools_doc literals)`);
  console.log("");
  ...
  if (sprawlWarn.length > 0) {
    ...
    for (const p of sprawlWarn.slice(0, 20)) console.log(`   - #${p.id} ${p.name}: ${p.count} tools mentioned (threshold ${p.threshold})`);
  ...
  if (staleCuratedDocs.length > 0) {
    console.log("❌ STALE CURATED DOCS (doc-only PERSONA_DOCS tools_doc mentions a tool-like token that is not a live registered tool):");
    for (const s of staleCuratedDocs.slice(0, 50)) console.log(`   - persona #${s.id}: \`${s.token}\``);
    if (staleCuratedDocs.length > 50) console.log(`   …and ${staleCuratedDocs.length - 50} more.`);
    console.log("   These literals are documentation-only since Task 115 but still feed the public mirror");
    console.log("   (scripts/generate-public-docs.ts), so a stale tool name here would be published.");
    console.log("   FIX: if the tool was renamed/removed, update the curated tools_doc literal in");
    console.log("        server/seed-persona-prompts.ts. If the token is a legitimate non-tool term");
    console.log("        (param name, sub-action), add it to CURATED_DOC_NON_TOOL_TOKENS in");
    console.log("        scripts/lib/curated-doc-staleness.ts with the audit date.");
    console.log("");
  }

  const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
  if (deadTools.length === 0 && driftPersonas.length === 0 && orphanSkills.length === 0 && schemaGaps.length === 0 && staleCuratedDocs.length === 0) {
  ...
  if (staleCuratedDocs.length > 0) exitCode |= 16;
  console.log(`❌ FAIL — exit code ${exitCode} (1=dead, 2=drift, 4=orphan-skill, 8=schema-gap, 16=stale-curated-doc; OR'd) (${elapsed}s)`);
