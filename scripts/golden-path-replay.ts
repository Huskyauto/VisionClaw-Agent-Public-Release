...
import { markInternalRenderStaging } from "../server/lib/internal-render-markers";
import { uploadAndShare } from "../server/google-drive";
import {
  completeGoldenPathRunRecords,
  formatGoldenPathRunSummary,
} from "./lib/golden-path-run-summary";
...
function loadFingerprints(): Record<string, FingerprintRecord> {
  ...
  try {
    ...
    throw new Error(`[golden-path-replay] FATAL: cannot read ${FINGERPRINTS_FILE}: ${err?.message || err}`);
  }
  try {
    return JSON.parse(raw);
  } catch (err: any) {
    throw new Error(`[golden-path-replay] FATAL: ${FINGERPRINTS_FILE} is corrupt JSON (${err?.message || err}). Refusing to overwrite with empty state — hand-fix or delete the file.`);
  }
}
...
async function archiveArtifact(
  ...
  artifactPath: string,
  ranAt: string,
): Promise<{ archive_view_url?: string; archive_error?: string }> {
  try {
    const ext = path.extname(artifactPath) || `.${gp.format}`;
    const stamp = ranAt.replace(/[:.]/g, "-");
    const fileName = `${stamp}_${gp.id}${ext}`;
    const result = await uploadAndShare({
      filePath: artifactPath,
      fileName,
      description: `Golden Path Replay artifact for ${gp.id}, generated ${ranAt}`,
      folderLabel: "Golden Path Replays",
      share: true,
      tenantId: ADMIN_TENANT_ID,
      generatedArtifact: {
        artifactKind: "golden_path_replay",
        logicalName: gp.id,
        sourceRunKey: ranAt,
        idempotencyKey: `golden-path:${gp.id}:${ranAt}`,
        metadata: { format: gp.format, fixtureId: gp.id, ranAt },
      },
    });
    if (!result.success || !result.viewUrl) {
      return { archive_error: (result.error || "Drive upload returned no review URL").slice(0, 300) };
    }
    return { archive_view_url: result.viewUrl };
  } catch (error: any) {
    return { archive_error: (error?.message || String(error)).slice(0, 300) };
  }
}
...
async function runOnePath(gp: GoldenPath): Promise<FingerprintRecord> {
  ...
  try {
    if (gp.format === "html_app") {
      ...
      producerResult = await Promise.race([
        executeTool("build_html_app", { topic: gp.prompt, description: gp.prompt, model: "gpt-5.4", _tenantId: ADMIN_TENANT_ID }, ADMIN_TENANT_ID),
      ...
      if (gp.id === "bwb_video_3chapter_multichapter_smoke") {
        ...
        producerResult = await Promise.race([
          executeTool("mpeg_produce_parallel", markInternalRenderStaging({
  ...
  try {
    ...
    return {
      ...
      ran_at: ranAt,
      ok: false,
      notes: `artifact became unreadable before review: ${(error?.message || String(error)).slice(0, 300)}`,
    };
  }
  const archive = await archiveArtifact(gp, artifactPath, ranAt);
  ...
  try {
    ...
    return { id: gp.id, format: gp.format, ran_at: ranAt, ok: false, artifact_size_bytes: sizeBytes, artifact_path: artifactPath, notes: `grader threw: ${(e?.message || String(e)).slice(0, 300)}`, ...archive };
  }
  ...
  return { id: gp.id, format: gp.format, ran_at: ranAt, ok, artifact_size_bytes: sizeBytes, artifact_path: artifactPath, page_count: pageCount, duration_sec: gradeRes?.metrics?.duration_sec, drift: drifts.length ? drifts.join(" | ") : undefined, tools_called: toolsCalled, trajectory_drift: trajectoryDrift, ...archive };
}
...
async function main(): Promise<void> {
  const runRecords: FingerprintRecord[] = [];
  let exitCode = 0;
  try {
    ensureDir(REPLAY_LOG_DIR);
    ensureDir(FREEZE_DIR);
  ...
  console.log(`[golden-path] DONE — ${okCount}/${GOLDEN_PATHS.length} pass${newFailures.length ? ` (${newFailures.length} new freezes)` : ""}`);
    exitCode = newFailures.length > 0 ? 1 : 0;
  } catch (error: any) {
    exitCode = 2;
    console.error(`[golden-path] FATAL: ${error?.message || String(error)}`);
  } finally {
    const completeRecords = completeGoldenPathRunRecords(runRecords, GOLDEN_PATHS);
    console.log(formatGoldenPathRunSummary(completeRecords));
