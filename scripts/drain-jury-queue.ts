 ...
 * any of them:
 *
 *   · mapJuryDecision: ONLY a strict-majority FIX (2/3, 3/4, …) with no escalation
 *     flag and adequate fix-concordance routes to repo_surgeon; a minority FIX /
 *     shouldEscalate / low concordance escalates to the owner instead of auto-fixing.
...
/** The EXACT precomputed-jury shape the drainer feeds to captureIncident →
 *  mapJuryDecision. Extracted as a pure exported helper so the integration test
 *  (tests/integration/majority-vote-repair.test.ts) pins the drainer's shape
 *  contract against the real majority-gate brain — a future refactor of this
 *  shape can't quietly bypass mapJuryDecision's strict-majority rule. */
export function buildPrecomputedJury(e: JuryQueueEntry) {
  return {
    verdict: e.verdict,
...
/** Injectable IO seams for drainOnce — production callers pass nothing (real
 *  queue/DB/captureIncident); the integration test injects all of them so the
 *  FULL drain path runs query-free (pg-pool-hang lesson: no live DB in tests). */
export interface DrainDeps {
  readQueue?: () => JuryQueueEntry[];
  mutateQueue?: (fn: (current: JuryQueueEntry[]) => void) => void;
  capture?: (raw: any) => Promise<any>;
  getDb?: () => Promise<any>;
  claimBudget?: typeof claimAutonomousBudget;
}
...
export async function drainOnce(deps: DrainDeps = {}): Promise<{ processed: number; routed: number; skipped: number; total: number }> {
  ...
  const entries = deps.readQueue ? deps.readQueue() : readQueueRaw<JuryQueueEntry>();
  let processed = 0;
  let routed = 0;
  let skipped = 0;
  ...
  const captureIncident =
    deps.capture ?? (await import("../server/agentic/repair-incident")).captureIncident;
  ...
  const db = deps.getDb ? await deps.getDb() : (await import("../server/db")).db;
  const claimBudget = deps.claimBudget ?? claimAutonomousBudget;
  let dirty = false;
  ...
  for (const e of entries) {
    ...
    try {
      claim = await claimBudget({ tenantId: entryTenant, estimatedUsd: EST_USD, label: "jury-drain" });
    } catch (err) {
    ...
    try {
      ...
      // NL fix proposal so the majority-FIX path has context for repo-surgeon.
      const precomputedJury = buildPrecomputedJury(e);
  ...
  if (dirty) {
    ...
    (deps.mutateQueue ?? mutateQueue)((current: JuryQueueEntry[]) => {
...
async function main() {
  ...
  // Strict argv gate: unknown flags fail closed — this drainer routes live
  // queue side effects; a typoed flag must never silently run the default path.
  const unknownArgs = process.argv.slice(2).filter((a) => a !== "--once");
  if (unknownArgs.length > 0) {
    console.error(`[jury-drain] ERROR: unknown flag(s) ${unknownArgs.join(", ")} (allowed: --once). Refusing to run.`);
    process.exit(2);
  }
