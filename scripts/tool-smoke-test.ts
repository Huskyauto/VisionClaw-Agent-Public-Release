...
function extractToolDefs(): Map<string, ExtractedDef> {
  ...
  let srcText: string;
  try {
    srcText = fs.readFileSync(TOOLS_SRC, "utf8");
  } catch {
    return out; // no docs enrichment available
  }
  const sf = ts.createSourceFile(TOOLS_SRC, srcText, ts.ScriptTarget.Latest, true);
  ...
  function visit(node: ts.Node) {
    if (
      ...
      node.name.text === "TOOL_DEFINITIONS" &&
      node.initializer &&
