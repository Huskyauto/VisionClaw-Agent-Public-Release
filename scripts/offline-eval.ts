 ...
 *   EVAL_CANDIDATES            candidates per case, 1..3 (default 1; opt-in Best-of-N)
...
const JUDGE_MODEL = process.env.EVAL_JUDGE_MODEL || pickDistinctJudgeModel([ANSWER_MODEL]).model;
const REGRESSION_TOLERANCE = clamp01(Number(process.env.EVAL_REGRESSION_TOLERANCE) || 0.05);
const ANSWER_TASK_TEMPLATE_HASH = buildHarnessManifest({
  promptContract: "golden-case prompt passed verbatim to runLlmTextTask",
}).hash;
...
const JUDGE_TASK_TEMPLATE_HASH = buildHarnessManifest({
  promptContract:
    "independent rubric grader; receives task, refusal state, answer, and rubric; returns one pass/fail item per rubric entry",
  outputSchema: JUDGE_OUTPUT_SCHEMA,
}).hash;
const JUDGE_OUTPUT_SCHEMA_HASH = buildHarnessManifest(JUDGE_OUTPUT_SCHEMA).hash;
...
function readCandidateCount(): 1 | 2 | 3 {
  try {
    return parseOfflineEvalCandidateCount(process.env.EVAL_CANDIDATES);
  } catch (e: any) {
    console.error(`[offline-eval] ${e?.message ?? e}`);
    process.exit(1);
  }
}
const CANDIDATE_COUNT = readCandidateCount();

const EVALUATION_CONFIG = {
  candidateCount: CANDIDATE_COUNT,
  strategy: CANDIDATE_COUNT > 1
    ? "best-of-n-independent-rubric-score-stable-index"
    : "single-candidate",
  answer: {
    model: ANSWER_MODEL,
    temperature: 0.2,
    maxTokens: 1200,
    timeoutMs: 45_000,
    promptTemplateHash: ANSWER_TASK_TEMPLATE_HASH,
  },
  judge: {
    model: JUDGE_MODEL,
    temperature: 0,
    maxTokens: 1500,
    timeoutMs: 45_000,
    promptTemplateHash: JUDGE_TASK_TEMPLATE_HASH,
    outputSchemaHash: JUDGE_OUTPUT_SCHEMA_HASH,
  },
};
...
interface CandidateResult extends OfflineEvalCandidate {
  answerModel: string;
  judgeModel?: string;
  passedItems: number;
  totalItems: number;
...
async function evaluateCandidate(c: GoldenCase, index: number): Promise<CandidateResult> {
  ...
  // Candidate attempts are deliberately sequential and bounded by
  // CANDIDATE_COUNT. This keeps the hard cost ceiling explicit and avoids a
  // provider fan-out burst.
  let answer = "";
  let refused = false;
  try {
  ...
  // Grade every candidate with the same independent judge model and rubric.
  ...
  try {
    ...
    if (!graded) {
      ...
      return { ...base, judgeModel, refused };
    }
    ...
    return {
      ...
      belowMin: typeof c.minScore === "number" ? score < c.minScore : false,
      judgeModel,
      refused,
    };
  } catch (e: any) {
...
async function evaluateCase(c: GoldenCase): Promise<CaseResult> {
  ...
  const { candidates, selection } = await runOfflineEvalCandidateLoop(
    CANDIDATE_COUNT,
    index => evaluateCandidate(c, index),
  );
  base.candidates = candidates;

  // Never select the best available answer from a partial or invalid set.
  // Leaving the case unevaluated makes computeVerdict fail closed on coverage.
  if (!selection.complete || selection.selectedIndex === null) {
    base.error = selection.reason || "candidate selection failed";
    return base;
  }

  const winner = candidates[selection.selectedIndex];
  base.evaluated = true;
  base.score = winner.score;
  base.passedItems = winner.passedItems;
  base.totalItems = winner.totalItems;
  base.belowMin = winner.belowMin;
  base.judgeModel = winner.judgeModel;
  base.refused = winner.refused;
  base.selectedCandidateIndex = selection.selectedIndex;
  base.scoreMargin = selection.scoreMargin;
  return base;
}

/**
 * Most-recent prior NON-DEGRADED run with the same evaluation configuration for
 * this tenant, or null if no compatible history exists. Reads from the DB
 * (eval_runs) — the DURABLE baseline store — so the baseline survives prod
 * redeploys (prod FS resets each publish, which is why the old FS-only history
 * degraded the gate to within-deployment-only).
 *
 ...
 */
async function readBaseline(evaluationConfigKey: string): Promise<{ suiteScore: number; source: string } | null> {
  try {
    const rows = await db
      .select({ suiteScore: evalRuns.suiteScore, createdAt: evalRuns.createdAt })
      .from(evalRuns)
      .where(and(
        eq(evalRuns.tenantId, TENANT_ID),
        eq(evalRuns.degraded, false),
        sql`${evalRuns.record}->'evaluationConfig'->>'key' = ${evaluationConfigKey}`,
      ))
...
async function main() {
  ...
  const goldenSetHash = buildHarnessManifest(cases).hash;
  const evaluationConfig = { ...EVALUATION_CONFIG, goldenSetHash };
  const evaluationConfigKey = buildHarnessManifest(evaluationConfig).hash;
  // Capture provenance before model calls. If capture fails while enabled, the
  // evaluator stops rather than spending money on a run that cannot be tied to
  // an exact, durable harness identity. The explicit operator kill switch is
  // represented as null in the resulting record.
  const manifest = NO_WRITE
    ? null
    : await captureHarnessManifest({
        ...
        profile: {
          kind: "offline-eval",
          goldenSetHash,
          goldenSetCaseCount: cases.length,
          hydeEnabled: process.env.MEMORY_HYDE_ENABLED !== "0",
          answer: {
            model: ANSWER_MODEL,
            temperature: 0.2,
            maxTokens: 1200,
          ...
          judge: {
           ...
           candidateSelection: evaluationConfig,
        },
      });
  log(`[offline-eval] running ${cases.length} case(s) | answer=${ANSWER_MODEL} | candidates=${CANDIDATE_COUNT} | tenant=${TENANT_ID}`);
  if (!NO_WRITE && !manifest) log("[offline-eval] harness manifest disabled by HARNESS_MANIFEST_ENABLED=0");
  ...
  const baseline = await readBaseline(evaluationConfigKey);
  ...
  const record = {
    ...
    tenantId: TENANT_ID,
    harnessManifest: manifest ? { id: manifest.id, hash: manifest.hash } : null,
    totalCases: results.length,
    evaluatedCases: evaluated.length,
    ...
    cases: results.map(r => ({
      ...
      error: r.error,
       ...
       candidates: r.candidates.map(candidate => ({
         ...
         answerModel: candidate.answerModel,
         judgeModel: candidate.judgeModel,
         passedItems: candidate.passedItems,
         totalItems: candidate.totalItems,
         belowMin: candidate.belowMin,
         refused: candidate.refused,
         error: candidate.error,
       })),
       selectedCandidateIndex: r.selectedCandidateIndex,
       scoreMargin: r.scoreMargin,
    })),
     evaluationConfig: {
       key: evaluationConfigKey,
       ...evaluationConfig,
     },
  };
  ...
  if (!NO_WRITE) {
    try {
      await db.insert(evalRuns).values({
        tenantId: TENANT_ID,
        harnessManifestId: manifest?.id ?? null,
