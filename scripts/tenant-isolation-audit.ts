...
import { loadSuppressions, matchSuppression } from "../server/lib/audit-suppressions";
...
async function main() {
  ...
  const severe = allow.kept;
  const allSuppressed = [...precision.suppressed, ...allow.suppressed];

  writeReport(findings, cov, allSuppressed);
  log(
    `=== ${findings.length} finding(s); ${severeRaw.length} CRITICAL/HIGH` +
      (precision.ran || allow.suppressed.length
        ? ` (${severe.length} kept, ${precision.suppressed.length} LLM-suppressed, ${allow.suppressed.length} allowlist-suppressed)`
        : "") +
