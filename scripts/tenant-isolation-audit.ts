 ...
 *     SECURITY_CORE_AUTOFIX=1, behind the majority-jury gate, the cardinal-sin
 ...
 *   AUDIT_CHUNK_TIMEOUT_MS=<n> hard deadline per model call (default 120000).
 *   AUDIT_TRIAGE_TIMEOUT_MS=<n> hard deadline per precision batch (default 120000).
 *   AUDIT_JURY_TIMEOUT_MS=<n> hard deadline per optional jury vote (default 120000).
 *   AUDIT_OPERATION_TIMEOUT_MS=<n> deadline for budget/notification I/O (default 30000).
...
} from "./lib/tenant-audit-evidence-hash";
import { writeTenantAuditReportFiles } from "./lib/tenant-audit-report-writer";
import {
  isTenantAuditCheckpointValid,
  tenantAuditCheckpointChecksum,
} from "./lib/tenant-audit-checkpoint";
import {
  buildTenantAuditSchemaIndex,
  type TenantAuditSchemaIndex,
  type TenantAuditSchemaIndexResult,
} from "./lib/tenant-audit-schema-index";
...
import { withDeadline } from "../server/reasoning-front-door-core";
import { meteredLlmEnabled } from "../server/provider-recovery";
...
const SLICE_CHUNKS = Number.isFinite(Number(process.env.AUDIT_SLICE_CHUNKS))
  ? Math.min(20, Math.max(0, Math.floor(Number(process.env.AUDIT_SLICE_CHUNKS))))
  : 0;
...
function boundedTimeoutMs(raw: string | undefined, fallbackMs = 120_000): number {
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed >= 1_000
    ? Math.min(parsed, 15 * 60_000)
    : fallbackMs;
}
const AUDIT_CHUNK_TIMEOUT_MS = boundedTimeoutMs(process.env.AUDIT_CHUNK_TIMEOUT_MS);
const AUDIT_TRIAGE_TIMEOUT_MS = boundedTimeoutMs(process.env.AUDIT_TRIAGE_TIMEOUT_MS);
const AUDIT_JURY_TIMEOUT_MS = boundedTimeoutMs(process.env.AUDIT_JURY_TIMEOUT_MS);
const AUDIT_OPERATION_TIMEOUT_MS = boundedTimeoutMs(process.env.AUDIT_OPERATION_TIMEOUT_MS, 30_000);
// Per-chunk re-roll budget for an UNPARSEABLE (non-truncated) response or a
// transient model-call failure. The free modelfarm lane (gpt-5.4) is a
...
const AUDIT_PARSE_RETRY_MAX = Number.isFinite(Number(process.env.AUDIT_PARSE_RETRY_MAX))
  ? Math.min(2, Math.max(0, Math.floor(Number(process.env.AUDIT_PARSE_RETRY_MAX))))
  : 2;

async function withAuditDeadline<T>(
  label: string,
  timeoutMs: number,
  ...
  const result = await withDeadline(pending, timeoutMs, () => controller.abort());
  if (result === null) throw new Error(`${label} timed out after ${timeoutMs}ms`);
  return result;
}
...
const DEGRADED_REPORT_MD = path.join("docs", "tenant-isolation-audit-report-degraded.md");
const REPORT_JSON_DIR = path.join("data", "tenant-isolation-audit");
const REPORT_JSON = path.join(REPORT_JSON_DIR, "latest.json");
const DEGRADED_REPORT_JSON = path.join(REPORT_JSON_DIR, "degraded-latest.json");
const CHECKPOINT_JSON = path.join(REPORT_JSON_DIR, "checkpoint.json");
...
interface AuditCheckpoint {
  ...
  model: string;
  chunksTotal: number;
  completed: Record<string, Finding[]>;
  updatedAt: string;
  checksum: string;
}
...
function sourceFingerprint(
  chunks: { files: string[]; text: string }[],
  evidenceHash: string,
): string {
  return computeTenantAuditCheckpointSourceHash(evidenceHash, {
    version: 2,
    model: MODEL,
    maxChunkChars: MAX_CHUNK_CHARS,
    overlapLines: OVERLAP_LINES,
    maxTokens: AUDIT_MAX_TOKENS,
    retryMaxTokens: AUDIT_RETRY_MAX_TOKENS,
    parseRetryMax: AUDIT_PARSE_RETRY_MAX,
    prompt: SYSTEM_PROMPT,
  }, chunks);
}
function freshCheckpoint(sourceHash: string, chunksTotal: number): AuditCheckpoint {
  const base: Omit<AuditCheckpoint, "checksum"> = {
    version: 1,
    sourceHash,
    model: MODEL,
    chunksTotal,
  ...
  return { ...base, checksum: tenantAuditCheckpointChecksum(base) };
}

function isFinding(value: unknown, allowedFiles: Set<string>): value is Finding {
  if (!value || typeof value !== "object") return false;
  const f = value as Partial<Finding>;
  return (
    typeof f.file === "string" &&
    allowedFiles.has(f.file) &&
    (f.line === undefined || (Number.isInteger(f.line) && Number(f.line) > 0)) &&
...
function loadCheckpoint(
  ...
  const fresh = () => freshCheckpoint(sourceHash, chunks.length);
  if (!fs.existsSync(CHECKPOINT_JSON)) return fresh();
  try {
    const parsed = JSON.parse(fs.readFileSync(CHECKPOINT_JSON, "utf8")) as Partial<AuditCheckpoint>;
    if (!isTenantAuditCheckpointValid({
      checkpoint: parsed,
      sourceHash,
      model: MODEL,
      chunksTotal: chunks.length,
      isCompletedEntryValid: (finding, index) => isFinding(finding, new Set(chunks[index].files)),
    })) {
      log("checkpoint fingerprint/integrity invalid — restarting from chunk 1.");
      return fresh();
    }
    return parsed as AuditCheckpoint;
  } catch (e) {
    log(`checkpoint unreadable — restarting fail-closed: ${(e as Error)?.message || e}`);
    return fresh();
  }
}

function saveCheckpoint(checkpoint: AuditCheckpoint): void {
  fs.mkdirSync(REPORT_JSON_DIR, { recursive: true });
  checkpoint.updatedAt = new Date().toISOString();
  const { checksum: _oldChecksum, ...unsigned } = checkpoint;
  checkpoint.checksum = tenantAuditCheckpointChecksum(unsigned);
  const body = JSON.stringify(checkpoint, null, 2);
  const tmp = `${CHECKPOINT_JSON}.tmp`;
  fs.writeFileSync(tmp, body);
  if (fs.statSync(tmp).size !== Buffer.byteLength(body)) throw new Error("checkpoint write verification failed");
  fs.renameSync(tmp, CHECKPOINT_JSON);
}

function clearCheckpoint(): void {
  try {
    fs.unlinkSync(CHECKPOINT_JSON);
  } catch (e: any) {
...
async function auditChunk(
  ...
  async function call(maxTokens: number): Promise<{ raw: string; truncated: boolean }> {
    const resp = await withAuditDeadline(
      `tenant audit chunk ${idx + 1}/${total}`,
      AUDIT_CHUNK_TIMEOUT_MS,
      async (signal) => {
        ...
        if (isOpenAIReasoning) params.max_completion_tokens = maxTokens;
        else {
...
function writeReport(
  findings: Finding[],
  cov: Coverage,
  suppressed: TriageVerdict[] = [],
  evidenceHash: string,
  dispositionGroups?: SevereDispositionGroups,
  forceDegraded = false,
) {
  ...
  const degraded =
    forceDegraded ||
    cov.chunksFailed > 0 ||
    (cov.chunksPending || 0) > 0 ||
    cov.sourceChangedDuringRun === true ||
    cov.filesAudited < cov.scanned ||
  ...
  const falsePositives = dispositionGroups
    ? [...dispositionGroups.precisionFalsePositive, ...dispositionGroups.allowlistFalsePositive]
    : suppressed.filter((v) => v.pattern !== "deferred");
  const deferredRisks = dispositionGroups?.deferredRealRisk || suppressed.filter((v) => v.pattern === "deferred");
  const dispositionLine = dispositionGroups
    ? `- Severe dispositions: **${dispositionGroups.kept.length} kept** · ` +
      `${dispositionGroups.precisionFalsePositive.length} precision false-positive · ` +
      `${dispositionGroups.allowlistFalsePositive.length} allowlist false-positive · ` +
      `${deferredRisks.length} deferred real risk\n`
    : "";
  const falsePositiveSection = falsePositives.length
    ? `\n### Resolved — confirmed false positives (${falsePositives.length})\n\n` +
      ...
      "\n"
    : "";
  const deferredSection = deferredRisks.length
    ? `\n### Deferred — accepted real risks (${deferredRisks.length})\n\n` +
      `_These are genuine findings under dated deferral. They still block republish readiness._\n\n` +
      deferredRisks
        .map(
          (v) =>
            `- **${v.finding.file}${v.finding.line ? `:${v.finding.line}` : ""}** — ${v.finding.issue}\n  - _Deferral:_ ${v.reason}`,
        )
        .join("\n") +
      "\n"
    : "";
  ...
  const md =
    ...
    (cov.chunksPending ? `, ${cov.chunksPending} pending` : "") +
    `)\n` +
  ...
  const reportJson = JSON.stringify(
      {
        ...
        severeDispositions: dispositionGroups
          ? [
              ...dispositionGroups.kept.map((f) => ({ ...f, disposition: "kept" })),
              ...dispositionGroups.precisionFalsePositive.map((v) => ({
                ...v.finding,
                disposition: "precision-false-positive",
                pattern: v.pattern,
                reason: v.reason,
              })),
              ...dispositionGroups.allowlistFalsePositive.map((v) => ({
                ...v.finding,
                disposition: "allowlist-false-positive",
                pattern: v.pattern,
                reason: v.reason,
              })),
              ...dispositionGroups.deferredRealRisk.map((v) => ({
                ...v.finding,
                disposition: "deferred-real-risk",
                pattern: v.pattern,
                reason: v.reason,
              })),
            ]
  ...
  const written = writeTenantAuditReportFiles({
    degraded,
    markdown: md,
    json: reportJson,
    targets: {
      ...
      degradedReportMd: DEGRADED_REPORT_MD,
      degradedReportJson: DEGRADED_REPORT_JSON,
    },
  });
  log(`report → ${written.markdownPath} + ${written.jsonPath}`);
  if (degraded) {
    log(`authoritative completed report preserved; degraded diagnostic written separately`);
  }
}
...
async function notifyOwner(subject: string, body: string) {
  ...
  try {
    await withAuditDeadline("tenant audit owner notification", AUDIT_OPERATION_TIMEOUT_MS, async () => {
...
interface SevereDispositionGroups {
  kept: Finding[];
  precisionFalsePositive: TriageVerdict[];
  allowlistFalsePositive: TriageVerdict[];
  deferredRealRisk: TriageVerdict[];
}
...
let _schemaIdxCache: TenantAuditSchemaIndex | null = null;
function buildSchemaIndex(): TenantAuditSchemaIndexResult {
  if (_schemaIdxCache) return { ok: true, index: _schemaIdxCache };
  const result = buildTenantAuditSchemaIndex(path.join("shared", "schema.ts"));
  if (!result.ok) {
    console.error("[tenant-isolation-audit] precision schema index unavailable", result.error);
    return result;
  }
  _schemaIdxCache = result.index;
  return result;
}
...
// ── Human-verified suppression allowlist ─────────────────────────────────────
...
// pattern + evidence). Applied AFTER the LLM precision pass, before the exit
// decision. FAIL CLOSED: a malformed file or entry suppresses NOTHING (loud
// log, findings kept); an entry only fires on exact file match + issue
// substring match + a recognized benign pattern. Allowlisted findings stay in
// the report's suppressed audit-trail section.
const SUPPRESSIONS_FILE = path.join(REPORT_JSON_DIR, "suppressions.json");
function applyAllowlist(severe: Finding[]): { kept: Finding[]; suppressed: TriageVerdict[] } {
  const entries = loadSuppressions(SUPPRESSIONS_FILE, SUPPRESSIBLE_PATTERNS, log);
  if (entries.length === 0) return { kept: severe, suppressed: [] };
  const kept: Finding[] = [];
  const suppressed: TriageVerdict[] = [];
  for (const f of severe) {
    const hit = matchSuppression(f, entries);
    if (hit) {
      suppressed.push({
        finding: f,
        verdict: "SUPPRESS",
        pattern: hit.pattern,
        reason: `allowlist (${hit.verifiedBy} ${hit.date}): ${hit.reason}`.slice(0, 500),
      });
    } else {
      kept.push(f);
    }
  }
  if (suppressed.length) log(`allowlist: ${suppressed.length} finding(s) suppressed by human-verified suppressions.json (${entries.length} entries).`);
  return { kept, suppressed };
}

// ── Dated deferrals (real findings the owner consciously parked) ─────────────
// data/tenant-isolation-audit/deferrals.json mirrors the deferred section of
// docs triage campaigns: each entry is a GENUINE (not false-positive) finding
// with a reason and a reviewBy date. Until that date it is excluded from the
// pass/fail exit (still reported, labelled "deferred"); once expired it goes
// red again automatically. FAIL CLOSED: malformed file/entries defer nothing.
const DEFERRALS_FILE = path.join(REPORT_JSON_DIR, "deferrals.json");
function applyDeferrals(severe: Finding[]): { kept: Finding[]; suppressed: TriageVerdict[] } {
  const entries = loadDeferrals(DEFERRALS_FILE, log);
  if (entries.length === 0) return { kept: severe, suppressed: [] };
  const kept: Finding[] = [];
  const suppressed: TriageVerdict[] = [];
  for (const f of severe) {
    const hit = matchDeferral(f, entries);
    if (hit) {
      suppressed.push({
        finding: f,
        verdict: "SUPPRESS",
        pattern: "deferred",
        reason: `DEFERRED until ${hit.reviewBy} (${hit.deferredBy} ${hit.date}): ${hit.reason}`.slice(0, 500),
      });
    } else {
      kept.push(f);
    }
  }
  if (suppressed.length) log(`deferrals: ${suppressed.length} finding(s) deferred (dated, expiring) via deferrals.json (${entries.length} active entries).`);
  return { kept, suppressed };
}
...
async function precisionTriage(
  severe: Finding[],
): Promise<{ kept: Finding[]; suppressed: TriageVerdict[]; ran: boolean; degraded: boolean; reason?: string }> {
  if (!PRECISION_ON) return { kept: severe, suppressed: [], ran: false, degraded: false, reason: "disabled" };
  if (severe.length === 0) return { kept: [], suppressed: [], ran: false, degraded: false };

  const schema = buildSchemaIndex();
  if (!schema.ok) {
    log(`precision triage schema unavailable — keeping all ${severe.length} severe finding(s) (fail closed).`);
    return { kept: severe, suppressed: [], ran: false, degraded: true, reason: "schema-unavailable" };
  }
  const idx = schema.index;
  ...
  try {
    ...
    const claim = await withAuditDeadline(
      "tenant audit precision budget claim",
      AUDIT_OPERATION_TIMEOUT_MS,
      async () => claimAutonomousBudget({
    ...
    if (!claim.ok) {
      ...
      return { kept: severe, suppressed: [], ran: false, degraded: true, reason: `budget: ${claim.reason}` };
    }
  } catch (e) {
    log(`precision triage budget claim errored: ${(e as Error)?.message || e} — keeping all severe (fail closed).`);
    return { kept: severe, suppressed: [], ran: false, degraded: true, reason: "budget-error" };
  }
  ...
  for (let b = 0; b < runBatches; b++) {
    ...
    try {
      const resp = await withAuditDeadline(
        `tenant audit precision batch ${b + 1}/${runBatches}`,
        AUDIT_TRIAGE_TIMEOUT_MS,
        async (signal) => {
          const { getClientForModel } = await import("../server/providers");
  ...
  return { kept, suppressed, ran: true, degraded };
}
...
async function remediate(severe: Finding[]): Promise<RemediationResult> {
  ...
  const { ADMIN_TENANT_ID } = await import("../server/tenant-constants");
  {
    const { claimAutonomousBudget } = await import("../server/agentic/autonomous-budget");
    const claim = await withAuditDeadline(
      "tenant audit enqueue budget claim",
      AUDIT_OPERATION_TIMEOUT_MS,
      async () => claimAutonomousBudget({
        tenantId: ADMIN_TENANT_ID,
  ...
  for (const f of batch) {
    ...
    try {
      ...
      juried++;
      const jury: any = await withAuditDeadline(
        `tenant audit jury vote for ${loc}`,
        AUDIT_JURY_TIMEOUT_MS,
        async () => juryTriage({
          issueText,
          context: `${loc} — ${f.issue}`,
          tenantId: ADMIN_TENANT_ID,
...
async function main() {
  ...
  const requiredChunks = chunks.length;
  const intentionallyCapped = MAX_CHUNKS > 0 && requiredChunks > MAX_CHUNKS;
  if (intentionallyCapped) {
    log(`capping ${requiredChunks} chunks → ${MAX_CHUNKS} (AUDIT_MAX_CHUNKS)`);
    chunks = chunks.slice(0, MAX_CHUNKS);
  }
  log(`${files.length} file(s) → ${requiredChunks} required chunk(s)${DRY_RUN ? " (DRY RUN — no email)" : ""}`);

  const resumable = SLICE_CHUNKS > 0 && MAX_CHUNKS === 0;
  const checkpoint = resumable
    ? loadCheckpoint(sourceFingerprint(chunks, evidenceHash), chunks)
    : null;
  const completedIndexes = checkpoint
    ...
    : [];
  let runStart = 0;
  while (checkpoint?.completed[String(runStart)] && runStart < chunks.length) runStart++;
  const runEnd = resumable
    ? Math.min(chunks.length, runStart + SLICE_CHUNKS)
    : chunks.length;
  if (resumable) {
    log(`resumable slice: chunks ${runStart + 1}-${runEnd} of ${chunks.length} (${completedIndexes.length} checkpointed)`);
  }

  // Autonomous-spend governor: every chunk is a paid model call. Gate BEFORE any
  // spend. A denied budget is DEGRADED (5), never green: no files were audited.
  {
    const { ADMIN_TENANT_ID } = await import("../server/tenant-constants");
    const { claimAutonomousBudget } = await import("../server/agentic/autonomous-budget");
    // When metered calls are disabled, providers.ts enforces the free
    // Replit/modelfarm substitution. Keep the atomic budget claim active at its
    // minimum reservation instead of falsely reserving $0.50 per zero-cost call.
    const chunksThisInvocation = Math.max(0, runEnd - runStart);
    const estimate = meteredLlmEnabled()
      ? Math.max(0.5, chunksThisInvocation * EST_USD_PER_CHUNK)
      : 0.01;
    let failureReason = "";
    try {
      const budget = await withAuditDeadline(
        "tenant audit budget claim",
        AUDIT_OPERATION_TIMEOUT_MS,
        async () => claimAutonomousBudget({
          tenantId: ADMIN_TENANT_ID,
      ...
      if (!budget.ok) {
        failureReason = `budget gate denied: ${budget.reason} (spent $${budget.spentUsd.toFixed(2)} / cap $${budget.capUsd.toFixed(2)})`;
      }
    } catch (e) {
      failureReason = `budget gate failed: ${(e as Error)?.message || e}`;
    }
    if (failureReason) {
      log(`${failureReason} — skipping this run as DEGRADED.`);
      const cov: Coverage = {
        scanned: files.length,
        chunksAttempted: requiredChunks,
        chunksSucceeded: 0,
        chunksFailed: 0,
        chunksPending: requiredChunks,
        filesAudited: 0,
        failures: [{ chunk: 0, reason: failureReason }],
        unreadableFiles: [...UNREADABLE_FILES],
      };
      writeReport([], cov, [], evidenceHash);
      if (!DRY_RUN) {
        await notifyOwner(
          "TENANT-ISOLATION AUDIT: DEGRADED — budget gate unavailable",
          `${failureReason}. No source files were audited. Full report: ${REPORT_MD}`,
        );
      }
      process.exit(5);
    }
  ...
  const all: Finding[] = checkpoint
    ? completedIndexes.flatMap((i) => checkpoint.completed[String(i)] || [])
    : [];
  let chunksSucceeded = completedIndexes.length;
  ...
  for (let i = runStart; i < runEnd; i++) {
    const r = await auditChunk(chunks[i], i, requiredChunks);
    if (r.ok) {
      chunksSucceeded++;
      for (const f of chunks[i].files) okFiles.add(f);
      all.push(...r.findings);
      if (checkpoint) {
        checkpoint.completed[String(i)] = r.findings;
  ...
  if (checkpoint) {
    for (const key of Object.keys(checkpoint.completed)) {
      const i = Number(key);
      if (!Number.isInteger(i) || !chunks[i]) continue;
      for (const f of chunks[i].files) okFiles.add(f);
    }
  }
  ...
  const cov: Coverage = {
    ...
    chunksPending: Math.max(0, requiredChunks - chunksSucceeded - failures.length),
    filesAudited,
    failures,
    unreadableFiles: [...UNREADABLE_FILES],
  };
  cov.sourceChangedDuringRun = computeTenantAuditEvidenceHash(ROOT) !== evidenceHash;
  ...
  let precision: { kept: Finding[]; suppressed: TriageVerdict[]; ran: boolean; degraded: boolean; reason?: string } = {
    kept: severeRaw,
    suppressed: [],
    ran: false,
    degraded: false,
  ...
  // Human-verified allowlist applies AFTER the LLM pass — only findings the
  // triage campaign affirmatively cleared (suppressions.json) are removed from
  // the pass/fail set. Everything stays in the report's audit trail.
  // Like precisionTriage, it runs on FULL COVERAGE only: a degraded run must
  // never under-report severe counts on partial data (it already exits 5).
  const allow = fullCoverage
    ? applyAllowlist(precision.kept)
    : { kept: precision.kept, suppressed: [] as TriageVerdict[] };
  // Dated deferrals last: real findings the owner consciously parked with an
  // expiring reviewBy date. Full coverage only, same as the allowlist.
  const defer = fullCoverage
    ? applyDeferrals(allow.kept)
    : { kept: allow.kept, suppressed: [] as TriageVerdict[] };
  const severe = defer.kept;
  const allSuppressed = [...precision.suppressed, ...allow.suppressed, ...defer.suppressed];

  // Re-check immediately before the only report write. Precision triage can take
  // long enough for source or audit inputs to change after the initial coverage
  // snapshot; that run must go to degraded diagnostics, never authoritative latest.
  cov.sourceChangedDuringRun =
    cov.sourceChangedDuringRun || computeTenantAuditEvidenceHash(ROOT) !== evidenceHash;
  writeReport(findings, cov, allSuppressed, evidenceHash, {
    kept: severe,
    precisionFalsePositive: precision.suppressed,
    allowlistFalsePositive: allow.suppressed,
    deferredRealRisk: defer.suppressed,
  }, precision.degraded);
  log(
    `=== ${findings.length} finding(s); ${severeRaw.length} CRITICAL/HIGH` +
      (precision.ran || allow.suppressed.length || defer.suppressed.length
        ? ` (${severe.length} kept, ${precision.suppressed.length} LLM-suppressed, ${allow.suppressed.length} allowlist-suppressed, ${defer.suppressed.length} deferred)`
        : "") +
  ...
  if (!fullCoverage) {
    const plannedPartial =
      resumable &&
      failures.length === 0 &&
      chunksSucceeded < chunks.length &&
      runEnd < chunks.length;
    if (plannedPartial) {
      log(`slice checkpointed: ${chunksSucceeded}/${chunks.length} chunks complete — resume required (exit 7, not green).`);
      process.exit(7);
    }
  ...
  if (precision.degraded) {
    if (!DRY_RUN) {
      await notifyOwner(
        "TENANT-ISOLATION AUDIT: DEGRADED PRECISION TRIAGE",
        `Full first-pass coverage completed, but precision triage was incomplete (${precision.reason || "one or more batches failed"}). ` +
          `No clean result is permitted. Full report: ${REPORT_MD}`,
      );
    }
    process.exit(5);
  }

  if (cov.sourceChangedDuringRun) {
    log("source/audit inputs changed during finalization — checkpoint retained; rerun required.");
    process.exit(5);
  }
  ...
  // ACCEPTED-RISK: nothing new is red, but dated deferrals are still parking
  // REAL findings. A security audit must never report a clean exit while
  // confirmed cross-tenant paths remain open, so this is a DISTINCT nonzero
  // status (6) with its own owner alert — visibly "risk accepted until the
  // reviewBy date", never "all clear". Exit 0 now means genuinely zero open
  // severe findings, deferred included.
  if (defer.suppressed.length > 0) {
    if (!DRY_RUN) {
      await notifyOwner(
        `TENANT-ISOLATION AUDIT: ACCEPTED-RISK — ${defer.suppressed.length} deferred REAL finding(s) still open`,
        `The nightly tenant-isolation audit found no NEW HIGH/CRITICAL findings, but ` +
          `${defer.suppressed.length} previously-triaged REAL finding(s) remain open under dated deferrals ` +
          `(data/tenant-isolation-audit/deferrals.json). This run exits with the distinct accepted-risk ` +
          `status (6), NOT green — these are confirmed tenant-scoping gaps awaiting remediation, and each ` +
          `goes red automatically after its reviewBy date.\n\n` +
          defer.suppressed
            .map((v) => `- [${v.finding.severity}] ${v.finding.file}${v.finding.line ? `:${v.finding.line}` : ""} — ${v.reason}`)
            .join("\n") +
          `\n\nFull report: ${REPORT_MD}`,
      );
    }
    log(`=== ACCEPTED-RISK: 0 new severe finding(s), ${defer.suppressed.length} deferred REAL finding(s) still open — exit 6 (not green). ===`);
    process.exit(6);
  }
  process.exit(0);
}
