 ...
 *      and re-run).
 *   3. Reads RENDER_ACCESS_KEY from Replit Secrets without printing it.
 ...
 *   6. Prints the non-secret next steps you need to do manually.
 *
 ...
 * Usage:
 *   # First time:
 *   #   1. Create Railway account-level token and a random 32-byte
 *   #      RENDER_ACCESS_KEY, then store both in Replit Secrets.
...
const RENDER_ACCESS_KEY = (process.env.RENDER_ACCESS_KEY || "").trim();
...
if (!/^[A-Za-z0-9_-]{43,128}$/.test(RENDER_ACCESS_KEY)) {
  console.error("[deploy] RENDER_ACCESS_KEY must be a 43–128 character random value stored in Replit Secrets");
  process.exit(1);
}
console.log("[deploy] Railway authentication configured");
...
async function main() {
  ...
  const accessKey = RENDER_ACCESS_KEY;
  console.log(`[deploy] step 3/6: loaded protected RENDER_ACCESS_KEY from Replit Secrets`);
  ...
  console.log(``);
  console.log(`3. In Replit Secrets, add the worker URL:`);
  console.log(``);
  ...
  console.log(``);
  console.log(`[deploy] RENDER_ACCESS_KEY remains protected in Replit Secrets and was not printed.`);
}
