...
import { readFileSyncEIO, existsSyncEIO, writeFileSyncEIO } from "./lib/eio-read";
import { bwbScriptCommand } from "./lib/bwb-script-runner";
import { renderFailureReason } from "./lib/bwb-render-failure";
...
import { autoPinWindowYmd, discoverWeeklyDriveClips, extractWeeklyDriveTranscripts, type DiscoveredDriveClip } from "./lib/drive-discover";
...
import { setBwbPhase, updateBwbChapters, bumpBwbHeartbeat } from "../server/lib/bwb-job-progress";
...
async function loadConfig(): Promise<WeeklyConfig> {
  ...
  if (!existsSyncEIO(heroImagePath)) fail(`Hero image not found: ${heroImagePath}`);
  if (!weekLabel) {
    // Prefer the pinned window's START date (main() auto-pins it when absent) so
    // the label names the recapped week, not whatever day the job happens to run
    // (a Sunday re-run of the Jul 5–11 week must not say "Week of July 12").
    const ws = /^(\d{4})-(\d{2})-(\d{2})$/.exec(process.env.BWB_WEEK_START || "");
    const labelDate = ws ? new Date(Number(ws[1]), Number(ws[2]) - 1, Number(ws[3])) : new Date();
    weekLabel = `Week of ${labelDate.toLocaleDateString("en-US", { month: "long", day: "numeric" })}`;
  }
...
Return STRICT JSON only (no markdown):
{
  ...
  // MODEL ROTATION for reliable weekly delivery (Bob 2026-07-04). The recap is the
  // ONE video that synthesizes fresh narration through an LLM every week, so it is
  // the only flow exposed to per-model variance + the weight-honesty guard on
  // every run. A single model can deterministically FABRICATE a weight the guard
  // rejects and re-emit it across ALL same-model retries (Opus did exactly this —
  // invented "530 lbs" 3x, ignoring the correction). So after a model exhausts its
  // bounded same-model corrective retries STILL failing the guard (or failing to
  // return usable JSON), we rotate to a DIFFERENT cross-vendor model — which does
  // not share the same numeric anchor — before giving up. The weight guard NEVER
  // weakens: this only grants more legitimate attempts; if EVERY model still
  // fabricates, we still fail closed and refuse to ship a wrong number.
  //
  // WEEKLY_MODEL still forces the PRIMARY model (deliberate override hatch); the
  // fallbacks are free cross-vendor modelfarm lanes (OpenAI, then Google),
  // deduped, primary first. Opus 5 is the default primary (flagship swap 2026-07-24; Bob 2026-06-12 rule carries forward):
  // faithful multi-transcript synthesis + strict factual-number discipline.
  const primaryModel = process.env.WEEKLY_MODEL || "claude-opus-5";
  const modelSequence = [primaryModel, "gpt-5.6-sol", "gemini-3.1-pro-preview"].filter(
  ...
  let planError = ""; // last non-weight plan failure (bad/no JSON) for the closed message
  // Per-model failure log for the fail-closed message. planError alone is
  // last-write-wins, which hid the REAL first failure in prod (2026-08-16: the
  // surfaced "404 (no body)" belonged only to the LAST model; the first model's
  // route error was lost). Keep one line per model so the failure email is
  // diagnosable without deployment-log archaeology.
  const perModelErrors: string[] = [];
  // Provider errors can embed whole response bodies — sanitize newlines and cap
  // each entry so the fail() message stays a readable single paragraph.
  const capErr = (s: string): string => {
    const flat = s.replace(/\s+/g, " ").trim();
    return flat.length > 300 ? `${flat.slice(0, 300)}…` : flat;
  };
  const perModelSuffix = (): string =>
    perModelErrors.length ? ` | per-model failures: ${perModelErrors.join(" ;; ")}` : "";
  let planned = false;

  outer: for (let mi = 0; mi < modelSequence.length; mi++) {
    const model = modelSequence[mi];
    if (mi > 0) {
      console.warn(
        `[build-bwb-weekly] planning still failing after ${MAX_PLAN_ATTEMPTS} attempt(s) on "${modelSequence[mi - 1]}" ` +
          `— rotating to cross-vendor model "${model}" (weight guard stays fail-closed).`,
      );
    }
    // Fresh corrective per model — NEVER carry a poisoned correction across
    // vendors, and never echo a rejected number back (repeating "you said 530,
    // remove 530" re-primes the model to say it again — the pink-elephant effect).
    let corrective = "";
    weightViolations = [];
    ...
    for (let attempt = 1; attempt <= MAX_PLAN_ATTEMPTS; attempt++) {
      const res = await runLlmTask({
        ...
        model,
        // PREFER the flat-rate runner (~$0): this is a pure structured-JSON task
        // (runLlmTask forces response_format:json_object), so it needs no
        // tool-calling. requiresTools:false routes through the free lane; a metered
        // fallback only matters if the free lane is down.
        requiresTools: false,
        // Cost-exempt bounded lane: ≤3 attempts per model, ≤3 models — so a
        // metered fallback gets the same breaker exemption as the jury and can
      ...
      if (!res.success || !res.json) {
        planError = `did not return valid JSON (${res.error || "no json"})`;
        perModelErrors.push(capErr(`${model} attempt ${attempt}: ${planError}`));
        ...
        continue outer; // this model can't produce usable JSON — rotate to the next
      }
      j = res.json as any;
      if (!Array.isArray(j.scenes) || j.scenes.length === 0) {
        planError = "returned no scenes";
        perModelErrors.push(capErr(`${model} attempt ${attempt}: returned no scenes`));
        if (attempt < MAX_PLAN_ATTEMPTS) {
          corrective = `Your previous attempt returned no scenes. Return the full ${targetScenes}-scene array.`;
          continue;
        }
        continue outer; // rotate
      }
      ...
      if (!scenes[0].narration) { planError = "scene 1 narration empty"; continue outer; }
      ...
      if (!weightViolations.length) {
        planned = true;
        // Make the self-recovery UNMISTAKABLE in prod logs. A clean plan on the
        // primary (mi===0) is the silent happy path; a clean plan AFTER rotating
        // off a fabricating model is the auto-recovery doing its job with no human
        // — log it loud so "did the recap run correctly?" is answerable at a glance.
        if (mi > 0) {
          console.warn(
            `[build-bwb-weekly] ✓ RECOVERED — script planning succeeded on "${model}" after auto-rotating off ` +
              `${modelSequence.slice(0, mi).map((m) => `"${m}"`).join(", ")} (weight guard honored throughout; ` +
              `self-recovery worked, no human intervention needed).`,
          );
        }
        break outer; // clean → ship it (first-person never blocks)
      }

      if (attempt < MAX_PLAN_ATTEMPTS) {
        // Corrective withOUT echoing the rejected number(s) back — state ONLY the
        // allowed set positively and forbid every other body-weight figure. (The
        // old code echoed the offenders, which re-primed the model to restate them.)
        corrective = allowed.size
          ? `WEIGHT: your draft stated a body-weight figure that is NOT one of the supplied facts. ` +
              `You may state ONLY these exact numbers: ${[...allowed].join(", ")} lbs — and NO other body-weight number ` +
              `(no goal weight, no milestone like "the 300s", no recomputed figure, no origin/peak/highest weight, nothing spelled-out). ` +
              `Rewrite any scene that mentions a different weight so it uses one of the allowed numbers or speaks qualitatively instead.`
          : // NO weight was supplied — the narration must contain ZERO body-weight
            // numbers. Forbid the whole 120–700 range and hand the model the
            // qualitative vocabulary so it stops reaching for a figure.
            `WEIGHT — CRITICAL: NO weight number was supplied for this week, so your narration must contain ` +
              `ZERO body-weight figures. Do NOT state ANY number between 120 and 700 as a body weight — not a current ` +
              `weight, not a total lost, not a goal, not a milestone ("the 300s"), not "down to NNN", not "lost NNN ` +
              `pounds", not spelled-out ("two thirty-five"). Speak about progress ONLY qualitatively, e.g. "the scale ` +
              `kept moving", "lighter than last week", "down another notch", "my clothes fit looser", "real progress ` +
              `this week" — feelings and momentum, never a number.`;
        console.warn(
          `[build-bwb-weekly] weight-guard violation on "${model}" attempt ${attempt}/${MAX_PLAN_ATTEMPTS} — bounded corrective retry.`,
        );
      }
    }
    // Inner loop ended without break-outer: this model still fails (weight or
    // JSON). Loop continues to the next model, or falls through to fail-closed.
  }

  // Fail-closed AFTER cross-vendor rotation exhausts: a weight guard still
  // flagging on EVERY model means no model would honor the facts → refuse to ship
  // (the guards never weaken). A pure JSON/no-scenes exhaustion fails the same way.
  if (!planned) {
    if (weightViolations.length) {
      fail(
        `Synthesized narration states weight figure(s) not in the supplied facts: ${weightViolations.join(", ")} ` +
          ...
          `Refusing to ship a wrong/invented weight even after cross-vendor model rotation (${modelSequence.join(" → ")}). ` +
          `Supply BWB_CURRENT_WEIGHT / BWB_TOTAL_LOST (or WEEKLY json currentWeight/totalLost), or re-run.` +
          perModelSuffix(),
      );
    }
    fail(
      `LLM script planning failed across all models (${modelSequence.join(" → ")}): ${planError || "no usable script"}` +
        perModelSuffix(),
    );
  }
...
async function main() {
  ...
  // AUTO-PIN the week window (2026-07-12, after the W28 wrong-window incident):
  // when NEITHER BWB_WEEK_START nor BWB_WEEK_END is supplied, resolve the default
  // Sun–Sat window (the just-completed week, or the week containing
  // BWB_WEEK_ANCHOR) and PIN it into the env BEFORE discovery + jobKey
  // computation. This makes every weekly run window-pinned by default:
  //   - the jobKey ALWAYS carries the `-<start>_<end>` suffix, so checkpoint
  //     identity always encodes the semantic window (no pure-key/pinned-key
  //     split for the same content week);
  //   - discovery and checkpointing provably agree on the same window;
  //   - operators/Felix no longer need to compute dates for the normal weekly
  //     run — explicit BWB_WEEK_START/END still win for redos of a named week.
  // If exactly ONE of the two is set we leave it alone — discovery fails loud
  // on partial ranges, which is the right outcome for a malformed pin.
  if (!process.env.BWB_WEEK_START && !process.env.BWB_WEEK_END) {
    const win = autoPinWindowYmd({ anchorRaw: process.env.BWB_WEEK_ANCHOR });
    process.env.BWB_WEEK_START = win.start;
    process.env.BWB_WEEK_END = win.end;
    console.log(
      `[build-bwb-weekly] auto-pinned week window ${win.start} → ${win.end} ` +
        `(${win.windowDesc}). Set BWB_WEEK_START/END to recap a different week.`,
    );
  }
  ...
  // Explicit-window suffix (2026-07-11 incident): two runs in the SAME ISO week
  // with DIFFERENT pinned BWB_WEEK_START/END windows must NOT share a manifest.
  // A wrong-window run checkpointed the prior week's transcripts under
  // `bwb-weekly-2026-W28`; the corrected-window re-run minutes later hit the
  // SAME key, its transcription stage checkpoint-reused the OLD week's
  // transcripts, and the recap narrated two-weeks-ago clips even though
  // discovery correctly listed the right week's files. Including the pinned
  // window in the key isolates each window's manifest end-to-end while keeping
  // the resume contract: same window ⇒ same key (a Sat→Sun re-run still
  // resumes). Since 2026-07-12 main() AUTO-PINS the default Sun–Sat window when
  // no env is supplied, so every weekly run keys with the window suffix — the
  // pure ISO-week key remains only as a legacy/BWB_JOB_KEY-compatible shape.
  // Normalize a pinned date to canonical zero-padded YYYY-MM-DD. Accepts the
  // same laxer forms discovery parses (e.g. "2026-7-5") so a pinned-window run
  // ALWAYS produces a deterministic suffix — a run that behaves as pinned but
  // keys as unpinned would re-open the collision. Fails CLOSED: if a window
  // env var is set but unparseable, we throw rather than silently keying
  // without the suffix (discovery would throw on it anyway).
  const normalizeYmd = (raw: string | undefined, name: string): string => {
    const s = (raw ?? "").trim();
    if (!s) return "";
    ...
    throw new Error(
      `[build-bwb-weekly] ${name}="${s}" is not a valid YYYY-MM-DD date — refusing to run: a pinned window that cannot be keyed deterministically would collide with other same-week manifests.`,
    );
  };
  const winStart = normalizeYmd(process.env.BWB_WEEK_START, "BWB_WEEK_START");
  const winEnd = normalizeYmd(process.env.BWB_WEEK_END, "BWB_WEEK_END");
  const winSuffix = winStart && winEnd ? `-${winStart}_${winEnd}` : "";
  const jobKey =
    process.env.BWB_JOB_KEY ||
    (process.env.BWB_FRESH === "1"
      ? `bwb-weekly-${isoWeek}${winSuffix}-fresh-${Date.now()}`
      : `bwb-weekly-${isoWeek}${winSuffix}`);
  ...
  if (process.env.BWB_JOB_KEY) {
    ...
    console.log(
      `[build-bwb-weekly] jobKey="${jobKey}" (${winSuffix ? "ISO week + pinned window — a different BWB_WEEK_START/END gets its OWN manifest" : "ISO week"}) — will REUSE this run's already-produced transcripts/script/images/audio if present. Set BWB_FRESH=1 for a clean rebuild.`,
    );
  }
  // BWB_PURGE=1 — delete ALL persisted checkpoint manifests for this ISO week
  // (pure key + every window-suffixed variant) BEFORE opening checkpoints, so
  // this run provably starts clean and NOTHING from a prior same-week run
  // (e.g. a wrong-window run's stale transcripts) can ever be resumed — by
  // this run OR a later unpinned run. Fail-CLOSED: if the purge itself fails
  // we stop rather than run on top of the artifacts the operator asked to kill.
  if (process.env.BWB_PURGE === "1") {
    const { purgeCheckpoints } = await import("../server/agentic/pipeline-checkpoint");
    const purged = await purgeCheckpoints({ tenantId: TENANT_ID, jobKeyPrefix: `bwb-weekly-${isoWeek}` });
    console.log(`[build-bwb-weekly] BWB_PURGE=1 — deleted ${purged} stale checkpoint row(s) for prefix "bwb-weekly-${isoWeek}".`);
  }
  ...
  const script = await ck.stage<VideoScript>(
    ...
    async () => {
      ...
      writeFileSyncEIO(out, JSON.stringify(s, null, 2));
  ...
  if (!existsSyncEIO(outPath)) {
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    writeFileSyncEIO(outPath, JSON.stringify(script, null, 2));
  }
  ...
  const runRender = (script: string): { code: number; out: string } => {
    const r = bwbScriptCommand(script);
  ...
  if (code !== 0 && useGithub) {
    ...
    farmReason = renderFailureReason(out) || `farm backend exited ${code}`;
    if (inProd) {
      console.error(
      ...
      if (code !== 0) farmReason = renderFailureReason(out) || farmReason;
  ...
  if (code !== 0) {
    const localWhy = renderFailureReason(out) || `render backend exited ${code}`;
    let reason: string;
  ...
  // Inspect the render backend's result sidecar for cleanup eligibility. The
  // orchestrator is the ONLY authority that marks the live job done, and does so
  // only after tenant-scoped durable-receipt verification. This builder must not
  // let an old date-keyed sidecar race that verification.
  if (code === 0) {
    ...
    try {
      ...
      const expectedRunId = process.env.BWB_ORCHESTRATOR_RUN_ID;
      if (expectedRunId && sidecar.orchestratorRunId !== expectedRunId) {
        console.error("[build-bwb-weekly] result sidecar belongs to a different orchestrator run — refusing completion.");
        process.exit(2);
      }
      ...
      console.error(`[build-bwb-weekly] could not validate result sidecar: ${e?.message || e}`);
      process.exit(2);
    }
    ...
    if (!deliverySucceeded) {
      ...
      if (!/^[A-Za-z0-9._-]+$/.test(script.videoId)) {
        ...
        try {
          // best-effort post-delivery cleanup inside try/catch — a transient EIO
          // here is caught below; next run re-bakes. eio-safe: cleanup (try/catch).
...
// NOTE: in the prod CJS bundle (dist/build-bwb-weekly.cjs) esbuild leaves
// import.meta.url EMPTY, so the URL check alone fails closed and main() would
// silently never run. The BWB_RUN_AS_MAIN sentinel (set by the spawner only)
// makes the direct-invocation case work under both `npx tsx` (dev) and
// `node dist/*.cjs` (prod) while still keeping main() dormant on plain import.
const isMain =
  process.env.BWB_RUN_AS_MAIN === "1" ||
