...
(async () => {
  try {
    ...
    lines.push('  Database tables:        148 (Postgres + pgvector)');
    lines.push('  Governance rules:       40 (enforced via guarded tool executor)');
    lines.push('  Codebase:               ~178k LOC TypeScript across 452 files');
    ...
    lines.push('================================================================');
    lines.push('');
    lines.push('  - OpenAI (gpt-5, gpt-5.4, gpt-image-2, embeddings)');
    ...
    lines.push(`  R78 + R78.1 build  |  ${date}`);
    lines.push('================================================================');
    ...
    pdfSections.push({
      ...
      twoColumn: {
        left: {
          title: 'AI Providers',
          bullets: [
            'OpenAI (gpt-5, gpt-5.4, gpt-image-2, embeddings)',
    ...
    pdfSections.push({
      ...
      highlight: `${tools.length} tools  |  ${skillsRaw.length} skills  |  ${personas.length} active personas  |  36 curated AI models + 1000+ daily  |  148 tables  |  ~178k LOC TypeScript`,
    });
    ...
    const pdfResult = await (generateStyledPdf as any)({
      ...
      coverStats: [
        ...
        { label: 'Tables', value: '148' },
        { label: 'LOC', value: '~178k' },
        { label: 'Files', value: '452' },
        { label: 'Latest Round', value: 'R78 + R78.1' },
    ...
    // Register both in project_files (project 15 = presentation project for Felix)
    ...
    const emailBody = `VisionClaw Comprehensive Features - R78 + R78.1 Build (${date})
...
What changed since the last comprehensive doc:
  - R78 — A2A v0.3 Agent Card published at /.well-known/agent.json
  - R78.1 — Five-architect whole-app code review hardening (6 fixes)
  ...
  - Tool count: 264 -> 266
  - Code: ~178k LOC across 452 files

Both files are also registered to project 15 (presentation project) for Felix access.

-- VisionClaw Agent Pipeline (R78.1 build)
`;

    const emailResult = await (sendEmail as any)({
      inboxId,
      to: ownerEmail,
      subject: 'VisionClaw Updated Features - PDF + Text (R78 + R78.1, May 2 2026)',
      text: emailBody,
    });
