...
async function main() {
  ...
  try {
    ...
    const counts = {
      ...
      capabilities: capabilities.rows[0].n,
...
| **Built-in tools** | **${counts.tools}** | Static unique-name extraction across \`server/tools.ts\` + migrated domain definitions |
...
| **Capabilities (active)** | **${counts.capabilities}** | \`SELECT count(*) FROM capabilities WHERE is_active=true\` |
...
`;
    ...
    console.log(`  tools=${counts.tools} skills=${counts.skills}(+4=${skillsTotal}) personas=${counts.personas} capabilities=${counts.capabilities} tables=${counts.tablesDeclared}(declared)/${counts.tablesLive}(live) gov=${counts.governance} indexes=${counts.platformIndexes}(release)/${counts.nonPrimaryKeyIndexes}(non-PK)`);
  } finally {
    await pool.end();
  }
}
