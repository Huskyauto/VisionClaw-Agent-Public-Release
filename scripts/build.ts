...
async function buildAll() {
  ...
  // The PDF inspection worker is intentionally a plain CommonJS child
  // process: production runs it with node, never npx/tsx.
  await mkdir("dist", { recursive: true });
  await copyFile("server/workers/pdf-inspect-worker.cjs", "dist/pdf-inspect-worker.cjs");
  console.log("[build] copied PDF inspection worker");
  ...
  const bwbChainEntries = [
    ...
    "scripts/wedge-lifecycle-update.ts",
