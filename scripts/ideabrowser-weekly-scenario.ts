...
(async () => {
    ...
    const pick = async (whereExtra: any) => {
      const r: any = await db.execute(sql`
        SELECT id, name, description,
               metadata->'priority'->>'tier' AS tier,
               (metadata->'priority'->>'composite')::int AS composite,
               metadata->'priority'->>'evidence_confidence' AS evidence_confidence
        FROM projects
        WHERE tenant_id = ${TENANT_ID}
          AND ('isenberg' = ANY(tags) OR 'isenberg-iotd' = ANY(tags) OR 'ideabrowser' = ANY(tags))
      ...
      return ((r.rows || r) as any[]).map((row) => ({
        ...
        evidenceConfidence: row.evidence_confidence || null,
      }));
    };
  ...
  md += `## Full ranking\n\n`;
  const confByProject = new Map(candidates.map((c) => [c.id, c.evidenceConfidence || "n/a"]));
  md += `| # | Idea | E[profit] | P(success) | ROI | 12-mo rev | Build cost | Hours | $/mo | Days→$1 | Evidence |\n|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|\n`;
  results.forEach((s, i) => {
    md += `| ${i + 1} | ${s.name.slice(0, 60)} (#${s.projectId}) | $${s.expectedProfitUsd.toLocaleString()} | ${s.successProbabilityPct}% | ${s.roi}x | $${s.revenue12moUsd.toLocaleString()} | $${s.buildCostUsd.toLocaleString()} | ${s.buildHours} | $${s.monthlyRunCostUsd} | ${s.daysToFirstDollar} | ${confByProject.get(s.projectId) || "n/a"} |\n`;
  });
  md += `\nRanked by **expected profit** = P(success) × 12-mo revenue − total cost (build + 12 mo run + Bob-hours @$50).\n`;
  md += `Evidence = confidence in the idea's underlying research (High/Medium/Low from linked evidence rows; n/a = scored before evidence tracking).\n`;
  md += `\n## Scenario details\n\n`;
  ...
  // The Drive artifact is durable. Owner email is sent separately through the
  // direct Gmail proof path below: AgentMail acceptance is not evidence that
  // Bob's Gmail Inbox received the report.
  try {
    ...
    const dr = await deliverDigitalProduct({
      ...
      emailSubject,
      emailBody,
      idempotencyKey: `ideabrowser-weekly-scenario:${today}`,
    ...
    if (dr.success) {
      ...
      if (!to) throw new Error("Owner email address is missing; cannot verify weekly report delivery.");
      const { gmailSendAndVerifyInbox } = await import("../server/google-workspace");
      const inboxResult = await db.transaction(async (tx) => {
        const lockResult: any = await tx.execute(sql`
          SELECT pg_try_advisory_xact_lock(${dr.deliveryId}) AS locked
        `);
        const locked = Boolean(((lockResult as any).rows || lockResult || [])[0]?.locked);
        if (!locked) throw new Error("Weekly owner-report email is already being sent by another run; refusing a duplicate send.");
        const [delivery] = await tx
          .select({ emailMessageId: deliveryLogs.emailMessageId })
          .from(deliveryLogs)
          .where(and(eq(deliveryLogs.id, dr.deliveryId), eq(deliveryLogs.tenantId, TENANT_ID)))
          .limit(1);
        const claimMarker = `pending:ideabrowser-weekly-scenario:${today}`;
        const claimResult: any = await tx.execute(sql`
          UPDATE delivery_logs
          SET email_message_id = ${claimMarker}, completed_at = NOW()
          WHERE id = ${dr.deliveryId}
            AND tenant_id = ${TENANT_ID}
            AND (
              email_message_id IS NULL
              OR (
                email_message_id LIKE 'pending:ideabrowser-weekly-scenario:%'
                AND completed_at < NOW() - INTERVAL '5 minutes'
              )
            )
          RETURNING email_message_id
        `);
        const claimed = ((claimResult as any).rows || claimResult || []).length > 0;
        if (!claimed && String(delivery?.emailMessageId || "").startsWith("pending:")) {
          throw new Error("Weekly owner-report email is already being sent by another run; refusing a duplicate send.");
        }
        return gmailSendAndVerifyInbox(
          TENANT_ID,
          to,
          emailSubject,
          `${emailBody}\n\nReport link: ${reportUrl || "Drive link unavailable; use the attached weekly-run project."}`,
          {
            existingMessageId: claimed ? undefined : delivery?.emailMessageId,
            stableMessageKey: `ideabrowser-weekly-scenario-${today}`,
            onMessageSent: async (messageId) => {
              await tx.update(deliveryLogs)
      ...
      if (!inboxResult.inboxVerified) {
        throw new Error(
          `Gmail accepted report ${inboxResult.messageId} but it was not visible in the owner Inbox after ${inboxResult.verificationAttempts} bounded checks.`,
        );
      }
      await db.update(deliveryLogs)
        .set({ emailSent: true, emailMessageId: inboxResult.messageId })
        .where(and(eq(deliveryLogs.id, dr.deliveryId), eq(deliveryLogs.tenantId, TENANT_ID)));
      console.log(`[weekly-scenario] delivered: drive=${reportUrl || "n/a"} gmail_inbox_verified=true message=${inboxResult.messageId}`);
    } else {
