 ...
 *   5. Output: markdown report mirrored in docs/ and durably staged in
 *      deliverables/, Drive upload to the "Bob's Replit
 *      Ideabrowser" project folder, owner email with the winner + full ranking.
 *
 * One-line agent-runnable, no prompts. Exit codes:
 *   0 — report written, uploaded, and owner email verified in Gmail INBOX
 *   1 — scenario pass failed for all candidates OR owner delivery was not verified
...
(async () => {
  ...
  const reportFileName = `ideabrowser-weekly-scenario-${today}.md`;
  ...
  console.log(`[weekly-scenario] wrote durable ${deliveryPath} and mirror ${outPath} (${md.length} chars)`);
  ...
  try {
    ...
    console.error("[weekly-scenario] winner metadata stamp failed:", e);
  }
  ...
  try {
    ...
    persistenceFailure = e instanceof Error ? e : new Error(String(e));
    console.error(`[weekly-scenario] weekly-run project filing failed: ${persistenceFailure.message}`);
  }
  ...
  // The Drive copy remains durable, but EMAIL is mandatory for this scheduled
  // run: do not report a green weekly job unless Gmail returned destination-
  // side INBOX evidence through the delivery pipeline.
  try {
    const to = resolveOwnerEmail();
    if (!to) {
      throw new Error("owner email is not configured — weekly report cannot be delivered");
    }
    ...
    if (dr.success && dr.emailSent) {
      ...
      console.error(`[weekly-scenario] ${deliveryFailure.message} (durable report at ${deliveryPath})`);
    }
  } catch (e: any) {
    deliveryFailure = e instanceof Error ? e : new Error(String(e));
    console.error(`[weekly-scenario] delivery failed: ${deliveryFailure.message} (durable report at ${deliveryPath})`);
  }
  ...
  if (runProjectId) {
    try {
      await db.execute(sql`
        ...
        SELECT ${runProjectId}, ${reportFileName}, ${deliveryPath}, ${reportUrl}, 'text/markdown', 'felix-weekly-scenario'
        WHERE NOT EXISTS (
      ...
      persistenceFailure = e instanceof Error ? e : new Error(String(e));
      console.error(`[weekly-scenario] report attach failed: ${persistenceFailure.message}`);
    }
  } else if (!persistenceFailure) {
    persistenceFailure = new Error("weekly-run project was not created; report attachment is impossible");
  }
