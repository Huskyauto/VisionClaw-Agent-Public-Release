...
(async () => {
  ...
  try {
    ...
    if (dr.success && dr.emailSent) {
      ...
      console.error(`[weekly-scenario] ${deliveryFailure.message} (report still at ${outPath})`);
    }
  } catch (e: any) {
    deliveryFailure = e instanceof Error ? e : new Error(String(e));
    console.error(`[weekly-scenario] delivery failed: ${deliveryFailure.message} (report still at ${outPath})`);
  }
