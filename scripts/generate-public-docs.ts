...
function extractTools(): ExtractedTools {
  ...
  // For the legacy monolith we anchor at the TOOL_DEFINITIONS declaration to skip
  // unrelated earlier text; domain files are scanned whole (they only carry defs).
  ...
  for (const file of getToolSourceFiles()) {
    ...
    let scan = src;
    if (path.resolve(file) === path.resolve(TOOLS_SRC)) {
      const declStart = src.indexOf("export const TOOL_DEFINITIONS");
      if (declStart >= 0) {
        scan = src.slice(declStart);
        foundDecl = true;
      } else {
        scan = ""; // facade end-state: monolith may no longer declare the array
      }
    } else {
      foundDecl = true; // domain files count as definition sources
    }
    let m: RegExpExecArray | null;
    re.lastIndex = 0;
  ...
  if (!foundDecl) throw new Error("TOOL_DEFINITIONS not found in any tool source file");
...
function extractPersonas(): Array<{ id: number; name: string; role: string; identity: string }> {
  ...
  // Personas added after the prompt-file era (17+ e.g. Hermes, Echo) live as
  // object literals in server/seed.ts (`name: "X", role: "Y", ... identity: \`...\``).
  // Append any active seed.ts persona not already captured above.
  try {
    const seedSrc = fs.readFileSync(path.join(ROOT, "server/seed.ts"), "utf8");
    const seedRe = /\{\s*name:\s*"([A-Za-z]+)",\s*role:\s*"([^"]+)",[\s\S]{0,400}?isActive:\s*true,[\s\S]*?identity:\s*`([^`]+)`/g;
    let sm: RegExpExecArray | null;
    let nextId = out.length ? Math.max(...out.map((p) => p.id)) + 1 : 1;
    const seenNames = new Set(out.map((p) => p.name.toLowerCase()));
    while ((sm = seedRe.exec(seedSrc))) {
      const name = sm[1].trim();
      if (seenNames.has(name.toLowerCase())) continue;
      seenNames.add(name.toLowerCase());
      out.push({ id: nextId++, name, role: sm[2].trim(), identity: sm[3].trim() });
    }
  } catch {
    // seed.ts unreadable — fall back to prompt-file personas only
  }
