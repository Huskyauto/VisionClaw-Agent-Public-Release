  ...
  for (const rel of SCAN) {
    ...
    if (rel === "README.md") {
      ...
      text = withoutJsxComments
        .split("\n")
        .filter((line) => /Platform Online —|Platform totals remain|^\s*18 personas, \d+ declared|Database tables", v:|active capabilities<\/span>|tools · \d+ curated models|Curated AI Models \+ 1000\+ Daily Discovery|420 total registered tools;|Start with 5 free conversations/.test(line))
        .join("\n");
