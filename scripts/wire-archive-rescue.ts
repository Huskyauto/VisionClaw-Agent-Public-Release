...
// Strict argv gate (72h review 2026-07-30): runs LIVE — reject unknown flags
// (e.g. --dry-run) instead of silently executing side effects.
const UNKNOWN_FLAGS = process.argv.slice(2).filter((a) => !a.startsWith("--tenant-id="));
if (UNKNOWN_FLAGS.length > 0) {
  console.error(`[wire-archive-rescue] unknown flag(s) ${UNKNOWN_FLAGS.join(", ")} — no dry-run mode; only --tenant-id=N is accepted. Aborting.`);
  process.exit(2);
}
...
async function upsertHeartbeatTask(t: typeof CRON_TASKS[number], personaId: number | null): Promise<{ id: number; created: boolean }> {
  ...
  const ins: any = await db.execute(sql`
    ...
    VALUES (${t.name}, ${t.description}, ${t.type}, ${t.cron}, true, ${t.prompt}, 'gpt-5-mini', ${personaId}, 'wire-archive-rescue', ${TENANT_ID}, ${nextCronRun(t.cron)})
    RETURNING id
  `);
  return { id: ((ins.rows || ins)[0] as any).id, created: true };
}
