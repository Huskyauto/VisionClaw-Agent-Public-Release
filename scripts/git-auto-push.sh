...
# Diverged-remote self-heal (extracted, testable — see the lib header).
source scripts/lib/auto-push-selfheal.sh
...
while true; do
  ...
  if [ "$AUTO_PUSH" = "1" ]; then
    ...
    if [ "${AHEAD:-0}" -gt 0 ]; then
      ...
      PUSH_OUT=$(bash scripts/git-push.sh "$BRANCH" 2>&1)
      PUSH_RC=$?
      printf '%s\n' "$PUSH_OUT"
      if [ "$PUSH_RC" -eq 0 ]; then
        ...
        echo "[auto-push] pending-push failed (status $PUSH_RC), will retry next cycle" >&2
        # ---- Diverged-remote self-heal (R125+52.x) ----
        # A task agent or the publish flow can push to GitHub directly, putting
        # the remote AHEAD of this workspace. The loop then rejects with
        # non-fast-forward forever (it deliberately never fetches — plain
        # `git fetch` hangs on the askpass/tty helper). Self-heal SAFELY:
        #   - only when the push output shows a non-fast-forward rejection
        #   - only when the working tree is completely clean
        #   - fetch via the token URL (no tty prompt possible)
        #   - rebase local commits onto the remote tip; on ANY failure,
        #     abort the rebase and leave everything untouched.
        # Never force-pushes, never discards commits.
        # Logic lives in scripts/lib/auto-push-selfheal.sh (R125+139 — extracted
        # so the branch behavior is covered by tests/unit/auto-push-selfheal.test.ts).
  ...
  STAMP=$(date -Iseconds)
  # Session provenance (Atlas-inspired, 2026-08-10): if the working agent left
  # a session marker, stamp it into the commit message so later archaeology can
  # answer "which session produced this commit?". Optional + fail-open: no
  # marker, no line. Single-line, capped at 200 chars, control chars stripped.
  SESSION_LINE=""
  if [ -f .local/session-context.txt ]; then
    SESSION_CTX=$(head -c 400 .local/session-context.txt 2>/dev/null | tr -d '\000-\011\013-\037' | head -n 1 | cut -c1-200)
    [ -n "$SESSION_CTX" ] && SESSION_LINE="
Session: ${SESSION_CTX}"
  fi
  MSG="auto: ${FILE_COUNT} file(s) @ ${STAMP}

Auto-committed by scripts/git-auto-push.sh after ${QUIET_SECONDS}s of stable changes.
Touched: ${CHANGED_PATHS}${SESSION_LINE}"
