...
// Strict argv gate (72h review 2026-07-30): this script runs LIVE on invocation
// — an operator habit like --dry-run must hard-fail, not silently execute.
const UNKNOWN_FLAGS = process.argv.slice(2).filter((a) => !a.startsWith("--tenant-id="));
if (UNKNOWN_FLAGS.length > 0) {
  console.error(`[wire-wedges] unknown flag(s) ${UNKNOWN_FLAGS.join(", ")} — this script has no dry-run mode and runs live. Only --tenant-id=N is accepted. Aborting.`);
  process.exit(2);
}
...
const WEDGES = [
  ...
  },
  {
    ...
    fullDescription:
      "R125+13.15 active wedge track (Bob's pick 2026-08-14 — highest composite score 27/S in the Isenberg portfolio, IOTD 2026-05-31). $99/$249/$499 per month automated lead enrichment: raw lead in → agent-researched, ensemble-drafted sales dossier out. HARD RULE: dossiers are drafted by the 3-proposer MoA frontier trio (gpt-5.6-sol, claude-sonnet-5, gemini-3.1-pro-preview) with claude-opus-5 as the 4th-model finisher — never a single model. SOP: data/output-skills/wedge-smart-leads-sop.md. Owner: Felix. Dogfood loop: every inbound VisionClaw lead runs through this pipeline first.",
    tags: ["wedge:smart-leads", "wedge", "stage:validation", "track:isenberg-iotd"],
    sopFile: "data/output-skills/wedge-smart-leads-sop.md",
    onePager: null,
  },
];
...
async function upsertHeartbeatTask(t: typeof CRON_TASKS[number], personaId: number | null): Promise<{ id: number; created: boolean }> {
  ...
  if (row) {
    ...
    await db.execute(sql`
      ...
      WHERE id=${row.id} AND tenant_id=${TENANT_ID}
    `);
  ...
  const ins: any = await db.execute(sql`
    ...
    VALUES (${t.name}, ${t.description}, ${t.type}, ${t.cron}, true, ${t.prompt}, 'gpt-5-mini', ${personaId}, 'wire-wedges', ${TENANT_ID}, ${nextCronRun(t.cron)})
    RETURNING id
  `);
  return { id: ((ins.rows || ins)[0] as any).id, created: true };
}
...
(async () => {
  ...
  for (const w of WEDGES) {
    ...
    try {
      const existingFolder: any = await db.execute(sql`SELECT drive_folder_id FROM projects WHERE id=${proj.id} AND tenant_id=${TENANT_ID}`);
      ...
      if (!dfid) {
        ...
        await db.execute(sql`UPDATE projects SET drive_folder_id=${folder.id}, drive_folder_url=${folder.url} WHERE id=${proj.id} AND tenant_id=${TENANT_ID}`);
