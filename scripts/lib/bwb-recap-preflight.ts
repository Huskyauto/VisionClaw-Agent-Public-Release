...
import { probeFsHealth } from "./eio-read";
...
export interface PreflightInput {
  ...
  /** skip the overlayFS health canary (BWB_SKIP_FS_CANARY=1 for a deliberate run) */
  skipFsCanary?: boolean;
}
...
function resolveInput(over: PreflightInput): PreflightInput {
  ...
  return {
    ...
    skipFsCanary: over.skipFsCanary ?? e.BWB_SKIP_FS_CANARY === "1",
  };
}
...
export function preflightWeeklyRecap(over: PreflightInput = {}): PreflightReport {
  ...
  // 0. FILESYSTEM HEALTH — the overlayFS canary. The Reserved-VM overlayFS
  //    intermittently enters a prolonged degraded-I/O spell (EIO on ordinary
  //    reads/writes for minutes-to-hours). A recap that STARTS during one pays
  //    for transcription + image gen + a paid GitHub-farm render before an EIO
  //    finally kills a late stage — the exact "burned ~3h and money" failure
  //    mode (2026-06-27). Probe the ACTUAL disk the render writes to up front so
  //    a degraded box fails fast & cheap here instead of expensive & late. BLOCK
  //    only on a genuinely fatal disk state (EIO/ENOSPC/EROFS or a corrupt
  //    read-back) — a transient blip self-heals inside the EIO retry budget, and
  //    any non-fatal canary quirk only WARNs so a healthy run is never gated.
  if (!i.skipFsCanary) {
    const canaryDir = path.join(process.cwd(), "data", ".bwb-fs-canary");
    const h = probeFsHealth(canaryDir);
    const FATAL_CODES = new Set(["EIO", "ENOSPC", "EROFS"]);
    const fatal = !h.ok && (h.failedOp === "verify" || (h.code ? FATAL_CODES.has(h.code) : false));
    checks.push({
      id: "fs-health",
      label: "Filesystem (overlayFS)",
      ok: h.ok,
      severity: fatal ? "block" : "warn",
      detail: h.ok ? h.detail : h.detail,
      fix: h.ok
        ? undefined
        : fatal
          ? "The box's overlayFS is in a degraded I/O spell (EIO/ENOSPC/corrupt read-back) — a render would burn a paid farm job, then die mid-pipeline. Wait for the spell to pass and re-run; if it persists, restart the deployment/workspace. To bypass deliberately, set BWB_SKIP_FS_CANARY=1."
          : undefined,
    });
  }
  ...
  checks.push({
    ...
    severity: "warn",
    detail: pinned
      ? `pinned ${i.weekStart} → ${i.weekEnd}.`
      : "no explicit week — builder AUTO-PINS the just-completed Sun–Sat window (set BWB_WEEK_START/END only for a redo of a named week).",
  });
