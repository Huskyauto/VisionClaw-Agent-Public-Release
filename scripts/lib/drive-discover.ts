...
/**
 * AUTO-PIN helper (2026-07-12): resolve the default recap window as local-time
 * YYYY-MM-DD strings ready to pin into BWB_WEEK_START/END. Wraps computeWeekWindow
 * (no explicit range) so the builder's env-pinning glue is pure + unit-testable
 * here without importing the builder (which drags in the DB pool).
 * Also returns the deterministic jobKey window suffix (`-<start>_<end>`) so
 * tests can assert checkpoint-identity encoding matches the pinned window.
 */
export function autoPinWindowYmd(args: { anchorRaw?: string; now?: Date } = {}): {
  start: string;
  end: string;
  windowDesc: string;
  jobKeySuffix: string;
} {
  const win = computeWeekWindow({ anchorRaw: args.anchorRaw, now: args.now });
  const ymd = (t: number) => {
    const d = new Date(t);
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
  };
  const start = ymd(win.cutoff);
  const end = ymd(win.upperBound);
  return { start, end, windowDesc: win.windowDesc, jobKeySuffix: `-${start}_${end}` };
}
