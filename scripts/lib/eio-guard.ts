 ...
 * / statSyncEIO / readdirSyncEIO / existsSyncEIO); the rare legitimately-raw op
 * (cosmetic log, or a call already inside a fall-through try/catch) carries an inline
 * `eio-safe:` comment.
 *
 * `existsSync` is guarded specifically because it is the INSIDIOUS one: it never
 * throws — it swallows a transient EIO and returns a wrong `false`, silently
 * branching a degraded run down the wrong path (the 2026-06-27 3h failure). The
 * `existsSyncEIO` helper restores honest semantics (throws on persistent EIO).
 *
...
// Read-class ops THROW or silently mis-branch on a transient overlayFS EIO.
// `writeFileSync` is guarded too: a degraded-FS spell can "succeed" a write that
// truncates the bytes, so every functional write must route through the verified
// `writeFileSyncEIO` (write → size-verify → retry) or carry an `eio-safe` marker.
const OPS = new Set(["readFileSync", "copyFileSync", "statSync", "readdirSync", "existsSync", "writeFileSync"]);
