...
function ffprobeDuration(file: string): number {
  const r = spawnSync("ffprobe", ["-v", "error", "-show_entries", "format=duration", "-of", "default=nw=1:nk=1", file], { encoding: "utf8", env: sanitizeSpawnEnv(process.env) });
...
function extractFromZip(zipPath: string, member: string, destDir: string): boolean {
  ...
  for (const [cmd, args] of attempts) {
    try {
      ...
      if (r.status === 0 && fs.existsSync(target) && fs.statSync(target).size > 0) return true;
...
async function genSceneAudio(
  ...
  // Handler sanitizes filename and writes project-assets/<sanitized>.<ext>.
  const sanitized = filename.replace(/[^a-zA-Z0-9_-]/g, "_");
  const candidates = [
    (res && (res.audioPath || res.path || res.filePath || res.file)) as string | undefined,
    path.resolve("project-assets", `${sanitized}.mp3`),
    path.resolve("project-assets", `${sanitized}.wav`),
...
function enforceDailyCap(fail: (msg: string) => never, log: (msg: string) => void): void {
  ...
  try {
    ...
    try {
      if (fs.existsSync(DAILY_CAP_FILE)) prev = JSON.parse(fs.readFileSync(DAILY_CAP_FILE, "utf8"));
    } catch (e: any) {
...
export async function renderOnGithubFarm(opts: RenderFarmOptions): Promise<RenderFarmResult> {
  ...
  // 1. Images (reuse on-disk where present)
  fs.mkdirSync(sceneDir, { recursive: true });
  for (let i = 0; i < opts.scenes.length; i++) {
    const s = opts.scenes[i];
    if (s.imagePath && fs.existsSync(s.imagePath)) continue;
    const dest = `${sceneDir}/scene-${i + 1}.png`;
    if (fs.existsSync(dest)) { s.imagePath = dest; continue; }
  ...
  for (let i = 0; i < opts.scenes.length; i++) {
    ...
    fs.copyFileSync(s.imagePath!, path.join(bundleDir, imgRel));
    fs.copyFileSync(audioSrc, path.join(bundleDir, audRel));
    sceneMeta.push({ image: imgRel, audio: audRel, duration: dur });
    log(`  scene ${i + 1}: audio ${audioDur.toFixed(1)}s`);
  }
  ...
  fs.copyFileSync("scripts/render-chapter-ffmpeg.mjs", path.join(bundleDir, "render-chapter-ffmpeg.mjs"));
  const tarball = path.join(path.dirname(bundleDir), `${jobId}.tar.gz`);
  const tarRes = spawnSync("tar", ["-czf", tarball, "-C", bundleDir, "."], { stdio: "inherit", env: sanitizeSpawnEnv(process.env) });
  if (tarRes.status !== 0) fail("tar failed");
  ...
  try {
    ...
    const sizeMB = (fs.statSync(outMp4).size / 1024 / 1024).toFixed(2);
