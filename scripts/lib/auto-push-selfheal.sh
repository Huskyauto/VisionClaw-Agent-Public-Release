...
auto_push_selfheal() {
  ...
  # git phrases the divergence reject two ways depending on version/path:
  #   "non-fast-forward" or a rejected-ref line ending in "(fetch first)".
  # Anchor the second form to the actual rejection record so unrelated
  # hook/stderr text containing "fetch first" can't consume a heal attempt.
  if ! printf '%s' "$PUSH_OUT" | grep -qE "non-fast-forward|^[[:space:]]*![[:space:]]+\[rejected\].*\(fetch first\)"; then
    return 0
  fi
  ...
  if GIT_TERMINAL_PROMPT=0 timeout --signal=TERM 60s git fetch "$HEAL_URL" "$BRANCH" 2>&1 | sed -E "s|${HEAL_TOKEN}|REDACTED|g"; then
