 ...
 * Runs the 12-pass weekly sweep and writes a summary to /tmp/weekly-maintenance-<date>.json
 ...
 *
 * Usage:
 *   npx tsx scripts/weekly-maintenance.ts                # run all 12 passes, write summary
...
async function main() {
  ...
  const independent = passDefs.filter(([n]) => shouldRun(n) && [1, 2, 3, 6, 7, 8, 9, 10, 11, 12].includes(n));
