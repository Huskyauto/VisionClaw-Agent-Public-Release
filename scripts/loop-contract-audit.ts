...
function main() {
  ...
  if (asJson) {
    console.log(JSON.stringify({ total: LOOP_CONTRACTS.length, errors: errors.length, warnings: warns.length, findings }, null, 2));
  } else {
    ...
    for (const c of LOOP_CONTRACTS) {
      ...
      console.log(`  ${mark} ${c.id.padEnd(28)} ${c.authority}/${c.failMode}  → ${c.workflow ?? "in-process"}`);
