...
timeout --signal=TERM 30s git ls-remote "$PUSH_URL" HEAD >/dev/null 2>&1
PREFLIGHT_RC=$?
...
if timeout --signal=TERM 120s git push "$PUSH_URL" "$BRANCH" "${EXTRA_ARGS[@]}" 2>&1 | sed -E "s|${TOKEN}|REDACTED|g"; then
