...
import { readFileSyncEIO, copyFileSyncEIO, statSyncEIO, existsSyncEIO, writeFileSyncEIO } from "./lib/eio-read";
...
async function main() {
  ...
  if (!existsSyncEIO(scriptPath!)) fail(`script not found: ${scriptPath}`);
  ...
  for (let i = 0; i < script.scenes.length; i++) {
    ...
    // best-effort stale-image delete — a transient EIO making existsSync read
    // false just skips the unlink; the checkpoint verify below independently
    // re-bakes any mismatched image. eio-safe: best-effort delete (verify re-bakes).
    ...
    const { result, reused } = await ck.stage<{ imagePath: string }>(
      {
        ...
        verify: (_r, p) => !!p && existsSyncEIO(p) && (!s.imagePrompt || imageMatchesPrompt(p, s.imagePrompt)),
      },
      async () => {
  ...
  const { result: delivery, reused: deliveryReused } = await ck.stage<{
    ...
    async () => {
      ...
      const d = await deliverDigitalProduct({
        ...
        mimeType: "video/mp4",
        // Weekly orchestration sends the approval email only after validating
        // this durable Drive receipt. Standalone BWB builds retain their normal
        // delivery email.
        sendEmail: !process.env.BWB_ORCHESTRATOR_RUN_ID,
