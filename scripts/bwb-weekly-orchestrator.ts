...
import { readFileSyncEIO, statSyncEIO, readdirSyncEIO, existsSyncEIO } from "./lib/eio-read";
import * as path from "node:path";
import { sanitizeSpawnEnv } from "../server/safety/spawn-env-guard";
import { completeBwbJob, failBwbJob, setBwbPhase } from "../server/lib/bwb-job-progress";
...
function findResultForRunSince(startMs: number, runId: string): string | null {
  if (!existsSyncEIO(SCRIPTS_DIR)) return null;
  const candidates = readdirSyncEIO(SCRIPTS_DIR)
    ...
    .filter((c) => {
      try {
        const candidate = JSON.parse(readFileSyncEIO(c.full, "utf8"));
        return candidate.orchestratorRunId === runId;
      } catch {
        return false;
      }
    })
...
async function main() {
  ...
  process.env.BWB_ORCHESTRATOR_RUN_ID = orchestratorRunId;
  const start = Date.now();
  console.log(`[bwb-weekly-orchestrator] starting (tenant=${TENANT_ID}, autopublish=${process.env.BWB_WEEKLY_AUTOPUBLISH === "1"})`);

  // Sentinel for the parent spawner (server/tools.ts `bwb_weekly_build`): writing
  // this tmpfs flag PROVES the orchestrator's `npx tsx` module graph loaded and
  // main() began. The parent re-spawns ONLY on a crash where this flag is ABSENT —
  // i.e. a pre-main tsx module-load EIO off the Reserved-VM overlayFS, where nothing
  // (budget claim, render dispatch, email) has run yet, so a re-spawn is safe and
  // idempotent. Once main() starts, the orchestrator owns its own bounded retry +
  // failure reporting and must NOT be re-spawned (that could double-claim the
  // autonomous budget).
  // The sentinel must be AUTHORITATIVE, not best-effort: the parent re-spawns iff
  // the flag is ABSENT, so if we silently failed to write it and then proceeded to
  // claim budget and crashed, the parent would re-spawn and double-claim. So write
  // AND verify; if it didn't land, exit BEFORE any spend (budget/render) so the
  // parent safely re-spawns a fresh attempt (nothing claimed here) — self-correcting.
  // Skipped entirely when the flag env is unset (standalone CLI / cron path runs
  // unchanged). tmpfs write+verify is near-infallible, so this is a fail-safe, not
  // a routine branch.
  const startedFlag = process.env.BWB_ORCH_STARTED_FLAG;
  if (startedFlag) {
    let flagWritten = false;
    try {
      fs.writeFileSync(startedFlag, String(Date.now())); // eio-safe: tmpfs (os.tmpdir, not overlayFS)
      flagWritten = fs.existsSync(startedFlag); // eio-safe: tmpfs verify, not overlayFS
    } catch (e) {
      logErr(`orchestrator-started sentinel write failed: ${(e as any)?.message || e}`);
    }
    if (!flagWritten) {
      logErr(
        "cannot write the started-sentinel (tmpfs); exiting before any spend so the parent re-spawns safely rather than risking a post-claim re-spawn double-spend.",
      );
      process.exit(1);
    }
  }
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (attempt > 1) {
      try {
        await setBwbPhase(
          `retrying (attempt ${attempt}/${MAX_ATTEMPTS}) after transient fault: ${lastTransient}`,
          { attempt },
        );
      } catch {
    ...
    const weeklyCmd = bwbScriptCommand("scripts/build-bwb-weekly.ts");
    build = spawnSync(weeklyCmd.cmd, weeklyCmd.args, {
      encoding: "utf8",
      maxBuffer: 64 * 1024 * 1024,
      // BWB_RUN_AS_MAIN makes build-bwb-weekly's direct-invocation guard fire
      // under the prod CJS bundle (where esbuild empties import.meta.url).
      env: { ...sanitizeSpawnEnv(process.env), BWB_RUN_AS_MAIN: "1" },
    });
  ...
  // 2. Pick up the produced result sidecar.
  const resultPath = findResultForRunSince(start, orchestratorRunId);
  if (!resultPath) {
  ...
  const invalidDeliveryReceipt =
    result.success !== true ||
    result.orchestratorRunId !== orchestratorRunId ||
    !Number.isSafeInteger(deliveryId) ||
    deliveryId <= 0 ||
    typeof result.driveFileId !== "string" ||
    result.driveFileId.trim().length === 0 ||
    ((typeof result.publicPlayLink !== "string" || result.publicPlayLink.trim().length === 0) &&
      (typeof result.driveViewUrl !== "string" || result.driveViewUrl.trim().length === 0));
  if (invalidDeliveryReceipt) {
    const message = "Build produced a sidecar without a confirmed durable Drive delivery receipt. Refusing approval/publish.";
    logErr(message);
    await failBwbJob(message);
    process.exit(2);
  }
  const { verifyCompletedArtifactReceipt } = await import("../server/delivery-pipeline");
  const verifiedReceipt = await verifyCompletedArtifactReceipt({
    deliveryId,
    tenantId: TENANT_ID,
    driveFileId: result.driveFileId,
  });
  if (!verifiedReceipt.ok) {
    const message = `Delivery receipt verification failed: ${verifiedReceipt.error}. Refusing approval/publish.`;
    logErr(message);
    await failBwbJob(message);
    process.exit(2);
  }
  await completeBwbJob({
    filePath: typeof result.filePath === "string" ? result.filePath : null,
    finalDriveUrl: result.driveViewUrl || result.publicPlayLink || null,
  ...
  const ctx = {
    ...
    tenantId: TENANT_ID,
    jobId: process.env.BWB_JOB_ID || null,
  ...
  try {
    ...
    try {
      const { recordBwbOversightShadow } = await import("../server/lib/bwb-oversight-shadow");
      await recordBwbOversightShadow({
        tenantId: TENANT_ID,
      ...
      logErr(`oversight shadow skipped after durable approval creation: ${error?.message || error}`);
    }
    ...
    await sendEmailDirect({
      ...
      html:
        ...
        // Durable link (2026-08-16): the self-hosted preview link dies when the
        // dev workspace sleeps or the app is republished (signed URL also
        // expires in 7 days). The Drive copy is permanent — always include it.
        (result.driveViewUrl && result.driveViewUrl !== ctx.deliveryUrl
          ? `<p><a href="${result.driveViewUrl}" style="color:#2563eb;">Google Drive copy (permanent — works even if the link above stops)</a></p>`
          : "") +
      ...
      text:
        ...
        `Preview/Download: ${ctx.deliveryUrl}\n` +
        (result.driveViewUrl && result.driveViewUrl !== ctx.deliveryUrl ? `Google Drive copy (permanent): ${result.driveViewUrl}\n` : "") +
        `\n` +
        `Channel readiness:\n${readinessLines(readiness)}\n` +
...
main().catch(async (e) => {
  ...
  // Catch-all so an UNHANDLED throw anywhere in main() (before the build/delivery
  // completes) still flips the /jobs card to "failed" instead of wedging on
  // "building…" until the 20-min stale reaper. failBwbJob is scoped notDone:true,
  // so a throw AFTER a successful build/delivery (e.g. a publish-notify error) is a
  // no-op here and won't mismark a job that already succeeded.
  try {
    await failBwbJob(`weekly recap orchestrator crashed: ${e?.message || String(e)}`);
  } catch {
    /* best-effort; logErr above already surfaced the cause */
  }
  process.exit(4);
});
