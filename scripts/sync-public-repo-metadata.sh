...
# public mirror repo with the live stats line in README.md. Idempotent.
#
# Reads counts from the bolded stats line at the top of README.md so
...
INDEXES=$(extract '[0-9]+\+? (production|platform) indexes')
...
[ -n "$INDEXES" ]  && PARTS+=("${INDEXES} platform indexes")
