 ...
 *   · Code is only ever touched via the jury-MAJORITY-FIX → repo_surgeon path,
 ...
 *   REPAIR_AUTOFIX_ENABLED=1            repo-surgeon actually applies majority FIX
 *
...
async function main() {
  // Strict argv gate: unknown flags fail closed (this script can take LIVE
  // backlog actions when ESCALATION_RESOLVER_LIVE=true) — never run on a typo.
  const unknownArgs = process.argv.slice(2).filter((a) => a !== "--once");
  if (unknownArgs.length > 0) {
    console.error(`[escalation-resolver] ERROR: unknown flag(s) ${unknownArgs.join(", ")} (allowed: --once). Refusing to run.`);
    process.exit(2);
  }
