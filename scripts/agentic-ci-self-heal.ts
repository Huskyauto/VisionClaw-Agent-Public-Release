...
// Credential contract: GITHUB_PERSONAL_ACCESS_TOKEN_2 is the preferred secure
// Replit Secret; GITHUB_TOKEN remains the existing fallback. See
// docs/github-automation.md. Never log either value.
...
const FIX_REGISTRY: FixRule[] = [
  ...
  {
    // Task 101 (2026-07-31) — docs platform-count drift ("225 tables" vs SoT
    // 226) was the #1 recurring NEEDS-HUMAN class: every Auto Git Push re-ran
    // the Docs workflow, re-failed verify-counts, and burned a jury call. The
    // fix is purely mechanical: replace the stale number inside the quoted
    // snippet verify-counts itself reports, in the file it names. No prose is
    // rewritten — only the digits verify-counts flagged, verified by re-running
    // the same gate.
    id: "docs-count-drift",
    description: "Docs platform-count drift vs docs/CURRENT_PLATFORM_TOTALS.md — auto-sync the stale number the verify-counts gate reports",
    match: /verify-counts: \d+ count mismatch\(es\)/,
    fix: async () => {
      let out = "";
      try {
        out = execSync("npx --yes tsx scripts/verify-counts.ts 2>&1 || true", { encoding: "utf8", env: SAFE_ENV });
      } catch (e: any) { out = e?.stdout?.toString?.() ?? ""; }
      const touched: string[] = [];
      const fixes: string[] = [];
      // ❌ ROADMAP.md:82 — "225 tables" but SoT says tables (live)=226 (also allowed: 169)
      const lineRe = /❌ ([^\s:]+):\d+ — "([^"]+)" but SoT says [^=]+=(\d+)/g;
      for (const m of out.matchAll(lineRe)) {
        const [, file, snippet, expected] = m;
        if (!fs.existsSync(file)) continue;
        const fixedSnippet = snippet.replace(/\d[\d,]*/, expected);
        if (fixedSnippet === snippet) continue;
        const src = fs.readFileSync(file, "utf8");
        if (!src.includes(snippet)) continue;
        fs.writeFileSync(file, src.split(snippet).join(fixedSnippet));
        if (!touched.includes(file)) touched.push(file);
        fixes.push(`${file}: "${snippet}" → "${fixedSnippet}"`);
      }
      return {
        summary: fixes.length
          ? `Synced ${fixes.length} stale platform count(s) to the SoT:\n${fixes.join("\n")}`
          : "verify-counts reported mismatches but none were auto-fixable (snippet not found or non-numeric)",
        touchedFiles: touched,
      };
    },
    verify: "npx --yes tsx scripts/verify-counts.ts",
  },
  {
    // Task 101 (2026-07-31) — silent-failure baseline regression was the #2
    // recurring NEEDS-HUMAN class. We NEVER auto-raise the baseline (that
    // defeats the ratchet gate). Instead, surface the exact new finding(s) —
    // current findings diffed against the committed report shape — so the
    // human decision is one audited baseline edit, not an investigation.
    // Mirrors the stale-string precedent: the email digest IS the fix.
    id: "silent-failure-baseline-regression",
    description: "Silent-failure findings above baseline — emit local finding counts + per-file breakdown for human audit (NEVER auto-raise the baseline)",
    match: /Silent-failure findings regressed above baseline/,
    fix: async () => {
      let report = "";
      try {
        report = execSync("npx tsx scripts/silent-failure-hunter.ts 2>&1 || true", { encoding: "utf8", env: SAFE_ENV });
      } catch (e: any) { report = e?.stdout?.toString?.() ?? "(hunter re-run failed)"; }
      const counts = report.match(/Findings:[^\n]*/)?.[0] ?? "(counts unparsed)";
      const findings = report.split("\n").filter(l => /\[(HIGH|MEDIUM)\]/.test(l)).join("\n");
      let baseline = "";
      try { baseline = fs.readFileSync("data/silent-failure-baseline.json", "utf8"); } catch { baseline = "(baseline unreadable)"; }
      return {
        summary: `Silent-failure gate regressed — NOT auto-fixed (ratchet gate; baseline is never auto-raised). Local re-run: ${counts}\n\nBaseline: ${baseline.match(/"high": \d+,\s*"medium": \d+/)?.[0] ?? baseline.slice(-120)}\n\nAll current HIGH/MEDIUM findings (diff against last audited window to find the new one):\n${findings.slice(0, 6000)}\n\nEither fix the new silent catch, or audit it and raise data/silent-failure-baseline.json with a dated note.`,
        touchedFiles: [],
      };
    },
    verify: "true", // don't loop; the structured digest entry is the deliverable
  },
  {
...
async function ghLogs(jobId: number): Promise<string> {
  ...
  if (!r.ok) {
    ...
    throw new Error(`GitHub job logs ${jobId} → ${r.status}: ${body}`);
  }
  return r.text();
}
