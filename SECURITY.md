...
## Public pull-request intake

The public repository is a sanitized mirror, not the private development
repository. Code from outside contributors is untrusted until reviewed.

- The `External PR review gate` runs from the trusted base branch and never
  checks out or executes pull-request workflow code.
- Public PR CI has no repository secrets and checkout credentials are not
  persisted into the working tree.
- Adoption requires passing CI plus a maintainer approval on the exact current
  commit. A later push makes the approval stale.
- GitHub branch protection must require the review gate and CODEOWNER approval
  on `main`. The mirror-push automation must be the only explicitly authorized
  bypass for its sanitized snapshot updates.
- Reviewers should treat workflow, dependency, auth, safety, tool, schema,
  migration, script, and deployment changes as high-risk even when tests pass.
